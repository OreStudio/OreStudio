...
- =read_for_cache= (bool, Flags) — emit a =read_for_cache= handler method
  that swaps to the request's tenant and calls the entity's existing
  paginated =list_<entity_plural_short>= service method with an
  unbounded limit. Requires =has_tenant_id=.

  Requires a valid signed JWT (=make_request_context=), like =list=/
  =history=, but no specific permission check (=has_permission=) —
  the caller (e.g. a =nats-event-cache= consumer warming its own
  cache) legitimately reads across tenants under one service-account
  identity, so a per-tenant permission would not model the real
  access pattern. A caller wanting to invoke this subject must
  therefore hold a valid service-account JWT — see the
  =nats-event-cache.cache_header= archetype's optional
  =token_provider= constructor parameter, which attaches one to every
  =load()= call.
        ...
        if (auto req = decode<get_{{entity_plural}}_request>(msg)) {
            try {
                ...
                resp.{{entity_plural_short}} = svc.list_{{entity_plural_short}}(
                    req->offset, req->limit, req->{{list_filter_column}});
                resp.total_available_count = static_cast<int>(
                    svc.count_{{entity_plural_short}}(req->{{list_filter_column}}));
                resp.success = true;
...
{{/system_tenant_validation}}
        if (auto req = decode<save_{{entity_singular}}_request>(msg)) {
            try {
{{#has_batch_save}}
                svc.save_{{entity_plural_short}}(req->{{entity_plural_short}});
{{/has_batch_save}}
{{^has_batch_save}}
...
{{^read_only}}
    void save(ores::nats::message msg) {
        ...
        if (auto req = decode<save_{{name_singular}}_request>(msg)) {
            ...
            try {
                for (const auto& row : req->{{name}})
                    svc.save_{{name_singular_short}}(row);
                resp.success = true;
            } catch (const std::exception& e) {
                BOOST_LOG_SEV({{name_singular}}_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
