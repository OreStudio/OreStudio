...
void country_list_widget::setup_toolbar() {
    ...
    add_button_ = toolbar->addWidget(std::make_unique<Wt::WPushButton>("Add Country"));
...
void country_list_widget::setup_table() {
    ...
    table_->elementAt(0, 0)->addWidget(std::make_unique<Wt::WText>("Alpha-2"));
    table_->elementAt(0, 1)->addWidget(std::make_unique<Wt::WText>("Alpha-3"));
    table_->elementAt(0, 2)->addWidget(std::make_unique<Wt::WText>("Name"));
    table_->elementAt(0, 3)->addWidget(std::make_unique<Wt::WText>("Numeric"));
...
void country_list_widget::set_countries(const std::vector<country_row>& countries) {
...
void country_list_widget::populate_table() {
    ...
    for (std::size_t i = 0; i < countries_.size(); ++i) {
        ...
        table_->elementAt(row, 0)->addWidget(std::make_unique<Wt::WText>(c.alpha2_code));
        table_->elementAt(row, 1)->addWidget(std::make_unique<Wt::WText>(c.alpha3_code));
        table_->elementAt(row, 2)->addWidget(std::make_unique<Wt::WText>(c.name));
        ...
        edit_btn->clicked().connect([this, code = c.alpha2_code] { edit_requested_.emit(code); });
        ...
        delete_btn->clicked().connect(
            [this, code = c.alpha2_code] { delete_requested_.emit(code); });
    }
}
