 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_DAY_COUNT_FRACTION_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_DAY_COUNT_FRACTION_TYPE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes day count fraction types to data storage.
 */
class ORES_REFDATA_CORE_EXPORT day_count_fraction_type_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.day_count_fraction_type_repository";
...
public:
    ...
    /**
     * @brief Writes day count fraction types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::day_count_fraction_type& v);
    void write(context ctx, const std::vector<domain::day_count_fraction_type>& v);
    /**@}*/

    /**
     * @brief Writes a day count fraction type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of day count fraction types, each honouring its own
    ...
    /**
     * @brief Reads latest day count fraction types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::day_count_fraction_type> read_latest(context ctx);
     ...
     */
    std::vector<domain::day_count_fraction_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single day count fraction type as it stood at a specific
    ...
    /**
     * @brief Deletes a day count fraction type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a day count fraction type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes day count fraction types by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::day_count_fraction_type apply_claim(context ctx,
                                                const domain::day_count_fraction_type& v,
                                                const ores::utility::domain::precondition& claim);
};
