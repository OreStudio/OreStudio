...
void equity_accumulator_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_accumulator_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_equity_accumulator_instrument(std::ref(out), std::ref(session), args);
        },
        "Add Equity Accumulator instrument (trade_type_code "
        "party_id "
        "trade_id "
        "underlying_name "
        "currency "
    ...
    equity_accumulator_instruments_menu->Insert(
        ...
        "Delete Equity Accumulator instrument by instrument_id");
    ...
    equity_accumulator_instruments_menu->Insert(
        ...
        "Show Equity Accumulator instrument's version history");
...
void equity_accumulator_instrument_commands::process_get_equity_accumulator_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity Accumulator instrument request.";
...
void equity_accumulator_instrument_commands::process_add_equity_accumulator_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add Equity Accumulator instrument." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 16 + 2;
    ...
    try {
        ...
        v.fixing_frequency = ores::shell::app::from_token<std::string>(parsed->positionals[next++],
                                                                       "fixing_frequency");
        v.long_short =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "long_short");
        v.knock_out_level = ores::shell::app::from_token<std::optional<double>>(
            parsed->positionals[next++], "knock_out_level");
        v.target_amount = ores::shell::app::from_token<std::optional<double>>(
            parsed->positionals[next++], "target_amount");
        v.target_type =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "target_type");
        v.payoff_type =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "payoff_type");
        v.description =
...
void equity_accumulator_instrument_commands::process_delete_equity_accumulator_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete Equity Accumulator instrument." << std::endl;
        return;
    }
