...
const std::string product_version("ores.trading.service v" ORES_VERSION);
...
using ores::trading::service::config::options;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.trading.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Trade booking and lifecycle service" << std::endl << std::endl
         << "Usage: ores.trading.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("TRADING_SERVICE"));
