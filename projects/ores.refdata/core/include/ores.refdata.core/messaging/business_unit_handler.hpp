...
public:
        ...
        if (auto req = decode<save_business_unit_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(business_unit_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(
                    nats_, msg, save_business_unit_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(business_unit_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
    ...
    void history(ores::nats::message msg) {
        ...
        if (auto req = decode<get_business_unit_history_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      get_business_unit_history_response{.history = std::move(hist),
                                                         .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(business_unit_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
    ...
    void remove(ores::nats::message msg) {
        ...
        if (auto req = decode<delete_business_unit_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      delete_business_unit_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(business_unit_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
    ...
    void hierarchy(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::business_unit_service svc(req_ctx);
        if (auto req = decode<get_business_unit_hierarchy_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      get_business_unit_hierarchy_response{.success = true,
                ...
                reply(nats_,
                      msg,
                      get_business_unit_hierarchy_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(business_unit_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
