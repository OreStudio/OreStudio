...
begin
    ...
    -- 2. Clear existing data for this dataset (Idempotency)
    -- We assume full replacement for a dataset snapshot
    delete from ores.dq_images_artefact_tbl
    where dataset_id = v_dataset_id;

    -- 3. Insert data from assets_images_tbl
    -- We are snapshotting the CURRENT state of images
...
select ores.upsert_dq_images_artefact('Country Flags from lipis/flag-icons');

-- =============================================================================
-- Cleanup
-- =============================================================================

drop function ores.upsert_dq_images_artefact(text);

-- =============================================================================
-- Summary
-- =============================================================================

select 'dq_images_artefact' as entity, count(*) as count
