...
create table if not exists "ores_refdata_crm_topology_configs_tbl" (
    "id" uuid not null,
...
on "ores_refdata_crm_topology_configs_tbl" (tenant_id, party_id, name)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists crm_topology_configs_version_uniq_idx
on "ores_refdata_crm_topology_configs_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists crm_topology_configs_id_uniq_idx
on "ores_refdata_crm_topology_configs_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists crm_topology_configs_tenant_idx
on "ores_refdata_crm_topology_configs_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_crm_topology_configs_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_refdata_crm_topology_configs_tbl"
...
create or replace trigger ores_refdata_crm_topology_configs_insert_trg
before insert on "ores_refdata_crm_topology_configs_tbl"
for each row execute function ores_refdata_crm_topology_configs_insert_fn();

create or replace rule ores_refdata_crm_topology_configs_delete_rule as
on delete to "ores_refdata_crm_topology_configs_tbl" do instead (
    update "ores_refdata_crm_topology_configs_tbl"
    set valid_to = clock_timestamp()
...
-- =============================================================================
create or replace function ores_refdata_validate_crm_topology_config_fn(
    p_tenant_id uuid,
    p_value uuid
) returns uuid as $$
begin
