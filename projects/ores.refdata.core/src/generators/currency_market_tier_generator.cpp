 ...
 *
 */
#include "ores.refdata.core/generators/currency_market_tier_generator.hpp"
...
domain::currency_market_tier generate_synthetic_currency_market_tier(
    ...
    r.version = 1;
    r.code = codes[idx];
    r.name = std::string(faker::word::adjective()) + " Market Tier";
...
generate_synthetic_currency_market_tiers(std::size_t n,
    ...
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_currency_market_tier(ctx));
    return r;
}
