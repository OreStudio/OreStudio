...
PostgreSQL server. If you have not yet added a password to =postgres=, login as
...
Installs required PostgreSQL extensions (btree_gist, TimescaleDB).

*Prerequisites:* TimescaleDB must be installed on your system and added to
...
#+begin_src sh
psql -U postgres -f projects/sql/setup_extensions.sql
#+end_src

** Step 2: Create ORES User
