 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_LEG_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_LEG_TYPE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes leg types to data storage.
 */
class ORES_REFDATA_CORE_EXPORT leg_type_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.leg_type_repository";
...
public:
    ...
    /**
     * @brief Writes leg types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::leg_type& v);
    void write(context ctx, const std::vector<domain::leg_type>& v);
    /**@}*/

    /**
     * @brief Writes a leg type, honouring the claim it states.
     *
     ...
     */
    void
    write(context ctx, const domain::leg_type& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of leg types, each honouring its own
    ...
    std::vector<domain::leg_type> read_latest(context ctx, const std::vector<std::string>& codes);
    /**@}*/
    ...
    /**
     * @brief Reads a single leg type as it stood at a specific
    ...
    /**
     * @brief Deletes a leg type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a leg type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes leg types by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::leg_type& v);
     ...
     */
    domain::leg_type apply_claim(context ctx,
