 ...
 */
#ifndef ORES_COMPUTE_CORE_REPOSITORY_PLATFORM_REPOSITORY_HPP
#define ORES_COMPUTE_CORE_REPOSITORY_PLATFORM_REPOSITORY_HPP
...
/**
 * @brief Reads and writes compute platforms to data storage.
 */
...
public:
    ...
    /**
     * @brief Writes compute platforms to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::platform& v);
    void write(context ctx, const std::vector<domain::platform>& v);
    /**@}*/

    /**
     * @brief Writes a compute platform, honouring the claim it states.
     *
     ...
     */
    void
    write(context ctx, const domain::platform& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of compute platforms, each honouring its own
    ...
    /**
     * @brief Reads latest compute platforms, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::platform> read_latest(context ctx);
    std::vector<domain::platform> read_latest(context ctx, const std::string& id);
    std::vector<domain::platform> read_latest(context ctx, const std::vector<std::string>& ids);
    /**@}*/

    /**
     * @brief Reads latest compute platforms filtered by code.
     */
    std::vector<domain::platform> read_latest_by_code(context ctx, const std::string& code);

    /**
     * @brief Reads the newest compute platforms filtered by code, current or not.
     *
     ...
     */
    std::vector<domain::platform> read_any_by_code(context ctx, const std::string& code);


    /**
     * @brief Reads all compute platforms, possibly filtered by primary key.
     */
    std::vector<domain::platform> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single compute platform as it stood at a specific
    ...
    /**
     * @brief Reads latest compute platforms with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_platform_count(context ctx);

    /**
     * @brief Deletes a compute platform by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a compute platform, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes compute platforms by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::platform& v);
     ...
     */
    domain::platform apply_claim(context ctx,
