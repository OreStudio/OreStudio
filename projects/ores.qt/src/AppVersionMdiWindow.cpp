...
void AppVersionMdiWindow::setupUi() {
...
void AppVersionMdiWindow::setupToolbar() {
...
void AppVersionMdiWindow::setupTable() {
    model_ = new ClientAppVersionModel(clientManager_, this);
...
void AppVersionMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void AppVersionMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;

    auto sourceIndex = proxyModel_->mapToSource(index);
    if (auto* app_version = model_->getVersion(sourceIndex.row())) {
        emit showVersionDetails(*app_version);
    }
}

void AppVersionMdiWindow::updateActionStates() {
...
void AppVersionMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new app version requested";
    emit addNewRequested();
}

void AppVersionMdiWindow::editSelected() {
...
void AppVersionMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void AppVersionMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (ids.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid app versions to delete";
        return;
    }
