 ...
 */
std::vector<domain::monetary_nature> read_one(repository::monetary_nature_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::monetary_nature_key& key) {
 ...
 */
messaging::monetary_nature_key key_from(const domain::monetary_nature& v) {
    messaging::monetary_nature_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::monetary_nature to_domain(const messaging::monetary_nature_write& write) {
    domain::monetary_nature v;
    v.code = write.code;
    v.name = write.name;
...
messaging::list_monetary_natures_response monetary_nature_service::list_monetary_natures(
    ...
    messaging::list_monetary_natures_response response;
...
messaging::get_monetary_nature_response monetary_nature_service::get_monetary_nature(
    const messaging::get_monetary_nature_request& request) {
    messaging::get_monetary_nature_response response;
    ...
    response.monetary_nature = std::move(found.front());
    return response;
}

messaging::get_many_monetary_natures_response monetary_nature_service::get_many_monetary_natures(
    const messaging::get_many_monetary_natures_request& request) {
    messaging::get_many_monetary_natures_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::monetary_nature_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.monetary_nature = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_monetary_nature_response monetary_nature_service::put_monetary_nature(
    const messaging::put_monetary_nature_request& request) {
    messaging::put_monetary_nature_response response;
    domain::monetary_nature value;
    ...
    if (!written.empty())
        response.monetary_nature = std::move(written.front());
    return response;
}

messaging::put_many_monetary_natures_response monetary_nature_service::put_many_monetary_natures(
    const messaging::put_many_monetary_natures_request& request) {
    messaging::put_many_monetary_natures_response response;
    std::vector<domain::monetary_nature> batch;
...
messaging::delete_monetary_nature_response monetary_nature_service::delete_monetary_nature(
    const messaging::delete_monetary_nature_request& request) {
    messaging::delete_monetary_nature_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::monetary_nature_repository::remove_status::removed:
            break;
        case repository::monetary_nature_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::monetary_nature_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::monetary_nature_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
monetary_nature_service::delete_many_monetary_natures(
    const messaging::delete_many_monetary_natures_request& request) {
    messaging::delete_many_monetary_natures_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
monetary_nature_service::list_monetary_nature_versions(
    const messaging::list_monetary_nature_versions_request& request) {
    messaging::list_monetary_nature_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
monetary_nature_service::get_monetary_nature_version(
    const messaging::get_monetary_nature_version_request& request) {
    messaging::get_monetary_nature_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.monetary_nature.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
monetary_nature_service::prepare_change(const messaging::monetary_nature_change& change,
...
monetary_nature_service::get_type_at_version(const std::string& code, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Getting monetary nature at version. " << "code: " << code
                               << " version: " << version;
    return repo_.read_at_version(ctx_, code, version);
}

std::optional<domain::monetary_nature> monetary_nature_service::get_type(const std::string& code) {
    BOOST_LOG_SEV(lg(), debug) << "Getting monetary nature. " << "code: " << code;
...
monetary_nature_service::get_types(const std::vector<std::string>& codes) {
...
void monetary_nature_service::save_type(const domain::monetary_nature& v) {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Saving monetary nature. " << "code: " << v.code;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved monetary nature. " << "code: " << v.code;
}
...
void monetary_nature_service::delete_type(const std::string& code) {
    ...
    BOOST_LOG_SEV(lg(), info) << "Removed monetary nature. " << "code: " << code;
}
...
monetary_nature_service::get_type_history(const std::string& code) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for monetary nature. " << "code: " << code;
