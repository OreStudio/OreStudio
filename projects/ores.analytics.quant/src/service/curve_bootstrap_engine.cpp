...
double
curve_bootstrap_engine::interpolate_discount_factor(const std::vector<bootstrapped_point>& points,
...
curve_bootstrap_engine::bootstrap(std::chrono::year_month_day value_date,
    ...
    if (discount_curve) {
        ...
        for (const auto& p : pillars) {
            if (!covers(p.end_date) ||
                ...
                throw discount_curve_required_error(
                    "curve_bootstrap_engine: discount_curve does not cover pillar '" +
    ...
    for (const auto& p : pillars) {
        ...
        case bootstrap_curve_role_code::SWAP: {
            if (p.fixed_leg_dates.empty() ||
                std::chrono::sys_days(p.fixed_leg_dates.back()) != std::chrono::sys_days(p.end_date))
                throw std::invalid_argument(
