...
#include "ui_CompositeInstrumentHistoryDialog.h"
...
CompositeInstrumentHistoryDialog::CompositeInstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::CompositeInstrumentHistoryDialog),
      id_(id),
...
void CompositeInstrumentHistoryDialog::setupUi() {
...
void CompositeInstrumentHistoryDialog::setupToolbar() {
...
void CompositeInstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &CompositeInstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &CompositeInstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &CompositeInstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void CompositeInstrumentHistoryDialog::loadHistory() {
    ...
    QPointer<CompositeInstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::composite_instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_composite_instrument_history_request request;
        request.id = id;
...
void CompositeInstrumentHistoryDialog::updateVersionList() {
...
void CompositeInstrumentHistoryDialog::onVersionSelected() {
...
void CompositeInstrumentHistoryDialog::updateChangesTable(
...
void CompositeInstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
...
void CompositeInstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CompositeInstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CompositeInstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
    int row = selected.first()->row();
    if (static_cast<std::size_t>(row) >= versions_.size()) return;
