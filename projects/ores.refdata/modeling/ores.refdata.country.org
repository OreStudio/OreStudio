...
:flag_inline_widget:          codeEdit
:flag_accessor:               getCountryFlagIcon
...
:icon:                        Globe
:changed_event_class:         refdata::eventing::country_changed_event
:changed_event_include:       ores.refdata.api/eventing/country_changed_event.hpp
:END:
...
domain::country generate_country_sentinel(utility::generation::generation_context& ctx) {
    ...
    return domain::country{.alpha2_code = "ZZ", .alpha3_code = "ZZZ", .numeric_code = "999",
                           .name = "Supranational / Not Country-Specific",
                           .official_name = "Supranational / Not Country-Specific",
                           .modified_by = modified_by, .change_reason_code = "system.initial_load",
                           .change_commentary = "ISO 3166-1 user-assigned sentinel for "
                                                 "supranational calendars", .recorded_at = now};
}
#+end_src

** Custom repository methods

The following methods go beyond the standard repository template: temporal
reads at a supplied time point and an unfiltered =read_all=. Pagination,
the active-count aggregate, and batch delete are now generated by the
standard template and are no longer carried here.
