...
QTabWidget* CounterpartyDetailDialog::tabWidget() const {
    return ui_->tabWidget;
}

QWidget* CounterpartyDetailDialog::provenanceTab() const {
    return ui_->provenanceTab;
}

ProvenanceWidget* CounterpartyDetailDialog::provenanceWidget() const {
    return ui_->provenanceWidget;
}

QString CounterpartyDetailDialog::code() const {
    return QString::fromStdString(counterparty_.short_code);
}
...
void CounterpartyDetailDialog::populateBusinessCenterCodeCombo() {
    ...
    QObject::connect(
        watcher, &QFutureWatcher<std::vector<std::string>>::finished, self, [self, watcher]() {
            ...
            // populate-call time, since setCounterparty() may run before or
            // after setClientManager() triggers this fetch. Preferred over
            // previous: setCounterparty() always runs synchronously before
            ...
            // setCounterparty() call that happened first.
...
void CounterpartyDetailDialog::setCreateMode(bool createMode) {
    ...
    if (createMode) {
        counterparty_.id = boost::uuids::random_generator()();
    }
    hasChanges_ = false;
    updateSaveButtonState();
}

void CounterpartyDetailDialog::markDirty() {
    hasChanges_ = true;
    updateSaveButtonState();
}
...
void CounterpartyDetailDialog::updateUiFromCounterparty() {
    ...
    populateProvenance(counterparty_.version,
                       ...
                       counterparty_.performed_by,
                       counterparty_.recorded_at,
                       counterparty_.change_reason_code,
                       counterparty_.change_commentary);
...
void CounterpartyDetailDialog::onSaveClicked() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot save counterparty while disconnected from server.");
        return;
    }
...
void CounterpartyDetailDialog::onDeleteClicked() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete counterparty while disconnected from server.");
        return;
    }
    ...
    auto task = [self, id_str = boost::uuids::to_string(counterparty_.id)]() -> DeleteResult {
        if (!self || !self->clientManager_) {
            return {false, "Dialog closed"};
        }
