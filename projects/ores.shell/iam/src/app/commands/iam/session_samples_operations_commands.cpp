...
void session_samples_operations_commands::process_get_session_samples(
    ...
    using request_type = ores::iam::messaging::get_session_samples_request;
    ...
    std::optional<ores::iam::messaging::get_session_samples_response> result;
    if constexpr (request_type::requires_session) {
        result = do_auth_request<ores::iam::messaging::get_session_samples_response>(
            out, session, std::string(req.nats_subject), req);
    } else {
        result = do_request<ores::iam::messaging::get_session_samples_response>(
            out, session, std::string(req.nats_subject), req);
    }
    if (!result)
        return;
