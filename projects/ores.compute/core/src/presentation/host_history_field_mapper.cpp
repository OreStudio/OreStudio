...
std::vector<ores::diff::domain::field_value> render_host_fields(const domain::host& v) {
    using ores::diff::domain::field_value;
    std::vector<field_value> fields;

    fields.push_back({.name = "ID", .value = boost::uuids::to_string(v.id)});
    fields.push_back({.name = "External ID", .value = v.external_id});
    fields.push_back({.name = "Location", .value = v.location});
