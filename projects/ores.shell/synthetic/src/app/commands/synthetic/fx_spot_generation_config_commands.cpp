...
void fx_spot_generation_config_commands::register_commands(cli::Menu& root_menu,
                                                           nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("fx_spot_generation_configs");
...
void fx_spot_generation_config_commands::process_list(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_fx_spot_generation_configs_request;
    ...
    auto result = do_auth_request<messaging::list_fx_spot_generation_configs_response>(
...
void fx_spot_generation_config_commands::process_get(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_fx_spot_generation_config_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_fx_spot_generation_config_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_get_many(std::ostream& out,
                                                          nats_client& session,
                                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_fx_spot_generation_configs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_fx_spot_generation_configs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_add(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_fx_spot_generation_config_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.quote_currency_code,
                   parsed->positionals[next++],
                   "quote_currency_code");
        read_token(req.change.write.source_name, parsed->positionals[next++], "source_name");
        read_token(req.change.write.ore_key, parsed->positionals[next++], "ore_key");
        read_token(req.change.write.price_source, parsed->positionals[next++], "price_source");
        read_token(
            req.change.write.gmm_initial_price, parsed->positionals[next++], "gmm_initial_price");
        read_token(req.change.write.ticks_per_hour, parsed->positionals[next++], "ticks_per_hour");
        read_token(req.change.write.process_type, parsed->positionals[next++], "process_type");
        read_token(req.change.write.enabled, parsed->positionals[next++], "enabled");
        read_token(req.change.write.auto_start, parsed->positionals[next++], "auto_start");
        ...
        read_token(req.change.write.folder_id, parsed->positionals[next++], "folder_id");
    ...
    auto result = do_auth_request<messaging::put_fx_spot_generation_config_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_set(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
...
void fx_spot_generation_config_commands::process_put_many(std::ostream& out,
                                                          nats_client& session,
                                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_fx_spot_generation_configs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            ...
            read_token(change.write.quote_currency_code,
                       parsed->positionals[next++],
                       "quote_currency_code");
            read_token(change.write.source_name, parsed->positionals[next++], "source_name");
            read_token(change.write.ore_key, parsed->positionals[next++], "ore_key");
            read_token(change.write.price_source, parsed->positionals[next++], "price_source");
            read_token(
                change.write.gmm_initial_price, parsed->positionals[next++], "gmm_initial_price");
            read_token(change.write.ticks_per_hour, parsed->positionals[next++], "ticks_per_hour");
            read_token(change.write.process_type, parsed->positionals[next++], "process_type");
            read_token(change.write.enabled, parsed->positionals[next++], "enabled");
            read_token(change.write.auto_start, parsed->positionals[next++], "auto_start");
            ...
            read_token(change.write.folder_id, parsed->positionals[next++], "folder_id");
    ...
    auto result = do_auth_request<messaging::put_many_fx_spot_generation_configs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_delete(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_fx_spot_generation_config_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_fx_spot_generation_config_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_delete_many(std::ostream& out,
                                                             nats_client& session,
                                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_fx_spot_generation_configs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(
                messaging::fx_spot_generation_config_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_fx_spot_generation_configs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_versions(std::ostream& out,
                                                          nats_client& session,
                                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_fx_spot_generation_config_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_fx_spot_generation_config_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_version(std::ostream& out,
                                                         nats_client& session,
                                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_fx_spot_generation_config_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_fx_spot_generation_config_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
