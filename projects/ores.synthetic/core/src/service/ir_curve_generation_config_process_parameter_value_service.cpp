...
read_one(repository::ir_curve_generation_config_process_parameter_value_repository& repo,
         const ores::database::context& ctx,
         const messaging::ir_curve_generation_config_process_parameter_value_key& key) {
 ...
 */
messaging::ir_curve_generation_config_process_parameter_value_key
key_from(const domain::ir_curve_generation_config_process_parameter_value& v) {
    messaging::ir_curve_generation_config_process_parameter_value_key key;
    key.id = v.id;
    return key;
}
...
to_domain(const messaging::ir_curve_generation_config_process_parameter_value_write& write) {
    domain::ir_curve_generation_config_process_parameter_value v;
    v.id = write.id;
    v.config_id = write.config_id;
    v.parameter_definition_id = write.parameter_definition_id;
...
ir_curve_generation_config_process_parameter_value_service::
    list_ir_curve_generation_config_process_parameter_values(
        const messaging::list_ir_curve_generation_config_process_parameter_values_request&
            request) {
    messaging::list_ir_curve_generation_config_process_parameter_values_response response;
    ...
    response.process_parameter_values = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_process_parameter_value_count(ctx_);
    return response;
}
...
ir_curve_generation_config_process_parameter_value_service::
    ...
    messaging::get_ir_curve_generation_config_process_parameter_value_response response;
    ...
    response.ir_curve_generation_config_process_parameter_value = std::move(found.front());
    return response;
}
...
ir_curve_generation_config_process_parameter_value_service::
    get_many_ir_curve_generation_config_process_parameter_values(
        const messaging::get_many_ir_curve_generation_config_process_parameter_values_request&
            request) {
    messaging::get_many_ir_curve_generation_config_process_parameter_values_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::ir_curve_generation_config_process_parameter_value_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.ir_curve_generation_config_process_parameter_value = std::move(found.front());
...
ir_curve_generation_config_process_parameter_value_service::
    ...
    messaging::put_ir_curve_generation_config_process_parameter_value_response response;
    domain::ir_curve_generation_config_process_parameter_value value;
    ...
    if (!written.empty())
        response.ir_curve_generation_config_process_parameter_value = std::move(written.front());
    return response;
}
...
ir_curve_generation_config_process_parameter_value_service::
    put_many_ir_curve_generation_config_process_parameter_values(
        const messaging::put_many_ir_curve_generation_config_process_parameter_values_request&
            request) {
    messaging::put_many_ir_curve_generation_config_process_parameter_values_response response;
    ...
    response.process_parameter_values.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.process_parameter_values.push_back(written.empty() ? value :
...
ir_curve_generation_config_process_parameter_value_service::
    delete_ir_curve_generation_config_process_parameter_value(
        const messaging::delete_ir_curve_generation_config_process_parameter_value_request&
            request) {
    messaging::delete_ir_curve_generation_config_process_parameter_value_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::ir_curve_generation_config_process_parameter_value_repository::
        ...
        case repository::ir_curve_generation_config_process_parameter_value_repository::
            ...
            break;
        case repository::ir_curve_generation_config_process_parameter_value_repository::
            remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::ir_curve_generation_config_process_parameter_value_repository::
            remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
ir_curve_generation_config_process_parameter_value_service::
    delete_many_ir_curve_generation_config_process_parameter_values(
        const messaging::delete_many_ir_curve_generation_config_process_parameter_values_request&
            request) {
    messaging::delete_many_ir_curve_generation_config_process_parameter_values_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
ir_curve_generation_config_process_parameter_value_service::
    list_ir_curve_generation_config_process_parameter_value_versions(
        const messaging::list_ir_curve_generation_config_process_parameter_value_versions_request&
            request) {
    messaging::list_ir_curve_generation_config_process_parameter_value_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
ir_curve_generation_config_process_parameter_value_service::
    get_ir_curve_generation_config_process_parameter_value_version(
        const messaging::get_ir_curve_generation_config_process_parameter_value_version_request&
            request) {
    messaging::get_ir_curve_generation_config_process_parameter_value_version_response response;
    auto found = repo_.read_at_version(
        ctx_,
        boost::uuids::to_string(request.key.ir_curve_generation_config_process_parameter_value.id),
        request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ir_curve_generation_config_process_parameter_value_service::prepare_change(
...
std::vector<domain::ir_curve_generation_config_process_parameter_value>
ir_curve_generation_config_process_parameter_value_service::get_process_parameter_values(
