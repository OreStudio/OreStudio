...
// notification" pattern end to end for crm_topology_config -- the
...
// CRM Topology Config writes are party-scoped: the session-level
...
TEST_CASE("write_crm_topology_config_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::crm_topology_config_changed_event>(
        [&nats](const ores::refdata::eventing::crm_topology_config_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::crm_topology_config_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.crm_topology_config",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.crm_topology_config_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::crm_topology_config_changed_event>(
        "ores.refdata.crm_topology_config", "ores_refdata_crm_topology_configs");
    ...
    auto v = generate_synthetic_crm_topology_config(ctx);
    v.change_reason_code = "system.test";
    v.party_id = *party_ctx.party_id();
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "CRM Topology Config: " << v;

    crm_topology_config_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.crm_topology_config") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for crm_topology_config " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for crm_topology_config " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::crm_topology_config_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_crm_topology_config_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_crm_topology_config(id_str);
        ...
        REQUIRE_FALSE(svc.get_crm_topology_config(id_str).has_value());
        REQUIRE(svc.get_crm_topology_config_history(id_str).size() == versions.size());
    }
}
