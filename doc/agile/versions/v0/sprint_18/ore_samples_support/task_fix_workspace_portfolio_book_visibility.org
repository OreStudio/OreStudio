...
| [[https://github.com/OreStudio/OreStudio/pull/865][#865]]  | Workspace list and detail dialog improvements                                                    |
...
| # | Comment summary                                                    | File                                    | Decision | Notes                                                    |
|---+--------------------------------------------------------------------+-----------------------------------------+----------+----------------------------------------------------------|
| 1 | =loadParentWorkspaces()= declared but not implemented (false positive) | WorkspaceDetailDialog.cpp           | Declined | Method fully implemented; reviewer missed the definition |
| 2 | QPointer data race in =loadParentWorkspaces= background lambda     | WorkspaceDetailDialog.cpp               | Fixed    | 3ab608a72 — capture raw =cm= before lambda              |
| 3 | QPointer data race in =deleteSelected= background lambda           | WorkspaceMdiWindow.cpp                  | Fixed    | 3ab608a72 — capture raw =cm= and =username= before lambda |
| 4 | Migration must UPDATE =valid_to= before INSERT                     | workspace_live_fix_modified_by.sql      | Declined | INSERT trigger closes previous version automatically     |
| 5 | Migration must UPDATE =valid_to= before INSERT                     | workspace_live_fix_owner_id.sql         | Declined | Same trigger mechanism; explicit UPDATE would fight it   |
