...
#include "ores.trading.core/messaging/bond_issue_conversion_target_handler.hpp"
#include <memory>
...
std::vector<ores::nats::service::subscription> register_bond_issue_conversion_target_handlers(
    ...
    auto h = std::make_shared<bond_issue_conversion_target_handler>(
        nats, std::move(ctx), std::move(verifier));
    subs.push_back(nats.queue_subscribe(get_bond_issue_conversion_targets_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->list(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(save_bond_issue_conversion_target_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->save(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(delete_bond_issue_conversion_target_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->remove(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(get_bond_issue_conversion_target_history_request::nats_subject,
                             queue_group,
