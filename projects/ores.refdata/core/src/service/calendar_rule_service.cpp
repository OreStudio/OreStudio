...
std::vector<domain::calendar_rule> read_one(repository::calendar_rule_repository& repo,
                                            ...
                                            const messaging::calendar_rule_key& key) {
 ...
 */
messaging::calendar_rule_key key_from(const domain::calendar_rule& v) {
 ...
 */
domain::calendar_rule to_domain(const messaging::calendar_rule_write& write) {
    domain::calendar_rule v;
    v.id = write.id;
    v.calendar_code = write.calendar_code;
    v.kind = write.kind;
    v.month = write.month;
    v.day = write.day;
    v.weekday = write.weekday;
    v.occurrence = write.occurrence;
    v.day_offset = write.day_offset;
    v.shift = write.shift;
    v.effective_from = write.effective_from;
    v.effective_to = write.effective_to;
    return v;
}
...
messaging::list_by_calendar_code_calendar_rules_response
...
calendar_rule_service::get_calendar_rule(const messaging::get_calendar_rule_request& request) {
    messaging::get_calendar_rule_response response;
    ...
    response.calendar_rule = std::move(found.front());
    return response;
}

messaging::get_many_calendar_rules_response calendar_rule_service::get_many_calendar_rules(
    const messaging::get_many_calendar_rules_request& request) {
    messaging::get_many_calendar_rules_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::calendar_rule_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.calendar_rule = std::move(found.front());
...
calendar_rule_service::put_calendar_rule(const messaging::put_calendar_rule_request& request) {
    messaging::put_calendar_rule_response response;
    domain::calendar_rule value;
    ...
    if (!written.empty())
        response.calendar_rule = std::move(written.front());
    return response;
}

messaging::put_many_calendar_rules_response calendar_rule_service::put_many_calendar_rules(
    const messaging::put_many_calendar_rules_request& request) {
    messaging::put_many_calendar_rules_response response;
    std::vector<domain::calendar_rule> batch;
    ...
    response.calendar_rules.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.calendar_rules.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_calendar_rule_response calendar_rule_service::delete_calendar_rule(
    const messaging::delete_calendar_rule_request& request) {
    messaging::delete_calendar_rule_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::calendar_rule_repository::remove_status::removed:
            break;
        case repository::calendar_rule_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::calendar_rule_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::calendar_rule_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_calendar_rules_response calendar_rule_service::delete_many_calendar_rules(
    const messaging::delete_many_calendar_rules_request& request) {
    messaging::delete_many_calendar_rules_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_calendar_rule_versions_response calendar_rule_service::list_calendar_rule_versions(
    const messaging::list_calendar_rule_versions_request& request) {
    messaging::list_calendar_rule_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_calendar_rule_version_response calendar_rule_service::get_calendar_rule_version(
    const messaging::get_calendar_rule_version_request& request) {
    messaging::get_calendar_rule_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.calendar_rule.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
calendar_rule_service::prepare_change(const messaging::calendar_rule_change& change,
...
std::vector<domain::calendar_rule>
calendar_rule_service::get_calendar_rules(const std::vector<std::string>& ids) {
