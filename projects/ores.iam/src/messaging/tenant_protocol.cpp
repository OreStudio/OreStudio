...
save_tenant_request
save_tenant_request::from(domain::tenant tenant) {
    return save_tenant_request{std::vector<domain::tenant>{std::move(tenant)}};
}

save_tenant_request
save_tenant_request::from(std::vector<domain::tenant> tenants) {
    return save_tenant_request{std::move(tenants)};
}
...
save_tenant_request::deserialize(std::span<const std::byte> data) {
    ...
    request.tenants.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_tenant(data);
        if (!e) return std::unexpected(e.error());
        request.tenants.push_back(std::move(*e));
    }
    return request;
}
