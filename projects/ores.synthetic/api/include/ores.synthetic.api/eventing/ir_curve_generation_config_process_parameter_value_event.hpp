 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_IR_CURVE_GENERATION_CONFIG_PROCESS_PARAMETER_VALUE_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_IR_CURVE_GENERATION_CONFIG_PROCESS_PARAMETER_VALUE_EVENT_HPP
...
/**
 * @brief Event traits for ir curve generation config process parameter value's changes.
 *
...
struct entity_event_traits<
    ...
    static constexpr std::string_view subject_prefix =
        "synthetic.v1.ir_curve_generation_config_process_parameter_values_events";

    static ores::synthetic::messaging::ir_curve_generation_config_process_parameter_value_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::ir_curve_generation_config_process_parameter_value_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<
                ores::synthetic::messaging::ir_curve_generation_config_process_parameter_value_key>(
                notification.key))
            event.key = *key;
