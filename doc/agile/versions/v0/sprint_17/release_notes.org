...
fragile to solid. The sprint opened with 14 hotfix-inherited C1202 workarounds
and closed with all of them reversed or superseded by the root fix (struct
decomposition). Symbol visibility migration landed globally in a single sprint —
=-fvisibility=hidden= across all shared libraries, plugin isolation phase 1, and
static CLI/shell libs. Service architecture hardening completed the final gap:
SECURITY DEFINER validators, IAM cross-service decoupling via NATS, and zero
hand-crafted SQL remaining where a codegen model exists. Feature delivery was
equally broad: Workspace (bitemporal), DQ publish (end-to-end workflow), ORE
conventions import (seven types, CLI Phase 3), and ORE roundtrip. The sprint
also closed out the documentation debt — all 16 prior sprints are now in the v2
