...
void tenor_schedule_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("tenor_schedules");
    ...
    menu->Insert(
        ...
        "add <code> <name> <description> <display_order> <schedule_source> <calendar_code> "
        "<diary_entry_type> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <code> <name> <description> <display_order> <schedule_source> <calendar_code> "
        "<diary_entry_type> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <code> <name> <description> <display_order> <schedule_source> "
        "<calendar_code> <diary_entry_type> <reason> <commentary>");
    ...
    menu->Insert(
        "by-calendar-code",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_calendar_code(std::ref(out), std::ref(session), std::move(args));
        },
        "by-calendar-code <calendar_code> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");

    menu->Insert(
        "by-diary-entry-type",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_diary_entry_type(std::ref(out), std::ref(session), std::move(args));
        },
        "by-diary-entry-type <diary_entry_type> [--offset <n>] [--limit <n>] [--order <field>] "
        "[--desc]");
...
void tenor_schedule_commands::process_list(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_tenor_schedules_request;
    ...
    auto result = do_auth_request<messaging::list_tenor_schedules_response>(
...
void tenor_schedule_commands::process_get(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_tenor_schedule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_tenor_schedule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_get_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_tenor_schedules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::tenor_schedule_key key;
    ...
    auto result = do_auth_request<messaging::get_many_tenor_schedules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_add(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_tenor_schedule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.change.write.schedule_source, parsed->positionals[next++], "schedule_source");
        read_token(req.change.write.calendar_code, parsed->positionals[next++], "calendar_code");
        read_token(
            req.change.write.diary_entry_type, parsed->positionals[next++], "diary_entry_type");
    ...
    auto result = do_auth_request<messaging::put_tenor_schedule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_set(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
...
void tenor_schedule_commands::process_put_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_tenor_schedules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::tenor_schedule_change change;
            ...
            read_token(
                change.write.schedule_source, parsed->positionals[next++], "schedule_source");
            read_token(change.write.calendar_code, parsed->positionals[next++], "calendar_code");
            read_token(
                change.write.diary_entry_type, parsed->positionals[next++], "diary_entry_type");
    ...
    auto result = do_auth_request<messaging::put_many_tenor_schedules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_delete(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_tenor_schedule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_tenor_schedule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_delete_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_tenor_schedules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::tenor_schedule_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_tenor_schedules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_by_calendar_code(std::ostream& out,
                                                       nats_client& session,
                                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-calendar-code request.";

    using request_type = messaging::list_by_calendar_code_tenor_schedules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-calendar-code." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.calendar_code = ores::shell::app::from_token<std::optional<std::string>>(
            parsed->positionals[next++], "calendar_code");
    ...
    auto result = do_auth_request<messaging::list_by_calendar_code_tenor_schedules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_by_diary_entry_type(std::ostream& out,
                                                          nats_client& session,
                                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-diary-entry-type request.";

    using request_type = messaging::list_by_diary_entry_type_tenor_schedules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-diary-entry-type." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.diary_entry_type = ores::shell::app::from_token<std::optional<std::string>>(
            parsed->positionals[next++], "diary_entry_type");
        if (const auto& raw = parsed->flag("scope"); !raw.empty()) {
            req.scope = raw == "subtree" ? ores::utility::domain::scope::subtree :
                                           ores::utility::domain::scope::direct;
        }
    ...
    auto result = do_auth_request<messaging::list_by_diary_entry_type_tenor_schedules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_versions(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_tenor_schedule_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_tenor_schedule_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_schedule_commands::process_version(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_tenor_schedule_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.tenor_schedule.code, parsed->positionals[next++], "code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_tenor_schedule_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
