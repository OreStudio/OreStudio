 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_CURRENCY_MARKET_TIER_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_CURRENCY_MARKET_TIER_EVENT_HPP
...
/**
 * @brief Event traits for currency market tier's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::currency_market_tier_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.currency_market_tiers_events";

    static ores::refdata::messaging::currency_market_tier_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::currency_market_tier_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::refdata::messaging::currency_market_tier_key>(
                notification.key))
            event.key = *key;
