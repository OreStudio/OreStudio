...
// notification" pattern end to end for yield_curve_process_type -- the
...
using ores::synthetic::domain::yield_curve_process_type;
using ores::synthetic::repository::yield_curve_process_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_yield_curve_process_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::synthetic::eventing::yield_curve_process_type_changed_event>(
        [&nats](const ores::synthetic::eventing::yield_curve_process_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::synthetic::eventing::yield_curve_process_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.synthetic.yield_curve_process_type",
    ...
    event_source
        .register_mapping<ores::synthetic::eventing::yield_curve_process_type_changed_event>(
            "ores.synthetic.yield_curve_process_type", "ores_synthetic_yield_curve_process_types");
    ...
    auto v = generate_synthetic_yield_curve_process_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Yield Curve Process Type: " << v;

    yield_curve_process_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.synthetic.yield_curve_process_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for yield_curve_process_type " << id_str
