...
create or replace function ores_iam_login_info_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    changed_key := jsonb_build_object('account_id', changed_account_id);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.iam.login_info',
    ...
    perform pg_notify('ores_iam_login_info', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_iam_login_info_notify_trg
after insert or update or delete on ores_iam_login_info_tbl
for each row execute function ores_iam_login_info_notify_fn();
