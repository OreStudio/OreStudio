...
domain::equity_instrument
equity_instrument_mapper::map(const equity_instrument_entity& v) {
    BOOST_LOG_SEV(lg(), trace) << "Mapping db entity: " << v;

    domain::equity_instrument r;
    ...
    r.quantity = v.quantity.value_or(0.0);
    ...
    r.lower_barrier = v.lower_barrier.value_or(0.0);
    r.upper_barrier = v.upper_barrier.value_or(0.0);
    r.average_type = v.average_type.value_or("");
    r.averaging_start_date = v.averaging_start_date.value_or("");
    r.variance_strike = v.variance_strike.value_or(0.0);
    r.cliquet_frequency_code = v.cliquet_frequency_code.value_or("");
    r.accumulation_amount = v.accumulation_amount.value_or(0.0);
    r.knock_out_barrier = v.knock_out_barrier.value_or(0.0);
    ...
    r.return_type = v.return_type.value_or("");
...
equity_instrument_mapper::map(const domain::equity_instrument& v) {
    BOOST_LOG_SEV(lg(), trace) << "Mapping domain entity: " << v;

    equity_instrument_entity r;
    ...
    r.currency = v.currency;
    r.notional = v.notional;
    r.quantity = v.quantity == 0.0 ? std::nullopt : std::optional(v.quantity);
    ...
    r.lower_barrier = v.lower_barrier == 0.0 ? std::nullopt : std::optional(v.lower_barrier);
    r.upper_barrier = v.upper_barrier == 0.0 ? std::nullopt : std::optional(v.upper_barrier);
    ...
    r.variance_strike = v.variance_strike == 0.0 ? std::nullopt : std::optional(v.variance_strike);
    r.cliquet_frequency_code = v.cliquet_frequency_code.empty() ? std::nullopt : std::optional(v.cliquet_frequency_code);
    r.accumulation_amount = v.accumulation_amount == 0.0 ? std::nullopt : std::optional(v.accumulation_amount);
    r.knock_out_barrier = v.knock_out_barrier == 0.0 ? std::nullopt : std::optional(v.knock_out_barrier);
    ...
    r.return_type = v.return_type.empty() ? std::nullopt : std::optional(v.return_type);
...
equity_instrument_mapper::map(const std::vector<equity_instrument_entity>& v) {
    return map_vector<equity_instrument_entity, domain::equity_instrument>(
        v,
...
std::vector<equity_instrument_entity>
equity_instrument_mapper::map(const std::vector<domain::equity_instrument>& v) {
    return map_vector<domain::equity_instrument, equity_instrument_entity>(
        v,
        [](const auto& ve) { return map(ve); },
        lg(),
        "domain entities");
