...
| V05  | Pass   | Every gate green, with the pre-existing repo-wide CMake staleness recorded: `check_component_drift.py --all` no drift, model drift clean, protocol twin coverage intact, handler permissions seeded, populate references resolve, recipe inventory checked, and this component's source lists up to date. `regenerate_cmake_component_files.py --all --check` reports 116 stale lists elsewhere, none of them ores.nats. `validate_docs.sh` passes all 40 components. The physical-space inventory check reports one stale table on main, `ores.doc.knowledge.org`, which this branch does not touch. Evidence in `evidence/v05_gates.txt`. |
...
that is not green repo-wide is the CMake source-list check, which reports 116
