...
options_description make_options_description() {
    ...
    return standard_service_options::make_options_description("ores.trading.service.log", storage);
}

void print_help(const options_description& od, std::ostream& info) {
    info << "Trade booking and lifecycle service" << std::endl
         << std::endl
         << "Usage: ores.trading.service [options]" << std::endl
         << std::endl
         << od << std::endl;
}
...
std::optional<options> parse_arguments(const std::vector<std::string>& arguments,
    ...
    const auto vm(standard_service_options::parse(od, arguments, "TRADING_SERVICE"));
