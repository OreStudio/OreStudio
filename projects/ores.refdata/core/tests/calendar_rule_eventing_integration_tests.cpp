...
// notification" pattern end to end for calendar_rule -- the
...
TEST_CASE("write_calendar_rule_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::calendar_rule_changed_event>(
        [&nats](const ores::refdata::eventing::calendar_rule_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::calendar_rule_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.calendar_rule",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.calendar_rule_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::calendar_rule_changed_event>(
        "ores.refdata.calendar_rule", "ores_refdata_calendar_rules");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::calendar_rule_changed_event>::name),
        10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.calendar_rule") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for calendar_rule " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for calendar_rule " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::calendar_rule_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_calendar_rule_history(id_str);
