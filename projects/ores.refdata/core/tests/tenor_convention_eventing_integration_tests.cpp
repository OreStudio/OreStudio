...
#include "ores.refdata.api/generators/tenor_convention_generator.hpp"
...
// notification" pattern end to end for tenor_convention -- the
...
TEST_CASE("write_tenor_convention_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::tenor_convention_changed_event>(
        [&nats](const ores::refdata::eventing::tenor_convention_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::tenor_convention_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.tenor_convention",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::tenor_convention_changed_event>(
        "ores.refdata.tenor_convention", "ores_refdata_tenor_conventions");
    ...
    auto v = generate_synthetic_tenor_convention(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Tenor Convention: " << v;

    tenor_convention_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.tenor_convention") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenor_convention " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenor_convention " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::tenor_convention_service svc(crud_ctx);
