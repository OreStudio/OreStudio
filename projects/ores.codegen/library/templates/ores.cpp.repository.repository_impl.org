...
:END:
#+title: ores.cpp.repository.repository_impl
#+description: CRUD + temporal queries over sqlgen; custom-method paste points. =repository= profile.
#+type: archetype
#+output: projects/{component_core_dir}/src/repository/{entity}_repository.cpp
...
{{#has_workspace_id}}
    ...
    if (!chain.empty()) {
        const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
            where({{{tenant_where}}} && "workspace_id"_c.in(chain) && "valid_to"_c == max.value()) |
    ...
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
        order_by({{{primary_key.order_by_cols}}});
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "valid_to"_c == max.value()) |
        order_by({{{primary_key.order_by_cols}}});
{{/has_workspace_id}}
{{/read_tenant_filtered}}
{{^read_tenant_filtered}}
...
{{#has_workspace_id}}
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid && {{{primary_key.where_and}}} && "valid_to"_c == max.value());
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && {{{primary_key.where_and}}} && "valid_to"_c == max.value());
{{/has_workspace_id}}
{{/read_tenant_filtered}}
{{^read_tenant_filtered}}
{{#has_workspace_id}}
...
{{#has_workspace_id}}
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid && {{#parent_column}}"{{parent_column}}"_c == {{parent_column}} && {{/parent_column}}"{{column}}"_c == {{column}} && "valid_to"_c == max.value());
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && {{#parent_column}}"{{parent_column}}"_c == {{parent_column}} && {{/parent_column}}"{{column}}"_c == {{column}} && "valid_to"_c == max.value());
{{/has_workspace_id}}
{{/read_tenant_filtered}}
{{^read_tenant_filtered}}
{{#has_workspace_id}}
...
{{#has_workspace_id}}
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid && "{{primary_key.column}}"_c == {{primary_key.column}} &&
              "valid_from"_c <= ts.value() && "valid_to"_c > ts.value());
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "{{primary_key.column}}"_c == {{primary_key.column}} &&
              "valid_from"_c <= ts.value() && "valid_to"_c > ts.value());
{{/has_workspace_id}}
{{/read_tenant_filtered}}
{{^read_tenant_filtered}}
...
{{#has_workspace_id}}
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid && "{{column}}"_c == {{column}} && "valid_to"_c == max.value()) |
        order_by("{{list_by_order_column}}"_c{{#list_by_order_desc}}.desc(){{/list_by_order_desc}}) | sqlgen::offset(offset) | sqlgen::limit(limit);
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "{{column}}"_c == {{column}} && "valid_to"_c == max.value()) |
        order_by("{{list_by_order_column}}"_c{{#list_by_order_desc}}.desc(){{/list_by_order_desc}}) | sqlgen::offset(offset) | sqlgen::limit(limit);
{{/has_workspace_id}}
{{/read_tenant_filtered}}
{{^read_tenant_filtered}}
...
{{#left.replace_by}}
void {{name_singular}}_repository::replace_by_{{left.column_short}}(
{{#left.is_uuid}}
    const boost::uuids::uuid& {{left.column}},
{{/left.is_uuid}}
{{^left.is_uuid}}
...
{{/has_tenant_id}}
    ...
    // {{name_short}} are re-inserted below; the insert trigger takes care
    // of the bitemporal bookkeeping.
    execute_parameterized_command(
        ctx_,
        "UPDATE {{product}}_{{component}}_{{name}}_tbl"
        "   SET valid_to = current_timestamp"
{{#has_tenant_id}}
        " WHERE tenant_id = $1::uuid"
        "   AND {{left.column}} = $2{{#left.is_uuid}}::uuid{{/left.is_uuid}}"
{{/has_tenant_id}}
{{^has_tenant_id}}
        " WHERE {{left.column}} = $1{{#left.is_uuid}}::uuid{{/left.is_uuid}}"
{{/has_tenant_id}}
        "   AND valid_to = ores_utility_infinity_timestamp_fn()",
{{#has_tenant_id}}
        {tid, {{#left.is_uuid}}{{left.column}}_str{{/left.is_uuid}}{{^left.is_uuid}}{{left.column}}{{/left.is_uuid}}},
{{/has_tenant_id}}
{{^has_tenant_id}}
        {{{#left.is_uuid}}{{left.column}}_str{{/left.is_uuid}}{{^left.is_uuid}}{{left.column}}{{/left.is_uuid}}},
{{/has_tenant_id}}
        lg(),
        "Closing existing {{name_words}} for {{left.column_title_lower}} " +
        {{#left.is_uuid}}{{left.column}}_str{{/left.is_uuid}}{{^left.is_uuid}}{{left.column}}{{/left.is_uuid}});

    for (auto {{name_singular_short}} : {{name_short}}) {
{{#has_tenant_id}}
        {{name_singular_short}}.tenant_id = tid;
{{/has_tenant_id}}
        {{name_singular_short}}.modified_by = modified_by;
        {{name_singular_short}}.performed_by = performed_by;
        {{name_singular_short}}.change_reason_code = change_reason_code;
        {{name_singular_short}}.change_commentary = change_commentary;
        write({{name_singular_short}});
    }
}
{{/left.replace_by}}
{{#right.replace_by}}
void {{name_singular}}_repository::replace_by_{{right.column_short}}(
{{#right.is_uuid}}
    const boost::uuids::uuid& {{right.column}},
{{/right.is_uuid}}
{{^right.is_uuid}}
...
{{#has_tenant_id}}
        " WHERE tenant_id = $1::uuid"
        "   AND {{right.column}} = $2{{#right.is_uuid}}::uuid{{/right.is_uuid}}"
{{/has_tenant_id}}
{{^has_tenant_id}}
        " WHERE {{right.column}} = $1{{#right.is_uuid}}::uuid{{/right.is_uuid}}"
{{/has_tenant_id}}
        "   AND valid_to = ores_utility_infinity_timestamp_fn()",
{{#has_tenant_id}}
        {tid, {{#right.is_uuid}}{{right.column}}_str{{/right.is_uuid}}{{^right.is_uuid}}{{right.column}}{{/right.is_uuid}}},
{{/has_tenant_id}}
{{^has_tenant_id}}
        {{{#right.is_uuid}}{{right.column}}_str{{/right.is_uuid}}{{^right.is_uuid}}{{right.column}}{{/right.is_uuid}}},
{{/has_tenant_id}}
        lg(),
        "Closing existing {{name_words}} for {{right.column_title_lower}} " +
        {{#right.is_uuid}}{{right.column}}_str{{/right.is_uuid}}{{^right.is_uuid}}{{right.column}}{{/right.is_uuid}});
