 ...
 */
#ifndef ORES_QT_TENOR_CONTROLLER_HPP
#define ORES_QT_TENOR_CONTROLLER_HPP
 ...
 *
 * Manages the lifecycle of tenor list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class TenorController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.tenor_controller";
...
private slots:
    void onShowDetails(const refdata::domain::tenor& tenor);
    void onAddNewRequested();
    void onShowHistory(const refdata::domain::tenor& tenor);
    void onRevertVersion(const refdata::domain::tenor& tenor);
    void onOpenVersion(const refdata::domain::tenor& tenor, int versionNumber);
...
private:
     ...
     * TenorDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(TenorDetailDialog* detailDialog);
    void showHistoryWindow(const QString& code);

    /**
     * @brief Fetches the full typed tenor history (the
     * existing per-entity refdata::messaging::get_tenor_history_request/
     * refdata::messaging::get_tenor_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed tenor, since the
     * generic dialog holds no typed domain data.
     */
    void fetchTenorHistory(
