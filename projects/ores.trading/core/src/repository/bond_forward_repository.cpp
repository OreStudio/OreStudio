...
std::string bond_forward_repository::sql() {
...
std::optional<domain::bond_forward> bond_forward_repository::read_at_version(
    ...
    const auto entities = execute_read_query<bond_forward_entity, domain::bond_forward>(
        ...
        [](const auto& entities) { return bond_forward_mapper::map(entities); },
        lg(),
        "Reading bond forward at version.");
...
std::vector<domain::bond_forward>
bond_forward_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
...
std::uint32_t bond_forward_repository::get_total_bond_forward_count(context ctx) {
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::select_from<bond_forward_entity>(sqlgen::count().as<"count">()) |
                       where("tenant_id"_c == tid && "valid_to"_c == max.value()) |
