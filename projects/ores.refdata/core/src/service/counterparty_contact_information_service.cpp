...
read_one(repository::counterparty_contact_information_repository& repo,
         const ores::database::context& ctx,
         const messaging::counterparty_contact_information_key& key) {
 ...
 */
messaging::counterparty_contact_information_key
key_from(const domain::counterparty_contact_information& v) {
...
to_domain(const messaging::counterparty_contact_information_write& write) {
    domain::counterparty_contact_information v;
    v.id = write.id;
...
messaging::get_counterparty_contact_information_response
counterparty_contact_information_service::get_counterparty_contact_information(
    ...
    messaging::get_counterparty_contact_information_response response;
    ...
    response.counterparty_contact_information = std::move(found.front());
    return response;
}

messaging::get_many_counterparty_contact_informations_response
counterparty_contact_information_service::get_many_counterparty_contact_informations(
    const messaging::get_many_counterparty_contact_informations_request& request) {
    messaging::get_many_counterparty_contact_informations_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::counterparty_contact_information_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.counterparty_contact_information = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_counterparty_contact_information_response
counterparty_contact_information_service::put_counterparty_contact_information(
    const messaging::put_counterparty_contact_information_request& request) {
    messaging::put_counterparty_contact_information_response response;
    domain::counterparty_contact_information value;
    ...
    if (!written.empty())
        response.counterparty_contact_information = std::move(written.front());
    return response;
}

messaging::put_many_counterparty_contact_informations_response
counterparty_contact_information_service::put_many_counterparty_contact_informations(
    const messaging::put_many_counterparty_contact_informations_request& request) {
    messaging::put_many_counterparty_contact_informations_response response;
    std::vector<domain::counterparty_contact_information> batch;
    ...
    response.counterparty_contact_informations.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.counterparty_contact_informations.push_back(
...
counterparty_contact_information_service::delete_counterparty_contact_information(
    const messaging::delete_counterparty_contact_information_request& request) {
    messaging::delete_counterparty_contact_information_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::counterparty_contact_information_repository::remove_status::removed:
            break;
        case repository::counterparty_contact_information_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::counterparty_contact_information_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::counterparty_contact_information_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
counterparty_contact_information_service::delete_many_counterparty_contact_informations(
    const messaging::delete_many_counterparty_contact_informations_request& request) {
    messaging::delete_many_counterparty_contact_informations_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
counterparty_contact_information_service::list_counterparty_contact_information_versions(
    const messaging::list_counterparty_contact_information_versions_request& request) {
    messaging::list_counterparty_contact_information_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
counterparty_contact_information_service::get_counterparty_contact_information_version(
    const messaging::get_counterparty_contact_information_version_request& request) {
    messaging::get_counterparty_contact_information_version_response response;
    auto found = repo_.read_at_version(
        ctx_,
        boost::uuids::to_string(request.key.counterparty_contact_information.id),
        request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result counterparty_contact_information_service::prepare_change(
    const messaging::counterparty_contact_information_change& change,
...
std::uint32_t counterparty_contact_information_service::
    count_counterparty_contact_informations_by_counterparty_id(
...
counterparty_contact_information_service::get_counterparty_contact_information_at_version(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Getting counterparty contact information at version. "
...
counterparty_contact_information_service::get_counterparty_contact_information(
    const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting counterparty contact information. " << "id: " << id;
...
counterparty_contact_information_service::find_counterparty_contact_information(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Finding counterparty contact information. " << "id: " << id;
...
counterparty_contact_information_service::find_counterparty_contact_information_by_code(
    const boost::uuids::uuid& counterparty_id, const std::string& contact_type) {
    BOOST_LOG_SEV(lg(), debug)
        << "Finding counterparty contact information by counterparty_id/contact_type: "
        << counterparty_id << "/" << contact_type;
    auto results =
        repo_.read_latest_by_code(ctx_, boost::uuids::to_string(counterparty_id), contact_type);
    if (results.empty())
        return std::nullopt;
    return results.front();
}
...
std::vector<domain::counterparty_contact_information>
counterparty_contact_information_service::get_counterparty_contact_informations(
...
void counterparty_contact_information_service::save_counterparty_contact_information(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Saving counterparty contact information. " << "id: " << v.id;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved counterparty contact information. " << "id: " << v.id;
}
...
void counterparty_contact_information_service::delete_counterparty_contact_information(
    ...
    BOOST_LOG_SEV(lg(), info) << "Removed counterparty contact information. " << "id: " << id;
}
...
counterparty_contact_information_service::get_counterparty_contact_information_history(
    const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for counterparty contact information. "
                               << "id: " << id;
