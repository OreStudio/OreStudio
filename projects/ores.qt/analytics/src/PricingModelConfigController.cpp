...
void PricingModelConfigController::showListWindow() {
    ...
    // Connect signals
    connect(listWindow_,
            &PricingModelConfigMdiWindow::statusChanged,
            this,
            &PricingModelConfigController::statusMessage);
    connect(listWindow_,
            &PricingModelConfigMdiWindow::errorOccurred,
            this,
            &PricingModelConfigController::errorMessage);
    connect(listWindow_,
            &PricingModelConfigMdiWindow::showConfigDetails,
            this,
            &PricingModelConfigController::onShowDetails);
    connect(listWindow_,
            &PricingModelConfigMdiWindow::addNewRequested,
            this,
            &PricingModelConfigController::onAddNewRequested);
    connect(listWindow_,
            &PricingModelConfigMdiWindow::showConfigHistory,
            this,
            &PricingModelConfigController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<PricingModelConfigController>(this), key]() {
                if (!self)
                    return;
...
void PricingModelConfigController::showAddWindow() {
    ...
    connect(detailDialog,
            ...
            this,
            [self = QPointer<PricingModelConfigController>(this)](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info)
                    << "Pricing Model Configuration saved: " << code.toStdString();
...
void PricingModelConfigController::showDetailWindow(
    ...
    connect(detailDialog,
            &PricingModelConfigDetailDialog::configDeleted,
            this,
            [self = QPointer<PricingModelConfigController>(this), key](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info)
                    << "Pricing Model Configuration deleted: " << code.toStdString();
...
void PricingModelConfigController::onOpenVersion(
    ...
    detailWindow->setWindowTitle(
        QString("Pricing Model Configuration: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void PricingModelConfigController::onRevertVersion(
    ...
    connect(detailDialog,
            ...
            [self = QPointer<PricingModelConfigController>(this)](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info)
                    << "Pricing Model Configuration reverted: " << code.toStdString();
                emit self->statusMessage(
                    QString("Pricing Model Configuration '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert Pricing Model Configuration: %1").arg(QString::fromStdString(config.name)));
