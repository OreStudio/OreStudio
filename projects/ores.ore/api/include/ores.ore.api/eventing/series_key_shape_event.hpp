 ...
 */
#ifndef ORES_ORE_API_EVENTING_SERIES_KEY_SHAPE_EVENT_HPP
#define ORES_ORE_API_EVENTING_SERIES_KEY_SHAPE_EVENT_HPP
...
/**
 * @brief Event traits for series key shape's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::ore::messaging::series_key_shape_event> {
    static constexpr std::string_view subject_prefix = "ore.v1.series_key_shapes_events";

    static ores::ore::messaging::series_key_shape_event
    from_notification(const entity_event_notification& notification) {
        ores::ore::messaging::series_key_shape_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::ore::messaging::series_key_shape_key>(notification.key))
            event.key = *key;
