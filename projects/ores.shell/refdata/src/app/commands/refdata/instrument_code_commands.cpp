...
void instrument_code_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("instrument_codes");
    ...
    menu->Insert(
        ...
        "add <code> <name> <description> <asset_class> <ore_trade_type> <display_order> "
        "<curve_role> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <code> <name> <description> <asset_class> <ore_trade_type> <display_order> "
        "<curve_role> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <code> <name> <description> <asset_class> <ore_trade_type> "
        "<display_order> <curve_role> <reason> <commentary>");
...
void instrument_code_commands::process_list(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_instrument_codes_request;
    ...
    auto result = do_auth_request<messaging::list_instrument_codes_response>(
...
void instrument_code_commands::process_get(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_instrument_code_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_instrument_code_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_get_many(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_instrument_codes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_instrument_codes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_add(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_instrument_code_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.ore_trade_type, parsed->positionals[next++], "ore_trade_type");
        read_token(req.change.write.display_order, parsed->positionals[next++], "display_order");
        read_token(req.change.write.curve_role, parsed->positionals[next++], "curve_role");
    ...
    auto result = do_auth_request<messaging::put_instrument_code_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_set(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
...
void instrument_code_commands::process_put_many(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_instrument_codes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::instrument_code_change change;
            ...
            read_token(change.write.ore_trade_type, parsed->positionals[next++], "ore_trade_type");
            read_token(change.write.display_order, parsed->positionals[next++], "display_order");
            read_token(change.write.curve_role, parsed->positionals[next++], "curve_role");
    ...
    auto result = do_auth_request<messaging::put_many_instrument_codes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_delete(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_instrument_code_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_instrument_code_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_delete_many(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_instrument_codes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::instrument_code_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_instrument_codes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_versions(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_instrument_code_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_instrument_code_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void instrument_code_commands::process_version(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_instrument_code_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.instrument_code.code, parsed->positionals[next++], "code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_instrument_code_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
