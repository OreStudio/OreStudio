...
---
name: compass-devops-setup-environment
...
2. /Sync the vcpkg submodule/ to what main's tree expects — a stale
   vcpkg pin causes hard-to-diagnose build failures later:

   #+begin_src sh :results verbatim
   git submodule update --init vcpkg
   #+end_src

3. /Sync the doc indexes/ (=compass-doc-sync-index=):

   #+begin_src sh :results verbatim
   ./projects/ores.compass/compass.sh index --org-roam-db-sync
   #+end_src

4. /Deploy the skills bundle/ (=compass-devops-deploy-skills=):

   #+begin_src sh :results verbatim
   ./projects/ores.compass/compass.sh build --direct skills
   #+end_src

5. /Deploy Claude Code settings/ (=compass-devops-deploy-settings=):

   #+begin_src sh :results verbatim
   ./projects/ores.compass/compass.sh build --direct settings
   #+end_src

6. /Initialise the environment/ — .env, certificates, IAM key. The
   =-y= flag answers the "reuse passwords?" prompt; without it, a
   non-interactive shell hits EOF and the command fails:

   #+begin_src sh :results verbatim
   ./projects/ores.compass/compass.sh env configure -y
   #+end_src

7. /Recreate the database/ (=compass-devops-recreate-db=) — stop services
...
8. /Build the project/ — required before services can start or the
   shell can provision anything; use =compass build=, never raw
   =cmake= invocations:
...
9. /Provision the Acme Corporation tenant via the shell/ — run the
   full Acme holding-group provisioning script from the generated
   library (the system's reference fully-populated test/demo entity):
...
10. /Verify/ — bearings clean. Do not start services as part of this
...
- [[id:DEAC11AE-3753-49FA-994A-98594162A605][How do I index notes for compass?]] — step 3 in detail.
- [[id:B5F231A9-5403-482C-8F2A-12D6917C58CD][How do I deploy the skills?]] — step 4 in detail.
