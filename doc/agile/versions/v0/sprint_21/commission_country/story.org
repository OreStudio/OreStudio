...
| [[id:CB3FEC64-DA43-46D7-A8CC-5CE43335B45F][Switch codegen model type detection to frontmatter]] | BACKLOG | | | Change get_model_type() to read #+type: frontmatter rather than filename suffix. Keep filename-suffix fallback temporarily. Unblocks the unified model migration (step 1 of 7, D2256981). |
| [[id:37AFB6CD-D316-44B5-92FA-2B20049729A3][Unify SQL template to handle full validation function and insert trigger]] | BACKLOG | | | Extend sql_schema_domain_entity_create.mustache (or merge both SQL templates) so the domain_entity data shape carries the full validation-function body and insert-trigger validations. Retires the template-level distinction between entity and table pathways (step 2 of 7, D2256981). |
| [[id:ABE8A5B8-6258-4215-8D46-8DD3A2A5E1AD][Merge table and entity org parsers]] | BACKLOG | | | Teach load_org_model() to parse the * Validation function and * Insert trigger sections and populate has_tenant_id, coding_scheme, and image_id. Reuse logic from load_org_table_model() then delete the table loader (step 3 of 7, D2256981). |
| [[id:FF67F12F-6815-4F97-9DBB-FDD6699D5473][Add sql_only and has_qt variability guards to codegen]] | BACKLOG | | | Introduce #+sql_only: true and #+has_qt: false flags. Make C++ and Qt field derivation conditional so SQL-only entities (currency, purpose_type) and C++-without-Qt entities are expressible (step 4 of 7, D2256981). |
