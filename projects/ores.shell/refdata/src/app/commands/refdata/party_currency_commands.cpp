...
void party_currency_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("party_currencies");
    ...
    menu->Insert(
        ...
        "get <party_id> <currency_iso_code>");
    ...
    menu->Insert(
        ...
        "get-many <party_id> <currency_iso_code>");
    ...
    menu->Insert(
        ...
        "add <party_id> <currency_iso_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <party_id> <currency_iso_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <party_id> <currency_iso_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <party_id> <currency_iso_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <party_id> <currency_iso_code> <reason> <commentary>");

    menu->Insert(
        "by-party-id",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_party_id(std::ref(out), std::ref(session), std::move(args));
        },
...
void party_currency_commands::process_list(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_party_currencies_request;
    ...
    auto result = do_auth_request<messaging::list_party_currencies_response>(
...
void party_currency_commands::process_get(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_party_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.party_id, parsed->positionals[next++], "party_id");
    ...
    auto result = do_auth_request<messaging::get_party_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_get_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_party_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            messaging::party_currency_key key;
            read_token(key.party_id, parsed->positionals[i + 0], "party_id");
            read_token(key.currency_iso_code, parsed->positionals[i + 1], "currency_iso_code");
    ...
    auto result = do_auth_request<messaging::get_many_party_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_add(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_party_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_party_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_set(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
...
void party_currency_commands::process_put_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_party_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::party_currency_change change;
    ...
    auto result = do_auth_request<messaging::put_many_party_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_delete(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_party_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.party_id, parsed->positionals[next++], "party_id");
        read_token(
    ...
    auto result = do_auth_request<messaging::delete_party_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_delete_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_party_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::party_currency_key key;
            read_token(key.party_id, parsed->positionals[i * 2 + 0], "party_id");
            read_token(key.currency_iso_code, parsed->positionals[i * 2 + 1], "currency_iso_code");
            req.removals.push_back(messaging::party_currency_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_party_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_currency_commands::process_by_party_id(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-party-id request.";

    using request_type = messaging::list_by_party_id_party_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-party-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.party_id = ores::shell::app::from_token<boost::uuids::uuid>(parsed->positionals[next++],
                                                                        "party_id");
    ...
    auto result = do_auth_request<messaging::list_by_party_id_party_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
