...
void bond_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    bond_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_bond_instrument(std::ref(out), std::ref(session), args);
        },
        "Add bond instrument (trade_type_code "
        "party_id "
        "trade_id "
        "issue_id "
        "change_reason_code "
    ...
    bond_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_bond_instrument(std::ref(out), std::ref(session), std::move(id));
        },
        "Delete bond instrument by instrument_id");

    bond_instruments_menu->Insert(
        "history",
        [&session](std::ostream& out, std::string id) {
            process_get_bond_instrument_history(std::ref(out), std::ref(session), std::move(id));
        },
        "Show bond instrument's version history");
...
void bond_instrument_commands::process_get_bond_instruments(std::ostream& out,
                                                            nats_client& session,
                                                            pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get bond instrument request.";
...
void bond_instrument_commands::process_add_bond_instrument(std::ostream& out,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add bond instrument." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 4 + 2;
    ...
    try {
        ...
        v.issue_id = ores::shell::app::from_token<boost::uuids::uuid>(parsed->positionals[next++],
                                                                      "issue_id");
    ...
    auto req = trading::messaging::save_bond_instrument_request::from(std::move(v));
...
void bond_instrument_commands::process_delete_bond_instrument(std::ostream& out,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete bond instrument." << std::endl;
        return;
    }
