...
- Deleting whole workflows is sequenced with the local-review task:
  =claude-code-review.yml= (auto review on PR events) and =misspell.yml=
  retire when the local code-review skill lands, since they are the
  review coverage that skill replaces; the on-demand =claude.yml=
  trigger stays. The gate-reduction task deletes only the retired jobs
  from =pr.yml=.
