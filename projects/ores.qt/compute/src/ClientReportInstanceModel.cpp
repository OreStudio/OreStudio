...
int ClientReportInstanceModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientReportInstanceModel::data(
    ...
    const auto& instance = instances_[row];
    ...
    if (role == Qt::ForegroundRole) {
        return recency_foreground_color(instance.name);
    }
...
void ClientReportInstanceModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh report instance model: disconnected.";
...
void ClientReportInstanceModel::onInstancesLoaded() {
    is_fetching_ = false;
    ...
    const int new_count = static_cast<int>(result.instances.size());
    ...
    if (new_count > 0) {
        ...
        const bool has_recent = recencyTracker_.update(instances_);
        if (has_recent && !pulseManager_->is_pulsing()) {
            pulseManager_->start_pulsing();
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " report instances newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " report instances."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientReportInstanceModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientReportInstanceModel::onPulsingComplete() {
