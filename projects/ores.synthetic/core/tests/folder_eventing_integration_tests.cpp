...
#include "ores.synthetic.api/eventing/folder_changed_event.hpp"
...
// notification" pattern end to end for folder -- the
...
// Folder writes are party-scoped: the session-level
...
using ores::synthetic::domain::folder;
using ores::synthetic::repository::folder_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_folder_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::synthetic::eventing::folder_changed_event>(
        [&nats](const ores::synthetic::eventing::folder_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::synthetic::eventing::folder_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.synthetic.folder",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.folder_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::synthetic::eventing::folder_changed_event>(
        "ores.synthetic.folder", "ores_synthetic_folders");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::synthetic::eventing::folder_changed_event>::name),
        10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.synthetic.folder") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for folder " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for folder " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_folder(id_str).has_value());
        REQUIRE(svc.get_folder_history(id_str).size() == versions.size());
    }
}
