 ...
 */
#ifndef ORES_QT_CURRENCY_PAIR_CLASSIFICATION_CONTROLLER_HPP
#define ORES_QT_CURRENCY_PAIR_CLASSIFICATION_CONTROLLER_HPP
...
class CurrencyPairClassificationMdiWindow;
class CurrencyPairClassificationDetailDialog;
class DetachableMdiSubWindow;
class ChangeReasonCache;

/**
 * @brief Controller for managing currency pair classification windows and operations.
 *
 * Manages the lifecycle of currency pair classification list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class CurrencyPairClassificationController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.currency_pair_classification_controller";
...
public:
    CurrencyPairClassificationController(QMainWindow* mainWindow,
                                         QMdiArea* mdiArea,
...
private slots:
    ...
    void onShowHistory(const refdata::domain::currency_pair_classification& classification);
...
private:
     ...
     * CurrencyPairClassificationDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(CurrencyPairClassificationDetailDialog* detailDialog);
