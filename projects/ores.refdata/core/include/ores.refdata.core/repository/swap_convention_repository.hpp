 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_SWAP_CONVENTION_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_SWAP_CONVENTION_REPOSITORY_HPP
 ...
 */
class ORES_REFDATA_CORE_EXPORT swap_convention_repository {
private:
...
public:
    ...
    /**
     * @brief Writes swap conventions to database.
     *
    ...
    /**
     * @brief Writes a swap convention, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of swap conventions, each honouring its own
    ...
    /**
     * @brief Reads latest swap conventions, possibly filtered by primary key.
     */
    /**@{*/
     ...
     */
    std::vector<domain::swap_convention> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single swap convention as it stood at a specific
    ...
    /**
     * @brief Reads latest swap conventions with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_swap_convention_count(context ctx);

    /**
     * @brief Deletes a swap convention by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a swap convention, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes swap conventions by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::swap_convention apply_claim(context ctx,
                                        const domain::swap_convention& v,
                                        const ores::utility::domain::precondition& claim);
};
