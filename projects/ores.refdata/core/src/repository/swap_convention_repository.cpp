...
swap_convention_repository::replace_claim(context ctx, const domain::swap_convention& v) {
...
swap_convention_repository::apply_claim(context ctx,
                                        const domain::swap_convention& v,
...
void swap_convention_repository::write(context ctx,
                                       const domain::swap_convention& v,
                                       const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing swap convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, swap_convention_mapper::map(t), lg(), "Writing swap convention to database.");
}

void swap_convention_repository::write(
    context ctx,
    const std::vector<domain::swap_convention>& v,
    ...
    execute_write_query(
        ctx, swap_convention_mapper::map(batch), lg(), "Writing swap conventions to database.");
}
...
std::vector<domain::swap_convention> swap_convention_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<swap_convention_entity, domain::swap_convention>(
            ...
            lg(),
            "Reading latest swap conventions (workspace resolution chain).");
    }
...
std::optional<domain::swap_convention> swap_convention_repository::read_at_version(
    ...
    const auto entities = execute_read_query<swap_convention_entity, domain::swap_convention>(
        ...
        [](const auto& entities) { return swap_convention_mapper::map(entities); },
        lg(),
        "Reading swap convention at version.");
...
std::vector<domain::swap_convention>
swap_convention_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest swap conventions with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    ...
    return execute_read_query<swap_convention_entity, domain::swap_convention>(
        ...
        [](const auto& entities) { return swap_convention_mapper::map(entities); },
        lg(),
        "Reading latest swap conventions with pagination.");
}

std::uint32_t swap_convention_repository::get_total_swap_convention_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active swap convention count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<swap_convention_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active swap convention count: " << count;
    return count;
}

std::vector<domain::swap_convention>
swap_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<swap_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<swap_convention_entity, domain::swap_convention>(
        ctx,
        query,
        [](const auto& entities) { return swap_convention_mapper::map(entities); },
        lg(),
        "Reading latest swap conventions by ids.");
    return result;
}

void swap_convention_repository::remove(context ctx, const std::vector<std::string>& ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<swap_convention_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing swap conventions.");
}
