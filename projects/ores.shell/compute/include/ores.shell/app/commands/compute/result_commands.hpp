 ...
 */
#ifndef ORES_SHELL_APP_COMMANDS_RESULT_COMMANDS_HPP
#define ORES_SHELL_APP_COMMANDS_RESULT_COMMANDS_HPP
...
/**
 * @brief Every verb compute results answer, as one command each.
 *
 ...
 */
class result_commands {
private:
    inline static std::string_view logger_name = "ores.shell.app.commands.compute.result_commands";
...
public:
    /**
     * @brief Register the compute results submenu.
     */
    ...
    /**
     * @brief by-workunit-id <workunit_id> [--offset <n>] [--limit <n>] [--order <field>] [--desc]
     */
    static void process_by_workunit_id(std::ostream& out,
                                       ores::nats::service::nats_client& session,
                                       const std::vector<std::string>& args);
