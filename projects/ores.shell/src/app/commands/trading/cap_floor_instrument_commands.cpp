...
#include "ores.trading.api/domain/cap_floor_instrument_table_io.hpp" // IWYU pragma: keep.
...
void cap_floor_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    cap_floor_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_cap_floor_instrument(std::ref(out),
                                             std::ref(session),
    ...
    root_menu.Insert(std::move(cap_floor_instruments_menu));
}

void cap_floor_instrument_commands::process_get_cap_floor_instruments(
    std::ostream& out, nats_client& session, pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Cap floor instruments request.";

    auto& state = pagination.state_for("cap_floor_instruments");

    trading::messaging::get_cap_floor_instruments_request req;
    req.offset = state.current_offset;
    req.limit = pagination.page_size();

    auto result = do_auth_request<trading::messaging::get_cap_floor_instruments_response>(
        out, session, "trading.v1.cap_floor_instruments.list", req);
    if (!result)
        return;
...
void cap_floor_instrument_commands::process_add_cap_floor_instrument(
    ...
    std::string change_reason_code,
    std::string change_commentary) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add Cap floor instrument request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an Cap floor instrument." << std::endl;
        return;
    }

    domain::cap_floor_instrument v;
    ...
    auto req = trading::messaging::save_cap_floor_instrument_request{.data = std::move(v)};

    auto result = do_auth_request<trading::messaging::save_cap_floor_instrument_response>(
        out, session, "trading.v1.cap_floor_instruments.save", req);
    if (!result)
        return;
    ...
    if (result->success) {
        ...
        fail(out) << "Failed to add Cap floor instrument: " << msg << std::endl;
    }
}
...
void cap_floor_instrument_commands::process_delete_cap_floor_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an Cap floor instrument." << std::endl;
        return;
    }

    trading::messaging::delete_cap_floor_instrument_request req;
    req.ids = {std::move(instrument_id)};

    auto result = do_auth_request<trading::messaging::delete_cap_floor_instrument_response>(
        out, session, "trading.v1.cap_floor_instruments.delete", req);
    if (!result)
        return;
...
void cap_floor_instrument_commands::process_get_cap_floor_instrument_history(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Cap floor instrument history for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get Cap floor instrument history." << std::endl;
        return;
    }
    ...
    if (!result->success) {
        BOOST_LOG_SEV(lg(), warn) << "Failed to get Cap floor instrument history: "
                                  << result->message;
    ...
    if (result->history.empty()) {
        out << "No history found for this Cap floor instrument." << std::endl;
        return;
    }
