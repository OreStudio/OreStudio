...
#+type: archetype
#+output: projects/ores.qt/{qt_component}/src/Client{EntityPascal}Model.cpp
#+facet: ores.cpp.qt
#+level: cross
...
{{/domain_entity.qt.has_icon_columns}}
{{#domain_entity.qt.has_date_columns}}
#include "ores.platform/time/datetime.hpp"
{{/domain_entity.qt.has_date_columns}}
{{#domain_entity.qt.columns}}
{{#formatter_include}}
#include "{{formatter_include}}"
{{/formatter_include}}
{{/domain_entity.qt.columns}}
...
namespace {
    std::string {{domain_entity.entity_snake}}_key_extractor(const {{domain_entity.qt.domain_class}}& e) {
        return {{domain_entity.qt.key_to_string_prefix}}e.{{#domain_entity.qt.key_field_access}}{{domain_entity.qt.key_field_access}}{{/domain_entity.qt.key_field_access}}{{^domain_entity.qt.key_field_access}}{{domain_entity.qt.key_field}}{{/domain_entity.qt.key_field_access}}{{domain_entity.qt.key_to_string_suffix}};
    }
}
...
{{^data_paste}}
        case {{enum_name}}:
{{#formatter}}
            return {{formatter}}({{domain_entity.qt.item_var}}.{{#field_access}}{{field_access}}{{/field_access}}{{^field_access}}{{field}}{{/field_access}});
{{/formatter}}
{{^formatter}}
{{#is_string}}
...
{{/is_string}}
{{#is_date}}
            return QString::fromStdString(ores::platform::time::datetime::to_iso8601_date(
                {{domain_entity.qt.item_var}}.{{#field_access}}{{field_access}}{{/field_access}}{{^field_access}}{{field}}{{/field_access}}));
{{/is_date}}
{{#is_optional_string}}
...
{{#is_optional_double}}
            return {{domain_entity.qt.item_var}}.{{#field_access}}{{field_access}}{{/field_access}}{{^field_access}}{{field}}{{/field_access}}
                ? QVariant(*{{domain_entity.qt.item_var}}.{{#field_access}}{{field_access}}{{/field_access}}{{^field_access}}{{field}}{{/field_access}})
                : QVariant();
{{/is_optional_double}}
{{#is_timestamp}}
...
{{#domain_entity.qt.columns}}
{{#tooltip_field}}
    if (role == Qt::ToolTipRole && index.column() == {{enum_name}}) {
        return QString::fromStdString({{domain_entity.qt.item_var}}.{{tooltip_field}});
    }
{{/tooltip_field}}
{{/domain_entity.qt.columns}}
...
std::vector<{{domain_entity.qt.domain_class}}> Client{{domain_entity.entity_pascal}}Model::get{{domain_entity.entity_pascal_short_plural}}() const {
    return {{domain_entity.qt.collection_name}}_;
}
{{/domain_entity.qt.has_csv_xml_io}}
