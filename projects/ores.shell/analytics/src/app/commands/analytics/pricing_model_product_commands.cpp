...
void pricing_model_product_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("pricing_model_products");
    ...
    menu->Insert(
        ...
        "get-many <pricing_engine_type_code>");
    ...
    menu->Insert(
        ...
        "delete <pricing_engine_type_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <pricing_engine_type_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <pricing_engine_type_code> [--offset <n>] [--limit <n>] [--order <field>] "
        "[--desc]");
    ...
    menu->Insert(
        ...
        "version <pricing_engine_type_code> --version <n>");

    root_menu.Insert(std::move(menu));
}

void pricing_model_product_commands::process_list(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_pricing_model_products_request;
    ...
    auto result = do_auth_request<messaging::list_pricing_model_products_response>(
...
void pricing_model_product_commands::process_get(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_pricing_model_product_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.pricing_engine_type_code,
    ...
    auto result = do_auth_request<messaging::get_pricing_model_product_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_get_many(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_pricing_model_products_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            ...
            read_token(key.pricing_engine_type_code,
                       parsed->positionals[i + 0],
                       "pricing_engine_type_code");
    ...
    auto result = do_auth_request<messaging::get_many_pricing_model_products_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_add(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_pricing_model_product_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.pricing_engine_type_code,
                   parsed->positionals[next++],
                   "pricing_engine_type_code");
    ...
    auto result = do_auth_request<messaging::put_pricing_model_product_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_set(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
...
void pricing_model_product_commands::process_put_many(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_pricing_model_products_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_many_pricing_model_products_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_delete(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_pricing_model_product_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.pricing_engine_type_code,
                   parsed->positionals[next++],
                   "pricing_engine_type_code");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
    ...
    auto result = do_auth_request<messaging::delete_pricing_model_product_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_delete_many(std::ostream& out,
                                                         nats_client& session,
                                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_pricing_model_products_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::pricing_model_product_key key;
            read_token(key.pricing_engine_type_code,
                       parsed->positionals[i * 1 + 0],
                       "pricing_engine_type_code");
            req.removals.push_back(messaging::pricing_model_product_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_pricing_model_products_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_versions(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_pricing_model_product_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_pricing_model_product_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_commands::process_version(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_pricing_model_product_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.pricing_model_product.pricing_engine_type_code,
                   parsed->positionals[next++],
                   "pricing_engine_type_code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_pricing_model_product_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
