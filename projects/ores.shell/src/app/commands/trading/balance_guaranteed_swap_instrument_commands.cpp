...
void balance_guaranteed_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    balance_guaranteed_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_balance_guaranteed_swap_instrument(std::ref(out),
                                                           std::ref(session),
                                                           std::move(trade_type_code),
                                                           std::move(start_date),
                                                           std::move(maturity_date),
                                                           std::move(lockout_days),
                                                           std::move(description),
                                                           std::move(change_reason_code),
                                                           std::move(change_commentary));
        },
        "Add an Balance guaranteed swap instrument (trade_type_code start_date maturity_date "
        "[lockout_days] [description] change_reason_code \"change_commentary\")",
        {"trade_type_code start_date maturity_date lockout_days description change_reason_code "
         "change_commentary"});
...
void balance_guaranteed_swap_instrument_commands::process_get_balance_guaranteed_swap_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Balance guaranteed swap instruments request.";

    auto& state = pagination.state_for("balance_guaranteed_swap_instruments");

    trading::messaging::get_balance_guaranteed_swap_instruments_request req;
...
void balance_guaranteed_swap_instrument_commands::process_add_balance_guaranteed_swap_instrument(
    ...
    try {
        v.lockout_days = parse_optional_int(lockout_days, "lockout_days");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }
    ...
    if (result->success) {
        ...
        fail(out) << "Failed to add Balance guaranteed swap instrument: " << msg << std::endl;
    }
}
...
void balance_guaranteed_swap_instrument_commands::process_delete_balance_guaranteed_swap_instrument(
    ...
    if (result->success) {
        BOOST_LOG_SEV(lg(), info) << "Successfully deleted Balance guaranteed swap instrument.";
        out << "✓ Balance guaranteed swap instrument deleted successfully!" << std::endl;
    } else {
...
void balance_guaranteed_swap_instrument_commands::process_get_balance_guaranteed_swap_instrument_history(
    ...
    if (!result->success) {
        BOOST_LOG_SEV(lg(), warn) << "Failed to get Balance guaranteed swap instrument history: "
                                  << result->message;
    ...
    if (result->history.empty()) {
        out << "No history found for this Balance guaranteed swap instrument." << std::endl;
        return;
    }
