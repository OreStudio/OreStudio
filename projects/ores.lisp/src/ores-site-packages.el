...
;; Emacs 30 deletes the *user's* quickstart cache during a batch install, which
;; aborts on a read-only HOME. The fix is shared with the other batch scripts so
;; it cannot drift between them; see ores-batch-packages.el.
