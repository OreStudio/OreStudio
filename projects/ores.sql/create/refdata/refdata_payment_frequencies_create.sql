...
create table if not exists "ores_refdata_payment_frequencies_tbl" (
    "code" text not null,
    ...
    check ("code" <> ''),
    check (("period_unit" = 'NONE') = ("period_multiplier" is null))
);

-- Unique name for active records
create unique index if not exists payment_frequencies_name_uniq_idx
on "ores_refdata_payment_frequencies_tbl" (tenant_id, name)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists payment_frequencies_version_uniq_idx
on "ores_refdata_payment_frequencies_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists payment_frequencies_code_uniq_idx
on "ores_refdata_payment_frequencies_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists payment_frequencies_tenant_idx
on "ores_refdata_payment_frequencies_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_payment_frequencies_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    NEW.period_unit := ores_refdata_validate_tenor_unit_fn(NEW.tenant_id, NEW.period_unit);
...
create or replace trigger ores_refdata_payment_frequencies_insert_trg
before insert on "ores_refdata_payment_frequencies_tbl"
for each row execute function ores_refdata_payment_frequencies_insert_fn();

create or replace rule ores_refdata_payment_frequencies_delete_rule as
on delete to "ores_refdata_payment_frequencies_tbl" do instead (
    update "ores_refdata_payment_frequencies_tbl"
    set valid_to = clock_timestamp()
...
-- =============================================================================
-- Validation function for payment_frequency
-- Validates that a code exists in the payment_frequencies table.
-- Returns the validated value, or default if null/empty.
-- Uses system tenant data (shared reference data).
-- =============================================================================
create or replace function ores_refdata_validate_payment_frequency_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid payment_frequency: value cannot be null or empty'
            using errcode = '23502';
    end if;

    -- Allow pass-through during bootstrap (no active rows for system tenant).
    if not exists (
        select 1 from ores_refdata_payment_frequencies_tbl
    ...
    if not exists (
        ...
        raise exception 'Invalid payment_frequency: %. Must be one of: %', p_value, (
