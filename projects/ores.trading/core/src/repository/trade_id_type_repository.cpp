...
void trade_id_type_repository::write(context ctx, const std::vector<domain::trade_id_type>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing trade ID types. Count: " << v.size();
    execute_write_query(
        ctx, trade_id_type_mapper::map(v), lg(), "Writing trade ID types to database.");
}
