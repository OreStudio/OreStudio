...
#include "ores.analytics.api/eventing/pricing_model_config_event.hpp"
...
TEST_CASE("write_pricing_model_config_publishes_an_event", tags) {
    ...
    using event_type = ores::analytics::messaging::pricing_model_config_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_analytics_pricing_model_configs");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.name == v.name)
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_config(v.id).has_value());
        REQUIRE(svc.get_config_history(v.name).size() == versions.size());
    }
}
