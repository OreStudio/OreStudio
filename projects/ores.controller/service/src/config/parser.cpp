...
#include "ores.controller.service/config/parser_exception.hpp"
...
const std::string product_version("ores.controller.service v" ORES_VERSION);
...
using ores::controller::service::config::options;
using ores::controller::service::config::parser_exception;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.controller.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Service lifecycle controller" << std::endl
         << std::endl
         << "Usage: ores.controller.service [options]" << std::endl
         << std::endl
         << od << std::endl;
}
...
std::optional<options> parse_arguments(const std::vector<std::string>& arguments,
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("CONTROLLER_SERVICE"));
