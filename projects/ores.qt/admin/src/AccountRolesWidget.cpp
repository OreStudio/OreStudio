...
void AccountRolesWidget::setupUi() {
    ...
    connect(rolesList_,
            ...
            this,
            &AccountRolesWidget::onRoleSelectionChanged);

    groupLayout->addWidget(rolesList_);

    auto* buttonsLayout = new QHBoxLayout();

    assignButton_->setIcon(IconUtils::createRecoloredIcon(Icon::Add, IconUtils::DefaultIconColor));
    assignButton_->setToolTip("Assign selected role");
    assignButton_->setToolButtonStyle(Qt::ToolButtonIconOnly);
    connect(assignButton_, &QToolButton::clicked, this, &AccountRolesWidget::onAssignRoleClicked);

    revokeButton_->setIcon(
        IconUtils::createRecoloredIcon(Icon::Delete, IconUtils::DefaultIconColor));
    revokeButton_->setToolTip("Revoke selected role");
    revokeButton_->setToolButtonStyle(Qt::ToolButtonIconOnly);
    connect(revokeButton_, &QToolButton::clicked, this, &AccountRolesWidget::onRevokeRoleClicked);
...
void AccountRolesWidget::load() {
    ...
    if (has_account) {
        BOOST_LOG_SEV(lg(), debug)
            << "Loading roles for account: " << boost::uuids::to_string(accountId_);
    } else {
    ...
    connect(watcher, &QFutureWatcher<LoadResult>::finished, this, [self, watcher]() {
        ...
        if (result.success) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Loaded " << self->assignedRoles_.size() << " assigned, "
                                       << self->allRoles_.size() << " total roles";
        } else {
...
void AccountRolesWidget::onRevokeRoleClicked() {
    ...
    const auto roleIdStr = selected.first()->data(Qt::UserRole).toString().toStdString();
    if (roleIdStr.empty())
        return;
...
void AccountRolesWidget::refreshRolesList() {
    ...
    auto findRoleName = [&](const boost::uuids::uuid& id) -> QString {
        auto it = std::ranges::find_if(allRoles_, [&id](const auto& r) { return r.id == id; });
        if (it != allRoles_.end())
    ...
    for (const auto& role : assignedRoles_) {
        if (!isRemoved(role.id)) {
            ...
            item->setData(Qt::UserRole, QString::fromStdString(boost::uuids::to_string(role.id)));
    ...
    for (const auto& roleId : pendingAdds_) {
        ...
        item->setData(Qt::UserRole, QString::fromStdString(boost::uuids::to_string(roleId)));
    ...
    for (const auto& role : allRoles_) {
        bool inEffective =
            std::ranges::any_of(effectiveIds, [&role](const auto& id) { return id == role.id; });
        if (!inEffective) {
            roleCombo_->addItem(QString::fromStdString(role.name),
                                QVariant(QString::fromStdString(boost::uuids::to_string(role.id))));
        }
    }

    groupBox_->setTitle(QString("Assigned Roles (%1)").arg(static_cast<int>(effectiveIds.size())));
