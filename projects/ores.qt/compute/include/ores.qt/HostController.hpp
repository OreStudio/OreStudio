...
private:
    inline static std::string_view logger_name = "ores.qt.host_controller";
...
public:
    HostController(QMainWindow* mainWindow,
                   QMdiArea* mdiArea,
...
private slots:
    ...
    void onOpenVersion(const compute::domain::host& host, int versionNumber);
...
private:
     ...
     * HostDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(HostDetailDialog* detailDialog);
    void showHistoryWindow(const compute::domain::host& host);

    /**
     * @brief Fetches the full typed compute host history (the
     * existing per-entity compute::messaging::get_host_history_request/
     * compute::messaging::get_host_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed compute host, since the
     * generic dialog holds no typed domain data.
     */
    void fetchHostHistory(
