...
// ============================================================================
// Party ID Scheme helpers
// ============================================================================

void write_party_id_scheme(std::vector<std::byte>& buffer,
    const domain::party_id_scheme& ps) {
    ...
    writer::write_string(buffer, ps.coding_scheme_code);
...
std::expected<domain::party_id_scheme, error_code>
read_party_id_scheme(std::span<const std::byte>& data) {
    domain::party_id_scheme ps;
    ...
    ps.coding_scheme_code = *coding_scheme_code_result;
...
// ============================================================================
// Party ID Scheme Messages Implementation
// ============================================================================

std::vector<std::byte> get_party_id_schemes_request::serialize() const {
    return {};
}

std::expected<get_party_id_schemes_request, error_code>
get_party_id_schemes_request::deserialize(std::span<const std::byte> data) {
    if (!data.empty()) {
        return std::unexpected(error_code::payload_too_large);
    }
    return get_party_id_schemes_request{};
}
...
std::vector<std::byte> get_party_id_schemes_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(schemes.size()));
    for (const auto& ps : schemes) {
        write_party_id_scheme(buffer, ps);
    }
    return buffer;
}

std::expected<get_party_id_schemes_response, error_code>
get_party_id_schemes_response::deserialize(std::span<const std::byte> data) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        ...
        response.schemes.push_back(std::move(*result));
    }
...
std::vector<std::byte> save_party_id_scheme_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_party_id_scheme_response, error_code>
save_party_id_scheme_response::deserialize(std::span<const std::byte> data) {
...
std::expected<delete_party_id_scheme_request, error_code>
delete_party_id_scheme_request::deserialize(std::span<const std::byte> data) {
    delete_party_id_scheme_request request;
...
std::expected<delete_party_id_scheme_response, error_code>
...
std::vector<std::byte> get_party_id_scheme_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_party_id_scheme_history_request, error_code>
get_party_id_scheme_history_request::deserialize(std::span<const std::byte> data) {
    get_party_id_scheme_history_request request;
...
std::vector<std::byte> get_party_id_scheme_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_party_id_scheme(buffer, v);
    }
    return buffer;
}

std::expected<get_party_id_scheme_history_response, error_code>
get_party_id_scheme_history_response::deserialize(std::span<const std::byte> data) {
