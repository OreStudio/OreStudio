...
void currency_currency_group_commands::register_commands(cli::Menu& root_menu,
                                                         nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("currency_currency_groups");
    ...
    menu->Insert(
        ...
        "get <currency_iso_code> <currency_group_code>");
    ...
    menu->Insert(
        ...
        "get-many <currency_iso_code> <currency_group_code>");
    ...
    menu->Insert(
        ...
        "add <currency_iso_code> <currency_group_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <currency_iso_code> <currency_group_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <currency_iso_code> <currency_group_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <currency_iso_code> <currency_group_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <currency_iso_code> <currency_group_code> <reason> <commentary>");

    menu->Insert(
        "by-currency-iso-code",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_currency_iso_code(std::ref(out), std::ref(session), std::move(args));
        },
        "by-currency-iso-code <currency_iso_code> [--offset <n>] [--limit <n>] [--order <field>] "
        "[--desc]");

    root_menu.Insert(std::move(menu));
}

void currency_currency_group_commands::process_list(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_currency_currency_groups_request;
    ...
    auto result = do_auth_request<messaging::list_currency_currency_groups_response>(
...
void currency_currency_group_commands::process_get(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_currency_currency_group_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.currency_iso_code, parsed->positionals[next++], "currency_iso_code");
        read_token(req.key.currency_group_code, parsed->positionals[next++], "currency_group_code");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_currency_currency_group_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_get_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_currency_currency_groups_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            ...
            read_token(key.currency_iso_code, parsed->positionals[i + 0], "currency_iso_code");
            read_token(key.currency_group_code, parsed->positionals[i + 1], "currency_group_code");
    ...
    auto result = do_auth_request<messaging::get_many_currency_currency_groups_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_add(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_currency_currency_group_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.change.write.currency_iso_code, parsed->positionals[next++], "currency_iso_code");
        read_token(req.change.write.currency_group_code,
    ...
    auto result = do_auth_request<messaging::put_currency_currency_group_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_set(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
...
void currency_currency_group_commands::process_put_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_currency_currency_groups_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::currency_currency_group_change change;
            read_token(
                change.write.currency_iso_code, parsed->positionals[next++], "currency_iso_code");
            read_token(change.write.currency_group_code,
                       parsed->positionals[next++],
                       "currency_group_code");
    ...
    auto result = do_auth_request<messaging::put_many_currency_currency_groups_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_delete(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_currency_currency_group_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.removal.key.currency_iso_code, parsed->positionals[next++], "currency_iso_code");
        read_token(req.removal.key.currency_group_code,
                   parsed->positionals[next++],
                   "currency_group_code");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
    ...
    auto result = do_auth_request<messaging::delete_currency_currency_group_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_delete_many(std::ostream& out,
                                                           nats_client& session,
                                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_currency_currency_groups_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::currency_currency_group_key key;
            read_token(key.currency_iso_code, parsed->positionals[i * 2 + 0], "currency_iso_code");
            read_token(
                key.currency_group_code, parsed->positionals[i * 2 + 1], "currency_group_code");
            req.removals.push_back(
                messaging::currency_currency_group_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_currency_currency_groups_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_currency_group_commands::process_by_currency_iso_code(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-currency-iso-code request.";

    using request_type = messaging::list_by_currency_iso_code_currency_currency_groups_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-currency-iso-code." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.currency_iso_code = ores::shell::app::from_token<std::string>(
            parsed->positionals[next++], "currency_iso_code");
    ...
    auto result =
        do_auth_request<messaging::list_by_currency_iso_code_currency_currency_groups_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
