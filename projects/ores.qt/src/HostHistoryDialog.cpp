...
#include "ui_HostHistoryDialog.h"
...
HostHistoryDialog::HostHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::HostHistoryDialog),
      id_(id),
      code_(code),
...
void HostHistoryDialog::setupUi() {
...
void HostHistoryDialog::setupToolbar() {
...
void HostHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &HostHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &HostHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &HostHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void HostHistoryDialog::loadHistory() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        emit errorOccurred("Not connected to server");
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Loading history for compute host: " << code_.toStdString();
...
void HostHistoryDialog::updateVersionList() {
...
void HostHistoryDialog::onVersionSelected() {
...
void HostHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.location != previous.location) {
        addChange("Name",
                  QString::fromStdString(previous.location),
                  QString::fromStdString(current.location));
    }

    if (current.gpu_type != previous.gpu_type) {
        addChange("Description",
                  QString::fromStdString(previous.gpu_type),
                  QString::fromStdString(current.gpu_type));
    }
...
void HostHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->codeValue->setText(QString::fromStdString(version.external_id));
    ui_->nameValue->setText(QString::fromStdString(version.location));
    ui_->descriptionValue->setText(QString::fromStdString(version.gpu_type));
...
void HostHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void HostHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void HostHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
