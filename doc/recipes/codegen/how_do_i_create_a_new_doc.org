...
#+description: Use compass add to create any document type with frontmatter, UUID, and skeleton sections pre-filled.
#+type: recipe
#+level: cross
...
calling codegen directly. It supports: =story=, =task=, =sprint=,
...
Call =compass add <type>= with the slug, parent
directory, and the document's title and description. The generator fills in a fresh
...
[[id:A7F8D9E2-3C4B-4A5D-B67C-8E9F0A1B2C3D][Investigation reports]] live in
/the folder of the story that commissioned them/, as
=investigation_<slug>.org=, at level =s1=. Pass that story's folder as
=--parent-dir=; there is no default, because an investigation belongs to a
story rather than to a fixed directory.

#+begin_src sh :results verbatim
compass add investigation \
  --slug msvc_c1202_rfl_complexity \
  --parent-dir doc/agile/versions/v0/sprint_18/rfl_complexity_resolution \
...
Then add it to the story's =* Investigations= section. Before you cite it from
code, a recipe, a model or a knowledge page, read
[[id:1366553A-4E0F-496F-91DD-19171C003631][Document type: investigation]]: the
durable half of what it concludes goes to the page that owns the subject, and
those surfaces cite that page.

** New user journey
:PROPERTIES:
:ID: 62D58EB6-9977-411C-8403-E3A12DB4CB3D
:END:

[[id:F44766FA-1E9C-416F-849A-E131101B2ECA][User journeys]] live in [[proj:doc/knowledge/journeys][=doc/knowledge/journeys/=]], one subfolder per
logical group of journeys. Pass the group folder as =--parent-dir=; the group
hub is expected to be =<group>.org= inside it.

#+begin_src sh :results verbatim
compass add user_journey \
  --slug present_myself \
  --parent-dir doc/knowledge/journeys/profile \
...
This writes =doc/knowledge/journeys/profile/journey_present_myself.org=. Wire it
into the group hub's journey table.
...
#+begin_src sh :results verbatim
compass add capture \
  --slug add_uptime_screen \
  --parent-dir doc/agile/product_backlog/next \
  --title "Add an uptime screen" \
  --description "Surface live service health in the shell UI." \
...
the [[id:7357C672-A49F-4974-9399-A166F761D222][compass-doc-add-memory skill]]; for the standalone recipe with body
...
1. *Generate all skeleton files first.* Each call mints a fresh UUID.
   ...
   compass add component --slug component_overview ...
   compass add component --slug component_overview ...
   #+end_src
