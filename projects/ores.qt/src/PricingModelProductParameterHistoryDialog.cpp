...
void PricingModelProductParameterHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &PricingModelProductParameterHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &PricingModelProductParameterHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &PricingModelProductParameterHistoryDialog::onRevertClicked);
...
void PricingModelProductParameterHistoryDialog::loadHistory() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Loading history for pricing model product parameter: " << code_.toStdString();
    ...
    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<analytics::domain::pricing_model_product_parameter> versions;
    };
    ...
    auto task = [self, id = id_]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_pricing_model_product_parameter_history_request,
            0, std::move(payload)
        );
        ...
        auto response = analytics::messaging::get_pricing_model_product_parameter_history_response::
