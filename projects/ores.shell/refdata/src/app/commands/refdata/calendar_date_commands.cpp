...
void calendar_date_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("calendar_dates");
...
void calendar_date_commands::process_list(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_calendar_dates_request;
    ...
    auto result = do_auth_request<messaging::list_calendar_dates_response>(
...
void calendar_date_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_calendar_date_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.date, parsed->positionals[next++], "date");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_calendar_date_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_date_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_calendar_dates_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            messaging::calendar_date_key key;
            read_token(key.calendar_code, parsed->positionals[i + 0], "calendar_code");
            read_token(key.date, parsed->positionals[i + 1], "date");
    ...
    auto result = do_auth_request<messaging::get_many_calendar_dates_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_date_commands::process_by_calendar_code(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-calendar-code request.";

    using request_type = messaging::list_by_calendar_code_calendar_dates_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_by_calendar_code_calendar_dates_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
