...
void crm_enabled_derived_pair_commands::register_commands(cli::Menu& root_menu,
                                                          nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("crm_enabled_derived_pairs");
...
void crm_enabled_derived_pair_commands::process_list(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_crm_enabled_derived_pairs_request;
    ...
    auto result = do_auth_request<messaging::list_crm_enabled_derived_pairs_response>(
...
void crm_enabled_derived_pair_commands::process_get(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_crm_enabled_derived_pair_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_crm_enabled_derived_pair_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_get_many(std::ostream& out,
                                                         nats_client& session,
                                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_crm_enabled_derived_pairs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_crm_enabled_derived_pairs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_add(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_crm_enabled_derived_pair_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_crm_enabled_derived_pair_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_set(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
...
void crm_enabled_derived_pair_commands::process_put_many(std::ostream& out,
                                                         nats_client& session,
                                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_crm_enabled_derived_pairs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_many_crm_enabled_derived_pairs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_delete(std::ostream& out,
                                                       nats_client& session,
                                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_crm_enabled_derived_pair_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_crm_enabled_derived_pair_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_delete_many(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_crm_enabled_derived_pairs_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(
                messaging::crm_enabled_derived_pair_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_crm_enabled_derived_pairs_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_versions(std::ostream& out,
                                                         nats_client& session,
                                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_crm_enabled_derived_pair_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_crm_enabled_derived_pair_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void crm_enabled_derived_pair_commands::process_version(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_crm_enabled_derived_pair_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_crm_enabled_derived_pair_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
