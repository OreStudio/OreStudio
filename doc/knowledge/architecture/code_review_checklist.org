...
against. The [[id:04CA6BD9-7739-4213-A9DC-17AAACE43252][compass-code-review-pr]] skill applies them to a PR delta
(changed files only); the [[id:DADF4117-757A-4EEF-A137-CC2EE8023C73][compass-code-review-component]] skill applies them
...
| Check               | Description                                              |
|---------------------+----------------------------------------------------------|
| snake_case          | Classes, methods, variables use snake_case.              |
...
For each new or modified domain type, verify the [[id:B1450696-8C51-4CC5-8910-84912A411AB6][compass-code-add-domain-type]]
...
For NATS protocol changes. The normative source is the
[[id:C650CBC9-FDED-4C68-981F-7902E296928E][NATS Entity Protocol Specification]];
a reviewer applies the checks below against it.

| Check              | Description                                              |
|--------------------+----------------------------------------------------------|
| Subject convention | ={component}.v1.{resource}.{verb}=, one subject per operation. |
| Request / response | Reflect-cpp types end =_request= / =_response=.          |
| Versioning         | =v1= changes only for a breaking shape change.           |
| Handler            | Each request has a handler in =ores.service=.            |
| Authorisation      | Server-side checks for privileged operations.            |
| Write payload      | A write carries the write record: no server-owned field, no tenancy, and a stated precondition. |
...
These apply to whole-component reviews (=compass-code-review-component=) but
...
Group related issues into stories per =compass-doc-add=; use
=:code:= for fixes, =:doc:= for documentation issues.

* See also

- [[id:04CA6BD9-7739-4213-A9DC-17AAACE43252][compass-code-review-pr skill]] — applies this checklist to a PR delta.
- [[id:DADF4117-757A-4EEF-A137-CC2EE8023C73][compass-code-review-component skill]] — applies it to a whole component.
