...
:END:
#+title: C++ service templates
#+description: Service scaffolding: application/host classes, config options and parser, service class, main entry point and stub test.
#+type: facet
#+level: cross
#+facet_group: cpp
...
This page is the literate source for the =cpp_service= facet of the
...
Fourteen templates in two distinct families that share the prefix:
the /component-service/ scaffolding (twelve templates — application,
host, config options/parser with their exceptions, =main.cpp= and the
stub test; =component-service= profile, generated once per service
component) and the /entity service/ pair (=cpp_service.hpp/.cpp=;
=service= profile, generated per entity to expose repository
operations to the messaging layer).

* The cpp_service facet
...
with its exception type. The per-entity service class is the
NATS-facing API over the entity's repository — the seam between the
component scaffold and the entity pipeline.
...
{{! GENERATED FILE — tangled from projects/ores.codegen/library/templates/cpp_service.org. Edit the org source. }}
{{{cpp_license}}}
{{#component}}
