...
- /Merge never touches task state./ =compass-agile-close-task= is the
  ...
  bookkeeping PR ever again. =compass-pr-merge= only merges, keeping its
...
- Canonical lifecycle: =compass-agile-start-task= -> work ->
  =compass-pr-address-review= (rounds) -> =compass-agile-close-task= -> =compass-pr-merge=.
