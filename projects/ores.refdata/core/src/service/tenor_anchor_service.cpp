 ...
 */
std::vector<domain::tenor_anchor> read_one(repository::tenor_anchor_repository& repo,
                                           const ores::database::context& ctx,
                                           const messaging::tenor_anchor_key& key) {
 ...
 */
messaging::tenor_anchor_key key_from(const domain::tenor_anchor& v) {
    messaging::tenor_anchor_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::tenor_anchor to_domain(const messaging::tenor_anchor_write& write) {
    domain::tenor_anchor v;
    v.code = write.code;
...
tenor_anchor_service::list_tenor_anchors(const messaging::list_tenor_anchors_request& request) {
    messaging::list_tenor_anchors_response response;
    ...
    response.anchors = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_anchor_count(ctx_);
    return response;
}

messaging::get_tenor_anchor_response
tenor_anchor_service::get_tenor_anchor(const messaging::get_tenor_anchor_request& request) {
    messaging::get_tenor_anchor_response response;
    ...
    response.tenor_anchor = std::move(found.front());
    return response;
}

messaging::get_many_tenor_anchors_response tenor_anchor_service::get_many_tenor_anchors(
    const messaging::get_many_tenor_anchors_request& request) {
    messaging::get_many_tenor_anchors_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenor_anchor_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenor_anchor = std::move(found.front());
...
tenor_anchor_service::put_tenor_anchor(const messaging::put_tenor_anchor_request& request) {
    messaging::put_tenor_anchor_response response;
    domain::tenor_anchor value;
    ...
    if (!written.empty())
        response.tenor_anchor = std::move(written.front());
    return response;
}

messaging::put_many_tenor_anchors_response tenor_anchor_service::put_many_tenor_anchors(
    const messaging::put_many_tenor_anchors_request& request) {
    messaging::put_many_tenor_anchors_response response;
    std::vector<domain::tenor_anchor> batch;
    ...
    response.anchors.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.anchors.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenor_anchor_response
tenor_anchor_service::delete_tenor_anchor(const messaging::delete_tenor_anchor_request& request) {
    messaging::delete_tenor_anchor_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::tenor_anchor_repository::remove_status::removed:
            break;
        case repository::tenor_anchor_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenor_anchor_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenor_anchor_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_tenor_anchors_response tenor_anchor_service::delete_many_tenor_anchors(
    const messaging::delete_many_tenor_anchors_request& request) {
    messaging::delete_many_tenor_anchors_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_tenor_anchor_versions_response tenor_anchor_service::list_tenor_anchor_versions(
    const messaging::list_tenor_anchor_versions_request& request) {
    messaging::list_tenor_anchor_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_tenor_anchor_version_response tenor_anchor_service::get_tenor_anchor_version(
    const messaging::get_tenor_anchor_version_request& request) {
    messaging::get_tenor_anchor_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.tenor_anchor.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tenor_anchor_service::prepare_change(const messaging::tenor_anchor_change& change,
...
tenor_anchor_service::get_anchors(const std::vector<std::string>& codes) {
