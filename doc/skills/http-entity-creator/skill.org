...
When you need to add HTTP REST API endpoints for a new entity in =ores.http.server=.
This skill guides you through creating the routes class, registering it with the
application, and implementing standard CRUD operations following established
patterns.
...
The following sections describe the step-by-step process for creating HTTP
...
- *Domain location*: Which domain the entity belongs to (e.g., =ores.risk=, =ores.iam=).
...
This phase creates the routes class with basic retrieval and save operations.
After completing Steps 1-3, raise a PR.
...
For new routes class:
=projects/ores.http.server/include/ores.http.server/routes/<domain>_routes.hpp=
...
register_routes(std::shared_ptr<http::net::router> router,
    ...
    {
        auto route = router->get("/api/v1/<entities>")
        ...
        router->add_route(route.build());
        registry->register_route(route.build());
    }

    // POST /api/v1/<entities> - Save <entity>
    {
        auto route = router->post("/api/v1/<entities>")
            .summary("Save <entity>")
...
handle_get_<entity>(const http_request& req) {
    ...
    try {
        ...
        auto <entity>_id = req.get_path_param("<identifier>");
        if (<entity>_id.empty()) {
            co_return http_response::bad_request("Missing <entity> identifier");
        }
...
2. Add registration in the appropriate setup method:
...
#+begin_src cpp
    ...
    {
        auto route = router->delete_("/api/v1/<entities>")
...
#+begin_src cpp
    // GET /api/v1/<entities>/{id}/history - Get <entity> history
    {
        auto route = router->get("/api/v1/<entities>/{<identifier>}/history")
...
Implement version history retrieval by <entity> identifier.
#+end_example
...
- GET /api/v1/<entities>/{id}/history - Version history retrieval
#+end_example
...
#+name: <entities>-get
#+begin_src sh :var jwt_token="" :results output :wrap example
...
#+begin_src sh :var jwt_token="" :results output :wrap example
curl -s -X GET "${base_url}/api/v1/<entities>/<entity_id>/history" \
  -H "Authorization: Bearer ${jwt_token}" \
  -H "Content-Type: application/json" | jq .
#+end_src
#+end_src
...
| =/api/v1/<entities>/{id}=         | Get single, Update, Delete single     |
| =/api/v1/<entities>/{id}/history= | Version history                       |
