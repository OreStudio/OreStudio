...
void fx_barrier_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    fx_barrier_option_instruments_menu->Insert(
        ...
        [&session, &pagination](std::ostream& out) {
            process_get_fx_barrier_option_instruments(std::ref(out), std::ref(session),
                                                      std::ref(pagination));
        },
        "Retrieve FX barrier option instruments from the server (paginated)");

    // Register list callback for navigation
    pagination.register_list_callback("fx_barrier_option_instruments",
                                      [&session, &pagination](std::ostream& out) {
                                          process_get_fx_barrier_option_instruments(out, session,
                                                                                    pagination);
                                      });
    ...
    fx_barrier_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::string instrument_id) {
            process_delete_fx_barrier_option_instrument(std::ref(out), std::ref(session),
    ...
    fx_barrier_option_instruments_menu->Insert(
        "history",
        [&session](std::ostream& out, std::string instrument_id) {
            process_get_fx_barrier_option_instrument_history(std::ref(out), std::ref(session),
                                        std::move(instrument_id));
        },
...
void fx_barrier_option_instrument_commands::process_delete_fx_barrier_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an FX barrier option instrument." << std::endl;
        return;
    }
    ...
    if (result->success) {
        ...
        BOOST_LOG_SEV(lg(), warn) << "Failed to delete FX barrier option instrument: "
                                  << result->message;
        fail(out) << "Failed to delete FX barrier option instrument: " << result->message << std::endl;
    }
}
...
void fx_barrier_option_instrument_commands::process_get_fx_barrier_option_instrument_history(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get FX barrier option instrument history." << std::endl;
        return;
    }
