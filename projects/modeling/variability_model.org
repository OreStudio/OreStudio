...
1. *Physical-space activation* — enabling and disabling regions of the
   ...
   archetype), which superseded the retired =facet_catalogue.org=
   monolith.
...
- A *structural predicate* (not variability) is a value the generator
  *computes* from other, already-known facts about the entity
  (=table['has_tenant_in_pk'] = has_tenant_id and not
  sql_section.get('system_scope')...=, =field['is_uuid'] = type ==
  'uuid'=). It is a /consequence/ of the structure, not a choice about
  projection. These belong to the logical model and are listed under
  [[*Excluded: structural predicates][Excluded: structural predicates]] so they are never mistaken for
  configuration.

The test above is necessary but, read alone, invites a mistake: a
knob whose effect is to add a field or column — =:has_tenant_id:=,
say — looks structural, because what it produces (a =tenant_id=
column) plainly is. It is not. Structure is measured strictly by what
appears in the generated type system: the classes, structs, fields,
and methods a facet emits. A knob is never itself one of those
things — authoring =:has_tenant_id: true= in an entity model's Flags
drawer produces no field, method, or class named =has_tenant_id=
anywhere in the generated code. It is an internal variable the
generator reads (=domain_entity.get('has_tenant_id', False)= in
=core.py=) to decide *whether* to branch into the template logic that
emits a =tenant_id= column, nothing more. That decision, and every
knob like it, is non-structural variability, full stop — whether it
goes on to add a field to the type system (=:has_tenant_id:=) or only
changes template behaviour that never touches the type system at all
(a naming convention, a cosmetic default). The resulting field or
column, once emitted, is of course a structural fact about that
particular entity — but the fact and the knob that decided it belong
to different models: the fact is logical, the knob is variability.

=:has_tenant_id:= and =:has_workspace_id:= (documented on [[id:0BD99407-1CBE-469B-B2D3-1C0AE3AA18DE][Keys and
columns]]) are exactly this case, and were previously misclassified
below as computed structural predicates — checked against
=core.py=/=org_loader.py=, both are read straight from the model's
frontmatter, with a default applied (=bool(table.get('has_tenant_id',
True))=), not derived from any other fact about the entity. They are
genuine features and are removed from the exclusion table below.
=has_tenant_in_pk=, by contrast, /is/ correctly excluded: it is
computed from =has_tenant_id= together with other already-known
facts, never authored directly.

* Part 1 — Physical-space activation (the ores.* graph)

ORE Studio does not maintain a formal VMM data structure for activation.
Instead, the [[id:D7D53D71-C9E8-43AE-AA42-E58E57CD76C8][ores physical-space graph]] under
=projects/ores.codegen/library/templates/= serves as the *Profile
Metamodel (PMM)*: one node per technical space (=ores.cpp.org=, ...),
fanning out to facet nodes (=ores.cpp.qt.org=, ...), each carrying an
=* Archetypes= table mapping every archetype to its output path and
profile membership.

*Historical note:* this activation catalogue previously lived as a
single flat file, =facet_catalogue.org=, parsed directly by
=generator.py= via =_load_profiles_from_org()=. That file and function
were retired once the ores.* graph above fully absorbed its content
(story: [[id:9023BDE0-979F-422F-BC73-985D6A80E7CB][Retire legacy codegen profile system]]). The =--profile= CLI flag
that named those legacy profiles has since been removed outright (task
B11) — =--address= against the =ores.*= graph is now the only
selector. The "Profile catalogue" table below is kept as a historical
map from each retired profile name to its current facet address, for
readers translating old commands or docs.
...
facet's archetypes can apply to it. A facet's =model_types= list acts as
the filter: only a model whose type appears in that list is a valid
target for the facet.
...
inadvertently running against entity models, and the =domain= facet from
running against SQL-only table models.

** Profile catalogue (historical)

All profiles defined in the (now-retired) =facet_catalogue.org= as of
its last state, and later exposed as the deprecated =--profile= CLI
shim (removed by task B11) — see the historical note above. Each name
below maps 1:1 to the current facet address of the same purpose (e.g.
=domain= → =ores.cpp.domain=, =sql= → =ores.sql.schema=; see
[[*The facet catalogue][the facet catalogue]] in [[id:43FF3591-AD18-4CE3-8239-A220862C9B5A][Applied MASD]] for the full current mapping).
The =all=/=all-cpp=/=non-temporal= composite aliases have no single
address equivalent — use the relevant technical-space or root address
(=ores.cpp=, =ores=) instead:
...
Omitting =--address= generates the entity's full supported set (every
facet its model type and =ores.*= drawer overrides admit); giving an
address restricts the run to the facets under that physical-space
subtree. See [[*Element-scope activation — the supported/target set model]] below
for the full address hierarchy, specificity resolution rules, and
target/supported-set semantics.

*Component scope* is handled by =src/codegen/manifest.py=: the manifest
maps component names to their model discovery roots (=models_dir= for
legacy JSON models; =modeling_dir= for org-mode model files). It does
not assign a default address — that is always either omitted (full
supported set) or given explicitly by the developer, a [[id:F663DF66-4F62-0884-DB23-BB2D6A42B644][skill]], or a
compass recipe.

*Product scope* — a MASD concept meaning activation applies across the
whole product — is expressed by a technical-space or root address
(=ores.cpp=, =ores=) and by [[id:F663DF66-4F62-0884-DB23-BB2D6A42B644][skills]] that bundle common multi-facet
...
The TS→facet map now lives in each technical space's own =* Facets=
table in the [[id:D7D53D71-C9E8-43AE-AA42-E58E57CD76C8][ores physical-space graph]] (e.g. =ores.cpp.org=,
=ores.sql.org=); this table reflects the mapping as it stood in the
retired =facet_catalogue.org=:
...
| =has_explorer_api= | bool | false | Controller exposes public =openAdd()=, =openEdit(entity)=, =openHistory(entity)= so a sibling explorer window (e.g. a tree mixing this entity with another) can drive it from outside, instead of only reacting to its own list/detail windows. |
| =parent_entity_singular= | string | (unset) | Only meaningful with =has_explorer_api=. Names another entity as this one's parent (e.g. book's parent is portfolio). Adds =openAddWithParent(boost::uuids::uuid parent<Pascal>Id)= and prefills that field when the add dialog opens nested under a parent explorer node. Assumes a =parent_<entity>_id= uuid column already exists on the domain entity. |
...
variability. Every one is *computed* by =generator.py= from other,
...
=has_workspace_id=, and =has_display_order= — genuine authored
features, all read verbatim from frontmatter with no derivation from
other facts — do *not* belong on this list and were removed from it;
=has_tenant_id=/=has_workspace_id= are documented on [[id:0BD99407-1CBE-469B-B2D3-1C0AE3AA18DE][Keys and
columns]]. See § "The test: authored knob vs computed predicate"
above. =has_image_id= and =has_coding_scheme= are split-personality:
...
|-------+------------+---------------|
| Presence gates | =has_tenant_in_pk=, =has_multiple_natural_keys=, =has_uuid_columns=, =has_badge_columns=, =has_description_column=, =has_combo_fields=, =has_text_edit_fields=, =has_uuid_detail_fields=, =has_uuid_left_or_right= | "does the entity have attribute / key X" |
...
| Profile | Named bundle of feature values + activation | Split: activation is an =ores.*= graph archetype entry (Part 1); feature values are authored per element (Part 2) |
...
| VMM | Meta-model of the full configuration space | =ores.*= physical-space graph (activation) + the feature catalogue in Part 2 (configuration) |
