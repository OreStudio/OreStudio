...
#include "ui_CreditInstrumentHistoryDialog.h"
...
CreditInstrumentHistoryDialog::CreditInstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::CreditInstrumentHistoryDialog),
      id_(id),
...
void CreditInstrumentHistoryDialog::setupUi() {
...
void CreditInstrumentHistoryDialog::setupToolbar() {
...
void CreditInstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &CreditInstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &CreditInstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &CreditInstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void CreditInstrumentHistoryDialog::loadHistory() {
    ...
    QPointer<CreditInstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::credit_instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_credit_instrument_history_request request;
        request.id = id;
...
void CreditInstrumentHistoryDialog::updateVersionList() {
...
void CreditInstrumentHistoryDialog::onVersionSelected() {
...
void CreditInstrumentHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.reference_entity != previous.reference_entity)
        addChange("Reference Entity",
            QString::fromStdString(previous.reference_entity),
            QString::fromStdString(current.reference_entity));
    ...
    if (current.notional != previous.notional)
        addChange("Notional",
            QString::number(previous.notional, 'f', 2),
            QString::number(current.notional, 'f', 2));

    if (current.spread != previous.spread)
        addChange("Spread",
            QString::number(previous.spread, 'f', 4),
            QString::number(current.spread, 'f', 4));

    if (current.recovery_rate != previous.recovery_rate)
        addChange("Recovery Rate",
            QString::number(previous.recovery_rate, 'f', 4),
            QString::number(current.recovery_rate, 'f', 4));

    if (current.tenor != previous.tenor)
        addChange("Tenor",
            QString::fromStdString(previous.tenor),
            QString::fromStdString(current.tenor));

    if (current.start_date != previous.start_date)
        addChange("Start Date",
            QString::fromStdString(previous.start_date),
            QString::fromStdString(current.start_date));
    ...
    if (current.payment_frequency_code != previous.payment_frequency_code)
        addChange("Payment Frequency",
            QString::fromStdString(previous.payment_frequency_code),
            QString::fromStdString(current.payment_frequency_code));

    if (current.index_name != previous.index_name)
        addChange("Index Name",
            QString::fromStdString(previous.index_name),
            QString::fromStdString(current.index_name));

    if (current.index_series != previous.index_series)
        addChange("Index Series",
            QString::number(previous.index_series),
            QString::number(current.index_series));

    if (current.seniority != previous.seniority)
        addChange("Seniority",
            QString::fromStdString(previous.seniority),
            QString::fromStdString(current.seniority));

    if (current.restructuring != previous.restructuring)
        addChange("Restructuring",
            QString::fromStdString(previous.restructuring),
            QString::fromStdString(current.restructuring));
...
void CreditInstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->referenceEntityValue->setText(
        QString::fromStdString(v.reference_entity));
    ui_->currencyValue->setText(
        QString::fromStdString(v.currency));
    ui_->notionalValue->setText(
        QString::number(v.notional, 'f', 2));
    ui_->spreadValue->setText(
        QString::number(v.spread, 'f', 4) + " bps");
    ui_->tenorValue->setText(
        QString::fromStdString(v.tenor));
...
void CreditInstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CreditInstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CreditInstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
    int row = selected.first()->row();
    if (static_cast<std::size_t>(row) >= versions_.size()) return;
