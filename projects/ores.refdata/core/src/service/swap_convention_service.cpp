 ...
 */
std::vector<domain::swap_convention> read_one(repository::swap_convention_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::swap_convention_key& key) {
 ...
 */
messaging::swap_convention_key key_from(const domain::swap_convention& v) {
    messaging::swap_convention_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::swap_convention to_domain(const messaging::swap_convention_write& write) {
    domain::swap_convention v;
    v.id = write.id;
...
messaging::get_swap_convention_response swap_convention_service::get_swap_convention(
    const messaging::get_swap_convention_request& request) {
    messaging::get_swap_convention_response response;
    ...
    response.swap_convention = std::move(found.front());
    return response;
}

messaging::get_many_swap_conventions_response swap_convention_service::get_many_swap_conventions(
    const messaging::get_many_swap_conventions_request& request) {
    messaging::get_many_swap_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::swap_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.swap_convention = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_swap_convention_response swap_convention_service::put_swap_convention(
    const messaging::put_swap_convention_request& request) {
    messaging::put_swap_convention_response response;
    domain::swap_convention value;
    ...
    if (!written.empty())
        response.swap_convention = std::move(written.front());
    return response;
}

messaging::put_many_swap_conventions_response swap_convention_service::put_many_swap_conventions(
    const messaging::put_many_swap_conventions_request& request) {
    messaging::put_many_swap_conventions_response response;
    std::vector<domain::swap_convention> batch;
    ...
    response.swap_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.swap_conventions.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_swap_convention_response swap_convention_service::delete_swap_convention(
    const messaging::delete_swap_convention_request& request) {
    messaging::delete_swap_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::swap_convention_repository::remove_status::removed:
            break;
        case repository::swap_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::swap_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::swap_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_swap_conventions_response
swap_convention_service::delete_many_swap_conventions(
    const messaging::delete_many_swap_conventions_request& request) {
    messaging::delete_many_swap_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_swap_convention_versions_response
swap_convention_service::list_swap_convention_versions(
    const messaging::list_swap_convention_versions_request& request) {
    messaging::list_swap_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_swap_convention_version_response
swap_convention_service::get_swap_convention_version(
    const messaging::get_swap_convention_version_request& request) {
    messaging::get_swap_convention_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.swap_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
swap_convention_service::prepare_change(const messaging::swap_convention_change& change,
                                        const ores::utility::domain::change_intent& intent,
                                        domain::swap_convention& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::swap_convention>
swap_convention_service::get_swap_conventions(const std::vector<std::string>& ids) {
