...
std::string instrument_option_premium_repository::sql() {
    return generate_create_table_sql<instrument_option_premium_entity>(lg());
}

void instrument_option_premium_repository::write(context ctx,
                                                 const domain::instrument_option_premium& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing instrument option premium. "
                               << "instrument_id: " << v.instrument_id
                               << " sequence_number: " << v.sequence_number;
    execute_write_query(ctx,
                        instrument_option_premium_mapper::map(v),
                        lg(),
                        "Writing instrument option premium to database.");
}

void instrument_option_premium_repository::write(
    context ctx, const std::vector<domain::instrument_option_premium>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing instrument option premiums. Count: " << v.size();
    execute_write_query(ctx,
                        instrument_option_premium_mapper::map(v),
                        lg(),
                        "Writing instrument option premiums to database.");
}

std::vector<domain::instrument_option_premium>
instrument_option_premium_repository::read_latest(context ctx) {
    ...
    return execute_read_query<instrument_option_premium_entity, domain::instrument_option_premium>(
        ...
        lg(),
        "Reading latest instrument option premiums");
}
...
std::vector<domain::instrument_option_premium> instrument_option_premium_repository::read_latest(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Reading latest instrument option premium. "
    ...
    return execute_read_query<instrument_option_premium_entity, domain::instrument_option_premium>(
        ...
        [](const auto& entities) { return instrument_option_premium_mapper::map(entities); },
        lg(),
        "Reading latest instrument option premium by instrument_id.");
}


std::vector<domain::instrument_option_premium> instrument_option_premium_repository::read_all(
    context ctx, const std::string& instrument_id, const std::string& sequence_number) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all instrument option premium versions. "
                               << "instrument_id: " << instrument_id
                               << " sequence_number: " << sequence_number;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<instrument_option_premium_entity>> |
    ...
    return execute_read_query<instrument_option_premium_entity, domain::instrument_option_premium>(
        ...
        [](const auto& entities) { return instrument_option_premium_mapper::map(entities); },
        lg(),
        "Reading all instrument option premium versions by instrument_id.");
}

std::optional<domain::instrument_option_premium>
instrument_option_premium_repository::read_at_version(context ctx,
                                                      const std::string& instrument_id,
                                                      const std::string& sequence_number,
                                                      std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading instrument option premium at version. "
                               << "instrument_id: " << instrument_id
                               << " sequence_number: " << sequence_number
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto entities =
        execute_read_query<instrument_option_premium_entity, domain::instrument_option_premium>(
            ctx,
            query,
            [](const auto& entities) { return instrument_option_premium_mapper::map(entities); },
            lg(),
            "Reading instrument option premium at version.");
...
void instrument_option_premium_repository::remove(context ctx,
                                                  const std::string& instrument_id,
                                                  const std::string& sequence_number) {
    ...
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<instrument_option_premium_entity> |
                       where("tenant_id"_c == tid && "instrument_id"_c == instrument_id &&
                             "sequence_number"_c == sequence_number && "valid_to"_c == max.value());

    execute_delete_query(ctx, query, lg(), "Removing instrument option premium from database.");
}

std::vector<domain::instrument_option_premium> instrument_option_premium_repository::read_latest(
    context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest instrument option premiums with offset: "
...
std::uint32_t instrument_option_premium_repository::get_total_option_premium_count(context ctx) {
...
void instrument_option_premium_repository::remove(
    context ctx,
    ...
    if (sequence_numbers.size() != instrument_ids.size())
        throw std::invalid_argument("instrument_option_premium_repository::remove: key column "
