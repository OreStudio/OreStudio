...
set; see [[id:818208C0-3465-4CEF-A8C8-62A460871B2C][Codegen junction
...
junctions render the base set -- the two reads, =save=, =delete= and the
two counts -- each with a =:list_by:= read: =calendar_dates=,
=currency_calendars=, =currency_countries=, =currency_currency_groups=,
=currency_pair_convention_calendars=, =party_counterparties=,
=party_countries=, =party_currencies= and
=tenor_convention_resolutions=. No refdata junction declares
=:replace_by:=, so no =replace_by= subject is generated here.

=tenor_convention_resolution= is read-only: its rows are provisioned
outside the application, so it renders the reads and the counts and no
write verb. =calendar_date= keeps its write verbs because the calendar
materialisation service writes its rows through the repository.
