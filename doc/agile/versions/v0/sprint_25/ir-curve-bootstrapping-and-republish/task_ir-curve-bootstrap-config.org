...
- =output_series_id= is a natural key alongside =party_id= (exactly
  one active config per output series) -- caught and fixed after the
  first codegen pass produced a wrong one-config-per-party unique
  index instead.
- Two-tier =discount_curve_config_id= enforcement: a check constraint
  shape-validates (nil-uuid iff =FUNDING=; non-nil and not
  self-referencing iff =PROJECTION=), and a custom insert-trigger
  block validates the *referenced* config is itself =FUNDING= (no
  chaining), using the same per-entity =:implements <uuid>= paste-marker
  mechanism =curve_role='s =Extra drops= already uses.
