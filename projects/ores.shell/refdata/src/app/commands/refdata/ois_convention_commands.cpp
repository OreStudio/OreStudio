...
void ois_convention_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("ois_conventions");
...
void ois_convention_commands::process_list(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_ois_conventions_request;
    ...
    auto result = do_auth_request<messaging::list_ois_conventions_response>(
...
void ois_convention_commands::process_get(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_ois_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_ois_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_get_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_ois_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_ois_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_add(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_ois_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.spot_lag, parsed->positionals[next++], "spot_lag");
        read_token(req.change.write.index, parsed->positionals[next++], "index");
        read_token(req.change.write.fixed_day_count_fraction,
                   parsed->positionals[next++],
                   "fixed_day_count_fraction");
        read_token(req.change.write.fixed_calendar, parsed->positionals[next++], "fixed_calendar");
        read_token(req.change.write.payment_lag, parsed->positionals[next++], "payment_lag");
        read_token(req.change.write.end_of_month, parsed->positionals[next++], "end_of_month");
        read_token(
            req.change.write.fixed_frequency, parsed->positionals[next++], "fixed_frequency");
        read_token(
            req.change.write.fixed_convention, parsed->positionals[next++], "fixed_convention");
        read_token(req.change.write.fixed_payment_convention,
                   parsed->positionals[next++],
                   "fixed_payment_convention");
        read_token(req.change.write.rule, parsed->positionals[next++], "rule");
        read_token(
    ...
    auto result = do_auth_request<messaging::put_ois_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_set(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
...
void ois_convention_commands::process_put_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_ois_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::ois_convention_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.spot_lag, parsed->positionals[next++], "spot_lag");
            read_token(change.write.index, parsed->positionals[next++], "index");
            read_token(change.write.fixed_day_count_fraction,
                       parsed->positionals[next++],
                       "fixed_day_count_fraction");
            read_token(change.write.fixed_calendar, parsed->positionals[next++], "fixed_calendar");
            read_token(change.write.payment_lag, parsed->positionals[next++], "payment_lag");
            read_token(change.write.end_of_month, parsed->positionals[next++], "end_of_month");
            read_token(
                change.write.fixed_frequency, parsed->positionals[next++], "fixed_frequency");
            read_token(
                change.write.fixed_convention, parsed->positionals[next++], "fixed_convention");
            read_token(change.write.fixed_payment_convention,
                       parsed->positionals[next++],
                       "fixed_payment_convention");
            read_token(change.write.rule, parsed->positionals[next++], "rule");
            read_token(
    ...
    auto result = do_auth_request<messaging::put_many_ois_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_delete(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_ois_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_ois_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_delete_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_ois_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::ois_convention_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_ois_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_versions(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_ois_convention_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_ois_convention_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ois_convention_commands::process_version(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_ois_convention_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.ois_convention.id, parsed->positionals[next++], "id");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_ois_convention_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
