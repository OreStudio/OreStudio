...
void ClientContactTypeModel::refresh() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Starting contact type fetch";
    is_fetching_ = true;
    ...
    QFuture<FetchResult> future = QtConcurrent::run([self]() -> FetchResult {
        return exception_helper::wrap_async_fetch<FetchResult>([&]() -> FetchResult {
            ...
            auto response = refdata::messaging::get_contact_types_response::
...
void ClientContactTypeModel::onTypesLoaded() {
    ...
    BOOST_LOG_SEV(lg(), info) << "Loaded " << types_.size() << " contact types";
    emit dataLoaded();
}
