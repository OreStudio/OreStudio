...
void currency_pair_convention_calendar_commands::register_commands(cli::Menu& root_menu,
                                                                   nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("currency_pair_convention_calendars");
    ...
    menu->Insert(
        ...
        "get-many <pair_code> <calendar_code>");
    ...
    menu->Insert(
        ...
        "add <pair_code> <calendar_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <pair_code> <calendar_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <pair_code> <calendar_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <pair_code> <calendar_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <pair_code> <calendar_code> <reason> <commentary>");

    menu->Insert(
        "by-pair-code",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_pair_code(std::ref(out), std::ref(session), std::move(args));
        },
        "by-pair-code <pair_code> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");

    root_menu.Insert(std::move(menu));
}

void currency_pair_convention_calendar_commands::process_list(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_currency_pair_convention_calendars_request;
    ...
    auto result = do_auth_request<messaging::list_currency_pair_convention_calendars_response>(
...
void currency_pair_convention_calendar_commands::process_get(std::ostream& out,
                                                             nats_client& session,
                                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_currency_pair_convention_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.pair_code, parsed->positionals[next++], "pair_code");
        read_token(req.key.calendar_code, parsed->positionals[next++], "calendar_code");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_currency_pair_convention_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_get_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_currency_pair_convention_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            ...
            read_token(key.pair_code, parsed->positionals[i + 0], "pair_code");
            read_token(key.calendar_code, parsed->positionals[i + 1], "calendar_code");
    ...
    auto result = do_auth_request<messaging::get_many_currency_pair_convention_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_add(std::ostream& out,
                                                             nats_client& session,
                                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_currency_pair_convention_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.pair_code, parsed->positionals[next++], "pair_code");
    ...
    auto result = do_auth_request<messaging::put_currency_pair_convention_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_set(std::ostream& out,
                                                             nats_client& session,
                                                             const std::vector<std::string>& args) {
...
void currency_pair_convention_calendar_commands::process_put_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_currency_pair_convention_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::currency_pair_convention_calendar_change change;
            read_token(change.write.pair_code, parsed->positionals[next++], "pair_code");
    ...
    auto result = do_auth_request<messaging::put_many_currency_pair_convention_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_delete(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_currency_pair_convention_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.pair_code, parsed->positionals[next++], "pair_code");
        read_token(req.removal.key.calendar_code, parsed->positionals[next++], "calendar_code");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_currency_pair_convention_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_delete_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_currency_pair_convention_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::currency_pair_convention_calendar_key key;
            read_token(key.pair_code, parsed->positionals[i * 2 + 0], "pair_code");
            read_token(key.calendar_code, parsed->positionals[i * 2 + 1], "calendar_code");
            req.removals.push_back(
                messaging::currency_pair_convention_calendar_removal{.key = std::move(key)});
        }
    ...
    auto result =
        do_auth_request<messaging::delete_many_currency_pair_convention_calendars_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_pair_convention_calendar_commands::process_by_pair_code(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-pair-code request.";

    using request_type = messaging::list_by_pair_code_currency_pair_convention_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-pair-code." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.pair_code =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "pair_code");
    ...
    auto result =
        do_auth_request<messaging::list_by_pair_code_currency_pair_convention_calendars_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
