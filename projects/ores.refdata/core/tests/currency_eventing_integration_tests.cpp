...
// notification" pattern end to end for currency -- the
...
TEST_CASE("write_currency_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::currency_changed_event>(
        [&nats](const ores::refdata::eventing::currency_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::currency_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::currency_changed_event>(
        "ores.refdata.currency", "ores_refdata_currencies");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::currency_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_currency(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.iso_code;
    BOOST_LOG_SEV(lg, debug) << "Currency: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.currency") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for currency " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for currency " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_currency(id_str).has_value());
        REQUIRE(svc.get_currency_history(id_str).size() == versions.size());
    }
}
