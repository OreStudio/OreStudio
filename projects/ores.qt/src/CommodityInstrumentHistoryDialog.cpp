...
#include "ui_CommodityInstrumentHistoryDialog.h"
...
CommodityInstrumentHistoryDialog::CommodityInstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::CommodityInstrumentHistoryDialog),
      id_(id),
...
void CommodityInstrumentHistoryDialog::setupUi() {
...
void CommodityInstrumentHistoryDialog::setupToolbar() {
...
void CommodityInstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &CommodityInstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &CommodityInstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &CommodityInstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void CommodityInstrumentHistoryDialog::loadHistory() {
    ...
    QPointer<CommodityInstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::commodity_instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_commodity_instrument_history_request request;
        request.id = id;
...
void CommodityInstrumentHistoryDialog::updateVersionList() {
...
void CommodityInstrumentHistoryDialog::onVersionSelected() {
...
void CommodityInstrumentHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.commodity_code != previous.commodity_code)
        addChange("Commodity",
            QString::fromStdString(previous.commodity_code),
            QString::fromStdString(current.commodity_code));
    ...
    if (current.quantity != previous.quantity)
        addChange("Quantity",
            QString::number(previous.quantity, 'f', 6),
            QString::number(current.quantity, 'f', 6));

    if (current.unit != previous.unit)
        addChange("Unit",
            QString::fromStdString(previous.unit),
            QString::fromStdString(current.unit));
    ...
    if (current.exercise_type != previous.exercise_type)
        addChange("Exercise Type",
            QString::fromStdString(previous.exercise_type),
            QString::fromStdString(current.exercise_type));
    ...
    if (current.barrier_type != previous.barrier_type)
        addChange("Barrier Type",
            QString::fromStdString(previous.barrier_type),
            QString::fromStdString(current.barrier_type));
    ...
    if (current.average_type != previous.average_type)
        addChange("Average Type",
            QString::fromStdString(previous.average_type),
            QString::fromStdString(current.average_type));
    ...
    if (current.spread_commodity_code != previous.spread_commodity_code)
        addChange("Spread Commodity",
            QString::fromStdString(previous.spread_commodity_code),
            QString::fromStdString(current.spread_commodity_code));
    ...
    if (current.basket_json != previous.basket_json)
        addChange("Basket JSON",
            QString::fromStdString(previous.basket_json),
            QString::fromStdString(current.basket_json));
...
void CommodityInstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->commodityCodeValue->setText(
        QString::fromStdString(v.commodity_code));
    ui_->currencyValue->setText(
        QString::fromStdString(v.currency));
    ui_->quantityValue->setText(
        QString::number(v.quantity, 'f', 6));
...
void CommodityInstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CommodityInstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CommodityInstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
    int row = selected.first()->row();
    if (static_cast<std::size_t>(row) >= versions_.size()) return;
