...
ChangeReasonController::ChangeReasonController(QMainWindow* mainWindow,
                                               QMdiArea* mdiArea,
    ...
    if (clientManager_) {
        connect(clientManager_,
                ...
                this,
                &ChangeReasonController::onNotificationReceived);

        // Subscribe to events when logged in
        connect(clientManager_,
                &ClientManager::loggedIn,
                this,
                [self = QPointer<ChangeReasonController>(this)]() {
                    if (!self)
                        return;
...
void ChangeReasonController::showListWindow() {
    ...
    // Connect signals
    connect(listWindow_,
            &ChangeReasonMdiWindow::statusChanged,
            this,
            &ChangeReasonController::statusMessage);
    connect(listWindow_,
            &ChangeReasonMdiWindow::errorOccurred,
            this,
            &ChangeReasonController::errorMessage);
    connect(listWindow_,
            &ChangeReasonMdiWindow::showReasonDetails,
            this,
            &ChangeReasonController::onShowDetails);
    connect(listWindow_,
            &ChangeReasonMdiWindow::addNewRequested,
            this,
            &ChangeReasonController::onAddNewRequested);
    connect(listWindow_,
            &ChangeReasonMdiWindow::showReasonHistory,
            this,
            &ChangeReasonController::onShowHistory);
    ...
    listMdiSubWindow_->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::NoteEdit, IconUtils::DefaultIconColor));
...
void ChangeReasonController::showAddWindow() {
    ...
    connect(detailDialog,
            &ChangeReasonDetailDialog::statusMessage,
            this,
            &ChangeReasonController::statusMessage);
    connect(detailDialog,
            &ChangeReasonDetailDialog::errorMessage,
            this,
            &ChangeReasonController::errorMessage);
    connect(detailDialog,
            &ChangeReasonDetailDialog::changeReasonSaved,
            this,
...
void ChangeReasonController::showDetailWindow(const dq::domain::change_reason& reason) {
    ...
    connect(detailDialog,
            &ChangeReasonDetailDialog::changeReasonDeleted,
            this,
...
void ChangeReasonController::onNotificationReceived(const QString& eventType,
...
void ChangeReasonController::showHistoryWindow(const QString& code) {
    BOOST_LOG_SEV(lg(), info) << "Opening history window for change reason: " << code.toStdString();
    ...
    connect(historyDialog,
            &ChangeReasonHistoryDialog::statusChanged,
            this,
            [self = QPointer<ChangeReasonController>(this)](const QString& message) {
                if (!self)
                    return;
                emit self->statusMessage(message);
            });
    connect(historyDialog,
            &ChangeReasonHistoryDialog::errorOccurred,
            this,
            [self = QPointer<ChangeReasonController>(this)](const QString& message) {
                if (!self)
                    return;
    ...
    connect(historyDialog,
            &ChangeReasonHistoryDialog::revertVersionRequested,
            this,
            &ChangeReasonController::onRevertVersion);
    connect(historyDialog,
            &ChangeReasonHistoryDialog::openVersionRequested,
            this,
            &ChangeReasonController::onOpenVersion);
...
void ChangeReasonController::onOpenVersion(const dq::domain::change_reason& reason,
    ...
    detailWindow->setWindowTitle(
        QString("Change Reason: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
    ...
    connect(detailDialog,
            ...
            [self = QPointer<ChangeReasonController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("Change reason '%1' reverted successfully").arg(code));
                if (self->listWindow_) {
                    self->listWindow_->markAsStale();
                }
            });
