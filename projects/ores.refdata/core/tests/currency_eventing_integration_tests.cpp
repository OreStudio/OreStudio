...
#include "ores.refdata.api/eventing/currency_event.hpp"
...
TEST_CASE("write_currency_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::currency_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_currencies");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.iso_code == v.iso_code)
