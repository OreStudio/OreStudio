...
:END:
#+title: Task: Fix Windows MSVC export and libpq link
...
Add ORES_QT_API export annotations to the three moved Cron widgets, and fix the libpq link order (pgport, pgcommon before libpq) for MSVC.
...
- MSVC export declarations are correct and =libpq= links cleanly on Windows.
- No undefined symbol errors remain for the affected DLLs on Windows.
- PR #708 landed.
