...
ClientCatalogModel::ClientCatalogModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ClientCatalogModel::onCatalogsLoaded);
...
void ClientCatalogModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh catalog model: disconnected.";
...
void ClientCatalogModel::onCatalogsLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug)
                << "Found " << recencyTracker_.recent_count() << " catalogs newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " catalogs."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientCatalogModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
QVariant ClientCatalogModel::recency_foreground_color(const std::string& code) const {
