...
#+description: Codegen entity model for the refdata =business_unit_type= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
// no_generator_suffix: validated soft FK, a "-<idx>" suffix would never
...
|-------------------+--------------------+---------------+-----------+-------+----------+---------------------|
| Code              | code               | Code          | string    | 120   | true     | business_unit_type |
