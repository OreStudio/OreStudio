...
gitignored (machine-local, never committed). =compass story new / compass task new= appends an entry
...
=compass story new / compass task new= writes the initial =State :: STARTED= entry automatically.
...
1. *Completing a task* — append a closing entry with =State :: DONE= and
   ...
   #+end_src
2. *Switching tasks mid-session* — run =compass story new / compass task new= for the new task;
   it will append a new =STARTED= entry automatically.

** Overlap detection before picking up a story

Before running =compass story new / compass task new=, check what every other worktree is working on:
