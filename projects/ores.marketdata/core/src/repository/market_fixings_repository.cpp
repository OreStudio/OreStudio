...
void market_fixings_repository::write(context ctx, const domain::market_fixing& v) {
...
void market_fixings_repository::write(context ctx, const std::vector<domain::market_fixing>& v) {
...
market_fixings_repository::read_latest(context ctx, const boost::uuids::uuid& series_id) {
...
std::vector<domain::market_fixing>
market_fixings_repository::read_latest(context ctx,
    ...
    const auto query =
        sqlgen::read<std::vector<market_fixing_entity>> |
        where("tenant_id"_c == tid && "series_id"_c == sid && "fixing_date"_c >= from_str &&
              "fixing_date"_c <= to_str && "valid_to"_c == max.value()) |
        order_by("fixing_date"_c);
    ...
    return execute_read_query<market_fixing_entity, domain::market_fixing>(
        ...
        [](const auto& entities) { return market_fixing_mapper::map(entities); },
        lg(),
        "Reading market fixings by date range.");
}

void market_fixings_repository::remove(context ctx, const boost::uuids::uuid& series_id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing fixings for series: " << series_id;
    const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto sid = boost::uuids::to_string(series_id);
    const auto query =
        sqlgen::delete_from<market_fixing_entity> |
        where("tenant_id"_c == tid && "series_id"_c == sid && "valid_to"_c == max.value());

    execute_delete_query(ctx, query, lg(), "Removing market fixings from database.");
}
