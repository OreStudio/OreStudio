...
| H01 | pass | `validate_docs.py` passes for the component. The leftover-artefact, naming and platform-placement checks are clean. The diagram was authored in two passes. The automated pass was regenerated from a corrected generator, and the manual pass then filled the boxes it leaves empty, which is every interface and every static facade, and drew the realizations and throws edges. One edge the first manual pass carried, `net` to `time`, was removed because `network_info.cpp` does not use the time facet. A refresh returns the file byte-identically. The audit page now names the facets that survive. |
