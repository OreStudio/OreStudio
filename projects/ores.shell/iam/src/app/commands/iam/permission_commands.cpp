...
void permission_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("permissions");
    ...
    menu->Insert(
        ...
        "set <id> <code> <description> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <code> <description> <reason> <commentary>");
...
void permission_commands::process_list(std::ostream& out,
                                       nats_client& session,
    ...
    auto result = do_auth_request<messaging::list_permissions_response>(
...
void permission_commands::process_get(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::get_permission_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void permission_commands::process_get_many(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::get_many_permissions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void permission_commands::process_add(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::put_permission_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void permission_commands::process_set(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
...
void permission_commands::process_put_many(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::permission_change change;
    ...
    auto result = do_auth_request<messaging::put_many_permissions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void permission_commands::process_delete(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::delete_permission_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void permission_commands::process_delete_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::delete_many_permissions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
