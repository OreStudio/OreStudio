 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_TENOR_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_TENOR_REPOSITORY_HPP
...
/**
 * @brief Reads and writes tenors to data storage.
 */
class ORES_REFDATA_CORE_EXPORT tenor_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.tenor_repository";
...
public:
    ...
    /**
     * @brief Writes tenors to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::tenor& v);
    void write(context ctx, const std::vector<domain::tenor>& v);
    /**@}*/

    /**
     * @brief Writes a tenor, honouring the claim it states.
     *
     ...
     */
    void
    write(context ctx, const domain::tenor& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of tenors, each honouring its own
    ...
    /**
     * @brief Reads latest tenors, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::tenor> read_latest(context ctx);
    std::vector<domain::tenor> read_latest(context ctx, const std::string& code);
    std::vector<domain::tenor> read_latest(context ctx, const std::vector<std::string>& codes);
    /**@}*/
     ...
     */
    std::vector<domain::tenor> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single tenor as it stood at a specific
    ...
    /**
     * @brief Reads latest tenors with pagination support.
     * @param ctx Repository context with database connection
     * @param offset Number of records to skip
     * @param limit Maximum number of records to return
     */
    std::vector<domain::tenor> read_latest(context ctx, std::uint32_t offset, std::uint32_t limit);
     ...
     */
    std::uint32_t get_total_tenor_count(context ctx);

    /**
     * @brief Deletes a tenor by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a tenor, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes tenors by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::tenor& v);
     ...
     */
    domain::tenor apply_claim(context ctx,
