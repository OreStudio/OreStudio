...
void CurrencyPairConventionMdiWindow::setupUi() {
...
void CurrencyPairConventionMdiWindow::setupToolbar() {
    ...
    addAction_->setToolTip(tr("Add new currency pair convention"));
    connect(addAction_, &QAction::triggered, this, &CurrencyPairConventionMdiWindow::addNew);

    editAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::Edit, IconUtils::DefaultIconColor), tr("Edit"));
    editAction_->setToolTip(tr("Edit selected currency pair convention"));
    editAction_->setEnabled(false);
    connect(editAction_, &QAction::triggered, this, &CurrencyPairConventionMdiWindow::editSelected);

    deleteAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::Delete, IconUtils::DefaultIconColor), tr("Delete"));
    deleteAction_->setToolTip(tr("Delete selected currency pair convention"));
    deleteAction_->setEnabled(false);
    connect(
        deleteAction_, &QAction::triggered, this, &CurrencyPairConventionMdiWindow::deleteSelected);

    historyAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor), tr("History"));
    historyAction_->setToolTip(tr("View currency pair convention history"));
...
void CurrencyPairConventionMdiWindow::setupConnections() {
    connect(model_,
            &ClientCurrencyPairConventionModel::dataLoaded,
            this,
            &CurrencyPairConventionMdiWindow::onDataLoaded);
    connect(model_,
            &ClientCurrencyPairConventionModel::loadError,
            this,
            &CurrencyPairConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &CurrencyPairConventionMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &CurrencyPairConventionMdiWindow::onDoubleClicked);
...
void CurrencyPairConventionMdiWindow::doReload() {
...
void CurrencyPairConventionMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 currency pair conventions").arg(loaded).arg(total));
...
void CurrencyPairConventionMdiWindow::onLoadError(const QString& error_message,
...
void CurrencyPairConventionMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void CurrencyPairConventionMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void CurrencyPairConventionMdiWindow::updateActionStates() {
...
void CurrencyPairConventionMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new currency pair convention requested";
    emit addNewRequested();
}

void CurrencyPairConventionMdiWindow::editSelected() {
...
void CurrencyPairConventionMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (auto* convention = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug)
            << "Emitting showConventionHistory for code: " << convention->pair_code;
        emit showConventionHistory(*convention);
    }
}

void CurrencyPairConventionMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete currency pair convention while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* convention = model_->getConvention(sourceIndex.row())) {
            codes.push_back(convention->pair_code);
        }
    }

    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid currency pair conventions to delete";
        return;
    }
    ...
    if (codes.size() == 1) {
        confirmMessage = QString("Are you sure you want to delete currency pair convention '%1'?")
                             .arg(QString::fromStdString(codes.front()));
    } else {
        confirmMessage = QString("Are you sure you want to delete %1 currency pair conventions?")
                             .arg(codes.size());
    }
    ...
    QPointer<CurrencyPairConventionMdiWindow> self = this;
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << codes.size() << " currency pair conventions";

        refdata::messaging::delete_currency_pair_convention_request request;
        request.pair_codes = codes;
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                BOOST_LOG_SEV(lg(), debug) << "Currency Pair Convention deleted: " << code;
                success_count++;
                emit self->conventionDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error) << "Currency Pair Convention deletion failed: " << code
                                           << " - " << result.second;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg =
                success_count == 1 ?
                    "Successfully deleted 1 currency pair convention" :
                    QString("Successfully deleted %1 currency pair conventions").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "currency pair convention" :
                                                        "currency pair conventions")
                              .arg(first_error);
