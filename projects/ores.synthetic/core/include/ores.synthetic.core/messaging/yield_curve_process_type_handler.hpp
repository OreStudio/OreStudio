...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        get_yield_curve_process_types_response resp;
        if (auto req = decode<get_yield_curve_process_types_request>(msg)) {
            try {
                resp.process_types = svc.list_process_types(req->offset, req->limit);
                resp.total_available_count = static_cast<int>(svc.count_process_types());
                resp.success = true;
        ...
        BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), debug) << "Completed " << msg.subject;
    ...
    void save(ores::nats::message msg) {
        ...
        service::yield_curve_process_type_service svc(req_ctx);
        if (auto req = decode<save_yield_curve_process_type_request>(msg)) {
            try {
                svc.save_process_type(req->data);
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg, save_yield_curve_process_type_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(
                    nats_,
                    msg,
                    save_yield_curve_process_type_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::yield_curve_process_type_service svc(req_ctx);
        if (auto req = decode<get_yield_curve_process_type_history_request>(msg)) {
            try {
                auto hist = svc.get_process_type_history(req->code);
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_,
                      msg,
                      get_yield_curve_process_type_history_response{.history = std::move(hist),
                                                                    .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_yield_curve_process_type_history_response{.success = false,
                                                                    .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), warn)
    ...
    void remove(ores::nats::message msg) {
        ...
        service::yield_curve_process_type_service svc(req_ctx);
        if (auto req = decode<delete_yield_curve_process_type_request>(msg)) {
            try {
                svc.delete_process_types(req->codes);
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg, delete_yield_curve_process_type_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      delete_yield_curve_process_type_response{.success = false,
                                                               .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(yield_curve_process_type_handler_lg(), warn)
