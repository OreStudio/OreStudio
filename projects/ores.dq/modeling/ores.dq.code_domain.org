...
** Qt
:PROPERTIES:
:domain_include:         ores.dq.api/domain/code_domain.hpp
:domain_class:           dq::domain::code_domain
:protocol_include:       ores.dq.api/messaging/code_domain_protocol.hpp
...
The live =CodeDomainDetailDialog= (=projects/ores.qt/data_management=) is
itself hand-maintained, not generated from this model -- there is a
separate, unused generated scaffold under =projects/ores.qt/dq= left over
from an earlier component layout (tracked as drift; see the codegen
drift-catalog story). Paste blocks don't apply here since the live file
isn't tangled/generated. Instead, a read-only "Badge Mappings" tab
(showing every entity_code -> badge_definition mapping registered for
the displayed code_domain) is wired directly into
=projects/ores.qt/data_management/src/CodeDomainDetailDialog.cpp= by
hand, mirroring the paste-block pattern used by =PartyHierarchyTab= in
shape (see =BadgeMappingsTab=) even though no actual paste block is
involved.
