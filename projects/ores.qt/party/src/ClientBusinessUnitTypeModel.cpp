...
void ClientBusinessUnitTypeModel::fetch_types(std::uint32_t offset, std::uint32_t limit) {
    is_fetching_ = true;
            ...
            [&]() -> FetchResult {
                ...
                if (!result) {
                    BOOST_LOG_SEV(lg(), error)
                        << "Failed to fetch business unit types: " << result.error();
                    return {.success = false,
                            .types = {},
                            .total_available_count = 0,
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << result->business_unit_types.size() << " business unit types";
