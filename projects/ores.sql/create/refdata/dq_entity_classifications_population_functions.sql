 ...
 *   SELECT * FROM ores_dq_preview_entity_classification_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_entity_classifications(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_entity_classification_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_entity_classifications_tbl existing
...
/**
 * Populate ores_refdata_entity_classifications_tbl from a DQ entity_classifications dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_entity_classifications(
    p_dataset_id uuid,
