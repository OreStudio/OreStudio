 ...
 */
std::vector<domain::rounding_type> read_one(repository::rounding_type_repository& repo,
                                            const ores::database::context& ctx,
                                            const messaging::rounding_type_key& key) {
 ...
 */
messaging::rounding_type_key key_from(const domain::rounding_type& v) {
    messaging::rounding_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::rounding_type to_domain(const messaging::rounding_type_write& write) {
    domain::rounding_type v;
    v.code = write.code;
    v.name = write.name;
...
rounding_type_service::list_rounding_types(const messaging::list_rounding_types_request& request) {
    messaging::list_rounding_types_response response;
...
messaging::get_rounding_type_response
rounding_type_service::get_rounding_type(const messaging::get_rounding_type_request& request) {
    messaging::get_rounding_type_response response;
    ...
    response.rounding_type = std::move(found.front());
    return response;
}

messaging::get_many_rounding_types_response rounding_type_service::get_many_rounding_types(
    const messaging::get_many_rounding_types_request& request) {
    messaging::get_many_rounding_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::rounding_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.rounding_type = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_rounding_type_response
rounding_type_service::put_rounding_type(const messaging::put_rounding_type_request& request) {
    messaging::put_rounding_type_response response;
    domain::rounding_type value;
    ...
    if (!written.empty())
        response.rounding_type = std::move(written.front());
    return response;
}

messaging::put_many_rounding_types_response rounding_type_service::put_many_rounding_types(
    const messaging::put_many_rounding_types_request& request) {
    messaging::put_many_rounding_types_response response;
    std::vector<domain::rounding_type> batch;
...
messaging::delete_rounding_type_response rounding_type_service::delete_rounding_type(
    const messaging::delete_rounding_type_request& request) {
    messaging::delete_rounding_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::rounding_type_repository::remove_status::removed:
            break;
        case repository::rounding_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::rounding_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::rounding_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_rounding_types_response rounding_type_service::delete_many_rounding_types(
    const messaging::delete_many_rounding_types_request& request) {
    messaging::delete_many_rounding_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_rounding_type_versions_response rounding_type_service::list_rounding_type_versions(
    const messaging::list_rounding_type_versions_request& request) {
    messaging::list_rounding_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_rounding_type_version_response rounding_type_service::get_rounding_type_version(
    const messaging::get_rounding_type_version_request& request) {
    messaging::get_rounding_type_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.rounding_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
rounding_type_service::prepare_change(const messaging::rounding_type_change& change,
                                      const ores::utility::domain::change_intent& intent,
                                      domain::rounding_type& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
rounding_type_service::get_types(const std::vector<std::string>& codes) {
