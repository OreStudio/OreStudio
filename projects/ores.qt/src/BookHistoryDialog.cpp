...
void BookHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &BookHistoryDialog::onVersionSelected);
...
void BookHistoryDialog::loadHistory() {
    ...
    auto task = [self, id = id_]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_book_history_request,
            0, std::move(payload)
        );
        ...
        auto response = refdata::messaging::get_book_history_response::
