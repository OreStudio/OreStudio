...
create unique index if not exists party_id_schemes_code_uniq_idx
on "ores_refdata_party_id_schemes_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists party_id_schemes_tenant_idx
...
-- Validates that a code exists in the party_id_schemes table.
-- Returns the validated value, or default if null/empty.
-- Uses system tenant data (shared reference data).
-- =============================================================================
create or replace function ores_refdata_validate_party_id_scheme_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid party_id_scheme: value cannot be null or empty'
            using errcode = '23502';
    end if;
    ...
    if not exists (
        ...
        raise exception 'Invalid party_id_scheme: %. Must be one of: %', p_value, (
