...
publication_repository::read_by_dataset(const boost::uuids::uuid& dataset_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading publications for dataset: " << dataset_id;

    const auto dataset_id_str = boost::uuids::to_string(dataset_id);
    const auto query = sqlgen::read<std::vector<publication_entity>> |
                       where("dataset_id"_c == dataset_id_str) | order_by("published_at"_c.desc());
    ...
    return execute_read_query<publication_entity, domain::publication>(
        ...
        lg(),
        "Reading publications by dataset");
}

std::vector<domain::publication> publication_repository::read_recent(std::uint32_t limit) {
...
void publication_repository::insert(const domain::publication& pub) {
    BOOST_LOG_SEV(lg(), debug) << "Inserting publication record for dataset: " << pub.dataset_code;
    ...
    if (!result) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to insert publication record: " << result.error().what();
        throw std::runtime_error("Failed to insert publication record");
    }

    BOOST_LOG_SEV(lg(), info) << "Inserted publication record for dataset: " << pub.dataset_code;
}
