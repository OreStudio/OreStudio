...
3. [[id:F3010EA7-6A0E-405C-81FA-1EF051749B5E][External market data identifiers]] — RIC, Bloomberg, ORE canonical key comparative analysis.
...
All five knowledge documents delivered and merged via PR #1308.  Scaffold task closes
alongside F8562D22 (inventory + architecture), which delivered the actual documents.
Architecture decisions recorded: =observation_date= → =observation_datetime= migration
required as a prerequisite; =IFxSpotFeed= coupling deferred to post-PoC (DQ8);
12 generator process types defined.  Story 8A65240D wired into sprint under
=*** Market data generation=; knowledge.org index updated with all five document IDs.
