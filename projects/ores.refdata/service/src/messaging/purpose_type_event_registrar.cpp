...
register_purpose_type_event_mapping(ev::service::postgres_event_source& event_source,
                                    ev::service::event_bus& event_bus,
                                    ores::nats::service::client& nats) {
    // The trigger publishes on the table's own channel, and the mapping turns
    // what it says into this entity's event.
    event_source.register_entity_event_mapping<refdata::messaging::purpose_type_event>(
        "ores_refdata_purpose_types");

    return event_bus.subscribe<refdata::messaging::purpose_type_event>(
        [&nats](const refdata::messaging::purpose_type_event& e) {
            ...
            ev::service::publish_entity_event(
                nats,
                ev::domain::event_subject<refdata::messaging::purpose_type_event>(e.action),
                e);
        });
}
