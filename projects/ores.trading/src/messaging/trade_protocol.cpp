...
save_trade_request
save_trade_request::from(domain::trade trade) {
    return save_trade_request{std::vector<domain::trade>{std::move(trade)}};
}

save_trade_request
save_trade_request::from(std::vector<domain::trade> trades) {
    return save_trade_request{std::move(trades)};
}
...
save_trade_request::deserialize(std::span<const std::byte> data) {
    ...
    request.trades.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_trade(data);
        if (!e) return std::unexpected(e.error());
        request.trades.push_back(std::move(*e));
    }
    return request;
}
