...
#include "ores.iam.api/eventing/tenant_status_event.hpp"
...
TEST_CASE("write_tenant_status_publishes_an_event", tags) {
    ...
    using event_type = ores::iam::messaging::tenant_status_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_iam_tenant_statuses");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.status == v.status)
