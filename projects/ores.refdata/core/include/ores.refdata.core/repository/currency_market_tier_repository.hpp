...
/**
 * @brief Reads and writes currency market tiers to data storage.
 */
class ORES_REFDATA_CORE_EXPORT currency_market_tier_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.currency_market_tier_repository";
...
public:
    ...
    /**
     * @brief Writes currency market tiers to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::currency_market_tier& v);
    void write(context ctx, const std::vector<domain::currency_market_tier>& v);
    /**@}*/

    /**
     * @brief Writes a currency market tier, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::currency_market_tier& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of currency market tiers, each honouring its own
    ...
    /**
     * @brief Reads latest currency market tiers, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads a single currency market tier as it stood at a specific
    ...
    /**
     * @brief Deletes a currency market tier by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a currency market tier, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes currency market tiers by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::currency_market_tier apply_claim(context ctx,
                                             const domain::currency_market_tier& v,
                                             const ores::utility::domain::precondition& claim);
};
