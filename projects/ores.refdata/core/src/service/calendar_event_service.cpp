...
std::vector<domain::calendar_event> read_one(repository::calendar_event_repository& repo,
                                             ...
                                             const messaging::calendar_event_key& key) {
 ...
 */
messaging::calendar_event_key key_from(const domain::calendar_event& v) {
 ...
 */
domain::calendar_event to_domain(const messaging::calendar_event_write& write) {
    domain::calendar_event v;
    v.id = write.id;
    v.calendar_code = write.calendar_code;
    v.event_date = write.event_date;
...
messaging::list_by_calendar_code_calendar_events_response
...
calendar_event_service::get_calendar_event(const messaging::get_calendar_event_request& request) {
    messaging::get_calendar_event_response response;
    ...
    response.calendar_event = std::move(found.front());
    return response;
}

messaging::get_many_calendar_events_response calendar_event_service::get_many_calendar_events(
    const messaging::get_many_calendar_events_request& request) {
    messaging::get_many_calendar_events_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::calendar_event_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.calendar_event = std::move(found.front());
...
calendar_event_service::put_calendar_event(const messaging::put_calendar_event_request& request) {
    messaging::put_calendar_event_response response;
    domain::calendar_event value;
    ...
    if (!written.empty())
        response.calendar_event = std::move(written.front());
    return response;
}

messaging::put_many_calendar_events_response calendar_event_service::put_many_calendar_events(
    const messaging::put_many_calendar_events_request& request) {
    messaging::put_many_calendar_events_response response;
    ...
    response.calendar_events.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.calendar_events.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_calendar_event_response calendar_event_service::delete_calendar_event(
    const messaging::delete_calendar_event_request& request) {
    messaging::delete_calendar_event_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::calendar_event_repository::remove_status::removed:
            break;
        case repository::calendar_event_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::calendar_event_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::calendar_event_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_calendar_events_response calendar_event_service::delete_many_calendar_events(
    const messaging::delete_many_calendar_events_request& request) {
    messaging::delete_many_calendar_events_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
calendar_event_service::list_calendar_event_versions(
    const messaging::list_calendar_event_versions_request& request) {
    messaging::list_calendar_event_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_calendar_event_version_response calendar_event_service::get_calendar_event_version(
    const messaging::get_calendar_event_version_request& request) {
    messaging::get_calendar_event_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.calendar_event.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
calendar_event_service::prepare_change(const messaging::calendar_event_change& change,
...
std::vector<domain::calendar_event>
calendar_event_service::get_calendar_events(const std::vector<std::string>& ids) {
