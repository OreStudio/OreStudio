...
on "ores"."assets_tags_tbl" (tag_id, version)
where valid_to = ores.utility_infinity_timestamp_fn();

create unique index if not exists assets_tags_name_uniq_idx
on "ores"."assets_tags_tbl" (name)
where valid_to = ores.utility_infinity_timestamp_fn();

create or replace function ores.assets_tags_insert_fn()
returns trigger as $$
declare
...
before insert on "ores"."assets_tags_tbl"
for each row
execute function ores.assets_tags_insert_fn();

create or replace rule assets_tags_delete_rule as
on delete to "ores"."assets_tags_tbl"
do instead
  update "ores"."assets_tags_tbl"
  set valid_to = current_timestamp
