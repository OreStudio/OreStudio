...
create unique index if not exists currency_market_tiers_name_uniq_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id, name)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists currency_market_tiers_version_uniq_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists currency_market_tiers_code_uniq_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists currency_market_tiers_tenant_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_currency_market_tiers_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_refdata_currency_market_tiers_tbl"
...
for each row execute function ores_refdata_currency_market_tiers_insert_fn();

create or replace rule ores_refdata_currency_market_tiers_delete_rule as
on delete to "ores_refdata_currency_market_tiers_tbl" do instead (
    update "ores_refdata_currency_market_tiers_tbl"
    set valid_to = clock_timestamp()
