...
values
    ...
    (ores_utility_system_tenant_id_fn(), 'SPOT', 0, 'Spot',
     ...
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    -- FOMC meeting tenors (story: FOMC-dated OIS short end): SPECIAL, since
    -- the duration is fixed by the meeting schedule, not by this table; the
    -- multiplier is the meeting ordinal (the n in nF).
    (ores_utility_system_tenant_id_fn(), '1F', 0, 'First FOMC Meeting',
     'Resolves to the first FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 46, 'SPECIAL', 'NONE', 1,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '2F', 0, 'Second FOMC Meeting',
     'Resolves to the second FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 47, 'SPECIAL', 'NONE', 2,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '3F', 0, 'Third FOMC Meeting',
     'Resolves to the third FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 48, 'SPECIAL', 'NONE', 3,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '4F', 0, 'Fourth FOMC Meeting',
     'Resolves to the fourth FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 49, 'SPECIAL', 'NONE', 4,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '5F', 0, 'Fifth FOMC Meeting',
     'Resolves to the fifth FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 50, 'SPECIAL', 'NONE', 5,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '6F', 0, 'Sixth FOMC Meeting',
     'Resolves to the sixth FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 51, 'SPECIAL', 'NONE', 6,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '7F', 0, 'Seventh FOMC Meeting',
     'Resolves to the seventh FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 52, 'SPECIAL', 'NONE', 7,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '8F', 0, 'Eighth FOMC Meeting',
     'Resolves to the eighth FOMC meeting on-or-after spot (central_bank_meeting events on US.FOMC).', 53, 'SPECIAL', 'NONE', 8,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
    (ores_utility_system_tenant_id_fn(), '1Y', 0, 'One Year',
     'Annual pillar; the swap-grid label for the one-year point (12M is the money-market label for the same duration). The FOMC segment split tenor: the boundary where the meeting-dated pillars hand over to the continuous swap grid.', 54, 'PERIOD', 'YEAR', 1,
     current_user, current_user, 'system.initial_load', 'Initial population of tenors'),
...
-- Summary
select 'refdata_tenors' as entity, count(*) as count
from ores_refdata_tenors_tbl
where valid_to = ores_utility_infinity_timestamp_fn();
