...
:END:
#+title: Gaps: country against the specification (superseded)
#+description: Superseded audit of the country entity screens against the ORE Studio web entity specification. The audit and the screens it audited are gone; kept as a record.
#+type: knowledge
#+level: cross
...
*This page is superseded. Do not act on its findings.*

Every finding below audits the entity screens and their shared machinery: the
list, detail and history pages, =DataTable.tsx=, =FieldControl.tsx=, the generated
UI metadata, and =scripts/verify-country.ts=. All of those are deleted. The
package now holds a shell and a transport, and no screens.

The findings cannot be closed and cannot be reproduced, so this page is a record
of an audit rather than a work list. Read
=component_specification.org= for what the component specifies today.

The original audit follows, unchanged, as history.
...
/The status column is frozen./ The file names, the line references and the
=verify-country.ts= checks below describe the tree as it stood on 2026-09-19. None
of them resolve now, and no status can be re-checked. Do not attempt to fix a
finding or to reproduce it.
...
There is nothing to do. This section is kept with the audit as part of the record.
