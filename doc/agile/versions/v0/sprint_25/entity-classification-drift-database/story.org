...
#+description: Run the compass-codegen-fix-drift method on ores.database: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the drift gate to cover database-cpp. Supersedes the ad hoc 'Apply safe drift to database-cpp' task.
#+type: story
#+level: s2
...
- database-cpp joins the =KNOWN_DRIFT_FREE= registry in check_component_drift.py (skill step 8), and the compass-pr-raise skill's local codegen-drift check runs =--all= over the registry, so future PRs touching ores.database fail on any regeneration drift.
