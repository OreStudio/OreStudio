...
QString ReportDefinitionDetailDialog::code() const {
    return QString::fromStdString(definition_.name);
}
...
void ReportDefinitionDetailDialog::setupConnections() {
    ...
    connect(ui_->reportTypeEdit,
            &QLineEdit::textChanged,
            this,
            &ReportDefinitionDetailDialog::onFieldChanged);
    connect(ui_->scheduleExpressionEdit,
            &QLineEdit::textChanged,
            this,
            &ReportDefinitionDetailDialog::onFieldChanged);
    connect(ui_->concurrencyPolicyEdit,
            &QLineEdit::textChanged,
            this,
            &ReportDefinitionDetailDialog::onFieldChanged);
}
...
void ReportDefinitionDetailDialog::markDirty() {
    hasChanges_ = true;
    updateSaveButtonState();
}
...
bool ReportDefinitionDetailDialog::validateInput() {
    ...
    return true && !name_val.isEmpty() && !report_type_val.isEmpty() &&
           !schedule_expression_val.isEmpty() && !concurrency_policy_val.isEmpty();
}
...
void ReportDefinitionDetailDialog::onDeleteClicked() {
    ...
    auto task = [self, id_str = boost::uuids::to_string(definition_.id)]() -> DeleteResult {
        if (!self || !self->clientManager_) {
            return {false, "Dialog closed"};
        }
