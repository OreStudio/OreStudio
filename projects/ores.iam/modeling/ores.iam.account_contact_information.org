...
The account table is hand-authored (account is not a codegen-bound
entity), so the eventing test cannot synthesize an account parent row
through the generated machinery. The =parent_seed_includes= block
names the generator and repository headers the snippet needs; the
=parent_seed= block is the seeding code the generated eventing
integration test emits verbatim: it writes a synthetic account through
the hand-authored account generator and repository, whose write API
predates the context-carrying form the auto-seeding blocks use.
