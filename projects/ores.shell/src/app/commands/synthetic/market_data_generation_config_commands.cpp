...
#include "ores.synthetic.api/domain/market_data_generation_config_table_io.hpp" // IWYU pragma: keep.
...
std::optional<domain::scope> parse_scope(const std::string& value) {
    if (value == "system")
        return domain::scope::system;
    if (value == "tenant")
        return domain::scope::tenant;
    if (value == "party")
        return domain::scope::party;
    return std::nullopt;
}

std::optional<domain::binding_mode> parse_binding_mode(const std::string& value) {
    if (value == "bound")
        return domain::binding_mode::bound;
    if (value == "sandboxed")
        return domain::binding_mode::sandboxed;
    return std::nullopt;
}
...
void market_data_generation_config_commands::process_add_market_data_generation_config(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add a market_data_generation_config." << std::endl;
        return;
    }
    ...
    // scope (enum:scope).
    const auto e_scope = parse_scope(s_scope);
    if (!e_scope) {
        fail(out) << "Invalid scope: " << s_scope << std::endl;
        return;
    }
    v.scope = *e_scope;

    // binding_mode (enum:binding_mode).
    const auto e_binding_mode = parse_binding_mode(s_binding_mode);
    if (!e_binding_mode) {
        fail(out) << "Invalid binding_mode: " << s_binding_mode << std::endl;
        return;
    }
    v.binding_mode = *e_binding_mode;
    ...
    // dataset_id (uuid_opt).
    if (!s_dataset_id.empty()) {
        const auto u_dataset_id = parse_uuid(s_dataset_id);
        if (!u_dataset_id) {
            fail(out) << "Invalid dataset_id: " << s_dataset_id << std::endl;
            return;
        }
        v.dataset_id = *u_dataset_id;
    }
    ...
    if (result->success) {
        BOOST_LOG_SEV(lg(), info) << "Successfully added market_data_generation_config.";
        out << "✓ Market Data Generation Config added successfully!" << std::endl;
    } else {
        const auto& msg = result->message.empty() ? "Unknown error" : result->message;
        BOOST_LOG_SEV(lg(), warn) << "Failed to add market_data_generation_config: " << msg;
        fail(out) << "Failed to add market_data_generation_config: " << msg << std::endl;
    }
}
...
void market_data_generation_config_commands::process_get_market_data_generation_config_history(
    ...
    if (result->history.empty()) {
        out << "No history found for this market_data_generation_config." << std::endl;
        return;
    }
