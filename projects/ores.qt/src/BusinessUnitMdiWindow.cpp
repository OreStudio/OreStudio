...
BusinessUnitMdiWindow::BusinessUnitMdiWindow(
...
QSize BusinessUnitMdiWindow::sizeHint() const {
    return {900, 400};
}
...
void BusinessUnitMdiWindow::setupToolbar() {
    ...
    connect(editAction_, &QAction::triggered, this,
            &BusinessUnitMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this,
            &BusinessUnitMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this,
            &BusinessUnitMdiWindow::viewHistorySelected);
}
...
void BusinessUnitMdiWindow::setupConnections() {
    connect(model_, &ClientBusinessUnitModel::dataLoaded,
    ...
    connect(tableView_, &QTableView::doubleClicked,
            this, &BusinessUnitMdiWindow::onDoubleClicked);
...
void BusinessUnitMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* business_unit = model_->getUnit(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showUnitHistory for code: "
...
void BusinessUnitMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(this, "Disconnected",
            "Cannot delete business unit while disconnected.");
        return;
    }
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage = QString("Are you sure you want to delete %1 business units?")
            .arg(ids.size());
    }
...
void BusinessUnitMdiWindow::setupColumnVisibility() {
    QHeaderView* header = tableView_->horizontalHeader();

    // Enable context menu on header for column visibility
    header->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(header, &QHeaderView::customContextMenuRequested,
            this, &BusinessUnitMdiWindow::showHeaderContextMenu);

    // Save header state when sections are moved or resized
    connect(header, &QHeaderView::sectionMoved, this,
            &BusinessUnitMdiWindow::saveSettings);
    connect(header, &QHeaderView::sectionResized, this,
            &BusinessUnitMdiWindow::saveSettings);
}

void BusinessUnitMdiWindow::showHeaderContextMenu(const QPoint& pos) {
    QHeaderView* header = tableView_->horizontalHeader();
    QMenu menu(this);
...
void BusinessUnitMdiWindow::saveSettings() {
    QSettings settings("OreStudio", "OreStudio");
    settings.beginGroup("BusinessUnitListWindow");
...
void BusinessUnitMdiWindow::restoreSettings() {
    QSettings settings("OreStudio", "OreStudio");
    settings.beginGroup("BusinessUnitListWindow");
