...
void account_party_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("account_parties");
    ...
    menu->Insert(
        ...
        "add <account_id> <party_id> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <account_id> <party_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <account_id> <party_id> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <account_id> <party_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <account_id> <party_id> <reason> <commentary>");
...
void account_party_commands::process_list(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_account_parties_request;
    ...
    auto result = do_auth_request<messaging::list_account_parties_response>(
...
void account_party_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_account_party_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.account_id, parsed->positionals[next++], "account_id");
    ...
    auto result = do_auth_request<messaging::get_account_party_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_account_parties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            messaging::account_party_key key;
            read_token(key.account_id, parsed->positionals[i + 0], "account_id");
            read_token(key.party_id, parsed->positionals[i + 1], "party_id");
    ...
    auto result = do_auth_request<messaging::get_many_account_parties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_add(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_account_party_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_account_party_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_set(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
...
void account_party_commands::process_put_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_account_parties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::account_party_change change;
    ...
    auto result = do_auth_request<messaging::put_many_account_parties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_delete(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_account_party_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.account_id, parsed->positionals[next++], "account_id");
    ...
    auto result = do_auth_request<messaging::delete_account_party_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_delete_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_account_parties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::account_party_key key;
            read_token(key.account_id, parsed->positionals[i * 2 + 0], "account_id");
            read_token(key.party_id, parsed->positionals[i * 2 + 1], "party_id");
            req.removals.push_back(messaging::account_party_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_account_parties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void account_party_commands::process_by_account_id(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-account-id request.";

    using request_type = messaging::list_by_account_id_account_parties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_by_account_id_account_parties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
