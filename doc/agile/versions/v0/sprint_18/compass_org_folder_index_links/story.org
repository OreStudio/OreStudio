...
Every folder entry in [[proj:doc/compass.org][=doc/compass.org=]] links to its index file via an
...
- Every =**= and =***= folder section in [[proj:doc/compass.org][=doc/compass.org=]] contains a
  prominent org-roam link to the folder's index file.
- Folders that were missing an index file have one: [[proj:doc/meta][=meta/=]], [[proj:doc/analysis][=analysis/=]],
  [[proj:doc/investigations][=investigations/=]], [[proj:doc/prompts][=prompts/=]], [[proj:doc/samples][=samples/=]].
- [[proj:doc/investigations][=investigations/=]] is added to the folder layout tree in =compass.org=.
...
- /One "See X for the full index." sentence at the top of each section./
  Consistent with the pattern already used in [[proj:doc/llm][=llm/=]]. Avoids duplicating
...
- Retroactive: existing files without org-roam IDs in [[proj:doc/analysis][=analysis/=]] are not
