...
;; This runs as `emacs -Q --batch' with package-user-dir pointed at ./.packages.
;; When `package-quickstart' is nil Emacs deletes the *user's* quickstart cache
;; under ~/.emacs.d after every install, and `delete-file' signals on a
;; read-only HOME, so the install aborts in a sandbox before it finishes. That
;; cache describes the user's own package dir, not this project-local one, so a
;; batch install must leave it alone.
