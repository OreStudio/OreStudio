...
std::string calendar_type_repository::sql() {
    return generate_create_table_sql<calendar_type_entity>(lg());
}

ores::utility::domain::precondition
calendar_type_repository::replace_claim(context ctx, const domain::calendar_type& v) {
...
domain::calendar_type calendar_type_repository::apply_claim(
    context ctx, const domain::calendar_type& v, const ores::utility::domain::precondition& claim) {
...
void calendar_type_repository::write(context ctx, const std::vector<domain::calendar_type>& v) {
...
void calendar_type_repository::write(context ctx,
                                     const domain::calendar_type& v,
                                     const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing calendar type. " << "code: " << v.code;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, calendar_type_mapper::map(t), lg(), "Writing calendar type to database.");
}

void calendar_type_repository::write(
    context ctx,
    const std::vector<domain::calendar_type>& v,
    const std::vector<ores::utility::domain::precondition>& claims) {
    BOOST_LOG_SEV(lg(), debug) << "Writing calendar types. Count: " << v.size();
    std::vector<domain::calendar_type> batch;
    batch.reserve(v.size());
    for (std::size_t i = 0; i < v.size(); ++i)
        batch.push_back(apply_claim(ctx, v[i], claims[i]));
    execute_write_query(
        ctx, calendar_type_mapper::map(batch), lg(), "Writing calendar types to database.");
}
...
std::vector<domain::calendar_type> calendar_type_repository::read_latest(context ctx,
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::read<std::vector<calendar_type_entity>> |
...
std::vector<domain::calendar_type> calendar_type_repository::read_all(context ctx,
    ...
    return execute_read_query<calendar_type_entity, domain::calendar_type>(
        ...
        [](const auto& entities) { return calendar_type_mapper::map(entities); },
        lg(),
        "Reading all calendar type versions by code.");
}
...
std::optional<domain::calendar_type> calendar_type_repository::read_at_version(
    ...
    const auto entities = execute_read_query<calendar_type_entity, domain::calendar_type>(
        ctx,
        query,
        [](const auto& entities) { return calendar_type_mapper::map(entities); },
        lg(),
        "Reading calendar type at version.");
...
calendar_type_repository::remove_status calendar_type_repository::remove(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<calendar_type_entity> |
                       where("tenant_id"_c == tid && "code"_c == code &&
                             "valid_to"_c == max.value() && "version"_c == expected);

    execute_delete_query(ctx, query, lg(), "Removing calendar type from database.");
...
calendar_type_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
...
std::uint32_t calendar_type_repository::get_total_type_count(context ctx) {
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::select_from<calendar_type_entity>(sqlgen::count().as<"count">()) |
                       where("tenant_id"_c == tid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active calendar type count: " << count;
    return count;
}

std::vector<domain::calendar_type>
calendar_type_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<calendar_type_entity, domain::calendar_type>(
        ctx,
        query,
        [](const auto& entities) { return calendar_type_mapper::map(entities); },
        lg(),
        "Reading latest calendar types by ids.");
    return result;
}

void calendar_type_repository::remove(context ctx, const std::vector<std::string>& codes) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::delete_from<calendar_type_entity> |
        where("tenant_id"_c == tid && "code"_c.in(codes) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing calendar types.");
}
