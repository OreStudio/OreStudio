...
through the generated machinery. The =parent_seed= block below is the
seeding code the generated eventing integration test emits verbatim:
it writes a synthetic account through the hand-authored account
generator and repository, whose write API predates the
context-carrying form the auto-seeding blocks use.
