...
result_mapper::map(const result_entity& v) {
    ...
    r.host_id = v.host_id.has_value() ? std::optional(boost::lexical_cast<boost::uuids::uuid>(*v.host_id)) : std::nullopt;
    ...
    r.received_at = v.received_at.value_or("");
...
result_mapper::map(const domain::result& v) {
    ...
    r.host_id = v.host_id.has_value() ? std::optional(boost::uuids::to_string(*v.host_id)) : std::nullopt;
    r.pgmq_msg_id = v.pgmq_msg_id.empty() ? std::nullopt : std::optional(v.pgmq_msg_id);
    r.server_state = v.server_state;
    r.outcome = v.outcome.empty() ? std::nullopt : std::optional(v.outcome);
    r.output_uri = v.output_uri.empty() ? std::nullopt : std::optional(v.output_uri);
    r.received_at = v.received_at.empty() ? std::nullopt : std::optional(v.received_at);
    r.modified_by = v.modified_by;
    r.performed_by = v.performed_by;
    r.change_reason_code = v.change_reason_code;
    r.change_commentary = v.change_commentary;
