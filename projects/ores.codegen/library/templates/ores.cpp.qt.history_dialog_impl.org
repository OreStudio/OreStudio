...
:END:
#+title: ores.cpp.qt.history_dialog_impl
#+description: Read-only history view over temporal rows.
#+type: archetype
#+output: projects/ores.qt/{component}/src/{EntityPascal}HistoryDialog.cpp
#+facet: ores.cpp.qt
#+default: disabled
#+level: cross
...
{{^is_uuid}}
{{#is_double}}
    checkDouble(diffs, tr("{{label}}"), curr.{{field}}, prev.{{field}});
{{/is_double}}
{{^is_double}}
    checkString(diffs, tr("{{label}}"), curr.{{field}}, prev.{{field}});
{{/is_double}}
