...
pricing_model_config_repository::replace_claim(context ctx, const domain::pricing_model_config& v) {
...
pricing_model_config_repository::apply_claim(context ctx,
...
void pricing_model_config_repository::write(context ctx,
    ...
    execute_write_query(ctx,
                        pricing_model_config_mapper::map(t),
                        lg(),
...
void pricing_model_config_repository::write(
    ...
    execute_write_query(ctx,
                        pricing_model_config_mapper::map(batch),
                        lg(),
...
std::vector<domain::pricing_model_config>
pricing_model_config_repository::read_any_by_name(context ctx, const std::string& name) {
    BOOST_LOG_SEV(lg(), debug) << "Reading any pricing model configuration by name: " << name;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<pricing_model_config_entity>> |
    ...
    return execute_read_query<pricing_model_config_entity, domain::pricing_model_config>(
        ...
        [](const auto& entities) { return pricing_model_config_mapper::map(entities); },
        lg(),
        "Reading any pricing model configuration by name.");
}
...
pricing_model_config_repository::remove_status pricing_model_config_repository::remove(
...
std::vector<domain::pricing_model_config>
pricing_model_config_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<pricing_model_config_entity, domain::pricing_model_config>(
        ctx,
        query,
        [](const auto& entities) { return pricing_model_config_mapper::map(entities); },
        lg(),
        "Reading latest pricing model configurations by ids.");
    return result;
}
