...
read_one(repository::party_contact_information_repository& repo,
         const ores::database::context& ctx,
         const messaging::party_contact_information_key& key) {
 ...
 */
messaging::party_contact_information_key key_from(const domain::party_contact_information& v) {
...
party_contact_information_service::get_party_contact_information(
    const messaging::get_party_contact_information_request& request) {
...
messaging::get_many_party_contact_informations_response
party_contact_information_service::get_many_party_contact_informations(
    const messaging::get_many_party_contact_informations_request& request) {
    messaging::get_many_party_contact_informations_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_contact_information_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party_contact_information = std::move(found.front());
...
party_contact_information_service::put_party_contact_information(
    const messaging::put_party_contact_information_request& request) {
    messaging::put_party_contact_information_response response;
    domain::party_contact_information value;
    ...
    if (!written.empty())
        response.party_contact_information = std::move(written.front());
    return response;
}

messaging::put_many_party_contact_informations_response
party_contact_information_service::put_many_party_contact_informations(
    const messaging::put_many_party_contact_informations_request& request) {
    messaging::put_many_party_contact_informations_response response;
    std::vector<domain::party_contact_information> batch;
    ...
    response.party_contact_informations.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.party_contact_informations.push_back(written.empty() ? value :
                                                                        std::move(written.front()));
    }
    return response;
}

messaging::delete_party_contact_information_response
party_contact_information_service::delete_party_contact_information(
    const messaging::delete_party_contact_information_request& request) {
    messaging::delete_party_contact_information_response response;
...
party_contact_information_service::delete_many_party_contact_informations(
    const messaging::delete_many_party_contact_informations_request& request) {
    messaging::delete_many_party_contact_informations_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
party_contact_information_service::list_party_contact_information_versions(
...
messaging::get_party_contact_information_version_response
party_contact_information_service::get_party_contact_information_version(
    const messaging::get_party_contact_information_version_request& request) {
    messaging::get_party_contact_information_version_response response;
    auto found =
        repo_.read_at_version(ctx_,
                              boost::uuids::to_string(request.key.party_contact_information.id),
                              request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result party_contact_information_service::prepare_change(
    const messaging::party_contact_information_change& change,
...
std::vector<domain::party_contact_information>
party_contact_information_service::list_party_contact_informations(std::uint32_t offset,
                                                                   std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all party contact informations";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t party_contact_information_service::count_party_contact_informations() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total party contact informations count";
...
party_contact_information_service::list_party_contact_informations_by_party_id(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Listing party contact informations by party_id: " << party_id;
    return repo_.read_latest_by_party_id(ctx_, party_id, offset, limit);
}

std::uint32_t party_contact_information_service::count_party_contact_informations_by_party_id(
    const std::string& party_id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting total party contact informations count by party_id: "
                               << party_id;
    return repo_.get_total_party_contact_information_count_by_party_id(ctx_, party_id);
}
...
std::uint32_t party_contact_information_service::count_party_contact_informations_by_party_id(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Getting total party contact informations count by party_id: "
                               << party_id;
    return repo_.get_total_party_contact_information_count_by_party_id(
        ctx_, boost::uuids::to_string(party_id));
}


std::vector<domain::party_contact_information>
party_contact_information_service::list_party_contact_informations_by_party_id_as_of(
    const std::string& party_id,
    std::chrono::system_clock::time_point valid_from_bound,
    std::chrono::system_clock::time_point valid_to_bound) {
    BOOST_LOG_SEV(lg(), debug) << "Listing party contact informations by party_id as of window: "
                               << party_id;
...
party_contact_information_service::find_party_contact_information(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Finding party contact information. " << "id: " << id;
...
std::optional<domain::party_contact_information>
party_contact_information_service::find_party_contact_information_by_code(
    const boost::uuids::uuid& party_id, const std::string& contact_type) {
    BOOST_LOG_SEV(lg(), debug) << "Finding party contact information by party_id/contact_type: "
                               << party_id << "/" << contact_type;
    auto results = repo_.read_latest_by_code(ctx_, boost::uuids::to_string(party_id), contact_type);
    if (results.empty())
        return std::nullopt;
    return results.front();
}
...
std::vector<domain::party_contact_information>
party_contact_information_service::get_party_contact_informations(
    const std::vector<std::string>& ids) {
    return repo_.read_latest(ctx_, ids);
}

void party_contact_information_service::save_party_contact_information(
...
void party_contact_information_service::remove_party_contact_information(
...
std::vector<domain::party_contact_information>
party_contact_information_service::get_party_contact_information_history(
    const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for party contact information. " << "id: " << id;
