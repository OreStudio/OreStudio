 ...
 *
 */
#include "ores.compute.core/generators/batch_generator.hpp"
...
domain::batch generate_synthetic_batch(
    ...
    r.external_ref = std::string("BATCH-") + std::to_string(idx);
    r.status = std::string("open");
...
generate_synthetic_batches(std::size_t n,
    ...
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_batch(ctx));
    return r;
}
