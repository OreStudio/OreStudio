...
int ClientReportTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientReportTypeModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh report type model: disconnected.";
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug)
                    ...
                    << " report types, total available: " << result->total_available_count;
                return {.success = true,
...
void ClientReportTypeModel::onTypesLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " report types newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " report types."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientReportTypeModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientReportTypeModel::onPulsingComplete() {
