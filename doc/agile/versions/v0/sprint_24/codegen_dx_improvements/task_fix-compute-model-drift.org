:PROPERTIES:
:ID: 21647FE2-9AF6-4F6C-BBA0-5BF96C7BD0B7
:END:
#+title: Task: Fix compute entity model drift: update to latest format and regenerate
#+description: Following the same pattern as the refdata sweep (task fix-refdata-model-drift): regenerate every ores.compute domain_entity and junction model against current codegen templates, updating any model found using an outdated format/convention before regenerating, and commit the resulting drift-free tree. compass build and rat must pass after.
#+type: task
#+level: s1
...
Mirror [[id:A2EDDEDC-D5D3-469D-88C9-7CE00951CC72][Fix refdata entity model drift]] for ores.compute: regenerate every
ores.compute =domain_entity= and =junction= model (=compass codegen entity
...
- Every ores.compute =domain_entity= and =junction= model is regenerated via
...
- =compass build= and full =rat= pass after the regenerated files are
  committed.
- No entity outside compute is touched.
