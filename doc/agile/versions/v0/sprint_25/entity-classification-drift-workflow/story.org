...
#+description: Run the compass-codegen-fix-drift method on ores.workflow: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the drift gate to cover workflow-cpp. Supersedes the ad hoc 'Apply safe drift to workflow-cpp' task.
#+type: story
#+level: s2
