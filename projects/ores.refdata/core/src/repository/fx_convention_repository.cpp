...
void fx_convention_repository::write(context ctx, const domain::fx_convention& v) {
    ...
    execute_write_query(
        ctx, fx_convention_mapper::map(v), lg(), "Writing FX convention to database.");
}

void fx_convention_repository::write(context ctx, const std::vector<domain::fx_convention>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing FX conventions. Count: " << v.size();
    execute_write_query(
        ctx, fx_convention_mapper::map(v), lg(), "Writing FX conventions to database.");
}

std::vector<domain::fx_convention> fx_convention_repository::read_latest(context ctx) {
    static auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::read<std::vector<fx_convention_entity>> |
...
std::vector<domain::fx_convention> fx_convention_repository::read_latest(context ctx,
    ...
    return execute_read_query<fx_convention_entity, domain::fx_convention>(
        ...
        [](const auto& entities) { return fx_convention_mapper::map(entities); },
        lg(),
        "Reading latest FX convention by id.");
}

std::vector<domain::fx_convention> fx_convention_repository::read_all(context ctx,
                                                                      const std::string& id) {
    ...
    return execute_read_query<fx_convention_entity, domain::fx_convention>(
        ...
        [](const auto& entities) { return fx_convention_mapper::map(entities); },
        lg(),
        "Reading all FX convention versions by id.");
}
