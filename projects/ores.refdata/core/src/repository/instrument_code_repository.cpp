...
instrument_code_repository::replace_claim(context ctx, const domain::instrument_code& v) {
...
instrument_code_repository::apply_claim(context ctx,
...
void instrument_code_repository::write(context ctx,
                                       const domain::instrument_code& v,
                                       const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing instrument code. " << "code: " << v.code;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, instrument_code_mapper::map(t), lg(), "Writing instrument code to database.");
}

void instrument_code_repository::write(
    context ctx,
    ...
    execute_write_query(
        ctx, instrument_code_mapper::map(batch), lg(), "Writing instrument codes to database.");
}
...
instrument_code_repository::remove_status instrument_code_repository::remove(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<instrument_code_entity> |
                       where("tenant_id"_c == tid && "code"_c == code &&
...
std::vector<domain::instrument_code>
instrument_code_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<instrument_code_entity, domain::instrument_code>(
        ctx,
        query,
        [](const auto& entities) { return instrument_code_mapper::map(entities); },
        lg(),
        "Reading latest instrument codes by ids.");
    return result;
}
