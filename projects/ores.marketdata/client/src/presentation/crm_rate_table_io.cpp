...
void print_crm_rate_table(std::ostream& s, const std::vector<crm_rate_display_service::row>& v) {
    s << std::endl << convert_to_table(v) << std::endl;
}

}

std::ostream& operator<<(std::ostream& s, const std::vector<crm_rate_display_service::row>& v) {
    print_crm_rate_table(s, v);
    return s;
}
