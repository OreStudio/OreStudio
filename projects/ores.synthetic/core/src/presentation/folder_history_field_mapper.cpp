 ...
 *
 */
#include "ores.synthetic.core/presentation/folder_history_field_mapper.hpp"
...
std::vector<ores::diff::domain::field_value> render_folder_fields(const domain::folder& v) {
    ...
    fields.push_back(
        {.name = "Parent ID",
         .value = v.parent_id ? boost::uuids::to_string(*v.parent_id) : std::string{}});
    fields.push_back({.name = "Name", .value = v.name});
    fields.push_back({.name = "Kind", .value = v.kind});
    fields.push_back(
        {.name = "Collection ID",
         .value = v.collection_id ? boost::uuids::to_string(*v.collection_id) : std::string{}});
