...
void FxVarianceSwapInstrumentForm::setupConnections() {
    ...
    connect(ui_->notionalSpinBox,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this, markChanged);
    connect(ui_->momentTypeCombo, &QComboBox::currentTextChanged,
            this, markChangedStr);
...
bool FxVarianceSwapInstrumentForm::isDirty() const { return dirty_; }
bool FxVarianceSwapInstrumentForm::isLoaded() const { return loaded_; }

void FxVarianceSwapInstrumentForm::setChangeReason(
...
void FxVarianceSwapInstrumentForm::writeUiToInstrument() {
    ...
    instrument_.moment_type =
        InstrumentFormUtils::getComboValue(ui_->momentTypeCombo);
    ...
    connect(watcher, &QFutureWatcher<SaveResult>::finished, self,
        ...
        if (!result.success) {
            BOOST_LOG_SEV(lg(), error)
                << "FX variance swap save failed: " << result.message;
