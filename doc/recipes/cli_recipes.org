...
* Recipes
  ...
  :header-args+: :var ORES_CLI_DB_PASSWORD=(auth-source-pick-first-password :host "localhost" :user "ores_cli_user")
  :header-args+: :var db_args="--db-user ores_cli_user --db-host localhost --db-port 5432 --db-database ores_frosty_leaf"
...
Country management operations from the [[id:8A1C5E2D-9B4F-4E8A-A7C3-6F2D1E0B9A5C][ORE Studio Geo Component]].
...
Change reason management operations from the [[id:8A2B3C4D-5E6F-7A8B-9C0D-1E2F3A4B5C6D][ORE Studio Telemetry Component]].
...
Change reason category management operations from the [[id:8A2B3C4D-5E6F-7A8B-9C0D-1E2F3A4B5C6D][ORE Studio Telemetry Component]].
...
Compute host management operations from the ORE Studio Compute Grid Component.
...
Compute application registry management from the ORE Studio Compute Grid Component.
...
Compute application version management from the ORE Studio Compute Grid Component.
...
Compute batch management from the ORE Studio Compute Grid Component.
...
Compute workunit management from the ORE Studio Compute Grid Component. Upload
input and config files to the HTTP server first, then reference their URIs here.
...
Compute result management from the ORE Studio Compute Grid Component.
