...
read_one(repository::counterparty_identifier_repository& repo,
         const ores::database::context& ctx,
         const messaging::counterparty_identifier_key& key) {
 ...
 */
messaging::counterparty_identifier_key key_from(const domain::counterparty_identifier& v) {
 ...
 */
domain::counterparty_identifier to_domain(const messaging::counterparty_identifier_write& write) {
    domain::counterparty_identifier v;
    v.id = write.id;
...
counterparty_identifier_service::get_counterparty_identifier(
    const messaging::get_counterparty_identifier_request& request) {
    messaging::get_counterparty_identifier_response response;
    ...
    response.counterparty_identifier = std::move(found.front());
    return response;
}

messaging::get_many_counterparty_identifiers_response
counterparty_identifier_service::get_many_counterparty_identifiers(
    const messaging::get_many_counterparty_identifiers_request& request) {
    messaging::get_many_counterparty_identifiers_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::counterparty_identifier_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.counterparty_identifier = std::move(found.front());
...
counterparty_identifier_service::put_counterparty_identifier(
    const messaging::put_counterparty_identifier_request& request) {
    messaging::put_counterparty_identifier_response response;
    domain::counterparty_identifier value;
    ...
    if (!written.empty())
        response.counterparty_identifier = std::move(written.front());
    return response;
}

messaging::put_many_counterparty_identifiers_response
counterparty_identifier_service::put_many_counterparty_identifiers(
    const messaging::put_many_counterparty_identifiers_request& request) {
    messaging::put_many_counterparty_identifiers_response response;
    ...
    response.counterparty_identifiers.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.counterparty_identifiers.push_back(written.empty() ? value :
                                                                      std::move(written.front()));
    }
    return response;
}

messaging::delete_counterparty_identifier_response
counterparty_identifier_service::delete_counterparty_identifier(
    const messaging::delete_counterparty_identifier_request& request) {
    messaging::delete_counterparty_identifier_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::counterparty_identifier_repository::remove_status::removed:
            break;
        case repository::counterparty_identifier_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::counterparty_identifier_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::counterparty_identifier_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_counterparty_identifiers_response
counterparty_identifier_service::delete_many_counterparty_identifiers(
    const messaging::delete_many_counterparty_identifiers_request& request) {
    messaging::delete_many_counterparty_identifiers_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_counterparty_identifier_versions_response
counterparty_identifier_service::list_counterparty_identifier_versions(
    const messaging::list_counterparty_identifier_versions_request& request) {
    messaging::list_counterparty_identifier_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_counterparty_identifier_version_response
counterparty_identifier_service::get_counterparty_identifier_version(
    const messaging::get_counterparty_identifier_version_request& request) {
    messaging::get_counterparty_identifier_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.counterparty_identifier.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result counterparty_identifier_service::prepare_change(
    const messaging::counterparty_identifier_change& change,
...
counterparty_identifier_service::get_counterparty_identifier_at_version(const std::string& id,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Getting counterparty identifier at version. " << "id: " << id
                               << " version: " << version;
...
std::vector<domain::counterparty_identifier>
counterparty_identifier_service::get_counterparty_identifiers(const std::vector<std::string>& ids) {
...
void counterparty_identifier_service::save_counterparty_identifier(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Saving counterparty identifier. " << "id: " << v.id;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved counterparty identifier. " << "id: " << v.id;
}
