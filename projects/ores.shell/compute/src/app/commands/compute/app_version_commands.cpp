...
void app_version_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("app_versions");
    ...
    menu->Insert(
        ...
        "add <id> <app_id> <wrapper_version> <engine_version> <min_ram_mb> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <id> <app_id> <wrapper_version> <engine_version> <min_ram_mb> <reason> <commentary> "
        "[--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <app_id> <wrapper_version> <engine_version> <min_ram_mb> "
        "<reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <wrapper_version> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <wrapper_version> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <wrapper_version> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <wrapper_version> --version <n>");

    root_menu.Insert(std::move(menu));
}

void app_version_commands::process_list(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_app_versions_request;
    ...
    auto result = do_auth_request<messaging::list_app_versions_response>(
...
void app_version_commands::process_get(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_app_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.wrapper_version, parsed->positionals[next++], "wrapper_version");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_app_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_get_many(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_app_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            ...
            read_token(key.wrapper_version, parsed->positionals[i + 0], "wrapper_version");
    ...
    auto result = do_auth_request<messaging::get_many_app_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_add(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_app_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.app_id, parsed->positionals[next++], "app_id");
        read_token(
            req.change.write.wrapper_version, parsed->positionals[next++], "wrapper_version");
        read_token(req.change.write.engine_version, parsed->positionals[next++], "engine_version");
        read_token(req.change.write.min_ram_mb, parsed->positionals[next++], "min_ram_mb");
    ...
    auto result = do_auth_request<messaging::put_app_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_set(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
...
void app_version_commands::process_put_many(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_app_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::app_version_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.app_id, parsed->positionals[next++], "app_id");
            read_token(
                change.write.wrapper_version, parsed->positionals[next++], "wrapper_version");
            read_token(change.write.engine_version, parsed->positionals[next++], "engine_version");
            read_token(change.write.min_ram_mb, parsed->positionals[next++], "min_ram_mb");
    ...
    auto result = do_auth_request<messaging::put_many_app_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_delete(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_app_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.wrapper_version, parsed->positionals[next++], "wrapper_version");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_app_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_delete_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_app_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::app_version_key key;
            read_token(key.wrapper_version, parsed->positionals[i * 1 + 0], "wrapper_version");
            req.removals.push_back(messaging::app_version_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_app_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_versions(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_app_version_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_app_version_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_commands::process_version(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_app_version_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.key.app_version.wrapper_version, parsed->positionals[next++], "wrapper_version");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_app_version_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
