 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CONTACT_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CONTACT_TYPE_REPOSITORY_HPP
...
private:
    inline static std::string_view logger_name = "ores.refdata.repository.contact_type_repository";
...
public:
    ...
    /**
     * @brief Writes contact types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::contact_type& v);
    void write(context ctx, const std::vector<domain::contact_type>& v);
    /**@}*/

    /**
     * @brief Writes a contact type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of contact types, each honouring its own
    ...
    /**
     * @brief Reads latest contact types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::contact_type> read_latest(context ctx);
     ...
     */
    std::vector<domain::contact_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single contact type as it stood at a specific
    ...
    /**
     * @brief Reads latest contact types with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Deletes a contact type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a contact type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes contact types by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::contact_type& v);
     ...
     */
    domain::contact_type apply_claim(context ctx,
                                     const domain::contact_type& v,
                                     const ores::utility::domain::precondition& claim);
};
