...
=* Result= section) when raising its PR — as part of =compass-pr-raise=, not
as a separate step deferred until the review round finishes.

*Why:* The =compass-agile-close-task= skill's original guidance ("close at end
...
*How to apply:* When running =compass-pr-raise= for a task-backed branch, do
