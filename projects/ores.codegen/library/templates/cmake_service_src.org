...
Library + executable split like the service flavour, but with no sibling core dependency — the pre-regroup standalone-service shape. *Referenced by no profile and no code*; retained pending the profiles-drift audit (see the =profiles.json= staleness capture). If the audit confirms it is dead, delete this section and the artefact together.  CMake target declaration for this component. Wires sources, include paths, link dependencies, and the export macro.
