...
#+description: The static indexFamilyCombo in IrCurveGenerationConfigDetailDialog.cpp is a hand-patched addition (no codegen paste-block seam exists for it), so widening index_family's enum via the org model doesn't propagate to it -- the interactive Qt create flow still only offers the original 6 families (libor/euribor/sofr/estr/sonia/tona), not the 16 added in task C80D94BB.
#+type: capture
#+level: cross
