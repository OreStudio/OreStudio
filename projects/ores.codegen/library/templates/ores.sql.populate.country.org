 ...
 *
 * Data Quality {{model_name}} Countries Artefact Population Script
 *
 * Populates the dq_countries_artefact_tbl with {{model_name}} countries data.
...
begin
    ...
    -- Insert {{model_name}} countries with flag image links
...
\echo ''
\echo '--- DQ {{model_name}} Countries Summary ---'

select 'Total {{model_name}} Countries' as metric, count(*) as count
