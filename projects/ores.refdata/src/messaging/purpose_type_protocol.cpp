...
void write_purpose_type(std::vector<std::byte>& buffer,
    const domain::purpose_type& pt) {
...
std::expected<domain::purpose_type, error_code>
read_purpose_type(std::span<const std::byte>& data) {
    domain::purpose_type pt;
...
// ============================================================================
// Purpose Type Messages Implementation
// ============================================================================

std::vector<std::byte> get_purpose_types_request::serialize() const {
    return {};
}

std::expected<get_purpose_types_request, error_code>
get_purpose_types_request::deserialize(std::span<const std::byte> data) {
    if (!data.empty()) {
        return std::unexpected(error_code::payload_too_large);
    }
    return get_purpose_types_request{};
}
...
std::vector<std::byte> get_purpose_types_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& pt : types) {
        write_purpose_type(buffer, pt);
    }
    return buffer;
}

std::expected<get_purpose_types_response, error_code>
get_purpose_types_response::deserialize(std::span<const std::byte> data) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        auto result = read_purpose_type(data);
...
save_purpose_type_request
save_purpose_type_request::from(domain::purpose_type type) {
    return save_purpose_type_request{std::vector<domain::purpose_type>{std::move(type)}};
}

save_purpose_type_request
save_purpose_type_request::from(std::vector<domain::purpose_type> types) {
    return save_purpose_type_request{std::move(types)};
}

std::vector<std::byte> save_purpose_type_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& e : types)
        write_purpose_type(buffer, e);
    return buffer;
}

std::expected<save_purpose_type_request, error_code>
save_purpose_type_request::deserialize(std::span<const std::byte> data) {
    auto count_result = reader::read_uint32(data);
    if (!count_result)
        return std::unexpected(count_result.error());

    save_purpose_type_request request;
    request.types.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_purpose_type(data);
        if (!e)
...
std::vector<std::byte> save_purpose_type_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_purpose_type_response, error_code>
save_purpose_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> delete_purpose_type_request::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_purpose_type_request, error_code>
delete_purpose_type_request::deserialize(std::span<const std::byte> data) {
    delete_purpose_type_request request;
...
std::vector<std::byte> delete_purpose_type_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    return buffer;
...
std::expected<delete_purpose_type_response, error_code>
delete_purpose_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> get_purpose_type_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_purpose_type_history_request, error_code>
get_purpose_type_history_request::deserialize(std::span<const std::byte> data) {
    get_purpose_type_history_request request;
...
std::vector<std::byte> get_purpose_type_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_purpose_type(buffer, v);
    }
    return buffer;
}

std::expected<get_purpose_type_history_response, error_code>
get_purpose_type_history_response::deserialize(std::span<const std::byte> data) {
