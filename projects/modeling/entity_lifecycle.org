...
A full-stack ORE Studio entity spans six layers: a domain type, a
SQL schema, and four surface layers (HTTP REST, CLI, shell REPL,
TypeScript UI metadata). All layers are *mandatory* for a regular
...
the ground truth for what each layer generates. Three layers (HTTP,
CLI, Shell) have no profile yet — see §Codegen gaps below.
...
that is still moving, and the SQL schema is what fixes that shape. The four
...
| TypeScript UI metadata | [[id:B1450696-8C51-4CC5-8910-84912A411AB6][compass-code-add-domain-type]] | [[id:2A2E8B6D-179D-452C-9169-04E6BB46CC07][Codegen input org-file schema reference]] §=** Presentation= | — |
...
*TypeScript UI metadata* (=ores.typescript=, generated by =--address ores.ts.ui=):

| File | Template |
|---|---|
| =src/{component}/ui/{entity}_ui.ts= | =ts_ui.ts.mustache= |
...
Three layers have no codegen profile. When the [[id:0CBA2F23-7F5F-433D-921A-D9C056BD9CA8][entity-creator]] reaches
