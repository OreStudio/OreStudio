...
#+CAPTION: Clock summary at [2026-02-12 Thu 00:24], for Thursday, February 12, 2026.
|       | <75>                                  |        |      |      |       |
| Tags  | Headline                              | Time   |      |      |     % |
|-------+---------------------------------------+--------+------+------+-------|
...
|       | Stories                               | 0:24   |      |      | 100.0 |
|       | Active                                |        | 0:24 |      | 100.0 |
| agile | Sprint and product backlog refinement |        |      | 0:24 | 100.0 |
#+end:

#+begin: clocktable :maxlevel 3 :scope subtree :tags t :indent nil :emphasize nil :scope file :narrow 75 :formula %
#+TBLNAME: sprint_summary
#+CAPTION: Clock summary at [2026-02-12 Thu 00:24]
|       | <75>                                               |         |       |      |       |
| Tags  | Headline                                           | Time    |       |      |     % |
|-------+----------------------------------------------------+---------+-------+------+-------|
...
|       | Stories                                            | 37:02   |       |      | 100.0 |
|       | Active                                             |         | 37:02 |      | 100.0 |
| agile | Sprint and product backlog refinement              |         |       | 1:42 |   4.6 |
| code  | Add bound parameters to sqlgen                     |         |       | 0:51 |   2.3 |
| code  | Add roles to account via codes                     |         |       | 0:34 |   1.5 |
| code  | Implement party related entities at database level |         |       | 4:23 |  11.8 |
| code  | Add party related support at the domain level      |         |       | 0:23 |   1.0 |
| code  | Add party related support to Qt                    |         |       | 6:41 |  18.0 |
| code  | Add GLEIF data to datasets                         |         |       | 6:41 |  18.0 |
| code  | Brainstorm on multi-party support                  |         |       | 4:00 |  10.8 |
| code  | Improve code coverage                              |         |       | 0:14 |   0.6 |
| code  | Improve generators with FK-aware test data         |         |       | 1:44 |   4.7 |
| code  | Enable all skipped tests                           |         |       | 0:29 |   1.3 |
| code  | Add central bank related LEIs                      |         |       | 0:35 |   1.6 |
| code  | Expand unit test coverage for ORE codegened types  |         |       | 0:49 |   2.2 |
| code  | Add system party support                           |         |       | 3:58 |  10.7 |
| code  | Add customer party support                         |         |       | 1:33 |   4.2 |
| code  | Add counterparty wizard                            |         |       | 1:12 |   3.2 |
...
*** STARTED Add a setup new tenant wizard                              :code:
    :LOGBOOK:
    CLOCK: [2026-02-12 Thu 07:20]
...
*** STARTED Implement party-level RLS isolation                        :code:

Add a second layer of Row-Level Security that isolates party-scoped data
(counterparties, books, portfolios, trades) within a tenant. At login, compute
the user's visible party set via a recursive CTE on the party hierarchy and
store it as a PostgreSQL session variable (=app.visible_party_ids=). RLS
policies on party-scoped tables filter using =party_id = ANY(...)=.
...
- =app.visible_party_ids= session variable set at login.
- Party-scoped tables have RLS policies filtering by visible party set.
- System party users see all parties in the tenant.
- Leaf party users see only their own data.
- Mid-level party users see their subtree.

*** Advanced counterparty dialog                                       :code:
...
**** Trading System Entity & Counterparty Management Specification

 1. SYSTEM CORE ENTITIES
** System Tenant
- **Name:** =Nexus_Kernel=
- **Purpose:** Storage of global system data and reference data.
** System Party
- **Name:** =Platform_Operator_Entity=
- **Purpose:** Internal legal entity for the system itself (House Account).

* 2. DATA ARCHITECTURE (JSON)
#+begin_src json
{
  "entity_id": "UUID-99201",
  "is_system_owned": false,
  "legal_name": "J.P. Morgan Securities LLC",
  "lei": "7H6GLXDR0MBN8G7RUT71",
  "bic_swift": "CHASUS33",
  "status": "ACTIVE",
  "hierarchy": { "parent_id": "UUID-ROOT-JPM", "aggregate_risk": true },
  "trading_limits": [
    { "asset_class": "FX_SPOT", "limit_usd": 50000000, "utilization_usd": 25000000 }
  ],
  "ssi": [
    { "currency": "USD", "bank_name": "BNY MELLON NY", "status": "VERIFIED" }
  ]
}
#+end_src

* 3. UI WIREFRAME SPECIFICATIONS
** Tab 1: General Info
#+begin_example
...
1. **System Party Identification:** If =is_system_owned= is true, UI should apply a distinct CSS theme (e.g., subtle blue background).
2. **Limit Validation:** If =utilization_usd= > =limit_usd=, the Status indicator in Tab 3 must change to [OVERLIMIT] in red.
3. **Four-Eyes Principle:** Any change to SSI or Trading Limits must enter a 'PENDING' state until approved by a second user.
4. **GLEIF Integration:** Use ISO 17442 for LEI validation.
...
*** Add GLEIF-based evaluation tenant onboarding wizard                :code:
