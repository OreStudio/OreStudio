...
#include "ores.refdata.api/generators/tenor_resolution_algorithm_generator.hpp"
...
// notification" pattern end to end for tenor_resolution_algorithm -- the
...
using ores::refdata::domain::tenor_resolution_algorithm;
using ores::refdata::repository::tenor_resolution_algorithm_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_tenor_resolution_algorithm_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::tenor_resolution_algorithm_changed_event>(
        [&nats](const ores::refdata::eventing::tenor_resolution_algorithm_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::refdata::eventing::tenor_resolution_algorithm_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.tenor_resolution_algorithm",
    ...
    event_source
        .register_mapping<ores::refdata::eventing::tenor_resolution_algorithm_changed_event>(
            "ores.refdata.tenor_resolution_algorithm", "ores_refdata_tenor_resolution_algorithms");
    ...
    auto v = generate_synthetic_tenor_resolution_algorithm(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Tenor Resolution Algorithm: " << v;

    tenor_resolution_algorithm_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.tenor_resolution_algorithm") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenor_resolution_algorithm " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenor_resolution_algorithm "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::tenor_resolution_algorithm_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_algorithm_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_algorithm(id_str);
        ...
        REQUIRE_FALSE(svc.get_algorithm(id_str).has_value());
        REQUIRE(svc.get_algorithm_history(id_str).size() == versions.size());
    }
}
