...
| =user_journey= | User journey: what one person is trying to do, written before it is built. |
...
| =cli= | Retired. The command-line interface was removed, so the tag survives only on historical documents. |
| =qt= | Retired. The Qt-based GUI client was removed, so the tag survives only on historical documents. |
...
| =workflow= | =ores.workflow=: the workflow engine, its instances and steps. |
...
| =journey= | User journeys, and how they are executed. |
...
| =orchestration= | Coordinating several writes across entities or services. |
...
| =saga= | Saga-style consistency: compensable steps instead of one transaction. |
