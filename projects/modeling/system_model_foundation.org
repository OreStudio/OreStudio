...
#+description: Component inventory for the Foundation layer: the bedrock utilities, platform abstractions, logging, telemetry, database access, security, org-mode parsing, and code generation that underpin all other layers.
#+type: knowledge
#+level: cross
...
helpers, repository mapper helpers, a health monitor, and LISTEN/NOTIFY
integration. All domain services build on this component to access their
isolated tables. A simple component with no sub-parts. It is hand-written
infrastructure with no codegen entity model, and it owns the bootstrap table
=ores_database_info_tbl= that records the checkout which built the database.

| Component      | Role                                        | Dependencies                                                     |
|----------------+---------------------------------------------+------------------------------------------------------------------|
| [[id:3A8F2E91-D4C7-8AB4-5E23-91D6F8C04A72][ores.database]] | PostgreSQL connection pool, bitemporal CRUD | [[id:1FBCC86B-8E04-4F81-8558-85C0B6C54836][ores.logging]], [[id:7F3A2E81-B5C9-4D6A-8E12-9C4B3F5A7D0E][ores.platform]] |
...
IP-to-country lookup using ip2country ranges stored in PostgreSQL. Answers a
country code only; city and coordinate data are not available. =ores.http=
calls it from its login route.

| Sub-component | Role                                     | Dependencies  |
|---------------+------------------------------------------+---------------|
| [[id:8A1C5E2D-9B4F-4E8A-A7C3-6F2D1E0B9A5C][ores.geo]]      | IP-to-country lookup via ip2country | [[id:3A8F2E91-D4C7-8AB4-5E23-91D6F8C04A72][ores.database]] |
...
Used by ores.iam for credential handling.
...
Parses the subset of org-mode this project's docs use — frontmatter keywords,
property drawers, headings, paragraphs, lists, tables, and =id:= links — into a
C++ document structure, and resolves those links against the org-roam SQLite
index that =compass= builds. A pure C++ library, consumed by =ores.shell=.

| Sub-component | Role                                      | Dependencies |
|---------------+-------------------------------------------+--------------|
| [[id:647EA42E-3E11-4898-97BA-8DD2B9949897][ores.orgmode]] | Org-mode parsing and org-roam link resolution | —            |
