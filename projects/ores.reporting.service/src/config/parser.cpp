...
void print_help(const options_description& od, std::ostream& info) {
    info << "Reporting service" << std::endl << std::endl
         << "Usage: ores.reporting.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("REPORTING_SERVICE"));
