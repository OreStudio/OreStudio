 ...
 *
 */
#ifndef ORES_DQ_EVENTING_DATASET_CHANGED_EVENT_HPP
#define ORES_DQ_EVENTING_DATASET_CHANGED_EVENT_HPP
...
/**
 * @brief Domain event indicating that dataset data has changed.
 *
 * This event is published when any dataset entity is created,
 * updated, or deleted in the database. Subscribers can use the timestamp to
 * query for changes since that point.
 */
struct dataset_changed_event final {
    /**
    ...
    /**
     * @brief IDs of datasets that changed.
     *
     * Contains the UUIDs (as strings) of datasets that were created, updated,
