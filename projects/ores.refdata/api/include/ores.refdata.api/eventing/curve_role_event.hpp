 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_CURVE_ROLE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_CURVE_ROLE_EVENT_HPP
...
/**
 * @brief Event traits for curve role's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::curve_role_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.curve_roles_events";

    static ores::refdata::messaging::curve_role_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::curve_role_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::curve_role_key>(notification.key))
            event.key = *key;
