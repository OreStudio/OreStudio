...
| Now          | Every item that does not depend on what the component owns is done: B01, B02, B03, B04, B05, M01 to M10, G01, G03, G06, H01, H02, H03, S02, V01, the database half of V03 and the core half of V02. Six findings are recorded, two of them defects. The remaining items are the protocol and wiring ones and the shell unit, and all of them change shape with the ownership decision. |
...
| Finding: two part test suites are empty | =ores.ore.api.tests= and =ores.ore.service.tests= each contain only =main.cpp=, so both binaries link, run and report "No tests ran". The core part carries 62 test files. The entity's own shell command test exists but is not compiled (D13), so the only test the generated surface has is the eventing integration test in core. | Running both binaries; =ls projects/ores.ore/{api,service}/tests/= |
