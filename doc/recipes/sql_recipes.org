...
** Step 5: Create Admin Database (Optional)
...
The template creation script automatically populates:
- *Reference data*: Currencies (158), country flags (270+), currency-flag mappings
- *RBAC permissions*: All system permissions (accounts:*, currencies:*, feature_flags:*, etc.)
- *RBAC roles*: =admin= (all permissions) and =user= (read-only permissions)
- *System flags*: =bootstrap_mode=, =user_signups=, =signup_requires_authorization=, =disable_password_validation=
...
The database template is pre-populated with RBAC data and system flags via SQL
scripts. These are idempotent and can be re-run safely.
...
This master script runs all population scripts in order:
