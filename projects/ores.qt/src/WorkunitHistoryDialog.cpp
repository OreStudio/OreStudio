...
WorkunitHistoryDialog::WorkunitHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::WorkunitHistoryDialog),
      id_(id),
      code_(code),
...
void WorkunitHistoryDialog::setupUi() {
...
void WorkunitHistoryDialog::setupToolbar() {
...
void WorkunitHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &WorkunitHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &WorkunitHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &WorkunitHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void WorkunitHistoryDialog::loadHistory() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        emit errorOccurred("Not connected to server");
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Loading history for workunit: " << code_.toStdString();
...
void WorkunitHistoryDialog::updateVersionList() {
...
void WorkunitHistoryDialog::onVersionSelected() {
...
void WorkunitHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.input_uri != previous.input_uri) {
        addChange("Input Uri",
                  QString::fromStdString(previous.input_uri),
                  QString::fromStdString(current.input_uri));
    }
    ...
    if (current.config_uri != previous.config_uri) {
        addChange("Description",
                  QString::fromStdString(previous.config_uri),
                  QString::fromStdString(current.config_uri));
    }
...
void WorkunitHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->codeValue->setText(QString::fromStdString(version.input_uri));
    ui_->nameValue->setText(QString::fromStdString(version.input_uri));
    ui_->descriptionValue->setText(QString::fromStdString(version.config_uri));
...
void WorkunitHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void WorkunitHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void WorkunitHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
