...
{{entity_singular}}_repository::read_at_version(context ctx, const std::string& {{primary_key.column}},
                                                std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading {{entity_singular_words}} at version. {{primary_key.column}}: "
                               << {{primary_key.column}} << " version: " << version;
{{#has_tenant_id}}
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where("tenant_id"_c == tid && "{{primary_key.column}}"_c == {{primary_key.column}} && "version"_c == version) |
        sqlgen::limit(1);
{{/has_tenant_id}}
{{^has_tenant_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where("{{primary_key.column}}"_c == {{primary_key.column}} && "version"_c == version) |
        sqlgen::limit(1);
{{/has_tenant_id}}
...
{{#has_workspace_id}}
    ...
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        ...
        order_by("{{primary_key.column}}"_c) | sqlgen::offset(offset) | sqlgen::limit(limit);
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
...
{{entity_singular}}_repository::get_hierarchy(context ctx,
    ...
    const std::string sql = "SELECT * FROM {{product}}_{{component}}_{{entity_plural}}_hierarchy_fn('" +
