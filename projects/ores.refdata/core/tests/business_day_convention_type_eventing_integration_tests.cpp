...
// notification" pattern end to end for business_day_convention_type -- the
...
using ores::refdata::domain::business_day_convention_type;
using ores::refdata::repository::business_day_convention_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_business_day_convention_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::business_day_convention_type_changed_event>(
        [&nats](const ores::refdata::eventing::business_day_convention_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::refdata::eventing::business_day_convention_type_changed_event>::name),
    ...
    event_source
        .register_mapping<ores::refdata::eventing::business_day_convention_type_changed_event>(
            "ores.refdata.business_day_convention_type",
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.business_day_convention_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for business_day_convention_type " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for business_day_convention_type "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::business_day_convention_type_service svc(crud_ctx);
