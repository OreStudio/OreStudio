 ...
 */
#ifndef ORES_IAM_API_EVENTING_TENANT_EVENT_HPP
#define ORES_IAM_API_EVENTING_TENANT_EVENT_HPP
...
/**
 * @brief Event traits for tenant's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::iam::messaging::tenant_event> {
    static constexpr std::string_view subject_prefix = "iam.v1.tenants_events";

    static ores::iam::messaging::tenant_event
    from_notification(const entity_event_notification& notification) {
        ores::iam::messaging::tenant_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::iam::messaging::tenant_key>(notification.key))
            event.key = *key;
