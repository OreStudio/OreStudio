 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CALENDAR_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CALENDAR_REPOSITORY_HPP
...
/**
 * @brief Reads and writes calendars to data storage.
 */
class ORES_REFDATA_CORE_EXPORT calendar_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.calendar_repository";
...
public:
    ...
    /**
     * @brief Writes calendars to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::calendar& v);
    void write(context ctx, const std::vector<domain::calendar>& v);
    /**@}*/

    /**
     * @brief Writes a calendar, honouring the claim it states.
     *
     ...
     */
    void
    write(context ctx, const domain::calendar& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of calendars, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::calendar>& v,
    ...
    std::vector<domain::calendar> read_latest(context ctx, const std::vector<std::string>& codes);
    /**@}*/
    ...
    /**
     * @brief Reads a single calendar as it stood at a specific
    ...
    /**
     * @brief Reads latest calendars with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_calendar_count(context ctx);

    /**
     * @brief Deletes a calendar by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a calendar, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes calendars by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::calendar& v);
     ...
     */
    domain::calendar apply_claim(context ctx,
                                 const domain::calendar& v,
