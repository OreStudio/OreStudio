...
|--------+---------+-------|
| =compass sprint charts= | Extracts per-sprint metrics from git history and agile doc state changes; outputs CSV files consumed by gnuplot chart scripts. | Absorbed from =sprint_metrics.py= (deleted). See [[id:1AF08218-EFBC-421B-BBF3-2FFD2332E426][How do I generate sprint charts with compass?]]. Companion gnuplot scripts remain in =scripts/=. |
| =parse_test_results.py= | Parses Catch2 XML test-result files; extracts per-suite statistics and failure details. | Python; shell wrapper: =parse_test_results.sh=. Pending absorption into =compass test results= (follow-on task). |
| =parse_test_results.sh= | Shell wrapper for =parse_test_results.py=; runs the parser from the repo root with standard arguments. | Delegates to =parse_test_results.py=. Pending absorption into =compass test results=. |
