...
void print_domain_help(std::ostream& info) {
    ...
    info << "  " << std::setfill(' ') << std::left << std::setw(command_width) << hosts_command_name
         << hosts_command_desc << std::endl;
    info << "  " << std::setfill(' ') << std::left << std::setw(command_width) << apps_command_name
         << apps_command_desc << std::endl;
    ...
    info << std::endl
         << "Use: ores.cli compute <entity> <operation> --help for details." << std::endl;
}

void validate_entity_name(const std::string& entity_name) {
    const bool is_valid(
        entity_name == hosts_command_name || entity_name == apps_command_name ||
        entity_name == app_versions_command_name || entity_name == batches_command_name ||
        entity_name == workunits_command_name || entity_name == results_command_name);
