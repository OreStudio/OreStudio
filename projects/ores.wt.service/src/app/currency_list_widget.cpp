...
void currency_list_widget::setup_toolbar() {
    ...
    add_button_ = toolbar->addWidget(std::make_unique<Wt::WPushButton>("Add Currency"));
...
void currency_list_widget::setup_table() {
    ...
    table_->elementAt(0, 0)->addWidget(std::make_unique<Wt::WText>("ISO Code"));
    table_->elementAt(0, 1)->addWidget(std::make_unique<Wt::WText>("Name"));
    table_->elementAt(0, 2)->addWidget(std::make_unique<Wt::WText>("Symbol"));
    table_->elementAt(0, 3)->addWidget(std::make_unique<Wt::WText>("Numeric Code"));
    table_->elementAt(0, 4)->addWidget(std::make_unique<Wt::WText>("Asset Class"));
    table_->elementAt(0, 5)->addWidget(std::make_unique<Wt::WText>("Market Tier"));
    table_->elementAt(0, 6)->addWidget(std::make_unique<Wt::WText>("Version"));
...
void currency_list_widget::set_currencies(const std::vector<currency_row>& currencies) {
...
void currency_list_widget::populate_table() {
    ...
    for (std::size_t i = 0; i < currencies_.size(); ++i) {
        ...
        table_->elementAt(row, 0)->addWidget(std::make_unique<Wt::WText>(c.iso_code));
        table_->elementAt(row, 1)->addWidget(std::make_unique<Wt::WText>(c.name));
        table_->elementAt(row, 2)->addWidget(std::make_unique<Wt::WText>(c.symbol));
        table_->elementAt(row, 3)->addWidget(std::make_unique<Wt::WText>(c.numeric_code));
        table_->elementAt(row, 4)->addWidget(std::make_unique<Wt::WText>(c.monetary_nature));
        table_->elementAt(row, 5)->addWidget(std::make_unique<Wt::WText>(c.market_tier));
        table_->elementAt(row, 6)->addWidget(
            std::make_unique<Wt::WText>(std::to_string(c.version)));

        auto actions =
            table_->elementAt(row, 7)->addWidget(std::make_unique<Wt::WContainerWidget>());
        ...
        edit_btn->clicked().connect([this, iso = c.iso_code] { edit_requested_.emit(iso); });
        ...
        delete_btn->clicked().connect([this, iso = c.iso_code] { delete_requested_.emit(iso); });
    }
}
