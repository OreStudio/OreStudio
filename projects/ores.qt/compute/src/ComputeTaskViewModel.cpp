...
QString format_duration(const compute::domain::result& r) {
    if (r.server_state != 5 || r.received_at == std::chrono::system_clock::time_point{} ||
        r.recorded_at == std::chrono::system_clock::time_point{}) {
        return QObject::tr("—");
    }
    const auto secs =
        std::chrono::duration_cast<std::chrono::seconds>(r.received_at - r.recorded_at).count();
    if (secs < 60)
...
ComputeTaskViewModel::ComputeTaskViewModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ComputeTaskViewModel::onTasksLoaded);
}
...
QVariant ComputeTaskViewModel::data(const QModelIndex& index, int role) const {
    if (!index.isValid())
        return {};
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            case Label:
                return tr("Batch #%1 / Task %2").arg(t.batch_ordinal).arg(t.task_ordinal);
            case State:
            ...
            case Host: {
                if (t.result.host_id == boost::uuids::uuid{})
                    return tr("—");
                const auto quuid =
                    QString::fromStdString(boost::uuids::to_string(t.result.host_id));
                return host_name_cache_ ? host_name_cache_->display_name_for(quuid) : quuid.left(8);
            }
            case Duration:
        ...
        switch (index.column()) {
            ...
            case Host:
                if (host_name_cache_ && t.result.host_id != boost::uuids::uuid{})
                    return QString::fromStdString(boost::uuids::to_string(t.result.host_id));
                return {};
            case Batch:
...
QVariant
ComputeTaskViewModel::headerData(int section, Qt::Orientation orientation, int role) const {
            ...
            [&]() -> FetchResult {
                ...
                if (!br)
                    return {.success = false,
                            .error_message =
                                QString::fromStdString("Failed to fetch batches: " + br.error())};
                ...
                if (!wr)
                    return {.success = false,
                            .error_message =
                                QString::fromStdString("Failed to fetch workunits: " + wr.error())};
                ...
                if (!rr)
                    return {.success = false,
                            .error_message =
                                QString::fromStdString("Failed to fetch results: " + rr.error())};
                ...
                for (auto& res : rr->results) {
                    const auto wu_key = boost::uuids::to_string(res.workunit_id);
                    auto wu_it = wu_map.find(wu_key);
                    if (wu_it == wu_map.end())
                        continue;

                    const auto batch_key = boost::uuids::to_string(wu_it->second.batch_id);
                    auto b_it = batch_map.find(batch_key);
                    if (b_it == batch_map.end())
                        continue;
                ...
                BOOST_LOG_SEV(lg(), debug) << "Joined " << tasks.size() << " tasks from "
...
void ComputeTaskViewModel::onTasksLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to load tasks: " << result.error_message.toStdString();
