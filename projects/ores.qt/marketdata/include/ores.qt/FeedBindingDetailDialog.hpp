...
/**
 * @brief Detail dialog for viewing and editing feed binding records.
 *
 * This dialog allows viewing, creating, and editing feed bindings.
 * It supports both create mode (for new records) and edit mode (for
 * existing records).
 */
class FeedBindingDetailDialog final : public DetailDialogBase {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.feed_binding_detail_dialog";
...
public:
    explicit FeedBindingDetailDialog(QWidget* parent = nullptr);
    ~FeedBindingDetailDialog() override;
...
private:
    ...
    void updateUiFromBinding();
    void updateBindingFromUi();
