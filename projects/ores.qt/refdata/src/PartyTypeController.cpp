...
constexpr std::string_view type_event_name =
    eventing::domain::event_traits<refdata::eventing::party_type_changed_event>::name;
}
...
void PartyTypeController::showListWindow() {
    ...
    // Connect signals
    connect(
        listWindow_, &PartyTypeMdiWindow::statusChanged, this, &PartyTypeController::statusMessage);
    connect(
        listWindow_, &PartyTypeMdiWindow::errorOccurred, this, &PartyTypeController::errorMessage);
    connect(listWindow_,
            &PartyTypeMdiWindow::showTypeDetails,
            this,
            &PartyTypeController::onShowDetails);
    connect(listWindow_,
            &PartyTypeMdiWindow::addNewRequested,
            this,
            &PartyTypeController::onAddNewRequested);
    connect(listWindow_,
            &PartyTypeMdiWindow::showTypeHistory,
            this,
            &PartyTypeController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<PartyTypeController>(this), key]() {
                if (!self)
                    return;
...
void PartyTypeController::onShowDetails(const refdata::domain::party_type& type) {
...
void PartyTypeController::onShowHistory(const refdata::domain::party_type& type) {
...
void PartyTypeController::showAddWindow() {
    ...
    connect(detailDialog,
            ...
            this,
            [self = QPointer<PartyTypeController>(this)](const QString& code) {
                if (!self)
                    return;
...
void PartyTypeController::showDetailWindow(const refdata::domain::party_type& type) {
    ...
    connect(detailDialog,
            &PartyTypeDetailDialog::typeDeleted,
            this,
            [self = QPointer<PartyTypeController>(this), key](const QString& code) {
                if (!self)
                    return;
...
void PartyTypeController::showHistoryWindow(const QString& code) {
    BOOST_LOG_SEV(lg(), info) << "Opening history window for party type: " << code.toStdString();
    ...
    auto* historyDialog =
        new HistoryDialog(std::string(entity_type_of(refdata::domain::party_type{})),
                          code.toStdString(),
                          clientManager_,
                          mainWindow_);
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<PartyTypeController>(this)](
...
void PartyTypeController::onOpenVersion(const refdata::domain::party_type& type,
                                        int versionNumber) {
    ...
    detailWindow->setWindowTitle(
        QString("Party Type: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void PartyTypeController::fetchPartyTypeHistory(
    const QString& entityId,
    std::function<void(std::expected<std::vector<refdata::domain::party_type>, QString>)>
        callback) {
    refdata::messaging::get_party_type_history_request request;
    request.code = entityId.toStdString();

    using FetchResult = std::expected<std::vector<refdata::domain::party_type>, QString>;
...
void PartyTypeController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<PartyTypeController> self = this;
    fetchPartyTypeHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<refdata::domain::party_type>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void PartyTypeController::onRevertHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<PartyTypeController> self = this;
    fetchPartyTypeHistory(
        entityId,
        [self, entityId, versionNumber](
...
void PartyTypeController::onRevertVersion(const refdata::domain::party_type& type) {
    BOOST_LOG_SEV(lg(), info) << "Reverting party type to version: " << type.version;
    ...
    connect(detailDialog,
            ...
            [self = QPointer<PartyTypeController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("Party Type '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert Party Type: %1").arg(QString::fromStdString(type.code)));
...
void PartyTypeController::notifyOpenDialogs(const QStringList& entityIds) {
