...
ClientDatasetBundleModel::ClientDatasetBundleModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ClientDatasetBundleModel::onBundlesLoaded);

    connect(pulseManager_,
            &RecencyPulseManager::pulse_state_changed,
            this,
            &ClientDatasetBundleModel::onPulseStateChanged);
    connect(pulseManager_,
            &RecencyPulseManager::pulsing_complete,
            this,
            &ClientDatasetBundleModel::onPulsingComplete);
}
...
QVariant ClientDatasetBundleModel::data(const QModelIndex& index, int role) const {
    if (!index.isValid())
        return {};
...
QVariant
ClientDatasetBundleModel::headerData(int section, Qt::Orientation orientation, int role) const {
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                BOOST_LOG_SEV(lg(), debug)
                    << "Making dataset bundles request with offset=" << offset
                    << ", limit=" << limit;
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << result->bundles.size() << " dataset bundles";
                const std::uint32_t count = static_cast<std::uint32_t>(result->bundles.size());
                return {.success = true,
...
void ClientDatasetBundleModel::onBundlesLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch dataset bundles: " << result.error_message.toStdString();
...
QVariant ClientDatasetBundleModel::recency_foreground_color(const std::string& code) const {
