...
create unique index if not exists ores_trading_fpml_event_types_version_uniq_idx
on "ores_trading_fpml_event_types_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_trading_fpml_event_types_code_uniq_idx
on "ores_trading_fpml_event_types_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_trading_fpml_event_types_tenant_idx
on "ores_trading_fpml_event_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_trading_fpml_event_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if not exists (
        ...
        raise exception 'Invalid fpml_event_type: %. Must be one of: %', p_value, (
