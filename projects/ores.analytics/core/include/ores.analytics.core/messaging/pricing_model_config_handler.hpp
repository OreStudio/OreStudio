...
public:
    ...
    void save(ores::nats::message msg) {
        ...
        service::pricing_model_config_service svc(req_ctx);
        if (auto req = decode<save_pricing_model_config_request>(msg)) {
            try {
                svc.save_config(req->data);
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg, save_pricing_model_config_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      save_pricing_model_config_response{.success = false, .message = e.what()});
            }
        } else {
    ...
    void history(ores::nats::message msg) {
        ...
        if (auto req = decode<get_pricing_model_config_history_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      get_pricing_model_config_history_response{.success = false,
                                                                .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(pricing_model_config_handler_lg(), warn)
    ...
    void remove(ores::nats::message msg) {
        ...
        if (auto req = decode<delete_pricing_model_config_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      delete_pricing_model_config_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(pricing_model_config_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
