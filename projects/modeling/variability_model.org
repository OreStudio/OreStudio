...
* Part 1 — Physical-space activation (profiles.json)

ORE Studio does not maintain a formal VMM data structure for activation.
Instead, =projects/ores.codegen/library/profiles.json= serves as the
...
| Profile | Named bundle of feature values + activation | Split: activation is a =profiles.json= entry (Part 1); feature values are authored per element (Part 2) |
...
| Element profile | Per-element overrides | Individual model file + explicit =--profile= and its authored =sql.*=/=qt.*= features |
| VMM | Meta-model of the full configuration space | =profiles.json= (activation) + the feature catalogue in Part 2 (configuration) |
