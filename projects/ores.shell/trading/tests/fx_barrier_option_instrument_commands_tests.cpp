...
using ores::shell::app::commands::fx_barrier_option_instrument_commands;
...
TEST_CASE(
    "fx_barrier_option_instrument_commands_register_commands_registers_the_pagination_callback",
    tags) {
    ...
    BOOST_LOG_SEV(lg, debug)
        << "Looking up the pagination callback for fx_barrier_option_instruments.";
    CHECK(pagination.get_list_callback("fx_barrier_option_instruments") != nullptr);
}

TEST_CASE("fx_barrier_option_instrument_commands_process_add_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    fx_barrier_option_instrument_commands::process_add_fx_barrier_option_instrument(
...
TEST_CASE("fx_barrier_option_instrument_commands_process_add_reports_the_expected_argument_count",
          tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("fx_barrier_option_instrument_commands_process_add_names_the_field_of_a_malformed_token",
          tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE(
    "fx_barrier_option_instrument_commands_process_add_sends_a_valid_token_vector_to_the_server",
    tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("fx_barrier_option_instrument_commands_process_delete_requires_a_logged_in_session",
          tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    fx_barrier_option_instrument_commands::process_delete_fx_barrier_option_instrument(
...
TEST_CASE("fx_barrier_option_instrument_commands_process_history_requires_a_logged_in_session",
          tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    fx_barrier_option_instrument_commands::process_get_fx_barrier_option_instrument_history(
        out, session, "missing-key");
