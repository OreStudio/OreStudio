...
void LegTypeMdiWindow::setupToolbar() {
...
void LegTypeMdiWindow::setupTable() {
    model_ = new ClientLegTypeModel(clientManager_, this);
    ...
    delegate->set_badge_color_resolver(
        0, [cache = badgeCache_](const QString& value) -> badge_color_pair {
            ...
            auto* def = cache->resolve("leg_type", value.toStdString());
            if (!def) {
...
void LegTypeMdiWindow::doReload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading leg types";
    clearStaleIndicator();
    emit statusChanged(tr("Loading leg types..."));
    model_->load_page(paginationWidget_->current_offset(), paginationWidget_->page_size());
}

void LegTypeMdiWindow::onDataLoaded() {
...
void LegTypeMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void LegTypeMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void LegTypeMdiWindow::updateActionStates() {
...
void LegTypeMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new leg type requested";
    emit addNewRequested();
}

void LegTypeMdiWindow::editSelected() {
...
void LegTypeMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void LegTypeMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid leg types to delete";
        return;
    }
    ...
    QPointer<LegTypeMdiWindow> self = this;
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                BOOST_LOG_SEV(lg(), debug) << "Leg Type deleted: " << code;
                success_count++;
