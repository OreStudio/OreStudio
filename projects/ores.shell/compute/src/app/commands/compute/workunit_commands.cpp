...
void workunit_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("workunits");
    ...
    menu->Insert(
        ...
        "set <id> <batch_id> <app_version_id> <input_uri> <config_uri> <priority> "
        "<target_redundancy> <canonical_result_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <batch_id> <app_version_id> <input_uri> <config_uri> <priority> "
        "<target_redundancy> <canonical_result_id> <reason> <commentary>");
    ...
    menu->Insert(
        "by-batch-id",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_batch_id(std::ref(out), std::ref(session), std::move(args));
        },
        "by-batch-id <batch_id> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
...
void workunit_commands::process_list(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_workunits_request;
    ...
    auto result = do_auth_request<messaging::list_workunits_response>(
...
void workunit_commands::process_get(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_workunit_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_workunit_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_get_many(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_workunits_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_workunits_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_add(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_workunit_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.config_uri, parsed->positionals[next++], "config_uri");
        read_token(req.change.write.priority, parsed->positionals[next++], "priority");
        read_token(
            req.change.write.target_redundancy, parsed->positionals[next++], "target_redundancy");
        read_token(req.change.write.canonical_result_id,
    ...
    auto result = do_auth_request<messaging::put_workunit_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_set(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
...
void workunit_commands::process_put_many(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_workunits_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::workunit_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.batch_id, parsed->positionals[next++], "batch_id");
            read_token(change.write.app_version_id, parsed->positionals[next++], "app_version_id");
            read_token(change.write.input_uri, parsed->positionals[next++], "input_uri");
            read_token(change.write.config_uri, parsed->positionals[next++], "config_uri");
            read_token(change.write.priority, parsed->positionals[next++], "priority");
            read_token(
                change.write.target_redundancy, parsed->positionals[next++], "target_redundancy");
            read_token(change.write.canonical_result_id,
                       parsed->positionals[next++],
                       "canonical_result_id");
    ...
    auto result = do_auth_request<messaging::put_many_workunits_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_delete(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_workunit_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_workunit_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_delete_many(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_workunits_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_many_workunits_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_by_batch_id(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-batch-id request.";

    using request_type = messaging::list_by_batch_id_workunits_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-batch-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.batch_id = ores::shell::app::from_token<boost::uuids::uuid>(parsed->positionals[next++],
                                                                        "batch_id");
    ...
    auto result = do_auth_request<messaging::list_by_batch_id_workunits_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_versions(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_workunit_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_workunit_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workunit_commands::process_version(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_workunit_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_workunit_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
