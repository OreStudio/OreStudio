...
#+description: Codegen entity model for the refdata =portfolio= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
#+begin_src cpp :name generator
// Left null: optional FK, and the insert trigger rejects ids that do
...
#+begin_src cpp :name generator
// X-0 is the sentinel code the eventing integration fixtures seed
...
:skip_check:      true
:nullable:        true
:error_message:   Invalid aggregation_ccy: %. No active currency found with this iso code.
:END:
