...
create unique index if not exists gmm_components_version_uniq_idx
on "ores_synthetic_gmm_components_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists gmm_components_id_uniq_idx
on "ores_synthetic_gmm_components_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists gmm_components_tenant_idx
...
for each row execute function ores_synthetic_gmm_components_insert_fn();

create or replace rule ores_synthetic_gmm_components_delete_rule as
on delete to "ores_synthetic_gmm_components_tbl" do instead
    update "ores_synthetic_gmm_components_tbl"
    set valid_to = current_timestamp
...
-- =============================================================================
-- Validation function for gmm_component
-- Validates that a id exists in the gmm_components table.
...
-- =============================================================================
create or replace function ores_synthetic_validate_gmm_component_fn(
    p_tenant_id uuid,
    p_value uuid
) returns uuid as $$
begin
    ...
    if p_value is null then
        raise exception 'Invalid gmm_component: value cannot be null'
            using errcode = '23502';
    end if;

    -- Allow pass-through if neither this tenant nor the system tenant has
    -- seeded active gmm_components yet (freshly provisioned tenant).
    if not exists (
        select 1 from ores_synthetic_gmm_components_tbl
    ...
    if not exists (
        ...
        raise exception 'Invalid gmm_component: %. Must be one of: %', p_value, (
