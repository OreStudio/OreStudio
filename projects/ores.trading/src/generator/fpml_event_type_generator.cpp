 ...
 *
 */
#include "ores.trading/generator/fpml_event_type_generator.hpp"
...
domain::fpml_event_type generate_synthetic_fpml_event_type(
    ...
    r.version = 1;
    r.code = std::string(faker::word::noun()) + "_fpml_" + std::to_string(++counter);
...
generate_synthetic_fpml_event_types(std::size_t n,
    ...
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_fpml_event_type(ctx));
    return r;
}
