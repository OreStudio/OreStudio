...
// Soft-FK parent seeding (ores_synthetic_process_parameter_definitions_tbl): the parent may live in
// another component, so its own component names the headers.
#include "ores.synthetic.api/generators/yield_curve_process_parameter_definition_generator.hpp"
...
TEST_CASE("write_ir_curve_generation_config_process_parameter_value_publishes_an_event", tags) {
    ...
    config_id_parent.config_id = market_data_generation_config_parent.id;
    ...
    // Seed the active yield_curve_process_parameter_definition row
    // ores_synthetic_process_parameter_definitions_tbl references: the insert trigger's existence
    // check rejects a synthetic key that matches no active row, so the parent must be written
    // first.
    auto parameter_definition_id_parent =
        ores::synthetic::generators::generate_synthetic_yield_curve_process_parameter_definition(
            ctx);
    parameter_definition_id_parent.change_reason_code = "system.test";
    ores::synthetic::repository::yield_curve_process_parameter_definition_repository
        parameter_definition_id_repo;
    parameter_definition_id_repo.write(party_ctx, parameter_definition_id_parent);
    v.parameter_definition_id = parameter_definition_id_parent.id;
