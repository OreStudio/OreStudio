...
=codegen.sh regenerate --component refdata --address ores.sql= produces zero diff against the
...
X --address ores.cpp=. All diffs collected and categorised as:
...
Running =codegen.sh regenerate --component X --address ores.cpp= is a valid command for
...
After all decisions are applied, running =codegen.sh regenerate --component X --address ores.cpp= for every registered component produces zero diff. CI passes.
...
- =codegen.sh regenerate --component X --address ores.cpp= produces zero diff for all
...
| [[id:C8A4D7F1-2E6B-4C3A-8F9D-5B1E3C8D2F7A][Apply safe drift to refdata-cpp (pilot)]] | DONE | 2026-08-05 | 2026-08-06 | Pilot. Add template flags refdata needs; update refdata models; pull party_repository custom methods from the org-mode mechanism; build + test + zero-diff invariant. Its own Waiting-on note is stale — the org-mode POC it names is now DONE — but its State field reads BACKLOG (paused, WIP stashed), not blocked; needs re-triage before resuming. |
| [[id:A7FA27E1-0D47-4BEB-BECA-ED06AA6AB4A7][Apply safe drift to trading-cpp]] | ABANDONED | | | Add service_pagination + service_batch_get template flags; update 21 instrument/lookup models; restore trade_service + fra_instrument_service. Superseded by the ores.trading classification story. |
...
| [[id:1ABAAF88-B7A4-4774-8843-CF9CBC727E4B][Apply safe drift to analytics-cpp]] | STARTED | 2026-08-06 | | Set find_by_uuid/find_by_code/find_prefix flags on the four pricing models. |
| [[id:193FF620-BB98-4EB4-9124-2D06CA8FFCE0][Apply safe drift to reporting-cpp]] | DONE | 2026-08-07 | 2026-08-09 | No model changes expected; clean regeneration. |
...
| [[id:E5EEFA91-BE41-4E0B-A69E-DDF965A7F39F][Retrospective: clang-format non-idempotency causing refdata codegen drift]] | ABANDONED | | 2026-09-23 | Write up the clang-format single-pass non-idempotency bug found and fixed while unblocking PR #1820's CI, and audit for other affected patterns/components beyond ores.refdata. |
| [[id:98D0B4FD-EBF9-4DB4-863F-CF0EACD2A1B2][Fix profile binding: single canonical point in * Flags section]] | DONE | 2026-08-08 | 2026-08-09 | read_physical_space_overrides reads :profile: from file-level :PROPERTIES: drawer for Physical-space facet enablement, but every entity model declares :profile: in * Flags. Unify to a single binding point in * Flags, add validation that throws if :profile: is in the wrong place, and revert the workaround on reporting entities. |
