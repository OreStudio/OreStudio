...
#+description: Codegen entity model for the workflow =workflow_instance= table. Drives the C++ domain class, mapper, repository, service, generator, and SQL schema generation from a single literate source.
...
The list query filters by state and orders by type, and the engine's recovery
pass reads every instance for a tenant, so the tenant index is the hot path.
