...
void party_counterparty_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("party_counterparties");
    ...
    menu->Insert(
        ...
        "get-many <party_id> <counterparty_id>");
    ...
    menu->Insert(
        ...
        "add <party_id> <counterparty_id> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <party_id> <counterparty_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <party_id> <counterparty_id> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <party_id> <counterparty_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <party_id> <counterparty_id> <reason> <commentary>");
...
void party_counterparty_commands::process_list(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_party_counterparties_request;
    ...
    auto result = do_auth_request<messaging::list_party_counterparties_response>(
...
void party_counterparty_commands::process_get(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_party_counterparty_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.counterparty_id, parsed->positionals[next++], "counterparty_id");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_party_counterparty_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_get_many(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_party_counterparties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            messaging::party_counterparty_key key;
            read_token(key.party_id, parsed->positionals[i + 0], "party_id");
            read_token(key.counterparty_id, parsed->positionals[i + 1], "counterparty_id");
    ...
    auto result = do_auth_request<messaging::get_many_party_counterparties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_add(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_party_counterparty_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_party_counterparty_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_set(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
...
void party_counterparty_commands::process_put_many(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_party_counterparties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::party_counterparty_change change;
    ...
    auto result = do_auth_request<messaging::put_many_party_counterparties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_delete(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_party_counterparty_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.counterparty_id, parsed->positionals[next++], "counterparty_id");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_party_counterparty_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_delete_many(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_party_counterparties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            read_token(key.counterparty_id, parsed->positionals[i * 2 + 1], "counterparty_id");
            req.removals.push_back(messaging::party_counterparty_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_party_counterparties_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void party_counterparty_commands::process_by_party_id(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-party-id request.";

    using request_type = messaging::list_by_party_id_party_counterparties_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
