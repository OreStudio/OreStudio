...
*Do not* use =compass codegen regenerate --component <component> --profile <profile>= for single-entity work — that command regenerates *every* entity in the component, and on a component with template/generator drift accumulated against stale entities (common in this codebase — see [[id:E1B8A9F3-9F5C-4A2E-8C7D-3B9A5F2E1D4C][Component-wide codegen regenerate produces large unrelated drift]]), the resulting diff can span dozens of unrelated files that then have to be manually discarded. =codegen regenerate= is for genuine component-wide work (e.g. rolling out a template change across every entity that uses it), not for adding one field to one entity.
