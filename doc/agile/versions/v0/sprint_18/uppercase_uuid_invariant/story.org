:PROPERTIES:
:ID: BAF1DEAE-59DF-49AB-BFCD-E6D09496DA43
:END:
#+title: Story: Lock down uppercase UUID invariant
#+description: Enforce uppercase UUIDs everywhere: org documents, codegen templates, and ID-generating Python scripts.
#+type: story
#+level: s2
#+filetags: :documentation:codegen:invariant:build_reliability:sprint_18:v0:
#+created: 2026-05-24
#+updated: 2026-05-24
#+todo: BACKLOG STARTED | DONE ABANDONED

This page documents a [[id:699A8D45-B67E-4D3E-9206-24FA17C51ADA][story]] in [[id:1737C00C-699A-475A-BC94-743C6CDA1001][Sprint 18]]. It captures the goal, current status, acceptance criteria, and the tasks that compose it.

* Goal

Make uppercase UUIDs a hard invariant of the documentation system: every =:ID:= property and every =[[id:UUID]]= link in [[proj:doc][=doc/=]] and =projects/*/modeling/= must use uppercase hex, and every code path that mints or templates a UUID must produce uppercase by construction. Prior commits ([[https://github.com/OreStudio/OreStudio/commit/abe6686ce][=abe6686ce=]], [[https://github.com/OreStudio/OreStudio/commit/a8ddd4920][=a8ddd4920=]], [[https://github.com/OreStudio/OreStudio/commit/d1022161e][=d1022161e=]]) normalised the existing corpus and uppercased the freshly-minted ID in the v2 doc generator; this story closes the remaining holes so the invariant cannot drift back.

* Status

| Field         | Value                                  |
|---------------+----------------------------------------|
| State         | DONE                                   |
| Parent sprint | [[id:1737C00C-699A-475A-BC94-743C6CDA1001][Sprint 18]] |
| Now           | Completed 2026-05-24.                  |
| Waiting on    | Nothing.                               |
| Next          | None.                                  |
| Last touched  | 2026-05-24                               |

* Acceptance

- All =v2_doc_*.org.mustache= templates carry uppercase concept-link UUIDs (=story=, =task=, =sprint=, =version=, =capture=, =investigation=, =product identity=).
- =v2_doc_generate.py= uppercases =parent_id= and =predecessor_id= defensively, so a lowercase value passed via =--parent-id= / =--predecessor-id= still produces an uppercase link in the output.
- [[proj:build/scripts/generate_release_notes.py][=build/scripts/generate_release_notes.py=]] uppercases the generated release-notes UUID.
- [[proj:projects/ores.codegen/scripts/regenerate_backlog_indexes.py][=projects/ores.codegen/scripts/regenerate_backlog_indexes.py=]] holds its hardcoded UUID constants in uppercase.
- A sweep across [[proj:doc][=doc/=]] and =projects/*/modeling/= for lowercase =:ID:= properties and lowercase =[[id:...]]= links returns zero matches. Recipe placeholders such as =[[id:UUID]]= and =[[id:<parent_id>]]= are excluded by design.
- Generating a fresh story or task end-to-end through =generate_v2_doc.sh= produces a file in which /every/ UUID is uppercase, without manual touch-up.

* Tasks

| Task | State | Start | End | Description |
|------+-------+-------+-----+-------------|
| [[id:C86FFED4-AD4E-4330-B542-95B36760ED2F][Task: Enforce uppercase UUID invariant in codegen and audit docs]] | DONE | 2026-05-24 | 2026-05-24 | Patch v2 doc templates and Python scripts so every generated UUID and link is uppercase; sweep org docs for stragglers. |

* Decisions

- /Why hardcode uppercase in the templates rather than post-process the rendered output./ The concept-link UUIDs are not generated, they are author-baked content. Uppercasing them at the template source matches how the rest of the doc graph already stores them, and avoids a post-render rewrite step in the generator.
- /Why also keep the defensive =.upper()= on =parent_id= and =predecessor_id=./ The generator auto-detects parent IDs from on-disk org files (already uppercase), but a user may pass =--parent-id= or =--predecessor-id= manually from a copy-paste source where casing is unknown. Belt-and-braces.
- /Why uppercase =GLOSS_CAPTURE= and =INDEX_IDS= constants in =regenerate_backlog_indexes.py= even though the on-disk index pages were already uppercased by prior normalisation./ The script regenerates =near.org= and =far.org= from these constants; if it ran with the old lowercase constants it would silently undo the on-disk normalisation. Sourcing them as uppercase makes the regen idempotent against the invariant.
- /Where the invariant is documented./ A single sentence under =:PROPERTIES: :ID:= in [[id:8B29A8D8-E004-4E87-B81D-ABEAF68F2B98][document types]] points at this story for the rationale and audit trail. Future contributors editing UUIDs by hand discover the rule there.
- /Malformed UUIDs in v1 plans treated as a separate bug./ The exhaustive sweep surfaced three v1 plan documents whose =:ID:= values were uppercase but contained =H= and =G= characters — invalid hex, predating this work. Rather than fold them into the case-enforcement commits, they ship as a self-contained =[doc] Fix invalid (non-hex) UUIDs in 3 v1 plan files= commit so the case/validity concerns stay distinct in =git log=.

* Out of scope

- Postgres / generated SQL UUID literals (=00000000-0000-0000-0000-000000000000= and live workspace sentinels). SQL has its own conventions and case rules; this story is about the documentation graph only.
- External docs in [[proj:external][=external/=]] and third-party samples.
