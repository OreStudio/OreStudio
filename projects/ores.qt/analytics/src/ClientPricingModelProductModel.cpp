...
int ClientPricingModelProductModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientPricingModelProductModel::data(const QModelIndex& index, int role) const {
    ...
    const auto& product = products_[row];
    ...
    if (role == Qt::ForegroundRole) {
        return recency_foreground_color(product.pricing_engine_type_code);
    }
...
void ClientPricingModelProductModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh pricing model product model: disconnected.";
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug) << "Fetched " << result->products.size()
                                           << " pricing model products, total available: "
...
void ClientPricingModelProductModel::onProductsLoaded() {
    is_fetching_ = false;
    ...
    const int new_count = static_cast<int>(result.products.size());
    ...
    if (new_count > 0) {
        ...
        const bool has_recent = recencyTracker_.update(products_);
        if (has_recent && !pulseManager_->is_pulsing()) {
            pulseManager_->start_pulsing();
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " pricing model products newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " pricing model products."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientPricingModelProductModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
const analytics::domain::pricing_model_product*
ClientPricingModelProductModel::getProduct(int row) const {
    const auto idx = static_cast<std::size_t>(row);
    if (idx >= products_.size())
        return nullptr;
...
void ClientPricingModelProductModel::onPulsingComplete() {
