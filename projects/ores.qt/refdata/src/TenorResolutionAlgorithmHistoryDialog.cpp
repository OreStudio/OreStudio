...
#include "ui_TenorResolutionAlgorithmHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

TenorResolutionAlgorithmHistoryDialog::TenorResolutionAlgorithmHistoryDialog(
    const QString& code, ClientManager* clientManager, QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::TenorResolutionAlgorithmHistoryDialog)
    , code_(code)
...
QString TenorResolutionAlgorithmHistoryDialog::code() const {
    return code_;
}

void TenorResolutionAlgorithmHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for tenor resolution algorithm: "
                               << code_.toStdString();
    ...
    QPointer<TenorResolutionAlgorithmHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](refdata::messaging::get_tenor_resolution_algorithm_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int TenorResolutionAlgorithmHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString TenorResolutionAlgorithmHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow TenorResolutionAlgorithmHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult TenorResolutionAlgorithmHistoryDialog::calculateDiffAt(int ci,
                                                                                     int pi) const {
    DiffResult diffs;
...
void TenorResolutionAlgorithmHistoryDialog::displayFullDetails(int index) {
...
void TenorResolutionAlgorithmHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void TenorResolutionAlgorithmHistoryDialog::revertToVersionAt(int index) {
