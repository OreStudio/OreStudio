...
*Schema upgrade policy:* there is no incremental migration runner; the
supported path for fresh databases is destroying and recreating them
with =compass db recreate -y -k=, and the =create/= scripts carry the
canonical schema. Persistent environments that predate a schema change
may additionally need the matching one-shot =ALTER TABLE= script from
=migration/=, applied by hand — no tool executes those files. A one-shot
migration must be idempotent and guarded (a no-op when the change is
already present) and must document its preconditions in the file
header; the =create/= scripts always carry the final canonical
definition.
