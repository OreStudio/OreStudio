 ...
 */
#ifndef ORES_QT_PARTY_IDENTIFIER_CONTROLLER_HPP
#define ORES_QT_PARTY_IDENTIFIER_CONTROLLER_HPP
...
class PartyIdentifierMdiWindow;
class PartyIdentifierDetailDialog;
class DetachableMdiSubWindow;
class ChangeReasonCache;

/**
 * @brief Controller for managing party identifier windows and operations.
 *
 * Manages the lifecycle of party identifier list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class PartyIdentifierController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.party_identifier_controller";
...
public:
    PartyIdentifierController(QMainWindow* mainWindow,
                              QMdiArea* mdiArea,
...
private slots:
    void onShowDetails(const refdata::domain::party_identifier& partyIdentifier);
...
private:
     ...
     * PartyIdentifierDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(PartyIdentifierDetailDialog* detailDialog);
