...
5. Update ~ores.iam.core~ to ~#include "ores.iam.api/..."~ and add
   ~ores.iam.api~ as a PUBLIC dependency.
6. Update clients (~ores.qt~, ~ores.shell~, ~ores.cli~) to link
   ~ores.iam.api~ instead of ~ores.iam~ (now ~ores.iam.core~).
7. Write serialisation round-trip tests in
   ~ores.iam.api/tests/~: for each protocol type, serialize and
...
~*.core~ or service implementation library. They link only ~*.api~
...
- Should ~ores.{service}.api~ be a shared library (~.lib~) or a
