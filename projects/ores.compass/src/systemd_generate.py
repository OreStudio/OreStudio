...
Reads the codegen service_registry.org model (deployment aspect:
replicas/enabled/depends_on/Extra args) directly -- no DB round-trip --
and renders systemd *concrete* unit files -- one per (service,
environment) pair, fully resolved at generation time from this
checkout's own .env -- so that `systemctl --user start
ores-<env>.target` starts NATS plus every service for that
...
# projects/modeling/service_registry.org (deployment aspect).
...
SERVICE_REGISTRY_PATH = "projects/modeling/service_registry.org"


def load_service_registry(project_root):
    """Load service_registry.org's deployment aspect via org_loader.py --
    one entry per fleet process, DB-role or not."""
    codegen_src = str(project_root / "projects" / "ores.codegen" / "src")
    if codegen_src not in sys.path:
        sys.path.insert(0, codegen_src)
    from codegen.org_loader import load_org_service_registry_model  # noqa: PLC0415

    model = load_org_service_registry_model(project_root / SERVICE_REGISTRY_PATH)
    return model["service_registry"]["services"]


def fetch_service_definitions(services):
    defs = []
    for svc in services:
        args_template = DEFAULT_ARGS_TEMPLATE
        extra_args = svc.get("extra_args", [])
        if extra_args:
            args_template += " " + " ".join(extra_args)
        defs.append({
            "service_name": svc["name"],
            "binary_name": svc["name"],
...
def fetch_dependencies(services):
    return {svc["name"]: svc["depends_on"] for svc in services if svc.get("depends_on")}
