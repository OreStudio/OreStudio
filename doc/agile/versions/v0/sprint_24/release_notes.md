...
-   **Market data notation: design an ORE Studio canonical URN mapping to ORE/Bloomberg/Reuters**: Our IR curve config/feed identifiers (`currency_code=+=index_name` qualifier) don't model ORE's real addressing scheme &#x2013; an index name, a `Yield/CCY/CURVE_ID` curve key, and a `TYPE/SUBTYPE/dim` raw quote key are three distinct, non-interchangeable key types, unlike FX's single explicit `ore_key`. Surveyed ORE, Bloomberg, and Reuters notations and designed *oresmd: ORE Studio Market Data URI*, a canonical ORE Studio URN that projects into each. See also *Market Data Identifier*, *ORE market data catalogue*, *Market Data Requirement*, and *External market data identifiers* (the Bloomberg/RIC/MDDL/FIX comparative survey).
