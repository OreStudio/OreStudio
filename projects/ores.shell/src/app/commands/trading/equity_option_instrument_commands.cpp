...
void equity_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_equity_option_instrument(std::ref(out),
                                                 std::ref(session),
                                                 ...
                                                 std::move(cliquet_frequency),
                                                 std::move(description),
                                                 std::move(change_reason_code),
                                                 std::move(change_commentary));
        },
        "Add an Equity option instrument (trade_type_code underlying_name currency notional "
...
void equity_option_instrument_commands::process_get_equity_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity option instruments request.";
...
void equity_option_instrument_commands::process_add_equity_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an Equity option instrument." << std::endl;
        return;
    }
    ...
    v.cliquet_frequency = (cliquet_frequency == "-") ? "" : std::move(cliquet_frequency);
...
void equity_option_instrument_commands::process_delete_equity_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an Equity option instrument." << std::endl;
        return;
    }
