...
constexpr std::string_view app_version_event_name =
    eventing::domain::event_traits<compute::eventing::app_version_changed_event>::name;
}

AppVersionController::AppVersionController(QMainWindow* mainWindow,
                                           QMdiArea* mdiArea,
                                           ClientManager* clientManager,
                                           ChangeReasonCache* changeReasonCache,
                                           const QString& username,
                                           QObject* parent)
    : EntityController(mainWindow, mdiArea, clientManager, username, app_version_event_name, parent)
...
void AppVersionController::showListWindow() {
    ...
    // Connect signals
    connect(listWindow_,
            &AppVersionMdiWindow::statusChanged,
            this,
            &AppVersionController::statusMessage);
    connect(listWindow_,
            &AppVersionMdiWindow::errorOccurred,
            this,
            &AppVersionController::errorMessage);
    connect(listWindow_,
            &AppVersionMdiWindow::showVersionDetails,
            this,
            &AppVersionController::onShowDetails);
    connect(listWindow_,
            &AppVersionMdiWindow::addNewRequested,
            this,
            &AppVersionController::onAddNewRequested);
    connect(listWindow_,
            &AppVersionMdiWindow::showVersionHistory,
            this,
            &AppVersionController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<AppVersionController>(this), key]() {
                if (!self)
                    return;
...
void AppVersionController::onShowDetails(const compute::domain::app_version& app_version) {
...
void AppVersionController::showAddWindow() {
    ...
    connect(detailDialog,
            ...
            this,
            [self = QPointer<AppVersionController>(this)](const QString& code) {
                if (!self)
                    return;
...
void AppVersionController::showDetailWindow(const compute::domain::app_version& app_version) {
    ...
    connect(detailDialog,
            &AppVersionDetailDialog::app_versionDeleted,
            this,
            [self = QPointer<AppVersionController>(this), key](const QString& code) {
                if (!self)
                    return;
...
void AppVersionController::showHistoryWindow(const compute::domain::app_version& app_version) {
...
void AppVersionController::onOpenVersion(const compute::domain::app_version& app_version,
                                         int versionNumber) {
    ...
    detailWindow->setWindowTitle(
        QString("App Version: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void AppVersionController::onRevertVersion(const compute::domain::app_version& app_version) {
    ...
    connect(detailDialog,
            ...
            [self = QPointer<AppVersionController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("App Version '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert App Version: %1").arg(QString::fromStdString(app_version.wrapper_version)));
