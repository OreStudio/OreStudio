...
QString PartyStatusHistoryDialog::code() const {
    return code_;
}
...
void PartyStatusHistoryDialog::loadHistory() {
    ...
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_party_status_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
HistoryDialogBase::DiffResult PartyStatusHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
