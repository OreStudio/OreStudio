...
A content-deduplication pass across all [[proj:external/ore/examples][=external/ore/examples=]] identifies
...
- [X] Roundtrip test: every datum in the parsed output of =market_20160205.txt=
  survives serialize → re-parse with identical field values
- [X] Roundtrip test: same for each fixings fixture file
- [X] Parser rejects lines with fewer than 3 whitespace-delimited tokens with a
