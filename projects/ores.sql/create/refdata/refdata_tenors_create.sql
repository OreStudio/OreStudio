...
create table if not exists "ores_refdata_tenors_tbl" (
    "code" text not null,
...
on "ores_refdata_tenors_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists tenors_code_uniq_idx
on "ores_refdata_tenors_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists tenors_tenant_idx
on "ores_refdata_tenors_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_tenors_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_refdata_tenors_tbl"
...
create or replace trigger ores_refdata_tenors_insert_trg
before insert on "ores_refdata_tenors_tbl"
for each row execute function ores_refdata_tenors_insert_fn();

create or replace rule ores_refdata_tenors_delete_rule as
on delete to "ores_refdata_tenors_tbl" do instead (
    update "ores_refdata_tenors_tbl"
    set valid_to = clock_timestamp()
