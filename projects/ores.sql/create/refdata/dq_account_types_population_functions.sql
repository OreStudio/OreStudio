 ...
 *   SELECT * FROM ores_dq_preview_account_type_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_account_types(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_account_type_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_account_types_tbl existing
...
/**
 * Populate ores_refdata_account_types_tbl from a DQ account_types dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_account_types(
    p_dataset_id uuid,
