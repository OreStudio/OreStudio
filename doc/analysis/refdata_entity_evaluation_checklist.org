...
#+description: Per-layer criteria for evaluating whether an ores.refdata entity is fully commissioned. Each criterion shows whether it applies to domain entities, auxiliary (lookup) types, or both, plus what to look for.
#+type: knowledge
#+level: cross
...
A standard =ores.refdata= entity spans twelve layers: DB, codegen, domain, repository,
service, messaging (JSON IO), Qt UI, shell REPL, CLI, HTTP routes, Wt web UI, and manual.
Each layer has a fixed set of criteria that determines whether the entity is fully
commissioned.
...
Use alongside the [[id:A3F8C2D1-7E4B-4F9A-B2C5-8D1E6A3F0B7C][Entity Coverage Matrix]] when commissioning or auditing any ores.refdata
entity. The =currency= entity (with its three auxiliaries) is the reference specimen.
