...
| Now          | Flatten fix implemented and committed; awaiting CI green.        |
| Waiting on   | Windows CI (PR push).                                            |
| Next         | Push to PR and verify CI passes.                                 |
...
| PR                                                                          | Title                                                                          |
|-----------------------------------------------------------------------------+--------------------------------------------------------------------------------|
| [[https://github.com/OreStudio/OreStudio/pull/1031][#1031]] | [swap_leg] Decompose into sub-structs via rfl::Flatten to fix MSVC C1202 |

* Decisions

- Chose Option A (rfl::Flatten decomposition) as planned.
- swap_leg_entity.hpp (repository layer) was not changed — it has its own flat
  entity struct and the mapper bridges them; no rfl exposure there.
- parse_swap_instruments.cpp audited: reads swap_leg via swap_legs_outer surrogate
  struct, not directly — no change needed.
- instrument.hpp stamp_ids template patched with if constexpr to dispatch
  differently for swap_leg (nested) vs composite_leg (flat).
...
Linux debug build clean (exit 0) with commit =c3ffa4533=. Awaiting CI push.
