 ...
 * straight through.
 */
std::vector<domain::party_counterparty> read_one(repository::party_counterparty_repository& repo,
                                                 const messaging::party_counterparty_key& key) {
    return repo.read_latest(key.party_id, key.counterparty_id);
}
 ...
 */
messaging::party_counterparty_key key_from(const domain::party_counterparty& v) {
    messaging::party_counterparty_key key;
    key.party_id = v.party_id;
    key.counterparty_id = v.counterparty_id;
    return key;
}
 ...
 */
domain::party_counterparty to_domain(const messaging::party_counterparty_write& write) {
    domain::party_counterparty v;
...
messaging::list_party_counterparties_response party_counterparty_service::list_party_counterparties(
    ...
    messaging::list_party_counterparties_response response;
    ...
    response.party_counterparties = repo_.read_latest(request.offset, request.limit);
    response.total = repo_.get_total_party_counterparty_count();
    return response;
}

messaging::list_by_party_id_party_counterparties_response
party_counterparty_service::list_by_party_id_party_counterparties(
    const messaging::list_by_party_id_party_counterparties_request& request) {
    messaging::list_by_party_id_party_counterparties_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
    ...
    response.total = repo_.get_total_party_counterparty_count_by_party(request.party_id);
    return response;
}
...
messaging::get_party_counterparty_response party_counterparty_service::get_party_counterparty(
    ...
    response.party_counterparty = std::move(found.front());
    return response;
}

messaging::get_many_party_counterparties_response
party_counterparty_service::get_many_party_counterparties(
    const messaging::get_many_party_counterparties_request& request) {
    messaging::get_many_party_counterparties_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_counterparty_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, k);
        if (!found.empty())
            entry.party_counterparty = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_party_counterparty_response party_counterparty_service::put_party_counterparty(
    const messaging::put_party_counterparty_request& request) {
    messaging::put_party_counterparty_response response;
    domain::party_counterparty value;
    ...
    if (!written.empty())
        response.party_counterparty = std::move(written.front());
    return response;
}

messaging::put_many_party_counterparties_response
party_counterparty_service::put_many_party_counterparties(
    const messaging::put_many_party_counterparties_request& request) {
    messaging::put_many_party_counterparties_response response;
    ...
    response.party_counterparties.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, key_from(value));
        response.party_counterparties.push_back(written.empty() ? value :
                                                                  std::move(written.front()));
    }
    return response;
}

messaging::delete_party_counterparty_response party_counterparty_service::delete_party_counterparty(
    const messaging::delete_party_counterparty_request& request) {
    messaging::delete_party_counterparty_response response;
    ...
    switch (
        repo_.remove(request.removal.key.party_id, request.removal.key.counterparty_id, expected)) {
        case repository::party_counterparty_repository::remove_status::removed:
            break;
        case repository::party_counterparty_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_counterparty_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_counterparty_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_party_counterparties_response
party_counterparty_service::delete_many_party_counterparties(
    const messaging::delete_many_party_counterparties_request& request) {
    messaging::delete_many_party_counterparties_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
    ...
    std::vector<boost::uuids::uuid> counterparty_id_keys;
    counterparty_id_keys.reserve(request.removals.size());
    for (const auto& removal : request.removals)
        counterparty_id_keys.push_back(removal.key.counterparty_id);
    repo_.remove(party_id_keys, counterparty_id_keys);
    return response;
}

ores::utility::domain::result
party_counterparty_service::prepare_change(const messaging::party_counterparty_change& change,
    ...
    stamp_party_counterparty(out, ctx_);
