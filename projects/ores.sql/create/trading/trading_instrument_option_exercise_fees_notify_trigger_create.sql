...
create or replace function ores_trading_instrument_option_exercise_fees_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    perform pg_notify('ores_trading_instrument_option_exercise_fees', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_trading_instrument_option_exercise_fees_notify_trg
after insert or update or delete on ores_trading_instrument_option_exercise_fees_tbl
for each row execute function ores_trading_instrument_option_exercise_fees_notify_fn();
