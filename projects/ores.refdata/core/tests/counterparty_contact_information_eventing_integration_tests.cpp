...
// notification" pattern end to end for counterparty_contact_information -- the
...
TEST_CASE("write_counterparty_contact_information_publishes_nats_changed_event", tags) {
    ...
    auto sub =
        bus.subscribe<ores::refdata::eventing::counterparty_contact_information_changed_event>(
            [&nats](
                const ores::refdata::eventing::counterparty_contact_information_changed_event& e) {
                ev::service::publish_entity_event(
                    nats,
                    std::string(ev::domain::event_traits<
                                ores::refdata::eventing::
                                    counterparty_contact_information_changed_event>::name),
    ...
    event_source
        .register_mapping<ores::refdata::eventing::counterparty_contact_information_changed_event>(
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<
                    ores::refdata::eventing::counterparty_contact_information_changed_event>::name),
        10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.counterparty_contact_information") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for counterparty_contact_information "
    ...
    BOOST_LOG_SEV(lg, info)
        ...
        << " matching NATS notification(s) for counterparty_contact_information " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::counterparty_contact_information_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_counterparty_contact_information_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_counterparty_contact_information(id_str);
        ...
        REQUIRE_FALSE(svc.get_counterparty_contact_information(id_str).has_value());
        REQUIRE(svc.get_counterparty_contact_information_history(id_str).size() == versions.size());
    }
}
