...
- Bad :: "This component uses NATS and PostgreSQL."
- Good :: "=ores.refdata.core= persists reference-data entities (currency,
  country, rounding types) and answers CRUD requests over NATS. Services,
  clients and scripts reach that data through it, never through the
  database directly."
...
For a client library: NATS request messages sent. For a service:
persisted records, NATS response messages, emitted events.

*** Entry points

Key files a developer opens first. Keep to 3–6 items. Prefer:
- The main plugin/service class header
- The aggregate include header (=ores.<comp>.hpp=), which every component keeps
  for its outermost namespace; it carries the namespace =@brief= that doxygen
  renders and that the diagram conventions read when they label the component.
  A multi-module component keeps one per module instead.
...
transitive dependencies (=ores.logging=, =Boost=) unless there is a specific
...
empty. Link to:
- Sibling components (=ores.<domain>.api= above a =*.core=; =*.service=
  above a =*.core=)
...
- /See also left empty./ At minimum, link from a =*.core= component to the
  =*.api= it consumes, and back from the API to its consumers.
...
- [[id:D00BEA0D-E501-C534-A013-E6F40C1A6097][ores.iam.core]] — example of a complete component overview with a well-populated See also section.
