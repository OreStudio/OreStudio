 ...
 */
std::vector<domain::ois_convention> read_one(repository::ois_convention_repository& repo,
                                             const ores::database::context& ctx,
                                             const messaging::ois_convention_key& key) {
 ...
 */
messaging::ois_convention_key key_from(const domain::ois_convention& v) {
 ...
 */
domain::ois_convention to_domain(const messaging::ois_convention_write& write) {
    domain::ois_convention v;
    v.id = write.id;
    v.spot_lag = write.spot_lag;
    v.index = write.index;
    v.fixed_day_count_fraction = write.fixed_day_count_fraction;
    v.fixed_calendar = write.fixed_calendar;
    v.payment_lag = write.payment_lag;
    v.end_of_month = write.end_of_month;
    v.fixed_frequency = write.fixed_frequency;
    v.fixed_convention = write.fixed_convention;
    v.fixed_payment_convention = write.fixed_payment_convention;
    v.rule = write.rule;
...
messaging::list_ois_conventions_response ois_convention_service::list_ois_conventions(
    ...
    messaging::list_ois_conventions_response response;
    ...
    response.ois_conventions = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_ois_convention_count(ctx_);
    return response;
}

messaging::get_ois_convention_response
ois_convention_service::get_ois_convention(const messaging::get_ois_convention_request& request) {
    messaging::get_ois_convention_response response;
    ...
    response.ois_convention = std::move(found.front());
    return response;
}

messaging::get_many_ois_conventions_response ois_convention_service::get_many_ois_conventions(
    const messaging::get_many_ois_conventions_request& request) {
    messaging::get_many_ois_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::ois_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.ois_convention = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_ois_convention_response
ois_convention_service::put_ois_convention(const messaging::put_ois_convention_request& request) {
    messaging::put_ois_convention_response response;
    domain::ois_convention value;
    ...
    if (!written.empty())
        response.ois_convention = std::move(written.front());
    return response;
}

messaging::put_many_ois_conventions_response ois_convention_service::put_many_ois_conventions(
    const messaging::put_many_ois_conventions_request& request) {
    messaging::put_many_ois_conventions_response response;
    std::vector<domain::ois_convention> batch;
    ...
    response.ois_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.ois_conventions.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_ois_convention_response ois_convention_service::delete_ois_convention(
    const messaging::delete_ois_convention_request& request) {
    messaging::delete_ois_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::ois_convention_repository::remove_status::removed:
            break;
        case repository::ois_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::ois_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::ois_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_ois_conventions_response ois_convention_service::delete_many_ois_conventions(
    const messaging::delete_many_ois_conventions_request& request) {
    messaging::delete_many_ois_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_ois_convention_versions_response
ois_convention_service::list_ois_convention_versions(
    const messaging::list_ois_convention_versions_request& request) {
    messaging::list_ois_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_ois_convention_version_response ois_convention_service::get_ois_convention_version(
    const messaging::get_ois_convention_version_request& request) {
    messaging::get_ois_convention_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.ois_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ois_convention_service::prepare_change(const messaging::ois_convention_change& change,
                                       const ores::utility::domain::change_intent& intent,
                                       domain::ois_convention& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::ois_convention>
ois_convention_service::get_ois_conventions(const std::vector<std::string>& ids) {
