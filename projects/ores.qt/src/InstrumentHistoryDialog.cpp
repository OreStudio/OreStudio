...
#include "ui_InstrumentHistoryDialog.h"
...
InstrumentHistoryDialog::InstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::InstrumentHistoryDialog),
      id_(id),
...
void InstrumentHistoryDialog::setupUi() {
...
void InstrumentHistoryDialog::setupToolbar() {
...
void InstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &InstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &InstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &InstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void InstrumentHistoryDialog::loadHistory() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        emit errorOccurred("Not connected to server");
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Loading history for instrument: " << id_.toStdString();
    emit statusChanged(tr("Loading history..."));

    QPointer<InstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_instrument_history_request request;
        request.id = id;
...
void InstrumentHistoryDialog::updateVersionList() {
...
void InstrumentHistoryDialog::onVersionSelected() {
...
void InstrumentHistoryDialog::updateChangesTable(int currentVersionIndex) {
...
void InstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->tradeTypeCodeValue->setText(
        QString::fromStdString(version.trade_type_code));
    ui_->notionalValue->setText(
        QString::number(version.notional, 'f', 2));
    ui_->currencyValue->setText(QString::fromStdString(version.currency));
    ui_->startDateValue->setText(QString::fromStdString(version.start_date));
    ui_->maturityDateValue->setText(
        QString::fromStdString(version.maturity_date));
    ui_->descriptionValue->setText(
        QString::fromStdString(version.description));
...
void InstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void InstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void InstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
