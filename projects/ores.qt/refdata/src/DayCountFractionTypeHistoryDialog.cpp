...
QString DayCountFractionTypeHistoryDialog::code() const {
    return code_;
}
...
void DayCountFractionTypeHistoryDialog::loadHistory() {
    ...
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](refdata::messaging::get_day_count_fraction_type_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
HistoryDialogBase::DiffResult DayCountFractionTypeHistoryDialog::calculateDiffAt(int ci,
                                                                                 int pi) const {
    DiffResult diffs;
