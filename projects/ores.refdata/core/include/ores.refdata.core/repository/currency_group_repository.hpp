 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CURRENCY_GROUP_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CURRENCY_GROUP_REPOSITORY_HPP
 ...
 */
class ORES_REFDATA_CORE_EXPORT currency_group_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.currency_group_repository";
...
public:
    ...
    /**
     * @brief Writes currency groups to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::currency_group& v);
    void write(context ctx, const std::vector<domain::currency_group>& v);
    /**@}*/

    /**
     * @brief Writes a currency group, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of currency groups, each honouring its own
    ...
    /**
     * @brief Reads latest currency groups, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::currency_group> read_latest(context ctx);
     ...
     */
    std::vector<domain::currency_group> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single currency group as it stood at a specific
    ...
    /**
     * @brief Reads latest currency groups with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_group_count(context ctx);

    /**
     * @brief Deletes a currency group by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a currency group, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes currency groups by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::currency_group& v);
     ...
     */
    domain::currency_group apply_claim(context ctx,
                                       const domain::currency_group& v,
                                       const ores::utility::domain::precondition& claim);
};
