...
std::string grid_sample_repository::sql() {
    return generate_create_table_sql<grid_sample_entity>(lg());
}

ores::utility::domain::precondition
grid_sample_repository::replace_claim(context ctx, const domain::grid_sample& v) {
...
domain::grid_sample grid_sample_repository::apply_claim(
    context ctx, const domain::grid_sample& v, const ores::utility::domain::precondition& claim) {
    ...
    if (claim.kind == precondition_kind::must_match_version)
        throw std::invalid_argument(
            "grid_sample_repository::write: this table keeps no version to match");
    if (claim.kind == precondition_kind::must_not_exist &&
        !read_latest(ctx,
                     boost::uuids::to_string(v.id),
                     ores::platform::time::datetime::to_db_string(v.sampled_at))
             .empty())
        throw std::invalid_argument("grid_sample_repository::write: a current row already exists");
    return t;
}

void grid_sample_repository::write(context ctx, const domain::grid_sample& v) {
    write(ctx, v, replace_claim(ctx, v));
}

void grid_sample_repository::write(context ctx, const std::vector<domain::grid_sample>& v) {
...
void grid_sample_repository::write(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Writing grid samples. Count: " << v.size();
    std::vector<domain::grid_sample> batch;
...
std::vector<domain::grid_sample> grid_sample_repository::read_latest(context ctx) {
    ...
    return execute_read_query<grid_sample_entity, domain::grid_sample>(
        ctx,
        query,
        [](const auto& entities) { return grid_sample_mapper::map(entities); },
        lg(),
        "Reading latest grid samples");
}
...
std::vector<domain::grid_sample> grid_sample_repository::read_latest(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<grid_sample_entity>> |
...
std::vector<domain::grid_sample> grid_sample_repository::read_all(context ctx,
                                                                  const std::string& id,
...
grid_sample_repository::remove_status
grid_sample_repository::remove(context ctx,
                               const std::string& id,
                               const std::string& sampled_at,
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<grid_sample_entity> |
                       where("tenant_id"_c == tid && "id"_c == id && "sampled_at"_c == sampled_at);
...
void grid_sample_repository::remove(context ctx,
                                    const std::string& id,
                                    const std::string& sampled_at) {
...
grid_sample_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest grid samples with offset: " << offset
                               << " and limit: " << limit;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<grid_sample_entity>> | where("tenant_id"_c == tid) |
...
std::optional<domain::grid_sample> grid_sample_repository::read_newest(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Reading newest sample";
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<grid_sample_entity>> | where("tenant_id"_c == tid) |
                       order_by("sampled_at"_c.desc()) | sqlgen::limit(1);
    const auto rows = execute_read_query<grid_sample_entity, domain::grid_sample>(
        ctx,
        query,
        [](const auto& entities) { return grid_sample_mapper::map(entities); },
        lg(),
        "Reading newest sample.");
    if (rows.empty())
        return std::nullopt;
    return rows.front();
}
...
std::vector<domain::grid_sample> grid_sample_repository::read_latest(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::read<std::vector<grid_sample_entity>> |
        where("tenant_id"_c == tid && "id"_c.in(ids) && "sampled_at"_c.in(sampled_ats));
    auto result = execute_read_query<grid_sample_entity, domain::grid_sample>(
        ctx,
        query,
        [](const auto& entities) { return grid_sample_mapper::map(entities); },
        lg(),
        "Reading latest grid samples by ids.");
    ...
    if (sampled_ats.size() != ids.size())
        throw std::invalid_argument(
            "grid_sample_repository::read_latest: key column vectors must be the same length");
    std::set<std::tuple<std::string, std::string>> requested;
    for (std::size_t i = 0; i < ids.size(); ++i)
        requested.emplace(ids[i], sampled_ats[i]);
    std::vector<domain::grid_sample> filtered;
...
void grid_sample_repository::remove(context ctx,
    ...
    if (sampled_ats.size() != ids.size())
        throw std::invalid_argument(
            "grid_sample_repository::remove: key column vectors must be the same length");
    for (std::size_t i = 0; i < ids.size(); ++i)
