...
// notification" pattern end to end for counterparty_identifier -- the
...
TEST_CASE("write_counterparty_identifier_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::counterparty_identifier_changed_event>(
        [&nats](const ores::refdata::eventing::counterparty_identifier_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::counterparty_identifier_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::counterparty_identifier_changed_event>(
        "ores.refdata.counterparty_identifier", "ores_refdata_counterparty_identifiers");
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.counterparty_identifier") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for counterparty_identifier " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for counterparty_identifier "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::counterparty_identifier_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_counterparty_identifier_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_counterparty_identifier(id_str);
        ...
        REQUIRE_FALSE(svc.get_counterparty_identifier(id_str).has_value());
        REQUIRE(svc.get_counterparty_identifier_history(id_str).size() == versions.size());
    }
}
