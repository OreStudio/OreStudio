...
create or replace function ores_trading_instrument_option_premiums_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    entity_name text := 'ores.trading.instrument_option_premium';
...
begin
    ...
    perform pg_notify('ores_trading_instrument_option_premiums', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_trading_instrument_option_premiums_notify_trg
after insert or update or delete on ores_trading_instrument_option_premiums_tbl
for each row execute function ores_trading_instrument_option_premiums_notify_fn();
