 ...
 *   SELECT * FROM ores_dq_preview_asset_measure_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_asset_measures(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_asset_measure_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_asset_measures_tbl existing
...
/**
 * Populate ores_refdata_asset_measures_tbl from a DQ asset_measures dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_asset_measures(
    p_dataset_id uuid,
