...
/**
 * @brief Model for displaying catalogs fetched from the server.
 *
 * This model extends AbstractClientModel and fetches catalog
 * data asynchronously using the ores.comms client.
 */
class ClientCatalogModel final : public AbstractClientModel {
    Q_OBJECT
...
public:
     ...
     */
    enum Column { Name, Description, Owner, Version, ModifiedBy, RecordedAt, ColumnCount };

    explicit ClientCatalogModel(ClientManager* clientManager, QObject* parent = nullptr);
    ~ClientCatalogModel() override = default;
    ...
    /**
     * @brief Refresh catalog data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get catalog at the specified row.
     *
     * @param row The row index.
     * @return The catalog, or nullptr if row is invalid.
     */
    const dq::domain::catalog* getCatalog(int row) const;
...
private:
    ...
    void fetch_catalogs(std::uint32_t offset, std::uint32_t limit);
