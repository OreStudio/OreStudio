 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_TENOR_KIND_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_TENOR_KIND_REPOSITORY_HPP
...
/**
 * @brief Reads and writes tenor kinds to data storage.
 */
class ORES_REFDATA_CORE_EXPORT tenor_kind_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.tenor_kind_repository";
...
public:
    ...
    /**
     * @brief Writes tenor kinds to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::tenor_kind& v);
    void write(context ctx, const std::vector<domain::tenor_kind>& v);
    /**@}*/

    /**
     * @brief Writes a tenor kind, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of tenor kinds, each honouring its own
    ...
    /**
     * @brief Reads latest tenor kinds, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::tenor_kind> read_latest(context ctx);
    std::vector<domain::tenor_kind> read_latest(context ctx, const std::string& code);
    std::vector<domain::tenor_kind> read_latest(context ctx, const std::vector<std::string>& codes);
    /**@}*/


    /**
     * @brief Reads all tenor kinds, possibly filtered by primary key.
     */
    std::vector<domain::tenor_kind> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single tenor kind as it stood at a specific
     ...
     */
    std::uint32_t get_total_kind_count(context ctx);

    /**
     * @brief Deletes a tenor kind by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a tenor kind, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes tenor kinds by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::tenor_kind& v);
     ...
     */
    domain::tenor_kind apply_claim(context ctx,
                                   const domain::tenor_kind& v,
                                   const ores::utility::domain::precondition& claim);
};
