...
render_tenor_schedule_fields(const domain::tenor_schedule& v) {
    ...
    fields.push_back({.name = "Schedule Source", .value = v.schedule_source});
    fields.push_back({.name = "Calendar Code", .value = v.calendar_code.value_or(std::string{})});
    fields.push_back(
        {.name = "Diary Entry Type", .value = v.diary_entry_type.value_or(std::string{})});
