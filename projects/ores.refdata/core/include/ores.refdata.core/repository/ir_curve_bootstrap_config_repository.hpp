 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_IR_CURVE_BOOTSTRAP_CONFIG_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_IR_CURVE_BOOTSTRAP_CONFIG_REPOSITORY_HPP
...
/**
 * @brief Reads and writes IR curve bootstrap configs to data storage.
 */
class ORES_REFDATA_CORE_EXPORT ir_curve_bootstrap_config_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.ir_curve_bootstrap_config_repository";
...
public:
    ...
    /**
     * @brief Writes IR curve bootstrap configs to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::ir_curve_bootstrap_config& v);
    void write(context ctx, const std::vector<domain::ir_curve_bootstrap_config>& v);
    /**@}*/

    /**
     * @brief Writes a IR curve bootstrap config, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of IR curve bootstrap configs, each honouring its own
    ...
    /**
     * @brief Reads latest IR curve bootstrap configs, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::ir_curve_bootstrap_config> read_latest(context ctx);
     ...
     */
    std::vector<domain::ir_curve_bootstrap_config> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single IR curve bootstrap config as it stood at a specific
     ...
     */
    std::uint32_t get_total_bootstrap_config_count(context ctx);

    /**
     * @brief Deletes a IR curve bootstrap config by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a IR curve bootstrap config, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes IR curve bootstrap configs by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::ir_curve_bootstrap_config apply_claim(context ctx,
                                                  const domain::ir_curve_bootstrap_config& v,
                                                  const ores::utility::domain::precondition& claim);
};
