...
// notification" pattern end to end for day_count_fraction_type -- the
...
using ores::refdata::domain::day_count_fraction_type;
using ores::refdata::repository::day_count_fraction_type_repository;
...
TEST_CASE("write_day_count_fraction_type_publishes_nats_changed_event", tags) {
    ...
    auto v = generate_synthetic_day_count_fraction_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Day Count Fraction Type: " << v;

    day_count_fraction_type_repository repo;
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for day_count_fraction_type " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for day_count_fraction_type "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::day_count_fraction_type_service svc(crud_ctx);
