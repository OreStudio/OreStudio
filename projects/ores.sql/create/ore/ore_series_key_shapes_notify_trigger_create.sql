...
create or replace function ores_ore_series_key_shapes_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    changed_key := jsonb_build_object('series_type', changed_series_type);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.ore.series_key_shape',
    ...
    perform pg_notify('ores_ore_series_key_shapes', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_ore_series_key_shapes_notify_trg
after insert or update or delete on ores_ore_series_key_shapes_tbl
for each row execute function ores_ore_series_key_shapes_notify_fn();
