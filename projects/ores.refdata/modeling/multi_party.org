...
#+begin_src sql
CREATE POLICY party_isolation ON ores_refdata_counterparty_relationships_tbl
  USING (
...
| State                     | Display                        | Style          |
|---------------------------+--------------------------------+----------------|
| System party active       | =Party: <name> [System]=       | Normal chip    |
| Operational party active  | =Party: <name>=                | Normal chip    |
| No party (misconfigured)  | =Party: [No Party]=            | Warning chip   |
...
shown at login to alert the user to the misconfiguration.
