...
create unique index if not exists catalogs_version_uniq_idx
on "ores_dq_catalogs_tbl" (tenant_id, name, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists catalogs_name_uniq_idx
...
-- =============================================================================
-- Validation function for catalog
-- Validates that a name exists in the catalogs table.
-- Returns the validated value, or default if null/empty.
-- Validates against the tenant's own data only.
-- =============================================================================
create or replace function ores_dq_validate_catalog_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid catalog: value cannot be null or empty'
            using errcode = '23502';
    end if;

    -- Allow pass-through if this tenant has no active catalogs yet.
    if not exists (
    ...
    if not exists (
        ...
        raise exception 'Invalid catalog: %. Must be one of: %', p_value, (
