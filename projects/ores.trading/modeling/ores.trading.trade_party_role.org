...
#+description: Codegen entity model for the trading =trade_party_role= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
:error_message: Invalid trade_id: %. Trade must exist for tenant.
:END:
