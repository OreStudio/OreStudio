 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CALENDAR_EXCEPTION_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CALENDAR_EXCEPTION_REPOSITORY_HPP
...
/**
 * @brief Reads and writes calendar exceptions to data storage.
 */
class ORES_REFDATA_CORE_EXPORT calendar_exception_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.calendar_exception_repository";
...
public:
    ...
    /**
     * @brief Writes calendar exceptions to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::calendar_exception& v);
    void write(context ctx, const std::vector<domain::calendar_exception>& v);
    /**@}*/

    /**
     * @brief Writes a calendar exception, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::calendar_exception& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of calendar exceptions, each honouring its own
    ...
    /**
     * @brief Reads latest calendar exceptions, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::calendar_exception> read_latest(context ctx);
     ...
     */
    std::vector<domain::calendar_exception> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single calendar exception as it stood at a specific
    ...
    /**
     * @brief Reads calendar exceptions filtered by calendar_code that were live at
     * any point during [valid_from_bound, valid_to_bound) — i.e. the set of
     * calendar exceptions that compose a parent entity's state as of one of
    ...
    /**
     * @brief Reads latest calendar exceptions with pagination support.
     * @param ctx Repository context with database connection
     * @param offset Number of records to skip
     * @param limit Maximum number of records to return
     */
    ...
    /**
     * @brief Removes a calendar exception, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes calendar exceptions by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::calendar_exception apply_claim(context ctx,
                                           const domain::calendar_exception& v,
                                           const ores::utility::domain::precondition& claim);
};
