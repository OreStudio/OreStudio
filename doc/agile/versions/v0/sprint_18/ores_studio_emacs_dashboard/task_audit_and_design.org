...
=.el= file under [[proj:projects/ores.lisp][=projects/ores.lisp/=]] as REUSABLE, PARTIALLY
...
| =ores-prodigy.el=         | SUPERSEDED          | Services are now managed entirely via [[proj:build/scripts][=build/scripts/=]] shell scripts. Prodigy is not needed; the dashboard  |
...
2. *Environment* — buttons: Init environment (=build/scripts/init-environment.sh=
   ...
   =.env.old=.
3. *Database* — buttons: Recreate env ([[proj:projects/ores.sql/recreate_env.sh][=projects/ores.sql/recreate_env.sh=]] —
   resets schema and data for the checkout) · Browse databases · SQL
   connections.
4. *Services* — buttons invoking [[proj:build/scripts][=build/scripts/=]] shell scripts directly:
...
5. *Compass* — buttons: =compass where= · =compass list= · =compass add= · open
   [[proj:doc/compass.org][=doc/compass.org=]].
6. *Build* — buttons: Configure · Build · Test.
7. *Site* — buttons:
   - Rebuild site → =cmake --build --preset $ORES_PRESET --target deploy_site=
   - Start site → [[proj:build/scripts/serve-site.sh][=build/scripts/serve-site.sh=]] (stops any prior instance, then serves)
...
8. *Skills* — link: open [[proj:doc/llm/skills][=doc/llm/skills/=]] in =dired= · button: Deploy skills
...
[[proj:doc/samples][=doc/samples/=]] contains UI reference screenshots only; no ORE Studio logo
...
| PR                                                                                 | Title                                                           |
|------------------------------------------------------------------------------------+-----------------------------------------------------------------|
| [[https://github.com/OreStudio/OreStudio/pull/828][#828]] | [agile] Audit ores.lisp modules and design dashboard architecture |
