 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_DEPOSIT_CONVENTION_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_DEPOSIT_CONVENTION_REPOSITORY_HPP
 ...
 */
class ORES_REFDATA_CORE_EXPORT deposit_convention_repository {
private:
...
public:
    ...
    /**
     * @brief Writes deposit conventions to database.
     *
    ...
    /**
     * @brief Writes a deposit convention, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of deposit conventions, each honouring its own
    ...
    /**
     * @brief Reads latest deposit conventions, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::deposit_convention> read_latest(context ctx);
    std::vector<domain::deposit_convention> read_latest(context ctx, const std::string& id);
    std::vector<domain::deposit_convention> read_latest(context ctx,
    ...
    /**
     * @brief Reads all deposit conventions, possibly filtered by primary key.
     */
    std::vector<domain::deposit_convention> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single deposit convention as it stood at a specific
    ...
    /**
     * @brief Reads latest deposit conventions with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_deposit_convention_count(context ctx);

    /**
     * @brief Deletes a deposit convention by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a deposit convention, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes deposit conventions by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::deposit_convention apply_claim(context ctx,
                                           const domain::deposit_convention& v,
                                           const ores::utility::domain::precondition& claim);
};
