...
| =parse_test_results.py= | Parses Catch2 XML test-result files; extracts per-suite statistics and failure details. | Python; shell wrapper: =parse_test_results.sh=. Used by the [[id:F6E88DCC-9240-4632-A9A3-9F36F79C9262][How do I monitor a PR until green?]] recipe. |
| =parse_test_results.sh= | Shell wrapper for =parse_test_results.py=; runs the parser from the repo root with standard arguments. | Delegates to =parse_test_results.py=. |
