...
register_monetary_nature_event_mapping(ev::service::postgres_event_source& event_source,
                                       ev::service::event_bus& event_bus,
                                       ores::nats::service::client& nats) {
    // The trigger publishes on the table's own channel, and the mapping turns
    // what it says into this entity's event.
    event_source.register_entity_event_mapping<refdata::messaging::monetary_nature_event>(
        "ores_refdata_monetary_natures");

    return event_bus.subscribe<refdata::messaging::monetary_nature_event>(
        [&nats](const refdata::messaging::monetary_nature_event& e) {
            ...
            ev::service::publish_entity_event(
                nats,
                ev::domain::event_subject<refdata::messaging::monetary_nature_event>(e.action),
                e);
        });
}
