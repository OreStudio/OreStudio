...
#include "ui_MarketDataGenerationConfigHistoryDialog.h"
...
MarketDataGenerationConfigHistoryDialog::MarketDataGenerationConfigHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::MarketDataGenerationConfigHistoryDialog)
    , id_(id)
    , code_(code)
...
QString MarketDataGenerationConfigHistoryDialog::code() const {
    return code_;
}

void MarketDataGenerationConfigHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for market data generation config: "
                               << code_.toStdString();
    ...
    QPointer<MarketDataGenerationConfigHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](synthetic::messaging::get_market_data_generation_config_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int MarketDataGenerationConfigHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString MarketDataGenerationConfigHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow MarketDataGenerationConfigHistoryDialog::versionRow(int index) const {
...
MarketDataGenerationConfigHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void MarketDataGenerationConfigHistoryDialog::displayFullDetails(int index) {
...
void MarketDataGenerationConfigHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void MarketDataGenerationConfigHistoryDialog::revertToVersionAt(int index) {
