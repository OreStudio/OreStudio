...
** Calendars tab: skip the convention save when only calendars changed

Same reasoning as [[id:E1196536-38E8-4365-B0E6-A269F7CA3923][ores.refdata.currency]]'s own guard: calendar
assignments are committed by their own change-reason prompt (see the
=calendars_tab_connections= block above) via
=CalendarAssignmentWidget::commitChanges=. Without this guard, staging
a calendar add/remove alone would still enable the Save button and
=onSaveClicked= would unconditionally re-save the convention's own
fields too, popping a second, unrelated change-reason dialog.
