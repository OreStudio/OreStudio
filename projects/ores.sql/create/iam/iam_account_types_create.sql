...
create table if not exists "ores_iam_account_types_tbl" (
    "type" text not null,
...
on "public"."ores_iam_account_types_tbl" (tenant_id, type, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_iam_account_types_type_uniq_idx
on "public"."ores_iam_account_types_tbl" (tenant_id, type)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_iam_account_types_tenant_idx
on "public"."ores_iam_account_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function public.ores_iam_account_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from "public"."ores_iam_account_types_tbl"
    ...
    if found then
        ...
        update "public"."ores_iam_account_types_tbl"
...
create or replace trigger ores_iam_account_types_insert_trg
before insert on "public"."ores_iam_account_types_tbl"
for each row
execute function public.ores_iam_account_types_insert_fn();

create or replace rule ores_iam_account_types_delete_rule as
on delete to "public"."ores_iam_account_types_tbl"
do instead
  update "public"."ores_iam_account_types_tbl"
  set valid_to = current_timestamp
...
begin
    ...
    if not exists (select 1 from ores_iam_account_types_tbl limit 1) then
        return p_value;
    end if;
