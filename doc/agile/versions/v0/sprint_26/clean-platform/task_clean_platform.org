...
count that rose from 201 to 253 with no vacuous case left, and the shared
scoped temp file and directory types, which replace seven ad-hoc
implementations across the tree once the other components migrate. Two diagram
