 ...
 */
std::vector<domain::zero_convention> read_one(repository::zero_convention_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::zero_convention_key& key) {
 ...
 */
messaging::zero_convention_key key_from(const domain::zero_convention& v) {
    messaging::zero_convention_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::zero_convention to_domain(const messaging::zero_convention_write& write) {
    domain::zero_convention v;
    v.id = write.id;
    v.tenor_based = write.tenor_based;
    v.day_count_fraction = write.day_count_fraction;
    v.compounding = write.compounding;
    v.compounding_frequency = write.compounding_frequency;
    v.tenor_calendar = write.tenor_calendar;
...
messaging::list_zero_conventions_response zero_convention_service::list_zero_conventions(
    ...
    messaging::list_zero_conventions_response response;
    ...
    response.zero_conventions = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_zero_convention_count(ctx_);
    return response;
}

messaging::get_zero_convention_response zero_convention_service::get_zero_convention(
    const messaging::get_zero_convention_request& request) {
    messaging::get_zero_convention_response response;
    ...
    response.zero_convention = std::move(found.front());
    return response;
}

messaging::get_many_zero_conventions_response zero_convention_service::get_many_zero_conventions(
    const messaging::get_many_zero_conventions_request& request) {
    messaging::get_many_zero_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::zero_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.zero_convention = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_zero_convention_response zero_convention_service::put_zero_convention(
    const messaging::put_zero_convention_request& request) {
    messaging::put_zero_convention_response response;
    domain::zero_convention value;
    ...
    if (!written.empty())
        response.zero_convention = std::move(written.front());
    return response;
}

messaging::put_many_zero_conventions_response zero_convention_service::put_many_zero_conventions(
    const messaging::put_many_zero_conventions_request& request) {
    messaging::put_many_zero_conventions_response response;
    std::vector<domain::zero_convention> batch;
    ...
    response.zero_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.zero_conventions.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_zero_convention_response zero_convention_service::delete_zero_convention(
    const messaging::delete_zero_convention_request& request) {
    messaging::delete_zero_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::zero_convention_repository::remove_status::removed:
            break;
        case repository::zero_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::zero_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::zero_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
zero_convention_service::delete_many_zero_conventions(
    const messaging::delete_many_zero_conventions_request& request) {
    messaging::delete_many_zero_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
zero_convention_service::list_zero_convention_versions(
    const messaging::list_zero_convention_versions_request& request) {
    messaging::list_zero_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
zero_convention_service::get_zero_convention_version(
    const messaging::get_zero_convention_version_request& request) {
    messaging::get_zero_convention_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.zero_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
zero_convention_service::prepare_change(const messaging::zero_convention_change& change,
...
std::vector<domain::zero_convention>
zero_convention_service::get_zero_conventions(const std::vector<std::string>& ids) {
