...
void equity_barrier_option_instrument_repository::write(
    ...
    execute_write_query(ctx,
                        ...
                        lg(),
                        "Writing Equity Barrier Option instrument to database.");
}
...
void equity_barrier_option_instrument_repository::write(
    ...
    execute_write_query(ctx,
                        equity_barrier_option_instrument_mapper::map(v),
                        lg(),
                        "Writing Equity Barrier Option instruments to database.");
}
...
equity_barrier_option_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<equity_barrier_option_instrument_entity,
                                  domain::equity_barrier_option_instrument>(
            ctx,
            query,
            [](const auto& entities) {
                return equity_barrier_option_instrument_mapper::map(entities);
            },
            lg(),
            "Reading latest Equity Barrier Option instruments (workspace resolution chain).");
    }
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::read<std::vector<equity_barrier_option_instrument_entity>> |
    ...
    return execute_read_query<equity_barrier_option_instrument_entity,
        ...
        lg(),
        "Reading latest Equity Barrier Option instruments");
}

std::vector<domain::equity_barrier_option_instrument>
equity_barrier_option_instrument_repository::read_latest(context ctx,
                                                         const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest Equity Barrier Option instrument. "
    ...
    return execute_read_query<equity_barrier_option_instrument_entity,
        ...
        [](const auto& entities) { return equity_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest Equity Barrier Option instrument by instrument_id.");
}


std::vector<domain::equity_barrier_option_instrument>
equity_barrier_option_instrument_repository::read_all(context ctx,
                                                      const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all Equity Barrier Option instrument versions. "
                               << "instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<equity_barrier_option_instrument_entity>> |
    ...
    return execute_read_query<equity_barrier_option_instrument_entity,
        ...
        [](const auto& entities) { return equity_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading all Equity Barrier Option instrument versions by instrument_id.");
}

std::optional<domain::equity_barrier_option_instrument>
equity_barrier_option_instrument_repository::read_at_version(context ctx,
                                                             const std::string& instrument_id,
                                                             std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading Equity Barrier Option instrument at version. "
    ...
    const auto entities = execute_read_query<equity_barrier_option_instrument_entity,
                                             domain::equity_barrier_option_instrument>(
        ctx,
        query,
        [](const auto& entities) { return equity_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading Equity Barrier Option instrument at version.");
...
void equity_barrier_option_instrument_repository::remove(context ctx,
                                                         const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing Equity Barrier Option instrument. "
                               << "instrument_id: " << instrument_id;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    ...
    execute_delete_query(
        ctx, query, lg(), "Removing Equity Barrier Option instrument from database.");
}
...
equity_barrier_option_instrument_repository::get_total_equity_barrier_option_instrument_count(
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<equity_barrier_option_instrument_entity>(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active Equity Barrier Option instrument count: " << count;
    return count;
}

std::vector<domain::equity_barrier_option_instrument>
equity_barrier_option_instrument_repository::read_latest(
    ...
    auto result = execute_read_query<equity_barrier_option_instrument_entity,
                                     domain::equity_barrier_option_instrument>(
        ctx,
        query,
        [](const auto& entities) { return equity_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest Equity Barrier Option instruments by ids.");
    return result;
}

void equity_barrier_option_instrument_repository::remove(
    context ctx, const std::vector<std::string>& instrument_ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<equity_barrier_option_instrument_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid &&
                             "instrument_id"_c.in(instrument_ids) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing Equity Barrier Option instruments.");
}
