 ...
 *   SELECT * FROM ores_dq_preview_business_centre_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_business_centres(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_business_centre_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
/**
 * Populate ores_refdata_business_centres_tbl from a DQ business_centres dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_business_centres(
    p_dataset_id uuid,
