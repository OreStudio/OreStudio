 ...
 */
std::vector<domain::diary_entry_type> read_one(repository::diary_entry_type_repository& repo,
                                               const ores::database::context& ctx,
                                               const messaging::diary_entry_type_key& key) {
 ...
 */
messaging::diary_entry_type_key key_from(const domain::diary_entry_type& v) {
    messaging::diary_entry_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::diary_entry_type to_domain(const messaging::diary_entry_type_write& write) {
    domain::diary_entry_type v;
    v.code = write.code;
    v.name = write.name;
...
messaging::get_diary_entry_type_response diary_entry_type_service::get_diary_entry_type(
    const messaging::get_diary_entry_type_request& request) {
    messaging::get_diary_entry_type_response response;
    ...
    response.diary_entry_type = std::move(found.front());
    return response;
}

messaging::get_many_diary_entry_types_response diary_entry_type_service::get_many_diary_entry_types(
    const messaging::get_many_diary_entry_types_request& request) {
    messaging::get_many_diary_entry_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::diary_entry_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.diary_entry_type = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_diary_entry_type_response diary_entry_type_service::put_diary_entry_type(
    const messaging::put_diary_entry_type_request& request) {
    messaging::put_diary_entry_type_response response;
    domain::diary_entry_type value;
    ...
    if (!written.empty())
        response.diary_entry_type = std::move(written.front());
    return response;
}

messaging::put_many_diary_entry_types_response diary_entry_type_service::put_many_diary_entry_types(
    const messaging::put_many_diary_entry_types_request& request) {
    messaging::put_many_diary_entry_types_response response;
    ...
    response.entry_types.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.entry_types.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_diary_entry_type_response diary_entry_type_service::delete_diary_entry_type(
    const messaging::delete_diary_entry_type_request& request) {
    messaging::delete_diary_entry_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::diary_entry_type_repository::remove_status::removed:
            break;
        case repository::diary_entry_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::diary_entry_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::diary_entry_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
diary_entry_type_service::delete_many_diary_entry_types(
    const messaging::delete_many_diary_entry_types_request& request) {
    messaging::delete_many_diary_entry_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_diary_entry_type_versions_response
diary_entry_type_service::list_diary_entry_type_versions(
    const messaging::list_diary_entry_type_versions_request& request) {
    messaging::list_diary_entry_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
diary_entry_type_service::get_diary_entry_type_version(
    const messaging::get_diary_entry_type_version_request& request) {
    messaging::get_diary_entry_type_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.diary_entry_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
diary_entry_type_service::prepare_change(const messaging::diary_entry_type_change& change,
...
std::vector<domain::diary_entry_type>
diary_entry_type_service::get_entry_types(const std::vector<std::string>& codes) {
