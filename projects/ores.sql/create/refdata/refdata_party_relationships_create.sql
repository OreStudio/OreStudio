...
create table if not exists "ores_refdata_party_relationships_tbl" (
    "code" text not null,
...
create unique index if not exists party_relationships_version_uniq_idx
on "ores_refdata_party_relationships_tbl" (tenant_id, code, coding_scheme_code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists party_relationships_code_uniq_idx
on "ores_refdata_party_relationships_tbl" (tenant_id, code, coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists party_relationships_tenant_idx
on "ores_refdata_party_relationships_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists party_relationships_coding_scheme_idx
