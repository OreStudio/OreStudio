 ...
 */
std::vector<domain::book> read_one(repository::book_repository& repo,
                                   const ores::database::context& ctx,
                                   const messaging::book_key& key) {
 ...
 */
messaging::book_key key_from(const domain::book& v) {
...
domain::book to_domain(const messaging::book_write& write) {
    ...
    v.owner_unit_id = write.owner_unit_id;
    v.functional_currency = write.functional_currency;
    v.gl_account_ref = write.gl_account_ref;
    v.cost_center = write.cost_center;
    v.book_status = write.book_status;
...
messaging::list_by_parent_portfolio_id_books_response
...
messaging::get_book_response book_service::get_book(const messaging::get_book_request& request) {
    messaging::get_book_response response;
    ...
    response.book = std::move(found.front());
    return response;
}

messaging::get_many_books_response
book_service::get_many_books(const messaging::get_many_books_request& request) {
    messaging::get_many_books_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::book_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.book = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_book_response book_service::put_book(const messaging::put_book_request& request) {
    messaging::put_book_response response;
    domain::book value;
    ...
    if (!written.empty())
        response.book = std::move(written.front());
    return response;
}

messaging::put_many_books_response
book_service::put_many_books(const messaging::put_many_books_request& request) {
    messaging::put_many_books_response response;
    ...
    response.books.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.books.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_book_response
book_service::delete_book(const messaging::delete_book_request& request) {
    messaging::delete_book_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::book_repository::remove_status::removed:
            break;
        case repository::book_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::book_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::book_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
book_service::delete_many_books(const messaging::delete_many_books_request& request) {
    messaging::delete_many_books_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
book_service::list_book_versions(const messaging::list_book_versions_request& request) {
    messaging::list_book_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
book_service::get_book_version(const messaging::get_book_version_request& request) {
    messaging::get_book_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.book.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
book_service::prepare_change(const messaging::book_change& change,
...
std::vector<domain::book> book_service::get_books(const std::vector<std::string>& ids) {
