 ...
 */
#ifndef ORES_COMPUTE_API_EVENTING_APP_VERSION_EVENT_HPP
#define ORES_COMPUTE_API_EVENTING_APP_VERSION_EVENT_HPP
...
/**
 * @brief Event traits for app version's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::compute::messaging::app_version_event> {
    static constexpr std::string_view subject_prefix = "compute.v1.app_versions_events";

    static ores::compute::messaging::app_version_event
    from_notification(const entity_event_notification& notification) {
        ores::compute::messaging::app_version_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::compute::messaging::app_version_key>(notification.key))
            event.key = *key;
