...
create table if not exists "ores_refdata_counterparty_contact_informations_tbl" (
    "id" uuid not null,
...
begin
    ...
    from "ores_refdata_counterparty_contact_informations_tbl"
    where tenant_id = NEW.tenant_id
      and id = NEW.id
      and valid_to = ores_utility_infinity_timestamp_fn()
    for update;
    ...
    if found then
        ...
        update "ores_refdata_counterparty_contact_informations_tbl"
...
for each row execute function ores_refdata_counterparty_contact_informations_insert_fn();

create or replace rule ores_refdata_counterparty_contact_informations_delete_rule as
on delete to "ores_refdata_counterparty_contact_informations_tbl" do instead
    update "ores_refdata_counterparty_contact_informations_tbl"
    set valid_to = current_timestamp
