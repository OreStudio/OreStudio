 ...
 */
#ifndef ORES_ANALYTICS_API_EVENTING_PRICING_ENGINE_TYPE_EVENT_HPP
#define ORES_ANALYTICS_API_EVENTING_PRICING_ENGINE_TYPE_EVENT_HPP
...
/**
 * @brief Event traits for pricing engine type's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::analytics::messaging::pricing_engine_type_event> {
    static constexpr std::string_view subject_prefix = "analytics.v1.pricing_engine_types_events";

    static ores::analytics::messaging::pricing_engine_type_event
    from_notification(const entity_event_notification& notification) {
        ores::analytics::messaging::pricing_engine_type_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::analytics::messaging::pricing_engine_type_key>(
                notification.key))
            event.key = *key;
