...
#include "ui_InstrumentCodeHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

InstrumentCodeHistoryDialog::InstrumentCodeHistoryDialog(const QString& code,
                                                         ClientManager* clientManager,
                                                         QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::InstrumentCodeHistoryDialog)
    , code_(code)
...
InstrumentCodeHistoryDialog::~InstrumentCodeHistoryDialog() {
    delete ui_;
}

QString InstrumentCodeHistoryDialog::code() const {
    return code_;
}

void InstrumentCodeHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for instrument code: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_instrument_code_history_request request;
    request.code = code_.toStdString();

    QPointer<InstrumentCodeHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_instrument_code_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int InstrumentCodeHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString InstrumentCodeHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow InstrumentCodeHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult InstrumentCodeHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
    ...
    checkString(diffs, tr("Asset Class"), curr.asset_class, prev.asset_class);
    checkInt(diffs, tr("Display Order"), curr.display_order, prev.display_order);
    return diffs;
}

void InstrumentCodeHistoryDialog::displayFullDetails(int index) {
    ...
    ui_->assetClassValue->setText(QString::fromStdString(version.asset_class));
...
void InstrumentCodeHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void InstrumentCodeHistoryDialog::revertToVersionAt(int index) {
