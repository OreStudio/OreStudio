...
The compass-pr-raise skill now mandates the full build and ctest for every PR, even doc-only ones. Replace that: classify the changed files, run only the checks the class needs, and record the verification in the PR description. compass-pr-address-review reuses the classification for its per-round verification.
...
- The compass-pr-raise skill classifies the changed files into docs / code (C++) / ci / python tooling; a mixed change runs the union of its classes.
...
- compass-pr-address-review applies the same classification to its per-round local verification.

* Plan

- The classification rule table lives in =compass-pr-raise= (one place both
  submit-PR skills read); =compass-pr-address-review= links to it.
...
| 3 | ctest runs twice per PR (plain run + CDash script form) | compass-pr-raise/SKILL.org | declined | Intentional: plain run for fast feedback; the script form rebuilds incrementally and submits for CDash visibility (story acceptance) |
| 4 | Codegen-drift step runs =build --direct codegen_templates=, a target that does not exist (only =tangle_codegen_templates=) — following the skill skips the check | compass-pr-raise/SKILL.org | accepted | Added =codegen_templates= alias to =BUILD_TARGET_ALIASES=; =test_direct_build.py= asserts every alias resolves to a known direct-build target |
| 5 | Classification table has no catch-all bucket, unlike =pr.yml='s classify =*) code=true= default — unmatched paths map to zero classes and skip all checks | compass-pr-raise/SKILL.org, ci-workflows-and-local-checks.org | accepted | Added "anything else" row (code class checks) to both tables |
...
=compass-pr-raise= now classifies the diff (docs / code (C++) / ci / python
tooling; mixed = union) and runs only the matching checks. The rule
table lives in =compass-pr-raise= alone; =compass-pr-address-review= links to it.
