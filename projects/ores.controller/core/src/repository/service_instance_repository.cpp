...
service_instance_repository::read_all(context ctx, const std::string& service_name_filter) {

    BOOST_LOG_SEV(lg(), debug) << "Reading service instances. Filter: '" << service_name_filter
                               << "'";
    ...
    if (!service_name_filter.empty()) {
        ...
        return execute_read_query<service_instance_entity, api::domain::service_instance>(
            ctx,
            query,
            [](const auto& e) { return service_instance_mapper::map(e); },
            lg(),
            "Reading service instances by service.");
    }
    ...
    return execute_read_query<service_instance_entity, api::domain::service_instance>(
        ...
        [](const auto& e) { return service_instance_mapper::map(e); },
        lg(),
        "Reading all service instances.");
}

std::optional<api::domain::service_instance>
service_instance_repository::read(context ctx, const std::string& service_name, int replica_index) {

    BOOST_LOG_SEV(lg(), debug) << "Reading instance: " << service_name << "[" << replica_index
                               << "]";
    const auto query =
        sqlgen::read<std::vector<service_instance_entity>> |
        where("service_name"_c == service_name && "replica_index"_c == replica_index);
    const auto results = execute_read_query<service_instance_entity, api::domain::service_instance>(
        ctx,
        query,
        [](const auto& e) { return service_instance_mapper::map(e); },
        lg(),
...
void service_instance_repository::insert(context ctx, const api::domain::service_instance& v) {

    BOOST_LOG_SEV(lg(), debug) << "Inserting instance: " << v.service_name << "[" << v.replica_index
                               << "]";
    execute_write_query(ctx, service_instance_mapper::map(v), lg(), "Inserting service instance.");
}

void service_instance_repository::update_phase(context ctx,
                                               const api::domain::service_instance& v) {

    BOOST_LOG_SEV(lg(), debug) << "Updating phase for " << v.service_name << "[" << v.replica_index
                               << "] -> " << v.phase;
    ...
    const auto query = sqlgen::update<service_instance_entity>(
                           ...
                           "last_command_line"_c.set(entity.last_command_line)) |
