...
void fx_barrier_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    fx_barrier_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_fx_barrier_option_instrument(std::ref(out), std::ref(session), args);
        },
        "Add FX barrier option instrument (trade_type_code "
        "party_id "
        "trade_id "
        "bought_currency "
        "bought_amount "
    ...
    fx_barrier_option_instruments_menu->Insert(
        ...
        "Delete FX barrier option instrument by instrument_id");
    ...
    fx_barrier_option_instruments_menu->Insert(
        ...
        "Show FX barrier option instrument's version history");
...
void fx_barrier_option_instrument_commands::process_get_fx_barrier_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX barrier option instrument request.";
...
void fx_barrier_option_instrument_commands::process_add_fx_barrier_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add FX barrier option instrument." << std::endl;
        return;
    }
...
void fx_barrier_option_instrument_commands::process_delete_fx_barrier_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete FX barrier option instrument." << std::endl;
        return;
    }
