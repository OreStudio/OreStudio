...
void calendar_rule_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("calendar_rules");
    ...
    menu->Insert(
        ...
        "set <id> <calendar_code> <kind> <month> <day> <weekday> <occurrence> <day_offset> <shift> "
        "<effective_from> <effective_to> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <calendar_code> <kind> <month> <day> <weekday> <occurrence> "
        "<day_offset> <shift> <effective_from> <effective_to> <reason> <commentary>");
...
void calendar_rule_commands::process_list(std::ostream& out,
    ...
    using request_type = messaging::list_calendar_rules_request;
    ...
    auto result = do_auth_request<messaging::list_calendar_rules_response>(
...
void calendar_rule_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_calendar_rule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_calendar_rule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_calendar_rules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::calendar_rule_key key;
    ...
    auto result = do_auth_request<messaging::get_many_calendar_rules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_add(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_calendar_rule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.day, parsed->positionals[next++], "day");
        read_token(req.change.write.weekday, parsed->positionals[next++], "weekday");
        read_token(req.change.write.occurrence, parsed->positionals[next++], "occurrence");
        read_token(req.change.write.day_offset, parsed->positionals[next++], "day_offset");
        read_token(req.change.write.shift, parsed->positionals[next++], "shift");
        read_token(req.change.write.effective_from, parsed->positionals[next++], "effective_from");
        read_token(req.change.write.effective_to, parsed->positionals[next++], "effective_to");
    ...
    auto result = do_auth_request<messaging::put_calendar_rule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_set(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
...
void calendar_rule_commands::process_put_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_calendar_rules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::calendar_rule_change change;
            ...
            read_token(change.write.month, parsed->positionals[next++], "month");
            read_token(change.write.day, parsed->positionals[next++], "day");
            read_token(change.write.weekday, parsed->positionals[next++], "weekday");
            read_token(change.write.occurrence, parsed->positionals[next++], "occurrence");
            read_token(change.write.day_offset, parsed->positionals[next++], "day_offset");
            read_token(change.write.shift, parsed->positionals[next++], "shift");
            read_token(change.write.effective_from, parsed->positionals[next++], "effective_from");
            read_token(change.write.effective_to, parsed->positionals[next++], "effective_to");
    ...
    auto result = do_auth_request<messaging::put_many_calendar_rules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_delete(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_calendar_rule_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_calendar_rule_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_delete_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_calendar_rules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::calendar_rule_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_calendar_rules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_by_calendar_code(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-calendar-code request.";

    using request_type = messaging::list_by_calendar_code_calendar_rules_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        req.calendar_code =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "calendar_code");
    ...
    auto result = do_auth_request<messaging::list_by_calendar_code_calendar_rules_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_versions(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_calendar_rule_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_calendar_rule_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_rule_commands::process_version(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_calendar_rule_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_calendar_rule_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
