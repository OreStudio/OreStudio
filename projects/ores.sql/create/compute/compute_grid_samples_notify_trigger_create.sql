...
create or replace function ores_compute_grid_samples_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity', 'ores.compute.grid_sample',
    ...
    perform pg_notify('ores_compute_grid_samples', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_compute_grid_samples_notify_trg
after insert or update or delete on ores_compute_grid_samples_tbl
for each row execute function ores_compute_grid_samples_notify_fn();
