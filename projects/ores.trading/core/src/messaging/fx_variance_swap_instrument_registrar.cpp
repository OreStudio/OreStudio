...
#include "ores.trading.core/messaging/fx_variance_swap_instrument_handler.hpp"
#include <memory>
...
std::vector<ores::nats::service::subscription> register_fx_variance_swap_instrument_handlers(
    ...
    auto h = std::make_shared<fx_variance_swap_instrument_handler>(
        nats, std::move(ctx), std::move(verifier));
    subs.push_back(nats.queue_subscribe(get_fx_variance_swap_instruments_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->list(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(save_fx_variance_swap_instrument_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->save(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(delete_fx_variance_swap_instrument_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->remove(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(get_fx_variance_swap_instrument_history_request::nats_subject,
                             queue_group,
