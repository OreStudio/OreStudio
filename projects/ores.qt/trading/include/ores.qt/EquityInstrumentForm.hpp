 ...
 *
 */
#ifndef ORES_QT_EQUITY_INSTRUMENT_FORM_HPP
#define ORES_QT_EQUITY_INSTRUMENT_FORM_HPP
...
/**
 * @brief @c IInstrumentForm subclass owning the equity instrument editor.
 *
 * Hosts an internal @c QTabWidget with three pages: core (always visible),
 * options (revealed when @c has_options is true, e.g. EquityOption variants),
 * and extensions (revealed when @c has_extension is true, e.g. EquityAsianOption,
 * EquityVarianceSwap, EquityAccumulator, EquitySwap).
 */
class EquityInstrumentForm final : public IInstrumentForm {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.equity_instrument_form";
...
public:
    explicit EquityInstrumentForm(QWidget* parent = nullptr);
    ~EquityInstrumentForm() override;
