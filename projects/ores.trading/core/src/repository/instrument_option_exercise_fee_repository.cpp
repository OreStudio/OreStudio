...
std::string instrument_option_exercise_fee_repository::sql() {
    return generate_create_table_sql<instrument_option_exercise_fee_entity>(lg());
}

void instrument_option_exercise_fee_repository::write(
    ...
    execute_write_query(ctx,
                        instrument_option_exercise_fee_mapper::map(v),
                        lg(),
                        "Writing instrument option exercise fee to database.");
}

void instrument_option_exercise_fee_repository::write(
    context ctx, const std::vector<domain::instrument_option_exercise_fee>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing instrument option exercise fees. Count: " << v.size();
    execute_write_query(ctx,
                        instrument_option_exercise_fee_mapper::map(v),
                        lg(),
                        "Writing instrument option exercise fees to database.");
}
...
instrument_option_exercise_fee_repository::read_latest(context ctx) {
    ...
    return execute_read_query<instrument_option_exercise_fee_entity,
        ...
        lg(),
        "Reading latest instrument option exercise fees");
}
...
instrument_option_exercise_fee_repository::read_at_version(context ctx,
    ...
    const auto entities = execute_read_query<instrument_option_exercise_fee_entity,
                                             domain::instrument_option_exercise_fee>(
        ctx,
        query,
        [](const auto& entities) { return instrument_option_exercise_fee_mapper::map(entities); },
        lg(),
        "Reading instrument option exercise fee at version.");
...
void instrument_option_exercise_fee_repository::remove(context ctx,
    ...
    execute_delete_query(
        ctx, query, lg(), "Removing instrument option exercise fee from database.");
}
...
instrument_option_exercise_fee_repository::read_latest(context ctx,
                                                       std::uint32_t offset,
                                                       std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest instrument option exercise fees with offset: "
...
std::uint32_t instrument_option_exercise_fee_repository::get_total_exercise_fee_count(context ctx) {
