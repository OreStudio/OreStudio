...
void currency_calendar_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("currency_calendars");
    ...
    menu->Insert(
        ...
        "get <currency_iso_code> <calendar_code>");
    ...
    menu->Insert(
        ...
        "get-many <currency_iso_code> <calendar_code>");
    ...
    menu->Insert(
        ...
        "add <currency_iso_code> <calendar_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <currency_iso_code> <calendar_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <currency_iso_code> <calendar_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <currency_iso_code> <calendar_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <currency_iso_code> <calendar_code> <reason> <commentary>");
...
void currency_calendar_commands::process_list(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_currency_calendars_request;
    ...
    auto result = do_auth_request<messaging::list_currency_calendars_response>(
...
void currency_calendar_commands::process_get(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_currency_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_currency_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_get_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_currency_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            messaging::currency_calendar_key key;
    ...
    auto result = do_auth_request<messaging::get_many_currency_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_add(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_currency_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_currency_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_set(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
...
void currency_calendar_commands::process_put_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_currency_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::currency_calendar_change change;
            read_token(
    ...
    auto result = do_auth_request<messaging::put_many_currency_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_delete(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_currency_calendar_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_currency_calendar_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_delete_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_currency_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::currency_calendar_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_currency_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_calendar_commands::process_by_currency_iso_code(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-currency-iso-code request.";

    using request_type = messaging::list_by_currency_iso_code_currency_calendars_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_by_currency_iso_code_currency_calendars_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
