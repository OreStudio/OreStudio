...
private:
    inline static std::string_view logger_name = "ores.qt.client_code_domain_model";
...
public:
    ...
    explicit ClientCodeDomainModel(ClientManager* clientManager, QObject* parent = nullptr);
    ~ClientCodeDomainModel() override = default;
    ...
    /**
     * @brief Refresh code domain data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get code domain at the specified row.
     *
...
private:
    ...
    struct FetchResult {
        bool success;
        std::vector<dq::domain::code_domain> domains;
    ...
    std::vector<dq::domain::code_domain> domains_;
    QFutureWatcher<FetchResult>* watcher_;
    std::uint32_t page_size_{100};
    std::uint32_t total_available_count_{0};
    bool is_fetching_{false};

    using CodeDomainKeyExtractor = std::string (*)(const dq::domain::code_domain&);
    RecencyTracker<dq::domain::code_domain, CodeDomainKeyExtractor> recencyTracker_;
