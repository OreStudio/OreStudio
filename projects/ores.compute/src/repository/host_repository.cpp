...
#include "ores.compute/domain/host_json_io.hpp" // IWYU pragma: keep.
#include "ores.compute/repository/host_entity.hpp"
#include "ores.compute/repository/host_mapper.hpp"
...
void host_repository::remove(context ctx, const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing compute host: " << id;
    static auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
