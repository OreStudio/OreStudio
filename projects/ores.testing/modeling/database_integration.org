...
#+begin_example
Test Process A (ores.iam.tests)     Test Process B (ores.shell.tests)
         |                                    |
         v                                    v
    Tenant A                             Tenant B
    (isolated)                           (isolated)
...
** parse_test_results.sh Script

The =scripts/parse_test_results.sh= script parses Catch2 XML output and correlates
failures with log files:

#+begin_src bash
# Default preset (linux-clang-debug)
./scripts/parse_test_results.sh

# Specific preset
./scripts/parse_test_results.sh linux-clang-release

# Specific directory
./scripts/parse_test_results.sh -d /path/to/bin
#+end_src
...
compass db sql -- -c "SELECT * FROM ores_telemetry_logs_tbl LIMIT 10;"
#+end_src
