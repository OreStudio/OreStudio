...
gmm_process::gmm_process(std::vector<double> means,
    ...
    if (means_.size() != stdevs_.size() || means_.size() != weights_.size())
        throw std::invalid_argument("gmm_process: means, stdevs, weights must have equal size");
    if (means_.empty())
        throw std::invalid_argument("gmm_process: at least one component required");
    if (initial_price <= 0.0)
        throw std::invalid_argument("gmm_process: initial_price must be positive");
}
...
double gmm_process::current() const {
    return price_;
}
