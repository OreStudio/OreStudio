...
vanilla_swap_instrument_service::list_vanilla_swap_instruments(std::uint32_t offset,
                                                               std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing vanilla_swap_instruments with offset=" << offset
                               << ", limit=" << limit;
...
vanilla_swap_instrument_service::get_vanilla_swap_instrument(const std::string& id) {
...
void vanilla_swap_instrument_service::save_vanilla_swap_instrument(
    ...
    if (v.identity.instrument_id.is_nil())
        throw std::invalid_argument("Vanilla swap instrument id cannot be empty.");
    BOOST_LOG_SEV(lg(), debug) << "Saving vanilla_swap_instrument: " << v.identity.instrument_id;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved vanilla_swap_instrument: " << t.identity.instrument_id;
}

void vanilla_swap_instrument_service::remove_vanilla_swap_instrument(const std::string& id) {
...
std::vector<domain::vanilla_swap_instrument>
vanilla_swap_instrument_service::get_vanilla_swap_instrument_history(const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for vanilla_swap_instrument: " << id;
    return repo_.read_all(ctx_, id);
}


std::vector<domain::vanilla_swap_instrument>
vanilla_swap_instrument_service::get_vanilla_swap_instruments(const std::vector<std::string>& ids) {
