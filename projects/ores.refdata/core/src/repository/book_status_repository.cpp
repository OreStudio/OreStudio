...
void book_status_repository::write(context ctx, const domain::book_status& status) {
    BOOST_LOG_SEV(lg(), debug) << "Writing book status to database: " << status.code;
    execute_write_query(
        ctx, book_status_mapper::map(status), lg(), "Writing book status to database.");
}

void book_status_repository::write(context ctx, const std::vector<domain::book_status>& statuses) {
    BOOST_LOG_SEV(lg(), debug) << "Writing book statuses to database. Count: " << statuses.size();
    execute_write_query(
        ctx, book_status_mapper::map(statuses), lg(), "Writing book statuses to database.");
}
...
void book_status_repository::remove(context ctx, const std::string& code) {
    BOOST_LOG_SEV(lg(), debug) << "Removing book status from database: " << code;
...
void book_status_repository::remove(context ctx, const std::vector<std::string>& codes) {
    const auto query = sqlgen::delete_from<book_status_entity> | where("code"_c.in(codes));
    execute_delete_query(ctx, query, lg(), "batch removing book_statuses");
}
