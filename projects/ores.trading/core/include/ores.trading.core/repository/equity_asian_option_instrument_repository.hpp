 ...
 */
class ORES_TRADING_CORE_EXPORT equity_asian_option_instrument_repository {
private:
...
public:
     ...
     */
    std::vector<domain::equity_asian_option_instrument> read_all(context ctx,
                                                                 const std::string& instrument_id);

    /**
     * @brief Reads a single Equity Asian Option instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest Equity Asian Option instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_equity_asian_option_instrument_count(context ctx);

    /**
     * @brief Deletes a Equity Asian Option instrument by closing its temporal validity.
     */
