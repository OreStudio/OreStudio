...
void vanilla_swap_instrument_repository::write(
    ...
    execute_write_query(ctx, vanilla_swap_instrument_mapper::map(v),
        lg(), "Writing vanilla swap instruments to database.");
}
...
vanilla_swap_instrument_repository::read_latest(context ctx) {
    ...
    return execute_read_query<vanilla_swap_instrument_entity, domain::vanilla_swap_instrument>(
        ...
        lg(), "Reading latest vanilla swap instruments");
}

std::vector<domain::vanilla_swap_instrument>
vanilla_swap_instrument_repository::read_latest(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest vanilla swap instrument. instrument_id: " << instrument_id;
    static auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<vanilla_swap_instrument_entity>> |
    ...
    return execute_read_query<vanilla_swap_instrument_entity, domain::vanilla_swap_instrument>(
        ctx, query,
        [](const auto& entities) { return vanilla_swap_instrument_mapper::map(entities); },
        lg(), "Reading latest vanilla swap instrument by instrument_id.");
}

std::vector<domain::vanilla_swap_instrument>
vanilla_swap_instrument_repository::read_all(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all vanilla swap instrument versions. instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<vanilla_swap_instrument_entity>> |
    ...
    return execute_read_query<vanilla_swap_instrument_entity, domain::vanilla_swap_instrument>(
        ctx, query,
        [](const auto& entities) { return vanilla_swap_instrument_mapper::map(entities); },
        lg(), "Reading all vanilla swap instrument versions by instrument_id.");
}
...
vanilla_swap_instrument_repository::read_latest(
    ...
    return execute_read_query<vanilla_swap_instrument_entity, domain::vanilla_swap_instrument>(
        ctx, query,
        [](const auto& entities) { return vanilla_swap_instrument_mapper::map(entities); },
        lg(), "Reading latest vanilla swap instruments by ids.");
}
