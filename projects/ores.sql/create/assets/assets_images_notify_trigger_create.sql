...
create or replace function ores_assets_images_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    changed_key := jsonb_build_object('key', changed_key);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.assets.image',
    ...
    perform pg_notify('ores_assets_images', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_assets_images_notify_trg
after insert or update or delete on ores_assets_images_tbl
for each row execute function ores_assets_images_notify_fn();
