...
constexpr std::string_view treatment_dimension_event_name =
    eventing::domain::event_traits<dq::eventing::treatment_dimension_changed_event>::name;
}

TreatmentDimensionController::TreatmentDimensionController(QMainWindow* mainWindow,
                                                           QMdiArea* mdiArea,
                                                           ...
                                                           QObject* parent)
    : EntityController(
          mainWindow, mdiArea, clientManager, username, treatment_dimension_event_name, parent)
...
void TreatmentDimensionController::showListWindow() {
    ...
    connect(listWindow_,
            &TreatmentDimensionMdiWindow::statusChanged,
            this,
            &TreatmentDimensionController::statusMessage);
    connect(listWindow_,
            &TreatmentDimensionMdiWindow::errorOccurred,
            this,
            &TreatmentDimensionController::errorMessage);
    connect(listWindow_,
            &TreatmentDimensionMdiWindow::showDimensionDetails,
            this,
            &TreatmentDimensionController::onShowDetails);
    connect(listWindow_,
            &TreatmentDimensionMdiWindow::addNewRequested,
            this,
            &TreatmentDimensionController::onAddNewRequested);
    connect(listWindow_,
            &TreatmentDimensionMdiWindow::showDimensionHistory,
            this,
            &TreatmentDimensionController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<TreatmentDimensionController>(this), key]() {
                if (!self)
                    return;
...
void TreatmentDimensionController::onShowDetails(const dq::domain::treatment_dimension& dimension) {
...
void TreatmentDimensionController::showAddWindow() {
    ...
    connect(detailDialog,
            &TreatmentDimensionDetailDialog::statusMessage,
            this,
            &TreatmentDimensionController::statusMessage);
    connect(detailDialog,
            &TreatmentDimensionDetailDialog::errorMessage,
            this,
            &TreatmentDimensionController::errorMessage);
    connect(detailDialog,
            &TreatmentDimensionDetailDialog::dimensionSaved,
            this,
            [self = QPointer<TreatmentDimensionController>(this)](const QString& code) {
                if (!self)
                    return;
...
void TreatmentDimensionController::showDetailWindow(
    ...
    connect(detailDialog,
            &TreatmentDimensionDetailDialog::dimensionDeleted,
            this,
            [self = QPointer<TreatmentDimensionController>(this), key](const QString& code) {
                if (!self)
                    return;
...
void TreatmentDimensionController::showHistoryWindow(const QString& code) {
    ...
    connect(historyDialog,
            &TreatmentDimensionHistoryDialog::statusChanged,
            this,
            [self = QPointer<TreatmentDimensionController>(this)](const QString& message) {
                if (!self)
                    return;
                emit self->statusMessage(message);
            });
    connect(historyDialog,
            &TreatmentDimensionHistoryDialog::errorOccurred,
            this,
            [self = QPointer<TreatmentDimensionController>(this)](const QString& message) {
                if (!self)
                    return;
    ...
    connect(historyDialog,
            &TreatmentDimensionHistoryDialog::revertVersionRequested,
            this,
            &TreatmentDimensionController::onRevertVersion);
    connect(historyDialog,
            &TreatmentDimensionHistoryDialog::openVersionRequested,
            this,
            &TreatmentDimensionController::onOpenVersion);
...
void TreatmentDimensionController::onOpenVersion(const dq::domain::treatment_dimension& dimension,
                                                 int versionNumber) {
    ...
    detailWindow->setWindowTitle(
        QString("Treatment Dimension: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void TreatmentDimensionController::onRevertVersion(
    const dq::domain::treatment_dimension& dimension) {
    BOOST_LOG_SEV(lg(), info) << "Reverting treatment dimension to version: " << dimension.version;
    ...
    connect(detailDialog,
            ...
            [self = QPointer<TreatmentDimensionController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("Treatment dimension '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert Treatment Dimension: %1").arg(QString::fromStdString(dimension.code)));
