 ...
 *
 */
#ifndef ORES_QT_CLIENT_BUSINESS_UNIT_MODEL_HPP
#define ORES_QT_CLIENT_BUSINESS_UNIT_MODEL_HPP
 ...
 *
 * This model extends AbstractClientModel and fetches business unit
 * data asynchronously using the ores.comms client.
 */
class ClientBusinessUnitModel final : public AbstractClientModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.client_business_unit_model";
...
public:
    ...
    explicit ClientBusinessUnitModel(ClientManager* clientManager, QObject* parent = nullptr);
...
protected:
    ...
    std::vector<int> iconColumns() const override {
        return {
            Column::BusinessCentreCode,
        };
    }

public:
    /**
     * @brief Refresh business unit data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get business unit at the specified row.
     *
     * @param row The row index.
     * @return The business unit, or nullptr if row is invalid.
     */
...
private:
    ...
    using BusinessUnitKeyExtractor = std::string (*)(const refdata::domain::business_unit&);
    RecencyTracker<refdata::domain::business_unit, BusinessUnitKeyExtractor> recencyTracker_;
