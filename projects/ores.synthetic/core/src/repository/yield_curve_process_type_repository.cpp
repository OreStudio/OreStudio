...
yield_curve_process_type_repository::replace_claim(context ctx,
...
yield_curve_process_type_repository::apply_claim(context ctx,
                                                 const domain::yield_curve_process_type& v,
...
void yield_curve_process_type_repository::write(
    context ctx, const std::vector<domain::yield_curve_process_type>& v) {
...
void yield_curve_process_type_repository::write(context ctx,
    ...
    execute_write_query(ctx,
                        yield_curve_process_type_mapper::map(t),
                        lg(),
                        "Writing yield curve process type to database.");
}
...
void yield_curve_process_type_repository::write(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Writing yield curve process types. Count: " << v.size();
    std::vector<domain::yield_curve_process_type> batch;
    batch.reserve(v.size());
    for (std::size_t i = 0; i < v.size(); ++i)
        batch.push_back(apply_claim(ctx, v[i], claims[i]));
    execute_write_query(ctx,
                        yield_curve_process_type_mapper::map(batch),
                        lg(),
                        "Writing yield curve process types to database.");
}

std::vector<domain::yield_curve_process_type>
yield_curve_process_type_repository::read_latest(context ctx) {
    ...
    return execute_read_query<yield_curve_process_type_entity, domain::yield_curve_process_type>(
        ...
        lg(),
        "Reading latest yield curve process types");
}
...
yield_curve_process_type_repository::read_latest(context ctx, const std::string& code) {
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::read<std::vector<yield_curve_process_type_entity>> |
...
yield_curve_process_type_repository::read_all(context ctx, const std::string& code) {
    ...
    return execute_read_query<yield_curve_process_type_entity, domain::yield_curve_process_type>(
        ...
        [](const auto& entities) { return yield_curve_process_type_mapper::map(entities); },
        lg(),
        "Reading all yield curve process type versions by code.");
}
...
yield_curve_process_type_repository::read_at_version(context ctx,
    ...
    const auto entities =
        execute_read_query<yield_curve_process_type_entity, domain::yield_curve_process_type>(
            ctx,
            query,
            [](const auto& entities) { return yield_curve_process_type_mapper::map(entities); },
            lg(),
            "Reading yield curve process type at version.");
...
yield_curve_process_type_repository::remove_status yield_curve_process_type_repository::remove(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<yield_curve_process_type_entity> |
                       where("tenant_id"_c == tid && "code"_c == code &&
                             "valid_to"_c == max.value() && "version"_c == expected);

    execute_delete_query(ctx, query, lg(), "Removing yield curve process type from database.");
...
yield_curve_process_type_repository::read_latest(context ctx,
    ...
    auto result =
        execute_read_query<yield_curve_process_type_entity, domain::yield_curve_process_type>(
            ...
            [](const auto& entities) { return yield_curve_process_type_mapper::map(entities); },
            lg(),
            "Reading latest yield curve process types by ids.");
    return result;
}

void yield_curve_process_type_repository::remove(context ctx,
                                                 const std::vector<std::string>& codes) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::delete_from<yield_curve_process_type_entity> |
        where("tenant_id"_c == tid && "code"_c.in(codes) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing yield curve process types.");
}
