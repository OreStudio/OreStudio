...
#include "ores.trading.core/messaging/vanilla_swap_instrument_handler.hpp"
#include <memory>
...
std::vector<ores::nats::service::subscription> register_vanilla_swap_instrument_handlers(
    ...
    auto h = std::make_shared<vanilla_swap_instrument_handler>(
        nats, std::move(ctx), std::move(verifier));
    subs.push_back(nats.queue_subscribe(get_vanilla_swap_instruments_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->list(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(save_vanilla_swap_instrument_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->save(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(delete_vanilla_swap_instrument_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->remove(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(get_vanilla_swap_instrument_history_request::nats_subject,
                             queue_group,
