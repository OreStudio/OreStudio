...
ClientMarketFixingModel::ClientMarketFixingModel(ClientManager* clientManager,
    ...
    connect(watcher_,
            ...
            this,
            &ClientMarketFixingModel::onDataLoaded);
}
...
QVariant ClientMarketFixingModel::data(const QModelIndex& index, int role) const {
...
QVariant
ClientMarketFixingModel::headerData(int section, Qt::Orientation orientation, int role) const {
            ...
            [&]() -> FetchResult {
                ...
                if (!result) {
                    return {.success = false,
                            .entries = {},
                            .total_available_count = 0,
                            .error_message = QString::fromStdString("Failed to fetch fixings: " +
                                                                    result.error()),
                            .error_details = {}};
                }
...
void ClientMarketFixingModel::onDataLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch fixings: " << result.error_message.toStdString();
