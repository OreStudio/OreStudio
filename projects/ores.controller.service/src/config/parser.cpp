...
const std::string product_version(
    "ores.controller.service v" ORES_VERSION);
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description(
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Service lifecycle controller" << std::endl << std::endl
         << "Usage: ores.controller.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments,
    ...
    const auto name_mapper(
        environment_mapper_factory::make_mapper("CONTROLLER_SERVICE"));
