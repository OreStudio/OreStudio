 ...
 *
 */
#include "ores.dq.service/config/parser.hpp"
...
const std::string product_version("ores.dq.service v" ORES_VERSION);
...
using ores::dq::service::config::options;
using ores::dq::service::config::parser_exception;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.dq.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Data quality service" << std::endl << std::endl
         << "Usage: ores.dq.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("DQ_SERVICE"));
