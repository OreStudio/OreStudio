...
=calendar_date= and =tenor_convention_resolution= are read-only: their rows
are provisioned outside the application, so they render the reads and the
counts and no write verb.
