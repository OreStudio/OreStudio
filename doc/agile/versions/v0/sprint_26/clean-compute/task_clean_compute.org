...
| Now          | Every section is worked and every applicable item passes except G02, on the two telemetry tables the codegen cannot express. |
| Waiting on   | A decision on whether the codegen gains an append-only fact-table entity shape. |
| Next         | That decision. Nothing else on the component is open. |
