...
void BusinessUnitTypeMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "BusinessUnitTypeListWindow", {}, {900, 400}, 2);
...
void BusinessUnitTypeMdiWindow::deleteSelected() {
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making batch delete request for " << ids.size() << " business unit types";

        for (std::size_t i = 0; i < ids.size(); ++i) {
            const std::string code = (i < codes.size()) ? codes[i] : "";
            refdata::messaging::delete_business_unit_type_request request;
            request.type = code;
            auto response_result =
                self->clientManager_->process_authenticated_request(std::move(request));
            if (!response_result) {
                BOOST_LOG_SEV(lg(), error) << "Failed to delete business unit type: " << code;
                results.push_back({ids[i], code, false, "Failed to communicate with server"});
            } else {
                results.push_back(
                    {ids[i], code, response_result->success, response_result->message});
            }
        }
