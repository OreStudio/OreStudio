...
generate_synthetic_yield_curve_process_type(utility::generation::generation_context& ctx) {
    ...
    r.code = std::string("YieldCurveProcessType") + "-" + std::to_string(idx);
    r.name = std::string(faker::word::adjective()) + " Process" + "-" + std::to_string(idx);
...
generate_synthetic_yield_curve_process_types(std::size_t n,
    ...
    std::vector<domain::yield_curve_process_type> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_yield_curve_process_type(ctx));
    return r;
}
