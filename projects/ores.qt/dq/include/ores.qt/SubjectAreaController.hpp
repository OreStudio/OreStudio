...
class SubjectAreaMdiWindow;
class DetachableMdiSubWindow;

/**
 * @brief Controller for managing subject area windows and operations.
 *
 * Manages the lifecycle of subject area list, detail, and history windows.
...
private:
    ...
    /**
     * @brief Fetches the full typed subject area history (the
     * existing per-entity /
     ...
     * version) signals back to a typed subject area, since the
     * generic dialog holds no typed domain data.
     */
    void
    fetchSubjectAreaHistory(const QString& entityId,
                            std::function<void(std::expected<std::vector<>, QString>)> callback);
