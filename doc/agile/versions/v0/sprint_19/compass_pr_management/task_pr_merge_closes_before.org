...
| PR | Title |
|----+-------|
| [[https://github.com/OreStudio/OreStudio/pull/1102][#1102]] | [ores.compass] Abort merge close-out on subprocess failures |
...
|---+-----------------+------+----------+-------|
| 1 | task done rc unchecked in merge close-out (PR #1099 review, post-merge) | compass_pr.py | Accepted | abort on failure; cae950db1 via PR #1102 |
| 2 | failed close-out commit did not abort merge (PR #1099 review) | compass_pr.py | Accepted | cae950db1 via PR #1102 |
| 3 | race capture: truncated title, placeholder What/Why, stale index (PR #1102) | backlog | Accepted | 9e0dad399 |

* Result

Shipped in PR [[https://github.com/OreStudio/OreStudio/pull/1099][#1099]] (merged 2026-06-05): close-out now rides the PR
branch — guards, task done, close-out commit, push, merge with
--admin, branch deletion, clean tree. Ends the closure-PR treadmill.
(This task itself was closed post-merge because the PR was merged
from the GitHub UI, where the compass flow does not run.)
