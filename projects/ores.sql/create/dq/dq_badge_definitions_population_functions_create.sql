...
/**
 * Badge Definitions Population Functions
 *
 * Functions to publish badge definitions from the DQ artefact table into a
 * tenant's own copy of ores_dq_badge_definitions_tbl. Self-referential (like
 * coding_schemes/badge_severities): DQ publishes into its own table, not
 * another service's.
 *
 * Usage:
 *   -- Preview what will be published
 *   SELECT * FROM ores_dq_badge_definition_preview_fn(dataset_id);
 *
 *   -- Publish to a tenant
 *   SELECT * FROM ores_dq_badge_definitions_publish_fn(dataset_id, target_tenant_id, 'upsert');
 */
...
create or replace function ores_dq_badge_definition_preview_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
    name text,
...
begin
    ...
    from ores_dq_badge_definitions_artefact_tbl dq
    left join ores_dq_badge_definitions_tbl existing
...
/**
 * Populate ores_dq_badge_definitions_tbl from a DQ badge definitions dataset.
 *
 ...
 */
create or replace function ores_dq_badge_definitions_publish_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        select exists (
            select 1 from ores_dq_badge_definitions_tbl existing
        ...
        insert into ores_dq_badge_definitions_tbl (
            tenant_id,
            code, version, name, description, background_colour, text_colour,
            severity_code, css_class, display_order,
            modified_by, performed_by, change_reason_code, change_commentary
        ) values (
            p_target_tenant_id,
            r.code, 0, r.name, r.description, r.background_colour, r.text_colour,
            r.severity_code, r.css_class, r.display_order,
