...
void EquityInstrumentForm::setupConnections() {
    ...
    connect(ui_->cliquetFrequencyCombo, &QComboBox::currentTextChanged, this, markChangedStr);
    ...
    connect(ui_->returnTypeCombo, &QComboBox::currentTextChanged, this, markChangedStr);
...
bool EquityInstrumentForm::isDirty() const {
    return dirty_;
}
bool EquityInstrumentForm::isLoaded() const {
    return loaded_;
}
...
void EquityInstrumentForm::writeUiToInstrument() {
    instrument_.underlying_name = ui_->underlyingCodeEdit->text().trimmed().toStdString();
...
void EquityInstrumentForm::populateFromInstrument() {
    ...
    InstrumentFormUtils::setComboValue(ui_->cliquetFrequencyCombo, instrument_.cliquet_frequency);
...
void EquityInstrumentForm::saveInstrument(std::function<void(const std::string&)> on_success,
    ...
    connect(
        ...
        [self, watcher, on_success = std::move(on_success), on_failure = std::move(on_failure)]() {
            ...
            if (!result.success) {
                BOOST_LOG_SEV(lg(), error) << "Equity instrument save failed: " << result.message;
