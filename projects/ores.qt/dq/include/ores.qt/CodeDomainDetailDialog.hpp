 ...
 *
 */
#ifndef ORES_QT_CODE_DOMAIN_DETAIL_DIALOG_HPP
#define ORES_QT_CODE_DOMAIN_DETAIL_DIALOG_HPP
 ...
 *
 * This dialog allows viewing, creating, and editing code domains.
 * It supports both create mode (for new records) and edit mode (for
 * existing records).
 */
class CodeDomainDetailDialog final : public DetailDialogBase {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.code_domain_detail_dialog";
...
public:
    explicit CodeDomainDetailDialog(QWidget* parent = nullptr);
    ~CodeDomainDetailDialog() override;

    void setClientManager(ClientManager* clientManager);
    void setUsername(const std::string& username);
    void setDomain(const dq::domain::code_domain& domain);
...
private:
    ...
    void updateUiFromDomain();
    void updateDomainFromUi();
