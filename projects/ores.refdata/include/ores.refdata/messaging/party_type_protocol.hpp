...
/**
 * @brief Request to save a party type (create or update).
 */
struct save_party_type_request final {
    domain::party_type type;
...
/**
 * @brief Response confirming party type save operation.
 */
...
/**
 * @brief Result for a single party type deletion.
 */
struct delete_party_type_result final {
    std::string code;  ///< Primary key
    bool success;
    std::string message;
};

std::ostream& operator<<(std::ostream& s, const delete_party_type_result& v);
...
struct delete_party_type_response final {
    std::vector<delete_party_type_result> results;
