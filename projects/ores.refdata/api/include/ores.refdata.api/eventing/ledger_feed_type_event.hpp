 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_LEDGER_FEED_TYPE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_LEDGER_FEED_TYPE_EVENT_HPP
...
/**
 * @brief Event traits for ledger feed type's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::ledger_feed_type_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.ledger_feed_types_events";

    static ores::refdata::messaging::ledger_feed_type_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::ledger_feed_type_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::ledger_feed_type_key>(notification.key))
            event.key = *key;
