...
void ImportTradeDialog::populateTradeTable() {
    ...
    for (size_t i = 0; i < items_.size(); ++i) {
        ...
        tradeTable_->setItem(row, 1, makeItem(item.trade.identity.external_id));
        tradeTable_->setItem(row, 2, makeItem(item.trade.classification.trade_type));
...
void ImportTradeDialog::onImportClicked() {
    ...
    for (int i = 0; i < tradeTable_->rowCount(); ++i) {
        ...
        tti.trade.audit.modified_by = username_.toStdString();

        // Apply global date/lifecycle defaults
        tti.trade.lifecycle.trade_date = tradeDate;
        tti.trade.lifecycle.effective_date = effectiveDate;
        tti.trade.lifecycle.termination_date = terminationDate;
        tti.trade.classification.activity_type_code = lifecycleEvent;
        tti.trade.lifecycle.execution_timestamp = executionTimestamp;

        // Per-row netting set
        auto* nsItem = tradeTable_->item(i, 4);
        if (nsItem)
            tti.trade.classification.netting_set_id = nsItem->text().toStdString();
