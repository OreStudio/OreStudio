 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_IR_CURVE_BOOTSTRAP_CONFIG_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_IR_CURVE_BOOTSTRAP_CONFIG_EVENT_HPP
...
/**
 * @brief Event traits for ir curve bootstrap config's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::ir_curve_bootstrap_config_event> {
    static constexpr std::string_view subject_prefix =
        "refdata.v1.ir_curve_bootstrap_configs_events";

    static ores::refdata::messaging::ir_curve_bootstrap_config_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::ir_curve_bootstrap_config_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::ir_curve_bootstrap_config_key>(
                    notification.key))
            event.key = *key;
