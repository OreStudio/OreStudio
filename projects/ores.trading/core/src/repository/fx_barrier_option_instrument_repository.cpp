...
fx_barrier_option_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<fx_barrier_option_instrument_entity,
            ...
            lg(),
            "Reading latest FX barrier option instruments (workspace resolution chain).");
    }
...
fx_barrier_option_instrument_repository::read_at_version(context ctx,
    ...
    const auto entities = execute_read_query<fx_barrier_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading FX barrier option instrument at version.");
...
fx_barrier_option_instrument_repository::read_latest(context ctx,
                                                     std::uint32_t offset,
                                                     std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FX barrier option instruments with offset: "
    ...
    return execute_read_query<fx_barrier_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX barrier option instruments with pagination.");
}

std::uint32_t
fx_barrier_option_instrument_repository::get_total_fx_barrier_option_instrument_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active FX barrier option instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<fx_barrier_option_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active FX barrier option instrument count: " << count;
    return count;
}
...
fx_barrier_option_instrument_repository::read_latest(
    ...
    auto result = execute_read_query<fx_barrier_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_barrier_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX barrier option instruments by ids.");
    return result;
}
