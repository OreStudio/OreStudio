...
- Expand the Barclays feed's IR Curves node and verify the seeded
  curves (the =2026 Realistic= archetype rows) are listed, each
  currently using the Cox-Ingersoll-Ross engine.
...
- Menu: Market Data > Synthetic > Market Simulator; expand the
  Barclays feed's FX Rates node.
- Reopen any seeded FX config (double-click a row), or use the "New
  FX Rate" action to add one; on the Behaviour tab select the
  Ornstein-Uhlenbeck engine from the combo.
