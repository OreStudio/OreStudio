...
on "public"."ores_iam_tenant_types_tbl" (tenant_id, type, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_iam_tenant_types_type_uniq_idx
on "public"."ores_iam_tenant_types_tbl" (tenant_id, type)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_iam_tenant_types_tenant_idx
on "public"."ores_iam_tenant_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function public.ores_iam_tenant_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from "public"."ores_iam_tenant_types_tbl"
    ...
    if found then
        ...
        update "public"."ores_iam_tenant_types_tbl"
...
before insert on "public"."ores_iam_tenant_types_tbl"
for each row
execute function public.ores_iam_tenant_types_insert_fn();

create or replace rule ores_iam_tenant_types_delete_rule as
on delete to "public"."ores_iam_tenant_types_tbl"
do instead
  update "public"."ores_iam_tenant_types_tbl"
  set valid_to = current_timestamp
...
-- Validates that a type exists in the tenant_types table.
-- Returns the validated value, or default if null/empty.
-- Uses system tenant data (shared reference data).
-- =============================================================================
create or replace function ores_iam_validate_tenant_type_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid tenant_type: value cannot be null or empty'
            using errcode = '23502';
    end if;
    ...
    if not exists (
        ...
        raise exception 'Invalid tenant_type: %. Must be one of: %', p_value, (
