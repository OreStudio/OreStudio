...
ReportInstanceMdiWindow::ReportInstanceMdiWindow(ClientManager* clientManager,
...
void ReportInstanceMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &ReportInstanceMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &ReportInstanceMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &ReportInstanceMdiWindow::deleteSelected);
    ...
    connect(
        historyAction_, &QAction::triggered, this, &ReportInstanceMdiWindow::viewHistorySelected);
}
...
void ReportInstanceMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "ReportInstanceListWindow", {}, {900, 400}, 1);
}

void ReportInstanceMdiWindow::setupConnections() {
    connect(model_,
            &ClientReportInstanceModel::dataLoaded,
            this,
            &ReportInstanceMdiWindow::onDataLoaded);
    connect(
        model_, &ClientReportInstanceModel::loadError, this, &ReportInstanceMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &ReportInstanceMdiWindow::onSelectionChanged);
    connect(
        tableView_, &QTableView::doubleClicked, this, &ReportInstanceMdiWindow::onDoubleClicked);
...
void ReportInstanceMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void ReportInstanceMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* instance = model_->getInstance(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showInstanceHistory for code: " << instance->name;
...
void ReportInstanceMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete report instance while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* instance = model_->getInstance(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(instance->id));
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " report instances";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 report instances?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Report Instance", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " report instances";
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Report Instance deletion failed: " << code << " - " << message;
                failure_count++;
