:PROPERTIES:
:ID: 15329B09-2CA2-47C8-A20E-70CC757970B3
:END:
#+title: ores.trading.balance_guaranteed_swap_instrument
#+description: Codegen entity model for the trading =balance_guaranteed_swap_instrument= table. Drives the C++ domain class, mapper, repository, service, generator, Qt UI, and SQL schema generation from a single literate source.
#+type: ores.codegen.entity
#+component: trading
#+filetags: :model:entity:trading:
#+brief: Balance Guaranteed Swap (BGS) instrument.
#+entity_singular: balance_guaranteed_swap_instrument
#+entity_plural: balance_guaranteed_swap_instruments
#+entity_title: Balance Guaranteed Swap Instrument
#+created: 2026-06-01
#+updated: 2026-06-01

Represents a Balance Guaranteed Swap instrument where the notional
amortises in line with an underlying pool of assets (e.g., mortgages).

* Flags
:PROPERTIES:
:schema:           public
:product:          ores
:component:        trading
:has_tenant_id:    true
:has_workspace_id: true
:END:

* Primary key
:PROPERTIES:
:column:   instrument_id
:type:     uuid
:cpp_type: boost::uuids::uuid
:END:

UUID uniquely identifying this BGS instrument.

Surrogate key for the instrument record.

* Natural keys

* Columns

** trade_type_code
:PROPERTIES:
:group:    identity
:type:     text
:cpp_type: std::string
:nullable: false
:END:

Trade type code (soft FK to ores_trading_trade_types_tbl).

#+begin_src cpp :name generator
std::string("BalanceGuaranteedSwap")
#+end_src

** party_id
:PROPERTIES:
:group:    identity
:type:     uuid
:cpp_type: boost::uuids::uuid
:nullable: false
:END:

Party that owns this instrument.

Set from session variable app.current_party_id.

** trade_id
:PROPERTIES:
:group:    identity
:type:     uuid
:cpp_type: std::optional<boost::uuids::uuid>
:nullable: true
:END:

Optional soft FK to the parent trade.

Links instrument to a trade if applicable.

** start_date
:PROPERTIES:
:type:     date
:cpp_type: std::string
:nullable: false
:END:

Swap effective start date.

ISO 8601 date string (YYYY-MM-DD).

#+begin_src cpp :name generator
std::string("2024-01-15")
#+end_src

** maturity_date
:PROPERTIES:
:type:     date
:cpp_type: std::string
:nullable: false
:END:

Swap maturity date.

Must be after start_date.

#+begin_src cpp :name generator
std::string("2029-01-15")
#+end_src

** lockout_days
:PROPERTIES:
:type:          integer
:cpp_type:      std::optional<int>
:nullable:      true
:default_value: std::nullopt
:END:

Optional lockout period in days.

Number of days during which prepayments are restricted.

** description
:PROPERTIES:
:type:     text
:cpp_type: std::string
:nullable: true
:END:

Optional free-text description.

Human-readable notes about this instrument.

* SQL

** Flags
:PROPERTIES:
:tablename: ores_trading_balance_guaranteed_swap_instruments_tbl
:END:

* C++

The C++ domain class is decomposed into nested sub-structs (the C1202
pattern from PRs #1047/#1071/#1075/#1083/#1085): the implicit scaffolding
columns (version, tenant_id, workspace_id, the primary key) and the
columns marked =:group: identity= below fold into an
=instrument_identity identity;= member (see the
ores.trading.instrument_identity field-group model), the implicit audit
columns fold into an =ores::dq::domain::audit_record audit;= member (see
the ores.dq.audit_record field-group model), and the remaining columns
stay flat in between. The SQL schema, DB entity and column lists are
unaffected — only the domain class and the mapper's domain-side accesses
nest. NOTE: the entity templates do not consume these annotations yet;
until that lands, regeneration of the domain/repository profiles would
emit the obsolete flat shape — do not regenerate those profiles for
instruments.

** Flags
:PROPERTIES:
:component_include:    trading.api
:component_core:       trading.core
:domain_identity_group: ores.trading.instrument_identity
:domain_audit_group:    ores.dq.audit_record
:has_batch_read:       true
:generator_facet_name: generators
:END:

** Repository
:PROPERTIES:
:entity_singular_short: bgs_instrument
:entity_plural_short:   bgs_instruments
:entity_singular_words: balance guaranteed swap instrument
:entity_plural_words:   balance guaranteed swap instruments
:END:

** Domain includes

#+begin_src cpp :name includes
#include <chrono>
#include <optional>
#include <string>
#include <boost/uuid/uuid.hpp>
#+end_src

** Conventions
:PROPERTIES:
:iterator_var: bg
:END:

** Custom repository methods
