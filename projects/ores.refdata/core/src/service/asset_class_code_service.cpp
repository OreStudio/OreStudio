 ...
 */
std::vector<domain::asset_class_code> read_one(repository::asset_class_code_repository& repo,
                                               const ores::database::context& ctx,
                                               const messaging::asset_class_code_key& key) {
 ...
 */
messaging::asset_class_code_key key_from(const domain::asset_class_code& v) {
    messaging::asset_class_code_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::asset_class_code to_domain(const messaging::asset_class_code_write& write) {
    domain::asset_class_code v;
    v.code = write.code;
    v.name = write.name;
...
messaging::list_asset_class_codes_response asset_class_code_service::list_asset_class_codes(
    ...
    messaging::list_asset_class_codes_response response;
    ...
    response.asset_classes = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_asset_class_count(ctx_);
    return response;
}

messaging::get_asset_class_code_response asset_class_code_service::get_asset_class_code(
    const messaging::get_asset_class_code_request& request) {
    messaging::get_asset_class_code_response response;
    ...
    response.asset_class_code = std::move(found.front());
    return response;
}

messaging::get_many_asset_class_codes_response asset_class_code_service::get_many_asset_class_codes(
    const messaging::get_many_asset_class_codes_request& request) {
    messaging::get_many_asset_class_codes_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::asset_class_code_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.asset_class_code = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_asset_class_code_response asset_class_code_service::put_asset_class_code(
    const messaging::put_asset_class_code_request& request) {
    messaging::put_asset_class_code_response response;
    domain::asset_class_code value;
    ...
    if (!written.empty())
        response.asset_class_code = std::move(written.front());
    return response;
}

messaging::put_many_asset_class_codes_response asset_class_code_service::put_many_asset_class_codes(
    const messaging::put_many_asset_class_codes_request& request) {
    messaging::put_many_asset_class_codes_response response;
    std::vector<domain::asset_class_code> batch;
    ...
    response.asset_classes.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.asset_classes.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_asset_class_code_response asset_class_code_service::delete_asset_class_code(
    const messaging::delete_asset_class_code_request& request) {
    messaging::delete_asset_class_code_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::asset_class_code_repository::remove_status::removed:
            break;
        case repository::asset_class_code_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::asset_class_code_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::asset_class_code_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
asset_class_code_service::delete_many_asset_class_codes(
    const messaging::delete_many_asset_class_codes_request& request) {
    messaging::delete_many_asset_class_codes_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_asset_class_code_versions_response
asset_class_code_service::list_asset_class_code_versions(
    const messaging::list_asset_class_code_versions_request& request) {
    messaging::list_asset_class_code_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
asset_class_code_service::get_asset_class_code_version(
    const messaging::get_asset_class_code_version_request& request) {
    messaging::get_asset_class_code_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.asset_class_code.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
asset_class_code_service::prepare_change(const messaging::asset_class_code_change& change,
...
std::vector<domain::asset_class_code>
asset_class_code_service::get_asset_classes(const std::vector<std::string>& codes) {
