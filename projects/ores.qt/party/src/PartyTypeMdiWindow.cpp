...
PartyTypeMdiWindow::PartyTypeMdiWindow(ClientManager* clientManager,
...
void PartyTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &PartyTypeMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &PartyTypeMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &PartyTypeMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &PartyTypeMdiWindow::viewHistorySelected);
}
...
void PartyTypeMdiWindow::setupConnections() {
    connect(model_, &ClientPartyTypeModel::dataLoaded, this, &PartyTypeMdiWindow::onDataLoaded);
    connect(model_, &ClientPartyTypeModel::loadError, this, &PartyTypeMdiWindow::onLoadError);
    connectModel(model_);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &PartyTypeMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &PartyTypeMdiWindow::onDoubleClicked);
...
void PartyTypeMdiWindow::doReload() {
...
void PartyTypeMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void PartyTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete party type while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " party types";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 party types?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Party Type", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete requests for " << codes.size() << " party types";
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Party Type deletion failed: " << code << " - " << result.second;
                failure_count++;
