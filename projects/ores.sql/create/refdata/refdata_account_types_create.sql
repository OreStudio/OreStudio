...
create table if not exists "ores_refdata_account_types_tbl" (
    "code" text not null,
...
on "public"."ores_refdata_account_types_tbl" (tenant_id, code, coding_scheme_code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_refdata_account_types_code_uniq_idx
on "public"."ores_refdata_account_types_tbl" (tenant_id, code, coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_account_types_tenant_idx
on "public"."ores_refdata_account_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_account_types_coding_scheme_idx
on "public"."ores_refdata_account_types_tbl" (coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function public.ores_refdata_account_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from "public"."ores_refdata_account_types_tbl"
    ...
    if found then
        ...
        update "public"."ores_refdata_account_types_tbl"
...
create or replace trigger ores_refdata_account_types_insert_trg
before insert on "public"."ores_refdata_account_types_tbl"
for each row
execute function public.ores_refdata_account_types_insert_fn();

create or replace rule ores_refdata_account_types_delete_rule as
on delete to "public"."ores_refdata_account_types_tbl"
do instead
  update "public"."ores_refdata_account_types_tbl"
  set valid_to = current_timestamp
...
-- Validates that a code exists in the account_types table.
-- Returns the validated value, or default if null/empty.
-- Uses current tenant data.
-- =============================================================================
create or replace function ores_refdata_validate_account_type_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if not exists (select 1 from ores_refdata_account_types_tbl limit 1) then
        return p_value;
    end if;

    -- Validate against reference data
    if not exists (
        select 1 from ores_refdata_account_types_tbl
