...
- =gh= CLI is authenticated; the session's SSH agent is used as-is (see [[id:F7E4C41F-AF34-43E5-ADC3-1D837126B3DE][Do not interfere with SSH session variables]]).
...
5a. /Run the local code review./ The automatic GitHub review
   (=claude-code-review.yml=) is retired; the local review is the
   review coverage now. Run the [[id:04CA6BD9-7739-4213-A9DC-17AAACE43252][compass-code-review-pr]] skill on the
   change, record its findings in the task's =* Review= table with
   their decisions, and address the accepted ones before raising the
   PR. The misspell check (=misspell-fixer .=; exit 0 = clean) is part
   of this review pass.
...
10. /Merge — immediately once CI is green post-approval./ Nothing
    ...
    stop, not a warning — see [[id:D36BCA19-8926-4E06-8C0B-7F607BD3D4EC][compass-pr-merge]]), retries GitHub's transient
