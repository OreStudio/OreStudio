...
| V02 | Pass, with one recorded exception. All four suites pass with the fleet up: =ores.synthetic.api.tests=, =ores.synthetic.core.tests=, =ores.synthetic.service.tests= and =ores.shell.synthetic.tests=, and the codegen suite passes with 524 tests. The exception is the eventing integration test for =ir_curve_generation_config_process_parameter_value=, disabled in its model because its insert trigger cross-checks the config and definition process types and no generic seeding satisfies that. The organisation publisher seed was also made idempotent: three GB cases share a tenant and collided on the second and third because the insert trigger rejects a duplicate before =ON CONFLICT= can absorb it. | =ctest -R synthetic= (4/4 passed); build/audit_synthetic/ctest_green.txt |
...
| V06 | PENDING the exception decision. The tests are green, so only the disabled eventing test stands between synthetic and =COMPONENTS_UNDER_TEST=. The standard keeps a component out until every item passes or an exception is accepted; the entry is a one-line change once the exception is accepted. | =component_registry.py= |
...
- The database recreates from scratch and the fleet serves. All four
  ...
  --component synthetic= reports no drift. The registry entry stays held
  until the exception below is accepted.
...
- One recorded exception. The eventing integration test for
  =ir_curve_generation_config_process_parameter_value= is disabled in the
  model's =* Physical space= table. Its insert trigger requires the
  referenced config and definition process types to agree, and both parents
  are system catalogue rows, so no generic seeding satisfies the check.
  Re-enable it when the template can seed a system-tenant parent that
  matches a sibling field.
* Test Scenarios
