...
ClientMarketSeriesModel::ClientMarketSeriesModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ClientMarketSeriesModel::onDataLoaded);
}
...
QVariant ClientMarketSeriesModel::data(const QModelIndex& index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            ...
            case Version:
                return s.version;
            case ModifiedBy:
                return QString::fromStdString(s.modified_by);
            case RecordedAt:
                return relative_time_helper::format(s.recorded_at);
            default:
                break;
        }
    }
...
QVariant
ClientMarketSeriesModel::headerData(int section, Qt::Orientation orientation, int role) const {
...
const marketdata::domain::market_series* ClientMarketSeriesModel::getSeries(int row) const {
...
void ClientMarketSeriesModel::fetch_data(std::uint32_t offset, std::uint32_t limit) {
    is_fetching_ = true;
...
void ClientMarketSeriesModel::onDataLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch market series: " << result.error_message.toStdString();
