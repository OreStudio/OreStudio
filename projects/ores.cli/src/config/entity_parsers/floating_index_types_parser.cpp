...
/**
 * @brief Creates the options related to adding floating index types.
 */
options_description make_add_floating_index_type_options_description() {
    options_description r("Add Floating index types Options");
...
/**
 * @brief Reads the add configuration from the variables map for floating index types.
 */
ores::cli::config::add_floating_index_type_options
read_add_floating_index_type_options(const variables_map& vm) {
    ores::cli::config::add_floating_index_type_options r;
...
std::optional<options> handle_floating_index_types_command(bool has_help,
    ...
    if (has_help && o.empty()) {
        const std::vector<std::pair<std::string, std::string>> operations = {
            {"export", "Export floating index types to external formats"},
            {"list", "List floating index types as JSON or table"},
            {"delete", "Delete a floating index type by code"},
            {"add", "Add a new floating index type"}};
        print_entity_help("floating_index_types", "Manage floating index types", operations, info);
        return {};
    }

    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception(
            "floating_index_types command requires an operation (export, list, delete, add)"));
    }

    const auto operation = o.front();
    o.erase(o.begin()); // Remove operation from args

    validate_operation("floating_index_types", operation, allowed_operations);
    ...
    if (operation == export_command_name) {
        ...
        if (has_help) {
            print_help_command("floating_index_types export", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::floating_index_types);
    } else if (operation == list_command_name) {
        auto d = add_common_options(make_export_options_description());
        if (has_help) {
            print_help_command("floating_index_types list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        ...
        if (has_help) {
            print_help_command("floating_index_types delete", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.deleting = read_delete_options(vm, entity::floating_index_types);
    } else if (operation == add_command_name) {
        auto d = add_common_options(make_add_floating_index_type_options_description());
        if (has_help) {
            print_help_command("floating_index_types add", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.adding = read_add_floating_index_type_options(vm);
    }
