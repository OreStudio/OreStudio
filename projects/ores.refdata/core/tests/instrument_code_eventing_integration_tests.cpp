...
// notification" pattern end to end for instrument_code -- the
...
using ores::refdata::repository::instrument_code_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_instrument_code_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::instrument_code_changed_event>(
        [&nats](const ores::refdata::eventing::instrument_code_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::instrument_code_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.instrument_code",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::instrument_code_changed_event>(
        "ores.refdata.instrument_code", "ores_refdata_instrument_codes");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::instrument_code_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_instrument_code(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Instrument Code: " << v;

    instrument_code_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.instrument_code") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for instrument_code " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for instrument_code " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_instrument(id_str).has_value());
        REQUIRE(svc.get_instrument_history(id_str).size() == versions.size());
    }
}
