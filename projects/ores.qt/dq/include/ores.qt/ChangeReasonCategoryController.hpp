...
/**
 * @brief Controller for managing change reason category windows and operations.
 *
 * Manages the lifecycle of change reason category list, detail, and history windows.
...
private:
    inline static std::string_view logger_name = "ores.qt.change_reason_category_controller";
...
public:
    ChangeReasonCategoryController(QMainWindow* mainWindow,
                                   QMdiArea* mdiArea,
...
private:
     ...
     * ChangeReasonCategoryDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(ChangeReasonCategoryDetailDialog* detailDialog);
     ...
     */
    void fetchChangeReasonCategoryHistory(
