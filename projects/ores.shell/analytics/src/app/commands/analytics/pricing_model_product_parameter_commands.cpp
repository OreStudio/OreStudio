...
namespace messaging = ores::analytics::messaging;
...
void pricing_model_product_parameter_commands::register_commands(cli::Menu& root_menu,
                                                                 nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("pricing_model_product_parameters");
...
void pricing_model_product_parameter_commands::process_list(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_pricing_model_product_parameters_request;
    ...
    auto result = do_auth_request<messaging::list_pricing_model_product_parameters_response>(
...
void pricing_model_product_parameter_commands::process_get(std::ostream& out,
                                                           nats_client& session,
                                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_pricing_model_product_parameter_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_pricing_model_product_parameter_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_get_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_pricing_model_product_parameters_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_pricing_model_product_parameters_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_add(std::ostream& out,
                                                           nats_client& session,
                                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_pricing_model_product_parameter_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.pricing_model_product_id,
                   parsed->positionals[next++],
                   "pricing_model_product_id");
        read_token(
            req.change.write.parameter_scope, parsed->positionals[next++], "parameter_scope");
    ...
    auto result = do_auth_request<messaging::put_pricing_model_product_parameter_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_set(std::ostream& out,
                                                           nats_client& session,
                                                           const std::vector<std::string>& args) {
...
void pricing_model_product_parameter_commands::process_put_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_pricing_model_product_parameters_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::pricing_model_product_parameter_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.pricing_model_config_id,
                       parsed->positionals[next++],
                       "pricing_model_config_id");
            read_token(change.write.pricing_model_product_id,
                       parsed->positionals[next++],
                       "pricing_model_product_id");
            read_token(
                change.write.parameter_scope, parsed->positionals[next++], "parameter_scope");
    ...
    auto result = do_auth_request<messaging::put_many_pricing_model_product_parameters_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_delete(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_pricing_model_product_parameter_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_pricing_model_product_parameter_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_delete_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_pricing_model_product_parameters_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(
                messaging::pricing_model_product_parameter_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_pricing_model_product_parameters_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_versions(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_pricing_model_product_parameter_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result =
        do_auth_request<messaging::list_pricing_model_product_parameter_versions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void pricing_model_product_parameter_commands::process_version(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_pricing_model_product_parameter_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.pricing_model_product_parameter.parameter_name,
                   parsed->positionals[next++],
                   "parameter_name");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_pricing_model_product_parameter_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
