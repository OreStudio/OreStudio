 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_DIARY_ENTRY_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_DIARY_ENTRY_TYPE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes diary entry types to data storage.
 */
class ORES_REFDATA_CORE_EXPORT diary_entry_type_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.diary_entry_type_repository";
...
public:
    ...
    /**
     * @brief Writes diary entry types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::diary_entry_type& v);
    void write(context ctx, const std::vector<domain::diary_entry_type>& v);
    /**@}*/

    /**
     * @brief Writes a diary entry type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of diary entry types, each honouring its own
    ...
    /**
     * @brief Reads latest diary entry types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::diary_entry_type> read_latest(context ctx);
    std::vector<domain::diary_entry_type> read_latest(context ctx, const std::string& code);
    std::vector<domain::diary_entry_type> read_latest(context ctx,
    ...
    /**
     * @brief Reads all diary entry types, possibly filtered by primary key.
     */
    std::vector<domain::diary_entry_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single diary entry type as it stood at a specific
    ...
    /**
     * @brief Reads latest diary entry types with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Removes a diary entry type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes diary entry types by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::diary_entry_type apply_claim(context ctx,
                                         const domain::diary_entry_type& v,
                                         const ores::utility::domain::precondition& claim);
};
