...
using ores::shell::app::commands::tenor_convention_resolution_commands;
...
TEST_CASE("tenor_convention_resolution_commands_registers_every_derived_verb", tags) {
...
TEST_CASE("tenor_convention_resolution_commands_process_list_requires_a_session", tags) {
    ...
    tenor_convention_resolution_commands::process_list(out, session, tokens(0));
...
TEST_CASE("tenor_convention_resolution_commands_process_get_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    tenor_convention_resolution_commands::process_get(out, session, tokens(2));
...
TEST_CASE("tenor_convention_resolution_commands_process_get_reports_the_expected_count", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    tenor_convention_resolution_commands::process_get(out, session, {});

    BOOST_LOG_SEV(lg, debug) << "Output for an empty argument list: " << out.str();
    CHECK(command_feedback::failed());
}

TEST_CASE("tenor_convention_resolution_commands_process_get_many_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("tenor_convention_resolution_commands_process_by_convention_code_requires_a_session",
          tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    tenor_convention_resolution_commands::process_by_convention_code(out, session, tokens(1));
...
TEST_CASE(
    "tenor_convention_resolution_commands_process_by_convention_code_reports_the_expected_count",
    tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    tenor_convention_resolution_commands::process_by_convention_code(out, session, {});
