 ...
 */
std::vector<domain::derivation_kind> read_one(repository::derivation_kind_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::derivation_kind_key& key) {
 ...
 */
messaging::derivation_kind_key key_from(const domain::derivation_kind& v) {
    messaging::derivation_kind_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::derivation_kind to_domain(const messaging::derivation_kind_write& write) {
    domain::derivation_kind v;
    v.code = write.code;
    v.name = write.name;
...
messaging::list_derivation_kinds_response derivation_kind_service::list_derivation_kinds(
    ...
    messaging::list_derivation_kinds_response response;
...
messaging::get_derivation_kind_response derivation_kind_service::get_derivation_kind(
    const messaging::get_derivation_kind_request& request) {
    messaging::get_derivation_kind_response response;
    ...
    response.derivation_kind = std::move(found.front());
    return response;
}

messaging::get_many_derivation_kinds_response derivation_kind_service::get_many_derivation_kinds(
    const messaging::get_many_derivation_kinds_request& request) {
    messaging::get_many_derivation_kinds_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::derivation_kind_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.derivation_kind = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_derivation_kind_response derivation_kind_service::put_derivation_kind(
    const messaging::put_derivation_kind_request& request) {
    messaging::put_derivation_kind_response response;
    domain::derivation_kind value;
    ...
    if (!written.empty())
        response.derivation_kind = std::move(written.front());
    return response;
}

messaging::put_many_derivation_kinds_response derivation_kind_service::put_many_derivation_kinds(
    const messaging::put_many_derivation_kinds_request& request) {
    messaging::put_many_derivation_kinds_response response;
...
messaging::delete_derivation_kind_response derivation_kind_service::delete_derivation_kind(
    const messaging::delete_derivation_kind_request& request) {
    messaging::delete_derivation_kind_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::derivation_kind_repository::remove_status::removed:
            break;
        case repository::derivation_kind_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::derivation_kind_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::derivation_kind_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
derivation_kind_service::delete_many_derivation_kinds(
    const messaging::delete_many_derivation_kinds_request& request) {
    messaging::delete_many_derivation_kinds_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
derivation_kind_service::list_derivation_kind_versions(
    const messaging::list_derivation_kind_versions_request& request) {
    messaging::list_derivation_kind_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
derivation_kind_service::get_derivation_kind_version(
    const messaging::get_derivation_kind_version_request& request) {
    messaging::get_derivation_kind_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.derivation_kind.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
derivation_kind_service::prepare_change(const messaging::derivation_kind_change& change,
...
derivation_kind_service::get_kinds(const std::vector<std::string>& codes) {
