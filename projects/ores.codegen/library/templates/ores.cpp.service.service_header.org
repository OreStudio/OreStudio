...
#+type: archetype
#+output: projects/{component_core_dir}/include/ores.{component_core}/service/{entity}_service.hpp
...
public:
    ...
    /**
     * @brief Lists {{name_words}} with pagination.
     */
    std::vector<domain::{{name_singular}}>
    list_{{name_short}}(std::uint32_t offset, std::uint32_t limit);
...
{{#wire_write_enabled}}
    ...
    /**
     * @brief Saves a batch of {{name_words}} in one transaction.
     *
     * @param {{name_short}} The {{name_words}} to save
     */
    void save_{{name_short}}(const std::vector<domain::{{name_singular}}>& {{name_short}});
...
{{#left.replace_by}}
    /**
     * @brief Replaces the {{name_words}} for a {{left.column_title_lower}}.
     *
     * Soft-closes the currently active rows for the given
     * {{left.column_title_lower}} and inserts the rows in @p {{name_short}},
     * so the active set exactly matches the caller's list.
     *
     * @param {{left.column}} The {{left.column_title_lower}}
     * @param {{name_short}} The rows that make up the new active set
     */
    void replace_{{name_short}}_by_{{left.column_short}}(
{{#left.is_uuid}}
        const boost::uuids::uuid& {{left.column}},
{{/left.is_uuid}}
{{^left.is_uuid}}
...
{{#right.replace_by}}
    /**
     * @brief Replaces the {{name_words}} for a {{right.column_title_lower}}.
     *
     * Soft-closes the currently active rows for the given
     * {{right.column_title_lower}} and inserts the rows in @p {{name_short}},
     * so the active set exactly matches the caller's list.
     *
     * @param {{right.column}} The {{right.column_title_lower}}
     * @param {{name_short}} The rows that make up the new active set
     */
    void replace_{{name_short}}_by_{{right.column_short}}(
{{#right.is_uuid}}
        const boost::uuids::uuid& {{right.column}},
{{/right.is_uuid}}
{{^right.is_uuid}}
