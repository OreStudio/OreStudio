...
void PartyMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &PartyMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &PartyMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &PartyMdiWindow::deleteSelected);
...
void PartyMdiWindow::setupConnections() {
    connect(model_, &ClientPartyModel::dataLoaded, this, &PartyMdiWindow::onDataLoaded);
    connect(model_, &ClientPartyModel::loadError, this, &PartyMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &PartyMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &PartyMdiWindow::onDoubleClicked);
...
void PartyMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 parties").arg(loaded).arg(total));
...
void PartyMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void PartyMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* party = model_->getParty(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showPartyHistory for code: " << party->short_code;
...
void PartyMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(this, "Disconnected", "Cannot delete party while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* party = model_->getParty(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(party->id));
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " parties";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage = QString("Are you sure you want to delete %1 parties?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Party", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug) << "Making delete request for " << ids.size() << " parties";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                ...
                BOOST_LOG_SEV(lg(), error) << "Party deletion failed: " << code << " - " << message;
                failure_count++;
