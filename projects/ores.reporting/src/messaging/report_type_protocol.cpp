...
void write_report_type(std::vector<std::byte>& buffer,
    const domain::report_type& rt) {
...
std::expected<domain::report_type, error_code>
read_report_type(std::span<const std::byte>& data) {
    domain::report_type rt;
...
// ============================================================================
// Report Type Messages Implementation
// ============================================================================

std::vector<std::byte> get_report_types_request::serialize() const {
    return {};
}

std::expected<get_report_types_request, error_code>
get_report_types_request::deserialize(std::span<const std::byte> data) {
    if (!data.empty()) {
        return std::unexpected(error_code::payload_too_large);
    }
    return get_report_types_request{};
}
...
std::vector<std::byte> get_report_types_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& rt : types) {
        write_report_type(buffer, rt);
    }
    return buffer;
}

std::expected<get_report_types_response, error_code>
get_report_types_response::deserialize(std::span<const std::byte> data) {
    get_report_types_response response;
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        auto result = read_report_type(data);
...
std::vector<std::byte> save_report_type_request::serialize() const {
    std::vector<std::byte> buffer;
    write_report_type(buffer, type);
    return buffer;
}

std::expected<save_report_type_request, error_code>
save_report_type_request::deserialize(std::span<const std::byte> data) {
    save_report_type_request request;
...
std::vector<std::byte> save_report_type_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_report_type_response, error_code>
save_report_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> delete_report_type_request::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_report_type_request, error_code>
delete_report_type_request::deserialize(std::span<const std::byte> data) {
    delete_report_type_request request;
...
std::vector<std::byte> delete_report_type_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_report_type_response, error_code>
delete_report_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> get_report_type_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_report_type_history_request, error_code>
get_report_type_history_request::deserialize(std::span<const std::byte> data) {
    get_report_type_history_request request;
...
std::vector<std::byte> get_report_type_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_report_type(buffer, v);
    }
    return buffer;
}

std::expected<get_report_type_history_response, error_code>
get_report_type_history_response::deserialize(std::span<const std::byte> data) {
