...
[[proj:assets/images/ore_studio-v0.0.18.png]]
...
- *Lock down uppercase UUID invariant*: All =:ID:= properties and =[[id:UUID]]= links in [[proj:doc][=doc/=]] must be uppercase; all code paths that mint UUIDs produce uppercase by construction.
...
- *Add org-roam index links to compass.org folder entries*: Every folder entry in [[proj:doc/compass.org][=doc/compass.org=]] links to its index file.
...
- *Port domain concept notes to knowledge documents*: Domain notes converted to knowledge documents under [[proj:doc/knowledge/domain][=doc/knowledge/domain/=]].
...
- *Move .build-*.el scripts to ores.lisp*: Relocate five build scripts from the project root into [[proj:projects/ores.lisp/src][=projects/ores.lisp/src/=]] with the =ores-= prefix convention.
