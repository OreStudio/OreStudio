 ...
 *
 */
#ifndef ORES_QT_PARTY_HIERARCHY_TAB_HPP
#define ORES_QT_PARTY_HIERARCHY_TAB_HPP
...
/**
 * @brief Owns and drives the "Hierarchy" tab embedded in PartyDetailDialog,
 * showing the subtree rooted at the displayed party via parent_party_id
 ...
 */
class PartyHierarchyTab final : public QObject {
    Q_OBJECT

public:
    explicit PartyHierarchyTab(QWidget* dialogParent);

    /** @brief Add the tab to the dialog's tab widget. Call once, from the constructor. */
    void attachTo(QTabWidget* tabWidget);

    /** @brief Re-fetch and redisplay the subtree rooted at partyId. No-op if partyId is nil. */
    void reload(const boost::uuids::uuid& partyId, ClientManager* clientManager);
