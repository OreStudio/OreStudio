...
void print_domain_help(std::ostream& info) {
    ...
    info << std::endl << "Use: ores.cli dq <entity> <operation> --help for details." << std::endl;
}
...
handle_dq_command(bool has_help, const parsed_options& po, std::ostream& info, variables_map& vm) {
    ...
    if (entity_name == change_reasons_command_name) {
        return entity_parsers::handle_change_reasons_command(has_help, new_po, info, vm);
    } else if (entity_name == change_reason_categories_command_name) {
        return entity_parsers::handle_change_reason_categories_command(has_help, new_po, info, vm);
    }
