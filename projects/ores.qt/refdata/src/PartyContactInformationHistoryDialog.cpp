...
#include "ui_PartyContactInformationHistoryDialog.h"
...
PartyContactInformationHistoryDialog::PartyContactInformationHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::PartyContactInformationHistoryDialog)
    , id_(id)
    , code_(code)
...
QString PartyContactInformationHistoryDialog::code() const {
    return code_;
}

void PartyContactInformationHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for party contact information: "
                               << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_party_contact_information_history_request request;
    request.id = boost::uuids::to_string(id_);

    QPointer<PartyContactInformationHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](refdata::messaging::get_party_contact_information_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int PartyContactInformationHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString PartyContactInformationHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow PartyContactInformationHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult PartyContactInformationHistoryDialog::calculateDiffAt(int ci,
                                                                                    int pi) const {
    DiffResult diffs;
...
void PartyContactInformationHistoryDialog::displayFullDetails(int index) {
...
void PartyContactInformationHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void PartyContactInformationHistoryDialog::revertToVersionAt(int index) {
