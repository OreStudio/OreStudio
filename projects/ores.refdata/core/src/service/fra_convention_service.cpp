 ...
 */
std::vector<domain::fra_convention> read_one(repository::fra_convention_repository& repo,
                                             const ores::database::context& ctx,
                                             const messaging::fra_convention_key& key) {
 ...
 */
messaging::fra_convention_key key_from(const domain::fra_convention& v) {
    messaging::fra_convention_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::fra_convention to_domain(const messaging::fra_convention_write& write) {
...
messaging::list_fra_conventions_response fra_convention_service::list_fra_conventions(
    ...
    messaging::list_fra_conventions_response response;
    ...
    response.fra_conventions = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_fra_convention_count(ctx_);
    return response;
}

messaging::get_fra_convention_response
fra_convention_service::get_fra_convention(const messaging::get_fra_convention_request& request) {
    messaging::get_fra_convention_response response;
    ...
    response.fra_convention = std::move(found.front());
    return response;
}

messaging::get_many_fra_conventions_response fra_convention_service::get_many_fra_conventions(
    const messaging::get_many_fra_conventions_request& request) {
    messaging::get_many_fra_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::fra_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.fra_convention = std::move(found.front());
...
fra_convention_service::put_fra_convention(const messaging::put_fra_convention_request& request) {
    messaging::put_fra_convention_response response;
    domain::fra_convention value;
    ...
    if (!written.empty())
        response.fra_convention = std::move(written.front());
    return response;
}

messaging::put_many_fra_conventions_response fra_convention_service::put_many_fra_conventions(
    const messaging::put_many_fra_conventions_request& request) {
    messaging::put_many_fra_conventions_response response;
    std::vector<domain::fra_convention> batch;
    ...
    response.fra_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.fra_conventions.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_fra_convention_response fra_convention_service::delete_fra_convention(
    const messaging::delete_fra_convention_request& request) {
    messaging::delete_fra_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::fra_convention_repository::remove_status::removed:
            break;
        case repository::fra_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::fra_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::fra_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_fra_conventions_response fra_convention_service::delete_many_fra_conventions(
    const messaging::delete_many_fra_conventions_request& request) {
    messaging::delete_many_fra_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
fra_convention_service::list_fra_convention_versions(
    const messaging::list_fra_convention_versions_request& request) {
    messaging::list_fra_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_fra_convention_version_response fra_convention_service::get_fra_convention_version(
    const messaging::get_fra_convention_version_request& request) {
    messaging::get_fra_convention_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.fra_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
fra_convention_service::prepare_change(const messaging::fra_convention_change& change,
...
std::vector<domain::fra_convention>
fra_convention_service::get_fra_conventions(const std::vector<std::string>& ids) {
