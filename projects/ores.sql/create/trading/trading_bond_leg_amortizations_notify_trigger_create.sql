...
create or replace function ores_trading_bond_leg_amortizations_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    entity_name text := 'ores.trading.bond_leg_amortization';
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity_ids', jsonb_build_array(changed_instrument_id, changed_leg_role, changed_leg_number, changed_sequence_number),
        'tenant_id', changed_tenant_id
    );

    perform pg_notify('ores_trading_bond_leg_amortizations', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_trading_bond_leg_amortizations_notify_trg
after insert or update or delete on ores_trading_bond_leg_amortizations_tbl
for each row execute function ores_trading_bond_leg_amortizations_notify_fn();
