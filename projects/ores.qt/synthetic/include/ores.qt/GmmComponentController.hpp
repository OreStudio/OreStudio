 ...
 */
#ifndef ORES_QT_GMM_COMPONENT_CONTROLLER_HPP
#define ORES_QT_GMM_COMPONENT_CONTROLLER_HPP
...
public:
    GmmComponentController(QMainWindow* mainWindow,
                           QMdiArea* mdiArea,
...
private:
     ...
     * GmmComponentDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(GmmComponentDetailDialog* detailDialog);
    void showHistoryWindow(const synthetic::domain::gmm_component& gmm_component);

    /**
     * @brief Fetches the full typed GMM component history (the
     * existing per-entity synthetic::messaging::get_gmm_component_history_request/
     * synthetic::messaging::get_gmm_component_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed GMM component, since the
     * generic dialog holds no typed domain data.
     */
    void fetchGmmComponentHistory(
