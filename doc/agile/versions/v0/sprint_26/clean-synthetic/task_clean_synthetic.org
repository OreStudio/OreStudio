...
| G05 | Partial. One generator defect found and fixed at source: the eventing test's FK-chain seed variable was named after the FK column, so a chain step whose column repeated an earlier step's column declared the same variable twice and compiled the wrong type. Two findings are recorded, not fixed, because both are template-level and house-wide: generated types carry no export macro, and the generator appends a uniqueness counter to every text code, so a code-validated lookup (synthetic's =process_type_code=) can never carry a valid value in a seeded test. | commit a7ee75fb4b; =pytest projects/ores.codegen/tests= (537 passed); V02 |
...
| V02 | Fail on the live half. The codegen suite passes (537 tests), and =ores.synthetic.api.tests=, =ores.synthetic.service.tests= and =ores.shell.synthetic.tests= pass with the fleet up. =ores.synthetic.core.tests= fails 3 of 67: the new parameter-value eventing test seeds a process-parameter definition whose =process_type_code= cannot be one of the five valid codes, because the generator template appends a uniqueness counter to every text code; and two pre-existing =Row already exists= reference-data collisions that a database recreate clears. | =ctest -R synthetic=; =ores.synthetic.core.tests= |
| V03 | PENDING. The database recreate and fleet start need the environment provisioned (24 service units are not loaded). | =compass services status= |
| V04 | PENDING. The generated shell commands need the live fleet. | |
...
| V06 | PENDING. The registry entry was held back: V02 still fails, and the standard keeps a component out of =COMPONENTS_UNDER_TEST= until every item passes or an exception is accepted. =check_component_drift.py --all= passes with synthetic temporarily listed; restoring the entry is a one-line change once V02 is green. | =check_component_drift.py --all= with synthetic listed |
...
- PR #2138 (=feature/remove-the-legacy-qt-remnants=) is open and edits the
  ...
  lands.
- The full tree builds and =check_component_drift.py --all= is green with
  synthetic registered. What stays open is the live half: the component's
  ctest suites with the fleet up (V02), the database recreate (V03) and the
  generated shell commands against the live fleet (V04). The environment
  has no service units loaded after the machine restart.
