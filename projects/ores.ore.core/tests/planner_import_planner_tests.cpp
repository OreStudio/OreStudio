...
TEST_CASE("plan_trades_have_book_id_and_portfolio_id_stamped", tags) {
    ...
    for (const auto& item : plan.trades) {
        INFO("Trade: " << item.trade.identity.get().identity.get().external_id);
        CHECK(item.trade.parties.get().book_id != nil);
        CHECK(item.trade.parties.get().portfolio_id != nil);
    }
...
TEST_CASE("plan_instrument_trade_id_matches_minted_trade_id", tags) {
    ...
    int checked = 0;
    for (const auto& item : plan.trades) {
        INFO("Trade external_id: " << item.trade.identity.get().identity.get().external_id);
        REQUIRE(item.trade.identity.get().identity.get().id != nil);
        ...
        std::visit([&](const auto& r) {
            ...
            if constexpr (std::is_same_v<T, std::monostate>) {
                ...
                CHECK(*r.instrument.trade_id == item.trade.identity.get().id);
                ++checked;
            } else {
...
TEST_CASE("plan_trade_defaults_override_parsed_values", tags) {
    ...
    for (const auto& item : plan.trades) {
        INFO("Trade: " << item.trade.identity.get().external_id);
        CHECK(item.trade.lifecycle.get().trade_date == "2026-01-01");
        CHECK(item.trade.classification.get().activity_type_code == "novation");
    }
}
