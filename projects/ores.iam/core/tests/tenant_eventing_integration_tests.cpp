...
TEST_CASE("write_tenant_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::iam::eventing::tenant_changed_event>(
        [&nats](const ores::iam::eventing::tenant_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<ores::iam::eventing::tenant_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.iam.tenant",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.tenant_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::iam::eventing::tenant_changed_event>("ores.iam.tenant",
                                                                             "ores_iam_tenants");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<ores::iam::eventing::tenant_changed_event>::name), 10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.iam.tenant") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenant " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenant " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::iam::service::tenant_service svc(crud_ctx);
