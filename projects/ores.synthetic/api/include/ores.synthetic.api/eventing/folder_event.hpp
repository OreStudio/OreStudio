 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_FOLDER_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_FOLDER_EVENT_HPP
...
/**
 * @brief Event traits for folder's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::synthetic::messaging::folder_event> {
    static constexpr std::string_view subject_prefix = "synthetic.v1.folders_events";

    static ores::synthetic::messaging::folder_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::folder_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::synthetic::messaging::folder_key>(notification.key))
            event.key = *key;
