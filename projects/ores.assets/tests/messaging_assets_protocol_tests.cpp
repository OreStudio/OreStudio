...
// get_currency_images_request tests

TEST_CASE("get_currency_images_request_serialize_empty", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_request serialization";

    get_currency_images_request request;
    auto serialized = request.serialize();

    REQUIRE(serialized.empty());
}

TEST_CASE("get_currency_images_request_deserialize_empty", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_request deserialization";

    std::span<const std::byte> empty_data;
    auto result = get_currency_images_request::deserialize(empty_data);

    REQUIRE(result.has_value());
}

TEST_CASE("get_currency_images_request_deserialize_non_empty_fails", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_request deserialization with non-empty data";

    std::vector<std::byte> data{std::byte{0x01}};
    auto result = get_currency_images_request::deserialize(data);

    REQUIRE_FALSE(result.has_value());
    REQUIRE(result.error() == error_code::invalid_request);
}

TEST_CASE("get_currency_images_request_roundtrip", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_request roundtrip";

    get_currency_images_request request;
    auto serialized = request.serialize();
    auto result = get_currency_images_request::deserialize(serialized);

    REQUIRE(result.has_value());
}

TEST_CASE("get_currency_images_request_stream_operator", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_request stream operator";
...
// get_currency_images_response tests

TEST_CASE("get_currency_images_response_serialize_empty", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response serialization with empty list";
...
TEST_CASE("get_currency_images_response_roundtrip_empty", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response roundtrip with empty list";

    get_currency_images_response original;
    auto serialized = original.serialize();
    auto result = get_currency_images_response::deserialize(serialized);

    REQUIRE(result.has_value());
    REQUIRE(result->currency_images.empty());
}

TEST_CASE("get_currency_images_response_roundtrip_single_item", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response roundtrip with single item";

    get_currency_images_response original;
    original.currency_images.push_back({
        .iso_code = "USD",
        .image_id = "img-001",
        .assigned_by = "admin",
    ...
    REQUIRE(result.has_value());
    REQUIRE(result->currency_images.size() == 1);
    REQUIRE(result->currency_images[0].iso_code == "USD");
    REQUIRE(result->currency_images[0].image_id == "img-001");
    REQUIRE(result->currency_images[0].assigned_by == "admin");
    REQUIRE(result->currency_images[0].assigned_at == "2025-01-01T00:00:00Z");
}

TEST_CASE("get_currency_images_response_roundtrip_multiple_items", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response roundtrip with multiple items";
    ...
    original.currency_images.push_back({
        ...
        .image_id = "img-002",
        .assigned_by = "user1",
        .assigned_at = "2025-01-02T12:30:00Z"
    });
    original.currency_images.push_back({
        .iso_code = "GBP",
        .image_id = "img-003",
        .assigned_by = "user2",
        .assigned_at = "2025-01-03T18:45:00Z"
    });
    ...
    REQUIRE(result.has_value());
    REQUIRE(result->currency_images.size() == 3);
    REQUIRE(result->currency_images[0].iso_code == "USD");
    REQUIRE(result->currency_images[1].iso_code == "EUR");
    REQUIRE(result->currency_images[2].iso_code == "GBP");
}

TEST_CASE("get_currency_images_response_stream_operator", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response stream operator";

    get_currency_images_response response;
    response.currency_images.push_back({
        .iso_code = "USD",
        .image_id = "img-001",
        .assigned_by = "admin",
        .assigned_at = "2025-01-01T00:00:00Z"
...
TEST_CASE("get_currency_images_response_deserialize_truncated_data", tags) {
    auto lg(make_logger(test_suite));
    BOOST_LOG_SEV(lg, info) << "Testing get_currency_images_response deserialization with truncated data";

    // Create a response with one item and truncate the serialized data
    get_currency_images_response original;
    original.currency_images.push_back({
        .iso_code = "USD",
        .image_id = "img-001",
