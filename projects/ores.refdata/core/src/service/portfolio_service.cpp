...
portfolio_service::list_portfolios(const messaging::list_portfolios_request& request) {
    messaging::list_portfolios_response response;
...
portfolio_service::get_portfolio(const messaging::get_portfolio_request& request) {
    messaging::get_portfolio_response response;
    ...
    response.portfolio = std::move(found.front());
    return response;
}

messaging::get_many_portfolios_response
portfolio_service::get_many_portfolios(const messaging::get_many_portfolios_request& request) {
    messaging::get_many_portfolios_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::portfolio_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.portfolio = std::move(found.front());
...
portfolio_service::put_portfolio(const messaging::put_portfolio_request& request) {
    messaging::put_portfolio_response response;
    ...
    if (!written.empty())
        response.portfolio = std::move(written.front());
    return response;
}

messaging::put_many_portfolios_response
portfolio_service::put_many_portfolios(const messaging::put_many_portfolios_request& request) {
    messaging::put_many_portfolios_response response;
    std::vector<domain::portfolio> batch;
    ...
    response.portfolios.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.portfolios.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_portfolio_response
portfolio_service::delete_portfolio(const messaging::delete_portfolio_request& request) {
    messaging::delete_portfolio_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        ...
        case repository::portfolio_repository::remove_status::missing:
            ...
            response.result.code = "not_found";
            break;
        case repository::portfolio_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::portfolio_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_portfolios_response portfolio_service::delete_many_portfolios(
    const messaging::delete_many_portfolios_request& request) {
    messaging::delete_many_portfolios_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_portfolio_versions_response portfolio_service::list_portfolio_versions(
    const messaging::list_portfolio_versions_request& request) {
    messaging::list_portfolio_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
portfolio_service::prepare_change(const messaging::portfolio_change& change,
...
std::optional<domain::portfolio> portfolio_service::find_portfolio(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Finding portfolio. " << "id: " << id;
...
portfolio_service::get_portfolios(const std::vector<std::string>& ids) {
