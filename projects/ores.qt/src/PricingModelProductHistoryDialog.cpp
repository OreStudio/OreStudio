...
void PricingModelProductHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &PricingModelProductHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &PricingModelProductHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &PricingModelProductHistoryDialog::onRevertClicked);
...
void PricingModelProductHistoryDialog::loadHistory() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Loading history for pricing model product: " << code_.toStdString();
...
void PricingModelProductHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.model != previous.model) {
        addChange("Model",
                  QString::fromStdString(previous.model),
                  QString::fromStdString(current.model));
    }
...
void PricingModelProductHistoryDialog::updateFullDetails(int versionIndex) {
    ...
    ui_->pricingEngineTypeCodeValue->setText(QString::fromStdString(version.pricing_engine_type_code));
    ui_->modelValue->setText(QString::fromStdString(version.model));
