...
swap_leg_mapper::map(const swap_leg_entity& v) {
    ...
    r.fixed_rate = v.fixed_rate.value_or(0.0);
    r.spread = v.spread.value_or(0.0);
...
swap_leg_mapper::map(const domain::swap_leg& v) {
    ...
    r.business_day_convention_code = v.business_day_convention_code;
    r.payment_frequency_code = v.payment_frequency_code;
    r.floating_index_code = v.floating_index_code.empty() ? std::nullopt : std::optional(v.floating_index_code);
    r.fixed_rate = std::optional(v.fixed_rate);
    r.spread = std::optional(v.spread);
    r.notional = v.notional;
    r.currency = v.currency;
    r.modified_by = v.modified_by;
    r.performed_by = v.performed_by;
