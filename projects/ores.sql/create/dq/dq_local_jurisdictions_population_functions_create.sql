 ...
 *   SELECT * FROM metadata.dq_preview_local_jurisdiction_population_fn(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM metadata.dq_populate_local_jurisdictions_fn(dataset_id, 'upsert');
 */
 ...
 */
create or replace function metadata.dq_preview_local_jurisdiction_population_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_local_jurisdictions_artefact_tbl dq
    left join production.refdata_local_jurisdictions_tbl existing
 ...
 */
create or replace function metadata.dq_populate_local_jurisdictions_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_local_jurisdictions_tbl existing
        ...
        insert into production.refdata_local_jurisdictions_tbl (
