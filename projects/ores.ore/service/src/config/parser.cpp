 ...
 *
 */
#include "ores.ore.service/config/parser.hpp"
...
const std::string product_version("ores.ore.service v" ORES_VERSION);
...
using ores::ore::service::config::options;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.ore.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "ORE import microservice" << std::endl << std::endl
         << "Usage: ores.ore.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("ORE_SERVICE"));
