...
instrument_schedule_date_service::get_instrument_schedule_date_at_version(
    ...
    return repo_.read_at_version(
        ctx_, instrument_id, leg_role, leg_number, schedule_role, sequence_number, version);
}

std::optional<domain::instrument_schedule_date>
instrument_schedule_date_service::get_instrument_schedule_date(const std::string& instrument_id,
                                                               const std::string& leg_role,
                                                               const std::string& leg_number,
                                                               const std::string& schedule_role,
                                                               const std::string& sequence_number) {
    ...
    auto results = repo_.read_latest(
        ctx_, instrument_id, leg_role, leg_number, schedule_role, sequence_number);
    if (results.empty())
        return std::nullopt;
...
void instrument_schedule_date_service::save_instrument_schedule_date(
    ...
    if (v.leg_role.empty())
        throw std::invalid_argument("Instrument Schedule Date leg_role cannot be empty.");
...
void instrument_schedule_date_service::delete_instrument_schedule_date(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Removing instrument schedule date. "
                               ...
                               << " schedule_role: " << schedule_role
                               << " sequence_number: " << sequence_number;
    repo_.remove(ctx_, instrument_id, leg_role, leg_number, schedule_role, sequence_number);
...
void instrument_schedule_date_service::delete_instrument_schedule_dates(
    ...
    repo_.remove(ctx_, instrument_ids, leg_roles, leg_numbers, schedule_roles, sequence_numbers);
}
