...
void knock_out_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    knock_out_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_knock_out_swap_instrument(std::ref(out),
                                                  std::ref(session),
                                                  ...
                                                  std::move(knock_out_dates_json),
                                                  std::move(description),
                                                  std::move(change_reason_code),
                                                  std::move(change_commentary));
        },
        "Add an Knock-Out swap instrument (trade_type_code start_date maturity_date barrier_level "
        "barrier_type [knock_out_dates_json] [description] change_reason_code "
        "\"change_commentary\")",
        {"trade_type_code start_date maturity_date barrier_level barrier_type knock_out_dates_json "
...
void knock_out_swap_instrument_commands::process_get_knock_out_swap_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Knock-Out swap instruments request.";
...
void knock_out_swap_instrument_commands::process_add_knock_out_swap_instrument(
    ...
    std::string change_reason_code,
    std::string change_commentary) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add Knock-Out swap instrument request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an Knock-Out swap instrument." << std::endl;
        return;
    }
    ...
    v.barrier_level = barrier_level;
    v.barrier_type = std::move(barrier_type);
    v.knock_out_dates_json = (knock_out_dates_json == "-") ? "" : std::move(knock_out_dates_json);
...
void knock_out_swap_instrument_commands::process_delete_knock_out_swap_instrument(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete Knock-Out swap instrument request for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an Knock-Out swap instrument." << std::endl;
        return;
    }
...
void knock_out_swap_instrument_commands::process_get_knock_out_swap_instrument_history(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Knock-Out swap instrument history for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get Knock-Out swap instrument history." << std::endl;
        return;
    }
