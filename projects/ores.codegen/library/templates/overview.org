...
#+description: Top of the literate template hierarchy: how the 140 mustache templates are organised into facet groups and facets, how tangling works, and where to add a new template.
#+type: knowledge
#+level: cross
...
The 140 mustache templates consumed by =generator.py= are organised
...
| [[id:1BBCEB58-83E0-40F7-BC85-FE2B98E9D2FD][doc]] | 18 | Org-mode document scaffolds behind =compass add= (including the facet/facet_group types). |
...
2. Add a section: prose first (role, model inputs, output artefact,
   ...
   first line.
3. Run the tangle; wire the template into [[file:../profiles.json][profiles.json]].
4. Update the group doc's =* Facets= table if the facet is new.

* Status

The cmake facet is fully literate (the pilot). The cpp, sql, doc and
assets groups are scaffolded; their facet docs are being migrated by
the rollout task of the literate-templates story.
