 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_FX_SPOT_GENERATION_CONFIG_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_FX_SPOT_GENERATION_CONFIG_EVENT_HPP
...
/**
 * @brief Event traits for fx spot generation config's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::synthetic::messaging::fx_spot_generation_config_event> {
    static constexpr std::string_view subject_prefix =
        "synthetic.v1.fx_spot_generation_configs_events";

    static ores::synthetic::messaging::fx_spot_generation_config_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::fx_spot_generation_config_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::synthetic::messaging::fx_spot_generation_config_key>(
                    notification.key))
            event.key = *key;
