 ...
 *
 */
#ifndef ORES_QT_CREDIT_INSTRUMENT_FORM_HPP
#define ORES_QT_CREDIT_INSTRUMENT_FORM_HPP
...
/**
 * @brief @c IInstrumentForm subclass owning the credit instrument editor.
 *
 * Hosts an internal @c QTabWidget with two pages: economics (always visible)
 * and extensions (revealed when @c has_extension is true on the trade type
 * row, e.g. CreditDefaultSwapOption, IndexCreditDefaultSwap, SyntheticCDO).
 */
class CreditInstrumentForm final : public IInstrumentForm {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.credit_instrument_form";
...
public:
    explicit CreditInstrumentForm(QWidget* parent = nullptr);
    ~CreditInstrumentForm() override;
