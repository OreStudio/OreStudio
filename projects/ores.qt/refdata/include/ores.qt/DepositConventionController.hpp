...
private:
    ...
    /**
     * @brief Fetches the full typed deposit convention history (the
     * existing per-entity refdata::messaging::get_deposit_convention_history_request/
     * refdata::messaging::get_deposit_convention_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed deposit convention, since the
     * generic dialog holds no typed domain data.
     */
    void fetchDepositConventionHistory(
