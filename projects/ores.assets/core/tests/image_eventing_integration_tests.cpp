...
#include "ores.assets.api/eventing/image_event.hpp"
...
// notification" pattern end to end for image -- the
...
TEST_CASE("write_image_publishes_an_event", tags) {
    ...
    using event_type = ores::assets::messaging::image_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_assets_images");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.key == v.key)
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for image " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for image " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::assets::service::image_service svc(crud_ctx);
