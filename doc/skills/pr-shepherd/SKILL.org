...
# e.g.
./build/scripts/pr-watch.sh 574
./build/scripts/pr-watch.sh 574 --interval 10   # poll every 10 minutes
#+END_SRC
...
[[*Detailed Instructions][Detailed Instructions]] below.
...
1. Check CI status immediately — if the build is already red, fix it before
   waiting.
...
Before the first sleep, check whether CI is already red.  A push can
immediately trigger a pre-existing failure (e.g. a broken dependency or a
lint check that fires on the new code).  Do not silently wait through a
known-broken build.
...
- If any check has already *failed*, go directly to [[*Step 5: Fix CI Failures][Step 5]] and fix it before
  sleeping.
- If checks are pending or all green, proceed to Step 2.

** Step 2: Wait for Additional Feedback

After confirming CI is not already red, pause for the chosen interval to allow
CI to complete and reviewers to post comments.
...
5. *Required*: reply to each review comment on GitHub, referencing the fix
   commit.  Do not skip this step — the reviewer must be able to see what
   changed and in which commit.
...
6. *Required*: resolve each review thread on GitHub using the GraphQL API.
   Threads must be marked resolved so the PR dashboard shows no outstanding
   comments.
...
4. Return to Step 2 (wait the chosen interval) and repeat the loop.
