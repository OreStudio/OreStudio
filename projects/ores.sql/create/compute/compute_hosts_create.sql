...
create table if not exists "ores_compute_hosts_tbl" (
    "id" uuid not null,
...
create or replace function ores_compute_hosts_insert_fn()
returns trigger as $$
declare
...
create or replace trigger ores_compute_hosts_insert_trg
before insert on "ores_compute_hosts_tbl"
for each row execute function ores_compute_hosts_insert_fn();

create or replace rule ores_compute_hosts_delete_rule as
on delete to "ores_compute_hosts_tbl" do instead
    update "ores_compute_hosts_tbl"
    set valid_to = current_timestamp
