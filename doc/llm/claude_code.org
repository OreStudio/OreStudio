...
that drives ORE Studio's day-to-day development. Claude is a [[id:0D919421-2B73-4322-9F79-9FDA38A67EA1][large
language model]], and the properties of that class of system, described
externally on its own page, set the limits this tool works within. It runs inside the
...
[[proj:.claude][=.claude/=]]; skills, runbooks, and recipes give Claude Code the
...
Session configuration is layered: [[proj:.claude/settings.json][=.claude/settings.json=]] controls
the tool permission allow-list and sandbox overrides; [[proj:.claude/skills][=.claude/skills/=]]
contains task-shaped playbooks; [[proj:doc/llm/runbooks][=doc/llm/runbooks/=]] contains
multi-step procedures; and [[proj:doc/recipes][=doc/recipes/=]] holds individual how-tos
...
[[proj:.claude/settings.json][=.claude/settings.json=]] directly; regenerate it with the
=deploy_settings= CMake target. Full details in
[[id:2D83251F-0E24-4B97-B736-08F710316634][Claude Code Settings]].

* See also

- [[id:0D919421-2B73-4322-9F79-9FDA38A67EA1][Large language models]] — the technology this tool provides an interface to, defined externally.
