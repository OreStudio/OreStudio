...
void MonetaryNatureHistoryDialog::markAsStale() {
    loadHistory();
}
...
void MonetaryNatureHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &MonetaryNatureHistoryDialog::onVersionSelected);
...
void MonetaryNatureHistoryDialog::loadHistory() {
    ...
    using HistoryResult = std::expected<refdata::messaging::get_monetary_nature_history_response, std::string>;
