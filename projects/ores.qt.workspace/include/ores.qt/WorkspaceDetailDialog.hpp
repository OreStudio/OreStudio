...
namespace Ui {
class WorkspaceDetailDialog;
}

namespace ores::qt {

/**
 * @brief Detail dialog for viewing and editing workspace records.
 *
 * This dialog allows viewing, creating, and editing workspaces.
 * It supports both create mode (for new records) and edit mode (for
 * existing records).
 */
class WorkspaceDetailDialog final : public DetailDialogBase {
    Q_OBJECT
...
public:
    explicit WorkspaceDetailDialog(QWidget* parent = nullptr);
...
signals:
    void workspaceSaved(const QString& code);
