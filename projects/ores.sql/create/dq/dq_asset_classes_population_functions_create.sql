 ...
 *   SELECT * FROM metadata.dq_asset_class_preview_fn(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM metadata.dq_asset_classes_publish_fn(dataset_id, 'upsert');
 */
 ...
 */
create or replace function metadata.dq_asset_class_preview_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_asset_classes_artefact_tbl dq
    left join production.refdata_asset_classes_tbl existing
 ...
 */
create or replace function metadata.dq_asset_classes_publish_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_asset_classes_tbl existing
        ...
        insert into production.refdata_asset_classes_tbl (
