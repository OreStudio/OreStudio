...
begin
    ...
    changed_key := jsonb_build_object('username', changed_username);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.iam.account',
    ...
    perform pg_notify('ores_iam_accounts', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_iam_accounts_notify_trg
after insert or update or delete on ores_iam_accounts_tbl
for each row execute function ores_iam_accounts_notify_fn();
