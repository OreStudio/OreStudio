...
// notification" pattern end to end for calendar_event -- the
...
using ores::refdata::repository::calendar_event_repository;
using ores::refdata::repository::country_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_calendar_event_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::calendar_event_changed_event>(
        [&nats](const ores::refdata::eventing::calendar_event_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::calendar_event_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.calendar_event",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.calendar_event_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::calendar_event_changed_event>(
        "ores.refdata.calendar_event", "ores_refdata_calendar_events");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::calendar_event_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_calendar_event(ctx);
    ...
    // Seed the active diary_entry_type row ores_refdata_diary_entry_types_tbl references:
    // the insert trigger's existence check rejects a synthetic key that
    // matches no active row, so the parent must be written first.
    auto diary_entry_type_parent =
        ores::refdata::generators::generate_synthetic_diary_entry_type(ctx);
    diary_entry_type_parent.change_reason_code = "system.test";
    ores::refdata::repository::diary_entry_type_repository diary_entry_type_repo;
    diary_entry_type_repo.write(party_ctx, diary_entry_type_parent);
    v.diary_entry_type = diary_entry_type_parent.code;
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Calendar Event: " << v;

    calendar_event_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.calendar_event") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for calendar_event " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for calendar_event " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::calendar_event_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_calendar_event_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_calendar_event(id_str);
        ...
        REQUIRE_FALSE(svc.get_calendar_event(id_str).has_value());
        REQUIRE(svc.get_calendar_event_history(id_str).size() == versions.size());
    }
}
