...
create table if not exists "ores_trading_instrument_strikes_tbl" (
    ...
    "yield_compounding" text null,
    "bare_value" numeric(28, 10) null,
    "bare_currency" text null,
...
on "ores_trading_instrument_strikes_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_trading_instrument_strikes_insert_fn()
returns trigger as $$
declare
...
create or replace trigger ores_trading_instrument_strikes_insert_trg
before insert on "ores_trading_instrument_strikes_tbl"
for each row execute function ores_trading_instrument_strikes_insert_fn();

create or replace rule ores_trading_instrument_strikes_delete_rule as
on delete to "ores_trading_instrument_strikes_tbl" do instead (
    update "ores_trading_instrument_strikes_tbl"
    set valid_to = clock_timestamp()
