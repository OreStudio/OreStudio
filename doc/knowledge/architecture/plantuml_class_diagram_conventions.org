...
diagram should look/. The [[id:D6D86317-4961-6754-A6DB-9C12B5FAC997][compass-doc-add-class-diagram]] skill drives the
...
| System (top)     | [[proj:projects/modeling/ores.puml][=projects/modeling/ores.puml=]]                     |
...
** Note targets and fully-qualified names

A =note ... of= line takes a /short/ name or a /double-quoted/ fully-qualified
one. An unquoted fully-qualified name is a trap: PlantUML reads the first
segment as the target and draws an empty extra class named after it. The
diagram still renders and the note still attaches, so the defect is invisible
unless someone reads the picture.

#+begin_src plantuml
note right of "ores::ore::xml::roundtrip_summary"  ' correct
note right of roundtrip_summary                    ' correct, when unambiguous
note right of ores::ore::xml::roundtrip_summary    ' adds a phantom class "ores"
#+end_src

Arrow lines are the opposite case: those take the unquoted fully-qualified
name.
...
The system-level diagram ([[proj:projects/modeling/ores.puml][=projects/modeling/ores.puml=]]) uses
...
| No phantom note target       | Search =note .* of= for an unquoted =::= name.           |
| Syntax valid                 | =cmake --build --target generate_<component>_diagram=. |

* See also

- [[id:D6D86317-4961-6754-A6DB-9C12B5FAC997][compass-doc-add-class-diagram skill]] — drives the authoring.
