...
TEST_CASE("write_party_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::party_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_parties");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for party " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_party(id_str).has_value());
        REQUIRE(svc.get_party_history(id_str).size() == versions.size());
    }
}
