...
void equity_forward_instrument_repository::write(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Writing equity forward instrument. "
                               << "instrument_id: " << v.identity.instrument_id;
    execute_write_query(ctx,
                        equity_forward_instrument_mapper::map(v),
                        lg(),
                        "Writing equity forward instrument to database.");
}
...
void equity_forward_instrument_repository::write(
    ...
    execute_write_query(ctx,
                        equity_forward_instrument_mapper::map(v),
                        lg(),
                        "Writing equity forward instruments to database.");
}
...
equity_forward_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<equity_forward_instrument_entity,
                                  domain::equity_forward_instrument>(
            ctx,
            query,
            [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
            lg(),
            "Reading latest equity forward instruments (workspace resolution chain).");
    }
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::read<std::vector<equity_forward_instrument_entity>> |
    ...
    return execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
        ...
        [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity forward instruments");
}

std::vector<domain::equity_forward_instrument>
equity_forward_instrument_repository::read_latest(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest equity forward instrument. "
    ...
    return execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
        ...
        [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity forward instrument by instrument_id.");
}


std::vector<domain::equity_forward_instrument>
equity_forward_instrument_repository::read_all(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all equity forward instrument versions. "
                               << "instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<equity_forward_instrument_entity>> |
    ...
    return execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
        ...
        [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
        lg(),
        "Reading all equity forward instrument versions by instrument_id.");
}

std::optional<domain::equity_forward_instrument>
equity_forward_instrument_repository::read_at_version(context ctx,
                                                      const std::string& instrument_id,
                                                      std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading equity forward instrument at version. "
    ...
    const auto entities =
        execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
            ctx,
            query,
            [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
            lg(),
            "Reading equity forward instrument at version.");
...
std::vector<domain::equity_forward_instrument> equity_forward_instrument_repository::read_latest(
    context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest equity forward instruments with offset: "
    ...
    return execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
        ...
        [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity forward instruments with pagination.");
}

std::uint32_t
equity_forward_instrument_repository::get_total_equity_forward_instrument_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active equity forward instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<equity_forward_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active equity forward instrument count: " << count;
    return count;
}
...
equity_forward_instrument_repository::read_latest(context ctx,
    ...
    auto result =
        execute_read_query<equity_forward_instrument_entity, domain::equity_forward_instrument>(
            ...
            [](const auto& entities) { return equity_forward_instrument_mapper::map(entities); },
            lg(),
            "Reading latest equity forward instruments by ids.");
    return result;
}

void equity_forward_instrument_repository::remove(context ctx,
                                                  const std::vector<std::string>& instrument_ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<equity_forward_instrument_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid &&
                             "instrument_id"_c.in(instrument_ids) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing equity forward instruments.");
}
