 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_YIELD_CURVE_PROCESS_PARAMETER_DEFINITION_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_YIELD_CURVE_PROCESS_PARAMETER_DEFINITION_EVENT_HPP
...
/**
 * @brief Event traits for yield curve process parameter definition's changes.
 *
...
struct entity_event_traits<
    ores::synthetic::messaging::yield_curve_process_parameter_definition_event> {
    static constexpr std::string_view subject_prefix =
        "synthetic.v1.yield_curve_process_parameter_definitions_events";

    static ores::synthetic::messaging::yield_curve_process_parameter_definition_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::yield_curve_process_parameter_definition_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<
                ores::synthetic::messaging::yield_curve_process_parameter_definition_key>(
                notification.key))
            event.key = *key;
