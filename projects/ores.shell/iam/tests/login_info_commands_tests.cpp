...
using ores::shell::app::commands::login_info_commands;
...
TEST_CASE("login_info_commands_registers_every_derived_verb", tags) {
    ...
    const auto completions = root_menu.GetCompletions("login_info ");
    for (const auto& verb : {
             std::string{"login_info list"},
             std::string{"login_info get"},
             std::string{"login_info get-many"},
         })
...
TEST_CASE("login_info_commands_process_list_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    login_info_commands::process_list(out, session, tokens(0));
...
TEST_CASE("login_info_commands_process_get_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    login_info_commands::process_get(out, session, tokens(1));
...
TEST_CASE("login_info_commands_process_get_reports_the_expected_count", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    login_info_commands::process_get(out, session, {});
...
TEST_CASE("login_info_commands_process_get_many_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    login_info_commands::process_get_many(out, session, tokens(1));
