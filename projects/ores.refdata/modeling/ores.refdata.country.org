...
#+description: Codegen entity model for the refdata =country= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
unfiltered =read_all=. Pagination, the active-count aggregate and batch
delete come from the standard template.
