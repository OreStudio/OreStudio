...
const std::string_view test_suite("ores.scheduler.service.tests");
const std::string tags("[config]");

}

using namespace ores::scheduler::service::config;
...
TEST_CASE("parse_defaults_returns_expected_values", tags) {
    ...
    CHECK(result->logging->filename == "ores.scheduler.service.log");
}
