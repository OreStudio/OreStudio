...
// notification" pattern end to end for currency_pair_classification -- the
...
using ores::refdata::domain::currency_pair_classification;
using ores::refdata::repository::currency_pair_classification_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_currency_pair_classification_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::currency_pair_classification_changed_event>(
        [&nats](const ores::refdata::eventing::currency_pair_classification_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::refdata::eventing::currency_pair_classification_changed_event>::name),
    ...
    event_source
        .register_mapping<ores::refdata::eventing::currency_pair_classification_changed_event>(
            "ores.refdata.currency_pair_classification",
            "ores_refdata_currency_pair_classifications");
    ...
    auto v = generate_synthetic_currency_pair_classification(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Currency Pair Classification: " << v;

    currency_pair_classification_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.currency_pair_classification") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for currency_pair_classification " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for currency_pair_classification "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::currency_pair_classification_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_classification_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_classification(id_str);
        ...
        REQUIRE_FALSE(svc.get_classification(id_str).has_value());
        REQUIRE(svc.get_classification_history(id_str).size() == versions.size());
    }
}
