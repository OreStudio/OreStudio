 ...
 *   SELECT * FROM ores_dq_preview_party_role_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_party_roles(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_party_role_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_party_roles_tbl existing
...
/**
 * Populate ores_refdata_party_roles_tbl from a DQ party_roles dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_party_roles(
    p_dataset_id uuid,
