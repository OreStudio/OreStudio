...
Confirmation already carries lifecycle weight even so. [[id:B8D4F2A1-9E37-4C6B-A8D5-2F1E0C9B7A3D][Trade lifecycle]]
makes Confirm the only transition from =new= to =live=, so a trade
that is never confirmed never becomes active. That binds the
confirmation model to the state machine more tightly than the missing
pieces below suggest, and it is why a trade that does not confirm —
an internal transfer, a test booking — needs either its own machine or
another way into =live=. [[id:03C12558-69AC-4925-AAE1-85FFF75B10BA][Counterparty scope and booking nature]] covers
...
- Trading does not adopt that design. Each entity versions on its own
  spine and the composite read is a temporal join over keys.
