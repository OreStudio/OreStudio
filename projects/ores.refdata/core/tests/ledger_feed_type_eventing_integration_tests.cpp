...
#include "ores.refdata.api/generators/ledger_feed_type_generator.hpp"
...
// notification" pattern end to end for ledger_feed_type -- the
...
using ores::refdata::domain::ledger_feed_type;
using ores::refdata::repository::ledger_feed_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_ledger_feed_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::ledger_feed_type_changed_event>(
        [&nats](const ores::refdata::eventing::ledger_feed_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::ledger_feed_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.ledger_feed_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::ledger_feed_type_changed_event>(
        "ores.refdata.ledger_feed_type", "ores_refdata_ledger_feed_types");
    ...
    auto v = generate_synthetic_ledger_feed_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Ledger Feed Type: " << v;

    ledger_feed_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.ledger_feed_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for ledger_feed_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for ledger_feed_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::ledger_feed_type_service svc(crud_ctx);
