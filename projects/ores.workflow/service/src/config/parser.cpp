...
void print_help(const options_description& od, std::ostream& info) {
    info << "Workflow orchestration microservice" << std::endl
         << std::endl
         << "Usage: ores.workflow.service [options]" << std::endl
         << std::endl
         << od << std::endl;
}
...
std::optional<options> parse_arguments(const std::vector<std::string>& arguments,
    ...
    const auto od(standard_service_options::make_options_description("ores.workflow.service.log"));
    const auto vm(standard_service_options::parse(od, arguments, "WORKFLOW_SERVICE"));
