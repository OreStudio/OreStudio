...
using ores::shell::app::commands::bond_instrument_commands;
...
TEST_CASE("bond_instrument_commands_register_commands_registers_the_pagination_callback", tags) {
    ...
    BOOST_LOG_SEV(lg, debug) << "Looking up the pagination callback for bond_instruments.";
    CHECK(pagination.get_list_callback("bond_instruments") != nullptr);
}

TEST_CASE("bond_instrument_commands_process_add_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    bond_instrument_commands::process_add_bond_instrument(out, session, add_tokens());
...
TEST_CASE("bond_instrument_commands_process_add_reports_the_expected_argument_count", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    bond_instrument_commands::process_add_bond_instrument(out, session, {"sample"});

    BOOST_LOG_SEV(lg, debug) << "Output for a short argument list: " << out.str();
    CHECK(out.str().find("Expected 6 arguments, got 1.") != std::string::npos);
    CHECK(command_feedback::failed());
}

TEST_CASE("bond_instrument_commands_process_add_names_the_field_of_a_malformed_token", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    bond_instrument_commands::process_add_bond_instrument(out, session, tokens);
...
TEST_CASE("bond_instrument_commands_process_add_sends_a_valid_token_vector_to_the_server", tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("bond_instrument_commands_process_delete_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    bond_instrument_commands::process_delete_bond_instrument(out, session, "missing-key");
...
TEST_CASE("bond_instrument_commands_process_history_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    bond_instrument_commands::process_get_bond_instrument_history(out, session, "missing-key");
