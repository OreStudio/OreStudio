 ...
 */
#ifndef ORES_QT_PURPOSE_TYPE_CONTROLLER_HPP
#define ORES_QT_PURPOSE_TYPE_CONTROLLER_HPP
...
/**
 * @brief Controller for managing purpose type windows and operations.
 *
 * Manages the lifecycle of purpose type list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class PurposeTypeController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.purpose_type_controller";
...
public:
    PurposeTypeController(QMainWindow* mainWindow,
                          QMdiArea* mdiArea,
...
private:
     ...
     * PurposeTypeDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(PurposeTypeDetailDialog* detailDialog);
    void showHistoryWindow(const QString& code);

    /**
     * @brief Fetches the full typed purpose type history (the
     * existing per-entity refdata::messaging::get_purpose_type_history_request/
     * refdata::messaging::get_purpose_type_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed purpose type, since the
     * generic dialog holds no typed domain data.
     */
    void fetchPurposeTypeHistory(
        const QString& entityId,
        std::function<void(std::expected<std::vector<refdata::domain::purpose_type>, QString>)>
            callback);
