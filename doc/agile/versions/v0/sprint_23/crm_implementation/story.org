...
| [[id:72FEB0B2-7E8B-4FD9-B9A4-3A27D4416999][Wire the CRM into ores.marketdata's ingest path]] | STARTED | 2026-07-12 | | Add persisted CRM topology config (codegened entities), wire ores.marketdata.service to run one rate_engine per tenant/party fed by driver ticks, and expose derived rates via a request/reply NATS endpoint -- drivers stay the only high-frequency published ticks, avoiding an O(n^2) tick fan-out. |
