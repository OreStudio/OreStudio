...
// notification" pattern end to end for pricing_model_config -- the
...
TEST_CASE("write_pricing_model_config_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::analytics::eventing::pricing_model_config_changed_event>(
        [&nats](const ores::analytics::eventing::pricing_model_config_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::analytics::eventing::pricing_model_config_changed_event>::name),
    ...
    event_source.register_mapping<ores::analytics::eventing::pricing_model_config_changed_event>(
        "ores.analytics.pricing_model_config", "ores_analytics_pricing_model_configs");
    ...
    auto v = generate_synthetic_pricing_model_config(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Pricing Model Configuration: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.analytics.pricing_model_config") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for pricing_model_config " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for pricing_model_config " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_config(id_str).has_value());
        REQUIRE(svc.get_config_history(id_str).size() == versions.size());
    }
}
