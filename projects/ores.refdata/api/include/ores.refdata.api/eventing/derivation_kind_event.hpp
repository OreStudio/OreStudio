 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_DERIVATION_KIND_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_DERIVATION_KIND_EVENT_HPP
...
/**
 * @brief Event traits for derivation kind's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::derivation_kind_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.derivation_kinds_events";

    static ores::refdata::messaging::derivation_kind_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::derivation_kind_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::derivation_kind_key>(notification.key))
            event.key = *key;
