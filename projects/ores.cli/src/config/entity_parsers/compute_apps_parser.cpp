...
options_description make_add_compute_app_options_description() {
    ...
    r.add_options()
        ("name",
            value<std::string>(),
            "Application name, e.g. 'ORE_STUDIO' (required)")
        ("description",
...
ores::cli::config::add_compute_app_options
read_add_compute_app_options(const variables_map& vm) {
    ...
    if (vm.count("name") == 0) {
        BOOST_THROW_EXCEPTION(
            parser_exception("Must supply --name for add app command."));
    }
...
handle_compute_apps_command(bool has_help,
    ...
    if (has_help && o.empty()) {
        const std::vector<std::pair<std::string, std::string>> operations = {
            {"list", "List compute apps as JSON or table"},
            {"add", "Add a new compute app"}
        };
    ...
    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception(
            "apps command requires an operation (list, add)"));
    }
