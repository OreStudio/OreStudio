...
- =application.cpp=/=host.cpp= unification scoped to the one clear
  ...
  there instead of a second implementation. =ores.cli=/=ores.shell=/
  =ores.compute/wrapper= kept hand-written since their differences are
  real behavioural divergence, not boilerplate drift.
