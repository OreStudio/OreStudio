...
// The index-name key space splits on '-' (e.g. "USD-LIBOR-3M"); exactly two or three
...
oresmd_projections::from_index_name(const std::string& index_name) {
    ...
    // ccy upper, index/family and tenor lower.
    ...
    if (parts->size() == 3) {
        ...
        // A term family without a tenor could not have come from a forward
        // projection (index_name_ir() only emits the two-segment form for
        // overnight families) and the parser rejects it; mirror both.
        return std::nullopt;
    }
    return id;
}
