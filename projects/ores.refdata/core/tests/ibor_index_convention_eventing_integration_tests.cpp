...
// notification" pattern end to end for ibor_index_convention -- the
...
using ores::refdata::domain::ibor_index_convention;
using ores::refdata::repository::ibor_index_convention_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_ibor_index_convention_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::ibor_index_convention_changed_event>(
        [&nats](const ores::refdata::eventing::ibor_index_convention_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::ibor_index_convention_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.ibor_index_convention",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::ibor_index_convention_changed_event>(
        "ores.refdata.ibor_index_convention", "ores_refdata_ibor_index_conventions");
    ...
    auto v = generate_synthetic_ibor_index_convention(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.id;
    BOOST_LOG_SEV(lg, debug) << "IBOR Index Convention: " << v;

    ibor_index_convention_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.ibor_index_convention") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for ibor_index_convention " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for ibor_index_convention "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::ibor_index_convention_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_ibor_index_convention_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_ibor_index_convention(id_str);
        ...
        REQUIRE_FALSE(svc.get_ibor_index_convention(id_str).has_value());
        REQUIRE(svc.get_ibor_index_convention_history(id_str).size() == versions.size());
    }
}
