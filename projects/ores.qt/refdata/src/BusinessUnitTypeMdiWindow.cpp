...
BusinessUnitTypeMdiWindow::BusinessUnitTypeMdiWindow(ClientManager* clientManager,
...
void BusinessUnitTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &BusinessUnitTypeMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &BusinessUnitTypeMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &BusinessUnitTypeMdiWindow::deleteSelected);
    ...
    connect(
        historyAction_, &QAction::triggered, this, &BusinessUnitTypeMdiWindow::viewHistorySelected);
}
...
void BusinessUnitTypeMdiWindow::setupTable() {
    ...
    delegate->set_badge_color_resolver(
        0, [cache = badgeCache_](const QString& value) -> badge_color_pair {
            ...
            auto* def = cache->resolve("business_unit_type", value.toStdString());
            if (!def) {
    ...
    initializeTableSettings(tableView_,
                            ...
                            {
                                ClientBusinessUnitTypeModel::Description,
                            },
                            {900, 400},
                            1);
}
...
void BusinessUnitTypeMdiWindow::setupConnections() {
    connect(model_,
            &ClientBusinessUnitTypeModel::dataLoaded,
            this,
            &BusinessUnitTypeMdiWindow::onDataLoaded);
    connect(model_,
            &ClientBusinessUnitTypeModel::loadError,
            this,
            &BusinessUnitTypeMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &BusinessUnitTypeMdiWindow::onSelectionChanged);
    connect(
        tableView_, &QTableView::doubleClicked, this, &BusinessUnitTypeMdiWindow::onDoubleClicked);
...
void BusinessUnitTypeMdiWindow::doReload() {
...
void BusinessUnitTypeMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void BusinessUnitTypeMdiWindow::onDoubleClicked(const QModelIndex& index) {
    ...
    if (auto* business_unit_type = model_->getType(sourceIndex.row())) {
        emit showTypeDetails(*business_unit_type);
    }
}
...
void BusinessUnitTypeMdiWindow::viewHistorySelected() {
    ...
    if (auto* business_unit_type = model_->getType(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug)
            << "Emitting showTypeHistory for code: " << business_unit_type->code;
        emit showTypeHistory(*business_unit_type);
    }
}
...
void BusinessUnitTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete business unit type while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* business_unit_type = model_->getType(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(business_unit_type->id));
            codes.push_back(business_unit_type->code);
        }
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " business unit types";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 business unit types?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Business Unit Type", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " business unit types";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                ...
                success_count++;
                emit self->business_unit_typeDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error)
                    << "Business Unit Type deletion failed: " << code << " - " << message;
                failure_count++;
