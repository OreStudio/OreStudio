...
void PricingEngineTypeMdiWindow::setupToolbar() {
...
void PricingEngineTypeMdiWindow::setupTable() {
    model_ = new ClientPricingEngineTypeModel(clientManager_, this);
...
void PricingEngineTypeMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void PricingEngineTypeMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void PricingEngineTypeMdiWindow::updateActionStates() {
...
void PricingEngineTypeMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new pricing engine type requested";
    emit addNewRequested();
}

void PricingEngineTypeMdiWindow::editSelected() {
...
void PricingEngineTypeMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void PricingEngineTypeMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid pricing engine types to delete";
        return;
    }
    ...
    QPointer<PricingEngineTypeMdiWindow> self = this;
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished,
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                BOOST_LOG_SEV(lg(), debug) << "Pricing Engine Type deleted: " << code;
                success_count++;
