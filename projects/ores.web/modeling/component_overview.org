...
- The org entity models under =projects/ores.*/modeling/=, which =ores.codegen=
  reads at build time to generate the TypeScript UI metadata and protocol types.
...
- Generated TypeScript in =packages/web/src/generated/= and
  =packages/wire-protocol/src/generated/=.
...
- =projects/ores.web/packages/web/src/ui-contract.ts= — the hand-written shapes
  that the generated UI metadata conforms to.
- =projects/ores.web/package.json= — the workspace scripts: =build=, =test=,
  =verify:login=, and =verify:browser=.
...
- =ores.codegen= — build-time generation of the TypeScript UI metadata and
  protocol types.
- Node.js 22 or later.
...
- [[id:67E68899-3D29-4861-B4F8-B0C9536EC4C1][The entity specification]] — the
  entity archetypes, the list and detail screens, the audit rule, and the
  implementation steps.
- [[id:DD0C6CEB-A285-40F5-8A97-B39DA6953B20][The component specification]] —
  the component declaration, the registry, the routes, and the file layout.
- [[id:AE16BF31-1ECF-47C3-82C9-994B43BC0BD6][Codegen request: TypeScript UI
  metadata]] — the request for the =ores.ts.ui= facet: what to emit and why.
...
- [[id:1B37530D-0729-4DA2-B586-629C08F9370F][Gaps: country against the
  specification]] — the audit findings against the reference entity.
