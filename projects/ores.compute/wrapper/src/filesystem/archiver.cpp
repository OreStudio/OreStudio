...
void archiver::pack(const fs::path& source_dir, const fs::path& output_archive) {
    archive_write_ptr a(archive_write_new());
    archive_write_add_filter_gzip(a.get());
    archive_write_set_format_pax_restricted(a.get());

#ifdef _WIN32
    const int open_rc = archive_write_open_filename_w(a.get(), output_archive.c_str());
#else
    const int open_rc = archive_write_open_filename(a.get(), output_archive.c_str());
#endif
    ...
    for (const auto& e : fs::recursive_directory_iterator(source_dir)) {
        ...
        while (in.read(buf.data(), buf.size()) || in.gcount()) {
            if (archive_write_data(a.get(), buf.data(), static_cast<std::size_t>(in.gcount())) <
                ARCHIVE_OK)
                throw std::runtime_error(std::string("archive write data error: ") +
                                         archive_error_string(a.get()));
        }
    }
}

void archiver::extract(const fs::path& archive_path, const fs::path& dest_dir) {
...
#ifdef _WIN32
    const int open_rc = archive_read_open_filename_w(a.get(), archive_path.c_str(), 16384);
#else
    const int open_rc = archive_read_open_filename(a.get(), archive_path.c_str(), 16384);
#endif
    if (open_rc != ARCHIVE_OK)
        throw std::runtime_error(std::string("Failed to open archive: ") + archive_path.string() +
                                 " — " + archive_error_string(a.get()));
        ...
        if (archive_entry_size(entry) > 0) {
            ...
            while (true) {
                ...
                if (rd == ARCHIVE_EOF)
                    break;
                if (rd < ARCHIVE_OK)
                    throw std::runtime_error(std::string("archive data read error: ") +
                                             archive_error_string(a.get()));
                if (archive_write_data_block(out.get(), buff, size, offset) < ARCHIVE_OK)
                    throw std::runtime_error(std::string("archive data write error: ") +
                                             archive_error_string(out.get()));
            }
        }
