...
#+description: compass-pr-address-review and its runbook synced/rebased before checking whether a round existed, churning CI and force-pushing for nothing.
#+type: task
#+level: s1
...
Add a step-0 gate to the handle-PR-review-round runbook and the compass-pr-address-review skill: read the comments first; stop (without syncing) when no comments exist yet, the reviewer is still running, or every comment is already replied and resolved. Sync/rebase only when there is an actual round to handle.
...
of the compass-pr-address-review skill; skills rebuilt. Trigger: review
