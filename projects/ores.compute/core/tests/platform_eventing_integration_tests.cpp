...
#include "ores.compute.api/eventing/platform_event.hpp"
#include "ores.compute.api/generators/platform_generator.hpp"
...
// notification" pattern end to end for platform -- the
...
using ores::compute::domain::platform;
using ores::compute::repository::platform_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_platform_publishes_an_event", tags) {
    ...
    using event_type = ores::compute::messaging::platform_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_compute_platforms");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for platform " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for platform " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_platform(v.id).has_value());
        REQUIRE(svc.get_platform_history(v.code).size() == versions.size());
    }
}
