...
{{#has_as_of_lookup}}
    ...
    if (request.as_of) {
        response.{{entity_plural_short}} = repo_.read_at_timepoint(ctx_, *request.as_of);
        response.total = response.{{entity_plural_short}}.size();
        return response;
    }
{{/has_as_of_lookup}}
...
{{#leading_is_optional}}
    ...
    if (!request.{{leading}}) {
        ...
        response.result.message = "This read is scoped by {{leading}}, and the request states none.";
        return response;
    }
{{/leading_is_optional}}
{{#leading_is_uuid}}
    const auto relation = boost::uuids::to_string({{#leading_is_optional}}*{{/leading_is_optional}}request.{{leading}});
{{/leading_is_uuid}}
{{^leading_is_uuid}}
    const auto relation = {{#leading_is_optional}}*{{/leading_is_optional}}request.{{leading}};
{{/leading_is_uuid}}
