 ...
 *
 */
#include "ores.trading.api/generators/inflation_swap_instrument_generator.hpp"
...
generate_synthetic_inflation_swap_instruments(std::size_t n,
    utility::generation::generation_context& ctx) {
    std::vector<domain::inflation_swap_instrument> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_inflation_swap_instrument(ctx));
    return r;
}
