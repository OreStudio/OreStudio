...
create table if not exists "production"."refdata_countries_tbl" (
...
on "production"."refdata_countries_tbl" (alpha2_code, version)
where valid_to = public.utility_infinity_timestamp_fn();

create index if not exists refdata_countries_alpha3_idx
on "production"."refdata_countries_tbl" (alpha3_code)
where valid_to = public.utility_infinity_timestamp_fn();

create index if not exists refdata_countries_numeric_idx
on "production"."refdata_countries_tbl" (numeric_code)
where valid_to = public.utility_infinity_timestamp_fn();

create or replace function production.refdata_countries_insert_fn()
returns trigger as $$
declare
...
before insert on "production"."refdata_countries_tbl"
for each row
execute function production.refdata_countries_insert_fn();

create or replace rule refdata_countries_delete_rule as
on delete to "production"."refdata_countries_tbl"
do instead
  update "production"."refdata_countries_tbl"
  set valid_to = current_timestamp
