...
render_inflation_swap_instrument_fields(const domain::inflation_swap_instrument& v) {
    ...
    fields.push_back({.name = "Inflation Index Code", .value = v.inflation_index_code});
    fields.push_back(
        {.name = "Base Cpi", .value = v.base_cpi ? std::to_string(*v.base_cpi) : std::string{}});
    fields.push_back({.name = "Lag Convention", .value = v.lag_convention});
