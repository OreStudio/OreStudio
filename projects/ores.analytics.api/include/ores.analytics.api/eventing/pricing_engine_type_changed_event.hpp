...
/**
 * @brief Domain event indicating that pricing engine type data has changed.
 *
 * Published when any pricing engine type entity is created, updated, or
 * deleted. Subscribers use the timestamp to query for changes since that point.
 */
struct pricing_engine_type_changed_event final {
    /**
