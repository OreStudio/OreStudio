...
export interface EntityRouteDescriptor {
  ...
  readonly subjects: {
    ...
    readonly get?: string;
    /**
     * Present only when the service can answer for one write.
     *
     * An entity whose writes are workflows derives no put request, so the
     * route serves the collection and no save. The screen then offers no
     * create and no edit, which is what the declaration states.
     */
