...
#include "ui_CurrencyAssetClassHistoryDialog.h"
...
CurrencyAssetClassHistoryDialog::CurrencyAssetClassHistoryDialog(
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::CurrencyAssetClassHistoryDialog),
      code_(code),
...
void CurrencyAssetClassHistoryDialog::setupUi() {
...
void CurrencyAssetClassHistoryDialog::setupToolbar() {
...
void CurrencyAssetClassHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &CurrencyAssetClassHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &CurrencyAssetClassHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &CurrencyAssetClassHistoryDialog::onRevertClicked);
}

void CurrencyAssetClassHistoryDialog::loadHistory() {
    ...
    QPointer<CurrencyAssetClassHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<refdata::domain::currency_asset_class> versions;
    };
    ...
    auto task = [self, code = code_.toStdString()]() -> HistoryResult {
        ...
        refdata::messaging::get_currency_asset_class_history_request request;
        request.code = code;
        auto payload = request.serialize();

        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_currency_asset_class_history_request,
            0, std::move(payload)
        );
        ...
        auto response = refdata::messaging::get_currency_asset_class_history_response::
...
void CurrencyAssetClassHistoryDialog::updateVersionList() {
...
void CurrencyAssetClassHistoryDialog::onVersionSelected() {
...
void CurrencyAssetClassHistoryDialog::updateChangesTable(int currentVersionIndex) {
...
void CurrencyAssetClassHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
...
void CurrencyAssetClassHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CurrencyAssetClassHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void CurrencyAssetClassHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
