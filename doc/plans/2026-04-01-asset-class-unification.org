...
- Define =asset_class= and =product_type= as two distinct, non-overlapping
...
- Rename =instrument_family= → =product_type= throughout the system to
...
- Changing how instrument extension tables are routed — =product_type=
...
- Rename PG enum type: =instrument_family_t= → =product_type_t=
  (DDL: =ALTER TYPE instrument_family_t RENAME TO product_type_t=)
...
- Phase 4's =asset_class= column is nullable, so existing trade rows are
  unaffected. A follow-up migration can backfill based on =product_type=
  heuristics for simple cases (e.g., =product_type = 'fx'= → =asset_class = 'fx'=).
