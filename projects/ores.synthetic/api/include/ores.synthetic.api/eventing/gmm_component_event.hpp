 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_GMM_COMPONENT_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_GMM_COMPONENT_EVENT_HPP
...
/**
 * @brief Event traits for gmm component's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::synthetic::messaging::gmm_component_event> {
    static constexpr std::string_view subject_prefix = "synthetic.v1.gmm_components_events";

    static ores::synthetic::messaging::gmm_component_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::gmm_component_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::synthetic::messaging::gmm_component_key>(notification.key))
            event.key = *key;
