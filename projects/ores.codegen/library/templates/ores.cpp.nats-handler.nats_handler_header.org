...
- =read_for_cache= (bool, Flags) — emit a =read_for_cache= handler method
  ...
  *Known gap, inherited from party's hand-written precedent*: unlike
  every other handler method, =read_for_cache= does not call
  =make_request_context= / =has_permission= — it trusts the
  caller-supplied =tenant_id= outright. This mirrors party's existing
  =read_parties_for_cache=, which is meant for trusted internal
  service-to-service NATS calls (e.g. IAM's party-cache warm-up), not
  end-user-facing traffic. If a future consumer exposes this subject to
  less-trusted callers, add the same auth/permission check the other
  methods use before relying on it. See the "Generic entity-mirror
  cache primitive + codegen facet" story for the follow-up task.
