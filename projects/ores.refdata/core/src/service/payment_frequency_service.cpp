 ...
 */
std::vector<domain::payment_frequency> read_one(repository::payment_frequency_repository& repo,
                                                const ores::database::context& ctx,
                                                const messaging::payment_frequency_key& key) {
 ...
 */
messaging::payment_frequency_key key_from(const domain::payment_frequency& v) {
 ...
 */
domain::payment_frequency to_domain(const messaging::payment_frequency_write& write) {
    domain::payment_frequency v;
    v.code = write.code;
    v.name = write.name;
    v.description = write.description;
    v.period_unit = write.period_unit;
    v.period_multiplier = write.period_multiplier;
...
messaging::get_payment_frequency_response payment_frequency_service::get_payment_frequency(
    const messaging::get_payment_frequency_request& request) {
    messaging::get_payment_frequency_response response;
    ...
    response.payment_frequency = std::move(found.front());
    return response;
}

messaging::get_many_payment_frequencies_response
payment_frequency_service::get_many_payment_frequencies(
    const messaging::get_many_payment_frequencies_request& request) {
    messaging::get_many_payment_frequencies_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::payment_frequency_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.payment_frequency = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_payment_frequency_response payment_frequency_service::put_payment_frequency(
    const messaging::put_payment_frequency_request& request) {
    messaging::put_payment_frequency_response response;
    domain::payment_frequency value;
    ...
    if (!written.empty())
        response.payment_frequency = std::move(written.front());
    return response;
}

messaging::put_many_payment_frequencies_response
payment_frequency_service::put_many_payment_frequencies(
    const messaging::put_many_payment_frequencies_request& request) {
    messaging::put_many_payment_frequencies_response response;
    ...
    response.payment_frequencies.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.payment_frequencies.push_back(written.empty() ? value :
                                                                 std::move(written.front()));
    }
    return response;
}

messaging::delete_payment_frequency_response payment_frequency_service::delete_payment_frequency(
    const messaging::delete_payment_frequency_request& request) {
    messaging::delete_payment_frequency_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::payment_frequency_repository::remove_status::removed:
            break;
        case repository::payment_frequency_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::payment_frequency_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::payment_frequency_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_payment_frequencies_response
payment_frequency_service::delete_many_payment_frequencies(
    const messaging::delete_many_payment_frequencies_request& request) {
    messaging::delete_many_payment_frequencies_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_payment_frequency_versions_response
payment_frequency_service::list_payment_frequency_versions(
    const messaging::list_payment_frequency_versions_request& request) {
    messaging::list_payment_frequency_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
payment_frequency_service::get_payment_frequency_version(
    const messaging::get_payment_frequency_version_request& request) {
    messaging::get_payment_frequency_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.payment_frequency.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
payment_frequency_service::prepare_change(const messaging::payment_frequency_change& change,
...
std::vector<domain::payment_frequency>
payment_frequency_service::get_payment_frequencies(const std::vector<std::string>& codes) {
