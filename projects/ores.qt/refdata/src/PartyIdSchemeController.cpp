...
#include "ores.refdata.api/eventing/party_id_scheme_changed_event.hpp"
...
namespace {
constexpr std::string_view scheme_event_name =
    eventing::domain::event_traits<refdata::eventing::party_id_scheme_changed_event>::name;
}

PartyIdSchemeController::PartyIdSchemeController(QMainWindow* mainWindow,
                                                 QMdiArea* mdiArea,
                                                 ClientManager* clientManager,
                                                 ChangeReasonCache* changeReasonCache,
                                                 const QString& username,
                                                 QObject* parent)
    : EntityController(mainWindow, mdiArea, clientManager, username, scheme_event_name, parent)
...
void PartyIdSchemeController::showListWindow() {
    ...
    // Connect signals
    connect(listWindow_,
            &PartyIdSchemeMdiWindow::statusChanged,
            this,
            &PartyIdSchemeController::statusMessage);
    connect(listWindow_,
            &PartyIdSchemeMdiWindow::errorOccurred,
            this,
            &PartyIdSchemeController::errorMessage);
    connect(listWindow_,
            &PartyIdSchemeMdiWindow::showSchemeDetails,
            this,
            &PartyIdSchemeController::onShowDetails);
    connect(listWindow_,
            &PartyIdSchemeMdiWindow::addNewRequested,
            this,
            &PartyIdSchemeController::onAddNewRequested);
    connect(listWindow_,
            &PartyIdSchemeMdiWindow::showSchemeHistory,
            this,
            &PartyIdSchemeController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<PartyIdSchemeController>(this), key]() {
                if (!self)
                    return;
...
void PartyIdSchemeController::onShowDetails(const refdata::domain::party_id_scheme& scheme) {
...
void PartyIdSchemeController::onShowHistory(const refdata::domain::party_id_scheme& scheme) {
...
void PartyIdSchemeController::showAddWindow() {
    ...
    connect(detailDialog,
            ...
            this,
            [self = QPointer<PartyIdSchemeController>(this)](const QString& code) {
                if (!self)
                    return;
...
void PartyIdSchemeController::showDetailWindow(const refdata::domain::party_id_scheme& scheme) {
    ...
    connect(detailDialog,
            &PartyIdSchemeDetailDialog::schemeDeleted,
            this,
            [self = QPointer<PartyIdSchemeController>(this), key](const QString& code) {
                if (!self)
                    return;
...
void PartyIdSchemeController::showHistoryWindow(const QString& code) {
    ...
    auto* historyDialog =
        new HistoryDialog(std::string(entity_type_of(refdata::domain::party_id_scheme{})),
                          code.toStdString(),
                          clientManager_,
                          mainWindow_);
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<PartyIdSchemeController>(this)](
...
void PartyIdSchemeController::onOpenVersion(const refdata::domain::party_id_scheme& scheme,
                                            int versionNumber) {
    ...
    detailWindow->setWindowTitle(
        QString("Party ID Scheme: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void PartyIdSchemeController::fetchPartyIdSchemeHistory(
    const QString& entityId,
    std::function<void(std::expected<std::vector<refdata::domain::party_id_scheme>, QString>)>
        callback) {
    refdata::messaging::get_party_id_scheme_history_request request;
    request.code = entityId.toStdString();

    using FetchResult = std::expected<std::vector<refdata::domain::party_id_scheme>, QString>;
...
void PartyIdSchemeController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<PartyIdSchemeController> self = this;
    fetchPartyIdSchemeHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<refdata::domain::party_id_scheme>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void PartyIdSchemeController::onRevertHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<PartyIdSchemeController> self = this;
    fetchPartyIdSchemeHistory(
        entityId,
        [self, entityId, versionNumber](
...
void PartyIdSchemeController::onRevertVersion(const refdata::domain::party_id_scheme& scheme) {
    BOOST_LOG_SEV(lg(), info) << "Reverting party ID scheme to version: " << scheme.version;
    ...
    connect(detailDialog,
            ...
            [self = QPointer<PartyIdSchemeController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("Party ID Scheme '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert Party ID Scheme: %1").arg(QString::fromStdString(scheme.code)));
...
void PartyIdSchemeController::notifyOpenDialogs(const QStringList& entityIds) {
