...
|---+-----------------+------+----------+-------|
| 1 | --limit error message says positive but 0 is valid (means all) | projects/ores.compass/src/compass.py | Accepted | Fixed in 8af71623b: "non-negative integer (0 = all)". |

* Result

Delivered in PR [[https://github.com/OreStudio/OreStudio/pull/1140][#1140]] (merged). =compass journal log= now prints
newest-first by default (git log semantics) with the order stated in
the header; =-n/--limit N= clips to the N most recent entries with a
shown-of-total header so clipping is never silent; =--chronological=
restores oldest-first and composes with =-n=. Negative limits are
rejected ("non-negative integer (0 = all)" after review round 1);
=journal where= is unchanged. Session journal recipe updated.
