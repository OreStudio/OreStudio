...
{{#primary_key.is_compound}}
                    const bool is_this_row = std::all_of(
                        key_parts.begin(), key_parts.end(), [&](const auto& part) {
...
{{/amend_activity}}
        ...
        auto versions = svc.get_{{entity_singular_short}}_history(id_str{{#primary_key.extra_columns}}, v.{{{scaffold_prefix}}}{{column}}{{/primary_key.extra_columns}});
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().{{{audit_prefix}}}change_commentary == "updated-by-crud-round-trip");

        svc.delete_{{entity_singular_short}}(id_str{{#primary_key.extra_columns}}, v.{{{scaffold_prefix}}}{{column}}{{/primary_key.extra_columns}});
        ...
        REQUIRE_FALSE(svc.{{#service_find_prefix}}find_{{/service_find_prefix}}{{^service_find_prefix}}get_{{/service_find_prefix}}{{entity_singular_short}}(id_str{{#primary_key.extra_columns}}, v.{{{scaffold_prefix}}}{{column}}{{/primary_key.extra_columns}}).has_value());
        REQUIRE(svc.get_{{entity_singular_short}}_history(id_str{{#primary_key.extra_columns}}, v.{{{scaffold_prefix}}}{{column}}{{/primary_key.extra_columns}}).size() == versions.size());
    }
}
{{/domain_entity}}
#+end_src
