...
// notification" pattern end to end for business_centre -- the
...
using ores::refdata::domain::business_centre;
using ores::refdata::repository::business_centre_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_business_centre_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::business_centre_changed_event>(
        [&nats](const ores::refdata::eventing::business_centre_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::business_centre_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::business_centre_changed_event>(
        "ores.refdata.business_centre", "ores_refdata_business_centres");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::business_centre_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_business_centre(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Business Centre: " << v;

    business_centre_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.business_centre") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for business_centre " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for business_centre " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_centre(id_str).has_value());
        REQUIRE(svc.get_centre_history(id_str).size() == versions.size());
    }
}
