...
/**
 * @brief Reads the add configuration from the variables map for day count fraction types.
 */
ores::cli::config::add_day_count_fraction_type_options
read_add_day_count_fraction_type_options(const variables_map& vm) {
...
handle_day_count_fraction_types_command(bool has_help,
    ...
    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception(
            "day_count_fraction_types command requires an operation (export, list, delete, add)"));
    }

    const auto operation = o.front();
    o.erase(o.begin()); // Remove operation from args

    validate_operation("day_count_fraction_types", operation, allowed_operations);
    ...
    if (operation == export_command_name) {
        ...
        if (has_help) {
            print_help_command("day_count_fraction_types export", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::day_count_fraction_types);
    } else if (operation == list_command_name) {
        auto d = add_common_options(make_export_options_description());
        if (has_help) {
            print_help_command("day_count_fraction_types list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        ...
        if (has_help) {
            print_help_command("day_count_fraction_types delete", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.deleting = read_delete_options(vm, entity::day_count_fraction_types);
    } else if (operation == add_command_name) {
        auto d = add_common_options(make_add_day_count_fraction_type_options_description());
        if (has_help) {
            print_help_command("day_count_fraction_types add", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.adding = read_add_day_count_fraction_type_options(vm);
    }
