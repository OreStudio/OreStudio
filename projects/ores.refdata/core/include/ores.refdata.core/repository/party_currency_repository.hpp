...
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.party_currency_repository";
...
public:
    ...
    /**
     * @brief Writes party currencies to database.
     *
     ...
     */
    /**@{*/
    void write(const domain::party_currency& v);
    void write(const std::vector<domain::party_currency>& v);
    /**@}*/

    /**
     * @brief Writes a party currency, honouring the claim it states.
     *
     ...
     */
    void write(const domain::party_currency& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of party currencies, each honouring its own claim, as
     * one statement.
     */
    void write(const std::vector<domain::party_currency>& v,
    ...
    std::vector<domain::party_currency> read_latest(std::uint32_t offset, std::uint32_t limit);

    /**
     * @brief Reads the party currency rows for the given pair of keys.
     *
     * A junction key is the whole pair the link names, so a read that states
     * only one half addresses a set and not a row.
     */
    std::vector<domain::party_currency> read_latest(const boost::uuids::uuid& party_id,
    ...
    /**
     * @brief Reads latest party currencies filtered by party_id, with pagination.
     */
    std::vector<domain::party_currency> read_latest_by_party(const boost::uuids::uuid& party_id,
                                                             std::uint32_t offset,
                                                             std::uint32_t limit);
    ...
    /**
     * @brief Deletes a party currency by its pair of keys.
     */
    ...
    /**
     * @brief Removes a party currency, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes party currencies by their pairs of keys.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(const domain::party_currency& v);
     ...
     */
    domain::party_currency apply_claim(const domain::party_currency& v,
