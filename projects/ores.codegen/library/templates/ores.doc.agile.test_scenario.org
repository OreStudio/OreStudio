...
# * Steps entries) -- fill in the *specific* persona this scenario
...
# up via `accounts list` rather than assuming a fixed name), which
# party/environment to select, and how to get back to that state if
# the database was recreated since (the provisioning script to
# re-run). Never leave the tester to guess credentials or starting
# state -- and never copy this placeholder verbatim.
