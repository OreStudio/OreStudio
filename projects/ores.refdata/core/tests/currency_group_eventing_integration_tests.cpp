...
#include "ores.refdata.api/generators/currency_group_generator.hpp"
...
// notification" pattern end to end for currency_group -- the
...
using ores::refdata::domain::currency_group;
using ores::refdata::repository::currency_group_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_currency_group_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::currency_group_changed_event>(
        [&nats](const ores::refdata::eventing::currency_group_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::currency_group_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.currency_group",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::currency_group_changed_event>(
        "ores.refdata.currency_group", "ores_refdata_currency_groups");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::currency_group_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_currency_group(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Currency Group: " << v;

    currency_group_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.currency_group") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for currency_group " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for currency_group " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_group(id_str).has_value());
        REQUIRE(svc.get_group_history(id_str).size() == versions.size());
    }
}
