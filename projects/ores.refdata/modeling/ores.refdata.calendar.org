...
Soft-enum: =quantlib= | =user=. =quantlib= means this template's rule
set was transcribed from QuantLib's published rules and matches one
of ORE's recognised built-in calendar names -- a taxonomic fact
driving read-only enforcement and the ORE-export skip logic, *not* an
instruction to call the QuantLib library (QuantLib is not linked at
runtime; see this task's =* Revision= section). =user= means a
calendar template authored in ORE Studio, fully editable.
