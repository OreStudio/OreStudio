...
std::vector<std::byte> get_monetary_nature_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_monetary_nature_history_request, error_code>
get_monetary_nature_history_request::deserialize(std::span<const std::byte> data) {
    get_monetary_nature_history_request request;
...
std::vector<std::byte> get_monetary_nature_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_monetary_nature(buffer, v);
    }
    return buffer;
}

std::expected<get_monetary_nature_history_response, error_code>
get_monetary_nature_history_response::deserialize(std::span<const std::byte> data) {
