...
save_dataset_request::from(domain::dataset dataset) {
    return save_dataset_request{std::vector<domain::dataset>{std::move(dataset)}};
}

save_dataset_request
save_dataset_request::from(std::vector<domain::dataset> datasets) {
    return save_dataset_request{std::move(datasets)};
}
...
save_dataset_request::deserialize(std::span<const std::byte> data) {
    ...
    request.datasets.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_dataset(data);
        if (!e) return std::unexpected(e.error());
        request.datasets.push_back(std::move(*e));
    }
    return request;
}
...
save_methodology_request
save_methodology_request::from(domain::methodology methodology) {
    return save_methodology_request{
        std::vector<domain::methodology>{std::move(methodology)}};
}

save_methodology_request
save_methodology_request::from(std::vector<domain::methodology> methodologies) {
    return save_methodology_request{std::move(methodologies)};
}

std::vector<std::byte> save_methodology_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(methodologies.size()));
    for (const auto& e : methodologies)
        write_methodology(buffer, e);
    return buffer;
}
...
save_methodology_request::deserialize(std::span<const std::byte> data) {
    ...
    request.methodologies.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_methodology(data);
        if (!e) return std::unexpected(e.error());
        request.methodologies.push_back(std::move(*e));
    }
    return request;
}
