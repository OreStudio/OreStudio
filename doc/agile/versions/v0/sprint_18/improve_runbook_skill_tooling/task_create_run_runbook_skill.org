...
Deleted draft =doc/llm/skills/run-runbook/SKILL.org= (manually written, non-compliant scaffold). Scaffolded replacement via =compass add skill --slug run_runbook=, producing =doc/llm/skills/run_runbook/SKILL.org= with correct v2 frontmatter and markdown export block. Skill directs the LLM to open the [[id:D1125E49-3C4E-48D4-B7F9-15CAA45B3ADD][Runbooks catalogue]] to find and match the user's request — it does not embed a runbook list. Registered under Meta-skills in =doc/llm/skills/claude_code_skills.org=.
