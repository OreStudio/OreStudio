...
| =recreate_template.sh=     | Recreate just =ores_template=              |
| =recreate_admin.sh=        | Recreate just =ores_admin=                 |
| =recreate_env.sh -e local2= | Recreate just =ores_dev_local2=           |
| =drop_database.sh=         | Drop a single database safely              |
| =teardown_database.sh=     | Kill connections + drop database           |

* Database Roles and Users

ORE Studio uses a role-based access control system for database connections.
This is separate from the application-level RBAC stored in the =production.iam_*=
tables.
