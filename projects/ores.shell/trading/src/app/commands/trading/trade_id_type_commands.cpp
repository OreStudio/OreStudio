...
void trade_id_type_commands::register_commands(cli::Menu& root_menu,
    ...
    trade_id_types_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_trade_id_type(std::ref(out), std::ref(session), args);
        },
        "Add trade ID type (code "
        "description "
        "change_reason_code "
        "\"change_commentary\")");
    ...
    trade_id_types_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_trade_id_type(std::ref(out), std::ref(session), std::move(id));
        },
        "Delete trade ID type by code");
    ...
    trade_id_types_menu->Insert(
        ...
        "Show trade ID type's version history (--diff for a unified diff, --version "
        "<n> to pick one)",
        {"code [--diff] [--version <n>]"});
...
void trade_id_type_commands::process_get_trade_id_types(std::ostream& out,
                                                        nats_client& session,
                                                        pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get trade ID type request.";
...
void trade_id_type_commands::process_add_trade_id_type(std::ostream& out,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating add trade ID type request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add trade ID type." << std::endl;
        return;
    }
    ...
    auto req = trading::messaging::save_trade_id_type_request::from(std::move(v));
...
void trade_id_type_commands::process_delete_trade_id_type(std::ostream& out,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete trade ID type." << std::endl;
        return;
    }
