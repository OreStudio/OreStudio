...
void lifecycle_event_commands::register_commands(cli::Menu& root_menu,
    ...
    lifecycle_events_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_lifecycle_event(std::ref(out), std::ref(session), args);
        },
        "Add lifecycle event (code "
        "description "
        "fsm_state_id "
        "change_reason_code "
        "\"change_commentary\")");
    ...
    lifecycle_events_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_lifecycle_event(std::ref(out), std::ref(session), std::move(id));
        },
        "Delete lifecycle event by code");
    ...
    lifecycle_events_menu->Insert(
        ...
        "Show lifecycle event's version history (--diff for a unified diff, --version "
        "<n> to pick one)",
        {"code [--diff] [--version <n>]"});
...
void lifecycle_event_commands::process_get_lifecycle_events(std::ostream& out,
                                                            nats_client& session,
                                                            pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get lifecycle event request.";
...
void lifecycle_event_commands::process_add_lifecycle_event(std::ostream& out,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating add lifecycle event request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add lifecycle event." << std::endl;
        return;
    }
    ...
    try {
        ...
        v.fsm_state_id = ores::shell::app::from_token<std::optional<boost::uuids::uuid>>(
            parsed->positionals[next++], "fsm_state_id");
    ...
    auto req = trading::messaging::save_lifecycle_event_request::from(std::move(v));
...
void lifecycle_event_commands::process_delete_lifecycle_event(std::ostream& out,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete lifecycle event." << std::endl;
        return;
    }
