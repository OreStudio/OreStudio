 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_PARTY_CONTACT_INFORMATION_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_PARTY_CONTACT_INFORMATION_EVENT_HPP
...
/**
 * @brief Event traits for party contact information's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::party_contact_information_event> {
    static constexpr std::string_view subject_prefix =
        "refdata.v1.party_contact_informations_events";

    static ores::refdata::messaging::party_contact_information_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::party_contact_information_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::party_contact_information_key>(
                    notification.key))
            event.key = *key;
