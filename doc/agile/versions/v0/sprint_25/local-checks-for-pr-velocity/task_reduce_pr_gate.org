...
- Update the knowledge doc (summary, gate section, fate table, local
  flow, open questions) and the =compass-pr-raise= skill (it still said "the
...
| 2 | codegen-drift's "runs unconditionally" rationale is not preserved in the local replacement: a model-only edit classifies as docs and skips the check that catches forgotten regeneration | compass-pr-raise/SKILL.org, ci-workflows-and-local-checks.org | accepted | Docs class now runs codegen drift when =projects/*/modeling/*= is changed, in both classification tables, with the retired job's rationale noted. |
...
| 6 | build-site.yml comments still describe the deleted gate: "Pull requests build the site via pr.yml's site job" and "the same build runs on doc-class pull requests via pr.yml" | build-site.yml | accepted | Comments now say doc-class PRs build the site locally via the compass-pr-raise skill, which is where broken id-links are caught; this workflow covers main pushes (build + deploy) and manual runs only. |
...
questions) and the =compass-pr-raise= skill now describe the no-gate reality;
