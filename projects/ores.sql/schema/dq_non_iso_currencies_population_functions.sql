...
/**
 * FPML non_iso_currencies Population Functions
 *
 * Functions to publish non_iso_currencies from DQ artefact table to production.
 *
 * Usage:
 *   -- Preview what will be published
 *   SELECT * FROM ores.dq_preview_non_iso_currency_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores.dq_populate_non_iso_currencies(dataset_id, 'upsert');
 */
...
/**
 * Preview what non_iso_currencies would be copied from a DQ dataset.
 */
create or replace function ores.dq_preview_non_iso_currency_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from ores.dq_non_iso_currencies_artefact_tbl dq
    left join ores.refdata_non_iso_currencies_tbl existing
...
/**
 * Populate refdata_non_iso_currencies_tbl from a DQ non_iso_currencies dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores.dq_populate_non_iso_currencies(
    p_dataset_id uuid,
...
begin
    ...
    if p_mode = 'replace_all' then
        update ores.refdata_non_iso_currencies_tbl
    ...
    loop
        ...
        select exists (
            select 1 from ores.refdata_non_iso_currencies_tbl existing
        ...
        insert into ores.refdata_non_iso_currencies_tbl (
