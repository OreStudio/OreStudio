...
#include "ores.synthetic.api/eventing/fx_spot_generation_config_event.hpp"
...
TEST_CASE("write_fx_spot_generation_config_publishes_an_event", tags) {
    ...
    using event_type = ores::synthetic::messaging::fx_spot_generation_config_event;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        svc.delete_fx_spot_generation_config(v.id);
        ...
        REQUIRE_FALSE(svc.get_fx_spot_generation_config(v.id).has_value());
