...
// notification" pattern end to end for party_contact_information -- the
...
TEST_CASE("write_party_contact_information_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::party_contact_information_changed_event>(
        [&nats](const ores::refdata::eventing::party_contact_information_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::refdata::eventing::party_contact_information_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::party_contact_information_changed_event>(
        "ores.refdata.party_contact_information", "ores_refdata_party_contact_informations");
    ...
    auto v = generate_synthetic_party_contact_information(ctx);
    ...
    BOOST_LOG_SEV(lg, debug) << "Party Contact Information: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.party_contact_information") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for party_contact_information " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for party_contact_information "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::party_contact_information_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_party_contact_information_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_party_contact_information(id_str);
        ...
        REQUIRE_FALSE(svc.get_party_contact_information(id_str).has_value());
        REQUIRE(svc.get_party_contact_information_history(id_str).size() == versions.size());
    }
}
