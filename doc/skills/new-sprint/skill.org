...
2. Update commits since in the =readme.org= to the current sprint (note - not
...
3. Update the version in the =readme.org= with the new sprint. Example, if the
...
4. Update the package version in =.github/workflows/= for the following files:
...
5. Update the top-level =CMakeLists.txt=. Example for new sprint 05:
...
6. Update =vcpkg.json=. Example for new sprint 05:
...
7. Update =doc/agile/v0/version_zero.org= adding a row to the Sprints table for
   the new sprint. The table has three columns: Sprint number, Backlog link, and
   Mission. Use the GUID from the new sprint backlog file you will create in step
   9. Example for sprint 08:
...
+|     08 | [[id:NEW-GUID-HERE][Sprint Backlog 08]] | Sprint mission from step 9.                                                         |
#+end_src

8. Update the version header file:
   =projects/ores.utility/src/version/version.hpp=:
...
9. Create a new agile sprint backlog for the new sprint in =doc/agile/v0=, with
