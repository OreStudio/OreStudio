...
struct get_fx_barrier_option_instruments_request {
    using response_type = struct get_fx_barrier_option_instruments_response;
...
struct get_fx_barrier_option_instruments_response {
...
struct save_fx_barrier_option_instrument_request {
    ...
    static save_fx_barrier_option_instrument_request
    from(ores::trading::domain::fx_barrier_option_instrument v) {
...
struct get_fx_barrier_option_instrument_history_request {
    using response_type = struct get_fx_barrier_option_instrument_history_response;
    static constexpr std::string_view nats_subject =
        "trading.v1.fx_barrier_option_instruments.history";
    std::string instrument_id;
};

struct get_fx_barrier_option_instrument_history_response {
    std::vector<ores::trading::domain::fx_barrier_option_instrument> history;
    bool success = false;
    std::string message;
};
