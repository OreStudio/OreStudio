...
| PR | Title |
|----+-------|
| [[https://github.com/OreStudio/OreStudio/pull/2092][#2092]] | [ores.refdata,ores.marketdata] Move classification into refdata catalogs |
...
- The refdata dialogs read labels from the catalog tables through the
  lookup fetcher. The market series grid does not:
  =ClientMarketSeriesModel::data= returns the raw =asset_class= and
  =series_subclass= codes, so the grid shows the code and not the catalog
  =name=. Corrected on review; this bullet claimed the grid read labels,
