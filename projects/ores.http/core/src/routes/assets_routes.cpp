...
/**
 * @brief The body this route accepts: the ids of the images to fetch.
 *
 * The route used to borrow the assets NATS request as its HTTP schema. That
 * request now addresses images by natural-key code, so the route states its
 * own body and keeps the published endpoint addressing images by id.
 */
struct get_images_body {
    std::vector<std::string> image_ids;
};

struct get_images_body_response {
...
void assets_routes::register_routes(std::shared_ptr<http::net::router> router,
    ...
    auto get_images =
        router->post("/api/v1/assets/images")
            ...
            .tags({"assets"})
            .auth_required()
            .body<get_images_body>()
            .response<get_images_body_response>()
...
asio::awaitable<http_response> assets_routes::handle_get_images(const http_request& req) {
    ...
    try {
        auto get_req = rfl::json::read<get_images_body>(req.body);
        if (!get_req) {
        ...
        assets::service::image_service service(ctx_);
        auto images = service.get_images(get_req->image_ids);

        get_images_body_response resp;
        resp.images = images;
