...
All authenticated users within a party hold =workspace::workspaces:read= by
default. Workspaces are not private ACL objects; any user within the party can
see any workspace owned by that party. Data restriction within a workspace is
handled by the =scope_portfolio_id= on the workspace itself (portfolio tree
scoping), not by per-workspace read grants. This keeps the permission model
simple.
