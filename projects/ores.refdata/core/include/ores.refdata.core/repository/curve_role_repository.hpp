 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CURVE_ROLE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CURVE_ROLE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes curve roles to data storage.
 */
class ORES_REFDATA_CORE_EXPORT curve_role_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.curve_role_repository";
...
public:
    ...
    /**
     * @brief Writes curve roles to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::curve_role& v);
    void write(context ctx, const std::vector<domain::curve_role>& v);
    /**@}*/

    /**
     * @brief Writes a curve role, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of curve roles, each honouring its own
    ...
    /**
     * @brief Reads latest curve roles, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::curve_role> read_latest(context ctx);
    std::vector<domain::curve_role> read_latest(context ctx, const std::string& code);
    std::vector<domain::curve_role> read_latest(context ctx, const std::vector<std::string>& codes);
    /**@}*/
     ...
     */
    std::vector<domain::curve_role> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single curve role as it stood at a specific
     ...
     */
    std::optional<domain::curve_role>
    read_at_version(context ctx, const std::string& code, std::uint32_t version);


    /**
     * @brief Reads latest curve roles with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Deletes a curve role by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a curve role, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes curve roles by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::curve_role& v);
     ...
     */
    domain::curve_role apply_claim(context ctx,
                                   const domain::curve_role& v,
                                   const ores::utility::domain::precondition& claim);
};
