...
TEST_CASE("validate_trade_with_all_required_fields", tags) {
    ...
    trade t;
    t.identity.get().external_id = "Swap_1";
    t.classification.get().trade_type = "Swap";
...
TEST_CASE("import_portfolio_from_minimal_swap", tags) {
    ...
    CHECK(t.identity.get().external_id == "Swap_20y");
    CHECK(t.classification.get().trade_type == "Swap");
    CHECK(t.classification.get().netting_set_id == "CPTY_A");
    CHECK(t.classification.get().activity_type_code == "new_booking");
    CHECK(t.audit.get().modified_by == "ores");
    CHECK(t.audit.get().change_reason_code == "system.external_data_import");
}
...
TEST_CASE("import_portfolio_from_example_1", tags) {
    ...
    CHECK(first.identity.get().external_id == "Swap_20");
    CHECK(first.classification.get().trade_type == "Swap");
    CHECK(first.classification.get().netting_set_id == "CPTY_A");

    // Verify all trades have the same counterparty netting set.
    for (const auto& item : items) {
        CHECK(item.trade.classification.get().netting_set_id == "CPTY_A");
    }
}
...
TEST_CASE("import_portfolio_from_minimal_swaptions", tags) {
    ...
    for (const auto& item : items) {
        CHECK(item.trade.classification.get().trade_type == "Swaption");
    }
}
...
TEST_CASE("import_portfolio_all_trades_pass_validation", tags) {
    ...
    for (const auto& item : items) {
        ...
        INFO("Trade " << item.trade.identity.get().external_id << " failed validation: " << errors);
        CHECK(errors.empty());
    }
...
TEST_CASE("import_portfolio_with_context_swap_has_instrument", tags) {
    ...
    INFO("Trade type: " << item.trade.classification.get().trade_type);
    ...
    item.trade.identity.get().id = gen();
    const auto instr_uuid = gen();
    ores::trading::domain::stamp_ids(item.instrument, instr_uuid, item.trade.identity.get().id);
    item.trade.classification.get().instrument_id = instr_uuid;
    ...
    CHECK(instr_id != item.trade.identity.get().id);
    REQUIRE(trade_id_opt.has_value());
    CHECK(*trade_id_opt == item.trade.identity.get().id);
    CHECK(item.trade.classification.get().instrument_id == instr_id);
    CHECK(item.trade.classification.get().product_type == ores::trading::domain::product_type::swap);
    CHECK(!r.legs.empty());
...
TEST_CASE("import_portfolio_with_context_fx_forward_has_instrument", tags) {
    ...
    CHECK(instr.instrument_id != item.trade.identity.get().id);
    REQUIRE(instr.trade_id.has_value());
    CHECK(*instr.trade_id == item.trade.identity.get().id);
    CHECK(item.trade.classification.get().instrument_id == instr.instrument_id);
    CHECK(item.trade.classification.get().product_type == ores::trading::domain::product_type::fx);
...
TEST_CASE("import_portfolio_with_context_bond_has_instrument", tags) {
    ...
    CHECK(r.instrument_id != item.trade.identity.get().id);
    CHECK(r.trade_id == item.trade.identity.get().id);
    CHECK(item.trade.classification.get().instrument_id == r.instrument_id);
    CHECK(item.trade.classification.get().product_type == ores::trading::domain::product_type::bond);
...
TEST_CASE("import_portfolio_with_context_unmapped_type_is_monostate", tags) {
    ...
    BOOST_LOG_SEV(lg, info) << "Unmapped trade type '" << item.trade.classification.get().trade_type
