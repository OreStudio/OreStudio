...
public:
    ...
    /**
     * @brief Reads latest books filtered by name.
     */
    std::vector<domain::book> read_latest_by_name(context ctx, const std::string& name);

    /**
     * @brief Reads the newest books filtered by name, current or not.
     *
     ...
     */
    std::vector<domain::book> read_any_by_name(context ctx, const std::string& name);
