        ...
        if (auto req = decode<get_counterparty_identifiers_request>(msg)) {
            try {
                resp.counterparty_identifiers =
                    svc.list_counterparty_identifiers(req->offset, req->limit);
    ...
    void history(ores::nats::message msg) {
        ...
        if (auto req = decode<get_counterparty_identifier_history_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      get_counterparty_identifier_history_response{.history = std::move(hist),
                                                                   .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(counterparty_identifier_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_counterparty_identifier_history_response{.success = false,
                                                                   .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(counterparty_identifier_handler_lg(), warn)
