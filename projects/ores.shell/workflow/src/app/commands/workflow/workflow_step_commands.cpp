...
namespace messaging = ores::workflow::messaging;
...
void workflow_step_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("workflow_steps");
    ...
    menu->Insert(
        "by-workflow-id",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_workflow_id(std::ref(out), std::ref(session), std::move(args));
        },
        "by-workflow-id <workflow_id> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
...
void workflow_step_commands::process_list(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_workflow_steps_request;
    ...
    auto result = do_auth_request<messaging::list_workflow_steps_response>(
...
void workflow_step_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_workflow_step_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_workflow_step_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_workflow_steps_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_workflow_steps_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_add(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_workflow_step_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.workflow_id, parsed->positionals[next++], "workflow_id");
        read_token(req.change.write.step_index, parsed->positionals[next++], "step_index");
        ...
        read_token(req.change.write.command_published_at,
                   parsed->positionals[next++],
                   "command_published_at");
        read_token(
            req.change.write.idempotency_key, parsed->positionals[next++], "idempotency_key");
        read_token(req.change.write.compensation_subject,
                   parsed->positionals[next++],
                   "compensation_subject");
        read_token(
            req.change.write.compensation_json, parsed->positionals[next++], "compensation_json");
    ...
    auto result = do_auth_request<messaging::put_workflow_step_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_set(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
...
void workflow_step_commands::process_put_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_workflow_steps_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::workflow_step_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.workflow_id, parsed->positionals[next++], "workflow_id");
            read_token(change.write.step_index, parsed->positionals[next++], "step_index");
            ...
            read_token(change.write.command_published_at,
                       parsed->positionals[next++],
                       "command_published_at");
            read_token(
                change.write.idempotency_key, parsed->positionals[next++], "idempotency_key");
            read_token(change.write.compensation_subject,
                       parsed->positionals[next++],
                       "compensation_subject");
            read_token(
                change.write.compensation_json, parsed->positionals[next++], "compensation_json");
    ...
    auto result = do_auth_request<messaging::put_many_workflow_steps_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_delete(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_workflow_step_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_workflow_step_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_delete_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_workflow_steps_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::workflow_step_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_workflow_steps_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_by_workflow_id(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-workflow-id request.";

    using request_type = messaging::list_by_workflow_id_workflow_steps_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-workflow-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.workflow_id = ores::shell::app::from_token<boost::uuids::uuid>(
            parsed->positionals[next++], "workflow_id");
    ...
    auto result = do_auth_request<messaging::list_by_workflow_id_workflow_steps_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_versions(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_workflow_step_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_workflow_step_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void workflow_step_commands::process_version(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_workflow_step_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_workflow_step_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
