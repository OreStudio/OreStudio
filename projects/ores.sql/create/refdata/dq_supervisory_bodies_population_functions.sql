 ...
 *   SELECT * FROM ores_dq_preview_supervisory_body_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_supervisory_bodies(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_supervisory_body_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_supervisory_bodies_tbl existing
...
/**
 * Populate ores_refdata_supervisory_bodies_tbl from a DQ supervisory_bodies dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_supervisory_bodies(
    p_dataset_id uuid,
