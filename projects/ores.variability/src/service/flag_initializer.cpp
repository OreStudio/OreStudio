...
std::size_t flag_initializer::ensure_system_flags_exist() {
    BOOST_LOG_SEV(lg(), info) << "Ensuring system flags exist in database";
    ...
    for (const auto& def : domain::system_flag_definitions) {
        ...
        BOOST_LOG_SEV(lg(), info) << "Creating system flag " << flag_name
            << " with default value: "
    ...
    BOOST_LOG_SEV(lg(), info) << "System flag initialization complete. "
        << "Created " << created_count << " new flags, "
        << (domain::system_flag_definitions.size() - created_count)
        << " already existed.";
