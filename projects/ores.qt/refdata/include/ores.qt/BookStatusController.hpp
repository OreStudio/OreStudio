...
private:
    inline static std::string_view logger_name = "ores.qt.book_status_controller";
...
private slots:
    ...
    void onOpenVersion(const refdata::domain::book_status& status, int versionNumber);
...
private:
    ...
    /**
     * @brief Fetches the full typed book status history (the
     * existing per-entity refdata::messaging::get_book_status_history_request/
     * refdata::messaging::get_book_status_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed book status, since the
     * generic dialog holds no typed domain data.
     */
    void fetchBookStatusHistory(
