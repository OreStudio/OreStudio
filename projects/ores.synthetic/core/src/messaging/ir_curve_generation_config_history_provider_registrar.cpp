...
void register_ir_curve_generation_config_history_provider(
    ...
    registry.register_history_provider(
        ...
        [](const ores::database::context& scoped_ctx, const std::string& entity_id) {
            service::ir_curve_generation_config_service svc(scoped_ctx);
            auto versions = svc.get_ir_curve_generation_config_history(entity_id);
            return ores::history::service::build_entity_history_versions(
                versions, presentation::render_ir_curve_generation_config_fields);
        });
}
