...
#include "ui_CurrencyPairHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

CurrencyPairHistoryDialog::CurrencyPairHistoryDialog(const QString& code,
                                                     ClientManager* clientManager,
                                                     QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::CurrencyPairHistoryDialog)
    , code_(code)
...
CurrencyPairHistoryDialog::~CurrencyPairHistoryDialog() {
    delete ui_;
}

QString CurrencyPairHistoryDialog::code() const {
    return code_;
}

void CurrencyPairHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for currency pair: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_currency_pair_history_request request;
    request.pair_code = code_.toStdString();

    QPointer<CurrencyPairHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_currency_pair_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int CurrencyPairHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString CurrencyPairHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow CurrencyPairHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult CurrencyPairHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void CurrencyPairHistoryDialog::displayFullDetails(int index) {
...
void CurrencyPairHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void CurrencyPairHistoryDialog::revertToVersionAt(int index) {
