 ...
 *   SELECT * FROM metadata.dq_preview_business_process_population_fn(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM metadata.dq_populate_business_processes_fn(dataset_id, 'upsert');
 */
 ...
 */
create or replace function metadata.dq_preview_business_process_population_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_business_processes_artefact_tbl dq
    left join production.refdata_business_processes_tbl existing
 ...
 */
create or replace function metadata.dq_populate_business_processes_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_business_processes_tbl existing
        ...
        insert into production.refdata_business_processes_tbl (
