...
void bond_instrument_repository::write(context ctx, const domain::bond_instrument& v) {
    ...
    execute_write_query(
        ctx, bond_instrument_mapper::map(v), lg(), "Writing bond instrument to database.");
}

void bond_instrument_repository::write(context ctx, const std::vector<domain::bond_instrument>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing bond instruments. Count: " << v.size();
    execute_write_query(
        ctx, bond_instrument_mapper::map(v), lg(), "Writing bond instruments to database.");
}

std::vector<domain::bond_instrument> bond_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
            ...
            lg(),
            "Reading latest bond instruments (workspace resolution chain).");
    }
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond instruments");
}
...
std::optional<domain::bond_instrument> bond_instrument_repository::read_at_version(
    ...
    const auto entities = execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading bond instrument at version.");
...
std::vector<domain::bond_instrument>
bond_instrument_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest bond instruments with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond instruments with pagination.");
}

std::uint32_t bond_instrument_repository::get_total_bond_instrument_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active bond instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<bond_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active bond instrument count: " << count;
    return count;
}
...
bond_instrument_repository::read_latest(context ctx,
    ...
    auto result = execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond instruments by ids.");
    return result;
}
