...
TEST_CASE("write_concurrency_policy_publishes_nats_changed_event", tags) {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for concurrency_policy " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_policy(id_str).has_value());
        REQUIRE(svc.get_policy_history(id_str).size() == versions.size());
    }
}
