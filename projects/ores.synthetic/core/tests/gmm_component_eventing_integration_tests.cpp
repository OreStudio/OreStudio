...
// notification" pattern end to end for gmm_component -- the
...
// GMM Component writes are party-scoped: the session-level
...
using ores::synthetic::repository::gmm_component_repository;
...
TEST_CASE("write_gmm_component_publishes_nats_changed_event", tags) {
    ...
    auto v = generate_synthetic_gmm_component(ctx);
    v.change_reason_code = "system.test";
    v.party_id = *party_ctx.party_id();
    // Seed the active fx_spot_generation_config row ores_synthetic_fx_spot_generation_configs_tbl
    ...
    BOOST_LOG_SEV(lg, debug) << "GMM Component: " << v;

    gmm_component_repository repo;
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for gmm_component " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for gmm_component " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::synthetic::service::gmm_component_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_gmm_component_history(id_str);
