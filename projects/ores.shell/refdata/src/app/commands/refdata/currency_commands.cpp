...
void currency_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("currencies");
    ...
    menu->Insert(
        ...
        "delete <iso_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <iso_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <iso_code> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <iso_code> --version <n>");

    root_menu.Insert(std::move(menu));
}

void currency_commands::process_list(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_currencies_request;
    ...
    auto result = do_auth_request<messaging::list_currencies_response>(
...
void currency_commands::process_get(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.iso_code, parsed->positionals[next++], "iso_code");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_get_many(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            ...
            read_token(key.iso_code, parsed->positionals[i + 0], "iso_code");
    ...
    auto result = do_auth_request<messaging::get_many_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_add(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.iso_code, parsed->positionals[next++], "iso_code");
        read_token(req.change.write.name, parsed->positionals[next++], "name");
        read_token(req.change.write.numeric_code, parsed->positionals[next++], "numeric_code");
        read_token(req.change.write.symbol, parsed->positionals[next++], "symbol");
        read_token(
            req.change.write.fraction_symbol, parsed->positionals[next++], "fraction_symbol");
        read_token(
            req.change.write.fractions_per_unit, parsed->positionals[next++], "fractions_per_unit");
        read_token(req.change.write.rounding_type, parsed->positionals[next++], "rounding_type");
        read_token(
            req.change.write.rounding_precision, parsed->positionals[next++], "rounding_precision");
        read_token(req.change.write.format, parsed->positionals[next++], "format");
        read_token(
        ...
        read_token(
            req.change.write.base_precedence, parsed->positionals[next++], "base_precedence");
    ...
    auto result = do_auth_request<messaging::put_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_set(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
...
void currency_commands::process_put_many(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::currency_change change;
            read_token(change.write.iso_code, parsed->positionals[next++], "iso_code");
            read_token(change.write.name, parsed->positionals[next++], "name");
            read_token(change.write.numeric_code, parsed->positionals[next++], "numeric_code");
            read_token(change.write.symbol, parsed->positionals[next++], "symbol");
            read_token(
                change.write.fraction_symbol, parsed->positionals[next++], "fraction_symbol");
            read_token(
                change.write.fractions_per_unit, parsed->positionals[next++], "fractions_per_unit");
            read_token(change.write.rounding_type, parsed->positionals[next++], "rounding_type");
            read_token(
                change.write.rounding_precision, parsed->positionals[next++], "rounding_precision");
            read_token(change.write.format, parsed->positionals[next++], "format");
            read_token(
            ...
            read_token(
                change.write.base_precedence, parsed->positionals[next++], "base_precedence");
    ...
    auto result = do_auth_request<messaging::put_many_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_delete(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_currency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.iso_code, parsed->positionals[next++], "iso_code");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_currency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_delete_many(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_currencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::currency_key key;
            read_token(key.iso_code, parsed->positionals[i * 1 + 0], "iso_code");
            req.removals.push_back(messaging::currency_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_currencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_versions(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_currency_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_currency_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_commands::process_version(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_currency_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.currency.iso_code, parsed->positionals[next++], "iso_code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_currency_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
