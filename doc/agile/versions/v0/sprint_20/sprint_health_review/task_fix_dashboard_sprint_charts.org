...
Three targeted changes landed in PR #1215:

1. *Commits chart removed.* Deleted the =VelocityCharts= component and its
   GitHub Commits API fetch entirely.  The component existed only to
   render "Commits per sprint day"; with it gone, the dashboard no longer
   makes an unauthenticated GitHub API call on every page load.

2. *Tasks chart relocated.* "Tasks done per sprint day" now renders
   inside =Panels= alongside the burn-up line, velocity bar, and task-state
   bar.  =velocityData= is computed once in =Panels= via =useMemo=.

3. *Sprint images fixed.* =render.js= =orgToHtml= now rewrites every
   ="proj:= prefix in the rendered HTML to =="/OreStudio/=, matching the
   Emacs =ores/proj-site-root= convention.  Sprint org files embed chart
   PNGs as =[[proj:doc/agile/...]]=; those now resolve to the correct
   absolute site path.
