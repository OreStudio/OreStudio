 ...
 * process type. Each row declares one parameter and the four display
 * fields that drive the Qt parameter table: display_name (the English
 * name, e.g. "Mean reversion speed"), symbol (the Greek letter where
 * one is conventional, e.g. κ for kappa; NULL otherwise), short_label
 * (the short layperson name shown in Simple mode, e.g. "Reversion
 * speed") and description (the rich tooltip text). The rows also carry
 * the data type, default value, and (where the model requires it)
 * min/max bounds; the bounds are enforced both by the UI spin boxes and
 * by the mapping layer (map_parameters_to_yield_curve_process) at
...
insert into ores_synthetic_process_parameter_definitions_tbl (
    tenant_id, id, version, process_type_code, parameter_name,
    display_name, symbol, short_label, description,
...
values
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0, 'VASICEK', 'kappa',
     'Mean reversion speed', 'κ', 'Reversion speed',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'VASICEK', 'theta',
     'Long-term mean', 'θ', 'Long-term mean',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'VASICEK', 'initial_rate',
     'Initial short rate', null, 'Initial rate',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'BLACK_KARASINSKI', 'kappa',
     'Mean reversion speed of ln r', 'κ', 'Reversion speed',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'BLACK_KARASINSKI', 'theta',
     'Long-run level of ln r', 'θ', 'Long-run level',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'BLACK_KARASINSKI', 'sigma',
     'Volatility of ln r', 'σ', 'Volatility',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'TWO_FACTOR_GAUSSIAN', 'kappa_x',
     'Mean reversion speed of the first factor', 'κ₁', 'Reversion speed of factor 1',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'TWO_FACTOR_GAUSSIAN', 'kappa_y',
     'Mean reversion speed of the second factor', 'κ₂', 'Reversion speed of factor 2',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'TWO_FACTOR_GAUSSIAN', 'sigma_x',
     'Volatility of the first factor', 'σ₁', 'Volatility of factor 1',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'TWO_FACTOR_GAUSSIAN', 'sigma_y',
     'Volatility of the second factor', 'σ₂', 'Volatility of factor 2',
    ...
    (ores_utility_system_tenant_id_fn(), gen_random_uuid(), 0,'TWO_FACTOR_GAUSSIAN', 'rho',
     'Correlation between the two factors', 'ρ', 'Correlation',
