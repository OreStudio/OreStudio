...
- =scoped_env_unset= is superseded by =scoped_environment_override= but
  not deleted — it still serves one test in scheduling that uses a
  different pattern. A follow-up capture should audit and either migrate
  or justify the remaining usage.
