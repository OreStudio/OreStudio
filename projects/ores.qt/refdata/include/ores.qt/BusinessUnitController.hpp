...
/**
 * @brief Controller for managing business unit windows and operations.
 *
...
private slots:
    void onShowDetails(const refdata::domain::business_unit& business_unit);
...
private:
    ...
    /**
     * @brief Fetches the full typed business unit history (the
     * existing per-entity refdata::messaging::get_business_unit_history_request/
     * refdata::messaging::get_business_unit_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed business unit, since the
     * generic dialog holds no typed domain data.
     */
    void fetchBusinessUnitHistory(
