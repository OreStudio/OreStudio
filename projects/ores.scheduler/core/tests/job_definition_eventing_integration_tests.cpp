...
#include "ores.scheduler.api/eventing/job_definition_event.hpp"
...
// notification" pattern end to end for job_definition -- the
...
// Job Definition writes are party-scoped: the session-level
...
using namespace ores::scheduler::generators;
using ores::scheduler::domain::job_definition;
using ores::scheduler::repository::job_definition_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_job_definition_publishes_an_event", tags) {
    ...
    using event_type = ores::scheduler::messaging::job_definition_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_scheduler_job_definitions");
    ...
    auto v = generate_synthetic_job_definition(ctx);
    v.change_reason_code = "system.test";
    v.party_id = *party_ctx.party_id();
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Job Definition: " << v;

    job_definition_repository repo;
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for job_definition " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for job_definition " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_definition(v.id).has_value());
        REQUIRE(svc.get_definition_history(id_str).size() == versions.size());
    }
}
