...
list. This template defines seven:
...
{{/is_optional_string}}
            ...
            if (self->imageCache())
                apply_flag_icons(combo, self->imageCache(), FlagSource::{{flag_source_pascal}});
        });
...
void {{domain_entity.entity_pascal}}DetailDialog::populate{{combo_setter_pascal}}() {
    ...
    populateDynamicCombo<{{combo_domain_type}}>(
        ...
        [this]() { setup_badge_combo(this, ui_->{{widget}}, badgeCache(), "{{badge_key}}"); }{{/badge_key}});
}
{{/combo_domain_type}}
{{^combo_domain_type}}
{{^combo_fetch_fn}}
