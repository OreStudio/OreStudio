...
--   - book_statuses, regulatory_book_types, book_purpose_types,
...
begin
    ...
    -- Book purpose types (Trading, Reserve, Funding, Wash, ... -- the risk
    -- role a book plays)
    ...
    select
        ...
        v_actor, v_actor, 'system.new_record',
        'Copied from system tenant during provisioning'
    from ores_refdata_book_purpose_types_tbl t
    where t.tenant_id = v_system_tenant_id
      and t.valid_to = ores_utility_infinity_timestamp_fn();

    get diagnostics v_copied_count = row_count;
    raise notice 'Copied % book purpose types', v_copied_count;

    -- Ledger feed types (None, Automatic, Manual -- how a book's ledger
    -- balance is fed)
    ...
    select
        ...
        v_actor, v_actor, 'system.new_record',
        'Copied from system tenant during provisioning'
    from ores_refdata_ledger_feed_types_tbl t
    where t.tenant_id = v_system_tenant_id
      and t.valid_to = ores_utility_infinity_timestamp_fn();

    get diagnostics v_copied_count = row_count;
    raise notice 'Copied % ledger feed types', v_copied_count;
    ...
    -- Monetary natures (e.g. fiat, commodity, synthetic, supranational)
    ...
    -- Series subclass codes (e.g. spot, yield, fra)
    ...
    select
        ...
        v_actor, v_actor, 'system.new_record',
        'Copied from system tenant during provisioning'
    from ores_refdata_series_subclass_codes_tbl t
    where t.tenant_id = v_system_tenant_id
      and t.valid_to = ores_utility_infinity_timestamp_fn();

    get diagnostics v_copied_count = row_count;
    raise notice 'Copied % series subclass codes', v_copied_count;
