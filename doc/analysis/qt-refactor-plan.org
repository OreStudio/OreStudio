...
Commit =af67db3062= (2026-09-19, "Remove the =ores.qt= and =ores.wt= trees")
deleted the Qt desktop client, so no step of this plan can be executed. The
nine steps below name targets that do not exist, and every =ores.qt.*= library
they extract is gone. The replacement surface is the TypeScript UI under
=ores.web=. See
[[id:A3F9B2C1-4E72-4D8A-B1C5-F2A73E9D0456][qt-plugin-architecture.org]] for
the architectural rationale, which is itself superseded.

See [[id:A3F9B2C1-4E72-4D8A-B1C5-F2A73E9D0456][qt-plugin-architecture.org]] for the architectural rationale.
...
3. Update the top-level =CMakeLists.txt= (projects list),
   [[proj:projects/modeling/system_model.org][=projects/modeling/system_model.org=]], and CI configuration.
4. Update [[id:59D662B0-D4E4-6A34-9EAB-EAC6E98F6BC9][Component Creator]] skill (now there are 7 Qt
