...
| notional                | DOUBLE        | No       | Contract notional or equity value        |
| quantity                | DOUBLE        | Yes      | Number of shares/contracts (positions)   |
...
| strike_price            | DOUBLE        | Yes      | Option strike                            |
| exercise_type           | TEXT          | Yes      | European / American / Bermudan           |
| barrier_type            | TEXT          | Yes      | UpIn / UpOut / DownIn / DownOut          |
| lower_barrier           | DOUBLE        | Yes      | Double-barrier lower level               |
| upper_barrier           | DOUBLE        | Yes      | Double-barrier upper level               |
| average_type            | TEXT          | Yes      | Arithmetic / Geometric (Asian options)   |
| averaging_start_date    | DATE          | Yes      | Asian averaging window start             |
| variance_strike         | DOUBLE        | Yes      | VarianceSwap strike variance             |
| cliquet_frequency_code  | TEXT          | Yes      | FK → payment_frequency_types (Cliquet)  |
| accumulation_amount     | DOUBLE        | Yes      | Per-fixing accumulation (Accumulator)    |
| knock_out_barrier       | DOUBLE        | Yes      | Accumulator knock-out level              |
...
| fixed_price             | DOUBLE        | Yes      | CommoditySwap fixed leg price                  |
...
| spread_amount           | DOUBLE        | Yes      | Spread option strike spread                    |
| strip_frequency_code    | TEXT          | Yes      | FK → payment_frequency_types (option strips)  |
| variance_strike         | DOUBLE        | Yes      | VarianceSwap strike variance                   |
| accumulation_amount     | DOUBLE        | Yes      | Accumulator per-fixing amount                  |
| knock_out_barrier       | DOUBLE        | Yes      | Accumulator knock-out                          |
| barrier_type            | TEXT          | Yes      | Barrier option type                            |
| lower_barrier           | DOUBLE        | Yes      | Window/generic barrier lower level             |
| upper_barrier           | DOUBLE        | Yes      | Window/generic barrier upper level             |
| basket_json             | TEXT          | Yes      | JSON array of {code, weight} for baskets       |
| day_count_code          | TEXT          | Yes      | FK → day_count_fraction_types (swaps)         |
| payment_frequency_code  | TEXT          | Yes      | FK → payment_frequency_types (swaps)          |
...
| option_strike     | DOUBLE | Yes      | BondOption strike (clean price)                     |
...
| option_strike     | DOUBLE | Yes      | CDS option strike spread (bps)                      |
| linked_asset_code | TEXT   | Yes      | Reference asset — CreditLinkedSwap                  |
| tranche_attachment | DOUBLE | Yes    | CBO tranche attachment point                        |
| tranche_detachment | DOUBLE | Yes    | CBO tranche detachment point                        |
...
| base_cpi             | DOUBLE | Yes      | InflationSwap base CPI level                     |
...
| currency        | TEXT   | No       | ISO 4217                                       |
| quantity        | DOUBLE | No       | Signed position quantity (negative = short)    |
| price           | DOUBLE | Yes      | Average cost or mark price                     |
