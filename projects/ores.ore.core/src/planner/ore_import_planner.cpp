    ...
    if (choices_.existing_target_book_id && choices_.existing_parent_portfolio_id) {
        ...
        for (const auto& node : nodes) {
            ...
            for (const auto& source_file : node.source_files) {
                ...
                for (auto& item : items) {
                    item.trade.identity.get().id = uuid_gen();
                    if (!std::holds_alternative<std::monostate>(item.instrument)) {
                        const auto instr_id = uuid_gen();
                        trading::domain::stamp_ids(item.instrument, instr_id, item.trade.identity.get().id);
                        item.trade.classification.get().instrument_id = instr_id;
                    }
                    item.trade.parties.get().book_id      = target_book_id;
                    item.trade.parties.get().portfolio_id = target_portfolio_id;
                    item.trade.identity.get().party_id    = choices_.party_id;
                    if (!defs.trade_date.empty())
                        item.trade.lifecycle.get().trade_date = defs.trade_date;
                    if (!defs.effective_date.empty())
                        item.trade.lifecycle.get().effective_date = defs.effective_date;
                    if (!defs.termination_date.empty())
                        item.trade.lifecycle.get().termination_date = defs.termination_date;
                    if (!defs.activity_type_code.empty())
                        item.trade.classification.get().activity_type_code = defs.activity_type_code;
                    if (defs.default_counterparty_id)
                        item.trade.parties.get().counterparty_id = defs.default_counterparty_id;
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        ...
        for (const auto& source_file : node.source_files) {
            ...
            for (auto& item : items) {
                ...
                item.trade.parties.get().book_id = b.id;
                item.trade.parties.get().portfolio_id = book_parent_id;
                item.trade.identity.get().party_id = choices_.party_id;
