...
create or replace function ores_iam_sessions_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    perform pg_notify('ores_iam_sessions', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_iam_sessions_notify_trg
after insert or update or delete on ores_iam_sessions_tbl
for each row execute function ores_iam_sessions_notify_fn();
