 ...
 *
 */
#include "ores.ore.service/app/application.hpp"
...
namespace {
constexpr std::string_view service_name = "ores.ore.service";
...
application::run(boost::asio::io_context& io_ctx,
    ...
    BOOST_LOG_SEV(lg(), info) << ores::utility::version::format_startup_message(
        "ores.ore.service", 0, 1);
    BOOST_LOG_SEV(lg(), info) << "Storage HTTP base URL: " << cfg.http_base_url;
    BOOST_LOG_SEV(lg(), info) << "Work directory: " << cfg.work_dir;
    ...
    co_await ores::service::service::run(
        io_ctx, nats, make_context(cfg.database), "ores.ore.service",
        [&svc_nats, &cfg](auto& n, auto c, auto v) {
            return ores::ore::service::messaging::registrar::register_handlers(
                n, std::move(c), std::move(*v), svc_nats,
                cfg.http_base_url, cfg.work_dir);
        },
