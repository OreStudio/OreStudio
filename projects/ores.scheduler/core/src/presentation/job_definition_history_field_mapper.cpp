...
render_job_definition_fields(const domain::job_definition& v) {
    ...
    fields.push_back({.name = "Job Name", .value = v.job_name});
    fields.push_back({.name = "Party ID",
                      .value = v.party_id ? boost::uuids::to_string(*v.party_id) : std::string{}});
    fields.push_back({.name = "Description", .value = v.description});
    fields.push_back({.name = "Command", .value = v.command});
    fields.push_back({.name = "Action Type", .value = v.action_type});
    fields.push_back({.name = "Action Payload", .value = v.action_payload});
