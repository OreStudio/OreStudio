...
read_one(repository::currency_pair_classification_repository& repo,
         const ores::database::context& ctx,
         const messaging::currency_pair_classification_key& key) {
 ...
 */
messaging::currency_pair_classification_key
key_from(const domain::currency_pair_classification& v) {
    messaging::currency_pair_classification_key key;
    key.code = v.code;
    return key;
}
...
to_domain(const messaging::currency_pair_classification_write& write) {
    domain::currency_pair_classification v;
    v.code = write.code;
    v.name = write.name;
...
messaging::get_currency_pair_classification_response
currency_pair_classification_service::get_currency_pair_classification(
    const messaging::get_currency_pair_classification_request& request) {
    messaging::get_currency_pair_classification_response response;
    ...
    response.currency_pair_classification = std::move(found.front());
    return response;
}

messaging::get_many_currency_pair_classifications_response
currency_pair_classification_service::get_many_currency_pair_classifications(
    const messaging::get_many_currency_pair_classifications_request& request) {
    messaging::get_many_currency_pair_classifications_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::currency_pair_classification_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.currency_pair_classification = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_currency_pair_classification_response
currency_pair_classification_service::put_currency_pair_classification(
    const messaging::put_currency_pair_classification_request& request) {
    messaging::put_currency_pair_classification_response response;
    domain::currency_pair_classification value;
    ...
    if (!written.empty())
        response.currency_pair_classification = std::move(written.front());
    return response;
}

messaging::put_many_currency_pair_classifications_response
currency_pair_classification_service::put_many_currency_pair_classifications(
    const messaging::put_many_currency_pair_classifications_request& request) {
    messaging::put_many_currency_pair_classifications_response response;
    std::vector<domain::currency_pair_classification> batch;
    ...
    response.classifications.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.classifications.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_currency_pair_classification_response
currency_pair_classification_service::delete_currency_pair_classification(
    const messaging::delete_currency_pair_classification_request& request) {
    messaging::delete_currency_pair_classification_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::currency_pair_classification_repository::remove_status::removed:
            break;
        case repository::currency_pair_classification_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::currency_pair_classification_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::currency_pair_classification_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
currency_pair_classification_service::delete_many_currency_pair_classifications(
    const messaging::delete_many_currency_pair_classifications_request& request) {
    messaging::delete_many_currency_pair_classifications_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
currency_pair_classification_service::list_currency_pair_classification_versions(
    const messaging::list_currency_pair_classification_versions_request& request) {
    messaging::list_currency_pair_classification_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
currency_pair_classification_service::get_currency_pair_classification_version(
    const messaging::get_currency_pair_classification_version_request& request) {
    messaging::get_currency_pair_classification_version_response response;
    auto found = repo_.read_at_version(
        ctx_, request.key.currency_pair_classification.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result currency_pair_classification_service::prepare_change(
    const messaging::currency_pair_classification_change& change,
...
std::vector<domain::currency_pair_classification>
currency_pair_classification_service::get_classifications(const std::vector<std::string>& codes) {
