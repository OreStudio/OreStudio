...
the others don't. Note: an earlier, standalone-service =src=
variant with no sibling-core dependency
(=cmake_service_src.mustache=, the pre-regroup shape) still exists in
this directory but is referenced by no profile or code — it has no
node in this graph and is a dead-code-audit candidate, not part of
this facet.
