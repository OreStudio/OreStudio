...
register_job_definition_event_mapping(ev::service::postgres_event_source& event_source,
                                      ev::service::event_bus& event_bus,
                                      ores::nats::service::client& nats) {
    // The trigger publishes on the table's own channel, and the mapping turns
    // what it says into this entity's event.
    event_source.register_entity_event_mapping<scheduler::messaging::job_definition_event>(
        "ores_scheduler_job_definitions");

    return event_bus.subscribe<scheduler::messaging::job_definition_event>(
        [&nats](const scheduler::messaging::job_definition_event& e) {
            ...
            ev::service::publish_entity_event(
                nats,
                ev::domain::event_subject<scheduler::messaging::job_definition_event>(e.action),
                e);
        });
}
