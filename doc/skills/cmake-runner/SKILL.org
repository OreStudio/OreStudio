...
4. Default to =linux-clang-debug-ninja= preset when on Linux unless otherwise
   specified.
...
Use the wrapper script for day-to-day local builds (make preset, all cores):
...
The script wraps =make -C build/output/$PRESET -j$(nproc)= and fails if the
build directory does not exist (run =cmake --preset= first).

Alternatively, invoke cmake directly (ninja preset):
...
Ninja auto-detects the number of CPU cores and builds in parallel by default.
Make does not — it requires =CMAKE_BUILD_PARALLEL_LEVEL= to be set in the
environment. Add the following to =~/.bashrc= (or equivalent) for parallel
Make builds:
...
Test logging is disabled by default for performance. To enable logging when
debugging test failures, use CMake options during configuration.
...
The =parse_test_results.py= script provides a summary of test results, including
failed tests and relevant log entries. It requires test logging to be enabled.
...
directory for the presets (e.g. =linux-clang-debug-ninja=, etc) and a top-level
