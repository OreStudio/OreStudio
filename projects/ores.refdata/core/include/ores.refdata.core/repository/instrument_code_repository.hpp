 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_INSTRUMENT_CODE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_INSTRUMENT_CODE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes instrument codes to data storage.
 */
class ORES_REFDATA_CORE_EXPORT instrument_code_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.instrument_code_repository";
...
public:
    ...
    /**
     * @brief Writes instrument codes to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::instrument_code& v);
    void write(context ctx, const std::vector<domain::instrument_code>& v);
    /**@}*/

    /**
     * @brief Writes a instrument code, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::instrument_code& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of instrument codes, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::instrument_code>& v,
               const std::vector<ores::utility::domain::precondition>& claims);

    /**
     * @brief Reads latest instrument codes, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::instrument_code> read_latest(context ctx);
     ...
     */
    std::vector<domain::instrument_code> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single instrument code as it stood at a specific
     ...
     */
    std::uint32_t get_total_instrument_count(context ctx);

    /**
     * @brief Deletes a instrument code by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a instrument code, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes instrument codes by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::instrument_code apply_claim(context ctx,
                                        const domain::instrument_code& v,
                                        const ores::utility::domain::precondition& claim);
};
