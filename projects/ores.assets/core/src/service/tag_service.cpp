...
tag_service::tag_service(context ctx)
    : ctx_(std::move(ctx)) {}
namespace {
 ...
 */
std::vector<domain::tag> read_one(repository::tag_repository& repo,
                                  const ores::database::context& ctx,
                                  const messaging::tag_key& key) {
 ...
 */
messaging::tag_key key_from(const domain::tag& v) {
    messaging::tag_key key;
    key.name = v.name;
    return key;
}
 ...
 */
domain::tag to_domain(const messaging::tag_write& write) {
    domain::tag v;
    v.id = write.id;
    v.name = write.name;
...
messaging::list_tags_response tag_service::list_tags(const messaging::list_tags_request& request) {
    ...
    response.tags = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_tag_count(ctx_);
    return response;
}

messaging::get_tag_response tag_service::get_tag(const messaging::get_tag_request& request) {
    messaging::get_tag_response response;
    ...
    response.tag = std::move(found.front());
    return response;
}

messaging::get_many_tags_response
tag_service::get_many_tags(const messaging::get_many_tags_request& request) {
    messaging::get_many_tags_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tag_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tag = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_tag_response tag_service::put_tag(const messaging::put_tag_request& request) {
    messaging::put_tag_response response;
    domain::tag value;
    ...
    if (!written.empty())
        response.tag = std::move(written.front());
    return response;
}

messaging::put_many_tags_response
tag_service::put_many_tags(const messaging::put_many_tags_request& request) {
    messaging::put_many_tags_response response;
    ...
    response.tags.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.tags.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tag_response
tag_service::delete_tag(const messaging::delete_tag_request& request) {
    messaging::delete_tag_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(row.id), expected)) {
        case repository::tag_repository::remove_status::removed:
            break;
        case repository::tag_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tag_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tag_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
tag_service::delete_many_tags(const messaging::delete_many_tags_request& request) {
    messaging::delete_many_tags_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
tag_service::list_tag_versions(const messaging::list_tag_versions_request& request) {
    messaging::list_tag_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
tag_service::get_tag_version(const messaging::get_tag_version_request& request) {
    messaging::get_tag_version_response response;
    ...
    const auto named = read_one(repo_, ctx_, request.key.tag);
    if (named.empty()) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tag_service::prepare_change(const messaging::tag_change& change,
...
std::vector<domain::tag> tag_service::list_tags(std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all asset tags";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t tag_service::count_tags() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total asset tags count";
    return repo_.get_total_tag_count(ctx_);
}


std::optional<domain::tag> tag_service::get_tag_at_version(const boost::uuids::uuid& id,
                                                           std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Getting asset tag at version. " << "id: " << id
                               << " version: " << version;
    return repo_.read_at_version(ctx_, boost::uuids::to_string(id), version);
}

std::optional<domain::tag> tag_service::get_tag(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting asset tag. " << "id: " << id;
...
std::optional<domain::tag> tag_service::get_tag_by_name(const std::string& name) {
    BOOST_LOG_SEV(lg(), debug) << "Getting asset tag by name: " << name;
    messaging::tag_key k;
    k.name = name;
    auto found = read_one(repo_, ctx_, k);
    if (found.empty())
...
std::vector<domain::tag> tag_service::get_tags(const std::vector<std::string>& ids) {
    return repo_.read_latest(ctx_, ids);
}

void tag_service::save_tag(const domain::tag& v) {
    if (v.id.is_nil())
        throw std::invalid_argument("Tag id cannot be empty.");
    BOOST_LOG_SEV(lg(), debug) << "Saving asset tag. " << "id: " << v.id;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved asset tag. " << "id: " << v.id;
}

void tag_service::save_tags(const std::vector<domain::tag>& tags) {
    for (const auto& e : tags) {
        if (e.id.is_nil())
            throw std::invalid_argument("Tag id cannot be empty.");
    }
    BOOST_LOG_SEV(lg(), debug) << "Saving " << tags.size() << " asset tags";
    auto ts = tags;
    for (auto& e : ts) {
        stamp(e, ctx_);
    }
...
void tag_service::delete_tag(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing asset tag. " << "id: " << id;
    repo_.remove(ctx_, boost::uuids::to_string(id));
    BOOST_LOG_SEV(lg(), info) << "Removed asset tag. " << "id: " << id;
}

void tag_service::delete_tags(const std::vector<std::string>& ids) {
    repo_.remove(ctx_, ids);
}

std::vector<domain::tag> tag_service::get_tag_history(const std::string& key) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for asset tag. key: " << key;
