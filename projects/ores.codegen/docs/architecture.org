...
- =get_model_type(filename, path=None)= — classify a model file's type
  (=domain_entity=, =junction=, =table=, =schema=, =field_group=, ...).
  Org models are classified via their =#+type:= frontmatter first;
  filename-suffix predicates (=is_table_model=, =is_junction_model=, ...)
  are the fallback for legacy JSON models with no frontmatter.
...
|-----------------------+-----------------+-----------------+----------------------------------------------|
| =*_table.json=        | =table=         | =table=         | =sql_schema_create.mustache= (address: ores.sql.schema) |
...
Run with =codegen.sh regenerate --component <name> --address ores.sql.schema=.
...
2. Run =codegen.sh regenerate --component <name> --address ores.sql.schema= to generate the SQL.
