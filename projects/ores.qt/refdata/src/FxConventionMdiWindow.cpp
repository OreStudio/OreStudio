...
FxConventionMdiWindow::FxConventionMdiWindow(ClientManager* clientManager,
...
void FxConventionMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &FxConventionMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &FxConventionMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &FxConventionMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &FxConventionMdiWindow::viewHistorySelected);
}
...
void FxConventionMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "FxConventionListWindow", {}, {900, 400}, 1);
}

void FxConventionMdiWindow::setupConnections() {
    connect(
        model_, &ClientFxConventionModel::dataLoaded, this, &FxConventionMdiWindow::onDataLoaded);
    connect(model_, &ClientFxConventionModel::loadError, this, &FxConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &FxConventionMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &FxConventionMdiWindow::onDoubleClicked);
...
void FxConventionMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void FxConventionMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* fxc = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showConventionHistory for code: " << fxc->id;
...
void FxConventionMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete FX convention while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " FX conventions";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 FX conventions?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete FX Convention", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making batch delete request for " << codes.size() << " FX conventions";
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "FX Convention deletion failed: " << code << " - " << result.second;
                failure_count++;
