...
Add =compass client start= and =compass client stop= subcommands so
the Qt client follows the same start/stop/status convention as every
other Operate-pillar command (=services=, =env=, =nats=), instead of
being a flat launch-only command with no way to stop an instance via
compass.
