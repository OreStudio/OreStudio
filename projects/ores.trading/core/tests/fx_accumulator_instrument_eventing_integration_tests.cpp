...
#include "ores.trading.api/eventing/fx_accumulator_instrument_changed_event.hpp"
...
// notification" pattern end to end for fx_accumulator_instrument -- the
...
// FX Accumulator Instrument writes are party-scoped: the session-level
...
TEST_CASE("write_fx_accumulator_instrument_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::trading::eventing::fx_accumulator_instrument_changed_event>(
        [&nats](const ores::trading::eventing::fx_accumulator_instrument_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::trading::eventing::fx_accumulator_instrument_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.trading.fx_accumulator_instrument",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.fx_accumulator_instrument_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::trading::eventing::fx_accumulator_instrument_changed_event>(
        "ores.trading.fx_accumulator_instrument", "ores_trading_fx_accumulator_instruments");
    ...
    auto v = generate_synthetic_fx_accumulator_instrument(ctx);
    ...
    BOOST_LOG_SEV(lg, debug) << "FX Accumulator Instrument: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.trading.fx_accumulator_instrument") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for fx_accumulator_instrument " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for fx_accumulator_instrument "
                            << id_str;
}
