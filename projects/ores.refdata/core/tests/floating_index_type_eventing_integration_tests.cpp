...
// notification" pattern end to end for floating_index_type -- the
...
TEST_CASE("write_floating_index_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::floating_index_type_changed_event>(
        [&nats](const ores::refdata::eventing::floating_index_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::floating_index_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.floating_index_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::floating_index_type_changed_event>(
        "ores.refdata.floating_index_type", "ores_refdata_floating_index_types");
    ...
    auto v = generate_synthetic_floating_index_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Floating Index Type: " << v;

    floating_index_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.floating_index_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for floating_index_type " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for floating_index_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::floating_index_type_service svc(crud_ctx);
