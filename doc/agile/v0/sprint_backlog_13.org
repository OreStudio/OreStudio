...
- Own table =ores_scheduler_job_definitions_tbl= (codegen, UUID PK, full audit
  trail, RLS for both tenant and party) linked to pg_cron via =cron_job_id=
  (bigint FK to =cron.job.jobid=).
...
- [ ] Step 1: SQL setup — pg_cron extension + codegen model + table
- [ ] Step 2: C++ library scaffold — directory, CMakeLists, umbrella header
- [ ] Step 3: Domain types — =cron_expression=, =job_status=, =job_definition=, =job_instance=
- [ ] Step 4: Builder — =job_definition_builder= fluent API
- [ ] Step 5: Repository — =job_definition_repository= (reads/writes our table)
- [ ] Step 6: Service — =cron_scheduler= (wraps pg_cron calls + uses repository)
- [ ] Step 7: Generators — =generate_synthetic_job_definition=
- [ ] Step 8: Tests — domain/builder unit tests
- [ ] Step 9: Wire into build — CMakeLists, vcpkg.json, register in projects/
