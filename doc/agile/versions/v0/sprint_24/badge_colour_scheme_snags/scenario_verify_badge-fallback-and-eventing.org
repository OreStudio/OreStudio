...
- In a terminal, run (the =delete= is rewritten by a table rule into a
  ...
  0= in the command output is expected):
  #+begin_src sh
  ./compass.sh sql -- -c "delete from ores_dq_badge_mappings_tbl where code_domain_code='party_status' and entity_code='Active';"
  #+end_src
