...
| Next         | Bring =ores.marketdata= to a regenerable state, then move the entity in one clean step. Move the shell's export step onto a protocol, which both remaining options need. Fix the import so it carries the calendar adjustment and conventions documents it can already read. |
