...
void equity_barrier_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_barrier_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_equity_barrier_option_instrument(std::ref(out), std::ref(session), args);
        },
        "Add Equity Barrier Option instrument (trade_type_code "
        "party_id "
        "trade_id "
        "underlying_name "
        "currency "
    ...
    equity_barrier_option_instruments_menu->Insert(
        ...
        "Delete Equity Barrier Option instrument by instrument_id");
    ...
    equity_barrier_option_instruments_menu->Insert(
        ...
        "Show Equity Barrier Option instrument's version history");
...
void equity_barrier_option_instrument_commands::process_get_equity_barrier_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity Barrier Option instrument request.";
...
void equity_barrier_option_instrument_commands::process_add_equity_barrier_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add Equity Barrier Option instrument." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 17 + 2;
    ...
    try {
        ...
        v.lower_barrier =
            ores::shell::app::from_token<double>(parsed->positionals[next++], "lower_barrier");
        v.lower_barrier_type = ores::shell::app::from_token<std::string>(
            parsed->positionals[next++], "lower_barrier_type");
        v.upper_barrier = ores::shell::app::from_token<std::optional<double>>(
            parsed->positionals[next++], "upper_barrier");
        v.upper_barrier_type = ores::shell::app::from_token<std::string>(
            parsed->positionals[next++], "upper_barrier_type");
        v.rebate = ores::shell::app::from_token<std::optional<double>>(parsed->positionals[next++],
                                                                       "rebate");
        v.description =
...
void equity_barrier_option_instrument_commands::process_delete_equity_barrier_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete Equity Barrier Option instrument."
                  << std::endl;
        return;
    }
