...
void cds_convention_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("cds_conventions");
...
void cds_convention_commands::process_list(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_cds_conventions_request;
    ...
    auto result = do_auth_request<messaging::list_cds_conventions_response>(
...
void cds_convention_commands::process_get(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_cds_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_cds_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_get_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_cds_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_cds_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_add(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_cds_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.frequency, parsed->positionals[next++], "frequency");
        read_token(
            req.change.write.payment_convention, parsed->positionals[next++], "payment_convention");
        ...
        read_token(
            req.change.write.settles_accrual, parsed->positionals[next++], "settles_accrual");
        read_token(req.change.write.pays_at_default_time,
                   parsed->positionals[next++],
                   "pays_at_default_time");
        read_token(req.change.write.upfront_settlement_days,
                   parsed->positionals[next++],
    ...
    auto result = do_auth_request<messaging::put_cds_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_set(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
...
void cds_convention_commands::process_put_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_cds_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::cds_convention_change change;
            ...
            read_token(change.write.frequency, parsed->positionals[next++], "frequency");
            read_token(
                change.write.payment_convention, parsed->positionals[next++], "payment_convention");
            ...
            read_token(
                change.write.settles_accrual, parsed->positionals[next++], "settles_accrual");
            read_token(change.write.pays_at_default_time,
                       parsed->positionals[next++],
                       "pays_at_default_time");
            read_token(change.write.upfront_settlement_days,
                       parsed->positionals[next++],
                       "upfront_settlement_days");
    ...
    auto result = do_auth_request<messaging::put_many_cds_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_delete(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_cds_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_cds_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_delete_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_cds_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::cds_convention_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_cds_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_versions(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_cds_convention_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_cds_convention_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void cds_convention_commands::process_version(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_cds_convention_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.cds_convention.id, parsed->positionals[next++], "id");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_cds_convention_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
