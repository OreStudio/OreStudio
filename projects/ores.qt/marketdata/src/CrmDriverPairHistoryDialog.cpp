 ...
 *
 */
#include "ores.qt/CrmDriverPairHistoryDialog.hpp"
#include "ores.marketdata.api/messaging/crm_driver_pair_protocol.hpp"
#include "ores.qt/RelativeTimeHelper.hpp"
#include "ui_CrmDriverPairHistoryDialog.h"
...
CrmDriverPairHistoryDialog::CrmDriverPairHistoryDialog(const boost::uuids::uuid& id,
                                                       const QString& code,
                                                       ClientManager* clientManager,
                                                       QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::CrmDriverPairHistoryDialog)
    , id_(id)
    , code_(code)
...
CrmDriverPairHistoryDialog::~CrmDriverPairHistoryDialog() {
    delete ui_;
}

QString CrmDriverPairHistoryDialog::code() const {
    return code_;
}

void CrmDriverPairHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for CRM driver pair: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    marketdata::messaging::get_crm_driver_pair_history_request request;
    request.id = boost::uuids::to_string(id_);

    QPointer<CrmDriverPairHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](marketdata::messaging::get_crm_driver_pair_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int CrmDriverPairHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString CrmDriverPairHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow CrmDriverPairHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult CrmDriverPairHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void CrmDriverPairHistoryDialog::displayFullDetails(int index) {
...
void CrmDriverPairHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void CrmDriverPairHistoryDialog::revertToVersionAt(int index) {
