...
|--------+-------|
| Status | FAIL |
| Notes  | seem to have two copies, saved by different services: DDL user (population scripts?) and IAM service. all else works correctly.; ; [[file:scenario_verify_dq_entity_commissioning_step8_20260807_022649.png]] |
...
|--------+-------|
| Status | FAIL |
| Notes  | same as change reason categories, weird duplication. also change reason categories needs to be a combo box. everything else works correctly.; ; [[file:scenario_verify_dq_entity_commissioning_step9_20260807_022906.png]] |
...
|--------+-------|
| Status | FAIL |
| Notes  | - seem to be missing most fields in details dialog (artefac table, target table, target subject etc).; - eventing not working, but manual reload shows new entry. |
...
|---------------+-------|
| Status        | FAILED |
| Completed at  | 2026-08-07T01:32:35Z |
