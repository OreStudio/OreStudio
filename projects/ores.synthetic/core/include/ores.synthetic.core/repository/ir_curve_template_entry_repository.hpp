...
public:
    ...
    /**
     * @brief Writes a IR curve template entry, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of IR curve template entries, each honouring its own
    ...
    /**
     * @brief Reads latest IR curve template entries, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Removes a IR curve template entry, refusing a row that moved on.
     *
...
private:
     ...
     */
    domain::ir_curve_template_entry apply_claim(context ctx,
                                                const domain::ir_curve_template_entry& v,
                                                const ores::utility::domain::precondition& claim);
};
