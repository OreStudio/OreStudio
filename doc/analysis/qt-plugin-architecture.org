...
#  define ORES_QT_API Q_DECL_EXPORT
#else
#  define ORES_QT_API Q_DECL_IMPORT
#endif
#+end_src

The =ores.qt.api= CMakeLists.txt defines =ORES_QT_API_LIBRARY= privately when
building the library and does not define it when consumers include the headers:
...
Each plugin similarly defines its own export macro (=ORES_QT_REFDATA_LIBRARY=,
etc.) for its own exported symbols.  The plugin classes themselves only need to
export their factory entry point, which Qt handles automatically via
~Q_PLUGIN_METADATA~.
