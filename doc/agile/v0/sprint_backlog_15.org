...
#+CAPTION: Clock summary at [2026-03-25 Wed 17:04], for Wednesday, March 25, 2026.
|      | <75>                                                    |        |      |      |       |
| Tags | Headline                                                | Time   |      |      |     % |
|------+---------------------------------------------------------+--------+------+------+-------|
|      | *Total time*                                            | *3:23* |      |      | 100.0 |
|------+---------------------------------------------------------+--------+------+------+-------|
|      | Stories                                                 | 3:23   |      |      | 100.0 |
|      | Active                                                  |        | 3:23 |      | 100.0 |
| code | Move generators to api layer and split http.server      |        |      | 2:49 |  83.3 |
| code | Implement full environment isolation for database roles |        |      | 0:34 |  16.7 |
#+end:

#+begin: clocktable :maxlevel 3 :scope subtree :tags t :indent nil :emphasize nil :scope file :narrow 75 :formula %
#+TBLNAME: sprint_summary
#+CAPTION: Clock summary at [2026-03-25 Wed 17:04]
|       | <75>                                                     |         |       |      |       |
| Tags  | Headline                                                 | Time    |       |      |     % |
|-------+----------------------------------------------------------+---------+-------+------+-------|
...
|       | Stories                                                  | 61:29   |       |      | 100.0 |
|       | Active                                                   |         | 61:29 |      | 100.0 |
| agile | Sprint and product backlog refinement                    |         |       | 0:54 |   1.5 |
| code  | Add BOINC-inspired distributed compute grid              |         |       | 7:03 |  11.5 |
| code  | Fix ORE import bugs                                      |         |       | 6:35 |  10.7 |
| code  | Add loading indicator to all entity list windows         |         |       | 2:49 |   4.6 |
| code  | Replace =feature_flags= with unified system_settings     |         |       | 5:59 |   9.7 |
| code  | Implement JWT refresh                                    |         |       | 3:16 |   5.3 |
| code  | Add compute grid telemetry pipeline                      |         |       | 2:10 |   3.5 |
| code  | Add service dashboard with live RAG status               |         |       | 2:49 |   4.6 |
| code  | E2E compute testing fixes and loading lifecycle refactor |         |       | 8:43 |  14.2 |
| code  | Add NATS service discovery for HTTP base URL             |         |       | 7:10 |  11.7 |
| code  | Extract API from main component                          |         |       | 8:13 |  13.4 |
| code  | Fix missing tenant_id in system settings tests           |         |       | 0:30 |   0.8 |
| code  | Fix sccache cache bloat causing slow/cold builds         |         |       | 0:30 |   0.8 |
| code  | Move generators to api layer and split http.server       |         |       | 2:55 |   4.7 |
| code  | Implement full environment isolation for database roles  |         |       | 1:53 |   3.1 |
#+end:
...
*** STARTED CDM Phase 1: Rates instrument domain model                 :code:
    :LOGBOOK:
    CLOCK: [2026-03-26 Thu 15:00]
    :END:
...
- [ ] SQL: =ores_trading_instruments_tbl= + notify trigger + drop files
- [ ] SQL: =ores_trading_swap_legs_tbl= + notify trigger + drop files
- [ ] SQL: Register in =trading_create.sql=, =drop_trading.sql=,
  =populate_trading.sql= + seed data
- [ ] Domain: =instrument= struct, JSON I/O, table I/O, generator, protocol
- [ ] Domain: =swap_leg= struct, JSON I/O, table I/O, generator, protocol
- [ ] Repository: =instrument= entity, mapper, repository, service
- [ ] Repository: =swap_leg= entity, mapper, repository, service
- [ ] Server: messaging handler + registrar registration
- [ ] Qt UI: =ClientInstrumentModel=, =InstrumentMdiWindow=,
  =SwapDetailDialog=, =InstrumentHistoryDialog=, =InstrumentController=,
  =MainWindow= integration
- [ ] CLI: =instruments list=, =instruments add=, =instruments delete=
