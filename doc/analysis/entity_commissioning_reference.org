...
1. Every generated file either:
   a. Produces zero diff against HEAD, or
   b. Has its delta classified (category from the drift table above) and the decision recorded (fix template / fix code / accept).
2. All "fix template" decisions have a corresponding template commit.
3. All "fix code" decisions have a corresponding code commit with justification.
4. All "accept" decisions are recorded in the task's =* Notes= or =* Review= table.
5. No generated files for non-target entities have been left modified.
6. The build passes (no C++ regressions).
