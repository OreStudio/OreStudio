 ...
 */
std::vector<domain::currency_pair> read_one(repository::currency_pair_repository& repo,
                                            const ores::database::context& ctx,
                                            const messaging::currency_pair_key& key) {
 ...
 */
messaging::currency_pair_key key_from(const domain::currency_pair& v) {
 ...
 */
domain::currency_pair to_domain(const messaging::currency_pair_write& write) {
    domain::currency_pair v;
    v.pair_code = write.pair_code;
    v.base_currency = write.base_currency;
    v.quote_currency = write.quote_currency;
    v.classification = write.classification;
    return v;
}
...
currency_pair_service::get_currency_pair(const messaging::get_currency_pair_request& request) {
    messaging::get_currency_pair_response response;
    ...
    response.currency_pair = std::move(found.front());
    return response;
}

messaging::get_many_currency_pairs_response currency_pair_service::get_many_currency_pairs(
    const messaging::get_many_currency_pairs_request& request) {
    messaging::get_many_currency_pairs_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::currency_pair_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.currency_pair = std::move(found.front());
...
currency_pair_service::put_currency_pair(const messaging::put_currency_pair_request& request) {
    messaging::put_currency_pair_response response;
    domain::currency_pair value;
    ...
    if (!written.empty())
        response.currency_pair = std::move(written.front());
    return response;
}

messaging::put_many_currency_pairs_response currency_pair_service::put_many_currency_pairs(
    const messaging::put_many_currency_pairs_request& request) {
    messaging::put_many_currency_pairs_response response;
    ...
    response.pairs.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.pairs.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_currency_pair_response currency_pair_service::delete_currency_pair(
    const messaging::delete_currency_pair_request& request) {
    messaging::delete_currency_pair_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.pair_code, expected)) {
        case repository::currency_pair_repository::remove_status::removed:
            break;
        case repository::currency_pair_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::currency_pair_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::currency_pair_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_currency_pairs_response currency_pair_service::delete_many_currency_pairs(
    const messaging::delete_many_currency_pairs_request& request) {
    messaging::delete_many_currency_pairs_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_currency_pair_versions_response currency_pair_service::list_currency_pair_versions(
    const messaging::list_currency_pair_versions_request& request) {
    messaging::list_currency_pair_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_currency_pair_version_response currency_pair_service::get_currency_pair_version(
    const messaging::get_currency_pair_version_request& request) {
    messaging::get_currency_pair_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.currency_pair.pair_code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
currency_pair_service::prepare_change(const messaging::currency_pair_change& change,
...
currency_pair_service::get_pairs(const std::vector<std::string>& pair_codes) {
