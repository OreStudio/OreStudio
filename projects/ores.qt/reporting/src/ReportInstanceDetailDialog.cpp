...
QString ReportInstanceDetailDialog::code() const {
    return QString::fromStdString(instance_.name);
}
...
void ReportInstanceDetailDialog::markDirty() {
    hasChanges_ = true;
    updateSaveButtonState();
}
...
void ReportInstanceDetailDialog::onDeleteClicked() {
    ...
    auto task = [self, id_str = boost::uuids::to_string(instance_.id)]() -> DeleteResult {
        if (!self || !self->clientManager_) {
            return {false, "Dialog closed"};
        }
