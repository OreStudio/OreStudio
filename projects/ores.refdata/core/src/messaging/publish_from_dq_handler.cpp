...
inline std::string_view logger_name =
    "ores.refdata.messaging.publish_from_dq_handler";
auto& lg() {
 ...
 *
 * Converts "refdata.v1.account-types.publish-from-dq" to
 * "ores_refdata_publish_account_types_from_dq_fn".
 */
std::string subject_to_fn(std::string_view subject) {
    const auto v1_pos = subject.find("v1.");
    if (v1_pos == std::string_view::npos) return {};
    ...
    return "ores_refdata_publish_" + entity + "_from_dq_fn";
}

} // namespace

publish_from_dq_handler::publish_from_dq_handler(
...
void publish_from_dq_handler::handle(ores::nats::message msg) {
    ...
    if (!parsed) {
        wf->fail("Failed to decode publish_from_dq_command: "
    ...
    if (fn_name.empty()) {
        wf->fail("Cannot derive SQL function from subject: "
    ...
    try {
        ...
        const std::string params_json =
            cmd.params_json.empty() ? "{}" : cmd.params_json;

        auto rows = execute_parameterized_multi_column_query(tenant_ctx, sql,
            {cmd.dataset_id, cmd.tenant_id, cmd.mode, params_json},
            lg(), fn_name);
        ...
        for (const auto& row : rows) {
            if (row.size() >= 2 && row[0].has_value() && row[1].has_value()) {
                const auto count =
                    static_cast<std::uint64_t>(std::stoll(*row[1]));
                if (*row[0] == "inserted")     result.records_inserted += count;
                else if (*row[0] == "updated") result.records_updated  += count;
                else if (*row[0] == "skipped") result.records_skipped  += count;
                else if (*row[0] == "deleted") result.records_deleted  += count;
            }
        }
        ...
        BOOST_LOG_SEV(lg(), error) << "publish_from_dq failed: fn=" << fn_name
            << " error=" << e.what();
        wf->fail(e.what());
    }
}
