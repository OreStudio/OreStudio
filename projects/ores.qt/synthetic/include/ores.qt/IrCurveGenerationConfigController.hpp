 ...
 */
#ifndef ORES_QT_IR_CURVE_GENERATION_CONFIG_CONTROLLER_HPP
#define ORES_QT_IR_CURVE_GENERATION_CONFIG_CONTROLLER_HPP
...
class IrCurveGenerationConfigMdiWindow;
class IrCurveGenerationConfigDetailDialog;
class DetachableMdiSubWindow;
class ChangeReasonCache;

/**
 * @brief Controller for managing IR curve generation config windows and operations.
 *
 * Manages the lifecycle of IR curve generation config list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class IrCurveGenerationConfigController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.ir_curve_generation_config_controller";
...
public:
    IrCurveGenerationConfigController(QMainWindow* mainWindow,
                                      QMdiArea* mdiArea,
...
private slots:
    void
    onShowDetails(const synthetic::domain::ir_curve_generation_config& ir_curve_generation_config);
    void onAddNewRequested();
    void
    onShowHistory(const synthetic::domain::ir_curve_generation_config& ir_curve_generation_config);
...
private:
     ...
     * IrCurveGenerationConfigDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(IrCurveGenerationConfigDetailDialog* detailDialog);
