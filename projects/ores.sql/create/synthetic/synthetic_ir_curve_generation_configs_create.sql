 ...
 *
 * IR Curve Generation Config Table
 *
...
create unique index if not exists ir_curve_generation_configs_version_uniq_idx
on "ores_synthetic_ir_curve_generation_configs_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ir_curve_generation_configs_id_uniq_idx
on "ores_synthetic_ir_curve_generation_configs_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ir_curve_generation_configs_tenant_idx
on "ores_synthetic_ir_curve_generation_configs_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_synthetic_ir_curve_generation_configs_insert_fn()
returns trigger as $$
declare
...
create or replace trigger ores_synthetic_ir_curve_generation_configs_insert_trg
before insert on "ores_synthetic_ir_curve_generation_configs_tbl"
for each row execute function ores_synthetic_ir_curve_generation_configs_insert_fn();

create or replace rule ores_synthetic_ir_curve_generation_configs_delete_rule as
on delete to "ores_synthetic_ir_curve_generation_configs_tbl" do instead (
    update "ores_synthetic_ir_curve_generation_configs_tbl"
    set valid_to = clock_timestamp()
...
-- =============================================================================
-- Validation function for ir_curve_generation_config
-- Validates that a id exists in the ir_curve_generation_configs table.
...
-- =============================================================================
create or replace function ores_synthetic_validate_ir_curve_generation_config_fn(
    p_tenant_id uuid,
    p_value uuid
) returns uuid as $$
begin
    ...
    if p_value is null then
        raise exception 'Invalid ir_curve_generation_config: value cannot be null'
            using errcode = '23502';
    end if;

    -- Allow pass-through if neither this tenant nor the system tenant has
    -- seeded active ir_curve_generation_configs yet (freshly provisioned tenant).
    if not exists (
    ...
    if not exists (
        ...
        raise exception 'Invalid ir_curve_generation_config: %. Must be one of: %', p_value, (
