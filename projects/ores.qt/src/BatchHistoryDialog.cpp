...
BatchHistoryDialog::BatchHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::BatchHistoryDialog),
      id_(id),
      code_(code),
...
void BatchHistoryDialog::setupUi() {
...
void BatchHistoryDialog::setupToolbar() {
...
void BatchHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &BatchHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &BatchHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &BatchHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void BatchHistoryDialog::loadHistory() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        emit errorOccurred("Not connected to server");
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Loading history for compute batch: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    using HistoryResult = std::pair<bool, std::vector<compute::domain::batch>>;
    QPointer<BatchHistoryDialog> self = this;
...
void BatchHistoryDialog::updateVersionList() {
...
void BatchHistoryDialog::onVersionSelected() {
...
void BatchHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.external_ref != previous.external_ref) {
        addChange("External Ref",
                  QString::fromStdString(previous.external_ref),
                  QString::fromStdString(current.external_ref));
    }
...
void BatchHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->codeValue->setText(QString::fromStdString(version.external_ref));
    ui_->nameValue->setText(QString::fromStdString(version.external_ref));
    ui_->descriptionValue->setText(QString::fromStdString(version.status));
...
void BatchHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void BatchHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void BatchHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
