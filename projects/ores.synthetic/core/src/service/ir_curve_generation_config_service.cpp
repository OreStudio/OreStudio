...
read_one(repository::ir_curve_generation_config_repository& repo,
         const ores::database::context& ctx,
         const messaging::ir_curve_generation_config_key& key) {
 ...
 */
messaging::ir_curve_generation_config_key key_from(const domain::ir_curve_generation_config& v) {
    messaging::ir_curve_generation_config_key key;
    key.id = v.id;
    return key;
}
...
ir_curve_generation_config_service::list_ir_curve_generation_configs(
    const messaging::list_ir_curve_generation_configs_request& request) {
    messaging::list_ir_curve_generation_configs_response response;
    ...
    response.ir_curve_generation_configs = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_ir_curve_generation_config_count(ctx_);
    return response;
}

messaging::get_ir_curve_generation_config_response
ir_curve_generation_config_service::get_ir_curve_generation_config(
    const messaging::get_ir_curve_generation_config_request& request) {
    messaging::get_ir_curve_generation_config_response response;
    ...
    response.ir_curve_generation_config = std::move(found.front());
    return response;
}

messaging::get_many_ir_curve_generation_configs_response
ir_curve_generation_config_service::get_many_ir_curve_generation_configs(
    const messaging::get_many_ir_curve_generation_configs_request& request) {
    messaging::get_many_ir_curve_generation_configs_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::ir_curve_generation_config_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.ir_curve_generation_config = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_ir_curve_generation_config_response
ir_curve_generation_config_service::put_ir_curve_generation_config(
    const messaging::put_ir_curve_generation_config_request& request) {
    messaging::put_ir_curve_generation_config_response response;
    domain::ir_curve_generation_config value;
    ...
    if (!written.empty())
        response.ir_curve_generation_config = std::move(written.front());
    return response;
}

messaging::put_many_ir_curve_generation_configs_response
ir_curve_generation_config_service::put_many_ir_curve_generation_configs(
    const messaging::put_many_ir_curve_generation_configs_request& request) {
    messaging::put_many_ir_curve_generation_configs_response response;
    std::vector<domain::ir_curve_generation_config> batch;
    ...
    response.ir_curve_generation_configs.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.ir_curve_generation_configs.push_back(
            written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_ir_curve_generation_config_response
ir_curve_generation_config_service::delete_ir_curve_generation_config(
    const messaging::delete_ir_curve_generation_config_request& request) {
    messaging::delete_ir_curve_generation_config_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::ir_curve_generation_config_repository::remove_status::removed:
            break;
        case repository::ir_curve_generation_config_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::ir_curve_generation_config_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::ir_curve_generation_config_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_ir_curve_generation_configs_response
ir_curve_generation_config_service::delete_many_ir_curve_generation_configs(
    const messaging::delete_many_ir_curve_generation_configs_request& request) {
    messaging::delete_many_ir_curve_generation_configs_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_ir_curve_generation_config_versions_response
ir_curve_generation_config_service::list_ir_curve_generation_config_versions(
    const messaging::list_ir_curve_generation_config_versions_request& request) {
    messaging::list_ir_curve_generation_config_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_ir_curve_generation_config_version_response
ir_curve_generation_config_service::get_ir_curve_generation_config_version(
    const messaging::get_ir_curve_generation_config_version_request& request) {
    messaging::get_ir_curve_generation_config_version_response response;
    auto found =
        repo_.read_at_version(ctx_,
                              boost::uuids::to_string(request.key.ir_curve_generation_config.id),
                              request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result ir_curve_generation_config_service::prepare_change(
    const messaging::ir_curve_generation_config_change& change,
    const ores::utility::domain::change_intent& intent,
    domain::ir_curve_generation_config& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::optional<domain::ir_curve_generation_config>
ir_curve_generation_config_service::get_ir_curve_generation_config(const boost::uuids::uuid& id) {
...
std::vector<domain::ir_curve_generation_config>
ir_curve_generation_config_service::get_ir_curve_generation_configs(
...
void ir_curve_generation_config_service::delete_ir_curve_generation_config(
