...
Business days to settlement for a spot trade in this currency
(USD=1, most others=2). Used to derive =currency_pair.spot_days= as
the max of the two legs' values.

#+begin_src cpp :name generator
faker::helper::arrayElement<int>({1, 2})
#+end_src
