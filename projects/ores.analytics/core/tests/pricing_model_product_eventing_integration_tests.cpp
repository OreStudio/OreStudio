...
#include "ores.analytics.api/eventing/pricing_model_product_event.hpp"
...
TEST_CASE("write_pricing_model_product_publishes_an_event", tags) {
    ...
    using event_type = ores::analytics::messaging::pricing_model_product_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_analytics_pricing_model_products");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.pricing_engine_type_code == v.pricing_engine_type_code)
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        auto versions = svc.get_product_history(v.pricing_engine_type_code);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_product(v.id);
        ...
        REQUIRE_FALSE(svc.get_product(v.id).has_value());
        REQUIRE(svc.get_product_history(v.pricing_engine_type_code).size() == versions.size());
    }
}
