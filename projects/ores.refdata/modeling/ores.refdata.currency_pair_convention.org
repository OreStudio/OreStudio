...
Soft FK to the owning pair, enforced by the insert trigger's existence
check in addition to the =ores_refdata_validate_currency_pair_fn=
validation below -- the same trigger-based (never hard-FK) pattern as
=pair_code= itself has always followed, and the same dual check the
pair's own base/quote legs carry. It also drives the generated
eventing test's parent seeding: the test must write an active pair
before it can write a convention.
