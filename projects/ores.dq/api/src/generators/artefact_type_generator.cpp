...
generate_synthetic_artefact_type(utility::generation::generation_context& ctx) {
    ...
    r.name = std::string(faker::word::adjective()) + " Artefact Type";
    r.description = std::string(faker::lorem::sentence());
    r.artefact_table = std::string(faker::lorem::word()) + "_artefact_tbl";
    r.target_table = std::string(faker::lorem::word()) + "_tbl";
    r.target_subject = std::string(faker::lorem::word()) + ".v1.publish-from-dq";
...
generate_synthetic_artefact_types(std::size_t n, utility::generation::generation_context& ctx) {
    std::vector<domain::artefact_type> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_artefact_type(ctx));
    return r;
}
