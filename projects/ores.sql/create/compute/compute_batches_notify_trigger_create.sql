...
create or replace function ores_compute_batches_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_external_ref text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_external_ref := OLD.external_ref;
        ...
        changed_external_ref := NEW.external_ref;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('external_ref', changed_external_ref);
    ...
    perform pg_notify('ores_compute_batches', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_compute_batches_notify_trg
after insert or update or delete on ores_compute_batches_tbl
for each row execute function ores_compute_batches_notify_fn();
