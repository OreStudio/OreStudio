...
| G08 | All | The namespace documentation headers are not dead headers. A component keeps one for each of its namespaces: =include/<component>/<component>.hpp= for a single-module component, and =include/<component>.<module>/<component>.<module>.hpp= for each module of a multi-module one, alongside the per-facet =<component>.<facet>.hpp= files. Each carries the namespace =@brief= that doxygen renders and, for the outermost one, the description the diagram conventions read. A consumer census does not apply to them: an empty namespace is their whole content, so nothing includes them and nothing should. A claim in one is a document bug, corrected in place, never a reason to delete the file. | The diagram conventions; the component's own header list |
...
standard now holds 47 items. The findings behind the new items:
