...
private:
    inline static std::string_view logger_name = "ores.qt.client_badge_severity_model";
...
public:
    ...
    explicit ClientBadgeSeverityModel(ClientManager* clientManager, QObject* parent = nullptr);
    ~ClientBadgeSeverityModel() override = default;
    ...
    /**
     * @brief Refresh badge severity data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get badge severity at the specified row.
     *
...
private:
    ...
    struct FetchResult {
        bool success;
        std::vector<dq::domain::badge_severity> severities;
        std::uint32_t total_available_count;
        QString error_message;
        QString error_details;
    };

    void fetch_severities(std::uint32_t offset, std::uint32_t limit);

    ClientManager* clientManager_;
    std::vector<dq::domain::badge_severity> severities_;
    QFutureWatcher<FetchResult>* watcher_;
    std::uint32_t page_size_{100};
    std::uint32_t total_available_count_{0};
    bool is_fetching_{false};

    using BadgeSeverityKeyExtractor = std::string (*)(const dq::domain::badge_severity&);
    RecencyTracker<dq::domain::badge_severity, BadgeSeverityKeyExtractor> recencyTracker_;
