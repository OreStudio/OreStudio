...
create table if not exists "ores"."refdata_regulatory_corporate_sectors_tbl" (
    "code" text not null,
...
on "ores"."refdata_regulatory_corporate_sectors_tbl" (code, version)
where valid_to = ores.utility_infinity_timestamp_fn();

create index if not exists refdata_regulatory_corporate_sectors_coding_scheme_idx
on "ores"."refdata_regulatory_corporate_sectors_tbl" (coding_scheme_code)
where valid_to = ores.utility_infinity_timestamp_fn();

create or replace function ores.refdata_regulatory_corporate_sectors_insert_fn()
returns trigger as $$
declare
...
before insert on "ores"."refdata_regulatory_corporate_sectors_tbl"
for each row
execute function ores.refdata_regulatory_corporate_sectors_insert_fn();

create or replace rule refdata_regulatory_corporate_sectors_delete_rule as
on delete to "ores"."refdata_regulatory_corporate_sectors_tbl"
do instead
  update "ores"."refdata_regulatory_corporate_sectors_tbl"
  set valid_to = current_timestamp
  where code = old.code
