...
create table if not exists "ores_dq_subject_areas_tbl" (
    ...
    primary key (tenant_id, name, domain_name, valid_from, valid_to),
    exclude using gist (
        tenant_id WITH =,
        name WITH =,
        domain_name WITH =,
...
create unique index if not exists subject_areas_name_domain_name_uniq_idx
...
begin
    ...
    NEW.domain_name := ores_dq_validate_data_domain_fn(NEW.tenant_id, NEW.domain_name);
    ...
    where tenant_id = NEW.tenant_id
      and name = NEW.name and domain_name = NEW.domain_name
...
on delete to "ores_dq_subject_areas_tbl" do instead (
    ...
    where tenant_id = OLD.tenant_id
      and name = OLD.name and domain_name = OLD.domain_name
      and valid_to = ores_utility_infinity_timestamp_fn();
);
