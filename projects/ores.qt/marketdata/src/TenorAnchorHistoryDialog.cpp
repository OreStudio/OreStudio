...
#include "ui_TenorAnchorHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

TenorAnchorHistoryDialog::TenorAnchorHistoryDialog(const QString& code,
                                                   ClientManager* clientManager,
                                                   QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::TenorAnchorHistoryDialog)
    , code_(code)
...
TenorAnchorHistoryDialog::~TenorAnchorHistoryDialog() {
    delete ui_;
}

QString TenorAnchorHistoryDialog::code() const {
    return code_;
}

void TenorAnchorHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for tenor anchor: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    marketdata::messaging::get_tenor_anchor_history_request request;
    request.code = code_.toStdString();

    QPointer<TenorAnchorHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](marketdata::messaging::get_tenor_anchor_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int TenorAnchorHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString TenorAnchorHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow TenorAnchorHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult TenorAnchorHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void TenorAnchorHistoryDialog::displayFullDetails(int index) {
...
void TenorAnchorHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void TenorAnchorHistoryDialog::revertToVersionAt(int index) {
