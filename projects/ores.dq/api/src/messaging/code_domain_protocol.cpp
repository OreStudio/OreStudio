...
void write_code_domain(std::vector<std::byte>& buffer, const domain::code_domain& cd) {
    ...
    writer::write_string(buffer, ores::platform::time::datetime::to_iso8601_utc(cd.recorded_at));
}

std::expected<domain::code_domain, error_code> read_code_domain(std::span<const std::byte>& data) {
    ...
    try {
        cd.recorded_at = ores::platform::time::datetime::from_iso8601_utc(*recorded_at_result);
