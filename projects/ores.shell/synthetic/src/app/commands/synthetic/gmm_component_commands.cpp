...
void gmm_component_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("gmm_components");
...
void gmm_component_commands::process_list(std::ostream& out,
    ...
    using request_type = messaging::list_gmm_components_request;
    ...
    auto result = do_auth_request<messaging::list_gmm_components_response>(
...
void gmm_component_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_gmm_component_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_gmm_component_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_gmm_components_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_gmm_components_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_add(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_gmm_component_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.change.write.component_index, parsed->positionals[next++], "component_index");
        read_token(req.change.write.description, parsed->positionals[next++], "description");
        read_token(req.change.write.mean, parsed->positionals[next++], "mean");
        read_token(req.change.write.stdev, parsed->positionals[next++], "stdev");
    ...
    auto result = do_auth_request<messaging::put_gmm_component_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_set(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
...
void gmm_component_commands::process_put_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_gmm_components_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            ...
            read_token(
                change.write.component_index, parsed->positionals[next++], "component_index");
            read_token(change.write.description, parsed->positionals[next++], "description");
            read_token(change.write.mean, parsed->positionals[next++], "mean");
            read_token(change.write.stdev, parsed->positionals[next++], "stdev");
    ...
    auto result = do_auth_request<messaging::put_many_gmm_components_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_delete(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_gmm_component_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_gmm_component_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_delete_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_gmm_components_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::gmm_component_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_gmm_components_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_versions(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_gmm_component_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_gmm_component_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void gmm_component_commands::process_version(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_gmm_component_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_gmm_component_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
