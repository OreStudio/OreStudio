...
void register_bond_future_history_provider(ores::history::service::dispatch_registry& registry) {
    registry.register_history_provider(
        "ores.trading.bond_future",
        [](const ores::database::context& scoped_ctx, const std::string& entity_id) {
            service::bond_future_service svc(scoped_ctx);
            auto versions = svc.get_future_history(entity_id);
            return ores::history::service::build_entity_history_versions(
                versions, presentation::render_bond_future_fields);
        });
}
