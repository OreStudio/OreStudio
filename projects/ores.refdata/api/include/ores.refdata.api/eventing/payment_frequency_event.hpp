 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_PAYMENT_FREQUENCY_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_PAYMENT_FREQUENCY_EVENT_HPP
...
/**
 * @brief Event traits for payment frequency's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::payment_frequency_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.payment_frequencies_events";

    static ores::refdata::messaging::payment_frequency_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::payment_frequency_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::payment_frequency_key>(notification.key))
            event.key = *key;
