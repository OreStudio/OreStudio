...
{{^list_filter_column}}
        if (auto req = decode<get_{{entity_plural}}_request>(msg)) {
            try {
                ...
                resp.total_available_count = svc.count_{{entity_plural_short}}();
                resp.success = true;
            } catch (const std::exception& e) {
                BOOST_LOG_SEV({{entity_singular}}_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
...
{{/system_tenant_validation}}
    ...
    void remove(ores::nats::message msg) {
        ...
        if (auto req = decode<delete_{{entity_singular}}_request>(msg)) {
            try {
{{#primary_key.is_text}}
                svc.delete_{{entity_plural_short}}(req->{{primary_key.column}}s);
{{/primary_key.is_text}}
{{#primary_key.is_uuid}}
                svc.delete_{{entity_plural_short}}(req->ids);
{{/primary_key.is_uuid}}
                BOOST_LOG_SEV({{entity_singular}}_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg,
