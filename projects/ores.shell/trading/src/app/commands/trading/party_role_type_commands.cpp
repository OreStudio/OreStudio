...
void party_role_type_commands::register_commands(cli::Menu& root_menu,
    ...
    party_role_types_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_party_role_type(std::ref(out), std::ref(session), args);
        },
        "Add party role type (code "
        "description "
        "change_reason_code "
        "\"change_commentary\")");
    ...
    party_role_types_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_party_role_type(std::ref(out), std::ref(session), std::move(id));
        },
        "Delete party role type by code");
    ...
    party_role_types_menu->Insert(
        ...
        "Show party role type's version history (--diff for a unified diff, --version "
        "<n> to pick one)",
        {"code [--diff] [--version <n>]"});
...
void party_role_type_commands::process_get_party_role_types(std::ostream& out,
                                                            nats_client& session,
                                                            pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get party role type request.";
...
void party_role_type_commands::process_add_party_role_type(std::ostream& out,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating add party role type request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add party role type." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 2 + 2;
    ...
    auto req = trading::messaging::save_party_role_type_request::from(std::move(v));
...
void party_role_type_commands::process_delete_party_role_type(std::ostream& out,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete party role type." << std::endl;
        return;
    }
