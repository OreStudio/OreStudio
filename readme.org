...
#+html: <a href="https://orestudio.github.io/OreStudio/doc/agile/v0/sprint_backlog_17.html"><img alt="Agile Sprint" src="https://img.shields.io/badge/Sprint-17-blue.svg"/></a>
...
#+html: <a href="https://github.com/OreStudio/OreStudio/commits/main"><img alt="Commits" src= "https://img.shields.io/github/commits-since/OreStudio/OreStudio/v0.0.16.svg"/></a>
...
the /service layer/ and the [[https://www.postgresql.org/][Postgres]] relational database. This simple
architecture has several implications:

- [[https://en.wikipedia.org/wiki/Business_logic][business logic]] will be divided between the service and database layers, as
...
We did start with a monolithic architecture first, but then it became too
complex. At any rate, our ultimate objective is to align ourselves with others
such as Schmidt, and heed his call for [[https://www.radicalsimpli.city/]["Radical Simplicity in Technology"]]:
...
Where =${VERSION}= is your ORE Studio version, such as =0.0.17=.
...
Linux (WSL)]] --- clone with submodules and install the build dependencies:
...
The install script pulls GCC, Clang, CMake, Ninja, PostgreSQL,
=libpq-dev=, the Qt6 development packages, Valgrind, and the X11/GL
headers --- all from the distro repositories. Nothing comes from
third-party APT sources.

Then configure and build with a CMake preset. The day-to-day default
on Linux is =linux-clang-debug-ninja=:
...
To run the test suite, target =rat=:
...
#+begin_src sh
cmake --build --preset linux-clang-debug-ninja --target run_ores_qt
#+end_src

A workflow preset chains configure + build in one go:

#+begin_src sh
cmake --workflow --preset linux-clang-release-ninja
#+end_src

If you bump into PostgreSQL errors during configure, set =ZIC=:

#+begin_src sh
ZIC=zic cmake --preset linux-clang-debug-ninja
#+end_src

** More commands

The above covers the quick start. For the full preset menu, the
output directory layout, parallel-build configuration, test logging
options, and the recipes for deploying the site and skills, see
[[id:5aae6900-8488-4e22-9f31-a1b9d5320efb][CMake setup]] and the recipes linked from the [[id:654d4a70-e63d-4c18-b5a5-886066e36314][CMake Runner skill]].
