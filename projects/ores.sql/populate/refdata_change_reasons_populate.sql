...
begin
    if not exists (
        ...
        insert into ores.refdata_change_reason_categories_tbl (
...
begin
    if not exists (
        select 1 from ores.refdata_change_reasons_tbl
        where code = p_code and valid_to = ores.utility_infinity_timestamp_fn()
    ) then
        insert into ores.refdata_change_reasons_tbl (
...
from ores.refdata_change_reasons_tbl where category_code = 'system' and valid_to = ores.utility_infinity_timestamp_fn()
union all
select 'Change Reasons (common)', count(*)
from ores.refdata_change_reasons_tbl where category_code = 'common' and valid_to = ores.utility_infinity_timestamp_fn()
union all
select 'Change Reasons (trade)', count(*)
from ores.refdata_change_reasons_tbl where category_code = 'trade' and valid_to = ores.utility_infinity_timestamp_fn()
order by entity;
