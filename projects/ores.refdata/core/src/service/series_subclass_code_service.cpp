...
read_one(repository::series_subclass_code_repository& repo,
         const ores::database::context& ctx,
         const messaging::series_subclass_code_key& key) {
 ...
 */
messaging::series_subclass_code_key key_from(const domain::series_subclass_code& v) {
    messaging::series_subclass_code_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::series_subclass_code to_domain(const messaging::series_subclass_code_write& write) {
    domain::series_subclass_code v;
    v.code = write.code;
    v.name = write.name;
...
messaging::get_series_subclass_code_response series_subclass_code_service::get_series_subclass_code(
    const messaging::get_series_subclass_code_request& request) {
    messaging::get_series_subclass_code_response response;
    ...
    response.series_subclass_code = std::move(found.front());
    return response;
}

messaging::get_many_series_subclass_codes_response
series_subclass_code_service::get_many_series_subclass_codes(
    const messaging::get_many_series_subclass_codes_request& request) {
    messaging::get_many_series_subclass_codes_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::series_subclass_code_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.series_subclass_code = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_series_subclass_code_response series_subclass_code_service::put_series_subclass_code(
    const messaging::put_series_subclass_code_request& request) {
    messaging::put_series_subclass_code_response response;
    domain::series_subclass_code value;
    ...
    if (!written.empty())
        response.series_subclass_code = std::move(written.front());
    return response;
}

messaging::put_many_series_subclass_codes_response
series_subclass_code_service::put_many_series_subclass_codes(
    const messaging::put_many_series_subclass_codes_request& request) {
    messaging::put_many_series_subclass_codes_response response;
    std::vector<domain::series_subclass_code> batch;
    ...
    response.series_subclasses.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.series_subclasses.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_series_subclass_code_response
series_subclass_code_service::delete_series_subclass_code(
    const messaging::delete_series_subclass_code_request& request) {
    messaging::delete_series_subclass_code_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::series_subclass_code_repository::remove_status::removed:
            break;
        case repository::series_subclass_code_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::series_subclass_code_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::series_subclass_code_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_series_subclass_codes_response
series_subclass_code_service::delete_many_series_subclass_codes(
    const messaging::delete_many_series_subclass_codes_request& request) {
    messaging::delete_many_series_subclass_codes_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_series_subclass_code_versions_response
series_subclass_code_service::list_series_subclass_code_versions(
    const messaging::list_series_subclass_code_versions_request& request) {
    messaging::list_series_subclass_code_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_series_subclass_code_version_response
series_subclass_code_service::get_series_subclass_code_version(
    const messaging::get_series_subclass_code_version_request& request) {
    messaging::get_series_subclass_code_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.series_subclass_code.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
series_subclass_code_service::prepare_change(const messaging::series_subclass_code_change& change,
...
std::vector<domain::series_subclass_code>
series_subclass_code_service::get_series_subclasses(const std::vector<std::string>& codes) {
