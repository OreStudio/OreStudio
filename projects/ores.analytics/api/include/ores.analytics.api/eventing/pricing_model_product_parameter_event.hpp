 ...
 */
#ifndef ORES_ANALYTICS_API_EVENTING_PRICING_MODEL_PRODUCT_PARAMETER_EVENT_HPP
#define ORES_ANALYTICS_API_EVENTING_PRICING_MODEL_PRODUCT_PARAMETER_EVENT_HPP
...
/**
 * @brief Event traits for pricing model product parameter's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::analytics::messaging::pricing_model_product_parameter_event> {
    static constexpr std::string_view subject_prefix =
        "analytics.v1.pricing_model_product_parameters_events";

    static ores::analytics::messaging::pricing_model_product_parameter_event
    from_notification(const entity_event_notification& notification) {
        ores::analytics::messaging::pricing_model_product_parameter_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::analytics::messaging::pricing_model_product_parameter_key>(
                    notification.key))
            event.key = *key;
