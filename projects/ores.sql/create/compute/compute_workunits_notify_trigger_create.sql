...
create or replace function ores_compute_workunits_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_input_uri text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_input_uri := OLD.input_uri;
        ...
        changed_input_uri := NEW.input_uri;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('input_uri', changed_input_uri);
    ...
    perform pg_notify('ores_compute_workunits', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_compute_workunits_notify_trg
after insert or update or delete on ores_compute_workunits_tbl
for each row execute function ores_compute_workunits_notify_fn();
