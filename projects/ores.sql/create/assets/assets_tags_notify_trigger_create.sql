...
create or replace function ores_assets_tags_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    perform pg_notify('ores_assets_tags', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_assets_tags_notify_trg
after insert or update or delete on ores_assets_tags_tbl
for each row execute function ores_assets_tags_notify_fn();
