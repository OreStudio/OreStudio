...
void ConcurrencyPolicyMdiWindow::setupUi() {
...
void ConcurrencyPolicyMdiWindow::setupToolbar() {
    ...
    connect(reloadAction_, &QAction::triggered, this,
            &ConcurrencyPolicyMdiWindow::reload);
...
void ConcurrencyPolicyMdiWindow::setupTable() {
    model_ = new ClientConcurrencyPolicyModel(clientManager_, this);
...
void ConcurrencyPolicyMdiWindow::reload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading concurrency policies";
    clearStaleIndicator();
    emit statusChanged(tr("Loading concurrency policies..."));
    model_->refresh();
}
...
void ConcurrencyPolicyMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void ConcurrencyPolicyMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;

    auto sourceIndex = proxyModel_->mapToSource(index);
    if (auto* policy = model_->getPolicy(sourceIndex.row())) {
        emit showPolicyDetails(*policy);
    }
}

void ConcurrencyPolicyMdiWindow::updateActionStates() {
...
void ConcurrencyPolicyMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new concurrency policy requested";
    emit addNewRequested();
}

void ConcurrencyPolicyMdiWindow::editSelected() {
...
void ConcurrencyPolicyMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void ConcurrencyPolicyMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* policy = model_->getPolicy(sourceIndex.row())) {
            codes.push_back(policy->code);
        }
    }

    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid concurrency policies to delete";
        return;
    }
    ...
    QPointer<ConcurrencyPolicyMdiWindow> self = this;
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished,
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                BOOST_LOG_SEV(lg(), debug) << "Concurrency Policy deleted: " << code;
                success_count++;
