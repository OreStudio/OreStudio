...
ore_import_result OreImporter::execute(
    ...
    try {
        ...
        for (auto& item : plan.trades) {
            auto it = portfolio_uuid_remap.find(item.trade.parties.get().portfolio_id);
            if (it != portfolio_uuid_remap.end())
                item.trade.parties.get().portfolio_id = it->second;
        }
        ...
        for (auto& item : plan.trades) {
            auto it = book_uuid_remap.find(item.trade.parties.get().book_id);
            if (it != book_uuid_remap.end())
                item.trade.parties.get().book_id = it->second;
        }
        ...
        // Fill required trade fields that ORE XML may omit.
        {
            ...
            for (auto& item : plan.trades) {
                auto& t = item.trade;
                bool any = false;
                if (!t.lifecycle.get().execution_timestamp || t.lifecycle.get().execution_timestamp->empty()) {
                    t.lifecycle.get().execution_timestamp = ts_fallback;
                    any = true;
                }
                if (!t.lifecycle.get().effective_date || t.lifecycle.get().effective_date->empty()) {
                    t.lifecycle.get().effective_date = date_fallback;
                    any = true;
                }
                if (!t.lifecycle.get().termination_date || t.lifecycle.get().termination_date->empty()) {
                    t.lifecycle.get().termination_date = std::string(fallback_termination_date);
                    any = true;
                }
                if (any) ++filled;
            }
        ...
        for (int offset = 0; offset < total_trades; offset += trade_batch_size) {
            ...
            for (int i = offset; i < end; ++i) {
                ...
                t.audit.get().change_reason_code = std::string(reason::codes::external_data_import);
                ...
                if constexpr (std::is_same_v<T, std::monostate>) {
                    ...
                    const auto record_error = [&](const auto& resp) {
                        ...
                        BOOST_LOG_SEV(lg(), warn)
                            ...
                            << item.trade.identity.get().external_id << ": " << msg;
                        res.instrument_errors.push_back(
                            {item.trade.identity.get().external_id, std::string(msg)});
                    };
