...
int ClientWorkspaceModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientWorkspaceModel::data(
    ...
    if (row >= workspaces_.size())
        return {};

    const auto& workspace = workspaces_[row];
...
void ClientWorkspaceModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh workspace model: disconnected.";
...
void ClientWorkspaceModel::onWorkspacesLoaded() {
    is_fetching_ = false;
    ...
    const int new_count = static_cast<int>(result.workspaces.size());
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " workspaces newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " workspaces."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientWorkspaceModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientWorkspaceModel::onPulsingComplete() {
