:PROPERTIES:
:ID: 12B0D01D-AD33-4327-AE59-BC8B99B38D8F
:END:
#+title: ores.sql.non-temporal
#+description: Non-temporal SQL schema variant.
#+type: facet
...
This page is the literate source for the =ores.sql.non-temporal= facet of the
...
* The ores.sql.non-temporal facet
...
|-----------+-------------|
| [[id:EA706EC1-C7BF-4C0F-81CB-F04C1098DB58][ores.sql.non-temporal.non_temporal_create]] | Entity table without =valid_from= / =valid_to= for reference data that never changes in place; otherwise the domain-entity shape (primary key, natural keys, =domain_entity.sql.tablename=). =non-temporal-sql= profile; same create-path output. Merge candidate of the Phase-3 temporal unification. |
