...
int ClientRoundingTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug)
                    ...
                    << " rounding types, total available: " << result->total_available_count;
                return {.success = true,
...
void ClientRoundingTypeModel::onTypesLoaded() {
    is_fetching_ = false;
...
void ClientRoundingTypeModel::onPulsingComplete() {
