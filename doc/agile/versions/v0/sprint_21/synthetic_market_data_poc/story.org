...
| [[id:B8644955-8FF8-427C-8CDF-C0CD6B4EB064][SyntheticFxSpotChartWindow — live chart with backfill]] | DONE | 2026-06-27 | 2026-06-28 | MDI chart window in ores.qt.mktdata: subscribes via fx_spot_subscription, backfills from marketdata.v1.observations.list, appends live ticks to a QLineSeries. Step 6 of the FX spot PoC. |
| [[id:21306CD6-8B9D-4119-A735-2B7B527EDDC3][Feed configuration + market data control panel UI]] | BACKLOG | | | There is no user-facing way to configure or start/stop synthetic market data feeds. Today the only trigger is a raw NATS request to marketdata.v1.market_feed_configs.start/stop (no menu, dialog, shell, or CLI). Add a Qt UI to (a) configure a feed (GMM means/stdevs/weights, initial price, ticks/hour, ore_key) and (b) a market data control panel to start/stop/monitor running feeds. |
| [[id:61AC6AB0-869B-469A-8CD8-9CB540F1BBAF][Candlestick (OHLC) trading-style view for the FX spot chart]] | DONE | | 2026-06-28 | Make FxSpotChartWindow look like a real trading chart: switch from a mid-price line to candlesticks (QCandlestickSeries), aggregating the tick stream and backfilled observations into OHLC bars per interval, with a centered symbol watermark and an interval selector. |
| [[id:58DEC5DA-5B24-4298-9A29-5A9CC2DA7283][Named feed sources: persisted configs + observation provenance]] | BACKLOG | | | Replace the hardcoded observation source 'SYNTHETIC' with named sources (e.g. synthetic.gmm123) backed by persisted feed configurations, so every observation records which feed/config produced it. Backend/data-model work; the UI to edit these configs is a separate task. |
| [[id:A34E4046-2E97-478B-800C-70EAC8F1D526][Synthetic feed should write market data under a business tenant, not system]] | BACKLOG | | | The synthetic service bootstraps the EUR/USD series and persists observations under the system tenant (ffffffff-...), because the series is created at service startup using the service account's own (system) tenant. Market data should belong to a business tenant chosen as feed configuration; otherwise normal tenant logins cannot see it and provenance is wrong. |
