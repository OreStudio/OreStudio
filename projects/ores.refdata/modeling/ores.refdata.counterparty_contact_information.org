...
#+description: Codegen entity model for the refdata =counterparty_contact_information= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
:service_find_by_code_column:        contact_type
:service_find_by_code_parent_column: counterparty_id
:END:
...
| contact_type  | Type           | contactTypeCombo  | dynamic_combo | false  | true        |                       |             |                     |                        | refdata::domain::contact_type   |    | fetch_contact_types    | contactTypeWatcher   | code               | description            | display_order      | contact types   | ContactTypeCombo       |            |
