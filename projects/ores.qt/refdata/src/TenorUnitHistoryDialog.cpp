...
#include "ui_TenorUnitHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

TenorUnitHistoryDialog::TenorUnitHistoryDialog(const QString& code,
                                               ClientManager* clientManager,
                                               QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::TenorUnitHistoryDialog)
    , code_(code)
...
TenorUnitHistoryDialog::~TenorUnitHistoryDialog() {
    delete ui_;
}

QString TenorUnitHistoryDialog::code() const {
    return code_;
}

void TenorUnitHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for tenor unit: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_tenor_unit_history_request request;
    request.code = code_.toStdString();

    QPointer<TenorUnitHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_tenor_unit_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int TenorUnitHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString TenorUnitHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow TenorUnitHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult TenorUnitHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void TenorUnitHistoryDialog::displayFullDetails(int index) {
...
void TenorUnitHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void TenorUnitHistoryDialog::revertToVersionAt(int index) {
