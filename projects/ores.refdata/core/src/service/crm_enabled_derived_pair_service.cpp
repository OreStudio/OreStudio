...
read_one(repository::crm_enabled_derived_pair_repository& repo,
         const ores::database::context& ctx,
         const messaging::crm_enabled_derived_pair_key& key) {
 ...
 */
messaging::crm_enabled_derived_pair_key key_from(const domain::crm_enabled_derived_pair& v) {
    messaging::crm_enabled_derived_pair_key key;
    key.id = v.id;
    return key;
}
...
crm_enabled_derived_pair_service::list_crm_enabled_derived_pairs(
    ...
    messaging::list_crm_enabled_derived_pairs_response response;
    ...
    response.crm_enabled_derived_pairs = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_crm_enabled_derived_pair_count(ctx_);
    return response;
}

messaging::get_crm_enabled_derived_pair_response
crm_enabled_derived_pair_service::get_crm_enabled_derived_pair(
    const messaging::get_crm_enabled_derived_pair_request& request) {
    messaging::get_crm_enabled_derived_pair_response response;
    ...
    response.crm_enabled_derived_pair = std::move(found.front());
    return response;
}

messaging::get_many_crm_enabled_derived_pairs_response
crm_enabled_derived_pair_service::get_many_crm_enabled_derived_pairs(
    const messaging::get_many_crm_enabled_derived_pairs_request& request) {
    messaging::get_many_crm_enabled_derived_pairs_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::crm_enabled_derived_pair_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.crm_enabled_derived_pair = std::move(found.front());
...
crm_enabled_derived_pair_service::put_crm_enabled_derived_pair(
    const messaging::put_crm_enabled_derived_pair_request& request) {
    messaging::put_crm_enabled_derived_pair_response response;
    domain::crm_enabled_derived_pair value;
    ...
    if (!written.empty())
        response.crm_enabled_derived_pair = std::move(written.front());
    return response;
}

messaging::put_many_crm_enabled_derived_pairs_response
crm_enabled_derived_pair_service::put_many_crm_enabled_derived_pairs(
    const messaging::put_many_crm_enabled_derived_pairs_request& request) {
    messaging::put_many_crm_enabled_derived_pairs_response response;
    ...
    response.crm_enabled_derived_pairs.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.crm_enabled_derived_pairs.push_back(written.empty() ? value :
                                                                       std::move(written.front()));
    }
    return response;
}

messaging::delete_crm_enabled_derived_pair_response
crm_enabled_derived_pair_service::delete_crm_enabled_derived_pair(
    const messaging::delete_crm_enabled_derived_pair_request& request) {
    messaging::delete_crm_enabled_derived_pair_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::crm_enabled_derived_pair_repository::remove_status::removed:
            break;
        case repository::crm_enabled_derived_pair_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::crm_enabled_derived_pair_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::crm_enabled_derived_pair_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_crm_enabled_derived_pairs_response
crm_enabled_derived_pair_service::delete_many_crm_enabled_derived_pairs(
    const messaging::delete_many_crm_enabled_derived_pairs_request& request) {
    messaging::delete_many_crm_enabled_derived_pairs_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
crm_enabled_derived_pair_service::list_crm_enabled_derived_pair_versions(
    const messaging::list_crm_enabled_derived_pair_versions_request& request) {
    messaging::list_crm_enabled_derived_pair_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_crm_enabled_derived_pair_version_response
crm_enabled_derived_pair_service::get_crm_enabled_derived_pair_version(
    const messaging::get_crm_enabled_derived_pair_version_request& request) {
    messaging::get_crm_enabled_derived_pair_version_response response;
    auto found =
        repo_.read_at_version(ctx_,
                              boost::uuids::to_string(request.key.crm_enabled_derived_pair.id),
                              request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result crm_enabled_derived_pair_service::prepare_change(
    const messaging::crm_enabled_derived_pair_change& change,
...
std::vector<domain::crm_enabled_derived_pair>
crm_enabled_derived_pair_service::get_crm_enabled_derived_pairs(
