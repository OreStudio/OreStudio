...
generate_synthetic_equity_variance_swap_instrument(utility::generation::generation_context& ctx) {
    ...
    r.identity.trade_type_code = std::string("EquityVarianceSwap");
...
generate_synthetic_equity_variance_swap_instruments(std::size_t n,
    ...
    std::vector<domain::equity_variance_swap_instrument> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_equity_variance_swap_instrument(ctx));
    return r;
}
