...
void print_domain_help(std::ostream& info) {
    ...
    info << "  " << std::setfill(' ') << std::left << std::setw(command_width) << roles_command_name
         << roles_command_desc << std::endl;
    info << "  " << std::setfill(' ') << std::left << std::setw(command_width)
         << permissions_command_name << permissions_command_desc << std::endl;
    info << "  " << std::setfill(' ') << std::left << std::setw(command_width)
         << login_info_command_name << login_info_command_desc << std::endl;

    info << std::endl << "Use: ores.cli iam <entity> <operation> --help for details." << std::endl;
}

void validate_entity_name(const std::string& entity_name) {
    const bool is_valid(entity_name == accounts_command_name || entity_name == roles_command_name ||
                        entity_name == permissions_command_name ||
                        entity_name == login_info_command_name);
...
handle_iam_command(bool has_help, const parsed_options& po, std::ostream& info, variables_map& vm) {
