 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_ASSET_CLASS_CODE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_ASSET_CLASS_CODE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes asset class codes to data storage.
 */
class ORES_REFDATA_CORE_EXPORT asset_class_code_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.asset_class_code_repository";
...
public:
    ...
    /**
     * @brief Writes asset class codes to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::asset_class_code& v);
    void write(context ctx, const std::vector<domain::asset_class_code>& v);
    /**@}*/

    /**
     * @brief Writes a asset class code, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of asset class codes, each honouring its own
    ...
    /**
     * @brief Reads latest asset class codes, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::asset_class_code> read_latest(context ctx);
    std::vector<domain::asset_class_code> read_latest(context ctx, const std::string& code);
    std::vector<domain::asset_class_code> read_latest(context ctx,
    ...
    /**
     * @brief Reads all asset class codes, possibly filtered by primary key.
     */
    std::vector<domain::asset_class_code> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single asset class code as it stood at a specific
    ...
    /**
     * @brief Reads latest asset class codes with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_asset_class_count(context ctx);

    /**
     * @brief Deletes a asset class code by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a asset class code, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes asset class codes by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::asset_class_code apply_claim(context ctx,
                                         const domain::asset_class_code& v,
                                         const ores::utility::domain::precondition& claim);
};
