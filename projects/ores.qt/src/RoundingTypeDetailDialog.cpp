...
void RoundingTypeDetailDialog::setClientManager(ClientManager* clientManager) {
    clientManager_ = clientManager;
}

void RoundingTypeDetailDialog::setUsername(const std::string& username) {
    username_ = username;
}
...
void RoundingTypeDetailDialog::setCreateMode(bool createMode) {
...
void RoundingTypeDetailDialog::setReadOnly(bool readOnly) {
    readOnly_ = readOnly;
...
void RoundingTypeDetailDialog::onCodeChanged(const QString& /* text */) {
    hasChanges_ = true;
    updateSaveButtonState();
}

void RoundingTypeDetailDialog::onFieldChanged() {
    hasChanges_ = true;
    updateSaveButtonState();
}

void RoundingTypeDetailDialog::updateSaveButtonState() {
...
void RoundingTypeDetailDialog::onSaveClicked() {
    ...
    BOOST_LOG_SEV(lg(), info) << "Saving rounding type: " << type_.code;

    QPointer<RoundingTypeDetailDialog> self = this;
    ...
    auto task = [self, type = type_]() -> SaveResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::save_rounding_type_request,
            0, std::move(payload)
        );
        ...
        auto response = refdata::messaging::save_rounding_type_response::
    ...
    connect(watcher, &QFutureWatcher<SaveResult>::finished,
        ...
        if (result.success) {
            BOOST_LOG_SEV(lg(), info) << "Rounding Type saved successfully";
            QString code = QString::fromStdString(self->type_.code);
            emit self->typeSaved(code);
            self->notifySaveSuccess(tr("Rounding Type '%1' saved").arg(code));
        } else {
...
void RoundingTypeDetailDialog::onDeleteClicked() {
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished,
        ...
        if (result.success) {
            BOOST_LOG_SEV(lg(), info) << "Rounding Type deleted successfully";
            emit self->statusMessage(QString("Rounding Type '%1' deleted").arg(code));
