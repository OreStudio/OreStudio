...
options_description make_add_compute_app_options_description() {
    options_description r("Add Compute App Options");
    r.add_options()("name", value<std::string>(), "Application name, e.g. 'ORE_STUDIO' (required)")(
        "description", value<std::string>()->default_value(""), "Human-readable description")(
        "modified-by", value<std::string>(), "Username of modifier (required)");

    return r;
}

ores::cli::config::add_compute_app_options read_add_compute_app_options(const variables_map& vm) {
    ores::cli::config::add_compute_app_options r;
    ...
    if (vm.count("name") == 0) {
        BOOST_THROW_EXCEPTION(parser_exception("Must supply --name for add app command."));
    }
...
std::optional<options> handle_compute_apps_command(bool has_help,
    ...
    if (has_help && o.empty()) {
        const std::vector<std::pair<std::string, std::string>> operations = {
            {"list", "List compute apps as JSON or table"}, {"add", "Add a new compute app"}};
        print_entity_help("apps", "Manage compute apps", operations, info);
        return {};
    }

    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception("apps command requires an operation (list, add)"));
    }

    const auto operation = o.front();
    o.erase(o.begin());

    validate_operation("apps", operation, allowed_operations);
    ...
    if (operation == list_command_name) {
        ...
        if (has_help) {
            print_help_command("apps list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::compute_apps);
    } else if (operation == add_command_name) {
        auto d = add_common_options(make_add_compute_app_options_description());
        if (has_help) {
            print_help_command("apps add", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.adding = read_add_compute_app_options(vm);
    }
