...
#include "ores.refdata.api/eventing/ois_convention_event.hpp"
...
TEST_CASE("write_ois_convention_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::ois_convention_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_ois_conventions");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for ois_convention " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::ois_convention_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_ois_convention_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_ois_convention(id_str);
        ...
        REQUIRE_FALSE(svc.get_ois_convention(id_str).has_value());
        REQUIRE(svc.get_ois_convention_history(id_str).size() == versions.size());
    }
}
