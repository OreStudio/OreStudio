...
// notification" pattern end to end for diary_entry_type -- the
...
using ores::refdata::domain::diary_entry_type;
using ores::refdata::repository::diary_entry_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_diary_entry_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::diary_entry_type_changed_event>(
        [&nats](const ores::refdata::eventing::diary_entry_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::diary_entry_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.diary_entry_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::diary_entry_type_changed_event>(
        "ores.refdata.diary_entry_type", "ores_refdata_diary_entry_types");
    ...
    auto v = generate_synthetic_diary_entry_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Diary Entry Type: " << v;

    diary_entry_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.diary_entry_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for diary_entry_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for diary_entry_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_entry_type(id_str).has_value());
        REQUIRE(svc.get_entry_type_history(id_str).size() == versions.size());
    }
}
