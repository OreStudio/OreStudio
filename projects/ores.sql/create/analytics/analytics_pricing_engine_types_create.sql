...
create table if not exists "ores_analytics_pricing_engine_types_tbl" (
    "code" text not null,
    "tenant_id" uuid not null,
    "version" integer not null,
    "description" text null,
    "instrument_type_code" text null,
...
create or replace function ores_analytics_pricing_engine_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_analytics_pricing_engine_types_tbl"
...
create or replace trigger ores_analytics_pricing_engine_types_insert_trg
before insert on "ores_analytics_pricing_engine_types_tbl"
for each row
execute function ores_analytics_pricing_engine_types_insert_fn();

create or replace rule ores_analytics_pricing_engine_types_delete_rule as
on delete to "ores_analytics_pricing_engine_types_tbl"
do instead
  update "ores_analytics_pricing_engine_types_tbl"
  set valid_to = current_timestamp
...
-- =============================================================================
-- Validation function for pricing_engine_type
-- Validates that a code exists in the pricing_engine_types table.
-- Returns the validated value, or default if null/empty.
-- Uses system tenant data (shared reference data).
-- =============================================================================
create or replace function ores_analytics_validate_pricing_engine_type_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid pricing_engine_type: value cannot be null or empty'
            using errcode = '23502';
    end if;

    -- Allow pass-through during bootstrap (empty table)
    if not exists (select 1 from ores_analytics_pricing_engine_types_tbl limit 1) then
        return p_value;
    end if;
