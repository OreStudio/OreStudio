...
portfolio_repository::replace_claim(context ctx, const domain::portfolio& v) {
...
domain::portfolio portfolio_repository::apply_claim(
    context ctx, const domain::portfolio& v, const ores::utility::domain::precondition& claim) {
...
void portfolio_repository::write(context ctx, const std::vector<domain::portfolio>& v) {
...
void portfolio_repository::write(context ctx,
                                 const domain::portfolio& v,
                                 const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing portfolio. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(ctx, portfolio_mapper::map(t), lg(), "Writing portfolio to database.");
}

void portfolio_repository::write(context ctx,
                                 const std::vector<domain::portfolio>& v,
    ...
    execute_write_query(ctx, portfolio_mapper::map(batch), lg(), "Writing portfolios to database.");
}
...
std::vector<domain::portfolio> portfolio_repository::read_all(context ctx, const std::string& id) {
    ...
    return execute_read_query<portfolio_entity, domain::portfolio>(
        ...
        [](const auto& entities) { return portfolio_mapper::map(entities); },
        lg(),
        "Reading all portfolio versions by id.");
}
...
portfolio_repository::remove_status portfolio_repository::remove(
...
portfolio_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
...
std::vector<domain::portfolio>
portfolio_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<portfolio_entity, domain::portfolio>(
        ...
        [](const auto& entities) { return portfolio_mapper::map(entities); },
        lg(),
        "Reading latest portfolios by ids.");
    return result;
}

void portfolio_repository::remove(context ctx, const std::vector<std::string>& ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<portfolio_entity> |
