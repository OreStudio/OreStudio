...
std::vector<std::byte> get_currency_market_tier_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_currency_market_tier_history_request, error_code>
get_currency_market_tier_history_request::deserialize(std::span<const std::byte> data) {
    get_currency_market_tier_history_request request;
...
std::vector<std::byte> get_currency_market_tier_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_currency_market_tier(buffer, v);
    }
    return buffer;
}

std::expected<get_currency_market_tier_history_response, error_code>
get_currency_market_tier_history_response::deserialize(std::span<const std::byte> data) {
