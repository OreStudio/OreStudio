:PROPERTIES:
:ID: CB7C1ECC-F47B-476C-9D72-888D1285F00B
:END:
#+title: ores.sql.populate.currency
#+description: ISO 4217 currencies joined to their countries (={{#country_currency}}=), with =monetary_nature= and lower-cased country code for flag linkage.
#+type: archetype
#+output: projects/ores.sql/populate/{dataset}/{prefix}_currency_populate.sql
...
ISO 4217 currencies joined to their countries (={{#country_currency}}=), with =monetary_nature= and lower-cased country code for flag linkage.
 ...
 *
 * Data Quality {{model_name}} Currencies Artefact Population Script
 *
 * Populates the dq_currencies_artefact_tbl with {{model_name}} currencies data.
...
begin
    ...
    -- Insert {{model_name}} currencies with flag image links
...
\echo ''
\echo '--- DQ {{model_name}} Currencies Summary ---'
...
select 'Total {{model_name}} Currencies' as metric, count(*) as count from currencies
union all
