...
Dependencies: =ores.compute= (domain + messaging), =ores.nats=, HTTP client
library (libcurl via vcpkg).
...
- What archive format for the package bundle? (tar.gz / zip)
- What HTTP client library? (libcurl already in vcpkg, or use a lighter wrapper)
- Heartbeat interval — should it be configurable or a fixed constant?
