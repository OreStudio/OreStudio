...
| PR  | Title                                                                              |
|-----+------------------------------------------------------------------------------------|
| 935 | Rename get_trade_detail → get_trade_instrument and rewrite client parse path       |
...
Protocol renamed (=get_trade_detail_*= → =get_trade_instrument_*=, NATS subject =trading.v1.trades.instrument=).
Sprint 18 workaround types deleted. =IInstrumentFormPopulator= interface created with 30 =populate()= overloads.
=ClientManagerTradeInstrument.cpp= rewrites dispatch with named wrappers in anonymous namespace — zero
=AddTagsToVariants=, zero =trade_instrument= constructions. GCC =-Wchanges-meaning= fixed by replacing
=using namespace ores::trading::domain= with =namespace td = ores::trading::domain=. PR #935 merged 2026-05-30.
