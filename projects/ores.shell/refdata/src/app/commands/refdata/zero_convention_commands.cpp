...
void zero_convention_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("zero_conventions");
...
void zero_convention_commands::process_list(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_zero_conventions_request;
    ...
    auto result = do_auth_request<messaging::list_zero_conventions_response>(
...
void zero_convention_commands::process_get(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_zero_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_zero_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_get_many(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_zero_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_zero_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_add(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_zero_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.tenor_based, parsed->positionals[next++], "tenor_based");
        read_token(
            req.change.write.day_count_fraction, parsed->positionals[next++], "day_count_fraction");
        read_token(req.change.write.compounding, parsed->positionals[next++], "compounding");
        read_token(req.change.write.compounding_frequency,
                   parsed->positionals[next++],
                   "compounding_frequency");
        read_token(req.change.write.tenor_calendar, parsed->positionals[next++], "tenor_calendar");
        read_token(req.change.write.spot_lag, parsed->positionals[next++], "spot_lag");
        read_token(req.change.write.spot_calendar, parsed->positionals[next++], "spot_calendar");
        read_token(
    ...
    auto result = do_auth_request<messaging::put_zero_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_set(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
...
void zero_convention_commands::process_put_many(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_zero_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::zero_convention_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.tenor_based, parsed->positionals[next++], "tenor_based");
            read_token(
                change.write.day_count_fraction, parsed->positionals[next++], "day_count_fraction");
            read_token(change.write.compounding, parsed->positionals[next++], "compounding");
            read_token(change.write.compounding_frequency,
                       parsed->positionals[next++],
                       "compounding_frequency");
            read_token(change.write.tenor_calendar, parsed->positionals[next++], "tenor_calendar");
            read_token(change.write.spot_lag, parsed->positionals[next++], "spot_lag");
            read_token(change.write.spot_calendar, parsed->positionals[next++], "spot_calendar");
            read_token(
    ...
    auto result = do_auth_request<messaging::put_many_zero_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_delete(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_zero_convention_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_zero_convention_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_delete_many(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_zero_conventions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::zero_convention_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_zero_conventions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_versions(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_zero_convention_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_zero_convention_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void zero_convention_commands::process_version(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_zero_convention_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.zero_convention.id, parsed->positionals[next++], "id");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_zero_convention_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
