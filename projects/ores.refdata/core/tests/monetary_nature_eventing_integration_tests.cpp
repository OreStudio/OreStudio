...
// notification" pattern end to end for monetary_nature -- the
...
using ores::refdata::domain::monetary_nature;
using ores::refdata::repository::monetary_nature_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_monetary_nature_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::monetary_nature_changed_event>(
        [&nats](const ores::refdata::eventing::monetary_nature_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::monetary_nature_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::monetary_nature_changed_event>(
        "ores.refdata.monetary_nature", "ores_refdata_monetary_natures");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::monetary_nature_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_monetary_nature(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Monetary Nature: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.monetary_nature") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for monetary_nature " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for monetary_nature " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::monetary_nature_service svc(crud_ctx);
