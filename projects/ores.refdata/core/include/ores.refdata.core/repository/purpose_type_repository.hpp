...
public:
    ...
    /**
     * @brief Writes a purpose type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of purpose types, each honouring its own
    ...
    /**
     * @brief Reads latest purpose types, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all purpose types, possibly filtered by primary key.
     */
    std::vector<domain::purpose_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single purpose type as it stood at a specific
    ...
    /**
     * @brief Removes a purpose type, refusing a row that moved on.
     *
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::purpose_type& v);
     ...
     */
    domain::purpose_type apply_claim(context ctx,
                                     const domain::purpose_type& v,
                                     const ores::utility::domain::precondition& claim);
};
