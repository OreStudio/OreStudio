...
- Introduce a step outcome tri-state (completed/completed_with_warnings/failed), per-trade step log entries in a JSON column, and service isolation improvements to the ORE import workflow.
- PR #748 merged and landed on main.
...
- *Test sample*: [[proj:external/ore/examples/Products/SupportedTrades/FX_Forward_Invalid.xml][=external/ore/examples/Products/SupportedTrades/FX_Forward_Invalid.xml=]] — a zero-amount FX Forward that passes XML parsing but fails the DB =bought_amount > 0= constraint, producing a =completed_with_warnings= step with a per-item log entry.
