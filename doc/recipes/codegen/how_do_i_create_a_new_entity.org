:PROPERTIES:
:ID: B6C5D4E3-F2A1-4B0C-8D9E-0F1A2B3C4D5E
:END:
#+title: How do I create a new entity?
#+description: Steps to add a complete domain entity across all layers using codegen-first approach.
#+type: recipe
#+level: cross
#+filetags: :entity:codegen:recipe:
#+created: 2026-05-21
#+updated: 2026-06-27

* Question

How do I add a new domain entity to ORE Studio?

* Answer

** 1. Read the standard first

Every entity must conform to [[id:608675E1-4284-4902-99AC-4DA565390AFB][entity lifecycle]] before writing any code.
That document specifies what each layer must contain and what the defaults
are. Partial entities or disabled defaults without written rationale are not
acceptable.

** 2. Create the codegen model

Scaffold the org model:

#+begin_src sh
./compass.sh add entity_org --component {component} --slug {entity} \
    --description "<one line>"
#+end_src

This writes =projects/ores.<component>/modeling/ores.<component>.<entity>.org=
with =#+type: ores.codegen.entity=. Fill in the columns, the profile flags,
and the knobs the [[id:BDE309BA-71DA-4A52-AA10-00F17AFDA476][meta-model]] requires.

If any of =has_history=, =has_pagination=, or =detail_fields= is absent
or disabled, add a =deviation_rationale_{dimension}= key with a one-sentence
justification before proceeding.

** 3. Generate and integrate each layer

#+begin_src sh
# SQL (table DDL, trigger, RLS, artefact table)
./compass.sh codegen entity generate <entity> --address ores.sql.schema

# C++ API and core (domain type, repository, service, handler)
./compass.sh codegen entity generate <entity> --address ores.cpp

# TypeScript UI metadata (table columns, detail fields, entity meta)
./compass.sh codegen entity generate <entity> --address ores.ts.ui
#+end_src

Follow the [[id:0CBA2F23-7F5F-433D-921A-D9C056BD9CA8][compass-codegen-add-entity]] skill for CMakeLists integration,
UI metadata wiring, and PR checkpoints at each layer boundary.

** 4. Validate

#+begin_src sh
# SQL schema integrity
./projects/ores.sql/utility/validate_schemas.sh

# Component documentation
./projects/ores.codegen/validate_docs.sh
#+end_src

Ask the user to build the affected targets before raising each PR.

* Script

See [[id:0CBA2F23-7F5F-433D-921A-D9C056BD9CA8][compass-codegen-add-entity]] skill for the full phase-by-phase execution guide and
layer-specific skills for deep detail on each layer.

* Tested by

Build and test suite for the component via =compass build= (not raw
=cmake --build=): =./compass.sh build {component}.core.tests=.
