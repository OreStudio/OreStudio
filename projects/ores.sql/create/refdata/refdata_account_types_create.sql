...
create table if not exists "production"."refdata_account_types_tbl" (
    "code" text not null,
...
create unique index if not exists refdata_account_types_version_uniq_idx
on "production"."refdata_account_types_tbl" (code, coding_scheme_code, version)
where valid_to = public.utility_infinity_timestamp_fn();

create index if not exists refdata_account_types_coding_scheme_idx
on "production"."refdata_account_types_tbl" (coding_scheme_code)
where valid_to = public.utility_infinity_timestamp_fn();

create or replace function production.refdata_account_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "production"."refdata_account_types_tbl"
...
create or replace trigger refdata_account_types_insert_trg
before insert on "production"."refdata_account_types_tbl"
for each row
execute function production.refdata_account_types_insert_fn();

create or replace rule refdata_account_types_delete_rule as
on delete to "production"."refdata_account_types_tbl"
do instead
  update "production"."refdata_account_types_tbl"
  set valid_to = current_timestamp
  where code = old.code
