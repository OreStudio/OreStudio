...
/**
 * @brief Model for displaying app versions fetched from the server.
 *
 * This model extends QAbstractTableModel and fetches app version
 * data asynchronously using the ores.comms client.
 */
class ClientAppVersionModel final : public AbstractClientModel {
    Q_OBJECT
...
public:
    ...
    /**
     * @brief Get app version at the specified row.
     *
     * @param row The row index.
     * @return The app version, or nullptr if row is invalid.
     */
...
private:
    ...
    struct FetchResult {
        bool success;
        std::vector<compute::domain::app_version> app_versions;
        std::uint32_t total_available_count;
        QString error_message;
        QString error_details;
    };

    void fetch_app_versions(std::uint32_t offset, std::uint32_t limit);
