...
sandbox. compass self-heals =SSH_AUTH_SOCK= for the git operations it
wraps; for raw push/fetch export the socket from =~/.ssh/agent/=
first. =gh= uses its own token, not the SSH agent.
