...
| Now           | Paused after the refdata pilot exposed a deeper format problem. Tasks 1–4 done; refdata-cpp pilot WIP stashed. |
| Waiting on    | [[id:D85E2B30-8C64-46DF-819D-B50D2E70B73C][Codegen unified model — org-mode migration]]. The JSON model cannot cleanly express custom methods; we are migrating the format before continuing the drift work. |
| Next          | Resume the refdata-cpp pilot (and the eleven follow-on per-component drift tasks) once all components are on org-mode. |
...
| [[id:615B7214-F87B-49D3-866E-CDC198FF2A06][Org-mode codegen POC — party as unified literate model]] | BACKLOG | | | Prove single org file can drive both C++ and SQL codegen, with custom methods inline. Blocks the per-component drift tasks because custom methods need a first-class mechanism instead of "restore from HEAD". |
| [[id:C8A4D7F1-2E6B-4C3A-8F9D-5B1E3C8D2F7A][Apply safe drift to refdata-cpp (pilot)]] | BLOCKED | | | Pilot. Add template flags refdata needs; update refdata models; pull party_repository custom methods from the org-mode mechanism; build + test + zero-diff invariant. |
