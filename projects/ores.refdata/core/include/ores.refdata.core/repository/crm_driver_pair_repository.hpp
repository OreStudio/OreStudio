 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CRM_DRIVER_PAIR_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CRM_DRIVER_PAIR_REPOSITORY_HPP
...
/**
 * @brief Reads and writes CRM driver pairs to data storage.
 */
class ORES_REFDATA_CORE_EXPORT crm_driver_pair_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.crm_driver_pair_repository";
...
public:
    ...
    /**
     * @brief Writes CRM driver pairs to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::crm_driver_pair& v);
    void write(context ctx, const std::vector<domain::crm_driver_pair>& v);
    /**@}*/

    /**
     * @brief Writes a CRM driver pair, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::crm_driver_pair& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of CRM driver pairs, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::crm_driver_pair>& v,
               const std::vector<ores::utility::domain::precondition>& claims);

    /**
     * @brief Reads latest CRM driver pairs, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::crm_driver_pair> read_latest(context ctx);
     ...
     */
    std::vector<domain::crm_driver_pair> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single CRM driver pair as it stood at a specific
    ...
    /**
     * @brief Reads latest CRM driver pairs with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_crm_driver_pair_count(context ctx);

    /**
     * @brief Deletes a CRM driver pair by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a CRM driver pair, refusing a row that moved on.
     *
...
private:
     ...
     */
    domain::crm_driver_pair apply_claim(context ctx,
                                        const domain::crm_driver_pair& v,
                                        const ores::utility::domain::precondition& claim);
};
