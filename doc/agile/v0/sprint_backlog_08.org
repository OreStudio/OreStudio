...
#+CAPTION: Clock summary at [2026-01-09 Fri 01:17], for Friday, January 09, 2026.
|      | <75>                                |        |      |      |       |
| Tags | Headline                            | Time   |      |      |     % |
|------+-------------------------------------+--------+------+------+-------|
...
|      | Stories                             | 1:16   |      |      | 100.0 |
|      | Active                              |        | 1:16 |      | 100.0 |
| code | Assorted fixes to currencies dialog |        |      | 1:16 | 100.0 |
#+end:

#+begin: clocktable :maxlevel 3 :scope subtree :tags t :indent nil :emphasize nil :scope file :narrow 75 :formula %
#+TBLNAME: sprint_summary
#+CAPTION: Clock summary at [2026-01-09 Fri 01:17]
|       | <75>                                        |         |       |       |       |
| Tags  | Headline                                    | Time    |       |       |     % |
|-------+---------------------------------------------+---------+-------+-------+-------|
...
|       | Stories                                     | 54:52   |       |       | 100.0 |
|       | Active                                      |         | 54:52 |       | 100.0 |
| agile | Sprint and product backlog refinement       |         |       |  1:50 |   3.3 |
| code  | Rerun SQL scripts from scratch              |         |       |  7:52 |  14.3 |
| code  | Run qt client and validate all new features |         |       |  1:19 |   2.4 |
| code  | Refactor options and parser                 |         |       |  0:27 |   0.8 |
| code  | Check functionality in =ores.wt=            |         |       |  2:03 |   3.7 |
| code  | Rename =ores.shell= to =ores.comms.shell=   |         |       |  1:30 |   2.7 |
| code  | Review HTTP service                         |         |       |  7:24 |  13.5 |
| code  | Assorted fixes to currency dialog           |         |       |  4:19 |   7.9 |
| code  | Add support for offsets in pagination       |         |       |  1:46 |   3.2 |
| code  | Assorted fixes to feature flags dialog      |         |       |  2:52 |   5.2 |
| code  | Add country support                         |         |       |  6:31 |  11.9 |
| code  | Assorted fixes to currencies dialog         |         |       |  6:21 |  11.6 |
| code  | Review comms analyser work                  |         |       |  0:27 |   0.8 |
| code  | Review telemetry work                       |         |       | 10:11 |  18.6 |
#+end:
...
*** STARTED Assorted fixes to currencies dialog                        :code:
    :LOGBOOK:
...
tried switching it off on the main currency window twice and that never seems to
work. can we make all windows the same type? same as currency history / currency
details.


log telemetry still in code, need to remove it. we moved it to ores.log but
seems like we created a backwards compatibility hack.
...
would be nice to be able to "locally modify" entity and then press save for the
batch.

server should peridically house keep sessions. if not connected, just mark it as
orphan. also when connection is dropped mark session as finished.

the show latest version dialog should have < > etc buttons to allow one to go
back and forwards in version space.
...
And the reason should say if its amend and/or delete.
