...
void payment_frequency_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("payment_frequencies");
    ...
    menu->Insert(
        ...
        "add <code> <name> <description> <period_unit> <period_multiplier> <display_order> "
        "<reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <code> <name> <description> <period_unit> <period_multiplier> <display_order> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <code> <name> <description> <period_unit> <period_multiplier> "
...
void payment_frequency_commands::process_list(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_payment_frequencies_request;
    ...
    auto result = do_auth_request<messaging::list_payment_frequencies_response>(
...
void payment_frequency_commands::process_get(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_payment_frequency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_payment_frequency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_get_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_payment_frequencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::payment_frequency_key key;
    ...
    auto result = do_auth_request<messaging::get_many_payment_frequencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_add(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_payment_frequency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.period_unit, parsed->positionals[next++], "period_unit");
        read_token(
            req.change.write.period_multiplier, parsed->positionals[next++], "period_multiplier");
    ...
    auto result = do_auth_request<messaging::put_payment_frequency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_set(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
...
void payment_frequency_commands::process_put_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_payment_frequencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::payment_frequency_change change;
            ...
            read_token(change.write.period_unit, parsed->positionals[next++], "period_unit");
            read_token(
                change.write.period_multiplier, parsed->positionals[next++], "period_multiplier");
    ...
    auto result = do_auth_request<messaging::put_many_payment_frequencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_delete(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_payment_frequency_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_payment_frequency_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_delete_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_payment_frequencies_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::payment_frequency_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_payment_frequencies_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_versions(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_payment_frequency_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_payment_frequency_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void payment_frequency_commands::process_version(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_payment_frequency_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.payment_frequency.code, parsed->positionals[next++], "code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_payment_frequency_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
