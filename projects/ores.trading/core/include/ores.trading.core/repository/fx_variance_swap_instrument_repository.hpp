 ...
 *
 */
#ifndef ORES_TRADING_CORE_REPOSITORY_FX_VARIANCE_SWAP_INSTRUMENT_REPOSITORY_HPP
#define ORES_TRADING_CORE_REPOSITORY_FX_VARIANCE_SWAP_INSTRUMENT_REPOSITORY_HPP
 ...
 */
class ORES_TRADING_CORE_EXPORT fx_variance_swap_instrument_repository {
private:
...
public:
    ...
    /**
     * @brief Writes FX variance swap instruments to database.
     */
    /**@{*/
    ...
    /**
     * @brief Reads latest FX variance swap instruments, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all FX variance swap instruments, possibly filtered by primary key.
     */
    std::vector<domain::fx_variance_swap_instrument> read_all(context ctx,
                                                              const std::string& instrument_id);

    /**
     * @brief Reads a single FX variance swap instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest FX variance swap instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_fx_variance_swap_instrument_count(context ctx);

    /**
     * @brief Deletes a FX variance swap instrument by closing its temporal validity.
     */
    void remove(context ctx, const std::string& instrument_id);

    /**
     * @brief Deletes FX variance swap instruments by closing their temporal validity.
     */
