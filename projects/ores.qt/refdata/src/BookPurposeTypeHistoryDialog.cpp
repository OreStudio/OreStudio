...
#include "ui_BookPurposeTypeHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

BookPurposeTypeHistoryDialog::BookPurposeTypeHistoryDialog(const QString& code,
                                                           ClientManager* clientManager,
                                                           QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::BookPurposeTypeHistoryDialog)
    , code_(code)
...
BookPurposeTypeHistoryDialog::~BookPurposeTypeHistoryDialog() {
    delete ui_;
}

QString BookPurposeTypeHistoryDialog::code() const {
    return code_;
}

void BookPurposeTypeHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for book purpose type: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_book_purpose_type_history_request request;
    request.code = code_.toStdString();

    QPointer<BookPurposeTypeHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_book_purpose_type_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int BookPurposeTypeHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString BookPurposeTypeHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow BookPurposeTypeHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult BookPurposeTypeHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void BookPurposeTypeHistoryDialog::displayFullDetails(int index) {
...
void BookPurposeTypeHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void BookPurposeTypeHistoryDialog::revertToVersionAt(int index) {
