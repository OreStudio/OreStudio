...
void CurrencyMarketTierHistoryDialog::markAsStale() {
    loadHistory();
}

QString CurrencyMarketTierHistoryDialog::code() const {
    return code_;
}
...
void CurrencyMarketTierHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &CurrencyMarketTierHistoryDialog::onVersionSelected);
...
void CurrencyMarketTierHistoryDialog::loadHistory() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Loading history for currency market tier: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    QPointer<CurrencyMarketTierHistoryDialog> self = this;

    using HistoryResult = std::expected<refdata::messaging::get_currency_market_tier_history_response, std::string>;
