...
BusinessDayConventionTypeMdiWindow::BusinessDayConventionTypeMdiWindow(ClientManager* clientManager,
...
void BusinessDayConventionTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &BusinessDayConventionTypeMdiWindow::addNew);
    ...
    connect(
        editAction_, &QAction::triggered, this, &BusinessDayConventionTypeMdiWindow::editSelected);
...
void BusinessDayConventionTypeMdiWindow::setupConnections() {
    connect(model_,
            &ClientBusinessDayConventionTypeModel::dataLoaded,
            this,
            &BusinessDayConventionTypeMdiWindow::onDataLoaded);
    connect(model_,
            &ClientBusinessDayConventionTypeModel::loadError,
            this,
            &BusinessDayConventionTypeMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &BusinessDayConventionTypeMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &BusinessDayConventionTypeMdiWindow::onDoubleClicked);
...
void BusinessDayConventionTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete business day convention type while disconnected.");
        return;
    }
    ...
    if (codes.size() == 1) {
        confirmMessage =
            QString("Are you sure you want to delete business day convention type '%1'?")
                .arg(QString::fromStdString(codes.front()));
    } else {
        confirmMessage =
            QString("Are you sure you want to delete %1 business day convention types?")
                .arg(codes.size());
    }
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Business Day Convention Type deletion failed: " << code << " - "
                    << result.second;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              ...
                              QString("Successfully deleted %1 business day convention types")
                                  .arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "business day convention type" :
                                                        "business day convention types")
                              .arg(first_error);
