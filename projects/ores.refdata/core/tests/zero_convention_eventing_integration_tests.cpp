...
// notification" pattern end to end for zero_convention -- the
...
using ores::refdata::domain::zero_convention;
using ores::refdata::repository::zero_convention_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_zero_convention_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::zero_convention_changed_event>(
        [&nats](const ores::refdata::eventing::zero_convention_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::zero_convention_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::zero_convention_changed_event>(
        "ores.refdata.zero_convention", "ores_refdata_zero_conventions");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::zero_convention_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_zero_convention(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.id;
    BOOST_LOG_SEV(lg, debug) << "Zero Convention: " << v;

    zero_convention_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.zero_convention") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for zero_convention " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for zero_convention " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::zero_convention_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_zero_convention_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_zero_convention(id_str);
        ...
        REQUIRE_FALSE(svc.get_zero_convention(id_str).has_value());
        REQUIRE(svc.get_zero_convention_history(id_str).size() == versions.size());
    }
}
