...
void trade_service::save_trade(const domain::trade& v,
    ...
    if (v.identity.get().id.is_nil())
        throw std::invalid_argument("Trade id cannot be empty.");
    BOOST_LOG_SEV(lg(), debug) << "Saving trade: " << v.identity.get().id;
    auto t = v;
    stamp(t.identity.get(), ctx_);
    stamp(t.audit.get(), ctx_);
    const std::optional<boost::uuids::uuid> current =
        t.classification.get().status_id.is_nil() ? std::nullopt
                             : std::make_optional(t.classification.get().status_id);
    t.classification.get().status_id = trade_status_service::resolve_status(
        ctx_, t.classification.get().activity_type_code, current, transitions);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved trade: " << t.identity.get().id;
}

void trade_service::save_trades(const std::vector<domain::trade>& trades,
    const fsm_transition_map& transitions) {
    for (const auto& t : trades) {
        if (t.identity.get().id.is_nil())
            throw std::invalid_argument("Trade id cannot be empty.");
    }
