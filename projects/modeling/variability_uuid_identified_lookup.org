...
#+description: UUID surrogate key, tenant scope, standard presentation tier, no parent or workspace relationship.
#+type: profile
#+level: cross
...
dimension is the same: standard presentation tier (=has_change_reason_cache=,
...
natural key (=name=), no parent, and standard presentation affordances.
