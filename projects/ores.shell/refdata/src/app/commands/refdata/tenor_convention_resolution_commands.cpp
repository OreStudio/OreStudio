...
void tenor_convention_resolution_commands::register_commands(cli::Menu& root_menu,
                                                             nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("tenor_convention_resolutions");
    ...
    menu->Insert(
        ...
        "get <convention_code> <tenor_code>");
    ...
    menu->Insert(
        ...
        "by-convention-code <convention_code> [--offset <n>] [--limit <n>] [--order <field>] "
        "[--desc]");

    root_menu.Insert(std::move(menu));
}

void tenor_convention_resolution_commands::process_list(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_tenor_convention_resolutions_request;
    ...
    auto result = do_auth_request<messaging::list_tenor_convention_resolutions_response>(
...
void tenor_convention_resolution_commands::process_get(std::ostream& out,
                                                       nats_client& session,
                                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_tenor_convention_resolution_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.convention_code, parsed->positionals[next++], "convention_code");
        read_token(req.key.tenor_code, parsed->positionals[next++], "tenor_code");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_tenor_convention_resolution_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenor_convention_resolution_commands::process_get_many(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_tenor_convention_resolutions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            ...
            read_token(key.convention_code, parsed->positionals[i + 0], "convention_code");
            read_token(key.tenor_code, parsed->positionals[i + 1], "tenor_code");
...
void tenor_convention_resolution_commands::process_by_convention_code(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-convention-code request.";

    using request_type = messaging::list_by_convention_code_tenor_convention_resolutions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-convention-code." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.convention_code = ores::shell::app::from_token<std::string>(parsed->positionals[next++],
                                                                        "convention_code");
    ...
    auto result =
        do_auth_request<messaging::list_by_convention_code_tenor_convention_resolutions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
