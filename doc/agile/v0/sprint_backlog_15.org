...
*** BLOCKED CDM Phase 1: Rates instrument domain model                 :code:
CLOSED: [2026-03-26 Thu 20:00]
    :LOGBOOK:
...
*** STARTED CDM Phase 4: Credit instrument domain model                :code:
...
- [ ] SQL: =ores_trading_credit_instruments_tbl= + notify trigger + drop files
- [ ] SQL: Register in =trading_create.sql=, =drop_trading.sql=
- [ ] Domain: =credit_instrument= struct, JSON I/O, table I/O, protocol messages
- [ ] Repository: =credit_instrument= entity, mapper, repository
- [ ] Service: =credit_instrument_service=
- [ ] Server: messaging handler + registrar registration
- [ ] Qt UI: =ClientCreditInstrumentModel=, =CreditInstrumentMdiWindow=,
  =CreditInstrumentDetailDialog=, =CreditInstrumentHistoryDialog=,
  =CreditInstrumentController=, =MainWindow= integration
- [ ] Database: recreate to pick up new table

*** STARTED CDM Phase 3: Bond instrument domain model                  :code:
