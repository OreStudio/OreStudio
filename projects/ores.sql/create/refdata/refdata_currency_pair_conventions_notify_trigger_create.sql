...
create or replace function ores_refdata_currency_pair_conventions_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_pair_code text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_pair_code := OLD.pair_code;
        ...
        changed_pair_code := NEW.pair_code;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('pair_code', changed_pair_code);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.refdata.currency_pair_convention',
    ...
    perform pg_notify('ores_refdata_currency_pair_conventions', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_refdata_currency_pair_conventions_notify_trg
after insert or update or delete on ores_refdata_currency_pair_conventions_tbl
for each row execute function ores_refdata_currency_pair_conventions_notify_fn();
