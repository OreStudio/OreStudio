...
#+type: archetype
#+output: projects/ores.qt/{qt_component}/src/{EntityPascal}Controller.cpp
#+facet: ores.cpp.qt
#+level: cross
...
- =DF72DFCF-FD65-4FE5-98BA-CB8168CE0FDF= — end of the generated
  =wireDetailDialogCommon()= helper (the single place every detail
  window construction site -- add/edit/history-version/revert --
  wires a freshly-created =<Entity>DetailDialog=). Extra
  =connect(detailDialog, ...)= wiring, e.g. relaying a custom dialog
  signal declared via the dialog header seam
  (=EC19A4EE-363D-4D41-A33A-D5F85A139C80= in
  [[id:0264FF98-86B8-4C43-AFE5-DF93567C63D2][ores.cpp.qt.detail_dialog_header]]) up through the controller
  signal declared via the seam above.
...
{{/domain_entity.qt.needs_image_cache}}
{{#domain_entity.qt.wires_change_reason_cache}}
