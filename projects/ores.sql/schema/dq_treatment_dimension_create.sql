...
on "ores"."dq_treatment_dimensions_tbl" (code, version)
where valid_to = ores.utility_infinity_timestamp_fn();

create unique index if not exists dq_treatment_dimensions_code_uniq_idx
on "ores"."dq_treatment_dimensions_tbl" (code)
where valid_to = ores.utility_infinity_timestamp_fn();

create or replace function ores.dq_treatment_dimensions_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores"."dq_treatment_dimensions_tbl"
...
before insert on "ores"."dq_treatment_dimensions_tbl"
for each row execute function ores.dq_treatment_dimensions_insert_fn();

create or replace rule dq_treatment_dimensions_delete_rule as
on delete to "ores"."dq_treatment_dimensions_tbl" do instead
    update "ores"."dq_treatment_dimensions_tbl"
    set valid_to = current_timestamp
    where code = OLD.code
      and valid_to = ores.utility_infinity_timestamp_fn();
