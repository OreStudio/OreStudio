...
{{/domain_entity.validation_fn}}
{{/domain_entity.has_tenant_id}}
{{#domain_entity.sql.has_parent_id}}
...
begin
    ...
    if p_from_root then
        with recursive ancestors as (
            select t.id, t.{{domain_entity.sql.parent_id_column}} as parent_id
    ...
    with recursive descendants as (
        select t.id, t.{{domain_entity.sql.parent_id_column}} as parent_id,
               t.{{domain_entity.sql.hierarchy_name_field}}::text as name
        from "{{domain_entity.product}}_{{domain_entity.component}}_{{domain_entity.entity_plural}}_tbl" t
        where t.tenant_id = p_tenant_id
          and t.id = v_root_id
          and t.valid_to = ores_utility_infinity_timestamp_fn()
        ...
        select t.id, t.{{domain_entity.sql.parent_id_column}} as parent_id,
               t.{{domain_entity.sql.hierarchy_name_field}}::text as name
        from "{{domain_entity.product}}_{{domain_entity.component}}_{{domain_entity.entity_plural}}_tbl" t
        join descendants d on t.{{domain_entity.sql.parent_id_column}} = d.id
        where t.tenant_id = p_tenant_id
          and t.valid_to = ores_utility_infinity_timestamp_fn()
    )
