...
| 5 | =codegen-drift.yml= runs on pull requests for the paths this change touches, so the "CI runs no PR gate" premise is wrong here. | =.github/workflows/codegen-drift.yml= | Decline | Process finding, not a code defect. Out of scope for this change. The same two checks run locally instead. |
