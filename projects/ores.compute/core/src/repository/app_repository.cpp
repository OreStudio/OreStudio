...
app_repository::read_at_version(context ctx, const std::string& id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading compute app at version. " << "id: " << id
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    static const std::string sys(ores::database::service::tenant_context::system_tenant_id);
    const auto query = sqlgen::read<std::vector<app_entity>> |
    ...
    const auto entities = execute_read_query<app_entity, domain::app>(
        ctx,
        query,
        [](const auto& entities) { return app_mapper::map(entities); },
        lg(),
        "Reading compute app at version.");
...
app_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest compute apps with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    static const std::string sys(ores::database::service::tenant_context::system_tenant_id);
    ...
    return execute_read_query<app_entity, domain::app>(
        ...
        [](const auto& entities) { return app_mapper::map(entities); },
        lg(),
        "Reading latest compute apps with pagination.");
}

std::uint32_t app_repository::get_total_app_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active compute app count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    static const std::string sys(ores::database::service::tenant_context::system_tenant_id);
    const auto query =
        sqlgen::select_from<app_entity>(sqlgen::count().as<"count">()) |
        where(("tenant_id"_c == tid || "tenant_id"_c == sys) && "valid_to"_c == max.value()) |
...
void app_repository::remove(context ctx, const std::vector<std::string>& ids) {
    ...
    execute_delete_query(ctx, query, lg(), "Batch removing compute apps.");
}
