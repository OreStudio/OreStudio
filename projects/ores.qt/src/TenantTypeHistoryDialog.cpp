...
void TenantTypeHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &TenantTypeHistoryDialog::onVersionSelected);
...
void TenantTypeHistoryDialog::loadHistory() {
    ...
    auto task = [self, code = code_.toStdString()]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_tenant_type_history_request,
            0, std::move(payload)
        );
        ...
        auto response = iam::messaging::get_tenant_type_history_response::
