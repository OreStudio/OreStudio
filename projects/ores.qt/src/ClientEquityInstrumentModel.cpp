...
namespace {
    std::string equity_instrument_key_extractor(
        const trading::domain::equity_instrument& t) {
        return boost::uuids::to_string(t.id);
    }
}

ClientEquityInstrumentModel::ClientEquityInstrumentModel(
    ClientManager* clientManager, QObject* parent)
    : QAbstractTableModel(parent),
      clientManager_(clientManager),
      watcher_(new QFutureWatcher<FetchResult>(this)),
      recencyTracker_(equity_instrument_key_extractor),
      pulseManager_(new RecencyPulseManager(this)) {

    connect(watcher_, &QFutureWatcher<FetchResult>::finished,
            this, &ClientEquityInstrumentModel::onEquityInstrumentsLoaded);
    connect(pulseManager_, &RecencyPulseManager::pulse_state_changed,
            this, &ClientEquityInstrumentModel::onPulseStateChanged);
    connect(pulseManager_, &RecencyPulseManager::pulsing_complete,
            this, &ClientEquityInstrumentModel::onPulsingComplete);
}

int ClientEquityInstrumentModel::rowCount(const QModelIndex& parent) const {
    if (parent.isValid()) return 0;
    return static_cast<int>(instruments_.size());
}

int ClientEquityInstrumentModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid()) return 0;
    return ColumnCount;
}

QVariant ClientEquityInstrumentModel::data(
    ...
    if (role == Qt::DisplayRole) {
        ...
        case Underlying:
            return QString::fromStdString(inst.underlying_code);
        case Currency:
...
QVariant ClientEquityInstrumentModel::headerData(
    ...
    case Underlying:  return tr("Underlying");
...
void ClientEquityInstrumentModel::refresh() {
    ...
    fetch_equity_instruments(0, page_size_);
}

void ClientEquityInstrumentModel::load_page(std::uint32_t offset,
                                            std::uint32_t limit) {
    ...
    fetch_equity_instruments(offset, limit);
}

void ClientEquityInstrumentModel::fetch_equity_instruments(
    std::uint32_t offset, std::uint32_t limit) {
    is_fetching_ = true;
    QPointer<ClientEquityInstrumentModel> self = this;
        ...
        QtConcurrent::run([self, offset, limit]() -> FetchResult {
            return exception_helper::wrap_async_fetch<FetchResult>(
                [&]() -> FetchResult {
                    ...
                    trading::messaging::get_equity_instruments_request request;
                    ...
                    if (!result) {
                        return {.success = false, .instruments = {},
                                ...
                                .error_message = QString::fromStdString(
                                    "Failed to fetch equity instruments: " +
                                    result.error()),
                                .error_details = {}};
                    }
...
void ClientEquityInstrumentModel::onEquityInstrumentsLoaded() {
    is_fetching_ = false;
    const auto result = watcher_->result();

    if (!result.success) {
        BOOST_LOG_SEV(lg(), error) << "Failed to fetch equity instruments: "
...
void ClientEquityInstrumentModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
        page_size_ = 100;
    } else {
        page_size_ = size;
...
const trading::domain::equity_instrument*
ClientEquityInstrumentModel::getEquityInstrument(int row) const {
    const auto idx = static_cast<std::size_t>(row);
    if (idx >= instruments_.size()) return nullptr;
    return &instruments_[idx];
}

QVariant ClientEquityInstrumentModel::recency_foreground_color(
...
void ClientEquityInstrumentModel::onPulseStateChanged(bool /*isOn*/) {
    if (!instruments_.empty()) {
...
void ClientEquityInstrumentModel::onPulsingComplete() {
    recencyTracker_.clear();
}
