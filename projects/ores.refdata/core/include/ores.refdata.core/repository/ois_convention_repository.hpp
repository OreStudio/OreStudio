 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_OIS_CONVENTION_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_OIS_CONVENTION_REPOSITORY_HPP
 ...
 */
class ORES_REFDATA_CORE_EXPORT ois_convention_repository {
private:
...
public:
    ...
    /**
     * @brief Writes OIS conventions to database.
     *
    ...
    /**
     * @brief Writes a OIS convention, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of OIS conventions, each honouring its own
    ...
    /**
     * @brief Reads latest OIS conventions, possibly filtered by primary key.
     */
    /**@{*/
     ...
     */
    std::vector<domain::ois_convention> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single OIS convention as it stood at a specific
    ...
    /**
     * @brief Reads latest OIS conventions with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_ois_convention_count(context ctx);

    /**
     * @brief Deletes a OIS convention by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a OIS convention, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes OIS conventions by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::ois_convention& v);
     ...
     */
    domain::ois_convention apply_claim(context ctx,
                                       const domain::ois_convention& v,
                                       const ores::utility::domain::precondition& claim);
};
