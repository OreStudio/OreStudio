...
void counterparty_contact_information_commands::register_commands(cli::Menu& root_menu,
                                                                  nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("counterparty_contact_informations");
    ...
    menu->Insert(
        ...
        "set <id> <counterparty_id> <contact_type> <street_line_1> <street_line_2> <city> <state> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <counterparty_id> <contact_type> <street_line_1> "
        "<street_line_2> <city> <state> <country_code> <postal_code> <phone> <email> <web_page> "
        "<reason> <commentary>");
...
void counterparty_contact_information_commands::process_list(std::ostream& out,
                                                             nats_client& session,
                                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_counterparty_contact_informations_request;
    ...
    auto result = do_auth_request<messaging::list_counterparty_contact_informations_response>(
...
void counterparty_contact_information_commands::process_get(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_counterparty_contact_information_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_counterparty_contact_information_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_get_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_counterparty_contact_informations_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_counterparty_contact_informations_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_add(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_counterparty_contact_information_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.city, parsed->positionals[next++], "city");
        read_token(req.change.write.state, parsed->positionals[next++], "state");
        read_token(req.change.write.country_code, parsed->positionals[next++], "country_code");
        read_token(req.change.write.postal_code, parsed->positionals[next++], "postal_code");
    ...
    auto result = do_auth_request<messaging::put_counterparty_contact_information_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_set(std::ostream& out,
                                                            nats_client& session,
                                                            const std::vector<std::string>& args) {
...
void counterparty_contact_information_commands::process_put_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_counterparty_contact_informations_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::counterparty_contact_information_change change;
            ...
            read_token(change.write.city, parsed->positionals[next++], "city");
            read_token(change.write.state, parsed->positionals[next++], "state");
            read_token(change.write.country_code, parsed->positionals[next++], "country_code");
            read_token(change.write.postal_code, parsed->positionals[next++], "postal_code");
    ...
    auto result = do_auth_request<messaging::put_many_counterparty_contact_informations_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_delete(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_counterparty_contact_information_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_counterparty_contact_information_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_delete_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_counterparty_contact_informations_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result =
        do_auth_request<messaging::delete_many_counterparty_contact_informations_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_by_counterparty_id(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-counterparty-id request.";

    using request_type =
        messaging::list_by_counterparty_id_counterparty_contact_informations_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
...
void counterparty_contact_information_commands::process_versions(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_counterparty_contact_information_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result =
        do_auth_request<messaging::list_counterparty_contact_information_versions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void counterparty_contact_information_commands::process_version(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_counterparty_contact_information_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_counterparty_contact_information_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
