 ...
 */
#ifndef ORES_QT_BUSINESS_DAY_CONVENTION_TYPE_CONTROLLER_HPP
#define ORES_QT_BUSINESS_DAY_CONVENTION_TYPE_CONTROLLER_HPP
 ...
 *
 * Manages the lifecycle of business day convention type list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class BusinessDayConventionTypeController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.business_day_convention_type_controller";
...
private:
     ...
     * BusinessDayConventionTypeDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(BusinessDayConventionTypeDetailDialog* detailDialog);
    void showHistoryWindow(const QString& code);

    /**
     * @brief Fetches the full typed business day convention type history (the
     * existing per-entity refdata::messaging::get_business_day_convention_type_history_request/
     * refdata::messaging::get_business_day_convention_type_history_response, unrelated to the
     * generic history.v1.get subject) and hands it to @p callback on the UI thread. Used to resolve
     * HistoryDialog's generic (entity_id, version) signals back to a typed business day convention
     * type, since the generic dialog holds no typed domain data.
     */
    void fetchBusinessDayConventionTypeHistory(
