...
std::string bond_leg_rate_repository::sql() {
    return generate_create_table_sql<bond_leg_rate_entity>(lg());
}

void bond_leg_rate_repository::write(context ctx, const domain::bond_leg_rate& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing bond leg rate. " << "instrument_id: " << v.instrument_id
                               << " leg_role: " << v.leg_role << " leg_number: " << v.leg_number;
    execute_write_query(
        ctx, bond_leg_rate_mapper::map(v), lg(), "Writing bond leg rate to database.");
}

void bond_leg_rate_repository::write(context ctx, const std::vector<domain::bond_leg_rate>& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing bond leg rates. Count: " << v.size();
    execute_write_query(
        ctx, bond_leg_rate_mapper::map(v), lg(), "Writing bond leg rates to database.");
}

std::vector<domain::bond_leg_rate> bond_leg_rate_repository::read_latest(context ctx) {
...
bond_leg_rate_repository::read_at_version(context ctx,
    ...
    const auto entities = execute_read_query<bond_leg_rate_entity, domain::bond_leg_rate>(
        ...
        [](const auto& entities) { return bond_leg_rate_mapper::map(entities); },
        lg(),
        "Reading bond leg rate at version.");
...
void bond_leg_rate_repository::remove(context ctx,
    ...
    execute_delete_query(ctx, query, lg(), "Removing bond leg rate from database.");
}

std::vector<domain::bond_leg_rate>
bond_leg_rate_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest bond leg rates with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<bond_leg_rate_entity>> |
...
std::uint32_t bond_leg_rate_repository::get_total_bond_leg_rate_count(context ctx) {
