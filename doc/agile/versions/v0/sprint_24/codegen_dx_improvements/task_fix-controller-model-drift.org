:PROPERTIES:
:ID: 9DBE6EDE-FF8A-4769-8997-50AFD00797D8
:END:
#+title: Task: Fix controller entity model drift: update to latest format and regenerate
#+description: Following the same pattern as the refdata sweep (task fix-refdata-model-drift): regenerate every ores.controller domain_entity and junction model against current codegen templates, updating any model found using an outdated format/convention before regenerating, and commit the resulting drift-free tree. compass build and rat must pass after.
#+type: task
#+level: s1
...
Mirror [[id:A2EDDEDC-D5D3-469D-88C9-7CE00951CC72][Fix refdata entity model drift]] for ores.controller: regenerate every
ores.controller =domain_entity= and =junction= model (=compass codegen entity
...
- Every ores.controller =domain_entity= and =junction= model is regenerated via
...
- =compass build= and full =rat= pass after the regenerated files are
  committed.
- No entity outside controller is touched.
