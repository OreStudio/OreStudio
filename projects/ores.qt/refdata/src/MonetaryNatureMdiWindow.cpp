...
void MonetaryNatureMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* type = model_->getNature(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showNatureHistory for code: " << type->code;
...
void MonetaryNatureMdiWindow::deleteSelected() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " monetary natures";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 monetary natures?").arg(codes.size());
    }
