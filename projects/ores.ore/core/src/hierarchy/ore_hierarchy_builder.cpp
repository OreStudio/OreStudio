...
ore_hierarchy_builder::ore_hierarchy_builder(std::vector<std::filesystem::path> portfolio_files,
...
ore_hierarchy_builder::filtered_components(const std::filesystem::path& relative) const {
...
std::size_t ore_hierarchy_builder::find_or_add_portfolio(const std::string& name,
                                                         std::optional<std::size_t> parent,
                                                         std::vector<import_node>& nodes) const {
    // Search for an existing portfolio with the same name and parent
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        if (nodes[i].type == import_node::node_type::portfolio && nodes[i].name == name &&
...
std::vector<import_node> ore_hierarchy_builder::build() {
    BOOST_LOG_SEV(lg(), debug) << "Building hierarchy from " << portfolio_files_.size()
                               << " portfolio files";

    std::vector<import_node> nodes;

    for (const auto& file : portfolio_files_) {
        const auto relative = std::filesystem::relative(file, root_);
        const auto components = filtered_components(relative);

        if (components.empty()) {
            BOOST_LOG_SEV(lg(), warn) << "No usable path components for: " << file;
            continue;
        }
        ...
        const auto stem = std::filesystem::path(components.back()).stem().string();
    ...
    BOOST_LOG_SEV(lg(), info) << "Hierarchy built: " << nodes.size() << " nodes";
    return nodes;
}
