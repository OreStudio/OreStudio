...
create or replace function ores_analytics_pricing_model_product_parameters_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_parameter_name text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_parameter_name := OLD.parameter_name;
        ...
        changed_parameter_name := NEW.parameter_name;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('parameter_name', changed_parameter_name);
    ...
    perform pg_notify('ores_analytics_pricing_model_product_parameters', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_analytics_pricing_model_product_parameters_notify_trg
after insert or update or delete on ores_analytics_pricing_model_product_parameters_tbl
for each row execute function ores_analytics_pricing_model_product_parameters_notify_fn();
