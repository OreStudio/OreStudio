...
ContactTypeMdiWindow::ContactTypeMdiWindow(ClientManager* clientManager,
...
void ContactTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &ContactTypeMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &ContactTypeMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &ContactTypeMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &ContactTypeMdiWindow::viewHistorySelected);
}
...
void ContactTypeMdiWindow::setupTable() {
    ...
    delegate->set_badge_color_resolver(
        0, [cache = badgeCache_](const QString& value) -> badge_color_pair {
            ...
            auto* def = cache->resolve("contact_type", value.toStdString());
            if (!def) {
...
void ContactTypeMdiWindow::setupConnections() {
    ...
    connect(tableView_->selectionModel(),
            ...
            this,
            &ContactTypeMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &ContactTypeMdiWindow::onDoubleClicked);
...
void ContactTypeMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void ContactTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete contact type while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " contact types";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 contact types?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Contact Type", confirmMessage, QMessageBox::Yes | QMessageBox::No);
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Contact Type deletion failed: " << code << " - " << result.second;
                failure_count++;
