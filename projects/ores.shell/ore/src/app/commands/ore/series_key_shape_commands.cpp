 ...
 */
#include "ores.shell/app/commands/ore/series_key_shape_commands.hpp"
...
namespace messaging = ores::ore::messaging;
...
void series_key_shape_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("series_key_shapes");
    ...
    menu->Insert(
        ...
        "add <series_type> <qualifier_depth> <has_point_dimension> <default_point> <description> "
        "<reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <series_type> <qualifier_depth> <has_point_dimension> <default_point> <description> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <series_type> <qualifier_depth> <has_point_dimension> "
        "<default_point> <description> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <series_type> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <series_type> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <series_type> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <series_type> --version <n>");

    root_menu.Insert(std::move(menu));
}

void series_key_shape_commands::process_list(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_series_key_shapes_request;
    ...
    auto result = do_auth_request<messaging::list_series_key_shapes_response>(
...
void series_key_shape_commands::process_get(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_series_key_shape_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.series_type, parsed->positionals[next++], "series_type");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_series_key_shape_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_get_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_series_key_shapes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            ...
            read_token(key.series_type, parsed->positionals[i + 0], "series_type");
    ...
    auto result = do_auth_request<messaging::get_many_series_key_shapes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_add(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_series_key_shape_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.change.write.qualifier_depth, parsed->positionals[next++], "qualifier_depth");
        read_token(req.change.write.has_point_dimension,
                   parsed->positionals[next++],
                   "has_point_dimension");
        read_token(req.change.write.default_point, parsed->positionals[next++], "default_point");
    ...
    auto result = do_auth_request<messaging::put_series_key_shape_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_set(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
...
void series_key_shape_commands::process_put_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_series_key_shapes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::series_key_shape_change change;
            read_token(change.write.series_type, parsed->positionals[next++], "series_type");
            read_token(
                change.write.qualifier_depth, parsed->positionals[next++], "qualifier_depth");
            read_token(change.write.has_point_dimension,
                       parsed->positionals[next++],
                       "has_point_dimension");
            read_token(change.write.default_point, parsed->positionals[next++], "default_point");
    ...
    auto result = do_auth_request<messaging::put_many_series_key_shapes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_delete(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_series_key_shape_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.series_type, parsed->positionals[next++], "series_type");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_series_key_shape_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_delete_many(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_series_key_shapes_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::series_key_shape_key key;
            read_token(key.series_type, parsed->positionals[i * 1 + 0], "series_type");
            req.removals.push_back(messaging::series_key_shape_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_series_key_shapes_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_versions(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_series_key_shape_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_series_key_shape_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void series_key_shape_commands::process_version(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_series_key_shape_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.key.series_key_shape.series_type, parsed->positionals[next++], "series_type");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_series_key_shape_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
