...
create table if not exists ""."refdata_supervisory_bodies_tbl" (
    "code" text not null,
...
on ""."refdata_supervisory_bodies_tbl" (code, version)
...
on ""."refdata_supervisory_bodies_tbl" (coding_scheme_code)
where valid_to = public.utility_infinity_timestamp_fn();

create or replace function .refdata_supervisory_bodies_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from ""."refdata_supervisory_bodies_tbl"
    where code = new.code
    ...
    if found then
        ...
        update ""."refdata_supervisory_bodies_tbl"
...
before insert on ""."refdata_supervisory_bodies_tbl"
for each row
execute function .refdata_supervisory_bodies_insert_fn();

create or replace rule refdata_supervisory_bodies_delete_rule as
on delete to ""."refdata_supervisory_bodies_tbl"
do instead
  update ""."refdata_supervisory_bodies_tbl"
  set valid_to = current_timestamp
  where code = old.code
