 ...
 *
 * This model extends AbstractClientModel and fetches change reason category
 * data asynchronously using the ores.comms client.
 */
class ClientChangeReasonCategoryModel final : public AbstractClientModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.client_change_reason_category_model";
...
public:
     ...
     */
    enum Column { Code, Description, ModifiedBy, RecordedAt, ColumnCount };
    ...
    /**
     * @brief Refresh change reason category data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get change reason category at the specified row.
     *
     * @param row The row index.
     * @return The change reason category, or nullptr if row is invalid.
     */
...
private:
    ...
    void fetch_categories(std::uint32_t offset, std::uint32_t limit);
    ...
    using ChangeReasonCategoryKeyExtractor =
        std::string (*)(const dq::domain::change_reason_category&);
    RecencyTracker<dq::domain::change_reason_category, ChangeReasonCategoryKeyExtractor>
        recencyTracker_;
