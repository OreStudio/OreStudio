 ...
 *
 */
#ifndef ORES_UTILITY_STREAMING_JSONIFY_HPP
#define ORES_UTILITY_STREAMING_JSONIFY_HPP
...
public:
    explicit json_string_type(Target target) : target_(std::move(target)) { }
...
public:
    explicit json_complex_type(Target target) : target_(std::move(target)) { }
    const Target& get() const { return(target_); }
