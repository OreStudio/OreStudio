...
:END:
#+title: Task: Implement product backlog next/deferred listing in compass
#+description: Add compass backlog command to list next and deferred captures with counts and titles, mirroring the sprint in-flight display.
#+type: task
#+level: s1
...
This page documents a [[id:31C868B9-306E-4282-BA8C-07E647021B34][task]] in the [[id:15CE567F-489C-4A80-9CEC-27FCF95E5C36][ores.compass Product Backlog — next and deferred listing]] story. It captures the goal, current status, acceptance, and any notes or results.

* Goal

Add a =backlog= subcommand to =compass.py= that reads the next and deferred
...
causes generated files to land inside [[proj:projects/ores.compass][=projects/ores.compass/=]] instead of
the repo root.
...
|--------------+--------------------------------------------------------------------|
| State        | DONE                                                            |
| Parent story | [[id:15CE567F-489C-4A80-9CEC-27FCF95E5C36][ores.compass Product Backlog — next and deferred listing]] |
| Now          | Complete.                                    |
| Waiting on   | Nothing.                                                           |
| Next         | None.                                                              |
...
- =compass.sh backlog= prints next captures then deferred captures, each group
  headed by a count line (e.g. =Next (12):=).
- Each capture prints as one line: title and relative path.
- =compass.sh backlog --next= prints only the next bucket; =--deferred= only deferred.
- =compass.sh backlog --json= emits ={"next": [...], "deferred": [...]}= with
  the same fields as the =where --json= in-flight entries.
- =compass.sh backlog --count= prints only the counts (=next=12 deferred=7=),
