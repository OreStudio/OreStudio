...
create table if not exists "ores_compute_batches_tbl" (
    "id" uuid not null,
...
create or replace function ores_compute_batches_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_compute_batches_tbl"
...
create or replace trigger ores_compute_batches_insert_trg
before insert on "ores_compute_batches_tbl"
for each row execute function ores_compute_batches_insert_fn();

create or replace rule ores_compute_batches_delete_rule as
on delete to "ores_compute_batches_tbl" do instead
    update "ores_compute_batches_tbl"
    set valid_to = current_timestamp
