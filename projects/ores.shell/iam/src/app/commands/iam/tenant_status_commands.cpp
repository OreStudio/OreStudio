...
void tenant_status_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("tenant_statuses");
    ...
    menu->Insert(
        ...
        "add <status> <name> <description> <display_order> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <status> <name> <description> <display_order> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <status> <name> <description> <display_order> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <status> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <status> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <status> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <status> --version <n>");

    root_menu.Insert(std::move(menu));
}

void tenant_status_commands::process_list(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_tenant_statuses_request;
    ...
    auto result = do_auth_request<messaging::list_tenant_statuses_response>(
...
void tenant_status_commands::process_get(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_tenant_status_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.status, parsed->positionals[next++], "status");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_tenant_status_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_get_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_tenant_statuses_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            ...
            read_token(key.status, parsed->positionals[i + 0], "status");
    ...
    auto result = do_auth_request<messaging::get_many_tenant_statuses_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_add(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_tenant_status_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_tenant_status_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_set(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
...
void tenant_status_commands::process_put_many(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_tenant_statuses_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::tenant_status_change change;
    ...
    auto result = do_auth_request<messaging::put_many_tenant_statuses_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_delete(std::ostream& out,
                                            nats_client& session,
                                            const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_tenant_status_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.status, parsed->positionals[next++], "status");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_tenant_status_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_delete_many(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_tenant_statuses_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::tenant_status_key key;
            read_token(key.status, parsed->positionals[i * 1 + 0], "status");
            req.removals.push_back(messaging::tenant_status_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_tenant_statuses_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_versions(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_tenant_status_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_tenant_status_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void tenant_status_commands::process_version(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_tenant_status_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.tenant_status.status, parsed->positionals[next++], "status");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_tenant_status_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
