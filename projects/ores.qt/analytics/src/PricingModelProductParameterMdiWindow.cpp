...
void PricingModelProductParameterMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &PricingModelProductParameterMdiWindow::addNew);
...
void PricingModelProductParameterMdiWindow::setupConnections() {
    connect(model_,
            &ClientPricingModelProductParameterModel::dataLoaded,
            this,
            &PricingModelProductParameterMdiWindow::onDataLoaded);
    connect(model_,
            &ClientPricingModelProductParameterModel::loadError,
            this,
            &PricingModelProductParameterMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &PricingModelProductParameterMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &PricingModelProductParameterMdiWindow::onDoubleClicked);
...
void PricingModelProductParameterMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(
        tr("Loaded %1 of %2 pricing model product parameters").arg(loaded).arg(total));
...
void PricingModelProductParameterMdiWindow::viewHistorySelected() {
    ...
    if (auto* parameter = model_->getParameter(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug)
            << "Emitting showParameterHistory for code: " << parameter->parameter_name;
...
void PricingModelProductParameterMdiWindow::deleteSelected() {
    ...
    if (ids.size() == 1) {
        confirmMessage =
            QString("Are you sure you want to delete pricing model product parameter '%1'?")
                .arg(QString::fromStdString(codes.front()));
    } else {
        confirmMessage =
            QString("Are you sure you want to delete %1 pricing model product parameters?")
                .arg(ids.size());
    }
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Pricing Model Product Parameter deletion failed: " << code << " - "
                    << message;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              ...
                              QString("Successfully deleted %1 pricing model product parameters")
                                  .arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "pricing model product parameter" :
                                                        "pricing model product parameters")
                              .arg(first_error);
