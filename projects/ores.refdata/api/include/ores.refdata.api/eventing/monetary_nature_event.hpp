 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_MONETARY_NATURE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_MONETARY_NATURE_EVENT_HPP
...
/**
 * @brief Event traits for monetary nature's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::monetary_nature_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.monetary_natures_events";

    static ores::refdata::messaging::monetary_nature_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::monetary_nature_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::monetary_nature_key>(notification.key))
            event.key = *key;
