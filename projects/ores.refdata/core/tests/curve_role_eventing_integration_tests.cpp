...
#include "ores.refdata.api/generators/curve_role_generator.hpp"
...
// notification" pattern end to end for curve_role -- the
...
using ores::refdata::domain::curve_role;
using ores::refdata::repository::curve_role_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_curve_role_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::curve_role_changed_event>(
        [&nats](const ores::refdata::eventing::curve_role_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::curve_role_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::curve_role_changed_event>(
        "ores.refdata.curve_role", "ores_refdata_curve_roles");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::curve_role_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_curve_role(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Curve Role: " << v;

    curve_role_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.curve_role") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for curve_role " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for curve_role " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_role(id_str).has_value());
        REQUIRE(svc.get_role_history(id_str).size() == versions.size());
    }
}
