...
// Report Instance writes are party-scoped: the session-level
...
TEST_CASE("write_report_instance_publishes_nats_changed_event", tags) {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for report_instance " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::reporting::service::report_instance_service svc(crud_ctx);
        ...
        REQUIRE_FALSE(svc.get_instance(id_str).has_value());
