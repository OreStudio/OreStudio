...
render_badge_definition_fields(const domain::badge_definition& v) {
    ...
    fields.push_back({.name = "Background Colour", .value = v.background_colour});
    fields.push_back({.name = "Text Colour", .value = v.text_colour});
    fields.push_back({.name = "Severity Code", .value = v.severity_code});
    fields.push_back({.name = "Css Class", .value = v.css_class});
