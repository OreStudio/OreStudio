...
:END:
#+title: Deploy skills via compass build --direct skills, not manually
#+description: Never create .claude/skills/<name>/SKILL.md manually; always deploy via compass build --direct skills after updating the .org source.
#+type: memory
