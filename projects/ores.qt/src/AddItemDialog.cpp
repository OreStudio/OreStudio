...
void AddItemDialog::setupUI() {
    ...
    typeCombo_->setCurrentIndex(1); // Default to Connection
...
void AddItemDialog::setItemType(ItemType type) {
    itemType_ = type;
    typeCombo_->setCurrentIndex(type == ItemType::Folder ? 0 : 1);
    updateFieldVisibility();
    updateSaveButtonState();
}
...
void AddItemDialog::setCreateMode(bool createMode) {
    ...
    if (!createMode) {
        setWindowTitle(tr("Edit %1").arg(
            itemType_ == ItemType::Folder ? tr("Folder") : tr("Connection")));
    } else {
...
void AddItemDialog::updateFieldVisibility() {
    ...
    // Connection-only fields
    hostLabel_->setVisible(isConnection);
    hostEdit_->setVisible(isConnection);
    hostEdit_->setEnabled(isConnection);

    portLabel_->setVisible(isConnection);
    portSpinBox_->setVisible(isConnection);
    portSpinBox_->setEnabled(isConnection);
    ...
    tagLabel_->setVisible(isConnection);
    tagSelector_->setVisible(isConnection);
    tagSelector_->setEnabled(isConnection);
...
void AddItemDialog::updateSaveButtonState() {
    ...
    if (itemType_ == ItemType::Connection) {
        valid = valid &&
...
void AddItemDialog::setEnvironment(const connections::domain::server_environment& env) {
    isCreateMode_ = false;
...
connections::domain::server_environment AddItemDialog::getEnvironment() const {
...
void AddItemDialog::saveConnection() {
    ...
    try {
        ...
        if (isCreateMode_) {
            ...
            manager_->update_environment(env, password);
            BOOST_LOG_SEV(lg(), info) << "Updated connection: " << env.name;
        }
...
void AddItemDialog::onTestClicked() {
    ...
    if (!isCreateMode_ && password.isEmpty() && !passwordChanged_) {
        try {
            password = QString::fromStdString(manager_->get_password(environmentId_));
        } catch (const std::exception& e) {
