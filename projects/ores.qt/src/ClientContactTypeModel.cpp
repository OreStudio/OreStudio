...
int ClientContactTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientContactTypeModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh contact type model: disconnected.";
                ...
                if (!result) {
                    ...
                    return {.success = false, .types = {},
                            ...
                            .error_message = QString::fromStdString(
                                "Failed to fetch contact types: " + comms::net::to_string(result.error())),
                            .error_details = {}};
                }
...
void ClientContactTypeModel::onTypesLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " contact types newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " contact types."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientContactTypeModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientContactTypeModel::onPulsingComplete() {
