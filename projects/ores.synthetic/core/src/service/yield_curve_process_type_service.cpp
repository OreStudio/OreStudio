...
read_one(repository::yield_curve_process_type_repository& repo,
         const ores::database::context& ctx,
         const messaging::yield_curve_process_type_key& key) {
 ...
 */
messaging::yield_curve_process_type_key key_from(const domain::yield_curve_process_type& v) {
    messaging::yield_curve_process_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::yield_curve_process_type to_domain(const messaging::yield_curve_process_type_write& write) {
...
yield_curve_process_type_service::list_yield_curve_process_types(
    ...
    messaging::list_yield_curve_process_types_response response;
    ...
    response.process_types = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_process_type_count(ctx_);
    return response;
}

messaging::get_yield_curve_process_type_response
yield_curve_process_type_service::get_yield_curve_process_type(
    const messaging::get_yield_curve_process_type_request& request) {
    messaging::get_yield_curve_process_type_response response;
    ...
    response.yield_curve_process_type = std::move(found.front());
    return response;
}

messaging::get_many_yield_curve_process_types_response
yield_curve_process_type_service::get_many_yield_curve_process_types(
    const messaging::get_many_yield_curve_process_types_request& request) {
    messaging::get_many_yield_curve_process_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::yield_curve_process_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.yield_curve_process_type = std::move(found.front());
...
yield_curve_process_type_service::put_yield_curve_process_type(
    const messaging::put_yield_curve_process_type_request& request) {
    messaging::put_yield_curve_process_type_response response;
    domain::yield_curve_process_type value;
    ...
    if (!written.empty())
        response.yield_curve_process_type = std::move(written.front());
    return response;
}

messaging::put_many_yield_curve_process_types_response
yield_curve_process_type_service::put_many_yield_curve_process_types(
    const messaging::put_many_yield_curve_process_types_request& request) {
    messaging::put_many_yield_curve_process_types_response response;
    std::vector<domain::yield_curve_process_type> batch;
    ...
    response.process_types.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.process_types.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_yield_curve_process_type_response
yield_curve_process_type_service::delete_yield_curve_process_type(
    const messaging::delete_yield_curve_process_type_request& request) {
    messaging::delete_yield_curve_process_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::yield_curve_process_type_repository::remove_status::removed:
            break;
        case repository::yield_curve_process_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::yield_curve_process_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::yield_curve_process_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_yield_curve_process_types_response
yield_curve_process_type_service::delete_many_yield_curve_process_types(
    const messaging::delete_many_yield_curve_process_types_request& request) {
    messaging::delete_many_yield_curve_process_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
yield_curve_process_type_service::list_yield_curve_process_type_versions(
    const messaging::list_yield_curve_process_type_versions_request& request) {
    messaging::list_yield_curve_process_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_yield_curve_process_type_version_response
yield_curve_process_type_service::get_yield_curve_process_type_version(
    const messaging::get_yield_curve_process_type_version_request& request) {
    messaging::get_yield_curve_process_type_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.yield_curve_process_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result yield_curve_process_type_service::prepare_change(
    const messaging::yield_curve_process_type_change& change,
...
std::vector<domain::yield_curve_process_type>
yield_curve_process_type_service::get_process_types(const std::vector<std::string>& codes) {
