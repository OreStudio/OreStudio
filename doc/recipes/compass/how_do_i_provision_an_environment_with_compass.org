...
2. Scans sibling =ores_dev_*= and legacy =OreStudio.*= directories for used
   ...
   base port — http (+0..+1), site (+4), NATS client (+5), NATS monitor
...
end of the configure step.  The package set does not depend on the
environment type written by =env provision=: both =full= and =light= get the
baseline set (X11/GL headers, NATS, etc.).  Add =--full-install= when you
also want the complete developer toolchain.
