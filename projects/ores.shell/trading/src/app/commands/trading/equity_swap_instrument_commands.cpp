...
#include "ores.trading.api/domain/equity_swap_instrument_table_io.hpp" // IWYU pragma: keep.
...
void equity_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_equity_swap_instrument(std::ref(out), std::ref(session), args);
        },
        "Add equity swap instrument (trade_type_code "
        "party_id "
        "trade_id "
        "underlying_name "
        "basket_json "
    ...
    equity_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_equity_swap_instrument(std::ref(out), std::ref(session), std::move(id));
        },
        "Delete equity swap instrument by instrument_id");
    ...
    equity_swap_instruments_menu->Insert(
        ...
        "Show equity swap instrument's version history");

    root_menu.Insert(std::move(equity_swap_instruments_menu));
}

void equity_swap_instrument_commands::process_get_equity_swap_instruments(
    std::ostream& out, nats_client& session, pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get equity swap instrument request.";

    auto& state = pagination.state_for("equity_swap_instruments");

    trading::messaging::get_equity_swap_instruments_request req;
    req.offset = state.current_offset;
    req.limit = pagination.page_size();

    auto result = do_auth_request<trading::messaging::get_equity_swap_instruments_response>(
        out, session, "trading.v1.equity_swap_instruments.list", req);
    if (!result)
        return;
    ...
    out << "\nPage " << page << " of " << total_pages << " ("
        << result->equity_swap_instruments.size() << " of " << state.total_count << " total)"
        << std::endl;
}

void equity_swap_instrument_commands::process_add_equity_swap_instrument(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add equity swap instrument request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add equity swap instrument." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 13 + 2;
    ...
    try {
        ...
        v.underlying_name = ores::shell::app::from_token<std::string>(parsed->positionals[next++],
                                                                      "underlying_name");
        v.basket_json =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "basket_json");
        v.currency =
        ...
        v.return_type =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "return_type");
        v.start_date =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "start_date");
        v.maturity_date =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "maturity_date");
        v.long_short =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "long_short");
        v.payment_frequency = ores::shell::app::from_token<std::string>(parsed->positionals[next++],
                                                                        "payment_frequency");
        v.description =
    ...
    auto req = trading::messaging::save_equity_swap_instrument_request::from(std::move(v));

    auto result = do_auth_request<trading::messaging::save_equity_swap_instrument_response>(
        out, session, "trading.v1.equity_swap_instruments.save", req);
    if (!result)
        return;
    ...
    if (result->success) {
        ...
        fail(out) << "Failed to add equity swap instrument: " << msg << std::endl;
    }
}

void equity_swap_instrument_commands::process_delete_equity_swap_instrument(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete equity swap instrument request for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete equity swap instrument." << std::endl;
        return;
    }

    trading::messaging::delete_equity_swap_instrument_request req;
    req.ids = {std::move(instrument_id)};

    auto result = do_auth_request<trading::messaging::delete_equity_swap_instrument_response>(
        out, session, "trading.v1.equity_swap_instruments.delete", req);
    if (!result)
        return;
...
void equity_swap_instrument_commands::process_get_equity_swap_instrument_history(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get equity swap instrument history for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get equity swap instrument history." << std::endl;
        return;
    }
    ...
    if (!result->success) {
        BOOST_LOG_SEV(lg(), warn) << "Failed to get equity swap instrument history: "
                                  << result->message;
    ...
    if (result->history.empty()) {
        out << "No history found for this equity swap instrument." << std::endl;
        return;
    }
