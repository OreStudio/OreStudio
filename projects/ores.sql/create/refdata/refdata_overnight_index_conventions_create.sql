...
create unique index if not exists overnight_index_conventions_version_uniq_idx
on "ores_refdata_overnight_index_conventions_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists overnight_index_conventions_id_uniq_idx
on "ores_refdata_overnight_index_conventions_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists overnight_index_conventions_tenant_idx
on "ores_refdata_overnight_index_conventions_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists overnight_index_conventions_workspace_idx
on "ores_refdata_overnight_index_conventions_tbl" (workspace_id)
where valid_to = ores_utility_infinity_timestamp_fn();
