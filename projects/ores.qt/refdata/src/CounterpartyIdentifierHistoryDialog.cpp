 ...
 *
 */
#include "ores.qt/CounterpartyIdentifierHistoryDialog.hpp"
#include "ores.qt/RelativeTimeHelper.hpp"
#include "ores.refdata.api/messaging/counterparty_identifier_protocol.hpp"
#include "ui_CounterpartyIdentifierHistoryDialog.h"
...
CounterpartyIdentifierHistoryDialog::CounterpartyIdentifierHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::CounterpartyIdentifierHistoryDialog)
    , id_(id)
    , code_(code)
...
QString CounterpartyIdentifierHistoryDialog::code() const {
    return code_;
}

void CounterpartyIdentifierHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for counterparty identifier: "
                               << code_.toStdString();
    ...
    QPointer<CounterpartyIdentifierHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](refdata::messaging::get_counterparty_identifier_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int CounterpartyIdentifierHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString CounterpartyIdentifierHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow CounterpartyIdentifierHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult CounterpartyIdentifierHistoryDialog::calculateDiffAt(int ci,
                                                                                   int pi) const {
    DiffResult diffs;
...
void CounterpartyIdentifierHistoryDialog::displayFullDetails(int index) {
...
void CounterpartyIdentifierHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void CounterpartyIdentifierHistoryDialog::revertToVersionAt(int index) {
