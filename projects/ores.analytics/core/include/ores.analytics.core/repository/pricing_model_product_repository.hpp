 ...
 */
#ifndef ORES_ANALYTICS_CORE_REPOSITORY_PRICING_MODEL_PRODUCT_REPOSITORY_HPP
...
/**
 * @brief Reads and writes pricing model products to data storage.
 */
class ORES_ANALYTICS_CORE_EXPORT pricing_model_product_repository {
private:
    inline static std::string_view logger_name =
        "ores.analytics.repository.pricing_model_product_repository";
...
public:
    ...
    /**
     * @brief Writes pricing model products to database.
     *
    ...
    /**
     * @brief Writes a pricing model product, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of pricing model products, each honouring its own
    ...
    /**
     * @brief Reads latest pricing model products, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::pricing_model_product> read_latest(context ctx);
    std::vector<domain::pricing_model_product> read_latest(context ctx, const std::string& id);
    std::vector<domain::pricing_model_product> read_latest(context ctx,
                                                           const std::vector<std::string>& ids);
    /**@}*/

    /**
     * @brief Reads latest pricing model products filtered by pricing_engine_type_code.
     */
    std::vector<domain::pricing_model_product>
    read_latest_by_pricing_engine_type_code(context ctx,
                                            const std::string& pricing_engine_type_code);

    /**
     * @brief Reads the newest pricing model products filtered by pricing_engine_type_code, current
     * or not.
     *
     ...
     */
    std::vector<domain::pricing_model_product>
    read_any_by_pricing_engine_type_code(context ctx, const std::string& pricing_engine_type_code);


    /**
     * @brief Reads all pricing model products, possibly filtered by primary key.
     */
    std::vector<domain::pricing_model_product> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single pricing model product as it stood at a specific
    ...
    /**
     * @brief Reads latest pricing model products with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Removes a pricing model product, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes pricing model products by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::pricing_model_product apply_claim(context ctx,
                                              const domain::pricing_model_product& v,
                                              const ores::utility::domain::precondition& claim);
};
