 ...
 */
std::vector<domain::tenor> read_one(repository::tenor_repository& repo,
                                    const ores::database::context& ctx,
                                    const messaging::tenor_key& key) {
 ...
 */
messaging::tenor_key key_from(const domain::tenor& v) {
 ...
 */
domain::tenor to_domain(const messaging::tenor_write& write) {
    domain::tenor v;
    v.code = write.code;
    v.display_name = write.display_name;
    v.description = write.description;
    v.sort_order = write.sort_order;
    v.kind = write.kind;
    v.unit = write.unit;
    v.multiplier = write.multiplier;
    return v;
}
...
tenor_service::list_tenors(const messaging::list_tenors_request& request) {
    messaging::list_tenors_response response;
    ...
    response.tenors = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_tenor_count(ctx_);
    return response;
}

messaging::get_tenor_response
tenor_service::get_tenor(const messaging::get_tenor_request& request) {
    messaging::get_tenor_response response;
    ...
    response.tenor = std::move(found.front());
    return response;
}

messaging::get_many_tenors_response
tenor_service::get_many_tenors(const messaging::get_many_tenors_request& request) {
    messaging::get_many_tenors_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenor_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenor = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_tenor_response
tenor_service::put_tenor(const messaging::put_tenor_request& request) {
    messaging::put_tenor_response response;
    domain::tenor value;
    ...
    if (!written.empty())
        response.tenor = std::move(written.front());
    return response;
}

messaging::put_many_tenors_response
tenor_service::put_many_tenors(const messaging::put_many_tenors_request& request) {
    messaging::put_many_tenors_response response;
    std::vector<domain::tenor> batch;
    ...
    response.tenors.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.tenors.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenor_response
tenor_service::delete_tenor(const messaging::delete_tenor_request& request) {
    messaging::delete_tenor_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::tenor_repository::remove_status::removed:
            break;
        case repository::tenor_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenor_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenor_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
tenor_service::delete_many_tenors(const messaging::delete_many_tenors_request& request) {
    messaging::delete_many_tenors_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
tenor_service::list_tenor_versions(const messaging::list_tenor_versions_request& request) {
    messaging::list_tenor_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
tenor_service::get_tenor_version(const messaging::get_tenor_version_request& request) {
    messaging::get_tenor_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.tenor.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tenor_service::prepare_change(const messaging::tenor_change& change,
...
std::vector<domain::tenor> tenor_service::get_tenors(const std::vector<std::string>& codes) {
