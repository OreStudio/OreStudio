 ...
 */
std::vector<domain::currency_group> read_one(repository::currency_group_repository& repo,
                                             const ores::database::context& ctx,
                                             const messaging::currency_group_key& key) {
 ...
 */
messaging::currency_group_key key_from(const domain::currency_group& v) {
    messaging::currency_group_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::currency_group to_domain(const messaging::currency_group_write& write) {
    domain::currency_group v;
    v.code = write.code;
    v.name = write.name;
...
messaging::list_currency_groups_response currency_group_service::list_currency_groups(
    ...
    messaging::list_currency_groups_response response;
    ...
    response.groups = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_group_count(ctx_);
    return response;
}

messaging::get_currency_group_response
currency_group_service::get_currency_group(const messaging::get_currency_group_request& request) {
    messaging::get_currency_group_response response;
    ...
    response.currency_group = std::move(found.front());
    return response;
}

messaging::get_many_currency_groups_response currency_group_service::get_many_currency_groups(
    const messaging::get_many_currency_groups_request& request) {
    messaging::get_many_currency_groups_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::currency_group_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.currency_group = std::move(found.front());
...
currency_group_service::put_currency_group(const messaging::put_currency_group_request& request) {
    messaging::put_currency_group_response response;
    domain::currency_group value;
    ...
    if (!written.empty())
        response.currency_group = std::move(written.front());
    return response;
}

messaging::put_many_currency_groups_response currency_group_service::put_many_currency_groups(
    const messaging::put_many_currency_groups_request& request) {
    messaging::put_many_currency_groups_response response;
    ...
    response.groups.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.groups.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_currency_group_response currency_group_service::delete_currency_group(
    const messaging::delete_currency_group_request& request) {
    messaging::delete_currency_group_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::currency_group_repository::remove_status::removed:
            break;
        case repository::currency_group_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::currency_group_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::currency_group_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_currency_groups_response currency_group_service::delete_many_currency_groups(
    const messaging::delete_many_currency_groups_request& request) {
    messaging::delete_many_currency_groups_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
currency_group_service::list_currency_group_versions(
    const messaging::list_currency_group_versions_request& request) {
    messaging::list_currency_group_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_currency_group_version_response currency_group_service::get_currency_group_version(
    const messaging::get_currency_group_version_request& request) {
    messaging::get_currency_group_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.currency_group.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
currency_group_service::prepare_change(const messaging::currency_group_change& change,
...
currency_group_service::get_groups(const std::vector<std::string>& codes) {
