...
1. *Identify the affected chapter.* The manual lives under
   =doc/manual/user_guide/=. The root file is =user_manual.org=; each
...
3. *Add screenshot placeholders for any new UI.* Every new dialog, page,
   ...
   =doc/manual/user_guide/= — three levels above the repo-root =assets/=,
   hence =../../../=.
...
- =doc/manual/user_guide/= — the root file plus the flat =chapter_N_*.org=
  source files.
- =deploy_manual= CMake target — builds the PDF via =projects/ores.lisp/src/ores-build-manual.el=.
