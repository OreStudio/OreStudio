...
QVariant ClientPartyIdSchemeModel::data(const QModelIndex& index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            ...
            case DisplayOrder:
                return static_cast<qlonglong>(scheme.display_order);
            case Version:
                return static_cast<qlonglong>(scheme.version);
            case ModifiedBy:
    ...
    QFuture<FetchResult> future = QtConcurrent::run([self, offset, limit]() -> FetchResult {
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << result->schemes.size() << " party ID schemes";
                const std::uint32_t count = static_cast<std::uint32_t>(result->schemes.size());
                return {.success = true,
