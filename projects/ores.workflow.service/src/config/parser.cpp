...
const std::string product_version("ores.workflow.service v" ORES_VERSION);
...
using ores::workflow::service::config::options;
using ores::workflow::service::config::parser_exception;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.workflow.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Workflow orchestration microservice" << std::endl << std::endl
         << "Usage: ores.workflow.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("WORKFLOW_SERVICE"));
