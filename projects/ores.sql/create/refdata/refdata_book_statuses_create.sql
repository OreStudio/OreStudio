...
create unique index if not exists book_statuses_name_uniq_idx
on "ores_refdata_book_statuses_tbl" (tenant_id, name)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists book_statuses_version_uniq_idx
on "ores_refdata_book_statuses_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists book_statuses_code_uniq_idx
on "ores_refdata_book_statuses_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists book_statuses_tenant_idx
...
for each row execute function ores_refdata_book_statuses_insert_fn();

create or replace rule ores_refdata_book_statuses_delete_rule as
on delete to "ores_refdata_book_statuses_tbl" do instead (
    update "ores_refdata_book_statuses_tbl"
    set valid_to = clock_timestamp()
