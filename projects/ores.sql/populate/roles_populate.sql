...
begin
    ...
    from ores.roles
    where name = p_role_name and valid_to = ores.infinity_timestamp();
    ...
    where code = p_permission_code and valid_to = ores.infinity_timestamp();
    ...
    if not exists (
        select 1 from ores.role_permissions
        ...
        values (v_role_id, v_permission_id, current_timestamp, ores.infinity_timestamp());
    end if;
end;
$$ language plpgsql;
...
-- Show summary
select 'Roles:' as summary, count(*) as count from ores.roles
where valid_to = ores.infinity_timestamp()
union all
select 'Role-Permission assignments:', count(*) from ores.role_permissions
where valid_to = ores.infinity_timestamp();
