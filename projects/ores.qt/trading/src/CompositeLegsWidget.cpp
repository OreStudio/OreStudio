...
void CompositeLegsWidget::setupUi() {
    ...
    ui_->legsTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
    ui_->legsTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
    ui_->legsTable->setEditTriggers(QAbstractItemView::DoubleClicked |
                                    QAbstractItemView::EditKeyPressed);
}

void CompositeLegsWidget::setupConnections() {
    connect(ui_->addLegButton, &QPushButton::clicked, this, &CompositeLegsWidget::onAddLegClicked);
    connect(ui_->removeLegButton,
            &QPushButton::clicked,
            this,
            &CompositeLegsWidget::onRemoveLegClicked);
    connect(
        ui_->legsTable, &QTableWidget::cellChanged, this, [this](int, int) { emit legsChanged(); });
}

void CompositeLegsWidget::setLegs(const std::vector<trading::domain::composite_leg>& legs) {
...
std::vector<trading::domain::composite_leg> CompositeLegsWidget::legs() const {
    ...
    for (int row = 0; row < rows; ++row) {
        ...
        const QString tradeId = tradeIdItem ? tradeIdItem->text().trimmed() : QString{};
        if (tradeId.isEmpty())
            continue;
...
void CompositeLegsWidget::onAddLegClicked() {
    const int row = ui_->legsTable->rowCount();
    ui_->legsTable->insertRow(row);
    ui_->legsTable->setItem(row, 0, new QTableWidgetItem(QString::number(row + 1)));
