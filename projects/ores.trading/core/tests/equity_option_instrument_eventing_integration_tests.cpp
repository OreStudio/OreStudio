...
// notification" pattern end to end for equity_option_instrument -- the
...
// Equity Option Instrument writes are party-scoped: the session-level
...
TEST_CASE("write_equity_option_instrument_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::trading::eventing::equity_option_instrument_changed_event>(
        [&nats](const ores::trading::eventing::equity_option_instrument_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::trading::eventing::equity_option_instrument_changed_event>::name),
    ...
    event_source.register_mapping<ores::trading::eventing::equity_option_instrument_changed_event>(
        "ores.trading.equity_option_instrument", "ores_trading_equity_option_instruments");
    ...
    auto v = generate_synthetic_equity_option_instrument(ctx);
    ...
    BOOST_LOG_SEV(lg, debug) << "Equity Option Instrument: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.trading.equity_option_instrument") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for equity_option_instrument " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for equity_option_instrument "
                            << id_str;
}
