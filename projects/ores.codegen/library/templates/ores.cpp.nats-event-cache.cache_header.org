...
Includes =ores.utility/rfl/reflectors.hpp= directly: =load()='s
=rfl::json::read/write= calls need it whenever the entity carries a
=boost::uuids::uuid=/=tenant_id= field (every =read_for_cache= entity
does, since =read_for_cache= itself requires =has_tenant_id=), and a
non-template header-only class's inline member bodies are fully
