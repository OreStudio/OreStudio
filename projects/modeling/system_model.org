...
| ores.eventing | High-level event dispatch over NATS — event type registration, publish/subscribe wrappers, and retry/dead-letter patterns. | nats |
| ores.testing | Catch2 integration with database isolation (test-local PostgreSQL schemas), preconfigured logger sinks, and test fixture helpers. | foundation, database |
| ores.connections | Connection management for external systems — SSH tunnels, database proxies, and credential rotation. | security |
...
| Component | Description | Depends On |
|-----------+-------------+------------|
| ores.refdata | Reference data management — currencies, countries, calendar conventions, market conventions, and coding schemes. CRUD with temporal versioning. | database, nats |
| ores.iam | Identity and Access Management — tenants, accounts, roles, permissions, session management, RBAC enforcement. | database, nats, security |
| ores.ore | ORE domain model — configuration of pricing and risk models, XML/CSV/JSON I/O, temporal versioning, and round-trip validation. | utility, nats |
| ores.dq | Data quality — audit and governance following the standard DQ-6 dimensions: completeness, uniqueness, timeliness, validity, accuracy, consistency. | database, nats |
| ores.marketdata | Market data — curves, surfaces, quotes, fixings. Schema, parser, repository, service, and Qt UI. | database, nats, refdata |
| ores.trading | Trading domain — trades, positions, portfolios, books. Trade lifecycle, FSM for state transitions, trade blotters. | database, nats, refdata |
| ores.reporting | Reporting and analytics — report definitions, measure sets, Greek configurations, EOD batch sets. | database, nats, refdata |
| ores.synthetic | Synthetic data generation for testing and demonstration — faker-based entity population, domain-specific seed data. | database, nats |
| ores.assets | Asset management — financial instrument definitions, asset class taxonomy (risk) and product type (structural routing), instrument families (rates, FX, equity, credit, commodity). | database, nats, refdata |
...
| ores.workflow | Workflow engine — event-driven saga orchestrator for multi-step business processes. Party provisioning, ORE import, report execution as concrete sagas. | database, nats |
| ores.scheduler | Scheduling library wrapping =pg_cron= with a Quartz.NET-style fluent API for periodic task execution. | database |
| ores.fpml | FpML integration — XML schema types, parsing, and serialisation for Financial Products Markup Language standards. | utility |
...
| ores.cli | Command-line interface — import/export currencies in multiple formats, ORE sample execution, batch operations. | All layers |
| ores.http | HTTP services framework — REST route structure, handler skeletons, CRUD conventions. | foundation, domain |
| ores.http.server | Boost.Asio HTTP server — wires all route handlers (IAM, assets, risk, storage, variability), runs the event loop. | ores.http |
| ores.wt.service | Wt C++ Web Toolkit frontend — account management, currency and country views, feature flags. Alternative to Qt desktop client. | All layers |
| ores.compute.service | ORE Engine compute wrapper — receives compute job requests via NATS, spawns ORE child processes, and publishes results. | domain, nats |
| ores.controller.core | Orchestration controller — assembles compute pipelines, sequences scheduler/compute/reporting stages, and tracks run state. | domain, nats |
| ores.lisp | Emacs Lisp developer tooling — per-checkout dashboard, database browser, shell integration, and org-roam export. Not part of the production system. | none |
