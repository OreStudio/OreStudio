...
#+description: Use compass env init/diff/list/version to generate and inspect the .env: behaviour-preserving credential generation, the .env.old diff, masked variable listing, and recording a new env-format version.
#+type: recipe
#+level: cross
...
=recreate_database= runs. Show the current canonical version:

#+begin_src sh :results verbatim
./projects/ores.compass/compass.sh env version
#+end_src

When you make a change that alters the =.env= — add a service to the
registry, add or rename a variable — record a new version:

#+begin_src sh :results verbatim
./projects/ores.compass/compass.sh env version new "added analytics service DB creds"
#+end_src

This appends a dated row to the [[id:BFE330BD-2AF8-43A4-BE8C-D066BD3FB1FD][environment format version log]] — the
single, parseable source read by both =compass env init= (which writes
=ORES_ENV_VERSION=) and the version check. Then re-run =compass env init=
to write the new version into your =.env=. Any checkout still on the old
version then fails the pre-=recreate_database= check with a clear "re-run
=compass env init=" message, instead of a later missing-password error.
