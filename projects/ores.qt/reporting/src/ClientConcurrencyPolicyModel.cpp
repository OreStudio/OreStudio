...
ClientConcurrencyPolicyModel::ClientConcurrencyPolicyModel(ClientManager* clientManager,
    ...
    connect(watcher_,
            ...
            this,
            &ClientConcurrencyPolicyModel::onPoliciesLoaded);
    ...
    QFuture<FetchResult> future = QtConcurrent::run([self, offset, limit]() -> FetchResult {
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << result->policies.size()
                    << " concurrency policies, total available: " << result->total_available_count;
                return {.success = true,
...
void ClientConcurrencyPolicyModel::onPoliciesLoaded() {
    is_fetching_ = false;
