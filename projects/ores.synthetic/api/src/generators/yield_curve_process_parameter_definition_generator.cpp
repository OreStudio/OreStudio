...
generate_synthetic_yield_curve_process_parameter_definition(
    ...
    r.min_value = std::nullopt;
    r.max_value = std::nullopt;
    r.display_order = faker::number::integer(1, 10);
...
generate_synthetic_yield_curve_process_parameter_definitions(
    ...
    std::vector<domain::yield_curve_process_parameter_definition> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_yield_curve_process_parameter_definition(ctx));
    return r;
}
