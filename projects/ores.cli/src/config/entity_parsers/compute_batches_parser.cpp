...
ores::cli::config::add_compute_batch_options
read_add_compute_batch_options(const variables_map& vm) {
    ...
    if (vm.count("external-ref") == 0) {
        BOOST_THROW_EXCEPTION(
            parser_exception("Must supply --external-ref for add batch command."));
    }
    r.external_ref = vm["external-ref"].as<std::string>();

    if (vm.count("status") != 0)
        r.status = vm["status"].as<std::string>();
...
handle_compute_batches_command(bool has_help,
    ...
    validate_operation("batches", operation, allowed_operations);
    ...
    if (operation == list_command_name) {
        ...
        if (has_help) {
            print_help_command("batches list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::compute_batches);
    } else if (operation == add_command_name) {
        auto d = add_common_options(make_add_compute_batch_options_description());
        if (has_help) {
            print_help_command("batches add", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.adding = read_add_compute_batch_options(vm);
    }
