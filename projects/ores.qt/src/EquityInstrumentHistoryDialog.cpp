...
#include "ui_EquityInstrumentHistoryDialog.h"
...
EquityInstrumentHistoryDialog::EquityInstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::EquityInstrumentHistoryDialog),
      id_(id),
...
void EquityInstrumentHistoryDialog::setupUi() {
...
void EquityInstrumentHistoryDialog::setupToolbar() {
...
void EquityInstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &EquityInstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &EquityInstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &EquityInstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void EquityInstrumentHistoryDialog::loadHistory() {
    ...
    QPointer<EquityInstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::equity_instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_equity_instrument_history_request request;
        request.id = id;
...
void EquityInstrumentHistoryDialog::updateVersionList() {
...
void EquityInstrumentHistoryDialog::onVersionSelected() {
...
void EquityInstrumentHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.underlying_code != previous.underlying_code)
        addChange("Underlying",
            QString::fromStdString(previous.underlying_code),
            QString::fromStdString(current.underlying_code));
...
void EquityInstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->underlyingCodeValue->setText(
        QString::fromStdString(v.underlying_code));
...
void EquityInstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void EquityInstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void EquityInstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
    int row = selected.first()->row();
    if (static_cast<std::size_t>(row) >= versions_.size()) return;
