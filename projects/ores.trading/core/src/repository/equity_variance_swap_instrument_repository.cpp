...
equity_variance_swap_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<equity_variance_swap_instrument_entity,
            ...
            query,
            [](const auto& entities) {
                return equity_variance_swap_instrument_mapper::map(entities);
            },
            lg(),
            "Reading latest equity variance swap instruments (workspace resolution chain).");
    }
...
equity_variance_swap_instrument_repository::read_latest(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Reading latest equity variance swap instrument. "
...
equity_variance_swap_instrument_repository::read_all(context ctx,
                                                     const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all equity variance swap instrument versions. "
                               << "instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<equity_variance_swap_instrument_entity>> |
...
equity_variance_swap_instrument_repository::read_at_version(context ctx,
                                                            const std::string& instrument_id,
                                                            std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading equity variance swap instrument at version. "
    ...
    const auto entities = execute_read_query<equity_variance_swap_instrument_entity,
                                             domain::equity_variance_swap_instrument>(
        ctx,
        query,
        [](const auto& entities) { return equity_variance_swap_instrument_mapper::map(entities); },
        lg(),
        "Reading equity variance swap instrument at version.");
...
equity_variance_swap_instrument_repository::read_latest(context ctx,
                                                        std::uint32_t offset,
                                                        std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest equity variance swap instruments with offset: "
    ...
    return execute_read_query<equity_variance_swap_instrument_entity,
        ...
        [](const auto& entities) { return equity_variance_swap_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity variance swap instruments with pagination.");
}

std::uint32_t
equity_variance_swap_instrument_repository::get_total_equity_variance_swap_instrument_count(
    context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active equity variance swap instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<equity_variance_swap_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active equity variance swap instrument count: " << count;
    return count;
}
...
equity_variance_swap_instrument_repository::read_latest(
    ...
    auto result = execute_read_query<equity_variance_swap_instrument_entity,
                                     domain::equity_variance_swap_instrument>(
        ctx,
        query,
        [](const auto& entities) { return equity_variance_swap_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity variance swap instruments by ids.");
    return result;
}
...
void equity_variance_swap_instrument_repository::remove(
    ...
    const auto query = sqlgen::delete_from<equity_variance_swap_instrument_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid &&
                             "instrument_id"_c.in(instrument_ids) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing equity variance swap instruments.");
}
