...
=ores.sql=, and the standalone scripts in [[proj:scripts][=scripts/=]] — is unreachable from
...
what any script does; the standalone scripts in [[proj:scripts][=scripts/=]] are
...
1. A new =* Tooling= section in [[id:7474935C-95D1-3034-B833-83DCD9FB3A09][Knowledge]] provides a single entry
   ...
   standalone [[proj:scripts][=scripts/=]] scripts and linking out to each tool's
   component overview.
...
- A =Developer scripts= knowledge doc exists under
  [[proj:doc/knowledge/architecture][=doc/knowledge/architecture/=]] cataloguing every standalone script in
  [[proj:scripts][=scripts/=]] by purpose group, with links to =ores.sql= and =ores.compass=
...
- /Developer scripts doc under architecture/, not a new top-level
  ...
  [[proj:doc/knowledge/architecture][=doc/knowledge/architecture/=]] pattern. A =doc/tooling/= top-level
...
- Documenting [[proj:build/scripts][=build/scripts/=]] (CI/build infrastructure scripts) — a
