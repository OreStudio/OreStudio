...
Calendar assignments and the currency's own fields must not pop two
independent change-reason dialogs for one Save click. Two cases:
...
- Only calendar assignments changed (=!hasChanges_=): commit them here,
  ...
  entirely so the currency's own fields aren't re-saved for no real
  change.
- Both changed, or only the currency's own fields changed: fall
  ...
  happens afterwards in the =on_save_success= paste point once the
  currency save has actually succeeded -- see that block below for why
  it isn't done here instead.
