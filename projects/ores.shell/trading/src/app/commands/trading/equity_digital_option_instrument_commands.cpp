...
void equity_digital_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_digital_option_instruments_menu->Insert(
        ...
        [&session, &pagination](std::ostream& out) {
            process_get_equity_digital_option_instruments(
    ...
    pagination.register_list_callback(
        "equity_digital_option_instruments", [&session, &pagination](std::ostream& out) {
            process_get_equity_digital_option_instruments(out, session, pagination);
        });

    equity_digital_option_instruments_menu->Insert(
        "add",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_equity_digital_option_instrument(std::ref(out), std::ref(session), args);
        },
        "Add Equity Digital Option instrument (trade_type_code "
        "party_id "
        "trade_id "
        "underlying_name "
        "currency "
    ...
    equity_digital_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::string id) {
            process_delete_equity_digital_option_instrument(
                std::ref(out), std::ref(session), std::move(id));
        },
        "Delete Equity Digital Option instrument by instrument_id");

    equity_digital_option_instruments_menu->Insert(
        "history",
        [&session](std::ostream& out, std::string id) {
            process_get_equity_digital_option_instrument_history(
                std::ref(out), std::ref(session), std::move(id));
        },
        "Show Equity Digital Option instrument's version history");
...
void equity_digital_option_instrument_commands::process_get_equity_digital_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity Digital Option instrument request.";
    ...
    auto result =
        do_auth_request<trading::messaging::get_equity_digital_option_instruments_response>(
    ...
    out << "\nPage " << page << " of " << total_pages << " ("
        << result->equity_digital_option_instruments.size() << " of " << state.total_count
        << " total)" << std::endl;
}
...
void equity_digital_option_instrument_commands::process_add_equity_digital_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add Equity Digital Option instrument." << std::endl;
        return;
    }
    ...
    try {
        ...
        v.strike = ores::shell::app::from_token<std::optional<double>>(parsed->positionals[next++],
                                                                       "strike");
        v.barrier_level = ores::shell::app::from_token<std::optional<double>>(
            parsed->positionals[next++], "barrier_level");
        v.barrier_type =
        ...
        v.payout_amount = ores::shell::app::from_token<std::optional<double>>(
            parsed->positionals[next++], "payout_amount");
        v.description =
    ...
    auto req =
        trading::messaging::save_equity_digital_option_instrument_request::from(std::move(v));

    auto result =
        do_auth_request<trading::messaging::save_equity_digital_option_instrument_response>(
...
void equity_digital_option_instrument_commands::process_delete_equity_digital_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete Equity Digital Option instrument."
                  << std::endl;
        return;
    }
    ...
    auto result =
        do_auth_request<trading::messaging::delete_equity_digital_option_instrument_response>(
    ...
    if (result->success) {
        ...
        BOOST_LOG_SEV(lg(), warn) << "Failed to delete Equity Digital Option instrument: "
                                  << result->message;
        fail(out) << "Failed to delete Equity Digital Option instrument: " << result->message
                  << std::endl;
    }
}

void equity_digital_option_instrument_commands::
    process_get_equity_digital_option_instrument_history(std::ostream& out,
                                                         nats_client& session,
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get Equity Digital Option instrument history."
                  << std::endl;
        return;
    }
