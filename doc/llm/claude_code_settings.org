...
Via the Build pillar (configures the preset first if needed; the
=skills= alias recreates the other generated half of [[proj:.claude][=.claude/=]]):

#+begin_src sh
compass build settings          # .claude/settings.json
compass build settings skills   # recreate all of .claude/
#+end_src

Or via the raw CMake target:

#+begin_src sh
cmake --build --preset linux-clang-debug-ninja --target deploy_settings
#+end_src

This invokes [[proj:projects/ores.lisp/src/ores-build-settings.el][=projects/ores.lisp/src/ores-build-settings.el=]], which tangles this file to
[[proj:.claude/settings.json][=.claude/settings.json=]]. Requires Emacs on PATH and a configured build
...
sandbox. Source [[proj:build/scripts/set_ssh_agent.sh][=build/scripts/set_ssh_agent.sh=]] beforehand for push/fetch
