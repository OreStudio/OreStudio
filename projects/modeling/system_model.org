...
- [[id:8A2B3C4D-5E6F-7A8B-9C0D-1E2F3A4B5C6D][ores.telemetry]]: Logging infrastructure (Boost.Log integration), distributed
  tracing (trace_id, span_id), log record export, severity levels. Depends on:
  platform.
...
- [[id:B7E9F3A1-C8D2-4E6B-A1F5-9D3C7E8B2A4F][ORE Studio SQL Schema]]: Postgres Database setup.
- Dependency order: utility → platform → telemetry → database.

** Infrastructure Layer

- Components: ores.assets.
...
- Components:
- [[id:D0876A24-69BA-F484-ED9B-71CD3AA8710C][ores.shell]]: REPL client. Exercises the service layer via the binary comms
  protocol.
