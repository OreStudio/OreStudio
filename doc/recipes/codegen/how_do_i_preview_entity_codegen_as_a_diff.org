...
[[id:08FBD248-257A-A474-DD43-DB08949978F1][ores.codegen]] for background on the physical-space graph and templates.
...
This generates the entity's full supported set into a temp directory and
prints a unified diff against the on-disk files. Nothing is written to the
working tree.

To restrict to one facet:
...
#+begin_example
Diffing country (domain_entity, refdata) — ores.sql.schema
...
=--diff= is handled by =_diff_entity= in
...
Run =./compass.sh codegen entity generate country --address ores.sql.schema --diff=
