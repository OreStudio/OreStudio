 ...
 */
#ifndef ORES_ASSETS_API_EVENTING_TAG_EVENT_HPP
#define ORES_ASSETS_API_EVENTING_TAG_EVENT_HPP
...
/**
 * @brief Event traits for tag's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::assets::messaging::tag_event> {
    static constexpr std::string_view subject_prefix = "assets.v1.tags_events";

    static ores::assets::messaging::tag_event
    from_notification(const entity_event_notification& notification) {
        ores::assets::messaging::tag_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::assets::messaging::tag_key>(notification.key))
            event.key = *key;
