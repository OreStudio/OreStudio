...
#include "ores.iam.api/eventing/account_type_event.hpp"
...
TEST_CASE("write_account_type_publishes_an_event", tags) {
    ...
    using event_type = ores::iam::messaging::account_type_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_iam_account_types");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.type == v.type)
