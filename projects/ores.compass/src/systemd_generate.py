...
# generated units on every run (same sync-if-changed pattern, one shared
# daemon-reload). Add future host-wide drop-ins here rather than writing a
# new one-off deploy path.
_SYSTEMD_SRC_DIR = Path(__file__).resolve().parent / "systemd"
STATIC_DROPINS = [
    # WS-5 (unprivileged half): give the interactive Emacs daemon priority.
    # See the drop-in's own header for the ancestor-clamping caveat -- this
    # alone has no effect until the WS-5 root half also lands.
    (_SYSTEMD_SRC_DIR / "emacs.service.d" / "50-protect.conf",
     Path("emacs.service.d") / "50-protect.conf"),
]
...
def cmd_deploy(project_root: Path, env: dict, args) -> int:
    ...
    # Static, host-wide drop-ins (not per-environment, never removed here --
    # they aren't generated, so there's no "no longer rendered" to detect).
    for src, dest_rel in STATIC_DROPINS:
