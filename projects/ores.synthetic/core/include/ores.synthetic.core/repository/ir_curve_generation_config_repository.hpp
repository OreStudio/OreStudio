 ...
 */
#ifndef ORES_SYNTHETIC_CORE_REPOSITORY_IR_CURVE_GENERATION_CONFIG_REPOSITORY_HPP
#define ORES_SYNTHETIC_CORE_REPOSITORY_IR_CURVE_GENERATION_CONFIG_REPOSITORY_HPP
...
/**
 * @brief Reads and writes IR curve generation configs to data storage.
 */
class ORES_SYNTHETIC_CORE_EXPORT ir_curve_generation_config_repository {
private:
    inline static std::string_view logger_name =
        "ores.synthetic.repository.ir_curve_generation_config_repository";
...
public:
    ...
    /**
     * @brief Writes IR curve generation configs to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::ir_curve_generation_config& v);
    void write(context ctx, const std::vector<domain::ir_curve_generation_config>& v);
    /**@}*/

    /**
     * @brief Writes a IR curve generation config, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of IR curve generation configs, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::ir_curve_generation_config>& v,
               const std::vector<ores::utility::domain::precondition>& claims);

    /**
     * @brief Reads latest IR curve generation configs, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::ir_curve_generation_config> read_latest(context ctx);
    std::vector<domain::ir_curve_generation_config> read_latest(context ctx, const std::string& id);
    ...
    /**
     * @brief Reads all IR curve generation configs, possibly filtered by primary key.
     */
    std::vector<domain::ir_curve_generation_config> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single IR curve generation config as it stood at a specific
    ...
    /**
     * @brief Reads latest IR curve generation configs with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_ir_curve_generation_config_count(context ctx);

    /**
     * @brief Deletes a IR curve generation config by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a IR curve generation config, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes IR curve generation configs by closing their temporal validity.
     */
