...
#include "ores.iam.api/generators/account_type_generator.hpp"
...
// notification" pattern end to end for account_type -- the
...
using ores::iam::repository::account_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_account_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::iam::eventing::account_type_changed_event>(
        [&nats](const ores::iam::eventing::account_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::iam::eventing::account_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.iam.account_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.types,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::iam::eventing::account_type_changed_event>(
        "ores.iam.account_type", "ores_iam_account_types");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::iam::eventing::account_type_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_account_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.type;
    BOOST_LOG_SEV(lg, debug) << "Account Type: " << v;

    account_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.iam.account_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for account_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for account_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::iam::service::account_type_service svc(crud_ctx);
        ...
        REQUIRE_FALSE(svc.find_type(id_str).has_value());
