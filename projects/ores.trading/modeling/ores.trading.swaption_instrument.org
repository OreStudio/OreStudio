...
#+description: Codegen entity model for the trading =swaption_instrument= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
