...
import sessions with a session selector at the top.  The body is a notebook
with one tab per data type.  All tabs are scoped to the selected session.

#+begin_example
+-------------------------------------------------------------------------------------+
|  DATA IMPORT SESSION                                       [New Import] [Refresh]    |
|  Session: [ 2026-05-17 10:15 — ORE XML — /ore/examples/Example_1 ▼ ]               |
+-------------------------------------------------------------------------------------+
| [ Trades (12) ] [ Market Data (4) ] [ Conventions (8) ] [ Curves (6) ] [ Pricing ] |
+-------------------------------------------------------------------------------------+
...
Dropdown listing recent import sessions, most recent first.  Each entry shows:
session timestamp, source type, source directory (truncated), and a summary
badge (e.g. "3 pending trades", "all data live").  A search/filter allows
finding sessions by date or path.
