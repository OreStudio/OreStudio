...
void ReportDefinitionMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "ReportDefinitionListWindow", {}, {900, 400}, 1);
}
...
void ReportDefinitionMdiWindow::deleteSelected() {
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* definition = model_->getDefinition(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(definition->id));
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " report definitions";
