...
CdsConventionMdiWindow::CdsConventionMdiWindow(ClientManager* clientManager,
...
void CdsConventionMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &CdsConventionMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &CdsConventionMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &CdsConventionMdiWindow::deleteSelected);
    ...
    connect(
        historyAction_, &QAction::triggered, this, &CdsConventionMdiWindow::viewHistorySelected);
}
...
void CdsConventionMdiWindow::setupConnections() {
    connect(
        model_, &ClientCdsConventionModel::dataLoaded, this, &CdsConventionMdiWindow::onDataLoaded);
    connect(
        model_, &ClientCdsConventionModel::loadError, this, &CdsConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &CdsConventionMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &CdsConventionMdiWindow::onDoubleClicked);
...
void CdsConventionMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void CdsConventionMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* cc = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showConventionHistory for code: " << cc->id;
...
void CdsConventionMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete CDS convention while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " CDS conventions";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 CDS conventions?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete CDS Convention", confirmMessage, QMessageBox::Yes | QMessageBox::No);
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "CDS Convention deletion failed: " << code << " - " << result.second;
                failure_count++;
