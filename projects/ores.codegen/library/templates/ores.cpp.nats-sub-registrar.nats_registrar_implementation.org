...
- =read_for_cache= (bool, Flags) — wire the generated
  ...
  standard archetype for the bulk cache-warming read; =party= still
  wires its own hand-written =read_parties_for_cache= via the
  custom-subscription paste point below pending a fix to its
  codegen drift, and is the template's motivating example, not this
  flag's.
