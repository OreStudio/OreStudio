...
void PricingModelConfigMdiWindow::setupToolbar() {
...
void PricingModelConfigMdiWindow::setupTable() {
    model_ = new ClientPricingModelConfigModel(clientManager_, this);
...
void PricingModelConfigMdiWindow::doReload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading pricing model configurations";
    clearStaleIndicator();
    emit statusChanged(tr("Loading pricing model configurations..."));
    model_->refresh();
}

void PricingModelConfigMdiWindow::onDataLoaded() {
...
void PricingModelConfigMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void PricingModelConfigMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void PricingModelConfigMdiWindow::updateActionStates() {
...
void PricingModelConfigMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new pricing model configuration requested";
    emit addNewRequested();
}

void PricingModelConfigMdiWindow::editSelected() {
...
void PricingModelConfigMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void PricingModelConfigMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (ids.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid pricing model configurations to delete";
        return;
    }
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::delete_pricing_model_config_request,
            0, std::move(payload)
        );
        ...
        auto response = analytics::messaging::delete_pricing_model_config_response::
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished,
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                BOOST_LOG_SEV(lg(), debug) << "Pricing Model Configuration deleted: " << code;
                success_count++;
