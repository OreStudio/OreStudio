...
void ContactTypeController::showListWindow() {
    ...
    // Create new window
    listWindow_ = new ContactTypeMdiWindow(clientManager_, username_, badgeCache_);

    // Connect signals
    connect(listWindow_,
            &ContactTypeMdiWindow::statusChanged,
            this,
            &ContactTypeController::statusMessage);
    connect(listWindow_,
            &ContactTypeMdiWindow::errorOccurred,
            this,
            &ContactTypeController::errorMessage);
    connect(listWindow_,
            &ContactTypeMdiWindow::showTypeDetails,
            this,
            &ContactTypeController::onShowDetails);
    connect(listWindow_,
            &ContactTypeMdiWindow::addNewRequested,
            this,
            &ContactTypeController::onAddNewRequested);
    connect(listWindow_,
            &ContactTypeMdiWindow::showTypeHistory,
            this,
            &ContactTypeController::onShowHistory);
    ...
    connect(listMdiSubWindow_,
            &QObject::destroyed,
            this,
            [self = QPointer<ContactTypeController>(this), key]() {
                if (!self)
                    return;
...
void ContactTypeController::onShowDetails(const refdata::domain::contact_type& type) {
...
void ContactTypeController::wireDetailDialogCommon(ContactTypeDetailDialog* detailDialog) {
    ...
    connect(detailDialog,
            ...
            &ContactTypeController::statusMessage);
    connect(detailDialog,
            &ContactTypeDetailDialog::errorMessage,
            this,
            &ContactTypeController::errorMessage);
}
...
void ContactTypeController::showAddWindow() {
    ...
    connect(detailDialog,
            &ContactTypeDetailDialog::typeSaved,
            this,
            [self = QPointer<ContactTypeController>(this)](const QString& code) {
                if (!self)
                    return;
...
void ContactTypeController::showDetailWindow(const refdata::domain::contact_type& type) {
    ...
    connect(detailDialog,
            &ContactTypeDetailDialog::typeDeleted,
            this,
            [self = QPointer<ContactTypeController>(this), key](const QString& code) {
                if (!self)
                    return;
...
void ContactTypeController::showHistoryWindow(const QString& code) {
    BOOST_LOG_SEV(lg(), info) << "Opening history window for contact type: " << code.toStdString();
    ...
    auto* historyDialog =
        new HistoryDialog(std::string(entity_type_of(refdata::domain::contact_type{})),
                          code.toStdString(),
                          clientManager_,
                          mainWindow_);
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<ContactTypeController>(this)](const QString& message) {
                if (!self)
                    return;
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<ContactTypeController>(this)](
...
void ContactTypeController::onOpenVersion(const refdata::domain::contact_type& type,
                                          int versionNumber) {
    ...
    detailWindow->setWindowTitle(
        QString("Contact Type: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void ContactTypeController::fetchContactTypeHistory(
    const QString& entityId,
    std::function<void(std::expected<std::vector<refdata::domain::contact_type>, QString>)>
        callback) {
    refdata::messaging::get_contact_type_history_request request;
    request.code = entityId.toStdString();

    using FetchResult = std::expected<std::vector<refdata::domain::contact_type>, QString>;
...
void ContactTypeController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<ContactTypeController> self = this;
    fetchContactTypeHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<refdata::domain::contact_type>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void ContactTypeController::onRevertHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<ContactTypeController> self = this;
    fetchContactTypeHistory(
        entityId,
        [self, entityId, versionNumber](
...
void ContactTypeController::onRevertVersion(const refdata::domain::contact_type& type) {
    BOOST_LOG_SEV(lg(), info) << "Reverting contact type to version: " << type.version;
    ...
    connect(detailDialog,
            ...
            [self = QPointer<ContactTypeController>(this)](const QString& code) {
                ...
                emit self->statusMessage(
                    QString("Contact Type '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
    ...
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(
        QString("Revert Contact Type: %1").arg(QString::fromStdString(type.code)));
