...
render_series_key_shape_fields(const domain::series_key_shape& v) {
    ...
    fields.push_back({.name = "Qualifier Depth", .value = std::to_string(v.qualifier_depth)});
    fields.push_back(
        {.name = "Has Point Dimension", .value = v.has_point_dimension ? "true" : "false"});
    fields.push_back({.name = "Default Point", .value = v.default_point});
