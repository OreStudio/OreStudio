...
namespace {
inline auto& composite_instrument_handler_lg() {
    static auto instance = ores::logging::make_logger(
        "ores.trading.messaging.composite_instrument_handler");
    return instance;
}
} // namespace
        ...
        try {
            if (auto req = decode<get_composite_instruments_request>(msg)) {
                ...
                resp.total_available_count =
                    static_cast<int>(svc.count_composite_instruments());
            }
    ...
    void save(ores::nats::message msg) {
        ...
        if (auto req = decode<save_composite_instrument_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(composite_instrument_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_, msg, save_composite_instrument_response{
        ...
        if (auto req = decode<delete_composite_instrument_request>(msg)) {
            ...
            for (const auto& id : req->ids) {
                try {
                    ...
                    BOOST_LOG_SEV(composite_instrument_handler_lg(), error)
                        << "Failed to delete composite_instrument " << id
                        << ": " << e.what();
    ...
    void history(ores::nats::message msg) {
        ...
        if (auto req = decode<get_composite_instrument_history_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(composite_instrument_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg, get_composite_instrument_history_response{
                    .success = true,
