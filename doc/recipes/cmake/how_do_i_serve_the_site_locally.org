...
Then open =http://localhost:<ORES_SITE_PORT>/OreStudio/= in a browser (=ORES_SITE_PORT= comes from =.env=; default for local1 is =51004=).
...
=51004= (last-resort default for local1).
