...
Every document in [[proj:doc][=doc/=]] and every component overview in =projects/*/modeling/=
...
The recursive scan covers the entire repo root; [[proj:build][=build/=]], =.git/=, and
[[proj:vcpkg][=vcpkg/=]] directories rarely contain =.org= files so the scan is fast in
...
Checking a compiled build into [[proj:external/org-roam-ui][=external/org-roam-ui=]] avoids needing
