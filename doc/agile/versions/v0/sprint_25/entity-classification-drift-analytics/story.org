...
#+description: Apply the ores.refdata pilot's proven pattern to ores.analytics: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the refdata-codegen-drift CI job to cover analytics-cpp. Supersedes the ad hoc 'Apply safe drift to analytics-cpp' task.
#+type: story
#+level: s2
