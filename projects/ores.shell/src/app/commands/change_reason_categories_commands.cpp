...
void change_reason_categories_commands::register_commands(cli::Menu& root_menu,
    ...
    menu->Insert(
        "get",
        [&session](std::ostream& out) { process_get_categories(std::ref(out), std::ref(session)); },
    ...
    menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_category(std::ref(out),
                                 std::ref(session),
                                 std::move(code),
    ...
    menu->Insert(
        ...
        [&session](std::ostream& out, std::string code) {
            process_delete_category(std::ref(out), std::ref(session), std::move(code));
        },
    ...
    menu->Insert(
        "history",
        [&session](std::ostream& out, std::string code) {
            process_get_category_history(std::ref(out), std::ref(session), std::move(code));
        },
...
void change_reason_categories_commands::process_get_categories(std::ostream& out,
                                                               nats_client& session) {
    ...
    BOOST_LOG_SEV(lg(), info) << "Successfully retrieved " << result->categories.size()
                              << " categories.";
    out << result->categories << std::endl;
}

void change_reason_categories_commands::process_add_category(std::ostream& out,
                                                             nats_client& session,
                                                             std::string code,
                                                             std::string description,
                                                             std::string change_commentary) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add change reason category for: " << code;
    ...
    auto req = dq::messaging::save_change_reason_category_request::from(
        dq::domain::change_reason_category{.version = 0,
                                           .code = std::move(code),
...
void change_reason_categories_commands::process_delete_category(std::ostream& out,
                                                                nats_client& session,
                                                                std::string code) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete category request for: " << code;
...
void change_reason_categories_commands::process_get_category_history(std::ostream& out,
                                                                     nats_client& session,
                                                                     std::string code) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get category history for: " << code;
