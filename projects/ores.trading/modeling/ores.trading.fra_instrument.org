...
#+description: Codegen entity model for the trading =fra_instrument= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
