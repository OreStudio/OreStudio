...
void write_currency_asset_class(std::vector<std::byte>& buffer,
    const domain::currency_asset_class& rt) {
...
std::expected<domain::currency_asset_class, error_code>
read_currency_asset_class(std::span<const std::byte>& data) {
    domain::currency_asset_class rt;
...
std::vector<std::byte> get_currency_asset_classes_request::serialize() const {
    return {};
}

std::expected<get_currency_asset_classes_request, error_code>
get_currency_asset_classes_request::deserialize(std::span<const std::byte> data) {
    if (!data.empty()) {
        return std::unexpected(error_code::payload_too_large);
    }
    return get_currency_asset_classes_request{};
}
...
std::vector<std::byte> get_currency_asset_classes_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& rt : types) {
        write_currency_asset_class(buffer, rt);
    }
    return buffer;
}

std::expected<get_currency_asset_classes_response, error_code>
get_currency_asset_classes_response::deserialize(std::span<const std::byte> data) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        auto result = read_currency_asset_class(data);
...
std::vector<std::byte> save_currency_asset_class_request::serialize() const {
    std::vector<std::byte> buffer;
    write_currency_asset_class(buffer, type);
    return buffer;
}

std::expected<save_currency_asset_class_request, error_code>
save_currency_asset_class_request::deserialize(std::span<const std::byte> data) {
    save_currency_asset_class_request request;
...
std::vector<std::byte> save_currency_asset_class_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_currency_asset_class_response, error_code>
save_currency_asset_class_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> delete_currency_asset_class_request::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_currency_asset_class_request, error_code>
delete_currency_asset_class_request::deserialize(std::span<const std::byte> data) {
    delete_currency_asset_class_request request;
...
std::vector<std::byte> delete_currency_asset_class_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_currency_asset_class_response, error_code>
delete_currency_asset_class_response::deserialize(std::span<const std::byte> data) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        delete_currency_asset_class_result r;
