...
(defun ores/define-nats-service ()
  "Register the NATS server as a prodigy service for this checkout.
Uses ORES_CHECKOUT_LABEL and ORES_NATS_PORT from .env to locate the
per-environment config generated by init-nats.sh."
  (let* ((dotenv  (ores/load-dotenv-for-prodigy))
         (label   (or (cdr (assoc "ORES_CHECKOUT_LABEL" dotenv))
                      ores/checkout-label))
         (config  (expand-file-name
                   (format "build/config/nats-%s.conf" label)
                   ores/project-root))
         (port    (or (cdr (assoc "ORES_NATS_PORT" dotenv)) "4222")))
    (prodigy-define-service
      :name    (format "ORE Studio NATS [%s] (port %s)" label port)
      :cwd     ores/project-root
      :command "nats-server"
      :args    `("--config" ,config)
      :tags    `(ores ,ores/checkout-tag nats-server)
      :on-output (lambda (&rest args)
                   (when (string-match-p "Server is ready"
                                         (plist-get args :output))
                     (prodigy-set-status (plist-get args :service) 'ready)))
      :stop-signal 'sigint
      :kill-process-buffer-on-stop t)))
...
;; Auto-register the NATS server service (checkout-level, not preset-specific).
(ores/define-nats-service)

;; Auto-register services for the preset configured in .env (set by
;; compass env init --preset <preset>).
(let* ((dotenv (ores/load-dotenv-for-prodigy))
