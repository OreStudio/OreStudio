...
- The full tree built and =check_component_drift.py --all= was green while
  synthetic was registered. The entry is now held back, because the live
  half still fails: the component's ctest suites (V02), the database
  recreate (V03) and the generated shell commands against the live fleet
  (V04). The fleet is up from this worktree; only the recreate is unrun.
