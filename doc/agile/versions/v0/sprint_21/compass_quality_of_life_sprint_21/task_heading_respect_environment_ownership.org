...
:END:
#+title: Task: compass heading: skip stories owned by another environment
#+description: compass heading recommends tasks regardless of which environment started them. Tasks and stories are stamped with #+environment: at task start. heading should read this field and deprioritise (or skip) items whose environment differs from the current ORES_CHECKOUT_LABEL, so two environments working the same sprint do not compete for the same stories.
#+type: task
#+level: s1
