...
fra_convention_repository::replace_claim(context ctx, const domain::fra_convention& v) {
...
fra_convention_repository::apply_claim(context ctx,
                                       const domain::fra_convention& v,
...
void fra_convention_repository::write(context ctx,
                                      const domain::fra_convention& v,
                                      const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing FRA convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, fra_convention_mapper::map(t), lg(), "Writing FRA convention to database.");
}

void fra_convention_repository::write(
    context ctx,
    const std::vector<domain::fra_convention>& v,
    ...
    execute_write_query(
        ctx, fra_convention_mapper::map(batch), lg(), "Writing FRA conventions to database.");
}
...
std::optional<domain::fra_convention> fra_convention_repository::read_at_version(
    context ctx, const std::string& id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading FRA convention at version. " << "id: " << id
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<fra_convention_entity>> |
    ...
    const auto entities = execute_read_query<fra_convention_entity, domain::fra_convention>(
        ctx,
        query,
        [](const auto& entities) { return fra_convention_mapper::map(entities); },
        lg(),
        "Reading FRA convention at version.");
...
fra_convention_repository::remove_status fra_convention_repository::remove(
...
fra_convention_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FRA conventions with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    ...
    return execute_read_query<fra_convention_entity, domain::fra_convention>(
        ...
        [](const auto& entities) { return fra_convention_mapper::map(entities); },
        lg(),
        "Reading latest FRA conventions with pagination.");
}

std::uint32_t fra_convention_repository::get_total_fra_convention_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active FRA convention count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<fra_convention_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active FRA convention count: " << count;
    return count;
}

std::vector<domain::fra_convention>
fra_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<fra_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<fra_convention_entity, domain::fra_convention>(
        ctx,
        query,
        [](const auto& entities) { return fra_convention_mapper::map(entities); },
        lg(),
        "Reading latest FRA conventions by ids.");
    return result;
}

void fra_convention_repository::remove(context ctx, const std::vector<std::string>& ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<fra_convention_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing FRA conventions.");
}
