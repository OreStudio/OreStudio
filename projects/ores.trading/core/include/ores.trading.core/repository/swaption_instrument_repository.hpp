 ...
 *
 */
#ifndef ORES_TRADING_CORE_REPOSITORY_SWAPTION_INSTRUMENT_REPOSITORY_HPP
#define ORES_TRADING_CORE_REPOSITORY_SWAPTION_INSTRUMENT_REPOSITORY_HPP
 ...
 */
class ORES_TRADING_CORE_EXPORT swaption_instrument_repository {
private:
...
public:
    ...
    /**
     * @brief Writes swaption instruments to database.
     */
    /**@{*/
    ...
    /**
     * @brief Reads latest swaption instruments, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all swaption instruments, possibly filtered by primary key.
     */
    std::vector<domain::swaption_instrument> read_all(context ctx,
                                                      const std::string& instrument_id);

    /**
     * @brief Reads a single swaption instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest swaption instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_swaption_instrument_count(context ctx);

    /**
     * @brief Deletes a swaption instrument by closing its temporal validity.
     */
    void remove(context ctx, const std::string& instrument_id);

    /**
     * @brief Deletes swaption instruments by closing their temporal validity.
     */
