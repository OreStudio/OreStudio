...
ClientTreatmentDimensionModel::ClientTreatmentDimensionModel(ClientManager* clientManager,
                                                             QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ClientTreatmentDimensionModel::onDimensionsLoaded);

    connect(pulseManager_,
            &RecencyPulseManager::pulse_state_changed,
            this,
            &ClientTreatmentDimensionModel::onPulseStateChanged);
    connect(pulseManager_,
            &RecencyPulseManager::pulsing_complete,
            this,
            &ClientTreatmentDimensionModel::onPulsingComplete);
}
...
QVariant ClientTreatmentDimensionModel::data(const QModelIndex& index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            case Code:
                return QString::fromStdString(dim.code);
            case Name:
                return QString::fromStdString(dim.name);
            case Description:
                return QString::fromStdString(dim.description);
            case Version:
                return dim.version;
            case ModifiedBy:
                return QString::fromStdString(dim.modified_by);
            case RecordedAt:
                return relative_time_helper::format(dim.recorded_at);
            default:
                return {};
        }
    }
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << response_result->treatment_dimensions.size()
                    << " treatment dimensions";
                return {.success = true,
                        .dimensions = std::move(response_result->treatment_dimensions),
                        .error_message = {},
                        .error_details = {}};
            },
            "treatment dimensions");
...
void ClientTreatmentDimensionModel::onDimensionsLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch treatment dimensions: " << result.error_message.toStdString();
    ...
    BOOST_LOG_SEV(lg(), debug) << "Loaded " << dimensions_.size() << " treatment dimensions";
    emit dataLoaded();
}

const dq::domain::treatment_dimension* ClientTreatmentDimensionModel::getDimension(int row) const {
    if (row < 0 || row >= static_cast<int>(dimensions_.size()))
        return nullptr;
    return &dimensions_[row];
}

QVariant ClientTreatmentDimensionModel::recency_foreground_color(const std::string& code) const {
