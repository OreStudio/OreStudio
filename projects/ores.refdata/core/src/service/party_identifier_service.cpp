 ...
 */
std::vector<domain::party_identifier> read_one(repository::party_identifier_repository& repo,
                                               const ores::database::context& ctx,
                                               const messaging::party_identifier_key& key) {
 ...
 */
messaging::party_identifier_key key_from(const domain::party_identifier& v) {
...
messaging::list_by_party_id_party_identifiers_response
...
messaging::get_party_identifier_response party_identifier_service::get_party_identifier(
    const messaging::get_party_identifier_request& request) {
    messaging::get_party_identifier_response response;
    ...
    response.party_identifier = std::move(found.front());
    return response;
}

messaging::get_many_party_identifiers_response party_identifier_service::get_many_party_identifiers(
    const messaging::get_many_party_identifiers_request& request) {
    messaging::get_many_party_identifiers_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_identifier_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party_identifier = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_party_identifier_response party_identifier_service::put_party_identifier(
    const messaging::put_party_identifier_request& request) {
    messaging::put_party_identifier_response response;
    domain::party_identifier value;
    ...
    if (!written.empty())
        response.party_identifier = std::move(written.front());
    return response;
}

messaging::put_many_party_identifiers_response party_identifier_service::put_many_party_identifiers(
    const messaging::put_many_party_identifiers_request& request) {
    messaging::put_many_party_identifiers_response response;
    ...
    response.party_identifiers.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.party_identifiers.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_party_identifier_response party_identifier_service::delete_party_identifier(
    const messaging::delete_party_identifier_request& request) {
    messaging::delete_party_identifier_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::party_identifier_repository::remove_status::removed:
            break;
        case repository::party_identifier_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_identifier_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_identifier_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
party_identifier_service::delete_many_party_identifiers(
    const messaging::delete_many_party_identifiers_request& request) {
    messaging::delete_many_party_identifiers_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_party_identifier_versions_response
party_identifier_service::list_party_identifier_versions(
    const messaging::list_party_identifier_versions_request& request) {
    messaging::list_party_identifier_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
party_identifier_service::get_party_identifier_version(
    const messaging::get_party_identifier_version_request& request) {
    messaging::get_party_identifier_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.party_identifier.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
party_identifier_service::prepare_change(const messaging::party_identifier_change& change,
...
std::uint32_t
party_identifier_service::count_party_identifiers_by_party_id(const boost::uuids::uuid& party_id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting total party identifiers count by party_id: " << party_id;
    return repo_.get_total_party_identifier_count_by_party_id(ctx_,
...
party_identifier_service::find_party_identifier(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Finding party identifier. " << "id: " << id;
...
std::optional<domain::party_identifier>
party_identifier_service::find_party_identifier_by_code(const boost::uuids::uuid& party_id,
                                                        const std::string& id_scheme) {
    BOOST_LOG_SEV(lg(), debug) << "Finding party identifier by party_id/id_scheme: " << party_id
                               << "/" << id_scheme;
    auto results = repo_.read_latest_by_code(ctx_, boost::uuids::to_string(party_id), id_scheme);
    if (results.empty())
        return std::nullopt;
    return results.front();
}
...
std::vector<domain::party_identifier>
party_identifier_service::get_party_identifiers(const std::vector<std::string>& ids) {
