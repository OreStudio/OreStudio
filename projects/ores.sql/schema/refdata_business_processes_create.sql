...
create table if not exists "ores"."refdata_business_processes_tbl" (
    "code" text not null,
...
on "ores"."refdata_business_processes_tbl" (code, version)
where valid_to = ores.utility_infinity_timestamp_fn();

create index if not exists refdata_business_processes_coding_scheme_idx
on "ores"."refdata_business_processes_tbl" (coding_scheme_code)
where valid_to = ores.utility_infinity_timestamp_fn();

create or replace function ores.refdata_business_processes_insert_fn()
returns trigger as $$
declare
...
before insert on "ores"."refdata_business_processes_tbl"
for each row
execute function ores.refdata_business_processes_insert_fn();

create or replace rule refdata_business_processes_delete_rule as
on delete to "ores"."refdata_business_processes_tbl"
do instead
  update "ores"."refdata_business_processes_tbl"
  set valid_to = current_timestamp
  where code = old.code
