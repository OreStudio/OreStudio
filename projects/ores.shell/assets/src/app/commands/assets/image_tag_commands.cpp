...
void image_tag_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("image_tags");
    ...
    menu->Insert(
        ...
        "add <image_id> <tag_id> <assigned_by> <assigned_at> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <image_id> <tag_id> <assigned_by> <assigned_at> <reason> <commentary> [--version "
        "<n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <image_id> <tag_id> <assigned_by> <assigned_at> <reason> "
        "<commentary>");
    ...
    menu->Insert(
        ...
        "delete <image_id> <tag_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <image_id> <tag_id> <reason> <commentary>");

    menu->Insert(
        "by-image-id",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_image_id(std::ref(out), std::ref(session), std::move(args));
        },
        "by-image-id <image_id> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");

    root_menu.Insert(std::move(menu));
}

void image_tag_commands::process_list(std::ostream& out,
                                      nats_client& session,
    ...
    auto result = do_auth_request<messaging::list_image_tags_response>(
...
void image_tag_commands::process_get(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    ...
    try {
        ...
        read_token(req.key.image_id, parsed->positionals[next++], "image_id");
        read_token(req.key.tag_id, parsed->positionals[next++], "tag_id");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_image_tag_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_get_many(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            ...
            read_token(key.image_id, parsed->positionals[i + 0], "image_id");
            read_token(key.tag_id, parsed->positionals[i + 1], "tag_id");
    ...
    auto result = do_auth_request<messaging::get_many_image_tags_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_add(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    ...
    try {
        ...
        read_token(req.change.write.tag_id, parsed->positionals[next++], "tag_id");
        read_token(req.change.write.assigned_by, parsed->positionals[next++], "assigned_by");
        read_token(req.change.write.assigned_at, parsed->positionals[next++], "assigned_at");
    ...
    auto result = do_auth_request<messaging::put_image_tag_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_set(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
...
void image_tag_commands::process_put_many(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::image_tag_change change;
            read_token(change.write.image_id, parsed->positionals[next++], "image_id");
            read_token(change.write.tag_id, parsed->positionals[next++], "tag_id");
            read_token(change.write.assigned_by, parsed->positionals[next++], "assigned_by");
            read_token(change.write.assigned_at, parsed->positionals[next++], "assigned_at");
    ...
    auto result = do_auth_request<messaging::put_many_image_tags_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_delete(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    ...
    try {
        ...
        read_token(req.removal.key.image_id, parsed->positionals[next++], "image_id");
        read_token(req.removal.key.tag_id, parsed->positionals[next++], "tag_id");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_image_tag_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_delete_many(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::image_tag_key key;
            read_token(key.image_id, parsed->positionals[i * 2 + 0], "image_id");
            read_token(key.tag_id, parsed->positionals[i * 2 + 1], "tag_id");
            req.removals.push_back(messaging::image_tag_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_image_tags_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_tag_commands::process_by_image_id(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-image-id request.";

    using request_type = messaging::list_by_image_id_image_tags_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-image-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.image_id = ores::shell::app::from_token<boost::uuids::uuid>(parsed->positionals[next++],
                                                                        "image_id");
    ...
    auto result = do_auth_request<messaging::list_by_image_id_image_tags_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
