 ...
 * straight through.
 */
std::vector<domain::party_currency> read_one(repository::party_currency_repository& repo,
                                             const messaging::party_currency_key& key) {
    return repo.read_latest(key.party_id, key.currency_iso_code);
}
 ...
 */
messaging::party_currency_key key_from(const domain::party_currency& v) {
 ...
 */
domain::party_currency to_domain(const messaging::party_currency_write& write) {
    domain::party_currency v;
...
messaging::list_party_currencies_response party_currency_service::list_party_currencies(
    ...
    messaging::list_party_currencies_response response;
    ...
    response.party_currencies = repo_.read_latest(request.offset, request.limit);
    response.total = repo_.get_total_party_currency_count();
    return response;
}

messaging::list_by_party_id_party_currencies_response
party_currency_service::list_by_party_id_party_currencies(
    const messaging::list_by_party_id_party_currencies_request& request) {
    messaging::list_by_party_id_party_currencies_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
    ...
    response.party_currencies =
        repo_.read_latest_by_party(request.party_id, request.offset, request.limit);
    response.total = repo_.get_total_party_currency_count_by_party(request.party_id);
    return response;
}

messaging::get_party_currency_response
party_currency_service::get_party_currency(const messaging::get_party_currency_request& request) {
    messaging::get_party_currency_response response;
    ...
    response.party_currency = std::move(found.front());
    return response;
}

messaging::get_many_party_currencies_response party_currency_service::get_many_party_currencies(
    const messaging::get_many_party_currencies_request& request) {
    messaging::get_many_party_currencies_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_currency_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, k);
        if (!found.empty())
            entry.party_currency = std::move(found.front());
...
party_currency_service::put_party_currency(const messaging::put_party_currency_request& request) {
    messaging::put_party_currency_response response;
    domain::party_currency value;
    ...
    if (!written.empty())
        response.party_currency = std::move(written.front());
    return response;
}

messaging::put_many_party_currencies_response party_currency_service::put_many_party_currencies(
    const messaging::put_many_party_currencies_request& request) {
    messaging::put_many_party_currencies_response response;
    ...
    response.party_currencies.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, key_from(value));
        response.party_currencies.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_party_currency_response party_currency_service::delete_party_currency(
    const messaging::delete_party_currency_request& request) {
    messaging::delete_party_currency_response response;
    ...
    switch (repo_.remove(
        request.removal.key.party_id, request.removal.key.currency_iso_code, expected)) {
        case repository::party_currency_repository::remove_status::removed:
            break;
        case repository::party_currency_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_currency_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_currency_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
party_currency_service::delete_many_party_currencies(
    const messaging::delete_many_party_currencies_request& request) {
    messaging::delete_many_party_currencies_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
    ...
    repo_.remove(party_id_keys, currency_iso_code_keys);
    return response;
}

ores::utility::domain::result
party_currency_service::prepare_change(const messaging::party_currency_change& change,
