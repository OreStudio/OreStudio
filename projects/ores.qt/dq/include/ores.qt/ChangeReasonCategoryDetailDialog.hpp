...
/**
 * @brief Detail dialog for viewing and editing change reason category records.
 *
 * This dialog allows viewing, creating, and editing change reason categories.
 * It supports both create mode (for new records) and edit mode (for
 * existing records).
 */
class ChangeReasonCategoryDetailDialog final : public DetailDialogBase {
    Q_OBJECT
...
public:
    ...
    void setCategory(const dq::domain::change_reason_category& item);
...
private:
    ...
    void updateUiFromCategory();
    void updateCategoryFromUi();
    ...
    dq::domain::change_reason_category item_;
