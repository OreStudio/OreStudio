...
create unique index if not exists ores_refdata_currency_market_tiers_version_uniq_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_refdata_currency_market_tiers_code_uniq_idx
on "ores_refdata_currency_market_tiers_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_currency_market_tiers_tenant_idx
...
for each row
execute function ores_refdata_currency_market_tiers_insert_fn();

create or replace rule ores_refdata_currency_market_tiers_delete_rule as
on delete to "ores_refdata_currency_market_tiers_tbl"
do instead
  update "ores_refdata_currency_market_tiers_tbl"
  set valid_to = current_timestamp
...
begin
    ...
    if not exists (select 1 from ores_refdata_currency_market_tiers_tbl
