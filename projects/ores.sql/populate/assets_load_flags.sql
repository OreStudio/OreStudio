...
INSERT INTO ores.assets_tags_tbl (tag_id, version, name, description, modified_by,
...
WHERE NOT EXISTS (
    SELECT 1 FROM ores.assets_tags_tbl
...
--
-- SELECT ores.assets_load_flag_fn(
--     'ro',
...
--     svg=$(cat "$f" | sed "s/'/''/g")
--     echo "SELECT ores.assets_load_flag_fn('$key', '$desc', '$svg');"
-- done > projects/ores.sql/load_flags_data.sql
-- echo "SELECT 'Loaded ' || COUNT(*) || ' flags' AS summary FROM ores.assets_image_tags_tbl it JOIN ores.assets_tags_tbl t ON it.tag_id = t.tag_id WHERE t.name = 'flag' AND t.valid_to = ores.utility_infinity_timestamp_fn() AND it.valid_to = ores.utility_infinity_timestamp_fn();" >> projects/ores.sql/load_flags_data.sql
--
