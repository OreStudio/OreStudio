...
create unique index if not exists datasets_name_uniq_idx
on "ores_dq_datasets_tbl" (tenant_id, name, subject_area_name, domain_name)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists datasets_code_uniq_idx
on "ores_dq_datasets_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists datasets_version_uniq_idx
on "ores_dq_datasets_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists datasets_tenant_idx
