...
Each case pins one thing the unit promises. Every derived verb appears in
the menu's completion list, which is the only public view of the menu's
children. The add verb refuses a signed-out session
before it reads a token. It reports the arity it expects and the count it
was given. It names the
