...
ResultMdiWindow::ResultMdiWindow(ClientManager* clientManager,
...
void ResultMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &ResultMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &ResultMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &ResultMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &ResultMdiWindow::viewHistorySelected);
}
...
void ResultMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "ResultListWindow", {}, {900, 400}, 1);
}

void ResultMdiWindow::setupConnections() {
    connect(model_, &ClientResultModel::dataLoaded, this, &ResultMdiWindow::onDataLoaded);
    connect(model_, &ClientResultModel::loadError, this, &ResultMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &ResultMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &ResultMdiWindow::onDoubleClicked);
...
void ResultMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void ResultMdiWindow::viewHistorySelected() {
    ...
    if (auto* result = model_->getResult(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug)
            << "Emitting showResultHistory for code: " << result->modified_by;
...
void ResultMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete compute result while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* result = model_->getResult(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(result->id));
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " compute results";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 compute results?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Result", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    QPointer<ResultMdiWindow> self = this;
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " compute results";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                BOOST_LOG_SEV(lg(), debug) << "Result deleted: " << code;
                success_count++;
                emit self->resultDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error)
                    << "Result deletion failed: " << code << " - " << message;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              "Successfully deleted 1 compute result" :
                              QString("Successfully deleted %1 compute results").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "compute result" : "compute results")
                              .arg(first_error);
