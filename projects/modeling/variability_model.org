...
drawer. See [[*Element-scope activation (physical-space binding)]] for the full
address hierarchy, specificity resolution rules, and examples.

** Element-scope activation (physical-space binding)

The model-type filter governs which profiles /can/ apply to a model class; element-scope
activation governs which profiles /do/ apply to a specific entity, overriding the defaults.

Entity model files declare =ores.*= properties in their =:PROPERTIES:= drawer following the
same convention Dogen uses for =:masd.cpp.enabled:= and =:masd.csharp.enabled:=:
...
| Address | Scope | Effect |
|---------+-------+--------|
| =:ores.sql.enabled: false= | technical space | Disables all SQL facets (=sql=, =non-temporal-sql=) |
| =:ores.cpp.enabled: false= | technical space | Disables all C++ facets (=domain=, =repository=, =qt=, …) |
| =:ores.cpp.qt.enabled: false= | facet | Disables only the =qt= facet |
| =:ores.cpp.domain.class_header.enabled: false= | archetype | Disables a single archetype |

*Specificity resolution.* More-specific addresses override less-specific ones, processed in
address-depth order. Setting =:ores.cpp.enabled: true= followed by =:ores.cpp.qt.enabled:
false= in the same drawer enables all C++ facets except Qt. This mirrors Dogen's property
resolution semantics exactly. Absence of any =ores.*= property means the entity uses the
profile's =model_types= filter as the sole gate — backwards-compatible with existing models.

The technical-space to facet map (used for hierarchical resolution) lives in a
=* Technical spaces= section of =facet_catalogue.org=:
...
The resolution logic is implemented in a dedicated Python module (=codegen/physical_space.py=)
with its own unit tests — not inline in dispatch code. This keeps the activation semantics
independently verifiable.

*Replaces ad-hoc boolean flags.* A SQL-only entity is expressed as =:ores.cpp.enabled:
false=; an entity without Qt as =:ores.cpp.qt.enabled: false=. Adding a future facet (e.g.
=cpp.grpc=) requires no new flags — the address-based mechanism covers it automatically.
