 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_ZERO_CONVENTION_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_ZERO_CONVENTION_EVENT_HPP
...
/**
 * @brief Event traits for zero convention's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::zero_convention_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.zero_conventions_events";

    static ores::refdata::messaging::zero_convention_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::zero_convention_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::zero_convention_key>(notification.key))
            event.key = *key;
