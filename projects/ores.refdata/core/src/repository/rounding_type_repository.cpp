...
rounding_type_repository::replace_claim(context ctx, const domain::rounding_type& v) {
...
domain::rounding_type rounding_type_repository::apply_claim(
    context ctx, const domain::rounding_type& v, const ores::utility::domain::precondition& claim) {
...
void rounding_type_repository::write(context ctx,
                                     const domain::rounding_type& v,
                                     const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing rounding type. " << "code: " << v.code;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, rounding_type_mapper::map(t), lg(), "Writing rounding type to database.");
}
...
void rounding_type_repository::write(
    ...
    execute_write_query(
        ctx, rounding_type_mapper::map(batch), lg(), "Writing rounding types to database.");
}
...
rounding_type_repository::remove_status rounding_type_repository::remove(
...
std::vector<domain::rounding_type>
rounding_type_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<rounding_type_entity, domain::rounding_type>(
        ctx,
        query,
        [](const auto& entities) { return rounding_type_mapper::map(entities); },
        lg(),
        "Reading latest rounding types by ids.");
    return result;
}
