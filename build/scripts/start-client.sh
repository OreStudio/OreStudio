...
#   --colour COLOUR    Window marker colour: red, green, blue, or a 6-digit hex value (e.g. FF5733)
#   --name NAME        Instance display name (defaults to ORES_CHECKOUT_LABEL from .env)
#   --log-level LEVEL  Log level: trace|debug|info|warn|error (default: debug)
#
# Requires a .env with ORES_ENV_VERSION >= 1 (generated by compass env init).
# Exits with an error if the .env is older than the required version.
#
...
# Use --preset to start against a specific preset without re-running compass env init.
...
# Fails with a clear message if .env is older than the required version.
...
# --- Instance name comes from --name, else ORES_CHECKOUT_LABEL in .env ---
# The colour is only a window-distinguishing marker; it never sets the name.
