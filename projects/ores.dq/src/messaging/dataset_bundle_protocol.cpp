...
save_dataset_bundle_request
save_dataset_bundle_request::from(domain::dataset_bundle bundle) {
    return save_dataset_bundle_request{
        std::vector<domain::dataset_bundle>{std::move(bundle)}};
}

save_dataset_bundle_request
save_dataset_bundle_request::from(std::vector<domain::dataset_bundle> bundles) {
    return save_dataset_bundle_request{std::move(bundles)};
}
...
save_dataset_bundle_request::deserialize(std::span<const std::byte> data) {
    ...
    request.bundles.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_dataset_bundle(data);
        if (!e) return std::unexpected(e.error());
        request.bundles.push_back(std::move(*e));
    }
    return request;
}
