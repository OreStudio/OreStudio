 ...
 */
std::vector<domain::calendar_exception> read_one(repository::calendar_exception_repository& repo,
                                                 const ores::database::context& ctx,
                                                 const messaging::calendar_exception_key& key) {
 ...
 */
messaging::calendar_exception_key key_from(const domain::calendar_exception& v) {
    messaging::calendar_exception_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::calendar_exception to_domain(const messaging::calendar_exception_write& write) {
    domain::calendar_exception v;
    v.id = write.id;
    v.calendar_code = write.calendar_code;
    v.exception_date = write.exception_date;
    v.is_business_day = write.is_business_day;
...
messaging::list_calendar_exceptions_response calendar_exception_service::list_calendar_exceptions(
    ...
    messaging::list_calendar_exceptions_response response;
    ...
    response.calendar_exceptions = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_calendar_exception_count(ctx_);
    return response;
}

messaging::list_by_calendar_code_calendar_exceptions_response
calendar_exception_service::list_by_calendar_code_calendar_exceptions(
    const messaging::list_by_calendar_code_calendar_exceptions_request& request) {
    messaging::list_by_calendar_code_calendar_exceptions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
    ...
    response.calendar_exceptions =
        repo_.read_latest_by_calendar_code(ctx_, relation, request.offset, request.limit);
    response.total = repo_.get_total_calendar_exception_count_by_calendar_code(ctx_, relation);
    return response;
}

messaging::get_calendar_exception_response calendar_exception_service::get_calendar_exception(
    const messaging::get_calendar_exception_request& request) {
    messaging::get_calendar_exception_response response;
    ...
    response.calendar_exception = std::move(found.front());
    return response;
}

messaging::get_many_calendar_exceptions_response
calendar_exception_service::get_many_calendar_exceptions(
    const messaging::get_many_calendar_exceptions_request& request) {
    messaging::get_many_calendar_exceptions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::calendar_exception_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.calendar_exception = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_calendar_exception_response calendar_exception_service::put_calendar_exception(
    const messaging::put_calendar_exception_request& request) {
    messaging::put_calendar_exception_response response;
    domain::calendar_exception value;
    ...
    if (!written.empty())
        response.calendar_exception = std::move(written.front());
    return response;
}

messaging::put_many_calendar_exceptions_response
calendar_exception_service::put_many_calendar_exceptions(
    const messaging::put_many_calendar_exceptions_request& request) {
    messaging::put_many_calendar_exceptions_response response;
    std::vector<domain::calendar_exception> batch;
    ...
    response.calendar_exceptions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.calendar_exceptions.push_back(written.empty() ? value :
                                                                 std::move(written.front()));
    }
    return response;
}

messaging::delete_calendar_exception_response calendar_exception_service::delete_calendar_exception(
    const messaging::delete_calendar_exception_request& request) {
    messaging::delete_calendar_exception_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::calendar_exception_repository::remove_status::removed:
            break;
        case repository::calendar_exception_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::calendar_exception_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::calendar_exception_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_calendar_exceptions_response
calendar_exception_service::delete_many_calendar_exceptions(
    const messaging::delete_many_calendar_exceptions_request& request) {
    messaging::delete_many_calendar_exceptions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_calendar_exception_versions_response
calendar_exception_service::list_calendar_exception_versions(
    const messaging::list_calendar_exception_versions_request& request) {
    messaging::list_calendar_exception_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_calendar_exception_version_response
calendar_exception_service::get_calendar_exception_version(
    const messaging::get_calendar_exception_version_request& request) {
    messaging::get_calendar_exception_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.calendar_exception.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
calendar_exception_service::prepare_change(const messaging::calendar_exception_change& change,
...
std::vector<domain::calendar_exception>
calendar_exception_service::get_calendar_exceptions(const std::vector<std::string>& ids) {
