...
void fx_digital_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    fx_digital_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_fx_digital_option_instrument(std::ref(out),
                                                     std::ref(session),
                                                     std::move(trade_type_code),
                                                     std::move(foreign_currency),
                                                     std::move(domestic_currency),
                                                     std::move(payoff_currency),
                                                     payoff_amount,
        ...
        "Add an FX digital option instrument (trade_type_code foreign_currency domestic_currency "
        "payoff_currency payoff_amount option_type expiry_date long_short strike barrier_type "
        "lower_barrier upper_barrier [description] change_reason_code \"change_commentary\")",
        {"trade_type_code foreign_currency domestic_currency payoff_currency payoff_amount "
         "option_type expiry_date long_short strike barrier_type lower_barrier upper_barrier "
...
void fx_digital_option_instrument_commands::process_get_fx_digital_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX digital option instruments request.";
...
void fx_digital_option_instrument_commands::process_add_fx_digital_option_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an FX digital option instrument." << std::endl;
        return;
    }
    ...
    v.foreign_currency = std::move(foreign_currency);
    v.domestic_currency = std::move(domestic_currency);
    v.payoff_currency = std::move(payoff_currency);
    v.payoff_amount = payoff_amount;
