 ...
 */
#ifndef ORES_SYNTHETIC_CORE_REPOSITORY_YIELD_CURVE_PROCESS_TYPE_REPOSITORY_HPP
#define ORES_SYNTHETIC_CORE_REPOSITORY_YIELD_CURVE_PROCESS_TYPE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes yield curve process types to data storage.
 */
class ORES_SYNTHETIC_CORE_EXPORT yield_curve_process_type_repository {
private:
    inline static std::string_view logger_name =
        "ores.synthetic.repository.yield_curve_process_type_repository";
...
public:
    ...
    /**
     * @brief Writes yield curve process types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::yield_curve_process_type& v);
    void write(context ctx, const std::vector<domain::yield_curve_process_type>& v);
    /**@}*/

    /**
     * @brief Writes a yield curve process type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of yield curve process types, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::yield_curve_process_type>& v,
               const std::vector<ores::utility::domain::precondition>& claims);

    /**
     * @brief Reads latest yield curve process types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::yield_curve_process_type> read_latest(context ctx);
     ...
     */
    std::vector<domain::yield_curve_process_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single yield curve process type as it stood at a specific
    ...
    /**
     * @brief Reads latest yield curve process types with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_process_type_count(context ctx);

    /**
     * @brief Deletes a yield curve process type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a yield curve process type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes yield curve process types by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::yield_curve_process_type apply_claim(context ctx,
                                                 const domain::yield_curve_process_type& v,
                                                 const ores::utility::domain::precondition& claim);
};
