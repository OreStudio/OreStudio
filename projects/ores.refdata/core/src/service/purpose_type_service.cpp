 ...
 */
std::vector<domain::purpose_type> read_one(repository::purpose_type_repository& repo,
                                           const ores::database::context& ctx,
                                           const messaging::purpose_type_key& key) {
 ...
 */
messaging::purpose_type_key key_from(const domain::purpose_type& v) {
    messaging::purpose_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::purpose_type to_domain(const messaging::purpose_type_write& write) {
    domain::purpose_type v;
    v.code = write.code;
    v.name = write.name;
...
purpose_type_service::list_purpose_types(const messaging::list_purpose_types_request& request) {
    messaging::list_purpose_types_response response;
...
messaging::get_purpose_type_response
purpose_type_service::get_purpose_type(const messaging::get_purpose_type_request& request) {
    messaging::get_purpose_type_response response;
    ...
    response.purpose_type = std::move(found.front());
    return response;
}

messaging::get_many_purpose_types_response purpose_type_service::get_many_purpose_types(
    const messaging::get_many_purpose_types_request& request) {
    messaging::get_many_purpose_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::purpose_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.purpose_type = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_purpose_type_response
purpose_type_service::put_purpose_type(const messaging::put_purpose_type_request& request) {
    messaging::put_purpose_type_response response;
    domain::purpose_type value;
    ...
    if (!written.empty())
        response.purpose_type = std::move(written.front());
    return response;
}

messaging::put_many_purpose_types_response purpose_type_service::put_many_purpose_types(
    const messaging::put_many_purpose_types_request& request) {
    messaging::put_many_purpose_types_response response;
    std::vector<domain::purpose_type> batch;
...
messaging::delete_purpose_type_response
purpose_type_service::delete_purpose_type(const messaging::delete_purpose_type_request& request) {
    messaging::delete_purpose_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::purpose_type_repository::remove_status::removed:
            break;
        case repository::purpose_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::purpose_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::purpose_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_purpose_types_response purpose_type_service::delete_many_purpose_types(
    const messaging::delete_many_purpose_types_request& request) {
    messaging::delete_many_purpose_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_purpose_type_versions_response purpose_type_service::list_purpose_type_versions(
    const messaging::list_purpose_type_versions_request& request) {
    messaging::list_purpose_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_purpose_type_version_response purpose_type_service::get_purpose_type_version(
    const messaging::get_purpose_type_version_request& request) {
    messaging::get_purpose_type_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.purpose_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
purpose_type_service::prepare_change(const messaging::purpose_type_change& change,
                                     const ores::utility::domain::change_intent& intent,
                                     domain::purpose_type& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
purpose_type_service::get_types(const std::vector<std::string>& codes) {
