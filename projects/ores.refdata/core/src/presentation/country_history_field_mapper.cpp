...
std::vector<ores::diff::domain::field_value> render_country_fields(const domain::country& v) {
    using ores::diff::domain::field_value;
    std::vector<field_value> fields;

    fields.push_back({.name = "Alpha2 Code", .value = v.alpha2_code});
    fields.push_back({.name = "Alpha3 Code", .value = v.alpha3_code});
    fields.push_back({.name = "Numeric Code", .value = v.numeric_code});
    fields.push_back({.name = "Name", .value = v.name});
    fields.push_back({.name = "Official Name", .value = v.official_name});
