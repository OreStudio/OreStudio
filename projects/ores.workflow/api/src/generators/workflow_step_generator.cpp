...
generate_synthetic_workflow_step(utility::generation::generation_context& ctx) {
    ...
    r.compensation_subject = std::string(faker::word::noun());
    r.compensation_json = std::string(faker::word::noun());
...
generate_synthetic_workflow_steps(std::size_t n, utility::generation::generation_context& ctx) {
    std::vector<domain::workflow_step> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_workflow_step(ctx));
    return r;
}
