...
QString CounterpartyHistoryDialog::code() const {
    return code_;
}
...
void CounterpartyHistoryDialog::loadHistory() {
    ...
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_counterparty_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
HistoryDialogBase::DiffResult CounterpartyHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
