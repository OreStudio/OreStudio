...
| [[id:3B8F2A1D-7C4E-4B6A-9E5D-2F1A8C3B7D9E][Refactor ores.codegen C++ generation]] | STARTED | 2026-05-30 | | Audit and resolve drift between C++ templates and production files: fix template bugs, fix model consistency, register all C++ components in the CLI, achieve zero-diff regeneration across all domain entities. |
...
| [[id:649C97F1-EB47-4CA8-A3AE-AA7D78FCD46D][Entity classification and drift baseline: ores.marketdata]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind marketdata entities to profiles, verify zero-diff regen, extend the drift gate. |
| [[id:9ABCB932-9501-4519-BECA-A30A93646770][Entity classification and drift baseline: ores.analytics]] | STARTED | 2026-09-17 | | Run the compass-codegen-fix-drift method: bind analytics-cpp entities to profiles, verify zero-diff regen, extend the drift gate. The bind is done and merged; the story waits on the post-merge live system test. |
...
| [[id:62193FDE-57C0-40BB-83B7-0B3D6F8C38AA][Entity classification and drift baseline: ores.database]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind database-cpp entities to profiles, verify zero-diff regen, extend the drift gate. |
| [[id:3E49EAF7-0545-4A4C-9DD8-34608FC41293][Entity classification and drift baseline: ores.reporting]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind reporting-cpp entities to profiles, verify zero-diff regen, extend the drift gate. |
| [[id:040527BC-58FA-4A5E-9ADC-3814FA319F5C][Entity classification and drift baseline: ores.scheduler]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind scheduler-cpp entities to profiles, verify zero-diff regen, extend the drift gate. |
| [[id:8D56EA57-1963-41D6-90C1-335F64275644][Entity classification and drift baseline: ores.workflow]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind workflow-cpp entities to profiles, verify zero-diff regen, extend the drift gate. |
| [[id:6ADAA56E-EFA0-4E30-B381-4F20E3C9BA13][Entity classification and drift baseline: ores.workspace]] | BACKLOG | | | Run the compass-codegen-fix-drift method: bind workspace-cpp entities to profiles, verify zero-diff regen, extend the drift gate. |
...
| [[id:7C9B6A2E-F1D1-4BA2-9960-C36359B77B9A][Generate shell command units per component]] | BACKLOG | | | Land the =ores.cpp.shell-command= facet and generate each component's shell command units into its own adapter part of =ores.shell=, retiring the hand-written units component by component. =ores.trading= goes first: its units land in the new =ores.shell.trading= part. |
...
| [[id:5FFB587D-D85A-4B51-8C80-0F8D485754A7][Migrate remaining ~100 entity models from JSON to org]] | BACKLOG | | | Complete the org-migration story: convert the remaining legacy JSON-format entity models to the literate .org entity-model format. |
...
| [[id:894B369A-CE00-4E6A-B71B-C7BAC8B158BE][Generate realistic trade populations for Acme using ORE's TradeGenerator]] | BLOCKED | 2026-09-07 | | Evaluate ORE's Python TradeGenerator as the source of Acme's synthetic trade population: generate ORE trade XML across asset classes, import it through the existing ores.ore mappers, and distribute the result across Acme's 36 portfolios in a way that is realistic rather than uniform. |
| [[id:A34F4835-5CA5-4F30-91D6-90975354534C][Update ORE to v17]] | STARTED | | | Sync the vendored ORE examples and XSD to v1.8.17.0, build and vendor the new engine package, and keep the v1.8.16.0 package in service beside it. |
...
| [[id:FA160AE1-CB9A-410F-A8B0-31FBF2247962][Generate protocol messages from one model into C++ and TypeScript]] | BACKLOG | | | Add an ores.codegen.operation model type so the hand-written IAM protocol headers generate from org with zero byte differences, then add a TypeScript target space so the TypeScript UI consumes the same model. |
