...
std::vector<ores::diff::domain::field_value> render_portfolio_fields(const domain::portfolio& v) {
    ...
    fields.push_back({.name = "Parent Portfolio ID",
                      .value = v.parent_portfolio_id ?
                                   boost::uuids::to_string(*v.parent_portfolio_id) :
                                   std::string{}});
    fields.push_back(
        {.name = "Owner Unit ID",
