...
on "ores_trading_bond_leg_amortizations_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_trading_bond_leg_amortizations_insert_fn()
returns trigger as $$
declare
...
create or replace trigger ores_trading_bond_leg_amortizations_insert_trg
before insert on "ores_trading_bond_leg_amortizations_tbl"
for each row execute function ores_trading_bond_leg_amortizations_insert_fn();

create or replace rule ores_trading_bond_leg_amortizations_delete_rule as
on delete to "ores_trading_bond_leg_amortizations_tbl" do instead (
    update "ores_trading_bond_leg_amortizations_tbl"
    set valid_to = clock_timestamp()
