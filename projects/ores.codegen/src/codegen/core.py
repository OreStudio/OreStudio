...
def resolve_output_path(output_pattern, model_data, model_type):
    ...
    if model_type == 'domain_entity' and 'domain_entity' in model_data:
        ...
        # by the dataset's model_name (e.g. demo_country_populate.sql); the
...
def generate_from_model(model_path, data_dir, templates_dir, output_dir, is_processing_batch=False, prefix=None, target_template=None, target_output=None, extra_model_paths=None):
    ...
    if is_domain_entity and isinstance(model, dict) and 'domain_entity' in model:
        ...
        # TypeScript domain member types, on the same field dicts the domain
        # template mirrors from cpp_domain_type_class.hpp.mustache: the
        # identity/audit prelude, the key lists and the plain columns. Read
        ...
        for _field in (
            ...
            + [domain_entity.get('primary_key') or {}]
        ):
