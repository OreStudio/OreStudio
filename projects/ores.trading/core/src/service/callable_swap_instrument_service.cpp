...
callable_swap_instrument_service::list_callable_swap_instruments() {
    BOOST_LOG_SEV(lg(), debug) << "Listing all callable_swap_instruments";
...
callable_swap_instrument_service::list_callable_swap_instruments(std::uint32_t offset,
                                                                 std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing callable_swap_instruments with offset=" << offset
                               << ", limit=" << limit;
...
std::uint32_t callable_swap_instrument_service::count_callable_swap_instruments() {
    BOOST_LOG_SEV(lg(), debug) << "Counting callable_swap_instruments";
    return static_cast<std::uint32_t>(repo_.read_latest(ctx_).size());
}

std::optional<domain::callable_swap_instrument>
callable_swap_instrument_service::get_callable_swap_instrument(const std::string& id) {
...
void callable_swap_instrument_service::save_callable_swap_instrument(
    ...
    if (v.identity.instrument_id.is_nil())
        throw std::invalid_argument("Callable swap instrument id cannot be empty.");
    BOOST_LOG_SEV(lg(), debug) << "Saving callable_swap_instrument: " << v.identity.instrument_id;
    auto t = v;
    stamp(t, ctx_);
    repo_.write(ctx_, t);
    BOOST_LOG_SEV(lg(), info) << "Saved callable_swap_instrument: " << t.identity.instrument_id;
}

void callable_swap_instrument_service::remove_callable_swap_instrument(const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing callable_swap_instrument: " << id;
    repo_.remove(ctx_, id);
    BOOST_LOG_SEV(lg(), info) << "Removed callable_swap_instrument: " << id;
}

std::vector<domain::callable_swap_instrument>
callable_swap_instrument_service::get_callable_swap_instrument_history(const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for callable_swap_instrument: " << id;
