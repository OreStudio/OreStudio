...
create or replace function ores_compute_hosts_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_external_id text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_external_id := OLD.external_id;
        ...
        changed_external_id := NEW.external_id;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('external_id', changed_external_id);

    notification_payload := jsonb_build_object(
        'event_id', gen_random_uuid()::text,
        'entity', 'ores.compute.host',
    ...
    perform pg_notify('ores_compute_hosts', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_compute_hosts_notify_trg
after insert or update or delete on ores_compute_hosts_tbl
for each row execute function ores_compute_hosts_notify_fn();
