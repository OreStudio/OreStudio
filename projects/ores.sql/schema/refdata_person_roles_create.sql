...
create table if not exists "ores"."refdata_person_roles_tbl" (
    "code" text not null,
...
on "ores"."refdata_person_roles_tbl" (code, version)
where valid_to = ores.utility_infinity_timestamp_fn();

create index if not exists refdata_person_roles_coding_scheme_idx
on "ores"."refdata_person_roles_tbl" (coding_scheme_code)
where valid_to = ores.utility_infinity_timestamp_fn();

create or replace function ores.refdata_person_roles_insert_fn()
returns trigger as $$
declare
...
before insert on "ores"."refdata_person_roles_tbl"
for each row
execute function ores.refdata_person_roles_insert_fn();

create or replace rule refdata_person_roles_delete_rule as
on delete to "ores"."refdata_person_roles_tbl"
do instead
  update "ores"."refdata_person_roles_tbl"
  set valid_to = current_timestamp
  where code = old.code
