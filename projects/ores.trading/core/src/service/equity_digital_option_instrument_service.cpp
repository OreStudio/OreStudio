...
equity_digital_option_instrument_service::list_equity_digital_option_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Listing all Equity Digital Option instruments";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t equity_digital_option_instrument_service::count_equity_digital_option_instruments() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total Equity Digital Option instruments count";
    return repo_.get_total_equity_digital_option_instrument_count(ctx_);
}


std::optional<domain::equity_digital_option_instrument>
equity_digital_option_instrument_service::get_equity_digital_option_instrument_at_version(
    const std::string& instrument_id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Getting Equity Digital Option instrument at version. "
...
equity_digital_option_instrument_service::get_equity_digital_option_instrument(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Getting Equity Digital Option instrument. "
...
void equity_digital_option_instrument_service::save_equity_digital_option_instrument(
    ...
    if (v.identity.instrument_id.is_nil())
        throw std::invalid_argument(
            "Equity Digital Option Instrument instrument_id cannot be empty.");
...
void equity_digital_option_instrument_service::save_equity_digital_option_instruments(
    const std::vector<domain::equity_digital_option_instrument>&
        equity_digital_option_instruments) {
    for (const auto& e : equity_digital_option_instruments)
...
void equity_digital_option_instrument_service::delete_equity_digital_option_instrument(
    const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing Equity Digital Option instrument. "
                               << "instrument_id: " << instrument_id;
    repo_.remove(ctx_, instrument_id);
    BOOST_LOG_SEV(lg(), info) << "Removed Equity Digital Option instrument. "
                              << "instrument_id: " << instrument_id;
}

void equity_digital_option_instrument_service::delete_equity_digital_option_instruments(
    const std::vector<std::string>& instrument_ids) {
    repo_.remove(ctx_, instrument_ids);
}

std::vector<domain::equity_digital_option_instrument>
equity_digital_option_instrument_service::get_equity_digital_option_instrument_history(
    const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for Equity Digital Option instrument. "
                               << "instrument_id: " << instrument_id;
