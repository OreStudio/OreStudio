...
// notification" pattern end to end for lifecycle_event -- the
...
using ores::trading::repository::lifecycle_event_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_lifecycle_event_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::trading::eventing::lifecycle_event_changed_event>(
        [&nats](const ores::trading::eventing::lifecycle_event_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::trading::eventing::lifecycle_event_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.trading.lifecycle_event",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::trading::eventing::lifecycle_event_changed_event>(
        "ores.trading.lifecycle_event", "ores_trading_lifecycle_events");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::trading::eventing::lifecycle_event_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_lifecycle_event(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Lifecycle Event: " << v;

    lifecycle_event_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.trading.lifecycle_event") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for lifecycle_event " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for lifecycle_event " << id_str;
}
