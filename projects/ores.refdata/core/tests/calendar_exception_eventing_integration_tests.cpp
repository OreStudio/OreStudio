...
// notification" pattern end to end for calendar_exception -- the
...
TEST_CASE("write_calendar_exception_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::calendar_exception_changed_event>(
        [&nats](const ores::refdata::eventing::calendar_exception_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::calendar_exception_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::calendar_exception_changed_event>(
        "ores.refdata.calendar_exception", "ores_refdata_calendar_exceptions");
    ...
    auto v = generate_synthetic_calendar_exception(ctx);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.calendar_exception") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for calendar_exception " << id_str << " after "
