...
| Now          | The checklist is complete except for the items that a decision, rather than work, holds open. P03, W01, S01 and D13 closed by removing the series key shape surface: the component declares six subjects and serves all six, and its api is a third smaller. Twelve findings are recorded, three of them defects, and two of those are fixed. |
| Waiting on   | Nothing for the clean standard. [[id:6EA32258-F1DC-45D2-8913-202E845CD969][What ores.ore should own]] records where the entity and the import saga should live and why the move is not free: adding one model to =ores.marketdata= materialises 135 files of that component's un-regenerated backlog, of which about thirty are the entity's. Clearing that is its own task. |
| Next         | Bring =ores.marketdata= to a regenerable state, then move the entity in one clean step. Move the shell's export step onto a protocol, which both remaining options need. Register =ore= in =COMPONENTS_UNDER_TEST= once the boundary is settled; every gate already passes with it there. |
...
| V05/V06 (ready, not taken) | Every codegen gate passes with =ore= added to =COMPONENTS_UNDER_TEST=: component drift =--all=, model drift, protocol twin coverage (22 headers in 2 components, no gaps), handler permissions, populate references, the shell recipe inventory, and the CMake source lists for this component. Registering is deliberately not done yet: the registry's own docstring says a component joins once its whole rollout is finished and verified, and the ownership question in [[id:6EA32258-F1DC-45D2-8913-202E845CD969][what ores.ore should own]] is still open -- the api and core boundary, and whether the import saga stays here. Once that lands, V06 is a one-line change to =component_registry.py= plus a re-run. | Each gate run with =ore= registered, then the registry reverted |
