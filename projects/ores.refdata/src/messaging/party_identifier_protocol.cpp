...
// ============================================================================
// Party Identifier helpers
// ============================================================================

void write_party_identifier(std::vector<std::byte>& buffer,
    ...
    writer::write_string(buffer,
        ores::platform::time::datetime::format_time_point(pi.recorded_at));
}

std::expected<domain::party_identifier, error_code>
read_party_identifier(std::span<const std::byte>& data) {
    domain::party_identifier pi;

    auto version_result = reader::read_uint32(data);
    if (!version_result) return std::unexpected(version_result.error());
    pi.version = static_cast<int>(*version_result);
    ...
    pi.change_reason_code = *change_reason_code_result;
    ...
    pi.change_commentary = *change_commentary_result;

    auto recorded_at_result = reader::read_string(data);
    if (!recorded_at_result) return std::unexpected(recorded_at_result.error());
    try {
        pi.recorded_at = ores::platform::time::datetime::parse_time_point(*recorded_at_result);
...
// ============================================================================
// Party Identifier Messages Implementation
// ============================================================================
...
std::vector<std::byte> get_party_identifiers_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(party_identifiers.size()));
    for (const auto& pi : party_identifiers) {
        write_party_identifier(buffer, pi);
    }
    return buffer;
}

std::expected<get_party_identifiers_response, error_code>
get_party_identifiers_response::deserialize(std::span<const std::byte> data) {
    ...
    response.party_identifiers.reserve(count);
    for (std::uint32_t i = 0; i < count; ++i) {
        auto result = read_party_identifier(data);
        if (!result) return std::unexpected(result.error());
        response.party_identifiers.push_back(std::move(*result));
    }
...
std::vector<std::byte> save_party_identifier_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_party_identifier_response, error_code>
save_party_identifier_response::deserialize(std::span<const std::byte> data) {
...
std::expected<delete_party_identifier_request, error_code>
delete_party_identifier_request::deserialize(std::span<const std::byte> data) {
    delete_party_identifier_request request;
...
std::expected<delete_party_identifier_response, error_code>
...
std::vector<std::byte> get_party_identifier_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uuid(buffer, id);
    return buffer;
}

std::expected<get_party_identifier_history_request, error_code>
get_party_identifier_history_request::deserialize(std::span<const std::byte> data) {
    get_party_identifier_history_request request;
...
std::vector<std::byte> get_party_identifier_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_party_identifier(buffer, v);
    }
    return buffer;
}

std::expected<get_party_identifier_history_response, error_code>
get_party_identifier_history_response::deserialize(std::span<const std::byte> data) {
