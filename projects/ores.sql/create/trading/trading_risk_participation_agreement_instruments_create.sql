...
-- Tenant index
create index if not exists ores_trading_rpa_instruments_tenant_idx
on "ores_trading_rpa_instruments_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Party index for RLS
create index if not exists ores_trading_rpa_instruments_party_idx
on "ores_trading_rpa_instruments_tbl" (tenant_id, party_id)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Soft FK back to trade (NULL for standalone instruments)
create unique index if not exists ores_trading_rpa_instruments_trade_id_idx
...
for each row execute function ores_trading_rpa_instruments_insert_fn();

create or replace rule ores_trading_rpa_instruments_delete_rule as
on delete to "ores_trading_rpa_instruments_tbl" do instead
    update "ores_trading_rpa_instruments_tbl"
    set valid_to = current_timestamp
