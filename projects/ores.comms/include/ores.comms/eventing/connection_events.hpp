 ...
 *
 */
#ifndef ORES_COMMS_EVENTING_CONNECTION_EVENTS_HPP
#define ORES_COMMS_EVENTING_CONNECTION_EVENTS_HPP

#include <chrono>
#include <string>
#include "ores.eventing/domain/event_traits.hpp"

namespace ores::comms::eventing {
...
template<>
struct event_traits<ores::comms::eventing::connected_event> {
    static constexpr std::string_view name = "ores.comms.connected";
};

template<>
struct event_traits<ores::comms::eventing::disconnected_event> {
    static constexpr std::string_view name = "ores.comms.disconnected";
};

template<>
struct event_traits<ores::comms::eventing::reconnecting_event> {
    static constexpr std::string_view name = "ores.comms.reconnecting";
};

template<>
struct event_traits<ores::comms::eventing::reconnected_event> {
