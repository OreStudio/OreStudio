...
void bootstrap_operations_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    ...
    menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_create_initial_admin(std::ref(out), std::ref(session), std::move(args));
        },
