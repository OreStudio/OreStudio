 ...
 */
#ifndef ORES_QT_ASSET_CLASS_CODE_CONTROLLER_HPP
#define ORES_QT_ASSET_CLASS_CODE_CONTROLLER_HPP
...
class AssetClassCodeMdiWindow;
class AssetClassCodeDetailDialog;
class DetachableMdiSubWindow;
class ChangeReasonCache;

/**
 * @brief Controller for managing asset class code windows and operations.
 *
 * Manages the lifecycle of asset class code list, detail, and history windows.
 * Handles event subscriptions and coordinates between windows.
 */
class ORES_QT_REFDATA_EXPORT AssetClassCodeController final : public EntityController {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.asset_class_code_controller";
...
public:
    AssetClassCodeController(QMainWindow* mainWindow,
                             QMdiArea* mdiArea,
...
private slots:
    void onShowDetails(const refdata::domain::asset_class_code& class_);
...
private:
     ...
     * AssetClassCodeDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(AssetClassCodeDetailDialog* detailDialog);
