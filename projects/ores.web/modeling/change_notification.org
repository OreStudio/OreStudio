...
#+description: Change notification for the ORE Studio web UI: NATS events, the channel to the browser, payload shape, shared routed subscriptions, visual language, and what the interface must not do.
#+type: knowledge
#+level: cross
