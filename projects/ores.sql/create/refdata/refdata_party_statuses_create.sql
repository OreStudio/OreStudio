...
create unique index if not exists ores_refdata_party_statuses_version_uniq_idx
on "public"."ores_refdata_party_statuses_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_refdata_party_statuses_code_uniq_idx
on "public"."ores_refdata_party_statuses_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_party_statuses_tenant_idx
on "public"."ores_refdata_party_statuses_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function public.ores_refdata_party_statuses_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from "public"."ores_refdata_party_statuses_tbl"
    ...
    if found then
        ...
        update "public"."ores_refdata_party_statuses_tbl"
...
before insert on "public"."ores_refdata_party_statuses_tbl"
for each row
execute function public.ores_refdata_party_statuses_insert_fn();

create or replace rule ores_refdata_party_statuses_delete_rule as
on delete to "public"."ores_refdata_party_statuses_tbl"
do instead
  update "public"."ores_refdata_party_statuses_tbl"
  set valid_to = current_timestamp
...
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid party_status: value cannot be null or empty'
            using errcode = '23502';
    end if;
