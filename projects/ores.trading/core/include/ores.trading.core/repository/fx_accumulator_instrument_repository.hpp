 ...
 *
 */
#ifndef ORES_TRADING_CORE_REPOSITORY_FX_ACCUMULATOR_INSTRUMENT_REPOSITORY_HPP
#define ORES_TRADING_CORE_REPOSITORY_FX_ACCUMULATOR_INSTRUMENT_REPOSITORY_HPP
 ...
 */
class ORES_TRADING_CORE_EXPORT fx_accumulator_instrument_repository {
private:
...
public:
    ...
    /**
     * @brief Writes FX accumulator instruments to database.
     */
    /**@{*/
    ...
    /**
     * @brief Reads latest FX accumulator instruments, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all FX accumulator instruments, possibly filtered by primary key.
     */
    std::vector<domain::fx_accumulator_instrument> read_all(context ctx,
                                                            const std::string& instrument_id);

    /**
     * @brief Reads a single FX accumulator instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest FX accumulator instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_fx_accumulator_instrument_count(context ctx);

    /**
     * @brief Deletes a FX accumulator instrument by closing its temporal validity.
     */
    void remove(context ctx, const std::string& instrument_id);

    /**
     * @brief Deletes FX accumulator instruments by closing their temporal validity.
     */
