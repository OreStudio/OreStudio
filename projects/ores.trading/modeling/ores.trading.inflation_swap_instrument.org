...
#+description: Codegen entity model for the trading =inflation_swap_instrument= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
| "inflation_index_code" <> '' |
| "base_cpi" is null or "base_cpi" > 0 |
...
|  inflation_index_code      |  Index        |
|  base_cpi                  |  Base CPI     |
|  lag_convention            |  Lag Convention |
