...
#include "ores.workflow.api/eventing/workflow_instance_event.hpp"
...
// notification" pattern end to end for workflow_instance -- the
...
using ores::workflow::domain::workflow_instance;
using ores::workflow::repository::workflow_instance_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_workflow_instance_publishes_an_event", tags) {
    ...
    using event_type = ores::workflow::messaging::workflow_instance_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_workflow_workflow_instances");
    ...
    auto v = generate_synthetic_workflow_instance(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Workflow Instance: " << v;

    workflow_instance_repository repo;
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for workflow_instance " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for workflow_instance " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_instance(v.id).has_value());
        REQUIRE(svc.get_instance_history(id_str).size() == versions.size());
    }
}
