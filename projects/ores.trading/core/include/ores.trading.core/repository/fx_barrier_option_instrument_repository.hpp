 ...
 *
 */
#ifndef ORES_TRADING_CORE_REPOSITORY_FX_BARRIER_OPTION_INSTRUMENT_REPOSITORY_HPP
#define ORES_TRADING_CORE_REPOSITORY_FX_BARRIER_OPTION_INSTRUMENT_REPOSITORY_HPP
 ...
 */
class ORES_TRADING_CORE_EXPORT fx_barrier_option_instrument_repository {
private:
...
public:
    ...
    /**
     * @brief Writes FX barrier option instruments to database.
     */
    /**@{*/
    ...
    /**
     * @brief Reads latest FX barrier option instruments, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all FX barrier option instruments, possibly filtered by primary key.
     */
    std::vector<domain::fx_barrier_option_instrument> read_all(context ctx,
                                                               const std::string& instrument_id);

    /**
     * @brief Reads a single FX barrier option instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest FX barrier option instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_fx_barrier_option_instrument_count(context ctx);

    /**
     * @brief Deletes a FX barrier option instrument by closing its temporal validity.
     */
    void remove(context ctx, const std::string& instrument_id);

    /**
     * @brief Deletes FX barrier option instruments by closing their temporal validity.
     */
