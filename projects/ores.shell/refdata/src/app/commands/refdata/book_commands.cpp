...
void book_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("books");
    ...
    menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_parent_portfolio_id(std::ref(out), std::ref(session), std::move(args));
        },
        "by-parent-portfolio-id <parent_portfolio_id> [--offset <n>] [--limit <n>] [--order "
        "<field>] [--desc]");
...
void book_commands::process_list(std::ostream& out,
                                 nats_client& session,
                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_books_request;
    ...
    auto result = do_auth_request<messaging::list_books_response>(
...
void book_commands::process_get(std::ostream& out,
                                nats_client& session,
                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_book_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_book_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_get_many(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_books_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_books_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_add(std::ostream& out,
                                nats_client& session,
                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_book_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.parent_portfolio_id,
                   ...
                   "parent_portfolio_id");
        read_token(req.change.write.owner_unit_id, parsed->positionals[next++], "owner_unit_id");
        read_token(req.change.write.functional_currency,
                   parsed->positionals[next++],
                   "functional_currency");
        read_token(req.change.write.gl_account_ref, parsed->positionals[next++], "gl_account_ref");
        read_token(req.change.write.cost_center, parsed->positionals[next++], "cost_center");
        read_token(req.change.write.book_status, parsed->positionals[next++], "book_status");
    ...
    auto result = do_auth_request<messaging::put_book_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_set(std::ostream& out,
                                nats_client& session,
                                const std::vector<std::string>& args) {
...
void book_commands::process_put_many(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_books_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            ...
            read_token(change.write.parent_portfolio_id,
                       parsed->positionals[next++],
                       "parent_portfolio_id");
            read_token(change.write.owner_unit_id, parsed->positionals[next++], "owner_unit_id");
            read_token(change.write.functional_currency,
                       parsed->positionals[next++],
                       "functional_currency");
            read_token(change.write.gl_account_ref, parsed->positionals[next++], "gl_account_ref");
            read_token(change.write.cost_center, parsed->positionals[next++], "cost_center");
            read_token(change.write.book_status, parsed->positionals[next++], "book_status");
    ...
    auto result = do_auth_request<messaging::put_many_books_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_delete(std::ostream& out,
                                   nats_client& session,
                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_book_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_book_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_delete_many(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_books_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_many_books_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_by_parent_portfolio_id(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-parent-portfolio-id request.";

    using request_type = messaging::list_by_parent_portfolio_id_books_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-parent-portfolio-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.parent_portfolio_id = ores::shell::app::from_token<boost::uuids::uuid>(
            parsed->positionals[next++], "parent_portfolio_id");
    ...
    auto result = do_auth_request<messaging::list_by_parent_portfolio_id_books_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_versions(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_book_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_book_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void book_commands::process_version(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_book_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_book_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
