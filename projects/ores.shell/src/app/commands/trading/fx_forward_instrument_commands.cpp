...
void fx_forward_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    auto fx_forward_instruments_menu =
        std::make_unique<cli::Menu>("fx_forward_instruments");

    fx_forward_instruments_menu->Insert(
        "get",
        [&session, &pagination](std::ostream& out) {
            process_get_fx_forward_instruments(std::ref(out), std::ref(session),
                                               std::ref(pagination));
        },
        "Retrieve FX forward instruments from the server (paginated)");

    // Register list callback for navigation
    pagination.register_list_callback("fx_forward_instruments",
                                      [&session, &pagination](std::ostream& out) {
                                          process_get_fx_forward_instruments(out, session,
                                                                             pagination);
                                      });
    ...
    fx_forward_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_fx_forward_instrument(std::ref(out),
                                              std::ref(session),
        ...
        "Add an FX forward instrument (trade_type_code bought_currency bought_amount sold_currency "
        "sold_amount value_date [settlement] [description] change_reason_code "
        "\"change_commentary\")",
        {"trade_type_code bought_currency bought_amount sold_currency sold_amount value_date "
         "settlement description change_reason_code change_commentary"});

    fx_forward_instruments_menu->Insert(
        "delete",
        [&session](std::ostream& out, std::string instrument_id) {
            process_delete_fx_forward_instrument(std::ref(out), std::ref(session),
                                   std::move(instrument_id));
        },
        "Delete an FX forward instrument by instrument id",
        {"instrument_id"});

    fx_forward_instruments_menu->Insert(
        "history",
        [&session](std::ostream& out, std::string instrument_id) {
            process_get_fx_forward_instrument_history(std::ref(out), std::ref(session),
                                        std::move(instrument_id));
        },
        "Show an FX forward instrument's version history",
        {"instrument_id"});
...
void fx_forward_instrument_commands::process_get_fx_forward_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX forward instruments request.";
    ...
    BOOST_LOG_SEV(lg(), info) << "Successfully retrieved "
                              << result->fx_forward_instruments.size()
...
void fx_forward_instrument_commands::process_add_fx_forward_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an FX forward instrument." << std::endl;
        return;
    }
    ...
    v.value_date = std::move(value_date);
...
void fx_forward_instrument_commands::process_delete_fx_forward_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an FX forward instrument." << std::endl;
        return;
    }
    ...
    if (result->success) {
        ...
        BOOST_LOG_SEV(lg(), warn) << "Failed to delete FX forward instrument: "
                                  << result->message;
...
void fx_forward_instrument_commands::process_get_fx_forward_instrument_history(
    ...
    auto result =
        do_auth_request<trading::messaging::get_fx_forward_instrument_history_response>(
