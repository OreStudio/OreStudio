...
inline auto& scheduler_status_handler_lg() {
    static auto instance =
        ores::logging::make_logger("ores.scheduler.messaging.scheduler_status_handler");
    return instance;
}
} // namespace
...
public:
    ...
    void status(ores::nats::message msg) {
        BOOST_LOG_SEV(scheduler_status_handler_lg(), debug) << "Handling " << msg.subject;
        ...
        if (!decode<get_scheduler_status_request>(msg)) {
            ...
            reply(nats_,
                  msg,
                  get_scheduler_status_response{.success = false,
        ...
        try {
            ...
            for (const auto& def : defs) {
                ...
                if (!recent.empty()) {
                    ...
                    switch (last.status) {
                        case domain::job_status::succeeded:
                            jss.last_run_status = "succeeded";
                            break;
                        case domain::job_status::failed:
                            jss.last_run_status = "failed";
                            break;
                        default:
                ...
                if (def.is_active) {
                    try {
                        ...
                        jss.next_fire_at = ores::platform::time::datetime::to_iso8601_utc(next);
                    } catch (...) {
                    }
                    ++resp.total_active;
                }
            ...
            reply(nats_, msg, get_scheduler_status_response{.success = false, .message = e.what()});
            return;
        }

        reply(nats_, msg, resp);
        BOOST_LOG_SEV(scheduler_status_handler_lg(), debug) << "Completed " << msg.subject;
    }
