...
The three steps above are the transitions: verification belongs to the
[[id:B8D4F2A1-9E37-4C6B-A8D5-2F1E0C9B7A3D][trade lifecycle]], while affirmation and confirmation belong here.
...
carries an external machine whose states are =unconfirmed=,
=queried=, =affirmed= and =confirmed=, moving on the external version
exactly as this rule describes, alongside an internal machine that
moves on the internal version. The three steps distinguished above map
onto them directly: verification is a transition in the internal
machine, while affirmation and confirmation are transitions in the
external one.
