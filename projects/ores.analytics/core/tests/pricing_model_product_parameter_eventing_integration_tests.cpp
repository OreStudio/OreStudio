...
// notification" pattern end to end for pricing_model_product_parameter -- the
...
TEST_CASE("write_pricing_model_product_parameter_publishes_nats_changed_event", tags) {
    ...
    auto v = generate_synthetic_pricing_model_product_parameter(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Pricing Model Product Parameter: " << v;
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for pricing_model_product_parameter " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for pricing_model_product_parameter "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::analytics::service::pricing_model_product_parameter_service svc(crud_ctx);
