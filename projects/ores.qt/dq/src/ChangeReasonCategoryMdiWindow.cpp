...
void ChangeReasonCategoryMdiWindow::setupToolbar() {
    ...
    addAction_->setToolTip(tr("Add new change reason category"));
    ...
    editAction_->setToolTip(tr("Edit selected change reason category"));
    ...
    deleteAction_->setToolTip(tr("Delete selected change reason category"));
    ...
    historyAction_->setToolTip(tr("View change reason category history"));
...
void ChangeReasonCategoryMdiWindow::doReload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading change reason categories";
    clearStaleIndicator();
    emit statusChanged(tr("Loading change reason categories..."));
...
void ChangeReasonCategoryMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 change reason categories").arg(loaded).arg(total));
...
void ChangeReasonCategoryMdiWindow::onDoubleClicked(const QModelIndex& index) {
    ...
    if (auto* item = model_->getCategory(sourceIndex.row())) {
        emit showCategoryDetails(*item);
    }
}
...
void ChangeReasonCategoryMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new change reason category requested";
...
void ChangeReasonCategoryMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* item = model_->getCategory(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showCategoryHistory for code: " << item->;
        emit showCategoryHistory(*item);
    }
}
...
void ChangeReasonCategoryMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete change reason category while disconnected.");
        return;
    }
    ...
    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid change reason categories to delete";
        return;
    }
    ...
    if (codes.size() == 1) {
        confirmMessage = QString("Are you sure you want to delete change reason category '%1'?")
                             .arg(QString::fromStdString(codes.front()));
    } else {
        confirmMessage = QString("Are you sure you want to delete %1 change reason categories?")
                             .arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Change Reason Category", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << codes.size() << " change reason categories";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                BOOST_LOG_SEV(lg(), debug) << "Change Reason Category deleted: " << code;
                success_count++;
                emit self->itemDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error)
                    << "Change Reason Category deletion failed: " << code << " - " << result.second;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg =
                success_count == 1 ?
                    "Successfully deleted 1 change reason category" :
                    QString("Successfully deleted %1 change reason categories").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg =
                QString("Failed to delete %1 %2: %3")
                    .arg(failure_count)
                    .arg(failure_count == 1 ? "change reason category" : "change reason categories")
                    .arg(first_error);
