 ...
 *
 */
#include "ores.refdata/generators/currency_asset_class_generator.hpp"
...
generate_synthetic_currency_asset_classes(std::size_t n,
    utility::generation::generation_context& ctx) {
    std::vector<domain::currency_asset_class> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_currency_asset_class(ctx));
    return r;
}
