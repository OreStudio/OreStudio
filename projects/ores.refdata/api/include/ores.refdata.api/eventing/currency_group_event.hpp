 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_CURRENCY_GROUP_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_CURRENCY_GROUP_EVENT_HPP
...
/**
 * @brief Event traits for currency group's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::currency_group_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.currency_groups_events";

    static ores::refdata::messaging::currency_group_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::currency_group_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::currency_group_key>(notification.key))
            event.key = *key;
