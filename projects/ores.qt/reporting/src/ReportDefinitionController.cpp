...
void ReportDefinitionController::showListWindow() {
    ...
    listMdiSubWindow_->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::FileText, IconUtils::DefaultIconColor));
...
void ReportDefinitionController::wireDetailDialogCommon(
    ReportDefinitionDetailDialog* detailDialog) {
...
void ReportDefinitionController::showHistoryWindow(
    ...
    const QString entityId = QString::fromStdString(boost::uuids::to_string(definition.id));
    auto* historyDialog =
        new HistoryDialog(std::string(entity_type_of(reporting::domain::report_definition{})),
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<ReportDefinitionController>(this)](
...
void ReportDefinitionController::fetchReportDefinitionHistory(
    const QString& entityId,
    std::function<void(std::expected<std::vector<reporting::domain::report_definition>, QString>)>
        callback) {
    reporting::messaging::get_report_definition_history_request request;
    request.id = entityId.toStdString();

    using FetchResult = std::expected<std::vector<reporting::domain::report_definition>, QString>;
...
void ReportDefinitionController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<ReportDefinitionController> self = this;
    fetchReportDefinitionHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<reporting::domain::report_definition>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void ReportDefinitionController::onRevertHistoryVersion(const QString& entityId,
                                                        int versionNumber) {
    QPointer<ReportDefinitionController> self = this;
    fetchReportDefinitionHistory(
        entityId,
...
void ReportDefinitionController::notifyOpenDialogs(const QStringList& entityIds) {
