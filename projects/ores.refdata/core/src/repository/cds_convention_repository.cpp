...
cds_convention_repository::replace_claim(context ctx, const domain::cds_convention& v) {
...
cds_convention_repository::apply_claim(context ctx,
                                       const domain::cds_convention& v,
...
void cds_convention_repository::write(context ctx,
                                      const domain::cds_convention& v,
                                      const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing CDS convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, cds_convention_mapper::map(t), lg(), "Writing CDS convention to database.");
}

void cds_convention_repository::write(
    context ctx,
    const std::vector<domain::cds_convention>& v,
    ...
    execute_write_query(
        ctx, cds_convention_mapper::map(batch), lg(), "Writing CDS conventions to database.");
}
...
std::vector<domain::cds_convention> cds_convention_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<cds_convention_entity, domain::cds_convention>(
            ...
            lg(),
            "Reading latest CDS conventions (workspace resolution chain).");
    }
...
std::optional<domain::cds_convention> cds_convention_repository::read_at_version(
    context ctx, const std::string& id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading CDS convention at version. " << "id: " << id
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<cds_convention_entity>> |
    ...
    const auto entities = execute_read_query<cds_convention_entity, domain::cds_convention>(
        ctx,
        query,
        [](const auto& entities) { return cds_convention_mapper::map(entities); },
        lg(),
        "Reading CDS convention at version.");
...
std::uint32_t cds_convention_repository::get_total_cds_convention_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active CDS convention count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<cds_convention_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active CDS convention count: " << count;
    return count;
}

std::vector<domain::cds_convention>
cds_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<cds_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<cds_convention_entity, domain::cds_convention>(
        ctx,
        query,
        [](const auto& entities) { return cds_convention_mapper::map(entities); },
        lg(),
        "Reading latest CDS conventions by ids.");
    return result;
}

void cds_convention_repository::remove(context ctx, const std::vector<std::string>& ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<cds_convention_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing CDS conventions.");
}
