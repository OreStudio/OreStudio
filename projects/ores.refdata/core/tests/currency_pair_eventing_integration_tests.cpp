...
// notification" pattern end to end for currency_pair -- the
...
TEST_CASE("write_currency_pair_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::currency_pair_changed_event>(
        [&nats](const ores::refdata::eventing::currency_pair_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::currency_pair_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.currency_pair",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.pair_codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::currency_pair_changed_event>(
        "ores.refdata.currency_pair", "ores_refdata_currency_pairs");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::currency_pair_changed_event>::name),
        10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.currency_pair") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for currency_pair " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for currency_pair " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_pair(id_str).has_value());
        REQUIRE(svc.get_pair_history(id_str).size() == versions.size());
    }
}
