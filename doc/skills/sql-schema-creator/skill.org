...
All SQL files are located under =projects/sql/= with the following structure:
...
Create a file =projects/sql/schema/TABLE_NAME_create.sql=.
...
Follow the pattern in =projects/sql/schema/accounts_create.sql=.

** Step 2: Create notification trigger (if needed)

If the table needs real-time notifications, create =projects/sql/schema/TABLE_NAME_notify_trigger.sql=:
...
Follow the pattern in =projects/sql/schema/accounts_notify_trigger.sql=.

** Step 3: Create drop script

Create =projects/sql/drop/TABLE_NAME_drop.sql=:
...
If you created a notify trigger, also create =projects/sql/drop/TABLE_NAME_notify_trigger_drop.sql=.

** Step 4: Create population script (if needed)

For reference data, create =projects/sql/data/TABLE_NAME_populate.sql=:
...
Follow the pattern in =projects/sql/data/currencies_populate.sql=.

** Step 5: Update orchestration scripts

1. Add the new creation script to =projects/sql/template/create_schema.sql=:
...
2. Add the drop script to =projects/sql/drop_all.sql= (in reverse dependency order):
...
#+begin_src sh
psql -U ores -d ores_silent_meadow -c "\d ores.TABLE_NAME"
#+end_src
