...
namespace {
std::string leg_type_key_extractor(const refdata::domain::leg_type& e) {
    return e.code;
}
}
...
int ClientLegTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientLegTypeModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh leg type model: disconnected.";
    ...
    QFuture<FetchResult> future = QtConcurrent::run([self, offset, limit]() -> FetchResult {
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                refdata::messaging::get_leg_types_request request;
                ...
                BOOST_LOG_SEV(lg(), debug)
                    ...
                    << " leg types, total available: " << result->total_available_count;
                return {.success = true,
...
void ClientLegTypeModel::onTypesLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " leg types newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " leg types."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientLegTypeModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
const refdata::domain::leg_type* ClientLegTypeModel::getType(int row) const {
...
void ClientLegTypeModel::onPulsingComplete() {
