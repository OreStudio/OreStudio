...
#+description: Literate source for .claude/settings.json — permission allow-list, generated via org-babel tangle. OS sandbox intentionally disabled (see Sandbox (removed)).
#+type: knowledge
#+level: cross
...
[[proj:.claude/settings.json][=.claude/settings.json=]]. Requires Emacs on PATH (the =ores-babel.el=
project detection needs a full environment; the CMake path also needs
compiler detection during reconfiguration).
...
explicit allow entry for reads; they are permitted by default.
...
subcommands). =plantuml_er_generate.sh= produces PlantUML ER diagrams.
=generate_doc.sh= is a legacy script being decommissioned — use
=compass= instead.
...
- =projects/ores.codegen/codegen.sh regenerate --component refdata --address ores.cpp=
- =projects/ores.codegen/codegen.sh generate --model path/to/model.json --address ores.cpp=
...
Raw =git= push/fetch operations need =SSH_AUTH_SOCK= exported first
(see [[id:F7E4C41F-AF34-43E5-ADC3-1D837126B3DE][Set SSH_AUTH_SOCK for git operations]]); =gh= uses its own token,
not the SSH agent.
...
=WebFetch(domain:*)= allows fetching from any domain. The permission
entry controls only whether Claude Code prompts.
...
The OS-level sandbox is intentionally disabled for this project. The
sandbox's filesystem write-virtualisation redirected repo writes to a
scratch btrfs layer that host processes (systemd units, services)
could not open — on 2026-08-06 this broke =compass services start=
for the festive_dijkstra environment (nats-server ENOENT on its
config) and the layer then decayed in place. Protection now comes
from the interactive permission prompts above and the git/PR review
flow; there is no project-level =sandbox= key in =settings.json=.

If a sandbox is ever re-enabled, the network/filesystem/excluded
overrides that used to live here must be restored first: localhost
(psql), the ccache/sccache dirs, and the =git *=/=compass.sh *= write
exclusions.
