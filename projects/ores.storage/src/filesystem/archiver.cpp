...
// archive_error_string() returns nullptr when no error message is available.
// Appending nullptr to std::string is undefined behaviour; use this wrapper
// at every call site.
const char* safe_error_string(archive* a) noexcept {
...
void archiver::pack(const fs::path& src_dir, const fs::path& dest_archive) {
    archive_write_ptr a(archive_write_new());
    archive_write_add_filter_gzip(a.get());
    archive_write_set_format_pax_restricted(a.get());

#ifdef _WIN32
    const int open_rc = archive_write_open_filename_w(a.get(), dest_archive.c_str());
#else
    ...
    if (open_rc != ARCHIVE_OK)
        throw std::runtime_error(std::string("archiver: failed to create archive: ") +
                                 dest_archive.string() + " — " + safe_error_string(a.get()));
...
void archiver::extract(const fs::path& archive, const fs::path& dest_dir) {
...
#ifdef _WIN32
    const int open_rc = archive_read_open_filename_w(a.get(), archive.c_str(), 16384);
#else
    ...
    if (open_rc != ARCHIVE_OK)
        throw std::runtime_error(std::string("archiver: failed to open archive: ") +
                                 archive.string() + " — " + safe_error_string(a.get()));
    ...
    while (true) {
        ...
        if (!raw)
            throw std::runtime_error("archiver: archive entry has no pathname");
