...
| Now          | All 47 items are recorded: 21 pass and 26 are not applicable, each with its reason. |
| Waiting on   | Nothing.                               |
| Next         | Record the result, close the task and the story, and raise the PR. |
...
The standard holds 47 items. This component is of kind *All*, and it has no
model and no protocol, so the *All* items are the work and the Protocol and
Entity items are recorded as not applicable with their reasons.
...
| V06  | N/A    | The component has no catalogue row and no models, so it cannot join `COMPONENTS_UNDER_TEST`. The gate resolves its members through the codegen catalogue and errors on an unknown slug. Listing it anyway was tried and reverted: it makes the codegen read the component overview as a scaffold and offer to create `domain/stub.hpp`, `src/domain/stub.cpp`, a stub test and the banner header this task deleted. A component with no models needs the registry to treat it as having nothing to regenerate, which is a codegen change for the six components it affects. Evidence, including the reverted experiment, in `evidence/v06_registry.txt`. |
...
| V01  | Pass   | `./compass.sh build` reaches 100 percent with exit code 0, run after the last source edit, so every consumer of the deleted API still builds. |
| V02  | Pass   | `ores.nats.tests` passes at 55 assertions in 28 test cases, and the codegen suite passes at 555 tests. The fleet-dependent half of V02 belongs to the components that serve; this one owns no service. |
| V05  | Pass   | Every gate green, with the pre-existing repo-wide CMake staleness recorded: `check_component_drift.py --all` no drift, model drift clean, protocol twin coverage intact, handler permissions seeded, populate references resolve, recipe inventory checked, and this component's source lists up to date. `regenerate_cmake_component_files.py --all --check` reports 116 stale lists elsewhere, none of them ores.nats. `validate_docs.sh` passes all 40 components. Evidence in `evidence/v05_gates.txt`. |
...
Standard]] for a component of kind *All*. All 47 items are recorded on this
task: 21 pass and 26 are not applicable, each with its reason. The
Protocol and Entity items do not apply because the component has no model
and no protocol, and V06 does not apply because the registry admits only
model-bearing catalogue entries, which is the standard's own recorded
finding.

The work itself came to three things. The **dead public surface** the B06
census found is deleted: the JetStream read and message API, the three
domain types behind it, two client verbs and the banner header, 600 lines
in all, and the four services that take an admin handle still build against
the one method they call. The **diagram** went from an automated skeleton
to a diagram with an API, relationships and the free-function facets, and
the generator defect that had silently dropped two classes is fixed at its
source with five regression cases. The **overview** now states the real
wire format: resolved at startup, msgpack by default, not JSON per payload.

Every gate is green for this component: the drift gate passes with the
component's own source lists up to date, model drift, protocol twin
coverage, handler permissions, populate references, the recipe inventory
and the documentation checks all pass, the whole tree builds with every
consumer, and the component's suite passes at 55 assertions in 28 cases.
The one gate that is not green repo-wide is the CMake source-list check,
which reports 116 stale lists in other components; none of them is this
one, and the clean-compute task recorded that sweep as a codegen-wide
finding.
