...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        if (auto req = decode<get_market_data_generation_configs_request>(msg)) {
            try {
                resp.market_data_generation_configs =
                    svc.list_market_data_generation_configs(req->offset, req->limit);
                resp.total_available_count =
                    static_cast<int>(svc.count_market_data_generation_configs());
                resp.success = true;
    ...
    void save(ores::nats::message msg) {
        ...
        service::market_data_generation_config_service svc(req_ctx);
        if (auto req = decode<save_market_data_generation_config_request>(msg)) {
            try {
                svc.save_market_data_generation_config(req->data);
                BOOST_LOG_SEV(market_data_generation_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::market_data_generation_config_service svc(req_ctx);
        if (auto req = decode<get_market_data_generation_config_history_request>(msg)) {
            try {
                auto hist = svc.get_market_data_generation_config_history(req->id);
                BOOST_LOG_SEV(market_data_generation_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_,
                      msg,
                      get_market_data_generation_config_history_response{.history = std::move(hist),
                                                                         .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(market_data_generation_config_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_market_data_generation_config_history_response{.success = false,
                                                                         .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(market_data_generation_config_handler_lg(), warn)
    ...
    void remove(ores::nats::message msg) {
        ...
        service::market_data_generation_config_service svc(req_ctx);
        if (auto req = decode<delete_market_data_generation_config_request>(msg)) {
            try {
                svc.delete_market_data_generation_configs(req->ids);
                BOOST_LOG_SEV(market_data_generation_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
