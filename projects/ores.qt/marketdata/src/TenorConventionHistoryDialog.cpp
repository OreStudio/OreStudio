...
#include "ui_TenorConventionHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

TenorConventionHistoryDialog::TenorConventionHistoryDialog(const QString& code,
                                                           ClientManager* clientManager,
                                                           QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::TenorConventionHistoryDialog)
    , code_(code)
...
TenorConventionHistoryDialog::~TenorConventionHistoryDialog() {
    delete ui_;
}

QString TenorConventionHistoryDialog::code() const {
    return code_;
}

void TenorConventionHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for tenor convention: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    marketdata::messaging::get_tenor_convention_history_request request;
    request.code = code_.toStdString();

    QPointer<TenorConventionHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](marketdata::messaging::get_tenor_convention_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int TenorConventionHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString TenorConventionHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow TenorConventionHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult TenorConventionHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void TenorConventionHistoryDialog::displayFullDetails(int index) {
...
void TenorConventionHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void TenorConventionHistoryDialog::revertToVersionAt(int index) {
