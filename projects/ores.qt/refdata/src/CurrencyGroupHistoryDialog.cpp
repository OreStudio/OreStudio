...
#include "ui_CurrencyGroupHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

CurrencyGroupHistoryDialog::CurrencyGroupHistoryDialog(const QString& code,
                                                       ClientManager* clientManager,
                                                       QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::CurrencyGroupHistoryDialog)
    , code_(code)
...
CurrencyGroupHistoryDialog::~CurrencyGroupHistoryDialog() {
    delete ui_;
}

QString CurrencyGroupHistoryDialog::code() const {
    return code_;
}

void CurrencyGroupHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for currency group: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_currency_group_history_request request;
    request.code = code_.toStdString();

    QPointer<CurrencyGroupHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_currency_group_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int CurrencyGroupHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString CurrencyGroupHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow CurrencyGroupHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult CurrencyGroupHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void CurrencyGroupHistoryDialog::displayFullDetails(int index) {
...
void CurrencyGroupHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void CurrencyGroupHistoryDialog::revertToVersionAt(int index) {
