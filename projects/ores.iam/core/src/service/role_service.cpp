 ...
 */
std::vector<domain::role> read_one(repository::role_repository& repo,
                                   const ores::database::context& ctx,
                                   const messaging::role_key& key) {
 ...
 */
messaging::role_key key_from(const domain::role& v) {
 ...
 */
domain::role to_domain(const messaging::role_write& write) {
    domain::role v;
    v.id = write.id;
    v.name = write.name;
...
role_service::list_roles(const messaging::list_roles_request& request) {
    messaging::list_roles_response response;
...
messaging::get_role_response role_service::get_role(const messaging::get_role_request& request) {
    messaging::get_role_response response;
    ...
    response.role = std::move(found.front());
    return response;
}

messaging::get_many_roles_response
role_service::get_many_roles(const messaging::get_many_roles_request& request) {
    messaging::get_many_roles_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::role_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.role = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_role_response role_service::put_role(const messaging::put_role_request& request) {
    messaging::put_role_response response;
    domain::role value;
...
role_service::put_many_roles(const messaging::put_many_roles_request& request) {
    messaging::put_many_roles_response response;
...
messaging::delete_role_response
role_service::delete_role(const messaging::delete_role_request& request) {
    messaging::delete_role_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(row.id), expected)) {
        case repository::role_repository::remove_status::removed:
            break;
        case repository::role_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::role_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::role_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
role_service::delete_many_roles(const messaging::delete_many_roles_request& request) {
    messaging::delete_many_roles_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
    ...
    // failure.
    std::vector<domain::role> resolved;
...
role_service::list_role_versions(const messaging::list_role_versions_request& request) {
    messaging::list_role_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
role_service::get_role_version(const messaging::get_role_version_request& request) {
    messaging::get_role_version_response response;
    ...
    const auto named = read_one(repo_, ctx_, request.key.role);
    if (named.empty()) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
role_service::prepare_change(const messaging::role_change& change,
...
std::optional<domain::role> role_service::get_role_at_version(const boost::uuids::uuid& id,
...
std::optional<domain::role> role_service::get_role(const boost::uuids::uuid& id) {
...
std::optional<domain::role> role_service::get_role_by_name(const std::string& name) {
    BOOST_LOG_SEV(lg(), debug) << "Getting role by name: " << name;
    messaging::role_key k;
    k.name = name;
    auto found = read_one(repo_, ctx_, k);
    if (found.empty())
        return std::nullopt;
...
std::vector<domain::role> role_service::get_roles(const std::vector<std::string>& ids) {
...
void role_service::delete_role(const boost::uuids::uuid& id) {
...
std::vector<domain::role> role_service::get_role_history(const std::string& key) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for role. key: " << key;
