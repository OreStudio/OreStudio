...
| 13 | resolve_context only packs tenant_id, discarding the party/roles/workspace that make_request_context already resolved | registrar.cpp | Noted, no code change | Correct for currency (tenant-scoped only, no party dimension). Risk is precedent: the next entity wired here that IS party-scoped (e.g. book, portfolio — RLS-restricted by party) must not copy this resolver as-is, or party filtering silently disappears. Extend the packing format (e.g. tenant_id \vert party_id) and the provider's unpacking together when that entity is wired, per dispatch_registry.hpp's history_provider doc |
