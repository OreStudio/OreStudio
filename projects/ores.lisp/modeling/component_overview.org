...
- =ores-capture.el= — product-backlog capture integration. Provides
  =ores/capture= (interactive command that delegates to =compass.sh
  capture --note=), =ores/setup-capture-templates= (registers key ="c"=
  in both =org-capture-templates= and =org-roam-capture-templates=
  targeting =inbox/= of the current checkout), and
  =ores/setup-capture-snippets= (loads the =v2-capture= yasnippet,
  trigger ="cap"=, for expanding v2 capture frontmatter in any org
  buffer).
