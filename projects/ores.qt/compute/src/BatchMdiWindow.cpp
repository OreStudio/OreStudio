...
BatchMdiWindow::BatchMdiWindow(ClientManager* clientManager,
...
void BatchMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &BatchMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &BatchMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &BatchMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &BatchMdiWindow::viewHistorySelected);
}
...
void BatchMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "BatchListWindow", {}, {900, 400}, 1);
}

void BatchMdiWindow::setupConnections() {
    connect(model_, &ClientBatchModel::dataLoaded, this, &BatchMdiWindow::onDataLoaded);
    connect(model_, &ClientBatchModel::loadError, this, &BatchMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &BatchMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &BatchMdiWindow::onDoubleClicked);
...
void BatchMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void BatchMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* batch = model_->getBatch(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showBatchHistory for code: " << batch->external_ref;
...
void BatchMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete compute batch while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* batch = model_->getBatch(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(batch->id));
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " compute batches";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 compute batches?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Batch", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    QPointer<BatchMdiWindow> self = this;
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " compute batches";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                BOOST_LOG_SEV(lg(), debug) << "Batch deleted: " << code;
                success_count++;
                emit self->batchDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error) << "Batch deletion failed: " << code << " - " << message;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              "Successfully deleted 1 compute batch" :
                              QString("Successfully deleted %1 compute batches").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "compute batch" : "compute batches")
                              .arg(first_error);
