 ...
 */
std::vector<domain::country> read_one(repository::country_repository& repo,
                                      const ores::database::context& ctx,
                                      const messaging::country_key& key) {
    return repo.read_latest(ctx, key.alpha2_code);
}
 ...
 */
messaging::country_key key_from(const domain::country& v) {
    messaging::country_key key;
    key.alpha2_code = v.alpha2_code;
    return key;
}
 ...
 */
domain::country to_domain(const messaging::country_write& write) {
    domain::country v;
    v.alpha2_code = write.alpha2_code;
    v.alpha3_code = write.alpha3_code;
    v.numeric_code = write.numeric_code;
    v.name = write.name;
    v.official_name = write.official_name;
...
country_service::list_countries(const messaging::list_countries_request& request) {
    messaging::list_countries_response response;
...
messaging::get_country_response
country_service::get_country(const messaging::get_country_request& request) {
    messaging::get_country_response response;
    ...
    response.country = std::move(found.front());
    return response;
}

messaging::get_many_countries_response
country_service::get_many_countries(const messaging::get_many_countries_request& request) {
    messaging::get_many_countries_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::country_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.country = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_country_response
country_service::put_country(const messaging::put_country_request& request) {
    messaging::put_country_response response;
    domain::country value;
    ...
    if (!written.empty())
        response.country = std::move(written.front());
    return response;
}

messaging::put_many_countries_response
country_service::put_many_countries(const messaging::put_many_countries_request& request) {
    messaging::put_many_countries_response response;
    ...
    response.countries.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.countries.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_country_response
country_service::delete_country(const messaging::delete_country_request& request) {
    messaging::delete_country_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.alpha2_code, expected)) {
        case repository::country_repository::remove_status::removed:
            break;
        case repository::country_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::country_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::country_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
country_service::delete_many_countries(const messaging::delete_many_countries_request& request) {
    messaging::delete_many_countries_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
    ...
    if (request.removals.empty())
        return response;
    std::vector<std::string> alpha2_code_keys;
    alpha2_code_keys.reserve(request.removals.size());
    for (const auto& removal : request.removals)
        alpha2_code_keys.push_back(removal.key.alpha2_code);
    repo_.remove(ctx_, alpha2_code_keys);
    return response;
}

messaging::list_country_versions_response
country_service::list_country_versions(const messaging::list_country_versions_request& request) {
    messaging::list_country_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
    ...
    auto all = repo_.read_all(ctx_, request.key.alpha2_code);
...
country_service::get_country_version(const messaging::get_country_version_request& request) {
    messaging::get_country_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.country.alpha2_code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
country_service::prepare_change(const messaging::country_change& change,
...
country_service::get_countries(const std::vector<std::string>& alpha2_codes) {
    return repo_.read_latest(ctx_, alpha2_codes);
}
