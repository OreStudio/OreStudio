...
#include "ores.synthetic.api/eventing/folder_event.hpp"
...
TEST_CASE("write_folder_publishes_an_event", tags) {
    ...
    using event_type = ores::synthetic::messaging::folder_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_synthetic_folders");
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_folder(v.id).has_value());
