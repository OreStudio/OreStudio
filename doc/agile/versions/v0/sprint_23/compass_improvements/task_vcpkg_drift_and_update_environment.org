...
:END:
#+title: Task: Add vcpkg drift check to bearings and a compass-devops-update-environment skill
#+description: Add a vcpkg-vs-main drift warning to compass bearings, and create a new compass-devops-update-environment skill that brings a worktree fully up to date with main (fetch, detached checkout, submodules, settings, skills).
#+type: task
#+level: s1
...
- New skill =compass-devops-update-environment= exists, scaffolded via the
...
to the =.env= version check. Scaffold the =compass-devops-update-environment=
...
| PR | Title |
|----+-------|
| [[https://github.com/OreStudio/OreStudio/pull/1520][#1520]] | [compass] Add vcpkg drift check to bearings; add compass-devops-update-environment skill |
...
| 2 | Skill #+title uses kebab-case name instead of readable Title Case | compass-devops-update-environment/SKILL.org | Accepted | Retitled to "Update a worktree to latest main" |
...
Scaffolded the =compass-devops-update-environment= skill
