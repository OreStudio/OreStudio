...
#include "ores.trading.api/domain/fx_vanilla_option_instrument_table_io.hpp" // IWYU pragma: keep.
...
void fx_vanilla_option_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    fx_vanilla_option_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_fx_vanilla_option_instrument(std::ref(out), std::ref(session), args);
        },
        "Add FX vanilla option instrument (trade_type_code "
        "party_id "
        "trade_id "
        "bought_currency "
        "bought_amount "
    ...
    fx_vanilla_option_instruments_menu->Insert(
        ...
        "Delete FX vanilla option instrument by instrument_id");
    ...
    fx_vanilla_option_instruments_menu->Insert(
        ...
        "Show FX vanilla option instrument's version history");

    root_menu.Insert(std::move(fx_vanilla_option_instruments_menu));
}

void fx_vanilla_option_instrument_commands::process_get_fx_vanilla_option_instruments(
    std::ostream& out, nats_client& session, pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX vanilla option instrument request.";

    auto& state = pagination.state_for("fx_vanilla_option_instruments");

    trading::messaging::get_fx_vanilla_option_instruments_request req;
    req.offset = state.current_offset;
    req.limit = pagination.page_size();

    auto result = do_auth_request<trading::messaging::get_fx_vanilla_option_instruments_response>(
        out, session, "trading.v1.fx_vanilla_option_instruments.list", req);
    if (!result)
        return;

    state.total_count = result->total_available_count;
    pagination.set_last_entity("fx_vanilla_option_instruments");

    BOOST_LOG_SEV(lg(), info) << "Successfully retrieved "
                              << result->fx_vanilla_option_instruments.size()
                              << " FX vanilla option instruments.";
    out << result->fx_vanilla_option_instruments << std::endl;
    ...
    out << "\nPage " << page << " of " << total_pages << " ("
        << result->fx_vanilla_option_instruments.size() << " of " << state.total_count << " total)"
        << std::endl;
}

void fx_vanilla_option_instrument_commands::process_add_fx_vanilla_option_instrument(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add FX vanilla option instrument request.";

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add FX vanilla option instrument." << std::endl;
        return;
    }
    ...
    constexpr std::size_t positional_count = 12 + 2;
    ...
    domain::fx_vanilla_option_instrument v;
    std::size_t next = 0;
    try {
        ...
        v.exercise_style = ores::shell::app::from_token<std::string>(parsed->positionals[next++],
                                                                     "exercise_style");
        v.settlement =
    ...
    auto req = trading::messaging::save_fx_vanilla_option_instrument_request::from(std::move(v));

    auto result = do_auth_request<trading::messaging::save_fx_vanilla_option_instrument_response>(
        out, session, "trading.v1.fx_vanilla_option_instruments.save", req);
    if (!result)
        return;
    ...
    if (result->success) {
        ...
        fail(out) << "Failed to add FX vanilla option instrument: " << msg << std::endl;
    }
}

void fx_vanilla_option_instrument_commands::process_delete_fx_vanilla_option_instrument(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete FX vanilla option instrument request for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete FX vanilla option instrument." << std::endl;
        return;
    }

    trading::messaging::delete_fx_vanilla_option_instrument_request req;
    req.ids = {std::move(instrument_id)};

    auto result = do_auth_request<trading::messaging::delete_fx_vanilla_option_instrument_response>(
        out, session, "trading.v1.fx_vanilla_option_instruments.delete", req);
    if (!result)
        return;

    if (result->success) {
        BOOST_LOG_SEV(lg(), info) << "Successfully deleted FX vanilla option instrument.";
        out << "✓ FX vanilla option instrument deleted successfully!" << std::endl;
    } else {
...
void fx_vanilla_option_instrument_commands::process_get_fx_vanilla_option_instrument_history(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX vanilla option instrument history for: "
                               << instrument_id;
    ...
    trading::messaging::get_fx_vanilla_option_instrument_history_request req;
    req.instrument_id = std::move(instrument_id);

    auto result =
        do_auth_request<trading::messaging::get_fx_vanilla_option_instrument_history_response>(
            out, session, "trading.v1.fx_vanilla_option_instruments.history", req);
    if (!result)
        return;

    if (!result->success) {
        BOOST_LOG_SEV(lg(), warn) << "Failed to get FX vanilla option instrument history: "
                                  << result->message;
    ...
    if (result->history.empty()) {
        out << "No history found for this FX vanilla option instrument." << std::endl;
        return;
    }
