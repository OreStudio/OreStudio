...
Add the entity to the enumeration of available entities.

*** File to modify

=projects/ores.cli/include/ores.cli/config/entity.hpp=

*** Changes required

Add the new entity to the =entity= enum:
...
#+begin_example
[cli] Add <entity> to entity enumeration
#+end_example


** Step 2: Create Entity Parser Header

Create the parser header declaring the command handler function.

*** File location

=projects/ores.cli/include/ores.cli/config/entity_parsers/<entities>_parser.hpp=
 ...
 * licensing text...
 */
#ifndef ORES_CLI_CONFIG_ENTITY_PARSERS_<ENTITIES>_PARSER_HPP
#define ORES_CLI_CONFIG_ENTITY_PARSERS_<ENTITIES>_PARSER_HPP
...
/**
 * @brief Handles parsing of <entities> entity commands.
 */
std::optional<options>
handle_<entities>_command(bool has_help,
...
#+begin_example
[cli] Add <entities>_parser header
#+end_example


** Step 3: Create Entity Parser Implementation

Implement the command parsing for list and export operations.

*** File location

=projects/ores.cli/src/config/entity_parsers/<entities>_parser.cpp=
 ...
 * licensing text...
 */
#include "ores.cli/config/entity_parsers/<entities>_parser.hpp"
...
// Initial allowed operations - add more in Phase 2
...
handle_<entities>_command(bool has_help,
    ...
    // Show help for <entities> command if requested with no operation
    if (has_help && o.empty()) {
        const std::vector<std::pair<std::string, std::string>> operations = {
            {"export", "Export <entities> to external formats (JSON, table)"},
            {"list", "List <entities> in internal formats (JSON, table)"}
        };
        print_entity_help("<entities>", "Manage <entities>", operations, info);
        return {};
    }

    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception(
            "<entities> command requires an operation (export, list)"));
    }
    ...
    // Validate operation
    validate_operation("<entities>", operation, allowed_operations);
    ...
    if (operation == export_command_name) {
        ...
        if (has_help) {
            print_help_command("<entities> export", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::<entities>);
    } else if (operation == list_command_name) {
        auto d = add_common_options(make_export_options_description());
        if (has_help) {
            print_help_command("<entities> list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
...
#+begin_example
[cli] Implement <entities>_parser for list and export operations
#+end_example


** Step 4: Register Parser and Application Logic

Register the parser in the main parser and add application logic.

*** Files to modify

**** 1. Update main parser

File: =projects/ores.cli/src/config/parser.cpp=
...
Add command constants (in anonymous namespace):
#+begin_src cpp
const std::string <entities>_command_name("<entities>");
const std::string <entities>_command_desc("Manage <entities> (list, export).");
#+end_src

Add to =validate_command_name()=:
#+begin_src cpp
const bool is_valid_command_name(
    command_name == currencies_command_name ||
    command_name == accounts_command_name ||
    // ... existing ...
    command_name == <entities>_command_name);
#+end_src

Add to =print_help()=:
#+begin_src cpp
lambda(<entities>_command_name, <entities>_command_desc);
#+end_src

Add to =handle_command()=:
#+begin_src cpp
} else if (command_name == <entities>_command_name) {
    return entity_parsers::handle_<entities>_command(has_help, po, info, vm);
}
#+end_src

**** 2. Update application header

File: =projects/ores.cli/include/ores.cli/app/application.hpp=

Add method declarations:
#+begin_src cpp
    void export_<entities>(const config::export_options& cfg) const;
#+end_src

**** 3. Update application implementation

File: =projects/ores.cli/src/app/application.cpp=

Add includes:
#+begin_src cpp
#include "ores.<component>/domain/<entity>_json.hpp"
#include "ores.<component>/domain/<entity>_table.hpp"
#include "ores.<component>/repository/<entity>_repository.hpp"
#+end_src

Add export method:
#+begin_src cpp
void application::
export_<entities>(const config::export_options& cfg) const {
    BOOST_LOG_SEV(lg(), debug) << "Exporting <entities>.";

    <component>::repository::<entity>_repository repo(context_);
    std::vector<<component>::domain::<entity>> items;

    if (!cfg.key.empty()) {
        // Export specific <entity> by key
        if (cfg.all_versions) {
    ...
    if (cfg.target_format == config::format::json) {
        output_stream_ << <component>::domain::convert_to_json(items) << std::endl;
    } else if (cfg.target_format == config::format::table) {
        output_stream_ << <component>::domain::convert_to_table(items) << std::endl;
    } else {
        BOOST_THROW_EXCEPTION(
            application_exception("Only JSON and table formats are supported for <entities>"));
    }

    BOOST_LOG_SEV(lg(), debug) << "Exported " << items.size() << " <entity>(s).";
}
#+end_src

Add to =export_data()= switch:
#+begin_src cpp
case config::entity::<entities>:
    export_<entities>(cfg);
    break;
#+end_src
...
#+begin_example
[cli] Register <entities> parser and export operations
#+end_example
...
Add all new commands to CLI recipes using the [[id:A962DF8E-783D-4DCF-BD60-9FEFAA2E1FBF][ores-cli-recipes]] skill. This
...
#+name: <entities>-add
#+begin_src sh :exports none
./ores.cli <entities> add --<key-field> test_value --modified-by admin
#+end_src
...
#+begin_src sh :exports none
./ores.cli <entities> delete --key test_value --modified-by admin
#+end_src
...
- Use =:exports none= on the command blocks (they are included via noweb)
- Use =<<db_env_vars>>= to include database environment variables
