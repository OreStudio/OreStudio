...
read_one(repository::tenor_resolution_algorithm_repository& repo,
         const ores::database::context& ctx,
         const messaging::tenor_resolution_algorithm_key& key) {
 ...
 */
messaging::tenor_resolution_algorithm_key key_from(const domain::tenor_resolution_algorithm& v) {
    messaging::tenor_resolution_algorithm_key key;
    key.code = v.code;
    return key;
}
...
to_domain(const messaging::tenor_resolution_algorithm_write& write) {
    domain::tenor_resolution_algorithm v;
    v.code = write.code;
    v.name = write.name;
...
tenor_resolution_algorithm_service::list_tenor_resolution_algorithms(
    ...
    messaging::list_tenor_resolution_algorithms_response response;
    ...
    response.algorithms = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_algorithm_count(ctx_);
    return response;
}

messaging::get_tenor_resolution_algorithm_response
tenor_resolution_algorithm_service::get_tenor_resolution_algorithm(
    const messaging::get_tenor_resolution_algorithm_request& request) {
    messaging::get_tenor_resolution_algorithm_response response;
    ...
    response.tenor_resolution_algorithm = std::move(found.front());
    return response;
}

messaging::get_many_tenor_resolution_algorithms_response
tenor_resolution_algorithm_service::get_many_tenor_resolution_algorithms(
    const messaging::get_many_tenor_resolution_algorithms_request& request) {
    messaging::get_many_tenor_resolution_algorithms_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenor_resolution_algorithm_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenor_resolution_algorithm = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_tenor_resolution_algorithm_response
tenor_resolution_algorithm_service::put_tenor_resolution_algorithm(
    const messaging::put_tenor_resolution_algorithm_request& request) {
    messaging::put_tenor_resolution_algorithm_response response;
    domain::tenor_resolution_algorithm value;
    ...
    if (!written.empty())
        response.tenor_resolution_algorithm = std::move(written.front());
    return response;
}

messaging::put_many_tenor_resolution_algorithms_response
tenor_resolution_algorithm_service::put_many_tenor_resolution_algorithms(
    const messaging::put_many_tenor_resolution_algorithms_request& request) {
    messaging::put_many_tenor_resolution_algorithms_response response;
    ...
    response.algorithms.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.algorithms.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenor_resolution_algorithm_response
tenor_resolution_algorithm_service::delete_tenor_resolution_algorithm(
    const messaging::delete_tenor_resolution_algorithm_request& request) {
    messaging::delete_tenor_resolution_algorithm_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::tenor_resolution_algorithm_repository::remove_status::removed:
            break;
        case repository::tenor_resolution_algorithm_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenor_resolution_algorithm_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenor_resolution_algorithm_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
tenor_resolution_algorithm_service::delete_many_tenor_resolution_algorithms(
    const messaging::delete_many_tenor_resolution_algorithms_request& request) {
    messaging::delete_many_tenor_resolution_algorithms_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
tenor_resolution_algorithm_service::list_tenor_resolution_algorithm_versions(
    const messaging::list_tenor_resolution_algorithm_versions_request& request) {
    messaging::list_tenor_resolution_algorithm_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
tenor_resolution_algorithm_service::get_tenor_resolution_algorithm_version(
    const messaging::get_tenor_resolution_algorithm_version_request& request) {
    messaging::get_tenor_resolution_algorithm_version_response response;
    auto found = repo_.read_at_version(
        ctx_, request.key.tenor_resolution_algorithm.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result tenor_resolution_algorithm_service::prepare_change(
    const messaging::tenor_resolution_algorithm_change& change,
...
std::vector<domain::tenor_resolution_algorithm>
tenor_resolution_algorithm_service::get_algorithms(const std::vector<std::string>& codes) {
