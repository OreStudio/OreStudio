 ...
 *   SELECT * FROM ores_dq_preview_business_process_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_business_processes(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_business_process_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_business_processes_tbl existing
...
/**
 * Populate ores_refdata_business_processes_tbl from a DQ business_processes dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_business_processes(
    p_dataset_id uuid,
