...
tenant. FX spot is already a first-class citizen: =series_type="FX"=, with every
observation on it carrying =point_id= =SPOT=. NATS request-reply subjects cover query and save operations for all
...
| =asset_class= | string              | Coarse class, a code from =refdata.asset_class_code=: =fx=, =interest_rates=, =credit=, =equity=, =commodity=, =inflation=, =bond=. |
| =subclass=    | string              | Fine class, a code from =refdata.series_subclass_code=: =yield=, =spot=, =forward=, =volatility=, =basis=, =xccy=, etc. |
...
| =point_id=         | string            | Tenor or surface coordinate (e.g. ="1Y"=, ="1Y/ATM"=). Never null: a series whose keys carry no coordinate of their own takes the one its series type names, which is =SPOT= for an FX rate and the empty string for a series with no coordinate at all. |
...
| =asset_class= | =fx= |
| =subclass=    | =spot= |
| =point_id=    | =SPOT= (all observations) |
