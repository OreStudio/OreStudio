...
read_one(repository::ir_curve_bootstrap_pillar_repository& repo,
         ...
         const messaging::ir_curve_bootstrap_pillar_key& key) {
 ...
 */
messaging::ir_curve_bootstrap_pillar_key key_from(const domain::ir_curve_bootstrap_pillar& v) {
    messaging::ir_curve_bootstrap_pillar_key key;
    key.id = v.id;
    return key;
}
...
to_domain(const messaging::ir_curve_bootstrap_pillar_write& write) {
    ...
    v.curve_role_code = write.curve_role_code;
    return v;
}
...
ir_curve_bootstrap_pillar_service::list_ir_curve_bootstrap_pillars(
    ...
    messaging::list_ir_curve_bootstrap_pillars_response response;
    ...
    response.pillars = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_pillar_count(ctx_);
    return response;
}

messaging::get_ir_curve_bootstrap_pillar_response
ir_curve_bootstrap_pillar_service::get_ir_curve_bootstrap_pillar(
    const messaging::get_ir_curve_bootstrap_pillar_request& request) {
    messaging::get_ir_curve_bootstrap_pillar_response response;
    ...
    response.ir_curve_bootstrap_pillar = std::move(found.front());
    return response;
}

messaging::get_many_ir_curve_bootstrap_pillars_response
ir_curve_bootstrap_pillar_service::get_many_ir_curve_bootstrap_pillars(
    const messaging::get_many_ir_curve_bootstrap_pillars_request& request) {
    messaging::get_many_ir_curve_bootstrap_pillars_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::ir_curve_bootstrap_pillar_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.ir_curve_bootstrap_pillar = std::move(found.front());
...
ir_curve_bootstrap_pillar_service::put_ir_curve_bootstrap_pillar(
    const messaging::put_ir_curve_bootstrap_pillar_request& request) {
    messaging::put_ir_curve_bootstrap_pillar_response response;
    domain::ir_curve_bootstrap_pillar value;
    ...
    if (!written.empty())
        response.ir_curve_bootstrap_pillar = std::move(written.front());
    return response;
}

messaging::put_many_ir_curve_bootstrap_pillars_response
ir_curve_bootstrap_pillar_service::put_many_ir_curve_bootstrap_pillars(
    const messaging::put_many_ir_curve_bootstrap_pillars_request& request) {
    messaging::put_many_ir_curve_bootstrap_pillars_response response;
    ...
    response.pillars.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.pillars.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_ir_curve_bootstrap_pillar_response
ir_curve_bootstrap_pillar_service::delete_ir_curve_bootstrap_pillar(
    const messaging::delete_ir_curve_bootstrap_pillar_request& request) {
    messaging::delete_ir_curve_bootstrap_pillar_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::ir_curve_bootstrap_pillar_repository::remove_status::removed:
            break;
        case repository::ir_curve_bootstrap_pillar_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::ir_curve_bootstrap_pillar_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::ir_curve_bootstrap_pillar_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
ir_curve_bootstrap_pillar_service::delete_many_ir_curve_bootstrap_pillars(
    const messaging::delete_many_ir_curve_bootstrap_pillars_request& request) {
    messaging::delete_many_ir_curve_bootstrap_pillars_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
ir_curve_bootstrap_pillar_service::list_ir_curve_bootstrap_pillar_versions(
    const messaging::list_ir_curve_bootstrap_pillar_versions_request& request) {
    messaging::list_ir_curve_bootstrap_pillar_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_ir_curve_bootstrap_pillar_version_response
ir_curve_bootstrap_pillar_service::get_ir_curve_bootstrap_pillar_version(
    const messaging::get_ir_curve_bootstrap_pillar_version_request& request) {
    messaging::get_ir_curve_bootstrap_pillar_version_response response;
    auto found =
        repo_.read_at_version(ctx_,
                              boost::uuids::to_string(request.key.ir_curve_bootstrap_pillar.id),
                              request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result ir_curve_bootstrap_pillar_service::prepare_change(
    const messaging::ir_curve_bootstrap_pillar_change& change,
...
std::vector<domain::ir_curve_bootstrap_pillar>
ir_curve_bootstrap_pillar_service::get_pillars(const std::vector<std::string>& ids) {
