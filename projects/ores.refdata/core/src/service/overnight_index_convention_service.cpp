...
read_one(repository::overnight_index_convention_repository& repo,
         const ores::database::context& ctx,
         const messaging::overnight_index_convention_key& key) {
 ...
 */
messaging::overnight_index_convention_key key_from(const domain::overnight_index_convention& v) {
    messaging::overnight_index_convention_key key;
    key.id = v.id;
    return key;
}
...
to_domain(const messaging::overnight_index_convention_write& write) {
    domain::overnight_index_convention v;
    v.id = write.id;
...
overnight_index_convention_service::list_overnight_index_conventions(
    const messaging::list_overnight_index_conventions_request& request) {
    messaging::list_overnight_index_conventions_response response;
    ...
    response.overnight_index_conventions = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_overnight_index_convention_count(ctx_);
    return response;
}

messaging::get_overnight_index_convention_response
overnight_index_convention_service::get_overnight_index_convention(
    const messaging::get_overnight_index_convention_request& request) {
    messaging::get_overnight_index_convention_response response;
    ...
    response.overnight_index_convention = std::move(found.front());
    return response;
}

messaging::get_many_overnight_index_conventions_response
overnight_index_convention_service::get_many_overnight_index_conventions(
    const messaging::get_many_overnight_index_conventions_request& request) {
    messaging::get_many_overnight_index_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::overnight_index_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.overnight_index_convention = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_overnight_index_convention_response
overnight_index_convention_service::put_overnight_index_convention(
    const messaging::put_overnight_index_convention_request& request) {
    messaging::put_overnight_index_convention_response response;
    domain::overnight_index_convention value;
    ...
    if (!written.empty())
        response.overnight_index_convention = std::move(written.front());
    return response;
}

messaging::put_many_overnight_index_conventions_response
overnight_index_convention_service::put_many_overnight_index_conventions(
    const messaging::put_many_overnight_index_conventions_request& request) {
    messaging::put_many_overnight_index_conventions_response response;
    std::vector<domain::overnight_index_convention> batch;
    ...
    response.overnight_index_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.overnight_index_conventions.push_back(
            written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_overnight_index_convention_response
overnight_index_convention_service::delete_overnight_index_convention(
    const messaging::delete_overnight_index_convention_request& request) {
    messaging::delete_overnight_index_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::overnight_index_convention_repository::remove_status::removed:
            break;
        case repository::overnight_index_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::overnight_index_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::overnight_index_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_overnight_index_conventions_response
overnight_index_convention_service::delete_many_overnight_index_conventions(
    const messaging::delete_many_overnight_index_conventions_request& request) {
    messaging::delete_many_overnight_index_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_overnight_index_convention_versions_response
overnight_index_convention_service::list_overnight_index_convention_versions(
    const messaging::list_overnight_index_convention_versions_request& request) {
    messaging::list_overnight_index_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_overnight_index_convention_version_response
overnight_index_convention_service::get_overnight_index_convention_version(
    const messaging::get_overnight_index_convention_version_request& request) {
    messaging::get_overnight_index_convention_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.overnight_index_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result overnight_index_convention_service::prepare_change(
    const messaging::overnight_index_convention_change& change,
    const ores::utility::domain::change_intent& intent,
    domain::overnight_index_convention& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::overnight_index_convention>
overnight_index_convention_service::get_overnight_index_conventions(
