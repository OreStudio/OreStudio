...
#include "ui_ScriptedInstrumentHistoryDialog.h"
...
ScriptedInstrumentHistoryDialog::ScriptedInstrumentHistoryDialog(
    const QString& id,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::ScriptedInstrumentHistoryDialog),
      id_(id),
...
void ScriptedInstrumentHistoryDialog::setupUi() {
...
void ScriptedInstrumentHistoryDialog::setupToolbar() {
...
void ScriptedInstrumentHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &ScriptedInstrumentHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &ScriptedInstrumentHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &ScriptedInstrumentHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void ScriptedInstrumentHistoryDialog::loadHistory() {
    ...
    QPointer<ScriptedInstrumentHistoryDialog> self = this;

    struct HistoryResult {
        bool success;
        std::string message;
        std::vector<trading::domain::scripted_instrument> versions;
    };
    ...
    auto task = [self, id = id_.toStdString()]() -> HistoryResult {
        ...
        trading::messaging::get_scripted_instrument_history_request request;
        request.id = id;
...
void ScriptedInstrumentHistoryDialog::updateVersionList() {
...
void ScriptedInstrumentHistoryDialog::onVersionSelected() {
...
void ScriptedInstrumentHistoryDialog::updateChangesTable(
    ...
    if (current.script_name != previous.script_name)
        addChange("Script Name",
            QString::fromStdString(previous.script_name),
            QString::fromStdString(current.script_name));
    ...
    if (current.script_body != previous.script_body)
        addChange("Script Body",
            QString::fromStdString(previous.script_body),
            QString::fromStdString(current.script_body));

    if (current.events_json != previous.events_json)
        addChange("Events JSON",
            QString::fromStdString(previous.events_json),
            QString::fromStdString(current.events_json));

    if (current.underlyings_json != previous.underlyings_json)
        addChange("Underlyings JSON",
            QString::fromStdString(previous.underlyings_json),
            QString::fromStdString(current.underlyings_json));

    if (current.parameters_json != previous.parameters_json)
        addChange("Parameters JSON",
            QString::fromStdString(previous.parameters_json),
            QString::fromStdString(current.parameters_json));
...
void ScriptedInstrumentHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->scriptNameValue->setText(
        QString::fromStdString(v.script_name));
...
void ScriptedInstrumentHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void ScriptedInstrumentHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void ScriptedInstrumentHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
    int row = selected.first()->row();
    if (static_cast<std::size_t>(row) >= versions_.size()) return;
