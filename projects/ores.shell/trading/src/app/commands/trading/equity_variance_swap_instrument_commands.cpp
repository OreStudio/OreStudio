...
#include "ores.trading.api/domain/equity_variance_swap_instrument_table_io.hpp" // IWYU pragma: keep.
...
void equity_variance_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_variance_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_equity_variance_swap_instrument(std::ref(out), std::ref(session), args);
        },
        "Add equity variance swap instrument (trade_type_code "
        "party_id "
        "trade_id "
        "underlying_name "
        "currency "
    ...
    equity_variance_swap_instruments_menu->Insert(
        ...
        "Delete equity variance swap instrument by instrument_id");
    ...
    equity_variance_swap_instruments_menu->Insert(
        ...
        "Show equity variance swap instrument's version history");

    root_menu.Insert(std::move(equity_variance_swap_instruments_menu));
}

void equity_variance_swap_instrument_commands::process_get_equity_variance_swap_instruments(
    std::ostream& out, nats_client& session, pagination_context& pagination) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get equity variance swap instrument request.";

    auto& state = pagination.state_for("equity_variance_swap_instruments");

    trading::messaging::get_equity_variance_swap_instruments_request req;
    ...
    pagination.set_last_entity("equity_variance_swap_instruments");

    BOOST_LOG_SEV(lg(), info) << "Successfully retrieved "
                              << result->equity_variance_swap_instruments.size()
                              << " equity variance swap instruments.";
    out << result->equity_variance_swap_instruments << std::endl;
...
void equity_variance_swap_instrument_commands::process_add_equity_variance_swap_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add equity variance swap instrument." << std::endl;
        return;
    }
    ...
    domain::equity_variance_swap_instrument v;
    std::size_t next = 0;
    try {
        ...
        v.variance_strike =
            ores::shell::app::from_token<double>(parsed->positionals[next++], "variance_strike");
        v.start_date =
    ...
    if (result->success) {
        ...
        fail(out) << "Failed to add equity variance swap instrument: " << msg << std::endl;
    }
}

void equity_variance_swap_instrument_commands::process_delete_equity_variance_swap_instrument(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete equity variance swap instrument request for: "
                               << instrument_id;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete equity variance swap instrument."
                  << std::endl;
        return;
    }
    ...
    if (result->success) {
        BOOST_LOG_SEV(lg(), info) << "Successfully deleted equity variance swap instrument.";
        out << "✓ equity variance swap instrument deleted successfully!" << std::endl;
    } else {
...
void equity_variance_swap_instrument_commands::process_get_equity_variance_swap_instrument_history(
    std::ostream& out, nats_client& session, std::string instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get equity variance swap instrument history for: "
                               << instrument_id;
    ...
    trading::messaging::get_equity_variance_swap_instrument_history_request req;
    req.instrument_id = std::move(instrument_id);

    auto result =
        do_auth_request<trading::messaging::get_equity_variance_swap_instrument_history_response>(
            out, session, "trading.v1.equity_variance_swap_instruments.history", req);
    if (!result)
        return;

    if (!result->success) {
        BOOST_LOG_SEV(lg(), warn) << "Failed to get equity variance swap instrument history: "
                                  << result->message;
    ...
    if (result->history.empty()) {
        out << "No history found for this equity variance swap instrument." << std::endl;
        return;
    }
