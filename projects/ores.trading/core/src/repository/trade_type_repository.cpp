...
void trade_type_repository::write(context ctx, const domain::trade_type& v) {
    BOOST_LOG_SEV(lg(), debug) << "Writing trade type. " << "code: " << v.code;
...
std::optional<domain::trade_type> trade_type_repository::read_at_version(context ctx,
                                                                         const std::string& code,
                                                                         std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading trade type at version. " << "code: " << code
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<trade_type_entity>> |
    ...
    const auto entities = execute_read_query<trade_type_entity, domain::trade_type>(
        ctx,
        query,
        [](const auto& entities) { return trade_type_mapper::map(entities); },
        lg(),
        "Reading trade type at version.");
...
trade_type_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest trade types with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<trade_type_entity>> |
    ...
    return execute_read_query<trade_type_entity, domain::trade_type>(
        ...
        [](const auto& entities) { return trade_type_mapper::map(entities); },
        lg(),
        "Reading latest trade types with pagination.");
}

std::uint32_t trade_type_repository::get_total_type_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active trade type count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::select_from<trade_type_entity>(sqlgen::count().as<"count">()) |
                       where("tenant_id"_c == tid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active trade type count: " << count;
    return count;
}
...
void trade_type_repository::remove(context ctx, const std::vector<std::string>& codes) {
    ...
    execute_delete_query(ctx, query, lg(), "Batch removing trade types.");
}
