...
create or replace function ores_workflow_workflow_instances_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity', 'ores.workflow.workflow_instance',
    ...
    perform pg_notify('ores_workflow_workflow_instances', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_workflow_workflow_instances_notify_trg
after insert or update or delete on ores_workflow_workflow_instances_tbl
for each row execute function ores_workflow_workflow_instances_notify_fn();
