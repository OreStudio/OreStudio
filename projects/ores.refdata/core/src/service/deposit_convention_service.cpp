 ...
 */
std::vector<domain::deposit_convention> read_one(repository::deposit_convention_repository& repo,
                                                 const ores::database::context& ctx,
                                                 const messaging::deposit_convention_key& key) {
 ...
 */
messaging::deposit_convention_key key_from(const domain::deposit_convention& v) {
    messaging::deposit_convention_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::deposit_convention to_domain(const messaging::deposit_convention_write& write) {
    domain::deposit_convention v;
    v.id = write.id;
    v.index_based = write.index_based;
    v.index = write.index;
...
messaging::get_deposit_convention_response deposit_convention_service::get_deposit_convention(
    const messaging::get_deposit_convention_request& request) {
    messaging::get_deposit_convention_response response;
    ...
    response.deposit_convention = std::move(found.front());
    return response;
}

messaging::get_many_deposit_conventions_response
deposit_convention_service::get_many_deposit_conventions(
    const messaging::get_many_deposit_conventions_request& request) {
    messaging::get_many_deposit_conventions_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::deposit_convention_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.deposit_convention = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_deposit_convention_response deposit_convention_service::put_deposit_convention(
    const messaging::put_deposit_convention_request& request) {
    messaging::put_deposit_convention_response response;
    domain::deposit_convention value;
    ...
    if (!written.empty())
        response.deposit_convention = std::move(written.front());
    return response;
}

messaging::put_many_deposit_conventions_response
deposit_convention_service::put_many_deposit_conventions(
    const messaging::put_many_deposit_conventions_request& request) {
    messaging::put_many_deposit_conventions_response response;
    ...
    response.deposit_conventions.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.deposit_conventions.push_back(written.empty() ? value :
                                                                 std::move(written.front()));
    }
    return response;
}

messaging::delete_deposit_convention_response deposit_convention_service::delete_deposit_convention(
    const messaging::delete_deposit_convention_request& request) {
    messaging::delete_deposit_convention_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.id, expected)) {
        case repository::deposit_convention_repository::remove_status::removed:
            break;
        case repository::deposit_convention_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::deposit_convention_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::deposit_convention_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_deposit_conventions_response
deposit_convention_service::delete_many_deposit_conventions(
    const messaging::delete_many_deposit_conventions_request& request) {
    messaging::delete_many_deposit_conventions_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_deposit_convention_versions_response
deposit_convention_service::list_deposit_convention_versions(
    const messaging::list_deposit_convention_versions_request& request) {
    messaging::list_deposit_convention_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_deposit_convention_version_response
deposit_convention_service::get_deposit_convention_version(
    const messaging::get_deposit_convention_version_request& request) {
    messaging::get_deposit_convention_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.deposit_convention.id, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
deposit_convention_service::prepare_change(const messaging::deposit_convention_change& change,
...
std::vector<domain::deposit_convention>
deposit_convention_service::get_deposit_conventions(const std::vector<std::string>& ids) {
