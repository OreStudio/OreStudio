...
create table if not exists "ores_trading_trade_id_types_tbl" (
    "code" text not null,
...
create or replace function ores_trading_trade_id_types_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_trading_trade_id_types_tbl"
...
create or replace trigger ores_trading_trade_id_types_insert_trg
before insert on "ores_trading_trade_id_types_tbl"
for each row
execute function ores_trading_trade_id_types_insert_fn();

create or replace rule ores_trading_trade_id_types_delete_rule as
on delete to "ores_trading_trade_id_types_tbl"
do instead
  update "ores_trading_trade_id_types_tbl"
  set valid_to = current_timestamp
...
-- =============================================================================
create or replace function ores_trading_validate_trade_id_type_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if not exists (select 1 from ores_trading_trade_id_types_tbl limit 1) then
        return p_value;
    end if;
