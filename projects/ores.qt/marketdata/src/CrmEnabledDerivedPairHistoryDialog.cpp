...
void CrmEnabledDerivedPairHistoryDialog::loadHistory() {
    ...
    marketdata::messaging::get_crm_enabled_derived_pair_history_request request;
    ...
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](marketdata::messaging::get_crm_enabled_derived_pair_history_response response) {
            if (!self)
                return;
            if (!response.success) {
