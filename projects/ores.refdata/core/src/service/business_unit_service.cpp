 ...
 */
std::vector<domain::business_unit> read_one(repository::business_unit_repository& repo,
                                            const ores::database::context& ctx,
                                            const messaging::business_unit_key& key) {
 ...
 */
messaging::business_unit_key key_from(const domain::business_unit& v) {
...
domain::business_unit to_domain(const messaging::business_unit_write& write) {
    ...
    v.business_centre_code = write.business_centre_code;
    v.unit_type_id = write.unit_type_id;
...
business_unit_service::list_business_units(const messaging::list_business_units_request& request) {
    messaging::list_business_units_response response;
    ...
    response.business_units = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_business_unit_count(ctx_);
    return response;
}

messaging::get_business_unit_response
business_unit_service::get_business_unit(const messaging::get_business_unit_request& request) {
    messaging::get_business_unit_response response;
    ...
    response.business_unit = std::move(found.front());
    return response;
}

messaging::get_many_business_units_response business_unit_service::get_many_business_units(
    const messaging::get_many_business_units_request& request) {
    messaging::get_many_business_units_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::business_unit_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.business_unit = std::move(found.front());
...
business_unit_service::put_business_unit(const messaging::put_business_unit_request& request) {
    messaging::put_business_unit_response response;
    domain::business_unit value;
    ...
    if (!written.empty())
        response.business_unit = std::move(written.front());
    return response;
}

messaging::put_many_business_units_response business_unit_service::put_many_business_units(
    const messaging::put_many_business_units_request& request) {
    messaging::put_many_business_units_response response;
    ...
    response.business_units.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.business_units.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_business_unit_response business_unit_service::delete_business_unit(
    const messaging::delete_business_unit_request& request) {
    messaging::delete_business_unit_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::business_unit_repository::remove_status::removed:
            break;
        case repository::business_unit_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::business_unit_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::business_unit_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_business_units_response business_unit_service::delete_many_business_units(
    const messaging::delete_many_business_units_request& request) {
    messaging::delete_many_business_units_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_business_unit_versions_response business_unit_service::list_business_unit_versions(
    const messaging::list_business_unit_versions_request& request) {
    messaging::list_business_unit_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_business_unit_version_response business_unit_service::get_business_unit_version(
    const messaging::get_business_unit_version_request& request) {
    messaging::get_business_unit_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.business_unit.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
business_unit_service::prepare_change(const messaging::business_unit_change& change,
...
business_unit_service::get_business_units(const std::vector<std::string>& ids) {
