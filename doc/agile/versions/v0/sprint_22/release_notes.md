...
Sprint 22 set out to continue commissioning the ores.refdata entity library and correct codegen C++ generation drift, carrying postponed sprint 21 stories forward. It overran badly: the sprint ran 9 days against a 7-day/300-commit budget, closing at 741 commits and 99 merged PRs, and three System 2 health reviews during the sprint (day 3, day 4, close) all verdicted RED, driven mainly by 20 stories running simultaneously STARTED at once rather than any single story running long. What did land is substantial and mostly outside the originally stated mission: a complete Currency Pairs & Conventions entity end to end, a new `ores.orgmode` component and the QA Validation Runner built on it, a proven-out org-mode literate codegen model, and a cluster of real Windows/macOS CI and multi-tenant correctness fixes. Of the sprint's 70 stories, 27 shipped DONE; the remaining 43 — including most of the entity-commissioning backlog and the codegen-drift-correction work the mission actually named — are carried back to the product backlog inbox rather than force-closed, to be re-triaged and re-planned into future sprints with a tighter WIP limit.
...
-   **Commission: currency**: 7/8 tasks done; Qt verification carried from sprint 20.
-   **Commission: rounding\_type**: Aux type: verify SQL, sync all codegen profiles, verify Qt UI, document in currency chapter.
-   **Commission: monetary\_nature**: Aux type: verify SQL, sync all codegen profiles, verify Qt UI, document in currency chapter.
-   **Commission: currency\_market\_tier**: Aux type: verify SQL, sync all codegen profiles, verify Qt UI, document in currency chapter.
...
-   **Commission: book**: Verify Qt; implement shell + CLI; manual; Wt/HTTP captures.
-   **Commission: book\_status**: Verify Qt; implement shell + CLI; manual; Wt/HTTP captures.
...
-   **Commission: purpose\_type**: Verify Qt; implement shell + CLI; manual; Wt/HTTP captures.
...
-   **Move book, book\_status, and portfolio to ores.trading**: These are trading/booking structures, not static reference data; relocate full stack out of ores.refdata.
-   **Regen entities with latest codegen templates**: Sweep already-commissioned entities (starting with country) against the shared codegen templates' substantial new capability landed via currency's commissioning.
...
-   **Temporal composite entity versioning**: Produce a comprehensive architecture document under doc/architecture laying out the target state for complex/composite object versioning (option A: child saves bump the parent's version; loading a parent as-of version N composes children as of that same version), and work out how to enforce it across SQL (triggers), codegen (templates/facets), and the Qt UI (including how a composite entity's version/history dialog should look, e.g.
-   **Implement temporal composite entity versioning**: Implement the target state: SQL touch-function mechanism, as-of window-join read-side composition, Qt composite history dialog.
...
-   **Synthetic data collections: Basic and Realistic**: Two sample FX collections seeded from real ORE reference data (2016-02-05 vintage), Market Simulator grouping by collection, feed namespacing, vintage-availability guard.
...
-   **Currency pair support in reference data**: Knowledge-base foundation (conventions, quoting, cross rates) followed by domain/db/codegen support for currency pairs as reference data.
...
-   **Refactor ores.codegen C++ generation**: Audit/resolve template-production drift; carried from sprint 21.
...
-   **Composite child-entity and hierarchy Qt widgets for codegen**: Handcrafted HierarchyTreeWidget + child-entity table facet, paste-block-bound, so party/counterparty can retire their hand-maintained EntityDetailDialog.
-   **compass generate catalogue**: Auto-regenerate catalogue/index org files by scanning tagged org files.
-   **Fix orphan documents and tag taxonomy**: Fix orphaned docs in org-roam graph; create tag inventory; add compass tag validation.
...
-   **QA Validation Runner: in-app test tracking for the multi-worktree fleet**: Qt panel, built on ores.orgmode, turning a test\_scenario org doc's checklist into a guided test session; results and story/task context rendered in-app, not a standalone JSON file.
...
-   **Commission: currency** (ABANDONED): deferred.
-   **Commission: rounding\_type** (ABANDONED): deferred.
-   **Commission: monetary\_nature** (ABANDONED): deferred.
-   **Commission: currency\_market\_tier** (ABANDONED): deferred.
...
-   **Commission: book** (ABANDONED): deferred.
-   **Commission: book\_status** (ABANDONED): deferred.
...
-   **Commission: purpose\_type** (ABANDONED): deferred.
-   **Commission: day\_count\_fraction\_type** (ABANDONED): deferred.
-   **Move book, book\_status, and portfolio to ores.trading** (ABANDONED): deferred.
-   **Regen entities with latest codegen templates** (ABANDONED): deferred.
...
-   **Implement temporal composite entity versioning** (ABANDONED): deferred.
-   **IR Rates synthetic data generation** (ABANDONED): deferred.
-   **Synthetic data collections: Basic and Realistic** (ABANDONED): deferred.
-   **Move trade import orchestration to service layer** (ABANDONED): deferred.
-   **Currency pair support in reference data** (ABANDONED): deferred.
-   **Decommission ores.codegen.table model type** (ABANDONED): deferred.
-   **Refactor ores.codegen C++ generation** (ABANDONED): deferred.
...
-   **Composite child-entity and hierarchy Qt widgets for codegen** (ABANDONED): deferred.
-   **compass generate catalogue** (ABANDONED): deferred.
-   **Fix orphan documents and tag taxonomy** (ABANDONED): deferred.
-   **Compass GitHub Actions support** (ABANDONED): deferred.
-   **QA Validation Runner: in-app test tracking for the multi-worktree fleet** (ABANDONED): deferred.
...
*Next sprint: re-triage the 43 carried stories with a hard WIP limit on simultaneously-STARTED work, and pick up the codegen drift correction the mission named but this sprint barely moved.*
