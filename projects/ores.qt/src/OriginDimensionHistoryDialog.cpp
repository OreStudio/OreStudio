...
void OriginDimensionHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &OriginDimensionHistoryDialog::onVersionSelected);
...
void OriginDimensionHistoryDialog::loadHistory() {
    ...
    auto task = [self, code = code_.toStdString()]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_origin_dimension_history_request,
            0, std::move(payload)
        );
        ...
        auto response = dq::messaging::get_origin_dimension_history_response::
