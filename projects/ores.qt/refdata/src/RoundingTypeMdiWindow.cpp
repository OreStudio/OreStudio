...
RoundingTypeMdiWindow::RoundingTypeMdiWindow(ClientManager* clientManager,
...
void RoundingTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &RoundingTypeMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &RoundingTypeMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &RoundingTypeMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &RoundingTypeMdiWindow::viewHistorySelected);
}
...
void RoundingTypeMdiWindow::setupConnections() {
    connect(
        model_, &ClientRoundingTypeModel::dataLoaded, this, &RoundingTypeMdiWindow::onDataLoaded);
    connect(model_, &ClientRoundingTypeModel::loadError, this, &RoundingTypeMdiWindow::onLoadError);
    connectModel(model_);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &RoundingTypeMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &RoundingTypeMdiWindow::onDoubleClicked);
...
void RoundingTypeMdiWindow::doReload() {
...
void RoundingTypeMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 rounding types").arg(loaded).arg(total));
...
void RoundingTypeMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void RoundingTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete rounding type while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " rounding types";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 rounding types?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Rounding Type", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making batch delete request for " << codes.size() << " rounding types";
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Rounding Type deletion failed: " << code << " - " << result.second;
                failure_count++;
