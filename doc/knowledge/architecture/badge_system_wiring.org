...
=(domain, entity_code)= pair to a colour. For codegen-backed entities,
adding a new badge needs no hand-written UI code — the entity's
presentation drawer carries the badge in two table columns
(=is_badge=/=badge_key=), and the generator emits those as =codeDomain=
in the entity's TypeScript UI metadata. Only the SQL population entry is
manual. Because a missing SQL entry fails silently (the resolver falls
back to the shared fallback colour, not an error), it is the step most
often forgotten. The UI-side resolver went with the Qt client and is not
rebuilt yet — see §UI resolution below. Return to
...
removed. The TypeScript web client in =ores.web= holds the generated UI
metadata and a shared =DataTable=, but it carries no badge resolver yet,
so nothing resolves or paints a badge today. *Do not look for a live resolver;*
there is none.
...
drawer, and the generator emits the pair as =codeDomain= in the
entity's =_ui.ts= file. A screen that reads =codeDomain= resolves it
against the code domains below and paints the badge definition's
...
- UI wiring: the =account_type= column carries =is_badge= and
  =badge_key: account_type=, which the generator emits as
  =codeDomain= in the entity's =_ui.ts= file. The delegate and the
  combo painting went with the Qt client (§UI resolution above).
