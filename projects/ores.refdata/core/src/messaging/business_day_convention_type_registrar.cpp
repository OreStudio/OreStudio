...
std::vector<ores::nats::service::subscription> register_business_day_convention_type_handlers(
    ...
    subs.push_back(nats.queue_subscribe(get_business_day_convention_types_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->list(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(save_business_day_convention_type_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->save(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(delete_business_day_convention_type_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->remove(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(get_business_day_convention_type_history_request::nats_subject,
                             queue_group,
