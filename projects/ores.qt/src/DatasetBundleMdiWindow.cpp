...
void DatasetBundleMdiWindow::setupTable() {
    model_ = new ClientDatasetBundleModel(clientManager_, this);
...
void DatasetBundleMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void DatasetBundleMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void DatasetBundleMdiWindow::updateActionStates() {
...
void DatasetBundleMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new dataset bundle requested";
    emit addNewRequested();
}

void DatasetBundleMdiWindow::editSelected() {
...
void DatasetBundleMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void DatasetBundleMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (ids.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid dataset bundles to delete";
        return;
    }
