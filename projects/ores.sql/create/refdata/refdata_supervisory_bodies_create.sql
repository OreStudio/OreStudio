...
create table if not exists "ores_refdata_supervisory_bodies_tbl" (
    "code" text not null,
...
create unique index if not exists supervisory_bodies_version_uniq_idx
on "ores_refdata_supervisory_bodies_tbl" (tenant_id, code, coding_scheme_code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists supervisory_bodies_code_uniq_idx
on "ores_refdata_supervisory_bodies_tbl" (tenant_id, code, coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists supervisory_bodies_tenant_idx
on "ores_refdata_supervisory_bodies_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists supervisory_bodies_coding_scheme_idx
...
-- =============================================================================
-- Validation function for supervisory_body
-- Validates that a code exists in the supervisory_bodies table.
-- Returns the validated value, or default if null/empty.
-- Uses current tenant data.
-- =============================================================================
create or replace function ores_refdata_validate_supervisory_body_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid supervisory_body: value cannot be null or empty'
            using errcode = '23502';
    end if;

    -- Allow pass-through during bootstrap (empty table)
    if not exists (select 1 from ores_refdata_supervisory_bodies_tbl limit 1) then
        return p_value;
    end if;

    -- Validate against reference data
    if not exists (
        select 1 from ores_refdata_supervisory_bodies_tbl
        where tenant_id = p_tenant_id
          and code = p_value
          and valid_to = ores_utility_infinity_timestamp_fn()
    ) then
        raise exception 'Invalid supervisory_body: %. Must be one of: %', p_value, (
