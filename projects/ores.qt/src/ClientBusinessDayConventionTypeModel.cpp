...
int ClientBusinessDayConventionTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientBusinessDayConventionTypeModel::refresh() {
...
void ClientBusinessDayConventionTypeModel::onTypesLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " business day convention types newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " business day convention types."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientBusinessDayConventionTypeModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientBusinessDayConventionTypeModel::onPulsingComplete() {
