...
private:
    ...
    static std::string synthetic_subject(const std::string& source_name, bool is_curve);
    ...
    // uses), so the kind token is chosen by membership in irCurves_ instead of string-sniffing.
