...
inline auto& tenant_status_handler_lg() {
    static auto instance = ores::logging::make_logger(
        "ores.iam.messaging.tenant_status_handler");
    return instance;
}
...
public:
    ...
    void save(ores::nats::message msg) {
        ...
        if (!req) {
            BOOST_LOG_SEV(tenant_status_handler_lg(), warn)
        ...
        try {
            ...
            BOOST_LOG_SEV(tenant_status_handler_lg(), debug)
                << "Completed " << msg.subject;
            reply(nats_, msg,
                save_tenant_status_response{.success = true});
        } catch (const std::exception& e) {
            BOOST_LOG_SEV(tenant_status_handler_lg(), error)
                << msg.subject << " failed: " << e.what();
            reply(nats_, msg, save_tenant_status_response{
    ...
    void del(ores::nats::message msg) {
        ...
        try {
            ...
            BOOST_LOG_SEV(tenant_status_handler_lg(), debug)
                << "Completed " << msg.subject;
            reply(nats_, msg,
                delete_tenant_status_response{.success = true});
        } catch (const std::exception& e) {
            BOOST_LOG_SEV(tenant_status_handler_lg(), error)
                << msg.subject << " failed: " << e.what();
            reply(nats_, msg, delete_tenant_status_response{
                .success = false, .message = e.what()});
        }
    }
    ...
    void history(ores::nats::message msg) {
        ...
        try {
            ...
            BOOST_LOG_SEV(tenant_status_handler_lg(), debug)
                << "Completed " << msg.subject;
            reply(nats_, msg, get_tenant_status_history_response{
                .success = true,
