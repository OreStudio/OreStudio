...
Add ~ores.<name>.service~ to the ~SERVICES~ array in
~build/scripts/generate_nats_certs.sh~, then generate the certificate:

#+begin_src sh
build/scripts/generate_nats_certs.sh
#+end_src

The script skips existing certificates, so running without ~--force~ is safe
...
| 7 | ~generate_nats_certs.sh~ + run script | Cert generated by script |
| 8 | Run ~recreate_database.sh -k -y~ | — |
| 9 | Run ~init-environment.sh~ | ~.env~ entries generated |
