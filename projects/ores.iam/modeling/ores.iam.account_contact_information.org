...
#+description: Contact details for an account (address/phone/email/web page) -- the person's name lives on the account itself, not here.
...
:key_field:               email
:get_request_class:       iam::messaging::list_account_contact_informations_request
:get_response_class:      iam::messaging::list_account_contact_informations_response
:get_message_type:        list_account_contact_informations_request
:save_request_class:      iam::messaging::put_account_contact_information_request
:save_response_class:     iam::messaging::put_account_contact_information_response
:save_message_type:       put_account_contact_information_request
...
:history_request_class:   iam::messaging::list_account_contact_information_versions_request
:history_response_class:  iam::messaging::list_account_contact_information_versions_response
:history_message_type:    list_account_contact_information_versions_request
:has_explorer_api:        true
:has_export_macro:        true
:parent_entity_singular:  account
:parent_id_field:         account_id
:parent_id_field_camel:   accountId
...
The publish trigger is declared here beside the derived protocol, because it
is a subject IAM owns and nothing else states it: the DQ artefact-type row
that publishes onto it carries the same string in SQL, and a constant
generated from here is what the registrar subscribes with. The separator is
the one the other components' publish-from-dq subjects use, which is not this
entity's protocol spelling; it is stated as it is on the wire.

** publish_account_contact_informations_from_dq_request
:PROPERTIES:
:subject: iam.v1.account-contact-informations.publish-from-dq
:END:
