...
CatalogController::CatalogController(QMainWindow* mainWindow,
                                     QMdiArea* mdiArea,
...
void CatalogController::showListWindow() {
    ...
    // Connect signals
    connect(listWindow_, &CatalogMdiWindow::statusChanged, this, &CatalogController::statusMessage);
    connect(listWindow_, &CatalogMdiWindow::errorOccurred, this, &CatalogController::errorMessage);
    connect(listWindow_,
            &CatalogMdiWindow::showCatalogDetails,
            this,
            &CatalogController::onShowDetails);
    connect(listWindow_,
            &CatalogMdiWindow::addNewRequested,
            this,
            &CatalogController::onAddNewRequested);
    connect(listWindow_,
            &CatalogMdiWindow::showCatalogHistory,
            this,
            &CatalogController::onShowHistory);
...
void CatalogController::wireDetailDialogCommon(CatalogDetailDialog* detailDialog) {
    ...
    connect(
        detailDialog, &CatalogDetailDialog::statusMessage, this, &CatalogController::statusMessage);
    connect(
        detailDialog, &CatalogDetailDialog::errorMessage, this, &CatalogController::errorMessage);
}
...
void CatalogController::showAddWindow() {
    ...
    connect(detailDialog,
            ...
            [self = QPointer<CatalogController>(this)](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info) << "Catalog saved: " << code.toStdString();
...
void CatalogController::showDetailWindow(const dq::domain::catalog& item) {
    ...
    connect(detailDialog,
            ...
            [self = QPointer<CatalogController>(this), key](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info) << "Catalog deleted: " << code.toStdString();
...
void CatalogController::showHistoryWindow(const QString& code) {
    BOOST_LOG_SEV(lg(), info) << "Opening history window for catalog: " << code.toStdString();
    ...
    auto* historyDialog = new HistoryDialog(std::string(entity_type_of(dq::domain::catalog{})),
                                            code.toStdString(),
                                            clientManager_,
                                            mainWindow_);
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<CatalogController>(this)](const QString& message) {
                if (!self)
                    return;
    ...
    connect(historyDialog,
            ...
            this,
            [self = QPointer<CatalogController>(this)](
    ...
    historyWindow->setWindowTitle(QString("Catalog History: %1").arg(code));
...
void CatalogController::onOpenVersion(const dq::domain::catalog& item, int versionNumber) {
    ...
    detailWindow->setAttribute(Qt::WA_DeleteOnClose);
    detailWindow->setWidget(detailDialog);
    detailWindow->setWindowTitle(QString("Catalog: %1 (Version %2)").arg(code).arg(versionNumber));
    detailWindow->setWindowIcon(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor));
...
void CatalogController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<CatalogController> self = this;
    fetchCatalogHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<dq::domain::catalog>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void CatalogController::onRevertHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<CatalogController> self = this;
    fetchCatalogHistory(
        entityId,
        [self, entityId, versionNumber](
...
void CatalogController::onRevertVersion(const dq::domain::catalog& item) {
    ...
    connect(detailDialog,
            ...
            [self = QPointer<CatalogController>(this)](const QString& code) {
                if (!self)
                    return;
                BOOST_LOG_SEV(lg(), info) << "Catalog reverted: " << code.toStdString();
                emit self->statusMessage(QString("Catalog '%1' reverted successfully").arg(code));
                self->handleEntitySaved();
            });
...
void CatalogController::notifyOpenDialogs(const QStringList& entityIds) {
