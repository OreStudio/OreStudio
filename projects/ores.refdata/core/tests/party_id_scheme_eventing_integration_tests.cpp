...
// notification" pattern end to end for party_id_scheme -- the
...
TEST_CASE("write_party_id_scheme_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::party_id_scheme_changed_event>(
        [&nats](const ores::refdata::eventing::party_id_scheme_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::party_id_scheme_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.party_id_scheme",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::party_id_scheme_changed_event>(
        "ores.refdata.party_id_scheme", "ores_refdata_party_id_schemes");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::party_id_scheme_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_party_id_scheme(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Party ID Scheme: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.party_id_scheme") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for party_id_scheme " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for party_id_scheme " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.find_scheme(id_str).has_value());
        REQUIRE(svc.get_scheme_history(id_str).size() == versions.size());
    }
}
