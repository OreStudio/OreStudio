...
#include "ores.refdata.api/domain/purpose_type_json_io.hpp" // IWYU pragma: keep.
#include "ores.refdata.api/eventing/purpose_type_changed_event.hpp"
#include "ores.refdata.api/generators/purpose_type_generator.hpp"
...
// notification" pattern end to end for purpose_type -- the
...
using ores::refdata::domain::purpose_type;
using ores::refdata::repository::purpose_type_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_purpose_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::purpose_type_changed_event>(
        [&nats](const ores::refdata::eventing::purpose_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::purpose_type_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::purpose_type_changed_event>(
        "ores.refdata.purpose_type", "ores_refdata_purpose_types");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::purpose_type_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_purpose_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Purpose Type: " << v;

    purpose_type_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.purpose_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for purpose_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for purpose_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::purpose_type_service svc(crud_ctx);
