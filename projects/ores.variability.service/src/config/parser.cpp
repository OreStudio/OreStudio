...
const std::string product_version("ores.variability.service v" ORES_VERSION);
...
using ores::variability::service::config::options;
using ores::variability::service::config::parser_exception;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.variability.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Feature flags and runtime configuration service" << std::endl << std::endl
         << "Usage: ores.variability.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("VARIABILITY_SERVICE"));
