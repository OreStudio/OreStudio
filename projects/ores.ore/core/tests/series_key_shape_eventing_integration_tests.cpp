...
#include "ores.ore.api/eventing/series_key_shape_changed_event.hpp"
#include "ores.ore.api/generators/series_key_shape_generator.hpp"
...
// notification" pattern end to end for series_key_shape -- the
...
const std::string_view test_suite("ore.tests");
const std::string tags("[eventing][integration]");


}

using namespace ores::ore::generators;
using ores::ore::domain::series_key_shape;
using ores::ore::repository::series_key_shape_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_series_key_shape_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::ore::eventing::series_key_shape_changed_event>(
        [&nats](const ores::ore::eventing::series_key_shape_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::ore::eventing::series_key_shape_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.ore.series_key_shape",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.series_types,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::ore::eventing::series_key_shape_changed_event>(
        "ores.ore.series_key_shape", "ores_ore_series_key_shapes");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::ore::eventing::series_key_shape_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_series_key_shape(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.series_type;
    BOOST_LOG_SEV(lg, debug) << "Series Key Shape: " << v;

    series_key_shape_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.ore.series_key_shape") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for series_key_shape " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for series_key_shape " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_shape(id_str).has_value());
        REQUIRE(svc.get_shape_history(id_str).size() == versions.size());
    }
}
