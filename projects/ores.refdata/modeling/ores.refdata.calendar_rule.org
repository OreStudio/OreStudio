...
TODO -- no natural key: a calendar can have many rules with no
per-column uniqueness, so =calendar_code= lives under Columns/Foreign
keys instead (mirrors =book.parent_portfolio_id=, a mandatory
non-unique parent reference).
...
| day_offset      | Day Offset    | dayOffsetSpin      | spin_box  |        |             |           |                             |                        |                             |                    |                                    |                       |                    |                      |                   |
