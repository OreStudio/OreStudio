 ...
 *
 */
#ifndef ORES_QT_CLIENT_CURRENCY_ASSET_CLASS_MODEL_HPP
#define ORES_QT_CLIENT_CURRENCY_ASSET_CLASS_MODEL_HPP
 ...
 */
class ClientCurrencyAssetClassModel final : public QAbstractTableModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name =
        "ores.qt.client_currency_asset_class_model";
...
public:
    ...
    explicit ClientCurrencyAssetClassModel(ClientManager* clientManager,
                                       QObject* parent = nullptr);
    ~ClientCurrencyAssetClassModel() override = default;
     ...
     */
    const refdata::domain::currency_asset_class* getClass(int row) const;
...
private:
    ...
    std::vector<refdata::domain::currency_asset_class> types_;
    QFutureWatcher<FetchResult>* watcher_;
    std::uint32_t page_size_{100};
    std::uint32_t total_available_count_{0};
    bool is_fetching_{false};

    using CurrencyAssetClassKeyExtractor = std::string(*)(const refdata::domain::currency_asset_class&);
    RecencyTracker<refdata::domain::currency_asset_class, CurrencyAssetClassKeyExtractor> recencyTracker_;
