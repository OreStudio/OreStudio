...
#+description: The ORE Studio web component specification: navigation model, component declaration, registry, routes, shared versus per-entity content, file layout, derived declarations, and decisions.
#+type: knowledge
#+level: cross
...
Read =entity_specification.org= for what an entity screen does. This document is
...
The test of whether the design is right: /adding an entity should touch one new file/ — its declaration and its column and field lists — /plus the protocol schemas that codegen is producing./ If it touches a shared component, the shared component is missing an abstraction.
...
grouping and the entity's description; see =codegen_ts_ui_request.org=.
