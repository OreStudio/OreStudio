...
/**
 * @brief Model for displaying rounding types fetched from the server.
 *
 * This model extends QAbstractTableModel and fetches rounding type
 * data asynchronously using the ores.comms client.
 */
class ClientRoundingTypeModel final : public QAbstractTableModel {
    Q_OBJECT
...
public:
    ...
    /**
     * @brief Default window size for the rounding type list window.
     */
     ...
     */
    static constexpr std::string_view kSettingsGroup = "RoundingTypeListWindow";
    ...
    /**
     * @brief Get rounding type at the specified row.
     *
     * @param row The row index.
     * @return The rounding type, or nullptr if row is invalid.
     */
    const refdata::domain::rounding_type* getType(int row) const;
...
private:
    ...
    std::vector<refdata::domain::rounding_type> types_;
