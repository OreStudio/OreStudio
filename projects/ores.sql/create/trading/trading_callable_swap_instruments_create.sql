...
-- =============================================================================
-- Callable Swap Instruments Table
--
-- Swap with embedded call (or put) options on specified dates. ORE product
-- type: CallableSwap. Leg economics live in ores_trading_swap_legs_tbl.
-- call_dates_json holds a JSON array of ISO-8601 dates on which the call
-- may be exercised. call_type distinguishes Bermudan from one-time options.
-- =============================================================================
...
create unique index if not exists ores_trading_callable_swap_instruments_version_uniq_idx
on "ores_trading_callable_swap_instruments_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Current record uniqueness
create unique index if not exists ores_trading_callable_swap_instruments_id_uniq_idx
on "ores_trading_callable_swap_instruments_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Tenant index
create index if not exists ores_trading_callable_swap_instruments_tenant_idx
on "ores_trading_callable_swap_instruments_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Party index for RLS
create index if not exists ores_trading_callable_swap_instruments_party_idx
on "ores_trading_callable_swap_instruments_tbl" (tenant_id, party_id)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Soft FK back to trade (NULL for standalone instruments)
create unique index if not exists ores_trading_callable_swap_instruments_trade_id_idx
