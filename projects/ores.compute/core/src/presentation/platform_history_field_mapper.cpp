...
std::vector<ores::diff::domain::field_value> render_platform_fields(const domain::platform& v) {
    ...
    fields.push_back({.name = "Os Family", .value = v.os_family});
    fields.push_back({.name = "Cpu Arch", .value = v.cpu_arch});
    fields.push_back({.name = "Abi", .value = v.abi});
