 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_CRM_TOPOLOGY_CONFIG_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_CRM_TOPOLOGY_CONFIG_EVENT_HPP
...
/**
 * @brief Event traits for crm topology config's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::crm_topology_config_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.crm_topology_configs_events";

    static ores::refdata::messaging::crm_topology_config_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::crm_topology_config_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::refdata::messaging::crm_topology_config_key>(
                notification.key))
            event.key = *key;
