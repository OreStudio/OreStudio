 ...
 *
 */
#ifndef ORES_QT_CLIENT_RESULT_MODEL_HPP
#define ORES_QT_CLIENT_RESULT_MODEL_HPP
...
/**
 * @brief Model for displaying compute results fetched from the server.
 *
 * This model extends QAbstractTableModel and fetches compute result
 * data asynchronously using the ores.comms client.
 */
class ORES_QT_API ClientResultModel final : public AbstractClientModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.client_result_model";
...
public:
    ...
    explicit ClientResultModel(ClientManager* clientManager, QObject* parent = nullptr);
    ...
    /**
     * @brief Refresh compute result data from server asynchronously.
     */
    void refresh();

    /**
     * @brief Get compute result at the specified row.
     *
     * @param row The row index.
     * @return The compute result, or nullptr if row is invalid.
     */
...
private:
    ...
    struct FetchResult {
        bool success;
        std::vector<compute::domain::result> results;
    ...
    using ResultKeyExtractor = std::string (*)(const compute::domain::result&);
    RecencyTracker<compute::domain::result, ResultKeyExtractor> recencyTracker_;
