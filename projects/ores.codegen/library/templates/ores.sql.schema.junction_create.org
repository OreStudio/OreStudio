...
Row level security is opt-in and tenant-scoped only. A junction turns
it on with =:rls_tenant_isolation:= in its =* SQL / ** Flags= drawer,
and =:rls_system_tenant_visible:= widens the policy to the system
tenant. =:rls_party_isolation:= belongs to the domain entity archetype
and is not read here, so setting it on a junction changes nothing.
...
{{#nullable}}
    ...
    if new.{{column}} is not null then
        if not exists (
            select 1 from {{table}}
            where {{#junction.has_tenant_id}}tenant_id = new.tenant_id
              and {{/junction.has_tenant_id}}{{target_column}} = new.{{column}}
              and valid_to = ores_utility_infinity_timestamp_fn()
        ) then
...
{{/nullable}}
{{/junction.foreign_keys}}
{{#junction.insert_trigger.validations}}
    -- Validate {{column}}
    new.{{column}} := {{validation_function}}({{#junction.has_tenant_id}}new.tenant_id, {{/junction.has_tenant_id}}new.{{column}});
...
do instead
  ...
  and valid_to = ores_utility_infinity_timestamp_fn();
{{#junction.sql.rls_tenant_isolation}}

-- =============================================================================
-- Row-level security: tenant isolation for {{junction.name_title}}
{{#junction.sql.rls_system_tenant_visible}}
-- System-tenant sessions may also read every tenant's rows.
{{/junction.sql.rls_system_tenant_visible}}
-- =============================================================================
alter table "{{junction.product}}_{{junction.component}}_{{junction.name}}_tbl" enable row level security;

create policy {{junction.name}}_tbl_tenant_isolation_policy
on "{{junction.product}}_{{junction.component}}_{{junction.name}}_tbl"
for all using (
    tenant_id = ores_iam_current_tenant_id_fn(){{#junction.sql.rls_system_tenant_visible}}
    OR ores_iam_current_tenant_id_fn() = ores_utility_system_tenant_id_fn(){{/junction.sql.rls_system_tenant_visible}}
)
with check (
    tenant_id = ores_iam_current_tenant_id_fn(){{#junction.sql.rls_system_tenant_visible}}
    OR ores_iam_current_tenant_id_fn() = ores_utility_system_tenant_id_fn(){{/junction.sql.rls_system_tenant_visible}}
