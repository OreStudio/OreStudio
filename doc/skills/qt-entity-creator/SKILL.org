...
pulsing color effect, use the =RecencyPulseManager= utility class:
...
private:
    ...
    std::unordered_set<std::string> recentIdentifiers_;
    QDateTime lastReloadTime_;
    RecencyPulseManager* pulseManager_;
#+end_src

2. In constructor, connect signals:
...
if (role == Qt::ForegroundRole) {
    return foregroundColor(entry.identifier);
}
#+end_src
...
#+begin_src cpp
void Client<Entity>Model::updateRecentEntries() {
    recentIdentifiers_.clear();
...
QVariant Client<Entity>Model::foregroundColor(const std::string& id) const {
    if (recentIdentifiers_.count(id) && pulseManager_->is_pulse_on()) {
...
5. After loading data, call =updateRecentEntries()= and start pulsing if needed:

#+begin_src cpp
updateRecentEntries();
if (!recentIdentifiers_.empty() && !pulseManager_->is_pulsing()) {
...
1. Database notification trigger exists (see [[id:7e4c9d3a-2f1b-4e5c-8d6a-9f2e3c4b5a6d][sql-schema-creator]] skill)
...
void <Entity>Controller::showListWindow() {
    ...
    // Track destruction
    connect(listMdiSubWindow_, &QObject::destroyed, this, [this]() {
        listWindow_ = nullptr;
        listMdiSubWindow_ = nullptr;
    });
...
void <Entity>Controller::showDetailWindow(
    ...
    connect(detailWindow, &QObject::destroyed, this, [this, windowKey]() {
        managed_windows_.remove(windowKey);
    });
