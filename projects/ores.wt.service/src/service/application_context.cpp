...
void application_context::initialize(const database::database_options& db_opts) {
    ...
    BOOST_LOG_SEV(lg(), info) << "Database: " << db_opts.host << ":" << db_opts.port << "/"
                              << db_opts.database;

    database::context_factory::configuration cfg{.database_options = db_opts,
                                                 .pool_size =
                                                     static_cast<std::size_t>(db_opts.pool_size),
                                                 .num_attempts = 10,
                                                 .wait_time_in_seconds = 1,
                                                 .service_account = db_opts.user};

    db_context_ = std::make_unique<database::context>(database::context_factory::make_context(cfg));
...
void application_context::setup_services() {
    ...
    authorization_service_ = std::make_shared<iam::service::authorization_service>(*db_context_);
    ...
    account_service_ = std::make_unique<iam::service::account_service>(*db_context_);

    account_setup_service_ = std::make_unique<iam::service::account_setup_service>(
        *account_service_, authorization_service_);

    currency_service_ = std::make_unique<refdata::service::currency_service>(*db_context_);

    country_service_ = std::make_unique<refdata::service::country_service>(*db_context_);
...
void application_context::setup_eventing() {
    ...
    event_source_ =
        std::make_unique<eventing::service::postgres_event_source>(*db_context_, *event_bus_);
    ...
    flags_subscription_ =
        event_bus_->subscribe<variability::eventing::system_setting_changed_event>(
            [this](const variability::eventing::system_setting_changed_event& e) {
                BOOST_LOG_SEV(lg(), info) << "System settings changed notification received, "
                                          << "refreshing settings cache (" << e.setting_names.size()
                                          << " settings changed)";
                system_settings_service_->refresh();
...
void application_context::check_bootstrap_mode() {
    ...
    iam::service::bootstrap_mode_service bootstrap_svc(
        *db_context_, database::service::tenant_context::system_tenant_id, authorization_service_);
