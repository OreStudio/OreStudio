...
create or replace function ores_iam_roles_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    perform pg_notify('ores_iam_roles', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_iam_roles_notify_trg
after insert or update or delete on ores_iam_roles_tbl
for each row execute function ores_iam_roles_notify_fn();
