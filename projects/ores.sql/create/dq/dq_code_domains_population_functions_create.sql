...
/**
 * Code Domains Population Functions
 *
 * Functions to publish code domains from the DQ artefact table into a
 * tenant's own copy of ores_dq_code_domains_tbl. Self-referential (like
 * coding_schemes/badge_severities/badge_definitions): DQ publishes into its
 * own table, not another service's.
 *
 * Usage:
 *   -- Preview what will be published
 *   SELECT * FROM ores_dq_code_domain_preview_fn(dataset_id);
 *
 *   -- Publish to a tenant
 *   SELECT * FROM ores_dq_code_domains_publish_fn(dataset_id, target_tenant_id, 'upsert');
 */
...
create or replace function ores_dq_code_domain_preview_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
    name text,
...
begin
    ...
    from ores_dq_code_domains_artefact_tbl dq
    left join ores_dq_code_domains_tbl existing
...
/**
 * Populate ores_dq_code_domains_tbl from a DQ code domains dataset.
 *
 ...
 */
create or replace function ores_dq_code_domains_publish_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        select exists (
            select 1 from ores_dq_code_domains_tbl existing
