...
read_one(repository::day_count_fraction_type_repository& repo,
         const ores::database::context& ctx,
         const messaging::day_count_fraction_type_key& key) {
 ...
 */
messaging::day_count_fraction_type_key key_from(const domain::day_count_fraction_type& v) {
    messaging::day_count_fraction_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::day_count_fraction_type to_domain(const messaging::day_count_fraction_type_write& write) {
    domain::day_count_fraction_type v;
    v.code = write.code;
    v.name = write.name;
...
messaging::get_day_count_fraction_type_response
day_count_fraction_type_service::get_day_count_fraction_type(
    const messaging::get_day_count_fraction_type_request& request) {
    messaging::get_day_count_fraction_type_response response;
    ...
    response.day_count_fraction_type = std::move(found.front());
    return response;
}

messaging::get_many_day_count_fraction_types_response
day_count_fraction_type_service::get_many_day_count_fraction_types(
    const messaging::get_many_day_count_fraction_types_request& request) {
    messaging::get_many_day_count_fraction_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::day_count_fraction_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.day_count_fraction_type = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_day_count_fraction_type_response
day_count_fraction_type_service::put_day_count_fraction_type(
    const messaging::put_day_count_fraction_type_request& request) {
    messaging::put_day_count_fraction_type_response response;
    domain::day_count_fraction_type value;
    ...
    if (!written.empty())
        response.day_count_fraction_type = std::move(written.front());
    return response;
}

messaging::put_many_day_count_fraction_types_response
day_count_fraction_type_service::put_many_day_count_fraction_types(
    const messaging::put_many_day_count_fraction_types_request& request) {
    messaging::put_many_day_count_fraction_types_response response;
    std::vector<domain::day_count_fraction_type> batch;
...
messaging::delete_day_count_fraction_type_response
day_count_fraction_type_service::delete_day_count_fraction_type(
    const messaging::delete_day_count_fraction_type_request& request) {
    messaging::delete_day_count_fraction_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::day_count_fraction_type_repository::remove_status::removed:
            break;
        case repository::day_count_fraction_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::day_count_fraction_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::day_count_fraction_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_day_count_fraction_types_response
day_count_fraction_type_service::delete_many_day_count_fraction_types(
    const messaging::delete_many_day_count_fraction_types_request& request) {
    messaging::delete_many_day_count_fraction_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_day_count_fraction_type_versions_response
day_count_fraction_type_service::list_day_count_fraction_type_versions(
    const messaging::list_day_count_fraction_type_versions_request& request) {
    messaging::list_day_count_fraction_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_day_count_fraction_type_version_response
day_count_fraction_type_service::get_day_count_fraction_type_version(
    const messaging::get_day_count_fraction_type_version_request& request) {
    messaging::get_day_count_fraction_type_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.day_count_fraction_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
ores::utility::domain::result day_count_fraction_type_service::prepare_change(
    const messaging::day_count_fraction_type_change& change,
    const ores::utility::domain::change_intent& intent,
    domain::day_count_fraction_type& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::day_count_fraction_type>
day_count_fraction_type_service::list_types(std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all day count fraction types";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t day_count_fraction_type_service::count_types() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total day count fraction types count";
...
std::vector<domain::day_count_fraction_type>
day_count_fraction_type_service::get_types(const std::vector<std::string>& codes) {
