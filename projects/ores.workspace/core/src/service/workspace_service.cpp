...
std::optional<domain::workspace> workspace_service::get_workspace(const std::string& id) {
...
std::string workspace_service::create_workspace(const domain::workspace& ws) {
    ...
    if (!ws.status_code.empty() && ws.status_code != "active" && ws.status_code != "archived") {
        throw std::invalid_argument("Invalid workspace status_code: '" + ws.status_code +
...
std::vector<std::string> workspace_service::resolve(const std::string& workspace_id) {
...
std::vector<domain::workspace> workspace_service::get_workspace_history(const std::string& id) {
