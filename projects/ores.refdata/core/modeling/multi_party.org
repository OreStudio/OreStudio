...
is shown when no party context is available; the removed desktop client also
showed a modal warning at login to alert the user to the misconfiguration. With strict RLS
...
struct (see =party_summary::party_category= below).
