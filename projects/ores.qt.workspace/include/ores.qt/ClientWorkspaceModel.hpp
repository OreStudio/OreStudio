...
/**
 * @brief Model for displaying workspaces fetched from the server.
 *
 * This model extends AbstractClientModel and fetches workspace
 * data asynchronously using the ores.comms client.
 */
class ClientWorkspaceModel final : public AbstractClientModel {
    Q_OBJECT
...
public:
     ...
     * @return The workspace, or nullptr if row is invalid.
     */
    const workspace::domain::workspace* getWorkspace(int row) const;
...
private:
    ...
    void fetch_workspaces(std::uint32_t offset, std::uint32_t limit);
