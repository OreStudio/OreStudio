...
void write_trade_type(std::vector<std::byte>& buffer,
    const domain::trade_type& tt) {
    writer::write_uint32(buffer, static_cast<std::uint32_t>(tt.version));
    writer::write_string(buffer, tt.code);
    writer::write_string(buffer, tt.description);
    writer::write_string(buffer, tt.modified_by);
    writer::write_string(buffer, tt.performed_by);
...
std::expected<domain::trade_type, error_code>
read_trade_type(std::span<const std::byte>& data) {
    domain::trade_type tt;
...
// ============================================================================
// Trade Type Messages Implementation
// ============================================================================

std::vector<std::byte> get_trade_types_request::serialize() const {
    return {};
}

std::expected<get_trade_types_request, error_code>
get_trade_types_request::deserialize(std::span<const std::byte> data) {
    if (!data.empty()) {
        return std::unexpected(error_code::payload_too_large);
    }
    return get_trade_types_request{};
}
...
std::vector<std::byte> get_trade_types_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& tt : types) {
        write_trade_type(buffer, tt);
    }
    return buffer;
}

std::expected<get_trade_types_response, error_code>
get_trade_types_response::deserialize(std::span<const std::byte> data) {
    get_trade_types_response response;
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        auto result = read_trade_type(data);
...
save_trade_type_request::from(domain::trade_type type) {
    return save_trade_type_request{std::vector<domain::trade_type>{std::move(type)}};
}

save_trade_type_request
save_trade_type_request::from(std::vector<domain::trade_type> types) {
    return save_trade_type_request{std::move(types)};
}

std::vector<std::byte> save_trade_type_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_uint32(buffer, static_cast<std::uint32_t>(types.size()));
    for (const auto& e : types)
        write_trade_type(buffer, e);
    return buffer;
}

std::expected<save_trade_type_request, error_code>
save_trade_type_request::deserialize(std::span<const std::byte> data) {
    auto count_result = reader::read_uint32(data);
    if (!count_result) return std::unexpected(count_result.error());
    save_trade_type_request request;
    request.types.reserve(*count_result);
    for (std::uint32_t i = 0; i < *count_result; ++i) {
        auto e = read_trade_type(data);
...
std::vector<std::byte> save_trade_type_response::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<save_trade_type_response, error_code>
save_trade_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> delete_trade_type_request::serialize() const {
    std::vector<std::byte> buffer;
...
std::expected<delete_trade_type_request, error_code>
delete_trade_type_request::deserialize(std::span<const std::byte> data) {
    delete_trade_type_request request;
...
std::vector<std::byte> delete_trade_type_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    return buffer;
...
std::expected<delete_trade_type_response, error_code>
delete_trade_type_response::deserialize(std::span<const std::byte> data) {
...
std::vector<std::byte> get_trade_type_history_request::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_string(buffer, code);
    return buffer;
}

std::expected<get_trade_type_history_request, error_code>
get_trade_type_history_request::deserialize(std::span<const std::byte> data) {
    get_trade_type_history_request request;
...
std::vector<std::byte> get_trade_type_history_response::serialize() const {
    std::vector<std::byte> buffer;
    writer::write_bool(buffer, success);
    writer::write_string(buffer, message);
    writer::write_uint32(buffer, static_cast<std::uint32_t>(versions.size()));
    for (const auto& v : versions) {
        write_trade_type(buffer, v);
    }
    return buffer;
}

std::expected<get_trade_type_history_response, error_code>
get_trade_type_history_response::deserialize(std::span<const std::byte> data) {
