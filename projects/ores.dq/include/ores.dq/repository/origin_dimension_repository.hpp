 ...
 *
 */
#ifndef ORES_DQ_REPOSITORY_ORIGIN_DIMENSION_REPOSITORY_HPP
#define ORES_DQ_REPOSITORY_ORIGIN_DIMENSION_REPOSITORY_HPP
...
/**
 * @brief Reads and writes origin_dimensions to data storage.
 */
class origin_dimension_repository {
private:
...
public:
    ...
    explicit origin_dimension_repository(context ctx);

    std::string sql();

    void write(const domain::origin_dimension& dimension);
    void write(const std::vector<domain::origin_dimension>& dimensions);
