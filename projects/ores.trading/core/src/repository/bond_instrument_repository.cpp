...
void bond_instrument_repository::write(context ctx, const std::vector<domain::bond_instrument>& v) {
    ...
    execute_write_query(
        ctx, bond_instrument_mapper::map(v), lg(), "Writing bond_instruments to database.");
}
...
std::vector<domain::bond_instrument> bond_instrument_repository::read_latest(context ctx) {
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        lg(),
        "Reading latest bond_instruments");
}
...
bond_instrument_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest bond_instruments with offset: " << offset
                               << " and limit: " << limit;
    const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<bond_instrument_entity>> |
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond_instruments with pagination");
}
...
std::uint32_t bond_instrument_repository::count_latest(context ctx) {
    ...
    const auto query = sqlgen::select_from<bond_instrument_entity>(sqlgen::count().as<"count">()) |
                       where("tenant_id"_c == tid && "valid_to"_c == max.value()) |
...
bond_instrument_repository::read_latest(context ctx, const std::string& id) {
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond_instrument by id.");
}

std::vector<domain::bond_instrument> bond_instrument_repository::read_all(context ctx,
                                                                          const std::string& id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all bond_instrument versions. id: " << id;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<bond_instrument_entity>> |
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading all bond_instrument versions by id.");
}
...
std::vector<domain::bond_instrument>
bond_instrument_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    return execute_read_query<bond_instrument_entity, domain::bond_instrument>(
        ...
        [](const auto& entities) { return bond_instrument_mapper::map(entities); },
        lg(),
        "Reading latest bond_instruments by ids.");
}
