...
ResultHistoryDialog::ResultHistoryDialog(
    const boost::uuids::uuid& id,
    const QString& code,
    ClientManager* clientManager,
    QWidget* parent)
    : QWidget(parent),
      ui_(new Ui::ResultHistoryDialog),
      id_(id),
      code_(code),
...
void ResultHistoryDialog::setupUi() {
...
void ResultHistoryDialog::setupToolbar() {
...
void ResultHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &ResultHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &ResultHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &ResultHistoryDialog::onRevertClicked);
    connect(ui_->closeButton, &QPushButton::clicked,
            this, [this]() { if (window()) window()->close(); });
}

void ResultHistoryDialog::loadHistory() {
    if (!clientManager_ || !clientManager_->isConnected()) {
        emit errorOccurred("Not connected to server");
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Loading history for compute result: " << code_.toStdString();
...
void ResultHistoryDialog::updateVersionList() {
...
void ResultHistoryDialog::onVersionSelected() {
...
void ResultHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.modified_by != previous.modified_by) {
        addChange("Modified By",
                  QString::fromStdString(previous.modified_by),
                  QString::fromStdString(current.modified_by));
    }

    if (current.output_uri != previous.output_uri) {
        addChange("Name",
                  QString::fromStdString(previous.output_uri),
                  QString::fromStdString(current.output_uri));
    }
...
void ResultHistoryDialog::updateFullDetails(int versionIndex) {
    if (versionIndex < 0 ||
    ...
    ui_->codeValue->setText(QString::fromStdString(version.modified_by));
    ui_->nameValue->setText(QString::fromStdString(version.output_uri));
    ui_->descriptionValue->setText(QString::fromStdString(version.modified_by));
...
void ResultHistoryDialog::updateActionStates() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void ResultHistoryDialog::onOpenVersionClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
...
void ResultHistoryDialog::onRevertClicked() {
    auto selected = ui_->versionListWidget->selectedItems();
    if (selected.isEmpty()) return;
