...
/**
 * @brief Request to save one or more contact types (create or update).
 */
struct save_contact_type_request final {
    std::vector<domain::contact_type> types;

    static save_contact_type_request from(domain::contact_type type);
    static save_contact_type_request from(std::vector<domain::contact_type> types);
...
/**
 * @brief Response confirming contact type save operation(s).
 */
