...
| File pattern         | Date formats           | Key semantics               |
|----------------------+------------------------+-----------------------------|
| =market*.txt=        | =YYYYMMDD= or =YYYY-MM-DD= | Instrument quote key    |
| =fixings*.txt=       | =YYYYMMDD= or =YYYY-MM-DD= | Index name              |

Both file types appear in both date formats in the ORE example library.
The parser auto-detects the date format per line.
...
*52 unique market data text files* (many example directories share identical
file content).  All 52 must parse without error.  The roundtrip test suite
