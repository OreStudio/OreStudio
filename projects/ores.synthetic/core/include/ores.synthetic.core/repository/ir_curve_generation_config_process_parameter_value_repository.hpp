...
public:
    ...
    /**
     * @brief Writes a IR curve generation config process parameter value, honouring the claim it
     * states.
     *
    ...
    /**
     * @brief Writes a set of IR curve generation config process parameter values, each honouring
     * its own claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::ir_curve_generation_config_process_parameter_value>& v,
    ...
    /**
     * @brief Removes a IR curve generation config process parameter value, refusing a row that
     * moved on.
     *
...
private:
    ...
    replace_claim(context ctx, const domain::ir_curve_generation_config_process_parameter_value& v);
