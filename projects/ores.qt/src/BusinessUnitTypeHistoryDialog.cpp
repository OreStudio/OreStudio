...
void BusinessUnitTypeHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &BusinessUnitTypeHistoryDialog::onVersionSelected);
    connect(openVersionAction_, &QAction::triggered,
            this, &BusinessUnitTypeHistoryDialog::onOpenVersionClicked);
    connect(revertAction_, &QAction::triggered,
            this, &BusinessUnitTypeHistoryDialog::onRevertClicked);
...
void BusinessUnitTypeHistoryDialog::loadHistory() {
    ...
    auto task = [self, id = id_]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_business_unit_type_history_request,
            0, std::move(payload)
        );
        ...
        auto response = refdata::messaging::get_business_unit_type_history_response::
...
void BusinessUnitTypeHistoryDialog::updateChangesTable(int currentVersionIndex) {
    ...
    if (current.level != previous.level) {
        addChange("Level",
                  QString::number(previous.level),
                  QString::number(current.level));
    }
