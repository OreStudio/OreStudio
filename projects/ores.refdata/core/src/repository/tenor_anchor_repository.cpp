...
tenor_anchor_repository::replace_claim(context ctx, const domain::tenor_anchor& v) {
...
domain::tenor_anchor tenor_anchor_repository::apply_claim(
    context ctx, const domain::tenor_anchor& v, const ores::utility::domain::precondition& claim) {
...
void tenor_anchor_repository::write(context ctx, const std::vector<domain::tenor_anchor>& v) {
...
void tenor_anchor_repository::write(context ctx,
                                    const domain::tenor_anchor& v,
                                    const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing tenor anchor. " << "code: " << v.code;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, tenor_anchor_mapper::map(t), lg(), "Writing tenor anchor to database.");
}

void tenor_anchor_repository::write(
    context ctx,
    const std::vector<domain::tenor_anchor>& v,
    const std::vector<ores::utility::domain::precondition>& claims) {
    BOOST_LOG_SEV(lg(), debug) << "Writing tenor anchors. Count: " << v.size();
    std::vector<domain::tenor_anchor> batch;
    batch.reserve(v.size());
    for (std::size_t i = 0; i < v.size(); ++i)
        batch.push_back(apply_claim(ctx, v[i], claims[i]));
    execute_write_query(
        ctx, tenor_anchor_mapper::map(batch), lg(), "Writing tenor anchors to database.");
}
...
std::vector<domain::tenor_anchor> tenor_anchor_repository::read_latest(context ctx,
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::read<std::vector<tenor_anchor_entity>> |
...
std::vector<domain::tenor_anchor> tenor_anchor_repository::read_all(context ctx,
    ...
    return execute_read_query<tenor_anchor_entity, domain::tenor_anchor>(
        ...
        [](const auto& entities) { return tenor_anchor_mapper::map(entities); },
        lg(),
        "Reading all tenor anchor versions by code.");
}
...
std::optional<domain::tenor_anchor> tenor_anchor_repository::read_at_version(
    ...
    const auto entities = execute_read_query<tenor_anchor_entity, domain::tenor_anchor>(
        ctx,
        query,
        [](const auto& entities) { return tenor_anchor_mapper::map(entities); },
        lg(),
        "Reading tenor anchor at version.");
...
tenor_anchor_repository::remove_status tenor_anchor_repository::remove(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<tenor_anchor_entity> |
                       where("tenant_id"_c == tid && "code"_c == code &&
                             "valid_to"_c == max.value() && "version"_c == expected);

    execute_delete_query(ctx, query, lg(), "Removing tenor anchor from database.");
...
std::uint32_t tenor_anchor_repository::get_total_anchor_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active tenor anchor count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::select_from<tenor_anchor_entity>(sqlgen::count().as<"count">()) |
                       where("tenant_id"_c == tid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active tenor anchor count: " << count;
    return count;
}

std::vector<domain::tenor_anchor>
tenor_anchor_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<tenor_anchor_entity, domain::tenor_anchor>(
        ctx,
        query,
        [](const auto& entities) { return tenor_anchor_mapper::map(entities); },
        lg(),
        "Reading latest tenor anchors by ids.");
    return result;
}

void tenor_anchor_repository::remove(context ctx, const std::vector<std::string>& codes) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::delete_from<tenor_anchor_entity> |
        where("tenant_id"_c == tid && "code"_c.in(codes) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing tenor anchors.");
}
