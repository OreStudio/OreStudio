 ...
 */
#ifndef ORES_QT_PARTY_STATUS_MDI_WINDOW_HPP
#define ORES_QT_PARTY_STATUS_MDI_WINDOW_HPP
 ...
 *
 * Provides a table view of party statuses with toolbar actions
 * for reload, add, edit, delete, and viewing history.
 */
class PartyStatusMdiWindow final : public EntityListMdiWindow {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.party_status_mdi_window";
...
public:
    explicit PartyStatusMdiWindow(ClientManager* clientManager,
...
signals:
    ...
    void showStatusDetails(const refdata::domain::party_status& status);
    void addNewRequested();
    void statusDeleted(const QString& code);
    void showStatusHistory(const refdata::domain::party_status& status);
...
protected:
    QString normalRefreshTooltip() const override {
        return tr("Refresh party statuses");
    }
