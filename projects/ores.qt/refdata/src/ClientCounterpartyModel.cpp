...
ClientCounterpartyModel::ClientCounterpartyModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(pulseManager_,
            ...
            this,
            &ClientCounterpartyModel::onPulseStateChanged);
    connect(pulseManager_,
...
QVariant ClientCounterpartyModel::data(const QModelIndex& index, int role) const {
    if (!index.isValid())
        return {};
    ...
    if (role == Qt::DecorationRole && index.column() == iconColumn()) {
        return flagDecoration(counterparty.image_id);
    }

    if (role == Qt::DecorationRole && imageCache_) {
        if (index.column() == Column::BusinessCenterCode)
            return business_centre_flag_icon(*imageCache_, counterparty.business_center_code);
    }
...
void ClientCounterpartyModel::load_page(std::uint32_t offset, std::uint32_t limit) {
...
void ClientCounterpartyModel::fetch_counterparties(std::uint32_t offset, std::uint32_t limit) {
    is_fetching_ = true;
    ...
    QFuture<FetchResult> future = QtConcurrent::run([self, offset, limit]() -> FetchResult {
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                BOOST_LOG_SEV(lg(), debug) << "Making counterparties request with offset=" << offset
                                           << ", limit=" << limit;
...
void ClientCounterpartyModel::onCounterpartiesLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch counterparties: " << result.error_message.toStdString();
