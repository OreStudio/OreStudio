...
instrument_option_exercise_fee_service::instrument_option_exercise_fee_service(context ctx)
    : ctx_(std::move(ctx)) {}

std::vector<domain::instrument_option_exercise_fee>
instrument_option_exercise_fee_service::list_exercise_fees(std::uint32_t offset,
                                                           std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all instrument option exercise fees";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t instrument_option_exercise_fee_service::count_exercise_fees() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total instrument option exercise fees count";
    return repo_.get_total_exercise_fee_count(ctx_);
}
