...
| Now           | Compression and batching done; two deferred follow-ups remain. |
| Waiting on    | Nothing.                               |
| Next          | Pick up a follow-up task if/when prioritised. |
...
| [[id:77F77107-23CE-46B4-B51E-F7DBAC742C3D][Send raw bytes instead of base64 for image-carrying NATS messages]] | STARTED | 2026-07-28 | | Follow-up to the compression task: stop routing image byte payloads through rfl::json's default base64 encoding. Not load-bearing (the generic transparent gzip mechanism already gives a 2.5x margin against the worst measured case per the decision task), but removes a pointless encode/decode round-trip and the remaining ~33% base64 overhead. Needs a custom wire encoding for image-carrying messages specifically, bypassing rfl::json for that field. |
