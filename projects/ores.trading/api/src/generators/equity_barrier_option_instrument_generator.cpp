...
generate_synthetic_equity_barrier_option_instrument(utility::generation::generation_context& ctx) {
    ...
    r.identity.trade_type_code = std::string("EquityBarrierOption");
...
generate_synthetic_equity_barrier_option_instruments(std::size_t n,
    ...
    std::vector<domain::equity_barrier_option_instrument> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_equity_barrier_option_instrument(ctx));
    return r;
}
