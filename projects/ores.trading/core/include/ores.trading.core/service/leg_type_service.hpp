 ...
 *
 */
#ifndef ORES_TRADING_SERVICE_LEG_TYPE_SERVICE_HPP
#define ORES_TRADING_SERVICE_LEG_TYPE_SERVICE_HPP
...
private:
    inline static std::string_view logger_name = "ores.trading.service.leg_type_service";
...
public:
    ...
    std::vector<domain::leg_type> list_types();

    std::optional<domain::leg_type> find_type(const std::string& code);

    void save_type(const domain::leg_type& v);

    void save_types(const std::vector<domain::leg_type>& v);

    void remove_type(const std::string& code);

    /**
     * @brief Removes multiple leg types.
     */
