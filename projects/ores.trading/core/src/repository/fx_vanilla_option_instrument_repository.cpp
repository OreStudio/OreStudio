...
void fx_vanilla_option_instrument_repository::write(context ctx,
    ...
    execute_write_query(ctx,
                        ...
                        lg(),
                        "Writing FX vanilla option instrument to database.");
}
...
void fx_vanilla_option_instrument_repository::write(
    ...
    execute_write_query(ctx,
                        fx_vanilla_option_instrument_mapper::map(v),
                        lg(),
                        "Writing FX vanilla option instruments to database.");
}
...
fx_vanilla_option_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<fx_vanilla_option_instrument_entity,
                                  domain::fx_vanilla_option_instrument>(
            ctx,
            query,
            [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
            lg(),
            "Reading latest FX vanilla option instruments (workspace resolution chain).");
    }
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::read<std::vector<fx_vanilla_option_instrument_entity>> |
    ...
    return execute_read_query<fx_vanilla_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX vanilla option instruments");
}

std::vector<domain::fx_vanilla_option_instrument>
fx_vanilla_option_instrument_repository::read_latest(context ctx,
                                                     const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FX vanilla option instrument. "
    ...
    return execute_read_query<fx_vanilla_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX vanilla option instrument by instrument_id.");
}


std::vector<domain::fx_vanilla_option_instrument>
fx_vanilla_option_instrument_repository::read_all(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all FX vanilla option instrument versions. "
                               << "instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<fx_vanilla_option_instrument_entity>> |
    ...
    return execute_read_query<fx_vanilla_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading all FX vanilla option instrument versions by instrument_id.");
}

std::optional<domain::fx_vanilla_option_instrument>
fx_vanilla_option_instrument_repository::read_at_version(context ctx,
                                                         const std::string& instrument_id,
                                                         std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading FX vanilla option instrument at version. "
    ...
    const auto entities = execute_read_query<fx_vanilla_option_instrument_entity,
                                             domain::fx_vanilla_option_instrument>(
        ctx,
        query,
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading FX vanilla option instrument at version.");
...
void fx_vanilla_option_instrument_repository::remove(context ctx,
                                                     const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Removing FX vanilla option instrument. "
                               << "instrument_id: " << instrument_id;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
...
fx_vanilla_option_instrument_repository::read_latest(context ctx,
                                                     std::uint32_t offset,
                                                     std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FX vanilla option instruments with offset: "
    ...
    return execute_read_query<fx_vanilla_option_instrument_entity,
        ...
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX vanilla option instruments with pagination.");
}

std::uint32_t
fx_vanilla_option_instrument_repository::get_total_fx_vanilla_option_instrument_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active FX vanilla option instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<fx_vanilla_option_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active FX vanilla option instrument count: " << count;
    return count;
}

std::vector<domain::fx_vanilla_option_instrument>
fx_vanilla_option_instrument_repository::read_latest(
    ...
    auto result = execute_read_query<fx_vanilla_option_instrument_entity,
                                     domain::fx_vanilla_option_instrument>(
        ctx,
        query,
        [](const auto& entities) { return fx_vanilla_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX vanilla option instruments by ids.");
    return result;
}
...
void fx_vanilla_option_instrument_repository::remove(
    ...
    const auto query = sqlgen::delete_from<fx_vanilla_option_instrument_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid &&
                             "instrument_id"_c.in(instrument_ids) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing FX vanilla option instruments.");
}
