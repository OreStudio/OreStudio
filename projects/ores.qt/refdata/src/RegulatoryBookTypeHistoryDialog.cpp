...
#include "ui_RegulatoryBookTypeHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

RegulatoryBookTypeHistoryDialog::RegulatoryBookTypeHistoryDialog(const QString& code,
                                                                 ClientManager* clientManager,
                                                                 QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::RegulatoryBookTypeHistoryDialog)
    , code_(code)
...
RegulatoryBookTypeHistoryDialog::~RegulatoryBookTypeHistoryDialog() {
    delete ui_;
}

QString RegulatoryBookTypeHistoryDialog::code() const {
    return code_;
}

void RegulatoryBookTypeHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for regulatory book type: "
                               << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_regulatory_book_type_history_request request;
    request.code = code_.toStdString();

    QPointer<RegulatoryBookTypeHistoryDialog> self = this;
    runHistoryRequest(
        clientManager_,
        std::move(request),
        [self](refdata::messaging::get_regulatory_book_type_history_response response) {
            if (!self)
                return;
            if (!response.success) {
...
int RegulatoryBookTypeHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString RegulatoryBookTypeHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow RegulatoryBookTypeHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult RegulatoryBookTypeHistoryDialog::calculateDiffAt(int ci,
                                                                               int pi) const {
    DiffResult diffs;
...
void RegulatoryBookTypeHistoryDialog::displayFullDetails(int index) {
...
void RegulatoryBookTypeHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void RegulatoryBookTypeHistoryDialog::revertToVersionAt(int index) {
