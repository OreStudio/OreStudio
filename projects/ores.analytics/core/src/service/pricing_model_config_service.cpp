...
read_one(repository::pricing_model_config_repository& repo,
         const ores::database::context& ctx,
         const messaging::pricing_model_config_key& key) {
 ...
 */
messaging::pricing_model_config_key key_from(const domain::pricing_model_config& v) {
    messaging::pricing_model_config_key key;
    key.name = v.name;
    return key;
}
...
pricing_model_config_service::list_pricing_model_configs(
    const messaging::list_pricing_model_configs_request& request) {
    messaging::list_pricing_model_configs_response response;
...
messaging::get_pricing_model_config_response pricing_model_config_service::get_pricing_model_config(
    const messaging::get_pricing_model_config_request& request) {
    messaging::get_pricing_model_config_response response;
    ...
    response.pricing_model_config = std::move(found.front());
    return response;
}

messaging::get_many_pricing_model_configs_response
pricing_model_config_service::get_many_pricing_model_configs(
    const messaging::get_many_pricing_model_configs_request& request) {
    messaging::get_many_pricing_model_configs_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::pricing_model_config_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.pricing_model_config = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_pricing_model_config_response pricing_model_config_service::put_pricing_model_config(
    const messaging::put_pricing_model_config_request& request) {
    messaging::put_pricing_model_config_response response;
    domain::pricing_model_config value;
    ...
    if (!written.empty())
        response.pricing_model_config = std::move(written.front());
    return response;
}

messaging::put_many_pricing_model_configs_response
pricing_model_config_service::put_many_pricing_model_configs(
    const messaging::put_many_pricing_model_configs_request& request) {
    messaging::put_many_pricing_model_configs_response response;
    std::vector<domain::pricing_model_config> batch;
    ...
    response.configs.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.configs.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_pricing_model_config_response
pricing_model_config_service::delete_pricing_model_config(
    const messaging::delete_pricing_model_config_request& request) {
    messaging::delete_pricing_model_config_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(row.id), expected)) {
        case repository::pricing_model_config_repository::remove_status::removed:
            break;
        case repository::pricing_model_config_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::pricing_model_config_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::pricing_model_config_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_pricing_model_configs_response
pricing_model_config_service::delete_many_pricing_model_configs(
    const messaging::delete_many_pricing_model_configs_request& request) {
    messaging::delete_many_pricing_model_configs_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
    ...
    // failure.
    std::vector<domain::pricing_model_config> resolved;
...
messaging::list_pricing_model_config_versions_response
pricing_model_config_service::list_pricing_model_config_versions(
    const messaging::list_pricing_model_config_versions_request& request) {
    messaging::list_pricing_model_config_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_pricing_model_config_version_response
pricing_model_config_service::get_pricing_model_config_version(
    const messaging::get_pricing_model_config_version_request& request) {
    messaging::get_pricing_model_config_version_response response;
    ...
    const auto named = read_one(repo_, ctx_, request.key.pricing_model_config);
    if (named.empty()) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
pricing_model_config_service::prepare_change(const messaging::pricing_model_config_change& change,
...
pricing_model_config_service::get_config_at_version(const boost::uuids::uuid& id,
...
std::optional<domain::pricing_model_config>
pricing_model_config_service::get_config(const boost::uuids::uuid& id) {
...
std::optional<domain::pricing_model_config>
pricing_model_config_service::get_config_by_name(const std::string& name) {
    BOOST_LOG_SEV(lg(), debug) << "Getting pricing model configuration by name: " << name;
    messaging::pricing_model_config_key k;
    k.name = name;
    auto found = read_one(repo_, ctx_, k);
    if (found.empty())
        return std::nullopt;
...
std::vector<domain::pricing_model_config>
pricing_model_config_service::get_configs(const std::vector<std::string>& ids) {
...
void pricing_model_config_service::delete_config(const boost::uuids::uuid& id) {
...
std::vector<domain::pricing_model_config>
pricing_model_config_service::get_config_history(const std::string& key) {
    BOOST_LOG_SEV(lg(), debug) << "Getting history for pricing model configuration. key: " << key;
