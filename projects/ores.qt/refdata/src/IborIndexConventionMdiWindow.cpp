...
IborIndexConventionMdiWindow::IborIndexConventionMdiWindow(ClientManager* clientManager,
...
void IborIndexConventionMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &IborIndexConventionMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &IborIndexConventionMdiWindow::editSelected);
    ...
    connect(
        deleteAction_, &QAction::triggered, this, &IborIndexConventionMdiWindow::deleteSelected);
...
void IborIndexConventionMdiWindow::setupConnections() {
    connect(model_,
            &ClientIborIndexConventionModel::dataLoaded,
            this,
            &IborIndexConventionMdiWindow::onDataLoaded);
    connect(model_,
            &ClientIborIndexConventionModel::loadError,
            this,
            &IborIndexConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &IborIndexConventionMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &IborIndexConventionMdiWindow::onDoubleClicked);
...
void IborIndexConventionMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* ic = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showConventionHistory for code: " << ic->id;
...
void IborIndexConventionMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete IBOR index convention while disconnected.");
        return;
    }
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 IBOR index conventions?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete IBOR Index Convention", confirmMessage, QMessageBox::Yes | QMessageBox::No);
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "IBOR Index Convention deletion failed: " << code << " - " << result.second;
                failure_count++;
