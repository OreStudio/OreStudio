...
int ClientBadgeDefinitionModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientBadgeDefinitionModel::data(
    ...
    if (role == Qt::ForegroundRole) {
        return recency_foreground_color(definition.code);
    }
...
void ClientBadgeDefinitionModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh badge definition model: disconnected.";
                ...
                if (!result) {
                    ...
                    return {.success = false, .definitions = {},
                            ...
                            .error_message = QString::fromStdString(
                                "Failed to fetch badge definitions: " + comms::net::to_string(result.error())),
                            .error_details = {}};
                }
...
void ClientBadgeDefinitionModel::onDefinitionsLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " badge definitions newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " badge definitions."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientBadgeDefinitionModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientBadgeDefinitionModel::onPulsingComplete() {
