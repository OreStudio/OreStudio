...
#include "ores.iam.api/eventing/account_contact_information_event.hpp"
...
TEST_CASE("write_account_contact_information_publishes_an_event", tags) {
    ...
    using event_type = ores::iam::messaging::account_contact_information_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_iam_account_contact_informations");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.email == v.email)
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::iam::service::account_contact_information_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_account_contact_information_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_account_contact_information(id_str);
        ...
        REQUIRE_FALSE(svc.get_account_contact_information(id_str).has_value());
        REQUIRE(svc.get_account_contact_information_history(id_str).size() == versions.size());
    }
}
