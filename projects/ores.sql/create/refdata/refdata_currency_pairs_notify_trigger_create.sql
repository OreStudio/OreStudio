...
create or replace function ores_refdata_currency_pairs_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity', 'ores.refdata.currency_pair',
    ...
    perform pg_notify('ores_refdata_currency_pairs', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_refdata_currency_pairs_notify_trg
after insert or update or delete on ores_refdata_currency_pairs_tbl
for each row execute function ores_refdata_currency_pairs_notify_fn();
