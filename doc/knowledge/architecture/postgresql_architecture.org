...
| =ores.sql/recreate_database.sh= | Full database teardown and rebuild |
...
DELETE and publishes a JSON payload on a per-domain PostgreSQL channel
(e.g. =ores_refdata_notifications=).  The service layer listens on its
channel and translates database events into NATS messages, which are
then routed to connected clients.  This is how live-update eventing
works: DB write → NOTIFY trigger → service listener → NATS event → Qt
=markAsStale()=.
