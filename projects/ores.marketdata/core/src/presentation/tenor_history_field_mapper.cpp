 ...
 *
 */
#include "ores.marketdata.core/presentation/tenor_history_field_mapper.hpp"
#include "ores.platform/time/datetime.hpp"

namespace ores::marketdata::presentation {

std::vector<ores::diff::domain::field_value> render_tenor_fields(const domain::tenor& v) {
    ...
    fields.push_back({.name = "Sort Order", .value = std::to_string(v.sort_order)});
    fields.push_back({.name = "Kind", .value = v.kind});
    fields.push_back({.name = "Unit", .value = v.unit});
    fields.push_back({.name = "Multiplier",
                      .value = v.multiplier ? std::to_string(*v.multiplier) : std::string{}});
