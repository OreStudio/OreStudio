...
#include "ores.refdata.api/eventing/country_event.hpp"
...
TEST_CASE("write_country_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::country_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_countries");
    ...
    for (int attempt = 1; attempt <= max_attempts && received.empty(); ++attempt) {
        ...
        for (int i = 0; i < polls_per_attempt && received.empty(); ++i) {
            ...
            for (const auto& msg : snap) {
                ...
                if (decoded && decoded->key.alpha2_code == v.alpha2_code)
