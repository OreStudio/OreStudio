...
create or replace function ores_trading_instrument_schedules_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    entity_name text := 'ores.trading.instrument_schedule';
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity_ids', jsonb_build_array(changed_instrument_id, changed_owner_role, changed_owner_number, changed_schedule_role, changed_sequence_number),
        'tenant_id', changed_tenant_id
    );

    perform pg_notify('ores_trading_instrument_schedules', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_trading_instrument_schedules_notify_trg
after insert or update or delete on ores_trading_instrument_schedules_tbl
for each row execute function ores_trading_instrument_schedules_notify_fn();
