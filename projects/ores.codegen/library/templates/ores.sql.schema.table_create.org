:PROPERTIES:
:ID: 1025E09D-7E2D-4366-AC51-8651C95B3ADC
:END:
#+title: ores.sql.schema.table_create
#+description: The earlier bi-temporal table create, sharing its output path and most of its body with =sql_schema_create= — the duplication the unified template exists to retire. =sql= profile.
#+type: archetype
#+model_types: schema
...
=library/templates/sql_schema_table_create.mustache=.
