 ...
 */
domain::tenant to_domain(const messaging::tenant_write& write) {
    domain::tenant v;
    v.id = write.id;
    v.code = write.code;
    v.name = write.name;
...
tenant_service::list_tenants(const messaging::list_tenants_request& request) {
    messaging::list_tenants_response response;
    ...
    response.tenants = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_tenant_count(ctx_);
    return response;
}

messaging::get_tenant_response
tenant_service::get_tenant(const messaging::get_tenant_request& request) {
    messaging::get_tenant_response response;
    ...
    response.tenant = std::move(found.front());
    return response;
}

messaging::get_many_tenants_response
tenant_service::get_many_tenants(const messaging::get_many_tenants_request& request) {
    messaging::get_many_tenants_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenant_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenant = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_tenant_response
tenant_service::put_tenant(const messaging::put_tenant_request& request) {
    messaging::put_tenant_response response;
    domain::tenant value;
    ...
    if (!written.empty())
        response.tenant = std::move(written.front());
    return response;
}

messaging::put_many_tenants_response
tenant_service::put_many_tenants(const messaging::put_many_tenants_request& request) {
    messaging::put_many_tenants_response response;
    std::vector<domain::tenant> batch;
    ...
    response.tenants.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.tenants.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenant_response
tenant_service::delete_tenant(const messaging::delete_tenant_request& request) {
    messaging::delete_tenant_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(row.id), expected)) {
        ...
        case repository::tenant_repository::remove_status::missing:
            ...
            response.result.code = "not_found";
            break;
        case repository::tenant_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenant_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
tenant_service::delete_many_tenants(const messaging::delete_many_tenants_request& request) {
    messaging::delete_many_tenants_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
tenant_service::list_tenant_versions(const messaging::list_tenant_versions_request& request) {
    messaging::list_tenant_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
tenant_service::prepare_change(const messaging::tenant_change& change,
