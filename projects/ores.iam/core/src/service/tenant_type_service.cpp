 ...
 */
std::vector<domain::tenant_type> read_one(repository::tenant_type_repository& repo,
                                          const ores::database::context& ctx,
                                          const messaging::tenant_type_key& key) {
 ...
 */
messaging::tenant_type_key key_from(const domain::tenant_type& v) {
 ...
 */
domain::tenant_type to_domain(const messaging::tenant_type_write& write) {
    domain::tenant_type v;
    v.type = write.type;
    v.name = write.name;
...
tenant_type_service::list_tenant_types(const messaging::list_tenant_types_request& request) {
    messaging::list_tenant_types_response response;
...
tenant_type_service::get_tenant_type(const messaging::get_tenant_type_request& request) {
    messaging::get_tenant_type_response response;
    ...
    response.tenant_type = std::move(found.front());
    return response;
}

messaging::get_many_tenant_types_response tenant_type_service::get_many_tenant_types(
    const messaging::get_many_tenant_types_request& request) {
    messaging::get_many_tenant_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenant_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenant_type = std::move(found.front());
...
tenant_type_service::put_tenant_type(const messaging::put_tenant_type_request& request) {
    messaging::put_tenant_type_response response;
    domain::tenant_type value;
    ...
    if (!written.empty())
        response.tenant_type = std::move(written.front());
    return response;
}

messaging::put_many_tenant_types_response tenant_type_service::put_many_tenant_types(
    const messaging::put_many_tenant_types_request& request) {
    messaging::put_many_tenant_types_response response;
    std::vector<domain::tenant_type> batch;
...
tenant_type_service::delete_tenant_type(const messaging::delete_tenant_type_request& request) {
    messaging::delete_tenant_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.type, expected)) {
        case repository::tenant_type_repository::remove_status::removed:
            break;
        case repository::tenant_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenant_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenant_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_tenant_types_response tenant_type_service::delete_many_tenant_types(
    const messaging::delete_many_tenant_types_request& request) {
    messaging::delete_many_tenant_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_tenant_type_versions_response tenant_type_service::list_tenant_type_versions(
    const messaging::list_tenant_type_versions_request& request) {
    messaging::list_tenant_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_tenant_type_version_response tenant_type_service::get_tenant_type_version(
    const messaging::get_tenant_type_version_request& request) {
    messaging::get_tenant_type_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.tenant_type.type, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tenant_type_service::prepare_change(const messaging::tenant_type_change& change,
...
tenant_type_service::get_types(const std::vector<std::string>& types) {
