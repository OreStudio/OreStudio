 ...
 */
std::vector<domain::party_status> read_one(repository::party_status_repository& repo,
                                           const ores::database::context& ctx,
                                           const messaging::party_status_key& key) {
 ...
 */
messaging::party_status_key key_from(const domain::party_status& v) {
    messaging::party_status_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::party_status to_domain(const messaging::party_status_write& write) {
    domain::party_status v;
    v.code = write.code;
    v.name = write.name;
...
party_status_service::list_party_statuses(const messaging::list_party_statuses_request& request) {
    messaging::list_party_statuses_response response;
...
messaging::get_party_status_response
party_status_service::get_party_status(const messaging::get_party_status_request& request) {
    messaging::get_party_status_response response;
    ...
    response.party_status = std::move(found.front());
    return response;
}

messaging::get_many_party_statuses_response party_status_service::get_many_party_statuses(
    const messaging::get_many_party_statuses_request& request) {
    messaging::get_many_party_statuses_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_status_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party_status = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_party_status_response
party_status_service::put_party_status(const messaging::put_party_status_request& request) {
    messaging::put_party_status_response response;
    domain::party_status value;
    ...
    if (!written.empty())
        response.party_status = std::move(written.front());
    return response;
}

messaging::put_many_party_statuses_response party_status_service::put_many_party_statuses(
    const messaging::put_many_party_statuses_request& request) {
    messaging::put_many_party_statuses_response response;
    std::vector<domain::party_status> batch;
...
messaging::delete_party_status_response
party_status_service::delete_party_status(const messaging::delete_party_status_request& request) {
    messaging::delete_party_status_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::party_status_repository::remove_status::removed:
            break;
        case repository::party_status_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_status_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_status_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_party_statuses_response party_status_service::delete_many_party_statuses(
    const messaging::delete_many_party_statuses_request& request) {
    messaging::delete_many_party_statuses_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_party_status_versions_response party_status_service::list_party_status_versions(
    const messaging::list_party_status_versions_request& request) {
    messaging::list_party_status_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_party_status_version_response party_status_service::get_party_status_version(
    const messaging::get_party_status_version_request& request) {
    messaging::get_party_status_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.party_status.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
party_status_service::prepare_change(const messaging::party_status_change& change,
                                     const ores::utility::domain::change_intent& intent,
                                     domain::party_status& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::party_status> party_status_service::list_statuses(std::uint32_t offset,
                                                                      std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all party statuses";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t party_status_service::count_statuses() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total party statuses count";
...
party_status_service::get_statuses(const std::vector<std::string>& codes) {
