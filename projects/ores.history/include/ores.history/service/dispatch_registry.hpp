 ...
 * history, given its id. Generated, mechanical glue per entity — call
 * the entity's own repository/service, then its field mapper.
 */
using history_provider =
    std::function<std::vector<messaging::entity_history_version>(const std::string& entity_id)>;
