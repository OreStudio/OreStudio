...
using ores::shell::app::commands::party_role_type_commands;
...
TEST_CASE("party_role_type_commands_register_commands_registers_the_pagination_callback", tags) {
    ...
    BOOST_LOG_SEV(lg, debug) << "Looking up the pagination callback for party_role_types.";
    CHECK(pagination.get_list_callback("party_role_types") != nullptr);
}

TEST_CASE("party_role_type_commands_process_add_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    party_role_type_commands::process_add_party_role_type(out, session, add_tokens());
...
TEST_CASE("party_role_type_commands_process_add_reports_the_expected_argument_count", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    party_role_type_commands::process_add_party_role_type(out, session, {"sample"});
...
TEST_CASE("party_role_type_commands_process_add_sends_a_valid_token_vector_to_the_server", tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("party_role_type_commands_process_delete_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    party_role_type_commands::process_delete_party_role_type(out, session, "missing-key");
...
TEST_CASE("party_role_type_commands_process_history_requires_a_logged_in_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    party_role_type_commands::process_get_party_role_type_history(out, session, {"missing-key"});
...
TEST_CASE("party_role_type_commands_process_history_rejects_a_version_without_diff", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    party_role_type_commands::process_get_party_role_type_history(
