...
begin
    ...
    -- Validate parent_business_unit_id (nullable self-referencing FK)
    if NEW.parent_business_unit_id is not null then
        if not exists (
            select 1 from ores_refdata_business_units_tbl
            where tenant_id = NEW.tenant_id and id = NEW.parent_business_unit_id
              and valid_to = ores_utility_infinity_timestamp_fn()
        ) then
    ...
    -- Validate business_centre_code (nullable)
    if NEW.business_centre_code is not null and NEW.business_centre_code != '' then
        NEW.business_centre_code := ores_refdata_validate_business_centre_fn(
            NEW.tenant_id, NEW.business_centre_code);
    end if;
