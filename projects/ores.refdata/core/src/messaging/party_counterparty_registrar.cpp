...
std::vector<ores::nats::service::subscription> register_party_counterparty_handlers(
    ...
    subs.push_back(nats.queue_subscribe(get_party_counterparties_request::nats_subject,
                                        queue_group,
                                        [h](ores::nats::message msg) { h->list(std::move(msg)); }));
    subs.push_back(
        nats.queue_subscribe(get_party_counterparties_by_party_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->list_by_party(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(save_party_counterparty_request::nats_subject,
                                        queue_group,
    ...
    subs.push_back(
        nats.queue_subscribe(count_party_counterparties_by_party_request::nats_subject,
                             queue_group,
                             [h](ores::nats::message msg) { h->count_by_party(std::move(msg)); }));
    subs.push_back(nats.queue_subscribe(
        count_party_counterparties_by_counterparty_request::nats_subject,
        queue_group,
        [h](ores::nats::message msg) { h->count_by_counterparty(std::move(msg)); }));
    return subs;
}
