...
generate_synthetic_day_count_fraction_type(utility::generation::generation_context& ctx) {
    ...
    r.code = std::string(faker::word::noun()) + "_dcf" + "-" + std::to_string(idx);
    r.name = "Test Day Count Fraction " + std::to_string(faker::number::integer(1000, 9999));
...
generate_synthetic_day_count_fraction_types(std::size_t n,
    ...
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_day_count_fraction_type(ctx));
    return r;
}
