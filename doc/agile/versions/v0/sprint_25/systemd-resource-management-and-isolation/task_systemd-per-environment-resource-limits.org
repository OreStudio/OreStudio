...
| Now          | WS-7 remainder (memory-capped build slice) implemented; raising as the fifth PR. All unprivileged workstreams (WS-1, WS-4, WS-5 unpriv, WS-7) now complete; WS-2/WS-3 confirmed already done pre-existing. |
| Waiting on   | Someone with root, for WS-5's root half.  |
| Next         | WS-5 (root half, needs root) and the plan's explicit deliberate-OOM acceptance test; WS-6 stays deferred. |
