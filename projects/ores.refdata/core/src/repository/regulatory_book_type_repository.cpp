...
regulatory_book_type_repository::replace_claim(context ctx, const domain::regulatory_book_type& v) {
...
domain::regulatory_book_type
regulatory_book_type_repository::apply_claim(context ctx,
                                             const domain::regulatory_book_type& v,
...
void regulatory_book_type_repository::write(context ctx,
    ...
    execute_write_query(ctx,
                        regulatory_book_type_mapper::map(t),
                        lg(),
                        "Writing regulatory book type to database.");
}

void regulatory_book_type_repository::write(
    context ctx,
    const std::vector<domain::regulatory_book_type>& v,
    ...
    execute_write_query(ctx,
                        regulatory_book_type_mapper::map(batch),
                        lg(),
...
regulatory_book_type_repository::remove_status regulatory_book_type_repository::remove(
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::delete_from<regulatory_book_type_entity> |
                       where("tenant_id"_c == tid && "code"_c == code &&
...
std::vector<domain::regulatory_book_type>
regulatory_book_type_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<regulatory_book_type_entity, domain::regulatory_book_type>(
        ctx,
        query,
        [](const auto& entities) { return regulatory_book_type_mapper::map(entities); },
        lg(),
        "Reading latest regulatory book types by ids.");
    return result;
}
