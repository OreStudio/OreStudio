...
*** Live workspace ownership — system tenant / system party

Unlike every other workspace, the Live workspace row in =ores_workspaces_tbl=
belongs to the *system tenant* and *system party*.  This is a deliberate
exception: the Live workspace is a cross-cutting infrastructure concept, not a
business entity owned by any particular operational tenant or party.

Consequences:

- =list_active= for an operational tenant never returns the Live workspace row.
  The Qt client handles this by hardcoding the Live sentinel as the default
  context — it is never fetched from the server.
- =ores_workspace_validate_fn= (used by workspace-aware data table triggers to
  validate a =workspace_id= FK) short-circuits unconditionally for the Live
  sentinel UUID and never inspects the row.
- =ores_workspace_resolution_order_fn= (the recursive CTE that walks the parent
  chain) runs without a tenant filter; since UUIDs are globally unique, the CTE
  will find the Live row in the system tenant and correctly include it at the
  end of every resolution chain.

The Live workspace is seeded once at DB initialisation by
=workspace_live_populate.sql=, which resolves the system tenant ID via
=ores_utility_system_tenant_id_fn()= and the system party ID via
=ores_iam_account_parties_system_party_id_fn()=.
...
One further exception: the Live workspace row in this table belongs to the
system tenant and system party rather than to an operational party.  See
§[[*Live Workspace — Dedicated Sentinel UUID]] for details.
...
The function does not explicitly filter by =tenant_id= or =party_id=.  It does
not need to: workspace UUIDs are globally unique, so a chain rooted at a
workspace belonging to party A cannot accidentally traverse into a chain
belonging to party B.  Party isolation for the workspace entity itself is
enforced in the service layer (all =list_active=, =find_by_id=, and =read_all=
queries include a =tenant_id= WHERE clause).
