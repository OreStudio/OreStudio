...
#include "ores.marketdata.api/domain/crm_enabled_derived_pair_json_io.hpp" // IWYU pragma: keep.
#include "ores.marketdata.core/repository/crm_enabled_derived_pair_entity.hpp"
#include "ores.marketdata.core/repository/crm_enabled_derived_pair_mapper.hpp"
...
std::string crm_enabled_derived_pair_repository::sql() {
...
crm_enabled_derived_pair_repository::read_latest(context ctx) {
    ...
    return execute_read_query<crm_enabled_derived_pair_entity, domain::crm_enabled_derived_pair>(
        ...
        lg(),
        "Reading latest CRM enabled derived pairs");
}
...
crm_enabled_derived_pair_repository::read_all(context ctx, const std::string& id) {
    ...
    return execute_read_query<crm_enabled_derived_pair_entity, domain::crm_enabled_derived_pair>(
        ...
        [](const auto& entities) { return crm_enabled_derived_pair_mapper::map(entities); },
        lg(),
        "Reading all CRM enabled derived pair versions by id.");
}
...
crm_enabled_derived_pair_repository::read_at_version(context ctx,
    ...
    const auto entities =
        execute_read_query<crm_enabled_derived_pair_entity, domain::crm_enabled_derived_pair>(
            ...
            [](const auto& entities) { return crm_enabled_derived_pair_mapper::map(entities); },
            lg(),
            "Reading CRM enabled derived pair at version.");
...
std::vector<domain::crm_enabled_derived_pair> crm_enabled_derived_pair_repository::read_latest(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Reading latest CRM enabled derived pairs with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<crm_enabled_derived_pair_entity>> |
    ...
    return execute_read_query<crm_enabled_derived_pair_entity, domain::crm_enabled_derived_pair>(
        ...
        [](const auto& entities) { return crm_enabled_derived_pair_mapper::map(entities); },
        lg(),
        "Reading latest CRM enabled derived pairs with pagination.");
}

std::uint32_t
crm_enabled_derived_pair_repository::get_total_crm_enabled_derived_pair_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active CRM enabled derived pair count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto tid = ctx.tenant_id().to_string();
    const auto query =
        sqlgen::select_from<crm_enabled_derived_pair_entity>(sqlgen::count().as<"count">()) |
...
void crm_enabled_derived_pair_repository::remove(context ctx, const std::vector<std::string>& ids) {
    ...
    execute_delete_query(ctx, query, lg(), "Batch removing CRM enabled derived pairs.");
}
