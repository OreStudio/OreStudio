 ...
 * each test case starts and cleans it up when the test completes. It also
 * logs all Catch2 test events (test start/end, sections, assertions) to the
 * Boost.Log file.
 *
 * Usage:
...
public:
    ...
    void testCaseStarting(Catch::TestCaseInfo const& testInfo) override {
        ...
        BOOST_LOG_SEV(detail::current_test_context.logger.value(), severity_level::info)
        ...
        if (!testInfo.tags.empty()) {
            ...
            BOOST_LOG_SEV(detail::current_test_context.logger.value(), severity_level::debug)
                << tags.str();
        }
    }

    void sectionStarting(Catch::SectionInfo const& sectionInfo) override {
        if (!detail::current_test_context.logger.has_value()) return;
    ...
    void assertionEnded(Catch::AssertionStats const& assertionStats) override {
        ...
        if (result.isOk()) {
            BOOST_LOG_SEV(detail::current_test_context.logger.value(), severity_level::trace)
                << "Assertion passed: " << result.getExpression();
        } else {
            BOOST_LOG_SEV(detail::current_test_context.logger.value(), severity_level::error)
    ...
    void testCaseEnded(Catch::TestCaseStats const& testCaseStats) override {
        ...
        BOOST_LOG_SEV(detail::current_test_context.logger.value(), log_level)
