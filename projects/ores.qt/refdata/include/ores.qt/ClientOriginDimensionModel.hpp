 ...
 */
class ClientOriginDimensionModel final : public AbstractClientModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.client_origin_dimension_model";
...
public:
    ...
    /**
     * @brief Default window size for the origin dimension list window.
     */
    inline static const QSize kDefaultWindowSize = {800, 400};

    /**
     * @brief Settings group name for persisting window and column state.
     */
    static constexpr std::string_view kSettingsGroup = "OriginDimensionListWindow";
    /**
    ...
    explicit ClientOriginDimensionModel(ClientManager* clientManager, QObject* parent = nullptr);
...
private:
    ...
    using OriginDimensionKeyExtractor = std::string (*)(const dq::domain::origin_dimension&);
    RecencyTracker<dq::domain::origin_dimension, OriginDimensionKeyExtractor> recencyTracker_;
