 ...
 *
 */
#include "ores.mq.service/config/parser.hpp"

#include <ostream>
#include <boost/program_options.hpp>
#include <boost/throw_exception.hpp>
#include "ores.mq.service/config/parser_exception.hpp"
...
const std::string product_version("ores.mq.service v" ORES_VERSION);
...
using ores::mq::service::config::options;
using ores::mq::service::config::parser_exception;
...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.mq.service.log"));
    r.add(database_configuration::make_options_description());
    r.add(nats_configuration::make_options_description("ores.mq.service"));
    return r;
}

void print_help(const options_description& od, std::ostream& info) {
    info << "Message queue service" << std::endl << std::endl
         << "Usage: ores.mq.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
namespace ores::mq::service::config {
