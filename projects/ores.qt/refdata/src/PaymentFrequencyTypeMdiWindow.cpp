...
PaymentFrequencyTypeMdiWindow::PaymentFrequencyTypeMdiWindow(ClientManager* clientManager,
...
void PaymentFrequencyTypeMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &PaymentFrequencyTypeMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &PaymentFrequencyTypeMdiWindow::editSelected);
    ...
    connect(
        deleteAction_, &QAction::triggered, this, &PaymentFrequencyTypeMdiWindow::deleteSelected);
...
void PaymentFrequencyTypeMdiWindow::setupConnections() {
    connect(model_,
            &ClientPaymentFrequencyTypeModel::dataLoaded,
            this,
            &PaymentFrequencyTypeMdiWindow::onDataLoaded);
    connect(model_,
            &ClientPaymentFrequencyTypeModel::loadError,
            this,
            &PaymentFrequencyTypeMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &PaymentFrequencyTypeMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &PaymentFrequencyTypeMdiWindow::onDoubleClicked);
...
void PaymentFrequencyTypeMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete payment frequency type while disconnected.");
        return;
    }
    ...
    auto reply = MessageBoxHelper::question(
        this, "Delete Payment Frequency Type", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making batch delete request for " << codes.size() << " payment frequency types";
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Payment Frequency Type deletion failed: " << code << " - " << result.second;
                failure_count++;
