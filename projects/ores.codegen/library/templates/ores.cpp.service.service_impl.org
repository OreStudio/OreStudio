...
{{entity_singular}}_service::get_{{entity_singular_short}}_at_version(const std::string& {{primary_key.column}},
                                                                     std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Getting {{entity_singular_words}} at version: " << {{primary_key.column}}
                               << " version: " << version;
    return repo_.read_at_version(ctx_, {{primary_key.column}}, version);
}
{{/has_audit_columns}}
