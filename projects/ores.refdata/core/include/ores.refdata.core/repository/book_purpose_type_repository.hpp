 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_BOOK_PURPOSE_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_BOOK_PURPOSE_TYPE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes book purpose types to data storage.
 */
class ORES_REFDATA_CORE_EXPORT book_purpose_type_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.book_purpose_type_repository";
...
public:
    ...
    /**
     * @brief Writes book purpose types to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::book_purpose_type& v);
    void write(context ctx, const std::vector<domain::book_purpose_type>& v);
    /**@}*/

    /**
     * @brief Writes a book purpose type, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of book purpose types, each honouring its own
    ...
    /**
     * @brief Reads latest book purpose types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::book_purpose_type> read_latest(context ctx);
     ...
     */
    std::vector<domain::book_purpose_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single book purpose type as it stood at a specific
    ...
    /**
     * @brief Reads latest book purpose types with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Deletes a book purpose type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a book purpose type, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes book purpose types by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::book_purpose_type apply_claim(context ctx,
                                          const domain::book_purpose_type& v,
                                          const ores::utility::domain::precondition& claim);
};
