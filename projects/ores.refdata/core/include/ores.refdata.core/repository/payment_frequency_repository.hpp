 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_PAYMENT_FREQUENCY_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_PAYMENT_FREQUENCY_REPOSITORY_HPP
...
/**
 * @brief Reads and writes payment frequencies to data storage.
 */
class ORES_REFDATA_CORE_EXPORT payment_frequency_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.payment_frequency_repository";
...
public:
    ...
    /**
     * @brief Writes payment frequencies to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::payment_frequency& v);
    void write(context ctx, const std::vector<domain::payment_frequency>& v);
    /**@}*/

    /**
     * @brief Writes a payment frequency, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of payment frequencies, each honouring its own
    ...
    /**
     * @brief Reads latest payment frequencies, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::payment_frequency> read_latest(context ctx);
     ...
     */
    std::vector<domain::payment_frequency> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single payment frequency as it stood at a specific
    ...
    /**
     * @brief Reads latest payment frequencies with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_payment_frequency_count(context ctx);

    /**
     * @brief Deletes a payment frequency by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a payment frequency, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes payment frequencies by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::payment_frequency apply_claim(context ctx,
                                          const domain::payment_frequency& v,
                                          const ores::utility::domain::precondition& claim);
};
