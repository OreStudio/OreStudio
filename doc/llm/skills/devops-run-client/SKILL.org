...
3. /Always include a connect/login step in the scenario itself/ — every
   ...
   panel doesn't track pass/fail for), a step stating the exact account
   to log in as and its password (e.g. =tenant_admin@barclays_plc= /
   =Secure-Password-123=) and which party to select, plus how to get
   back to that state if the DB was recreated since (the provisioning
   script to re-run). Never leave the tester to guess credentials or
   starting state.
