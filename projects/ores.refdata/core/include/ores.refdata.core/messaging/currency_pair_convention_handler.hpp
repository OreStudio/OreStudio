        ...
        if (auto req = decode<assign_currency_pair_convention_calendar_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(currency_pair_convention_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(
                    nats_, msg, assign_currency_pair_convention_calendar_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(currency_pair_convention_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      assign_currency_pair_convention_calendar_response{.success = false,
                                                                        .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(currency_pair_convention_handler_lg(), warn)
        ...
        if (auto req = decode<revoke_currency_pair_convention_calendar_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(currency_pair_convention_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(
                    nats_, msg, revoke_currency_pair_convention_calendar_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(currency_pair_convention_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      revoke_currency_pair_convention_calendar_response{.success = false,
                                                                        .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(currency_pair_convention_handler_lg(), warn)
