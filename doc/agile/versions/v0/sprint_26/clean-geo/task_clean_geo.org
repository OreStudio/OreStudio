...
| B05 | pass | Two live captures name the component. =geo_location_tests_fail_for_some_ip_addresses.org= records the test defect this task confirms and closes. The other is this task's own story. No in-flight branch touches the component. |
...
| G05 | pass | The diagram capture is a no-op for this component, so it is not exposed to the generator defect. The one SQL defect found is not a template defect, and V02 records its fix and its regression test. |
...
| V05 | pass | Every gate run and green: component drift (=iam=), model drift (2 known exceptions), protocol twin coverage, handler permissions, populate references, shell recipe inventory, =regenerate_cmake_component_files.py --component ores.geo=, =validate_docs.sh= (apart from the pre-existing =ores.shell.iam= finding), =compass lint=, and the six checks of =.github/workflows/doc-lint.yml=. |
...
530, and the =ores.compass= suite 230 with 1 skipped. The database recreates
