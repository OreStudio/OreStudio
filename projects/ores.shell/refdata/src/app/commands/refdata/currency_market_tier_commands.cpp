...
void currency_market_tier_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("currency_market_tiers");
...
void currency_market_tier_commands::process_list(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_currency_market_tiers_request;
    ...
    auto result = do_auth_request<messaging::list_currency_market_tiers_response>(
...
void currency_market_tier_commands::process_get(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_currency_market_tier_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_currency_market_tier_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_get_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_currency_market_tiers_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_currency_market_tiers_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_add(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_currency_market_tier_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::put_currency_market_tier_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_set(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
...
void currency_market_tier_commands::process_put_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_currency_market_tiers_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::currency_market_tier_change change;
    ...
    auto result = do_auth_request<messaging::put_many_currency_market_tiers_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_delete(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_currency_market_tier_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_currency_market_tier_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_delete_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_currency_market_tiers_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::currency_market_tier_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_currency_market_tiers_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_versions(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_currency_market_tier_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_currency_market_tier_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void currency_market_tier_commands::process_version(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_currency_market_tier_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.currency_market_tier.code, parsed->positionals[next++], "code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_currency_market_tier_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
