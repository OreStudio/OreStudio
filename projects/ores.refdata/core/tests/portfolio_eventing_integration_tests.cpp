...
#include "ores.refdata.api/eventing/portfolio_event.hpp"
...
TEST_CASE("write_portfolio_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::portfolio_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_portfolios");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for portfolio " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_portfolio(id_str).has_value());
        REQUIRE(svc.get_portfolio_history(id_str).size() == versions.size());
    }
}
