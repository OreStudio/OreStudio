...
void TenorDetailDialog::setTenor(const marketdata::domain::tenor& tenor) {
    tenor_ = tenor;
    updateUiFromTenor();
}
...
void TenorDetailDialog::onSaveClicked() {
    ...
    auto task = [self, tenor = tenor_]() -> SaveResult {
        ...
        marketdata::messaging::save_tenor_request request;
        request.data = tenor;
...
void TenorDetailDialog::onDeleteClicked() {
    ...
    auto task = [self, code = tenor_.code]() -> DeleteResult {
        ...
        marketdata::messaging::delete_tenor_request request;
        request.codes = {code};
        auto response_result =
            self->clientManager_->process_authenticated_request(std::move(request));
