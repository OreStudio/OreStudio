 ...
 */
std::vector<domain::party_id_scheme> read_one(repository::party_id_scheme_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::party_id_scheme_key& key) {
 ...
 */
messaging::party_id_scheme_key key_from(const domain::party_id_scheme& v) {
    messaging::party_id_scheme_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::party_id_scheme to_domain(const messaging::party_id_scheme_write& write) {
    domain::party_id_scheme v;
    v.code = write.code;
    v.name = write.name;
    v.description = write.description;
    v.coding_scheme_code = write.coding_scheme_code;
    v.display_order = write.display_order;
    v.max_cardinality = write.max_cardinality;
    return v;
}
...
messaging::list_party_id_schemes_response party_id_scheme_service::list_party_id_schemes(
    ...
    messaging::list_party_id_schemes_response response;
...
messaging::get_party_id_scheme_response party_id_scheme_service::get_party_id_scheme(
    const messaging::get_party_id_scheme_request& request) {
    messaging::get_party_id_scheme_response response;
    ...
    response.party_id_scheme = std::move(found.front());
    return response;
}

messaging::get_many_party_id_schemes_response party_id_scheme_service::get_many_party_id_schemes(
    const messaging::get_many_party_id_schemes_request& request) {
    messaging::get_many_party_id_schemes_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_id_scheme_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party_id_scheme = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_party_id_scheme_response party_id_scheme_service::put_party_id_scheme(
    const messaging::put_party_id_scheme_request& request) {
    messaging::put_party_id_scheme_response response;
    domain::party_id_scheme value;
    ...
    if (!written.empty())
        response.party_id_scheme = std::move(written.front());
    return response;
}

messaging::put_many_party_id_schemes_response party_id_scheme_service::put_many_party_id_schemes(
    const messaging::put_many_party_id_schemes_request& request) {
    messaging::put_many_party_id_schemes_response response;
    std::vector<domain::party_id_scheme> batch;
...
messaging::delete_party_id_scheme_response party_id_scheme_service::delete_party_id_scheme(
    const messaging::delete_party_id_scheme_request& request) {
    messaging::delete_party_id_scheme_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::party_id_scheme_repository::remove_status::removed:
            break;
        case repository::party_id_scheme_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_id_scheme_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_id_scheme_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
party_id_scheme_service::delete_many_party_id_schemes(
    const messaging::delete_many_party_id_schemes_request& request) {
    messaging::delete_many_party_id_schemes_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
party_id_scheme_service::list_party_id_scheme_versions(
    const messaging::list_party_id_scheme_versions_request& request) {
    messaging::list_party_id_scheme_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
party_id_scheme_service::get_party_id_scheme_version(
    const messaging::get_party_id_scheme_version_request& request) {
    messaging::get_party_id_scheme_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.party_id_scheme.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
party_id_scheme_service::prepare_change(const messaging::party_id_scheme_change& change,
...
party_id_scheme_service::find_scheme(const std::string& code) {
    BOOST_LOG_SEV(lg(), debug) << "Finding party ID scheme. " << "code: " << code;
...
party_id_scheme_service::get_schemes(const std::vector<std::string>& codes) {
