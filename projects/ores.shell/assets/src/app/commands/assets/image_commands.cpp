...
void image_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("images");
...
void image_commands::process_list(std::ostream& out,
                                  nats_client& session,
    ...
    auto result = do_auth_request<messaging::list_images_response>(
...
void image_commands::process_get(std::ostream& out,
                                 nats_client& session,
                                 const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::get_image_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_get_many(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::get_many_images_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_add(std::ostream& out,
                                 nats_client& session,
                                 const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::put_image_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_set(std::ostream& out,
                                 nats_client& session,
                                 const std::vector<std::string>& args) {
...
void image_commands::process_put_many(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::put_many_images_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_delete(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::delete_image_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_delete_many(std::ostream& out,
                                         nats_client& session,
                                         const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::delete_many_images_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_versions(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    ...
    auto result = do_auth_request<messaging::list_image_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void image_commands::process_version(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    ...
    try {
        ...
        read_token(req.key.image.code, parsed->positionals[next++], "code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_image_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
