...
#include "ores.trading.api/eventing/equity_accumulator_instrument_changed_event.hpp"
...
// notification" pattern end to end for equity_accumulator_instrument -- the
...
// Equity Accumulator Instrument writes are party-scoped: the session-level
...
TEST_CASE("write_equity_accumulator_instrument_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::trading::eventing::equity_accumulator_instrument_changed_event>(
        [&nats](const ores::trading::eventing::equity_accumulator_instrument_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::trading::eventing::equity_accumulator_instrument_changed_event>::
                                name),
                ev::domain::entity_change_event{.entity =
                                                    "ores.trading.equity_accumulator_instrument",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.equity_accumulator_instrument_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source
        .register_mapping<ores::trading::eventing::equity_accumulator_instrument_changed_event>(
            "ores.trading.equity_accumulator_instrument",
            "ores_trading_equity_accumulator_instruments");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<
                    ores::trading::eventing::equity_accumulator_instrument_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_equity_accumulator_instrument(ctx);
    ...
    BOOST_LOG_SEV(lg, debug) << "Equity Accumulator Instrument: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.trading.equity_accumulator_instrument") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for equity_accumulator_instrument " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for equity_accumulator_instrument "
                            << id_str;
}
