 ...
 *
 */
#ifndef ORES_TRADING_CORE_REPOSITORY_EQUITY_FORWARD_INSTRUMENT_REPOSITORY_HPP
#define ORES_TRADING_CORE_REPOSITORY_EQUITY_FORWARD_INSTRUMENT_REPOSITORY_HPP
 ...
 */
class ORES_TRADING_CORE_EXPORT equity_forward_instrument_repository {
private:
...
public:
    ...
    /**
     * @brief Writes equity forward instruments to database.
     */
    /**@{*/
    ...
    /**
     * @brief Reads latest equity forward instruments, possibly filtered by primary key.
     */
    /**@{*/
    ...
    /**
     * @brief Reads all equity forward instruments, possibly filtered by primary key.
     */
    std::vector<domain::equity_forward_instrument> read_all(context ctx,
                                                            const std::string& instrument_id);

    /**
     * @brief Reads a single equity forward instrument as it stood at a specific
    ...
    /**
     * @brief Reads latest equity forward instruments with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_equity_forward_instrument_count(context ctx);

    /**
     * @brief Deletes a equity forward instrument by closing its temporal validity.
     */
    void remove(context ctx, const std::string& instrument_id);

    /**
     * @brief Deletes equity forward instruments by closing their temporal validity.
     */
