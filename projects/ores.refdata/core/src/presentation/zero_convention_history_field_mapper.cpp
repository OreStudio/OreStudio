...
render_zero_convention_fields(const domain::zero_convention& v) {
    ...
    fields.push_back({.name = "Compounding Frequency",
                      .value = v.compounding_frequency.value_or(std::string{})});
    fields.push_back({.name = "Tenor Calendar", .value = v.tenor_calendar.value_or(std::string{})});
    fields.push_back(
        {.name = "Spot Lag", .value = v.spot_lag ? std::to_string(*v.spot_lag) : std::string{}});
    fields.push_back({.name = "Spot Calendar", .value = v.spot_calendar.value_or(std::string{})});
    fields.push_back(
        {.name = "Roll Convention", .value = v.roll_convention.value_or(std::string{})});
    fields.push_back(
        {.name = "End Of Month",
