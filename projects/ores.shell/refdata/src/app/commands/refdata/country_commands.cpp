...
void country_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("countries");
    ...
    menu->Insert(
        ...
        "add <alpha2_code> <alpha3_code> <numeric_code> <name> <official_name> <image_id> "
        "<coding_scheme_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <alpha2_code> <alpha3_code> <numeric_code> <name> <official_name> <image_id> "
        "<coding_scheme_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <alpha2_code> <alpha3_code> <numeric_code> <name> <official_name> "
        "<image_id> <coding_scheme_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "delete <alpha2_code> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <alpha2_code> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <alpha2_code> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <alpha2_code> --version <n>");

    root_menu.Insert(std::move(menu));
}

void country_commands::process_list(std::ostream& out,
                                    nats_client& session,
                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_countries_request;
    ...
    auto result = do_auth_request<messaging::list_countries_response>(
...
void country_commands::process_get(std::ostream& out,
                                   nats_client& session,
                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_country_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.alpha2_code, parsed->positionals[next++], "alpha2_code");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_country_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_get_many(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_countries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::country_key key;
            read_token(key.alpha2_code, parsed->positionals[i + 0], "alpha2_code");
    ...
    auto result = do_auth_request<messaging::get_many_countries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_add(std::ostream& out,
                                   nats_client& session,
                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_country_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.alpha2_code, parsed->positionals[next++], "alpha2_code");
        read_token(req.change.write.alpha3_code, parsed->positionals[next++], "alpha3_code");
        ...
        read_token(req.change.write.official_name, parsed->positionals[next++], "official_name");
    ...
    auto result = do_auth_request<messaging::put_country_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_set(std::ostream& out,
                                   nats_client& session,
                                   const std::vector<std::string>& args) {
...
void country_commands::process_put_many(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_countries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::country_change change;
            read_token(change.write.alpha2_code, parsed->positionals[next++], "alpha2_code");
            read_token(change.write.alpha3_code, parsed->positionals[next++], "alpha3_code");
            ...
            read_token(change.write.official_name, parsed->positionals[next++], "official_name");
            read_token(change.write.image_id, parsed->positionals[next++], "image_id");
            read_token(
                change.write.coding_scheme_code, parsed->positionals[next++], "coding_scheme_code");
    ...
    auto result = do_auth_request<messaging::put_many_countries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_delete(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_country_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.alpha2_code, parsed->positionals[next++], "alpha2_code");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_country_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_delete_many(std::ostream& out,
                                           nats_client& session,
                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_countries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::country_key key;
            read_token(key.alpha2_code, parsed->positionals[i * 1 + 0], "alpha2_code");
            req.removals.push_back(messaging::country_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_countries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_versions(std::ostream& out,
                                        nats_client& session,
                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_country_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_country_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void country_commands::process_version(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_country_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.country.alpha2_code, parsed->positionals[next++], "alpha2_code");
        req.key.version =
    ...
    auto result = do_auth_request<messaging::get_country_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
