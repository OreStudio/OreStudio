...
int ClientPricingModelProductParameterModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientPricingModelProductParameterModel::data(const QModelIndex& index, int role) const {
    ...
    if (row >= parameters_.size())
        return {};

    const auto& parameter = parameters_[row];
    ...
    if (role == Qt::ForegroundRole) {
        return recency_foreground_color(parameter.parameter_name);
    }
...
void ClientPricingModelProductParameterModel::refresh() {
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug) << "Fetched " << result->parameters.size()
                                           << " pricing model product parameters, total available: "
...
void ClientPricingModelProductParameterModel::onParametersLoaded() {
    is_fetching_ = false;

    const auto result = watcher_->result();

    if (!result.success) {
        BOOST_LOG_SEV(lg(), error) << "Failed to fetch pricing model product parameters: "
    ...
    const int new_count = static_cast<int>(result.parameters.size());

    if (new_count > 0) {
        beginResetModel();
        parameters_ = std::move(result.parameters);
        endResetModel();
    ...
    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " pricing model product parameters."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientPricingModelProductParameterModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
const analytics::domain::pricing_model_product_parameter*
ClientPricingModelProductParameterModel::getParameter(int row) const {
...
void ClientPricingModelProductParameterModel::onPulsingComplete() {
