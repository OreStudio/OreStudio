...
on "ores_refdata_purpose_types_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_purpose_types_tenant_idx
on "ores_refdata_purpose_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_purpose_types_insert_fn()
returns trigger as $$
declare
