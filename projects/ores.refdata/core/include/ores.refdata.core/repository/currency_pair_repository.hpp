 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CURRENCY_PAIR_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CURRENCY_PAIR_REPOSITORY_HPP
...
/**
 * @brief Reads and writes currency pairs to data storage.
 */
class ORES_REFDATA_CORE_EXPORT currency_pair_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.currency_pair_repository";
...
public:
    ...
    /**
     * @brief Writes currency pairs to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::currency_pair& v);
    void write(context ctx, const std::vector<domain::currency_pair>& v);
    /**@}*/

    /**
     * @brief Writes a currency pair, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of currency pairs, each honouring its own
    ...
    /**
     * @brief Reads a single currency pair as it stood at a specific
     ...
     */
    std::uint32_t get_total_pair_count(context ctx);

    /**
     * @brief Deletes a currency pair by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a currency pair, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes currency pairs by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::currency_pair& v);
     ...
     */
    domain::currency_pair apply_claim(context ctx,
                                      const domain::currency_pair& v,
                                      const ores::utility::domain::precondition& claim);
};
