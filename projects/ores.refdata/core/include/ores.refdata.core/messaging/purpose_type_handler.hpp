...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        if (auto req = decode<get_purpose_types_request>(msg)) {
            try {
    ...
    void save(ores::nats::message msg) {
        ...
        service::purpose_type_service svc(req_ctx);
        if (auto req = decode<save_purpose_type_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(purpose_type_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(
                    nats_, msg, save_purpose_type_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(purpose_type_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::purpose_type_service svc(req_ctx);
        if (auto req = decode<get_purpose_type_history_request>(msg)) {
            try {
                ...
                reply(
                    nats_,
                    msg,
                    get_purpose_type_history_response{.history = std::move(hist), .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(purpose_type_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
    ...
    void remove(ores::nats::message msg) {
        ...
        service::purpose_type_service svc(req_ctx);
        if (auto req = decode<delete_purpose_type_request>(msg)) {
            try {
                ...
                reply(nats_,
                      msg,
                      delete_purpose_type_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(purpose_type_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
