 ...
 */
std::vector<domain::curve_role> read_one(repository::curve_role_repository& repo,
                                         const ores::database::context& ctx,
                                         const messaging::curve_role_key& key) {
 ...
 */
messaging::curve_role_key key_from(const domain::curve_role& v) {
    messaging::curve_role_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::curve_role to_domain(const messaging::curve_role_write& write) {
    domain::curve_role v;
    v.code = write.code;
    v.name = write.name;
...
curve_role_service::list_curve_roles(const messaging::list_curve_roles_request& request) {
    messaging::list_curve_roles_response response;
...
messaging::get_curve_role_response
curve_role_service::get_curve_role(const messaging::get_curve_role_request& request) {
    messaging::get_curve_role_response response;
    ...
    response.curve_role = std::move(found.front());
    return response;
}

messaging::get_many_curve_roles_response
curve_role_service::get_many_curve_roles(const messaging::get_many_curve_roles_request& request) {
    messaging::get_many_curve_roles_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::curve_role_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.curve_role = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_curve_role_response
curve_role_service::put_curve_role(const messaging::put_curve_role_request& request) {
    messaging::put_curve_role_response response;
    domain::curve_role value;
    ...
    if (!written.empty())
        response.curve_role = std::move(written.front());
    return response;
}

messaging::put_many_curve_roles_response
curve_role_service::put_many_curve_roles(const messaging::put_many_curve_roles_request& request) {
    messaging::put_many_curve_roles_response response;
...
messaging::delete_curve_role_response
curve_role_service::delete_curve_role(const messaging::delete_curve_role_request& request) {
    messaging::delete_curve_role_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::curve_role_repository::remove_status::removed:
            break;
        case repository::curve_role_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::curve_role_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::curve_role_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_curve_roles_response curve_role_service::delete_many_curve_roles(
    const messaging::delete_many_curve_roles_request& request) {
    messaging::delete_many_curve_roles_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_curve_role_versions_response curve_role_service::list_curve_role_versions(
    const messaging::list_curve_role_versions_request& request) {
    messaging::list_curve_role_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_curve_role_version_response curve_role_service::get_curve_role_version(
    const messaging::get_curve_role_version_request& request) {
    messaging::get_curve_role_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.curve_role.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
curve_role_service::prepare_change(const messaging::curve_role_change& change,
                                   const ores::utility::domain::change_intent& intent,
                                   domain::curve_role& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
curve_role_service::get_roles(const std::vector<std::string>& codes) {
