...
ois_convention_repository::replace_claim(context ctx, const domain::ois_convention& v) {
...
ois_convention_repository::apply_claim(context ctx,
                                       const domain::ois_convention& v,
...
void ois_convention_repository::write(context ctx,
                                      const domain::ois_convention& v,
                                      const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing OIS convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, ois_convention_mapper::map(t), lg(), "Writing OIS convention to database.");
}

void ois_convention_repository::write(
    context ctx,
    const std::vector<domain::ois_convention>& v,
    ...
    execute_write_query(
        ctx, ois_convention_mapper::map(batch), lg(), "Writing OIS conventions to database.");
}
...
std::vector<domain::ois_convention> ois_convention_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<ois_convention_entity, domain::ois_convention>(
            ...
            lg(),
            "Reading latest OIS conventions (workspace resolution chain).");
    }
...
std::optional<domain::ois_convention> ois_convention_repository::read_at_version(
    context ctx, const std::string& id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading OIS convention at version. " << "id: " << id
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<ois_convention_entity>> |
    ...
    const auto entities = execute_read_query<ois_convention_entity, domain::ois_convention>(
        ctx,
        query,
        [](const auto& entities) { return ois_convention_mapper::map(entities); },
        lg(),
        "Reading OIS convention at version.");
...
ois_convention_repository::read_latest(context ctx, std::uint32_t offset, std::uint32_t limit) {
...
std::uint32_t ois_convention_repository::get_total_ois_convention_count(context ctx) {
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<ois_convention_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active OIS convention count: " << count;
    return count;
}

std::vector<domain::ois_convention>
ois_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<ois_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<ois_convention_entity, domain::ois_convention>(
        ctx,
        query,
        [](const auto& entities) { return ois_convention_mapper::map(entities); },
        lg(),
        "Reading latest OIS conventions by ids.");
    return result;
}
