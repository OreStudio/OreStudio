...
on "public"."ores_refdata_party_roles_tbl" (tenant_id, code, coding_scheme_code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_refdata_party_roles_code_uniq_idx
on "public"."ores_refdata_party_roles_tbl" (tenant_id, code, coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_party_roles_tenant_idx
on "public"."ores_refdata_party_roles_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_party_roles_coding_scheme_idx
on "public"."ores_refdata_party_roles_tbl" (coding_scheme_code)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function public.ores_refdata_party_roles_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    from "public"."ores_refdata_party_roles_tbl"
    ...
    if found then
        ...
        update "public"."ores_refdata_party_roles_tbl"
...
before insert on "public"."ores_refdata_party_roles_tbl"
for each row
execute function public.ores_refdata_party_roles_insert_fn();

create or replace rule ores_refdata_party_roles_delete_rule as
on delete to "public"."ores_refdata_party_roles_tbl"
do instead
  update "public"."ores_refdata_party_roles_tbl"
  set valid_to = current_timestamp
