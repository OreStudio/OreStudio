 ...
 */
#ifndef ORES_IAM_API_EVENTING_ACCOUNT_TYPE_EVENT_HPP
#define ORES_IAM_API_EVENTING_ACCOUNT_TYPE_EVENT_HPP
...
/**
 * @brief Event traits for account type's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::iam::messaging::account_type_event> {
    static constexpr std::string_view subject_prefix = "iam.v1.account_types_events";

    static ores::iam::messaging::account_type_event
    from_notification(const entity_event_notification& notification) {
        ores::iam::messaging::account_type_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::iam::messaging::account_type_key>(notification.key))
            event.key = *key;
