...
#include "ui_PartyIdentifierHistoryDialog.h"
...
PartyIdentifierHistoryDialog::PartyIdentifierHistoryDialog(const boost::uuids::uuid& id,
                                                           const QString& code,
                                                           ClientManager* clientManager,
                                                           QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::PartyIdentifierHistoryDialog)
    , id_(id)
    , code_(code)
...
PartyIdentifierHistoryDialog::~PartyIdentifierHistoryDialog() {
    delete ui_;
}

QString PartyIdentifierHistoryDialog::code() const {
    return code_;
}

void PartyIdentifierHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for party identifier: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_party_identifier_history_request request;
    request.id = boost::uuids::to_string(id_);

    QPointer<PartyIdentifierHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_party_identifier_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int PartyIdentifierHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString PartyIdentifierHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow PartyIdentifierHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult PartyIdentifierHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
    ...
    checkString(diffs, tr("Scheme"), curr.id_scheme, prev.id_scheme);
    checkString(diffs, tr("Value"), curr.id_value, prev.id_value);
    checkString(diffs, tr("Description"), curr.description, prev.description);
    return diffs;
}

void PartyIdentifierHistoryDialog::displayFullDetails(int index) {
    ...
    ui_->idSchemeValue->setText(QString::fromStdString(version.id_scheme));
    ui_->idValueValue->setText(QString::fromStdString(version.id_value));
...
void PartyIdentifierHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void PartyIdentifierHistoryDialog::revertToVersionAt(int index) {
