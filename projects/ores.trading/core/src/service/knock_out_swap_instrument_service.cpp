...
knock_out_swap_instrument_service::list_knock_out_swap_instruments(std::uint32_t offset,
                                                                   std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all knock-out swap instruments";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t knock_out_swap_instrument_service::count_knock_out_swap_instruments() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total knock-out swap instruments count";
    return repo_.get_total_knock_out_swap_instrument_count(ctx_);
}
...
std::optional<domain::knock_out_swap_instrument>
knock_out_swap_instrument_service::get_knock_out_swap_instrument(const std::string& instrument_id) {
...
void knock_out_swap_instrument_service::save_knock_out_swap_instruments(
    ...
    auto ts = knock_out_swap_instruments;
    for (auto& e : ts)
        stamp(e, ctx_);
    repo_.write(ctx_, ts);
}
