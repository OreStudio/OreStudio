...
using ores::accounts::security::password_policy_validator;
...
TEST_CASE("valid_password_passes_validation", tags) {
    ...
    auto result = password_policy_validator::validate(password);
...
TEST_CASE("weak_password_passes_when_validation_disabled", tags) {
    ...
    auto result = password_policy_validator::validate(weak_password, false);
    CHECK(result.is_valid);
    CHECK(result.error_message.empty());
}
...
TEST_CASE("validation_enabled_by_default", tags) {
    ...
    auto result = password_policy_validator::validate(weak_password);
    CHECK(!result.is_valid);
    CHECK(!result.error_message.empty());
}
