...
rebuild the site itself: run =compass build --direct site= first
whenever the content changes, then =site start= (or just leave it
running and rebuild again -- the already-running server picks up the
new files immediately, since it serves the directory live).
