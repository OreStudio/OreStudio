...
std::string pricing_engine_type_repository::sql() {
    return generate_create_table_sql<pricing_engine_type_entity>(lg());
}

ores::utility::domain::precondition
pricing_engine_type_repository::replace_claim(context ctx, const domain::pricing_engine_type& v) {
...
pricing_engine_type_repository::apply_claim(context ctx,
...
void pricing_engine_type_repository::write(context ctx,
    ...
    execute_write_query(
        ctx, pricing_engine_type_mapper::map(t), lg(), "Writing pricing engine type to database.");
}
...
void pricing_engine_type_repository::write(
    ...
    execute_write_query(ctx,
                        pricing_engine_type_mapper::map(batch),
                        lg(),
                        "Writing pricing engine types to database.");
}
...
pricing_engine_type_repository::remove_status pricing_engine_type_repository::remove(
...
std::vector<domain::pricing_engine_type>
pricing_engine_type_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<pricing_engine_type_entity, domain::pricing_engine_type>(
        ctx,
        query,
        [](const auto& entities) { return pricing_engine_type_mapper::map(entities); },
        lg(),
        "Reading latest pricing engine types by ids.");
    return result;
}
