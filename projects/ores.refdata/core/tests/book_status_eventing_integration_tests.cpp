...
// notification" pattern end to end for book_status -- the
...
TEST_CASE("write_book_status_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::book_status_changed_event>(
        [&nats](const ores::refdata::eventing::book_status_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::book_status_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.book_status",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::book_status_changed_event>(
        "ores.refdata.book_status", "ores_refdata_book_statuses");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::book_status_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_book_status(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Book Status: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.book_status") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for book_status " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for book_status " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::book_status_service svc(crud_ctx);
        ...
        REQUIRE_FALSE(svc.get_status(id_str).has_value());
