...
| =quant= | =ores.analytics= | Dependency-light quantitative math library — CRM spanning-tree topology, the derived-rate engine and stochastic price processes. Separated from =core= because it carries no database or NATS dependency and is consumed for its mathematics alone, by =core=, by the synthetic data generators and by tests. |
