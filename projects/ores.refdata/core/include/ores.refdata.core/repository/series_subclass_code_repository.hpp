 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_SERIES_SUBCLASS_CODE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_SERIES_SUBCLASS_CODE_REPOSITORY_HPP
...
/**
 * @brief Reads and writes series subclass codes to data storage.
 */
class ORES_REFDATA_CORE_EXPORT series_subclass_code_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.series_subclass_code_repository";
...
public:
    ...
    /**
     * @brief Writes series subclass codes to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::series_subclass_code& v);
    void write(context ctx, const std::vector<domain::series_subclass_code>& v);
    /**@}*/

    /**
     * @brief Writes a series subclass code, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::series_subclass_code& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of series subclass codes, each honouring its own
    ...
    /**
     * @brief Reads latest series subclass codes, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::series_subclass_code> read_latest(context ctx);
    std::vector<domain::series_subclass_code> read_latest(context ctx, const std::string& code);
    std::vector<domain::series_subclass_code> read_latest(context ctx,
    ...
    /**
     * @brief Reads all series subclass codes, possibly filtered by primary key.
     */
    std::vector<domain::series_subclass_code> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single series subclass code as it stood at a specific
    ...
    /**
     * @brief Reads latest series subclass codes with pagination support.
     * @param ctx Repository context with database connection
     ...
     */
    std::uint32_t get_total_series_subclass_count(context ctx);
    ...
    /**
     * @brief Removes a series subclass code, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes series subclass codes by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::series_subclass_code apply_claim(context ctx,
                                             const domain::series_subclass_code& v,
                                             const ores::utility::domain::precondition& claim);
};
