...
on "ores_refdata_party_types_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ores_refdata_party_types_code_uniq_idx
on "ores_refdata_party_types_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ores_refdata_party_types_tenant_idx
on "ores_refdata_party_types_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_party_types_insert_fn()
returns trigger as $$
declare
...
before insert on "ores_refdata_party_types_tbl"
for each row
execute function ores_refdata_party_types_insert_fn();

create or replace rule ores_refdata_party_types_delete_rule as
on delete to "ores_refdata_party_types_tbl"
do instead
  update "ores_refdata_party_types_tbl"
  set valid_to = current_timestamp
