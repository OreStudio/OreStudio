...
#include "ores.refdata.api/generators/asset_class_code_generator.hpp"
...
// notification" pattern end to end for asset_class_code -- the
...
using ores::refdata::domain::asset_class_code;
using ores::refdata::repository::asset_class_code_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_asset_class_code_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::asset_class_code_changed_event>(
        [&nats](const ores::refdata::eventing::asset_class_code_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::asset_class_code_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.asset_class_code",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::asset_class_code_changed_event>(
        "ores.refdata.asset_class_code", "ores_refdata_asset_class_codes");
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.asset_class_code") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for asset_class_code " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for asset_class_code " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_asset_class(id_str).has_value());
        REQUIRE(svc.get_asset_class_history(id_str).size() == versions.size());
    }
}
