 ...
 */
std::vector<domain::folder> read_one(repository::folder_repository& repo,
                                     const ores::database::context& ctx,
                                     const messaging::folder_key& key) {
 ...
 */
messaging::folder_key key_from(const domain::folder& v) {
    messaging::folder_key key;
    key.id = v.id;
    return key;
}
 ...
 */
domain::folder to_domain(const messaging::folder_write& write) {
    domain::folder v;
    v.id = write.id;
    v.parent_id = write.parent_id;
    v.name = write.name;
    v.kind = write.kind;
    v.collection_id = write.collection_id;
    return v;
}
...
folder_service::list_folders(const messaging::list_folders_request& request) {
    messaging::list_folders_response response;
    ...
    response.folders = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_folder_count(ctx_);
    return response;
}

messaging::get_folder_response
folder_service::get_folder(const messaging::get_folder_request& request) {
    messaging::get_folder_response response;
    ...
    response.folder = std::move(found.front());
    return response;
}

messaging::get_many_folders_response
folder_service::get_many_folders(const messaging::get_many_folders_request& request) {
    messaging::get_many_folders_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::folder_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.folder = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_folder_response
folder_service::put_folder(const messaging::put_folder_request& request) {
    messaging::put_folder_response response;
    domain::folder value;
    ...
    if (!written.empty())
        response.folder = std::move(written.front());
    return response;
}

messaging::put_many_folders_response
folder_service::put_many_folders(const messaging::put_many_folders_request& request) {
    messaging::put_many_folders_response response;
    std::vector<domain::folder> batch;
    ...
    response.folders.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.folders.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_folder_response
folder_service::delete_folder(const messaging::delete_folder_request& request) {
    messaging::delete_folder_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::folder_repository::remove_status::removed:
            break;
        case repository::folder_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::folder_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::folder_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
folder_service::delete_many_folders(const messaging::delete_many_folders_request& request) {
    messaging::delete_many_folders_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
folder_service::list_folder_versions(const messaging::list_folder_versions_request& request) {
    messaging::list_folder_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
folder_service::get_folder_version(const messaging::get_folder_version_request& request) {
    messaging::get_folder_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.folder.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
folder_service::prepare_change(const messaging::folder_change& change,
...
std::optional<domain::folder> folder_service::get_folder_at_version(const boost::uuids::uuid& id,
...
std::optional<domain::folder> folder_service::get_folder(const boost::uuids::uuid& id) {
...
std::vector<domain::folder> folder_service::get_folders(const std::vector<std::string>& ids) {
...
void folder_service::delete_folder(const boost::uuids::uuid& id) {
