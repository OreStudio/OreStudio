...
create or replace function ores_refdata_currencies_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    changed_key := jsonb_build_object('iso_code', changed_iso_code);
    ...
    perform pg_notify('ores_refdata_currencies', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_refdata_currencies_notify_trg
after insert or update or delete on ores_refdata_currencies_tbl
for each row execute function ores_refdata_currencies_notify_fn();
