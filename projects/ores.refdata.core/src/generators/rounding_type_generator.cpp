 ...
 *
 */
#include "ores.refdata.core/generators/rounding_type_generator.hpp"
...
domain::rounding_type generate_synthetic_rounding_type(
    ...
    r.version = 1;
    r.code = "Code" + std::to_string(++counter);
    r.name = std::string(faker::word::adjective()) + " Rounding";
...
generate_synthetic_rounding_types(std::size_t n,
    ...
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_rounding_type(ctx));
    return r;
}
