...
void equity_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_equity_swap_instrument(std::ref(out),
                                               std::ref(session),
                                               ...
                                               std::move(long_short),
                                               std::move(payment_frequency),
                                               std::move(description),
                                               std::move(change_reason_code),
                                               std::move(change_commentary));
        },
        "Add an Equity swap instrument (trade_type_code underlying_name [basket_json] currency "
        "notional return_type start_date maturity_date long_short payment_frequency [description] "
        "change_reason_code \"change_commentary\")",
        {"trade_type_code underlying_name basket_json currency notional return_type start_date "
         "maturity_date long_short payment_frequency description change_reason_code "
         "change_commentary"});
...
void equity_swap_instrument_commands::process_get_equity_swap_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity swap instruments request.";
...
void equity_swap_instrument_commands::process_add_equity_swap_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an Equity swap instrument." << std::endl;
        return;
    }
...
void equity_swap_instrument_commands::process_delete_equity_swap_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an Equity swap instrument." << std::endl;
        return;
    }
