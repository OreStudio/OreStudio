...
{{/seed_currency}}
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for {{entity_singular}} " << id_str
...
{{/has_audit_group}}
        ...
        auto versions = svc.get_{{entity_singular_short}}_history(id_str);
...
{{/has_audit_group}}
        ...
        svc.delete_{{entity_singular_short}}(id_str);
        ...
        REQUIRE_FALSE(svc.{{#service_find_prefix}}find_{{/service_find_prefix}}{{^service_find_prefix}}get_{{/service_find_prefix}}{{entity_singular_short}}(id_str).has_value());
        REQUIRE(svc.get_{{entity_singular_short}}_history(id_str).size() == versions.size());
    }
}
{{/domain_entity}}
#+end_src
