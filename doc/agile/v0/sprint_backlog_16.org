...
- *Instrument type* (=instrument_family=, to be renamed =instrument_type=) —
...
- Market data series: has =asset_class=, no =instrument_type=.
- Trades: has both =instrument_type= (routing) AND =asset_class= (risk).
...
- =instrument_family= is renamed =instrument_type= everywhere: PG enum type,
...
- [X] Establish that =instrument_family= and =asset_class= are different
      ...
      FpML/CDM (=asset_class= = risk taxonomy, =instrument_type= = structural
...
- [X] Decide =instrument_family= → =instrument_type= rename (values unchanged)
...
- [ ] Phase 1: Rename =instrument_family= → =instrument_type= throughout
