...
#+description: The * SQL section of an entity model: table naming, the has_parent_id hierarchy knob, and the ores.sql.schema variability feature bundle (tenant-scoping policy, extra checks, tenant-scoping FK validations). Generates the bi-temporal table DDL, insert triggers, and notify triggers via the ores.sql.schema facet.
#+type: knowledge
#+level: cross
...
- [[id:CE95C751-2125-4BA9-A78D-C30F9540FC5E][Variability]] — MASD's formal vocabulary for =:has_parent_id:= as a configuration point; =:tablename:= is structural (§ "Flags" above), not variability.
