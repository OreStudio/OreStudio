...
#include "ores.refdata.api/generators/payment_frequency_generator.hpp"
...
// notification" pattern end to end for payment_frequency -- the
...
using ores::refdata::repository::payment_frequency_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_payment_frequency_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::payment_frequency_changed_event>(
        [&nats](const ores::refdata::eventing::payment_frequency_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::payment_frequency_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.payment_frequency",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::payment_frequency_changed_event>(
        "ores.refdata.payment_frequency", "ores_refdata_payment_frequencies");
    ...
    auto v = generate_synthetic_payment_frequency(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Payment Frequency: " << v;

    payment_frequency_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.payment_frequency") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for payment_frequency " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for payment_frequency " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::payment_frequency_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_payment_frequency_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_payment_frequency(id_str);
        ...
        REQUIRE_FALSE(svc.get_payment_frequency(id_str).has_value());
        REQUIRE(svc.get_payment_frequency_history(id_str).size() == versions.size());
    }
}
