...
FraConventionMdiWindow::FraConventionMdiWindow(ClientManager* clientManager,
...
void FraConventionMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &FraConventionMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &FraConventionMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &FraConventionMdiWindow::deleteSelected);
    ...
    connect(
        historyAction_, &QAction::triggered, this, &FraConventionMdiWindow::viewHistorySelected);
}
...
void FraConventionMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "FraConventionListWindow", {}, {900, 400}, 1);
}

void FraConventionMdiWindow::setupConnections() {
    connect(
        model_, &ClientFraConventionModel::dataLoaded, this, &FraConventionMdiWindow::onDataLoaded);
    connect(
        model_, &ClientFraConventionModel::loadError, this, &FraConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &FraConventionMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &FraConventionMdiWindow::onDoubleClicked);
...
void FraConventionMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void FraConventionMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* fc = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showConventionHistory for code: " << fc->id;
...
void FraConventionMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete FRA convention while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " FRA conventions";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 FRA conventions?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete FRA Convention", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    auto task = [self, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making batch delete request for " << codes.size() << " FRA conventions";
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "FRA Convention deletion failed: " << code << " - " << result.second;
                failure_count++;
