...
zero_convention_repository::replace_claim(context ctx, const domain::zero_convention& v) {
...
zero_convention_repository::apply_claim(context ctx,
...
void zero_convention_repository::write(context ctx, const std::vector<domain::zero_convention>& v) {
...
void zero_convention_repository::write(context ctx,
                                       const domain::zero_convention& v,
                                       const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing zero convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, zero_convention_mapper::map(t), lg(), "Writing zero convention to database.");
}
...
void zero_convention_repository::write(
    ...
    execute_write_query(
        ctx, zero_convention_mapper::map(batch), lg(), "Writing zero conventions to database.");
}
...
std::vector<domain::zero_convention> zero_convention_repository::read_latest(context ctx) {
    ...
    return execute_read_query<zero_convention_entity, domain::zero_convention>(
        ...
        [](const auto& entities) { return zero_convention_mapper::map(entities); },
        lg(),
        "Reading latest zero conventions");
}
...
std::vector<domain::zero_convention>
zero_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<zero_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<zero_convention_entity, domain::zero_convention>(
        ctx,
        query,
        [](const auto& entities) { return zero_convention_mapper::map(entities); },
        lg(),
        "Reading latest zero conventions by ids.");
    return result;
}
