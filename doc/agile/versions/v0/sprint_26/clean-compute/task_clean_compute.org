...
| Now          | Every section is worked. Three items do not pass: G02, on the two telemetry tables the codegen cannot express; V03, on the iam bootstrap that creates no account; and V04, which is blocked by V03. |
| Waiting on   | A decision on the append-only fact-table shape (G02) and a fix to the iam bootstrap (V03, V04). |
| Next         | Whichever of those two the reader wants first. |
...
| V03  | Partial | The database recreates from scratch: =compass db recreate -y -k= exits 0, and the recreated platform table carries the generated notify trigger. The fleet starts and serves: the services reconnect after the recreate, hosts save, and node samples land in =ores_compute_node_samples_tbl=. A session does not: the recreated database is in bootstrap mode and both bootstrap commands report success while persisting no account and no tenant, so every login fails. That is an iam bootstrap defect. Evidence in =evidence/v03_v04_fleet.txt=. |
| V04  | Blocked | The fourteen generated compute recipes all stop at their login step for the V03 reason, so no command has been run against the live fleet. The commands themselves are linked and exercised by the shell's own tests. Evidence in =evidence/v03_v04_fleet.txt=. |
