...
// Soft-FK parent seeding (ores_synthetic_market_data_generation_configs_tbl): the parent may live
...
// notification" pattern end to end for fx_spot_generation_config -- the
...
// FX Spot Generation Config writes are party-scoped: the session-level
...
using ores::synthetic::repository::fx_spot_generation_config_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_fx_spot_generation_config_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::synthetic::eventing::fx_spot_generation_config_changed_event>(
        [&nats](const ores::synthetic::eventing::fx_spot_generation_config_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::synthetic::eventing::fx_spot_generation_config_changed_event>::name),
                ev::domain::entity_change_event{.entity =
                                                    "ores.synthetic.fx_spot_generation_config",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.fx_spot_generation_config_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source
        .register_mapping<ores::synthetic::eventing::fx_spot_generation_config_changed_event>(
            "ores.synthetic.fx_spot_generation_config",
            "ores_synthetic_fx_spot_generation_configs");
    ...
    auto v = generate_synthetic_fx_spot_generation_config(ctx);
    v.change_reason_code = "system.test";
    v.party_id = *party_ctx.party_id();
    // Seed the active market_data_generation_config row
    // ores_synthetic_market_data_generation_configs_tbl references: the insert trigger's existence
    ...
    ores::synthetic::repository::market_data_generation_config_repository config_id_repo;
    ...
    BOOST_LOG_SEV(lg, debug) << "FX Spot Generation Config: " << v;

    fx_spot_generation_config_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.synthetic.fx_spot_generation_config") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for fx_spot_generation_config " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for fx_spot_generation_config "
                            << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::synthetic::service::fx_spot_generation_config_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_fx_spot_generation_config_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_fx_spot_generation_config(id_str);
        ...
        REQUIRE_FALSE(svc.get_fx_spot_generation_config(id_str).has_value());
        REQUIRE(svc.get_fx_spot_generation_config_history(id_str).size() == versions.size());
    }
}
