...
inline auto& pricing_model_config_handler_lg() {
    static auto instance = ores::logging::make_logger(
        "ores.analytics.messaging.pricing_model_config_handler");
    return instance;
}
} // namespace
    ...
    void list(ores::nats::message msg) {
        ...
        try {
            ...
            resp.total_available_count =
                static_cast<int>(resp.configs.size());
        ...
        if (auto req = decode<save_pricing_model_config_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg,
                    save_pricing_model_config_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_, msg, save_pricing_model_config_response{
        ...
        if (auto req = decode<get_pricing_model_config_history_request>(msg)) {
            try {
                ...
                reply(nats_, msg, get_pricing_model_config_history_response{
                    .configs = std::move(hist), .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_, msg, get_pricing_model_config_history_response{
        ...
        if (auto req = decode<delete_pricing_model_config_request>(msg)) {
            try {
                ...
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), debug)
                    << "Completed " << msg.subject;
                reply(nats_, msg,
                    delete_pricing_model_config_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(pricing_model_config_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_, msg, delete_pricing_model_config_response{
                    .success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(pricing_model_config_handler_lg(), warn)
