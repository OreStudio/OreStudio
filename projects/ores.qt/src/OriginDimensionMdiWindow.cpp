...
void OriginDimensionMdiWindow::setupToolbar() {
    ...
    reloadAction_->setToolTip(tr("Reload origin dimensions from server"));
...
void OriginDimensionMdiWindow::startPulseAnimation() {
    pulseCount_ = 0;
    pulseState_ = false;
    pulseTimer_->start();
}
