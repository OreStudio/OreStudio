...
the 7 more files (15 occurrences) using other direct =rfl::json=
patterns that don't go through =do_request= at all (e.g.
=application.cpp='s bootstrap/login calls, =repl.cpp='s logout call,
=reports_commands.cpp=, =lei_commands.cpp=, =parties_commands.cpp=,
=connection_commands.cpp=, =history_diff_renderer.cpp=,
=synthetic_commands.cpp=, =workflow_commands.cpp=). Full breakdown
recorded in the write-up task's =* Plan=.
