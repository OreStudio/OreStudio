...
// notification" pattern end to end for business_unit -- the
...
TEST_CASE("write_business_unit_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::business_unit_changed_event>(
        [&nats](const ores::refdata::eventing::business_unit_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::business_unit_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.business_unit",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.business_unit_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::business_unit_changed_event>(
        "ores.refdata.business_unit", "ores_refdata_business_units");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::business_unit_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_business_unit(ctx);
    ...
    BOOST_LOG_SEV(lg, debug) << "Business Unit: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.business_unit") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for business_unit " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for business_unit " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_business_unit(id_str).has_value());
        REQUIRE(svc.get_business_unit_history(id_str).size() == versions.size());
    }
}
