...
/**
 * Badge Severities Population Functions
 *
 * Functions to publish badge severities from the DQ artefact table into a
 * tenant's own copy of ores_dq_badge_severities_tbl. Self-referential (like
 * coding_schemes): DQ publishes into its own table, not another service's.
 *
 * Usage:
 *   -- Preview what will be published
 *   SELECT * FROM ores_dq_badge_severity_preview_fn(dataset_id);
 *
 *   -- Publish to a tenant
 *   SELECT * FROM ores_dq_badge_severities_publish_fn(dataset_id, target_tenant_id, 'upsert');
 */
...
create or replace function ores_dq_badge_severity_preview_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
    name text,
...
begin
    ...
    left join ores_dq_badge_severities_tbl existing
