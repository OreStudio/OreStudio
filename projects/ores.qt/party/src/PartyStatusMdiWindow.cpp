...
PartyStatusMdiWindow::PartyStatusMdiWindow(ClientManager* clientManager,
...
void PartyStatusMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &PartyStatusMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &PartyStatusMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &PartyStatusMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &PartyStatusMdiWindow::viewHistorySelected);
}
...
void PartyStatusMdiWindow::setupConnections() {
    ...
    connect(tableView_->selectionModel(),
            ...
            this,
            &PartyStatusMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &PartyStatusMdiWindow::onDoubleClicked);
...
void PartyStatusMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 party statuses").arg(loaded).arg(total));
...
void PartyStatusMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void PartyStatusMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete party status while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " party statuses";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 party statuses?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Party Status", confirmMessage, QMessageBox::Yes | QMessageBox::No);
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Party Status deletion failed: " << code << " - " << result.second;
                failure_count++;
