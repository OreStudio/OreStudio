...
std::vector<tenor_convention_resolution> test_resolutions() {
    return {make_resolution("SPOT"),
            make_resolution("3M"),
            make_resolution("6M"),
            make_resolution("1Y")};
}

constexpr std::chrono::year_month_day horizon{
    std::chrono::year{2026}, std::chrono::month{1}, std::chrono::day{1}};
constexpr std::chrono::year_month_day spot{
    std::chrono::year{2026}, std::chrono::month{1}, std::chrono::day{3}};
...
TEST_CASE("validate_curve_template accepts a single deposit-equivalent entry",
    ...
    const std::vector<curve_template_entry_ref> entries{{.sequence_index = 0,
                                                         .start_tenor_code = "SPOT",
                                                         .end_tenor_code = "3M",
                                                         .instrument_code = "DEPO"}};
    const auto result = validate_curve_template(entries,
                                                test_tenors(),
