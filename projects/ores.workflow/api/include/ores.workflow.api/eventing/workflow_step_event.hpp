 ...
 */
#ifndef ORES_WORKFLOW_API_EVENTING_WORKFLOW_STEP_EVENT_HPP
#define ORES_WORKFLOW_API_EVENTING_WORKFLOW_STEP_EVENT_HPP
...
/**
 * @brief Event traits for workflow step's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::workflow::messaging::workflow_step_event> {
    static constexpr std::string_view subject_prefix = "workflow.v1.workflow_steps_events";

    static ores::workflow::messaging::workflow_step_event
    from_notification(const entity_event_notification& notification) {
        ores::workflow::messaging::workflow_step_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::workflow::messaging::workflow_step_key>(notification.key))
            event.key = *key;
