 ...
 * versions in the compute grid. Idempotent: guarded by name/version lookups
...
-- -------------------------------------------------------------------------
-- ORE App Version 1.8.16.0-4-g3b62ba248
-- -------------------------------------------------------------------------
with ore_app as (
...
inserted_version as (
    ...
    where not exists (
        ...
        where av.engine_version = '1.8.16.0-4-g3b62ba248'
...
-- -------------------------------------------------------------------------
-- ORE App Version 1.8.17.0
--
-- Kept alongside 1.8.16.0 so a job can be dispatched against either
-- engine and the two compared. The version is the provisional tag on the
-- engine fork; rebuild and update the sha256 once upstream tags 1.8.17.0
-- properly. See external/ore/packages/manifest.json.
-- -------------------------------------------------------------------------
with ore_app as (
    select id from ores_compute_apps_tbl
    where name = 'ORE'
...
inserted_version as (
    ...
    where not exists (
        select 1 from ores_compute_app_versions_tbl av
        join ore_app on av.app_id = ore_app.id
        where av.engine_version = '1.8.17.0'
          and av.wrapper_version = '1.0.0'
          and av.tenant_id = ores_utility_system_tenant_id_fn()
          and av.valid_to = ores_utility_infinity_timestamp_fn()
    )
...
select
    ...
    app_version_id.id,
    p.id,
    '/api/v1/storage/compute/packages/ORE/1.8.17.0/ORE-1.8.17.0-x64-linux.tar.gz',
    '8099ac8f51067cf59378137a327f273b073390e1cc47ea862f5cfcc2ab519868',
    current_user,
    current_user,
    'system.new_record',
    '',
