...
void BookStatusHistoryDialog::setupConnections() {
    connect(ui_->versionListWidget, &QTableWidget::itemSelectionChanged,
            this, &BookStatusHistoryDialog::onVersionSelected);
...
void BookStatusHistoryDialog::loadHistory() {
    ...
    auto task = [self, code = code_.toStdString()]() -> HistoryResult {
        ...
        comms::messaging::frame request_frame(
            comms::messaging::message_type::get_book_status_history_request,
            0, std::move(payload)
        );
        ...
        auto response = refdata::messaging::get_book_status_history_response::
