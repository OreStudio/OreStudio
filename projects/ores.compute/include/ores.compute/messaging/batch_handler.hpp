...
namespace {
inline auto& batch_handler_lg() {
    static auto instance = ores::logging::make_logger(
        "ores.compute.messaging.batch_handler");
    return instance;
}
} // namespace
        ...
        try {
            if (auto req = decode<list_batches_request>(msg)) {
                ...
                resp.total_available_count =
                    static_cast<int>(resp.batches.size());
            }
        } catch (...) {}
    ...
    void save(ores::nats::message msg) {
        ...
        if (auto req = decode<save_batch_request>(msg)) {
            try {
                ...
                reply(nats_, msg, save_batch_response{
                    .success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(batch_handler_lg(), warn)
