...
| 13 | resolve_context only packs tenant_id, discarding the party/roles/workspace that make_request_context already resolved | registrar.cpp | Accepted (revised) | Initially recorded as a decision with no code change (correct-for-currency-today reasoning); revisited and fixed for real — see history_caller_context.hpp/.cpp: resolve_context now round-trips the full resolved context (tenant, party, visible parties, actor, roles, workspace) as JSON, so every future history_provider in this component gets the same visibility any other subject would, not just tenant scoping |
