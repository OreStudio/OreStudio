...
| B06 | fixed | Consumer census by `grep -rl --include=*.cpp --include=*.hpp` for each header, run from the repository root and excluding the component's own directory. The count is therefore of C++ sources and headers that include the header: `make_generation_context.hpp` 167 files, `scoped_database_helper.hpp` 155, `nats_options_helper.hpp` 115, `logging_listener.hpp` 69, `database_helper.hpp` 58, `project_root.hpp` 53, `database_lifecycle_listener.hpp` 46, `test_timeout_listener.hpp` 16, `scoped_environment_override.hpp` 11, `series_key_shape_seed.hpp` 4, `series_classification_rule_seed.hpp` 1. 348 C++ files outside the component include at least one of its headers; a `git grep` without the include filter counts more, because the header path also appears in generated CMake lists. Four headers have no consumer outside the component: `sql_values_rows.hpp`, `test_database_manager.hpp`, `export.hpp`, and the namespace header `ores.testing.hpp`, which G08 exempts from a census. `run_coroutine_test.hpp` had no consumer anywhere, its own tests included, so it is deleted. `sql_values_rows` and `test_database_manager` have production consumers inside the component: the two seed readers call the scanner, and the helper, listener and manager translation units call the manager. A test is not a consumer, and neither is one. |
...
| G06 | fixed | `regenerate_cmake_component_files.py --component ores.testing --check` is green for both lists. The repo-wide `--all --check` still reports 105 stale files that belong to other components, which is the pre-existing condition the standard records and each component's own pass fixes. |
...
| V07 | pass | No vacuous test. Every new case asserts a literal expected value, apart from the two root-discovery checks, which assert the observable fact that the root it found holds `.git` and a `projects` directory. The standard's own proof holds, measured at this head: stub the row parser's `is_space` to return false and `project_root::resolve` to return its argument, then rebuild, and 13 of the 27 cases fail on 15 assertions. The two `project_root` assertions that fail are the absolute-path and existence checks, and every parser case that reads a column with surrounding whitespace fails with them. The subjects were restored, and the suite is green at 58 assertions in 27 test cases. |
...
- Scale. 23 hand-written C++ files in 16 headers and 7 sources. The component
  had no tests at all.
- Reach. 348 files outside the component include at least one of its headers.
  `make_generation_context.hpp` alone reaches 167 files, `scoped_database_helper.hpp`
  155 and `nats_options_helper.hpp` 115. The consumers are
...
The local code review ran before the PR was raised, over `git diff origin/main...HEAD`. It found no Critical issue and raised five Important and six Minor findings. All are fixed except the scope note.
...
| 4 | The resolve-append case asserted one production run against another, so it pinned no literal | projects/ores.testing/tests/project_root_tests.cpp | fixed | It now asserts that the resolved path ends with the relative path it was given. The other two cases assert the file name and the parent directory of a named model. |
...
| 6 | `trim` stripped four whitespace characters while the scanner skips six, so a vertical tab or a form feed survived into a column | projects/ores.testing/include/ores.testing/sql_values_rows.hpp | fixed | `trim` now uses `std::isspace` like the scanner, and a case covers both characters. |
...
The component gained a Catch2 suite of 58 assertions in 27 test cases, its
