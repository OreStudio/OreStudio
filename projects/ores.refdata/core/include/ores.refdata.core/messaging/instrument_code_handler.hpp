...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        get_instrument_codes_response resp;
        if (auto req = decode<get_instrument_codes_request>(msg)) {
            try {
                resp.instruments = svc.list_instruments(req->offset, req->limit);
                resp.total_available_count = static_cast<int>(svc.count_instruments());
                resp.success = true;
    ...
    void save(ores::nats::message msg) {
        ...
        service::instrument_code_service svc(req_ctx);
        if (auto req = decode<save_instrument_code_request>(msg)) {
            try {
                svc.save_instrument(req->data);
                BOOST_LOG_SEV(instrument_code_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, save_instrument_code_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(instrument_code_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      save_instrument_code_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(instrument_code_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::instrument_code_service svc(req_ctx);
        if (auto req = decode<get_instrument_code_history_request>(msg)) {
            try {
                auto hist = svc.get_instrument_history(req->code);
                BOOST_LOG_SEV(instrument_code_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_,
                      msg,
                      get_instrument_code_history_response{.history = std::move(hist),
                                                           .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(instrument_code_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_instrument_code_history_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(instrument_code_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
    ...
    void remove(ores::nats::message msg) {
        ...
        service::instrument_code_service svc(req_ctx);
        if (auto req = decode<delete_instrument_code_request>(msg)) {
            try {
                svc.delete_instruments(req->codes);
                BOOST_LOG_SEV(instrument_code_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, delete_instrument_code_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(instrument_code_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      delete_instrument_code_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(instrument_code_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
