...
void session_repository::create(const domain::session& session) {
    BOOST_LOG_SEV(lg(), debug) << "Creating session: " << boost::uuids::to_string(session.id);
...
void session_repository::update(const domain::session& session) {
    BOOST_LOG_SEV(lg(), debug) << "Updating session: " << boost::uuids::to_string(session.id);

    auto entity = session_mapper::map(session);
    const auto query =
        sqlgen::update<session_entity>("end_time"_c.set(entity.end_time),
                                       "bytes_sent"_c.set(entity.bytes_sent),
                                       "bytes_received"_c.set(entity.bytes_received),
                                       "country_code"_c.set(entity.country_code)) |
        where("id"_c == entity.id.value() && "start_time"_c == entity.start_time.value());
...
void session_repository::update_bytes(const boost::uuids::uuid& session_id,
    ...
    const auto query = sqlgen::update<session_entity>(
                       ...
                       where("id"_c == session_id_str && "start_time"_c == start_time_ts);
...
void session_repository::end_session(const boost::uuids::uuid& session_id,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Ending session: " << boost::uuids::to_string(session_id);
...
std::optional<domain::session> session_repository::read(const boost::uuids::uuid& session_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading session: " << boost::uuids::to_string(session_id);
...
std::vector<domain::session> session_repository::read_by_account(
    const boost::uuids::uuid& account_id, std::uint32_t limit, std::uint32_t offset) {
    ...
    if (limit > 0) {
        const auto query =
            ...
            order_by("start_time"_c.desc()) | sqlgen::offset(static_cast<std::size_t>(offset)) |
...
std::uint32_t session_repository::count_active_by_account(const boost::uuids::uuid& account_id) {
...
std::uint32_t session_repository::count_by_account(const boost::uuids::uuid& account_id) {
...
std::vector<domain::session>
session_repository::read_by_time_range(const std::chrono::system_clock::time_point& start,
...
std::vector<domain::session> session_repository::read_all_active() {
...
std::uint32_t session_repository::count_all_active() {
    ...
    const auto query = sqlgen::select_from<session_entity>(sqlgen::count().as<"count">()) |
                       where("end_time"_c == empty_end_time) | sqlgen::to<count_result>;
...
session_repository::read_daily_statistics(const boost::uuids::uuid& account_id,
    ...
    const auto query =
        sqlgen::read<std::vector<session_statistics_entity>> |
        where("account_id"_c == account_id_str && "day"_c >= start_str && "day"_c <= end_str) |
...
std::vector<domain::session_statistics> session_repository::read_aggregate_daily_statistics(
    const std::chrono::system_clock::time_point& start,
    const std::chrono::system_clock::time_point& end) {
...
void session_repository::close_orphaned_sessions(
    ...
    for (const auto& s : active) {
        BOOST_LOG_SEV(lg(), info) << "Closing orphaned session: " << boost::uuids::to_string(s.id);
        try {
...
void session_repository::remove_by_account(const boost::uuids::uuid& account_id) {
    ...
    const auto account_id_str = boost::lexical_cast<std::string>(account_id);
    const auto query =
        sqlgen::delete_from<session_entity> | where("account_id"_c == account_id_str);
