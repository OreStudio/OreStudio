...
{{entity_singular}}_repository::read_at_timepoint(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Reading {{entity_singular_words}} at timepoint. "
                               << "{{primary_key.column}}: " << {{primary_key.column}};
    const auto ts = make_timestamp(as_of, lg());
{{#read_tenant_filtered}}
    const auto tid = ctx.tenant_id().to_string();{{sys_decl}}
{{#has_workspace_id}}
    ...
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
        where({{{tenant_where}}} && "workspace_id"_c == wid &&
              "{{primary_key.column}}"_c == {{primary_key.column}} &&
              "valid_from"_c <= ts.value() && "valid_to"_c > ts.value());
{{/has_workspace_id}}
{{^has_workspace_id}}
    const auto query = sqlgen::read<std::vector<{{entity_singular}}_entity>> |
...
{{/read_tenant_filtered}}
    ...
    return execute_read_query<{{entity_singular}}_entity, domain::{{entity_singular}}>(
        ctx, query,
        [](const auto& entities) { return {{entity_singular}}_mapper::map(entities); },
        lg(), "Reading {{entity_singular_words}} at timepoint.");
}
{{/has_as_of_lookup}}
...
{{#junction}}
{{#junction}}
{{#left.query_needs_str}}
#include "ores.platform/time/datetime.hpp"
{{/left.query_needs_str}}
{{#right.query_needs_str}}
#include "ores.platform/time/datetime.hpp"
{{/right.query_needs_str}}
{{/junction}}
...
std::vector<domain::{{name_singular}}>
{{name_singular}}_repository::read_latest(
    const {{left.cpp_type}}& {{left.column}},
    const {{right.cpp_type}}& {{right.column}}) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest {{name_singular_words}}. "
                               << {{left.column}} << "/" << {{right.column}};

    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto {{left.column}}_str = {{#left.is_uuid}}boost::uuids::to_string({{left.column}}){{/left.is_uuid}}{{#left.is_date}}ores::platform::time::datetime::to_iso8601_date({{left.column}}){{/left.is_date}}{{#left.is_timestamp}}ores::platform::time::datetime::to_iso8601({{left.column}}){{/left.is_timestamp}}{{^left.query_needs_str}}{{left.column}}{{/left.query_needs_str}};
    const auto {{right.column}}_str = {{#right.is_uuid}}boost::uuids::to_string({{right.column}}){{/right.is_uuid}}{{#right.is_date}}ores::platform::time::datetime::to_iso8601_date({{right.column}}){{/right.is_date}}{{#right.is_timestamp}}ores::platform::time::datetime::to_iso8601({{right.column}}){{/right.is_timestamp}}{{^right.query_needs_str}}{{right.column}}{{/right.query_needs_str}};
{{#has_tenant_id}}
    const auto tid = ctx_.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<{{name_singular}}_entity>> |
...
{{name_singular}}_repository::read_latest_by_{{left.column_short}}(
{{#left.query_needs_str}}
    const {{left.cpp_type}}& {{left.column}}, std::uint32_t offset, std::uint32_t limit) {
    const auto {{left.column}}_str = {{#left.is_uuid}}boost::uuids::to_string({{left.column}}){{/left.is_uuid}}{{#left.is_date}}ores::platform::time::datetime::to_iso8601_date({{left.column}}){{/left.is_date}}{{#left.is_timestamp}}ores::platform::time::datetime::to_iso8601({{left.column}}){{/left.is_timestamp}}{{^left.query_needs_str}}{{left.column}}{{/left.query_needs_str}};
{{/left.query_needs_str}}
{{^left.query_needs_str}}
...
{{name_singular}}_repository::read_latest_by_{{right.column_short}}(
{{#right.query_needs_str}}
    const {{right.cpp_type}}& {{right.column}}, std::uint32_t offset, std::uint32_t limit) {
    const auto {{right.column}}_str = {{#right.is_uuid}}boost::uuids::to_string({{right.column}}){{/right.is_uuid}}{{#right.is_date}}ores::platform::time::datetime::to_iso8601_date({{right.column}}){{/right.is_date}}{{#right.is_timestamp}}ores::platform::time::datetime::to_iso8601({{right.column}}){{/right.is_timestamp}}{{^right.query_needs_str}}{{right.column}}{{/right.query_needs_str}};
{{/right.query_needs_str}}
{{^right.query_needs_str}}
