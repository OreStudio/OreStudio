 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_TENOR_SCHEDULE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_TENOR_SCHEDULE_EVENT_HPP
...
/**
 * @brief Event traits for tenor schedule's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::tenor_schedule_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.tenor_schedules_events";

    static ores::refdata::messaging::tenor_schedule_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::tenor_schedule_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::tenor_schedule_key>(notification.key))
            event.key = *key;
