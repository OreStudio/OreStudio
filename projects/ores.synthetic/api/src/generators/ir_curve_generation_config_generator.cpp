...
generate_synthetic_ir_curve_generation_config(utility::generation::generation_context& ctx) {
    ...
    r.index_name = std::string("USD-SOFR") + "-" + std::to_string(idx);
...
generate_synthetic_ir_curve_generation_configs(std::size_t n,
    ...
    std::vector<domain::ir_curve_generation_config> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_ir_curve_generation_config(ctx));
    return r;
}
