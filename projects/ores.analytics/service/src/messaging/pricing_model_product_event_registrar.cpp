...
register_pricing_model_product_event_mapping(ev::service::postgres_event_source& event_source,
                                             ev::service::event_bus& event_bus,
                                             ores::nats::service::client& nats) {
    // The trigger publishes on the table's own channel, and the mapping turns
    // what it says into this entity's event.
    event_source.register_entity_event_mapping<analytics::messaging::pricing_model_product_event>(
        "ores_analytics_pricing_model_products");

    return event_bus.subscribe<analytics::messaging::pricing_model_product_event>(
        [&nats](const analytics::messaging::pricing_model_product_event& e) {
            ...
            ev::service::publish_entity_event(
                nats,
                ev::domain::event_subject<analytics::messaging::pricing_model_product_event>(
                    e.action),
                e);
        });
}
