...
void variability_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    ...
    variability_menu->Insert(
        "list-settings",
        [&session](std::ostream& out) { process_list_settings(std::ref(out), std::ref(session)); },
    ...
    variability_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_save_setting(std::ref(out),
                                 std::ref(session),
                                 std::move(name),
                                 std::move(value),
                                 std::move(data_type),
    ...
    variability_menu->Insert(
        "delete-setting",
        [&session](std::ostream& out, std::string name) {
            process_delete_setting(std::ref(out), std::ref(session), std::move(name));
        },
    ...
    variability_menu->Insert(
        "setting-history",
        [&session](std::ostream& out, std::string name) {
            process_get_setting_history(std::ref(out), std::ref(session), std::move(name));
        },
...
void variability_commands::process_list_settings(std::ostream& out, nats_client& session) {
    ...
    BOOST_LOG_SEV(lg(), info) << "Successfully retrieved " << result->settings.size()
                              << " settings.";
    out << result->settings << std::endl;
}

void variability_commands::process_save_setting(std::ostream& out,
                                                nats_client& session,
                                                std::string name,
                                                std::string value,
                                                std::string data_type,
                                                ...
                                                std::string change_reason_code,
                                                std::string change_commentary) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating save setting request for: " << name;
    ...
    auto req = variability::messaging::save_setting_request::from(
        variability::domain::system_setting{.version = 0,
                                            .name = std::move(name),
...
void variability_commands::process_delete_setting(std::ostream& out,
                                                  nats_client& session,
                                                  std::string name) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete setting request for: " << name;
...
void variability_commands::process_get_setting_history(std::ostream& out,
                                                       nats_client& session,
                                                       std::string name) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get setting history for: " << name;
