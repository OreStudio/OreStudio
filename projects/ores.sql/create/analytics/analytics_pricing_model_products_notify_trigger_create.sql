...
create or replace function ores_analytics_pricing_model_products_notify_fn()
returns trigger as $$
declare
    notification_payload jsonb;
    change_action text;
    changed_version integer := 0;
    changed_pricing_engine_type_code text;
    changed_key jsonb;
    changed_tenant_id text;
begin
    if TG_OP = 'DELETE' then
        change_action := 'deleted';
        changed_pricing_engine_type_code := OLD.pricing_engine_type_code;
        ...
        changed_pricing_engine_type_code := NEW.pricing_engine_type_code;
        changed_tenant_id := NEW.tenant_id::text;
    end if;

    changed_key := jsonb_build_object('pricing_engine_type_code', changed_pricing_engine_type_code);
    ...
    perform pg_notify('ores_analytics_pricing_model_products', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_analytics_pricing_model_products_notify_trg
after insert or update or delete on ores_analytics_pricing_model_products_tbl
for each row execute function ores_analytics_pricing_model_products_notify_fn();
