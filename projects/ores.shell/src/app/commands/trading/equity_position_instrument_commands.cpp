...
void equity_position_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    equity_position_instruments_menu->Insert(
        ...
        [&session](std::ostream& out,
            ...
            process_add_equity_position_instrument(std::ref(out),
                                                   std::ref(session),
...
void equity_position_instrument_commands::process_get_equity_position_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get Equity position instruments request.";
...
void equity_position_instrument_commands::process_add_equity_position_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add an Equity position instrument." << std::endl;
        return;
    }
    ...
    v.quantity = quantity;
    v.option_data_json = (option_data_json == "-") ? "" : std::move(option_data_json);
    v.description = std::move(description);

    try {
        v.price = parse_optional_double(price, "price");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }
...
void equity_position_instrument_commands::process_delete_equity_position_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete an Equity position instrument." << std::endl;
        return;
    }
