...
create table if not exists ""."refdata_party_roles_tbl" (
    "code" text not null,
...
on ""."refdata_party_roles_tbl" (code, version)
where valid_to = public.utility_infinity_timestamp_fn();

create index if not exists refdata_party_roles_coding_scheme_idx
on ""."refdata_party_roles_tbl" (coding_scheme_code)
where valid_to = public.utility_infinity_timestamp_fn();

create or replace function .refdata_party_roles_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update ""."refdata_party_roles_tbl"
...
before insert on ""."refdata_party_roles_tbl"
for each row
execute function .refdata_party_roles_insert_fn();

create or replace rule refdata_party_roles_delete_rule as
on delete to ""."refdata_party_roles_tbl"
do instead
  update ""."refdata_party_roles_tbl"
  set valid_to = current_timestamp
  where code = old.code
