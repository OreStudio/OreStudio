...
void FxSpotGenerationConfigMdiWindow::setupUi() {
...
void FxSpotGenerationConfigMdiWindow::setupToolbar() {
    ...
    addAction_->setToolTip(tr("Add new FX spot generation config"));
    connect(addAction_, &QAction::triggered, this, &FxSpotGenerationConfigMdiWindow::addNew);

    editAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::Edit, IconUtils::DefaultIconColor), tr("Edit"));
    editAction_->setToolTip(tr("Edit selected FX spot generation config"));
    editAction_->setEnabled(false);
    connect(editAction_, &QAction::triggered, this, &FxSpotGenerationConfigMdiWindow::editSelected);
...
void FxSpotGenerationConfigMdiWindow::setupConnections() {
    connect(model_,
            &ClientFxSpotGenerationConfigModel::dataLoaded,
            this,
            &FxSpotGenerationConfigMdiWindow::onDataLoaded);
    connect(model_,
            &ClientFxSpotGenerationConfigModel::loadError,
            this,
            &FxSpotGenerationConfigMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &FxSpotGenerationConfigMdiWindow::onSelectionChanged);
    connect(tableView_,
            &QTableView::doubleClicked,
            this,
            &FxSpotGenerationConfigMdiWindow::onDoubleClicked);
...
void FxSpotGenerationConfigMdiWindow::doReload() {
...
void FxSpotGenerationConfigMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 FX spot generation configs").arg(loaded).arg(total));
...
void FxSpotGenerationConfigMdiWindow::onLoadError(const QString& error_message,
...
void FxSpotGenerationConfigMdiWindow::onSelectionChanged() {
    updateActionStates();
}

void FxSpotGenerationConfigMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void FxSpotGenerationConfigMdiWindow::updateActionStates() {
...
void FxSpotGenerationConfigMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new FX spot generation config requested";
    emit addNewRequested();
}

void FxSpotGenerationConfigMdiWindow::editSelected() {
...
void FxSpotGenerationConfigMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete FX spot generation config while disconnected.");
        return;
    }
    ...
    if (ids.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid FX spot generation configs to delete";
        return;
    }
