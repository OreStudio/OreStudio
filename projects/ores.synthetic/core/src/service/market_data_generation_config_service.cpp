...
read_one(repository::market_data_generation_config_repository& repo,
         const ores::database::context& ctx,
         const messaging::market_data_generation_config_key& key) {
 ...
 */
messaging::market_data_generation_config_key
key_from(const domain::market_data_generation_config& v) {
    messaging::market_data_generation_config_key key;
    key.id = v.id;
    return key;
}
...
to_domain(const messaging::market_data_generation_config_write& write) {
    domain::market_data_generation_config v;
    v.id = write.id;
    v.scope = write.scope;
    v.binding_mode = write.binding_mode;
    v.name = write.name;
...
market_data_generation_config_service::list_market_data_generation_configs(
    const messaging::list_market_data_generation_configs_request& request) {
    messaging::list_market_data_generation_configs_response response;
    ...
    response.market_data_generation_configs =
        repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_market_data_generation_config_count(ctx_);
    return response;
}

messaging::get_market_data_generation_config_response
market_data_generation_config_service::get_market_data_generation_config(
    const messaging::get_market_data_generation_config_request& request) {
    messaging::get_market_data_generation_config_response response;
    ...
    response.market_data_generation_config = std::move(found.front());
    return response;
}

messaging::get_many_market_data_generation_configs_response
market_data_generation_config_service::get_many_market_data_generation_configs(
    const messaging::get_many_market_data_generation_configs_request& request) {
    messaging::get_many_market_data_generation_configs_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::market_data_generation_config_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.market_data_generation_config = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_market_data_generation_config_response
market_data_generation_config_service::put_market_data_generation_config(
    const messaging::put_market_data_generation_config_request& request) {
    messaging::put_market_data_generation_config_response response;
    domain::market_data_generation_config value;
    ...
    if (!written.empty())
        response.market_data_generation_config = std::move(written.front());
    return response;
}

messaging::put_many_market_data_generation_configs_response
market_data_generation_config_service::put_many_market_data_generation_configs(
    const messaging::put_many_market_data_generation_configs_request& request) {
    messaging::put_many_market_data_generation_configs_response response;
    std::vector<domain::market_data_generation_config> batch;
    ...
    response.market_data_generation_configs.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.market_data_generation_configs.push_back(
            written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_market_data_generation_config_response
market_data_generation_config_service::delete_market_data_generation_config(
    const messaging::delete_market_data_generation_config_request& request) {
    messaging::delete_market_data_generation_config_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::market_data_generation_config_repository::remove_status::removed:
            break;
        case repository::market_data_generation_config_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::market_data_generation_config_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::market_data_generation_config_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_market_data_generation_configs_response
market_data_generation_config_service::delete_many_market_data_generation_configs(
    const messaging::delete_many_market_data_generation_configs_request& request) {
    messaging::delete_many_market_data_generation_configs_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_market_data_generation_config_versions_response
market_data_generation_config_service::list_market_data_generation_config_versions(
    const messaging::list_market_data_generation_config_versions_request& request) {
    messaging::list_market_data_generation_config_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_market_data_generation_config_version_response
market_data_generation_config_service::get_market_data_generation_config_version(
    const messaging::get_market_data_generation_config_version_request& request) {
    messaging::get_market_data_generation_config_version_response response;
    auto found =
        repo_.read_at_version(ctx_,
                              boost::uuids::to_string(request.key.market_data_generation_config.id),
                              request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
...
ores::utility::domain::result market_data_generation_config_service::prepare_change(
    const messaging::market_data_generation_config_change& change,
    const ores::utility::domain::change_intent& intent,
    domain::market_data_generation_config& out) {
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
std::vector<domain::market_data_generation_config>
market_data_generation_config_service::get_market_data_generation_configs(
