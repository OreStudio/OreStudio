...
| Now          | Every applicable item passes. The component is at the standard. |
| Waiting on   | Nothing. The two decisions this task raised are recorded and taken. |
| Next         | Raise the PR, after a rebase onto the current =origin/main=. |
...
Two findings are left for their own task rather than fixed here. The
generated SQL emits a notify trigger for every entity, including the three
tables whose event surface is switched off: the trigger publishes an event
no registrar serves. And the schema validator did not see the hand-written
link table at all, because its =CREATE TABLE= was spelled in the
hand-written style; the generated file is spelled in the parsed style, so
the table is now checked. A create file the parser cannot read is silently
unchecked, which is worth its own look.
