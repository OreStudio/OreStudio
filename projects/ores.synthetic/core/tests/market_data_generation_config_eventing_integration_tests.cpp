...
// notification" pattern end to end for market_data_generation_config -- the
...
using ores::synthetic::domain::market_data_generation_config;
using ores::synthetic::repository::market_data_generation_config_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_market_data_generation_config_publishes_nats_changed_event", tags) {
    ...
    auto sub =
        bus.subscribe<ores::synthetic::eventing::market_data_generation_config_changed_event>(
            [&nats](
                const ores::synthetic::eventing::market_data_generation_config_changed_event& e) {
                ev::service::publish_entity_event(
                    nats,
                    std::string(ev::domain::event_traits<
                                ores::synthetic::eventing::
                                    market_data_generation_config_changed_event>::name),
                    ev::domain::entity_change_event{
                        .entity = "ores.synthetic.market_data_generation_config",
                        .timestamp = e.timestamp,
                        .entity_ids = e.market_data_generation_config_ids,
                        .tenant_id = e.tenant_id});
            });

    event_source
        .register_mapping<ores::synthetic::eventing::market_data_generation_config_changed_event>(
            "ores.synthetic.market_data_generation_config",
            "ores_synthetic_market_data_generation_configs");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<
                    ores::synthetic::eventing::market_data_generation_config_changed_event>::name),
        10);
    ...
    market_data_generation_config_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.synthetic.market_data_generation_config") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for market_data_generation_config " << id_str
