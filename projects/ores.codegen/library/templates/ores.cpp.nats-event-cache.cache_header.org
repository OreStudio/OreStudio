...
#+description: Consumer-side entity-mirror cache class: a partitioned_cache instantiation warmed by the entity's paged list read and reloaded by canonical events. =nats-event-cache= facet.
#+type: archetype
#+output: projects/{cache_component_dir}/{cache_subcomponent}/include/ores.{cache_component}.{cache_subcomponent}/service/cache/{entity}_cache.hpp
...
method that pages the entity's canonical =list= read to its end (a full
read is a list paged to its end; see
...
a =boost::uuids::uuid=/=tenant_id= field (every =cached_by= entity
does, since the per-tenant cache requires =has_tenant_id=),
...
=nats-handler= archetype). The generated class takes an optional
...
header is sent — only valid against a producer whose =list= handler
has no auth check, which none should.
 ...
 *
 * Populated by paging {{component}}'s list_{{entity_plural}} read to its
 * end at startup, and reloaded on receipt of a {{entity_singular}} event
        ...
        try {
            ...
            while (true) {
                const auto bytes = codec.encode(
                    ores::{{component}}::messaging::list_{{entity_plural}}_request{
                        .offset = offset, .limit = page_size});
                const auto reply = nats_.request_sync(
                    ores::{{component}}::messaging::list_{{entity_plural}}_request::nats_subject,
                    bytes, headers);
                auto resp = codec.decode<
                    ores::{{component}}::messaging::list_{{entity_plural}}_response>(reply.data);
                if (!resp || resp->result.outcome != ores::utility::domain::outcome::ok) {
                    const auto msg = resp ? resp->result.message
                                          : "parse error (malformed or error reply)";
                    BOOST_LOG_SEV({{entity_singular}}_cache_lg(), warn)
                        << "{{entity_pascal}} cache load failed for tenant " << tenant_id << ": "
                        << msg;
                    return msg;
                }
                const auto page_count = resp->{{entity_plural_short}}.size();
