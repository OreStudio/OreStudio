...
#include "ores.trading.api/eventing/equity_forward_instrument_changed_event.hpp"
...
TEST_CASE("write_equity_forward_instrument_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::trading::eventing::equity_forward_instrument_changed_event>(
        [&nats](const ores::trading::eventing::equity_forward_instrument_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<
                        ores::trading::eventing::equity_forward_instrument_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.trading.equity_forward_instrument",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.equity_forward_instrument_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::trading::eventing::equity_forward_instrument_changed_event>(
        "ores.trading.equity_forward_instrument", "ores_trading_equity_forward_instruments");
    ...
    for (int i = 0; i < 50 && received.empty(); ++i) {
        ...
        for (const auto& msg : snap) {
            ...
            if (decoded && decoded->entity == "ores.trading.equity_forward_instrument") {
