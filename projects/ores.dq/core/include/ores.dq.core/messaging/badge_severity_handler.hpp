...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        if (auto req = decode<get_badge_severities_request>(msg)) {
            try {
                resp.severities = svc.list_severities(req->offset, req->limit);
                resp.total_available_count = static_cast<int>(svc.count_severities());
                resp.success = true;
    ...
    void save(ores::nats::message msg) {
        ...
        service::badge_severity_service svc(req_ctx);
        if (auto req = decode<save_badge_severity_request>(msg)) {
            try {
                svc.save_severity(req->data);
                BOOST_LOG_SEV(badge_severity_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, save_badge_severity_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(badge_severity_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      save_badge_severity_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(badge_severity_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::badge_severity_service svc(req_ctx);
        if (auto req = decode<get_badge_severity_history_request>(msg)) {
            try {
                auto hist = svc.get_severity_history(req->code);
                BOOST_LOG_SEV(badge_severity_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_,
                      msg,
                      get_badge_severity_history_response{.history = std::move(hist),
                                                          .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(badge_severity_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_badge_severity_history_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(badge_severity_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
    ...
    void remove(ores::nats::message msg) {
        ...
        service::badge_severity_service svc(req_ctx);
        if (auto req = decode<delete_badge_severity_request>(msg)) {
            try {
                svc.delete_severities(req->codes);
                BOOST_LOG_SEV(badge_severity_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, delete_badge_severity_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(badge_severity_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      delete_badge_severity_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(badge_severity_handler_lg(), warn) << "Failed to decode: " << msg.subject;
            error_reply(nats_, msg, ores::service::error_code::bad_request);
