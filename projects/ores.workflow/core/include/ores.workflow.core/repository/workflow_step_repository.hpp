...
/**
 * @brief Reads and writes workflow steps to data storage.
 */
...
public:
    ...
    /**
     * @brief Writes workflow steps to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::workflow_step& v);
    void write(context ctx, const std::vector<domain::workflow_step>& v);
    /**@}*/

    /**
     * @brief Writes a workflow step, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of workflow steps, each honouring its own
    ...
    /**
     * @brief Reads latest workflow steps, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::workflow_step> read_latest(context ctx);
    std::vector<domain::workflow_step> read_latest(context ctx, const std::string& id);
    std::vector<domain::workflow_step> read_latest(context ctx,
    ...
    /**
     * @brief Reads all workflow steps, possibly filtered by primary key.
     */
    std::vector<domain::workflow_step> read_all(context ctx, const std::string& id);

    /**
     * @brief Reads a single workflow step as it stood at a specific
     ...
     */
    std::uint32_t get_total_step_count(context ctx);

    /**
     * @brief Deletes a workflow step by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a workflow step, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes workflow steps by closing their temporal validity.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::workflow_step& v);
     ...
     */
    domain::workflow_step apply_claim(context ctx,
                                      const domain::workflow_step& v,
                                      const ores::utility::domain::precondition& claim);
};
