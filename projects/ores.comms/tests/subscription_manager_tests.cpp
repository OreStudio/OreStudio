...
TEST_CASE("subscription_manager can register and unregister sessions", tags) {
    ...
    mgr.register_session("session1", [](const std::string&, auto, auto, auto, auto, auto) { return true; });
...
TEST_CASE("subscription_manager notify invokes callback for subscribers", tags) {
    ...
    mgr.register_session("session1",
        [&](const std::string& event_type, auto ts, auto, auto, auto, auto) {
            ++callback_count;
...
TEST_CASE("subscription_manager notify does not invoke for unsubscribed events", tags) {
    ...
    int callback_count = 0;
    mgr.register_session("session1",
        [&](const std::string&, auto, auto, auto, auto, auto) {
            ++callback_count;
            return true;
        });
...
TEST_CASE("subscription_manager multiple sessions can subscribe to same event", tags) {
    ...
    mgr.register_session("session1",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++total_calls; return true; });
    mgr.register_session("session2",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++total_calls; return true; });
...
TEST_CASE("subscription_manager re-registering session updates callback", tags) {
    ...
    mgr.register_session("session1",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++first_callback_count; return true; });
    mgr.subscribe("session1", "test.event");

    // Re-register with new callback
    mgr.register_session("session1",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++second_callback_count; return true; });
...
TEST_CASE("subscription_manager callback exception is handled gracefully", tags) {
    ...
    mgr.register_session("session1",
        [](const std::string&, auto, auto, auto, auto, auto) -> bool {
...
TEST_CASE("subscription_manager notify with tenant_id only delivers to matching tenant", tags) {
    ...
    mgr.register_session("session_acme",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++acme_calls; return true; });
    mgr.register_session("session_example",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++example_calls; return true; });
...
TEST_CASE("subscription_manager notify with empty tenant_id broadcasts to all", tags) {
    ...
    mgr.register_session("session1",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++session1_calls; return true; });
    mgr.register_session("session2",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++session2_calls; return true; });
...
TEST_CASE("subscription_manager pre-login sessions do not receive tenant notifications", tags) {
    ...
    mgr.register_session("session_prelogin",
        [&](const std::string&, auto, auto, auto, auto, auto) { ++prelogin_calls; return true; });
