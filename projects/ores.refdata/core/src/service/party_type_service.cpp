 ...
 */
std::vector<domain::party_type> read_one(repository::party_type_repository& repo,
                                         const ores::database::context& ctx,
                                         const messaging::party_type_key& key) {
 ...
 */
messaging::party_type_key key_from(const domain::party_type& v) {
    messaging::party_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::party_type to_domain(const messaging::party_type_write& write) {
    domain::party_type v;
    v.code = write.code;
    v.name = write.name;
...
party_type_service::list_party_types(const messaging::list_party_types_request& request) {
    messaging::list_party_types_response response;
...
party_type_service::get_party_type(const messaging::get_party_type_request& request) {
    messaging::get_party_type_response response;
    ...
    response.party_type = std::move(found.front());
    return response;
}

messaging::get_many_party_types_response
party_type_service::get_many_party_types(const messaging::get_many_party_types_request& request) {
    messaging::get_many_party_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party_type = std::move(found.front());
...
party_type_service::put_party_type(const messaging::put_party_type_request& request) {
    messaging::put_party_type_response response;
    domain::party_type value;
    ...
    if (!written.empty())
        response.party_type = std::move(written.front());
    return response;
}

messaging::put_many_party_types_response
party_type_service::put_many_party_types(const messaging::put_many_party_types_request& request) {
    messaging::put_many_party_types_response response;
    std::vector<domain::party_type> batch;
...
party_type_service::delete_party_type(const messaging::delete_party_type_request& request) {
    messaging::delete_party_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::party_type_repository::remove_status::removed:
            break;
        case repository::party_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_party_types_response party_type_service::delete_many_party_types(
    const messaging::delete_many_party_types_request& request) {
    messaging::delete_many_party_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_party_type_versions_response party_type_service::list_party_type_versions(
    const messaging::list_party_type_versions_request& request) {
    messaging::list_party_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_party_type_version_response party_type_service::get_party_type_version(
    const messaging::get_party_type_version_request& request) {
    messaging::get_party_type_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.party_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
party_type_service::prepare_change(const messaging::party_type_change& change,
...
std::uint32_t party_type_service::count_types() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total party types count";
...
party_type_service::get_types(const std::vector<std::string>& codes) {
