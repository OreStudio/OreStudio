 ...
 */
std::vector<domain::business_centre> read_one(repository::business_centre_repository& repo,
                                              const ores::database::context& ctx,
                                              const messaging::business_centre_key& key) {
 ...
 */
messaging::business_centre_key key_from(const domain::business_centre& v) {
 ...
 */
domain::business_centre to_domain(const messaging::business_centre_write& write) {
    domain::business_centre v;
    v.code = write.code;
    v.source = write.source;
    v.description = write.description;
    v.city_name = write.city_name;
...
messaging::get_business_centre_response business_centre_service::get_business_centre(
    const messaging::get_business_centre_request& request) {
    messaging::get_business_centre_response response;
    ...
    response.business_centre = std::move(found.front());
    return response;
}

messaging::get_many_business_centres_response business_centre_service::get_many_business_centres(
    const messaging::get_many_business_centres_request& request) {
    messaging::get_many_business_centres_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::business_centre_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.business_centre = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_business_centre_response business_centre_service::put_business_centre(
    const messaging::put_business_centre_request& request) {
    messaging::put_business_centre_response response;
    domain::business_centre value;
    ...
    if (!written.empty())
        response.business_centre = std::move(written.front());
    return response;
}

messaging::put_many_business_centres_response business_centre_service::put_many_business_centres(
    const messaging::put_many_business_centres_request& request) {
    messaging::put_many_business_centres_response response;
    std::vector<domain::business_centre> batch;
    ...
    response.centres.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.centres.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_business_centre_response business_centre_service::delete_business_centre(
    const messaging::delete_business_centre_request& request) {
    messaging::delete_business_centre_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::business_centre_repository::remove_status::removed:
            break;
        case repository::business_centre_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::business_centre_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::business_centre_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
business_centre_service::delete_many_business_centres(
    const messaging::delete_many_business_centres_request& request) {
    messaging::delete_many_business_centres_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
business_centre_service::list_business_centre_versions(
    const messaging::list_business_centre_versions_request& request) {
    messaging::list_business_centre_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
business_centre_service::get_business_centre_version(
    const messaging::get_business_centre_version_request& request) {
    messaging::get_business_centre_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.business_centre.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
business_centre_service::prepare_change(const messaging::business_centre_change& change,
...
business_centre_service::get_centres(const std::vector<std::string>& codes) {
