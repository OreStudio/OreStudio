...
create or replace function ores_workflow_workflow_steps_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    notification_payload := jsonb_build_object(
        ...
        'entity', 'ores.workflow.workflow_step',
    ...
    perform pg_notify('ores_workflow_workflow_steps', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_workflow_workflow_steps_notify_trg
after insert or update or delete on ores_workflow_workflow_steps_tbl
for each row execute function ores_workflow_workflow_steps_notify_fn();
