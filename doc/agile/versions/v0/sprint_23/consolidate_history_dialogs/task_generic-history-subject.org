...
| Now          | Added build_entity_history_versions(): shared glue that renders a newest-first version list's fields and diffs each against the next-older one via ores.diff::engine::compute — the last piece every per-entity history_provider needs before wiring. Still no service registers a real provider. |
| Waiting on   | Nothing.                               |
| Next         | Wire the first real per-entity provider (currency, as pilot): a currency_service::get_currency_history() + render_currency_fields() + build_entity_history_versions() provider, registered against a dispatch_registry owned by ores.refdata.service, with a caller_context_resolver validating the JWT. Leave the old get_currency_history subject/handler in place until every consuming frontend (Qt/HTTP/shell) switches to history.v1.get; remove it, and repeat for the remaining 26 entities, in later increments. |
