 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_CALENDAR_TYPE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_CALENDAR_TYPE_EVENT_HPP
...
/**
 * @brief Event traits for calendar type's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::calendar_type_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.calendar_types_events";

    static ores::refdata::messaging::calendar_type_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::calendar_type_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::refdata::messaging::calendar_type_key>(notification.key))
            event.key = *key;
