...
| =is_base64=     | True for a =:base64: true= column: the domain member is =std::vector<std::uint8_t>= and the row member is its base64 =std::string=. | =true= |
...
| =is_value_type= | The column's =:cpp_type:= is a value type opt-in: the entity member is the =std::string= the type wraps and the mapper converts through the type's =to_string()= and =from_string()=. Set by the model's =:is_value_type: true=. | =true= |
...
| =nullable_tenant_id= | Entity's tenant is nullable; SQL NULL means no tenant. |
...
| =has_base64_columns=| At least one column declares =:base64: true=, so the entity has a base64 hop to convert. |
