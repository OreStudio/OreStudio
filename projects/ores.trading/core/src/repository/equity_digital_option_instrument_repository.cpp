...
equity_digital_option_instrument_repository::read_latest(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Reading latest equity digital option instrument. instrument_id: "
                               << instrument_id;
    static auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<equity_digital_option_instrument_entity>> |
...
equity_digital_option_instrument_repository::read_all(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug)
        << "Reading all equity digital option instrument versions. instrument_id: "
        << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto query = sqlgen::read<std::vector<equity_digital_option_instrument_entity>> |
                       where("tenant_id"_c == tid && "instrument_id"_c == instrument_id) |
...
equity_digital_option_instrument_repository::read_latest(context ctx,
    ...
    return execute_read_query<equity_digital_option_instrument_entity,
        ...
        [](const auto& entities) { return equity_digital_option_instrument_mapper::map(entities); },
        lg(),
        "Reading latest equity_digital_option_instruments by ids.");
}
