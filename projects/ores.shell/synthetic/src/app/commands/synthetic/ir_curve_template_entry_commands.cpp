...
void ir_curve_template_entry_commands::register_commands(cli::Menu& root_menu,
                                                         nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("ir_curve_template_entries");
    ...
    menu->Insert(
        ...
        "set <id> <ir_curve_config_id> <sequence_index> <start_tenor_code> <end_tenor_code> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <ir_curve_config_id> <sequence_index> <start_tenor_code> "
...
void ir_curve_template_entry_commands::process_list(std::ostream& out,
                                                    nats_client& session,
                                                    const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_ir_curve_template_entries_request;
    ...
    auto result = do_auth_request<messaging::list_ir_curve_template_entries_response>(
...
void ir_curve_template_entry_commands::process_get(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_ir_curve_template_entry_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_ir_curve_template_entry_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_get_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_ir_curve_template_entries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_ir_curve_template_entries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_add(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_ir_curve_template_entry_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(
            req.change.write.start_tenor_code, parsed->positionals[next++], "start_tenor_code");
        read_token(req.change.write.end_tenor_code, parsed->positionals[next++], "end_tenor_code");
        read_token(
            req.change.write.instrument_code, parsed->positionals[next++], "instrument_code");
    ...
    auto result = do_auth_request<messaging::put_ir_curve_template_entry_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_set(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
...
void ir_curve_template_entry_commands::process_put_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_ir_curve_template_entries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::ir_curve_template_entry_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(
                change.write.ir_curve_config_id, parsed->positionals[next++], "ir_curve_config_id");
            read_token(change.write.sequence_index, parsed->positionals[next++], "sequence_index");
            read_token(
                change.write.start_tenor_code, parsed->positionals[next++], "start_tenor_code");
            read_token(change.write.end_tenor_code, parsed->positionals[next++], "end_tenor_code");
            read_token(
                change.write.instrument_code, parsed->positionals[next++], "instrument_code");
    ...
    auto result = do_auth_request<messaging::put_many_ir_curve_template_entries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_delete(std::ostream& out,
                                                      nats_client& session,
                                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_ir_curve_template_entry_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_ir_curve_template_entry_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_delete_many(std::ostream& out,
                                                           nats_client& session,
                                                           const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_ir_curve_template_entries_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(
                messaging::ir_curve_template_entry_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_ir_curve_template_entries_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_versions(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_ir_curve_template_entry_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_ir_curve_template_entry_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void ir_curve_template_entry_commands::process_version(std::ostream& out,
                                                       nats_client& session,
                                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_ir_curve_template_entry_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_ir_curve_template_entry_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
