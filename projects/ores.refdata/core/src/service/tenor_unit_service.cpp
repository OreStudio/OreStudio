 ...
 */
std::vector<domain::tenor_unit> read_one(repository::tenor_unit_repository& repo,
                                         const ores::database::context& ctx,
                                         const messaging::tenor_unit_key& key) {
 ...
 */
messaging::tenor_unit_key key_from(const domain::tenor_unit& v) {
    messaging::tenor_unit_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::tenor_unit to_domain(const messaging::tenor_unit_write& write) {
    domain::tenor_unit v;
    v.code = write.code;
    v.name = write.name;
...
tenor_unit_service::get_tenor_unit(const messaging::get_tenor_unit_request& request) {
    messaging::get_tenor_unit_response response;
    ...
    response.tenor_unit = std::move(found.front());
    return response;
}

messaging::get_many_tenor_units_response
tenor_unit_service::get_many_tenor_units(const messaging::get_many_tenor_units_request& request) {
    messaging::get_many_tenor_units_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenor_unit_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenor_unit = std::move(found.front());
...
tenor_unit_service::put_tenor_unit(const messaging::put_tenor_unit_request& request) {
    messaging::put_tenor_unit_response response;
    domain::tenor_unit value;
    ...
    if (!written.empty())
        response.tenor_unit = std::move(written.front());
    return response;
}

messaging::put_many_tenor_units_response
tenor_unit_service::put_many_tenor_units(const messaging::put_many_tenor_units_request& request) {
    messaging::put_many_tenor_units_response response;
    ...
    response.units.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.units.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenor_unit_response
tenor_unit_service::delete_tenor_unit(const messaging::delete_tenor_unit_request& request) {
    messaging::delete_tenor_unit_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::tenor_unit_repository::remove_status::removed:
            break;
        case repository::tenor_unit_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenor_unit_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenor_unit_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_tenor_units_response tenor_unit_service::delete_many_tenor_units(
    const messaging::delete_many_tenor_units_request& request) {
    messaging::delete_many_tenor_units_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_tenor_unit_versions_response tenor_unit_service::list_tenor_unit_versions(
    const messaging::list_tenor_unit_versions_request& request) {
    messaging::list_tenor_unit_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_tenor_unit_version_response tenor_unit_service::get_tenor_unit_version(
    const messaging::get_tenor_unit_version_request& request) {
    messaging::get_tenor_unit_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.tenor_unit.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tenor_unit_service::prepare_change(const messaging::tenor_unit_change& change,
...
tenor_unit_service::get_units(const std::vector<std::string>& codes) {
