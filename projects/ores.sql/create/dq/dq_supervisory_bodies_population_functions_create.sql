 ...
 *   SELECT * FROM metadata.dq_preview_supervisory_body_population_fn(dataset_id);
 *
 ...
 */
create or replace function metadata.dq_preview_supervisory_body_population_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_supervisory_bodies_artefact_tbl dq
    left join production.refdata_supervisory_bodies_tbl existing
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_supervisory_bodies_tbl existing
        ...
        insert into production.refdata_supervisory_bodies_tbl (
