...
#include "ores.refdata.core/repository/purpose_type_mapper.hpp"
...
purpose_type_repository::replace_claim(context ctx, const domain::purpose_type& v) {
...
domain::purpose_type purpose_type_repository::apply_claim(
    context ctx, const domain::purpose_type& v, const ores::utility::domain::precondition& claim) {
...
void purpose_type_repository::write(context ctx,
                                    const domain::purpose_type& v,
                                    const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing purpose type. " << "code: " << v.code;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(
        ctx, purpose_type_mapper::map(t), lg(), "Writing purpose type to database.");
}

void purpose_type_repository::write(
    context ctx,
    const std::vector<domain::purpose_type>& v,
    ...
    execute_write_query(
        ctx, purpose_type_mapper::map(batch), lg(), "Writing purpose types to database.");
}
...
std::vector<domain::purpose_type>
purpose_type_repository::read_latest(context ctx, const std::vector<std::string>& codes) {
    if (codes.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    auto result = execute_read_query<purpose_type_entity, domain::purpose_type>(
        ctx,
        query,
        [](const auto& entities) { return purpose_type_mapper::map(entities); },
        lg(),
        "Reading latest purpose types by ids.");
    return result;
}
