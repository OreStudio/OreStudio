 ...
 */
std::vector<domain::party> read_one(repository::party_repository& repo,
                                    const ores::database::context& ctx,
                                    const messaging::party_key& key) {
 ...
 */
messaging::party_key key_from(const domain::party& v) {
 ...
 */
domain::party to_domain(const messaging::party_write& write) {
    domain::party v;
    v.id = write.id;
    v.short_code = write.short_code;
    v.full_name = write.full_name;
    v.codename = write.codename;
    v.transliterated_name = write.transliterated_name;
    v.party_category = write.party_category;
    v.party_type = write.party_type;
    v.parent_party_id = write.parent_party_id;
...
party_service::list_parties(const messaging::list_parties_request& request) {
    messaging::list_parties_response response;
    ...
    response.parties = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_party_count(ctx_);
    return response;
}

messaging::get_party_response
party_service::get_party(const messaging::get_party_request& request) {
    messaging::get_party_response response;
    ...
    response.party = std::move(found.front());
    return response;
}

messaging::get_many_parties_response
party_service::get_many_parties(const messaging::get_many_parties_request& request) {
    messaging::get_many_parties_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::party_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.party = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_party_response
party_service::put_party(const messaging::put_party_request& request) {
    messaging::put_party_response response;
    domain::party value;
    ...
    if (!written.empty())
        response.party = std::move(written.front());
    return response;
}

messaging::put_many_parties_response
party_service::put_many_parties(const messaging::put_many_parties_request& request) {
    messaging::put_many_parties_response response;
    ...
    response.parties.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.parties.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_party_response
party_service::delete_party(const messaging::delete_party_request& request) {
    messaging::delete_party_response response;
    ...
    switch (repo_.remove(ctx_, boost::uuids::to_string(request.removal.key.id), expected)) {
        case repository::party_repository::remove_status::removed:
            break;
        case repository::party_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::party_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::party_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
party_service::delete_many_parties(const messaging::delete_many_parties_request& request) {
    messaging::delete_many_parties_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
party_service::list_party_versions(const messaging::list_party_versions_request& request) {
    messaging::list_party_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
party_service::get_party_version(const messaging::get_party_version_request& request) {
    messaging::get_party_version_response response;
    auto found = repo_.read_at_version(
        ctx_, boost::uuids::to_string(request.key.party.id), request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
party_service::prepare_change(const messaging::party_change& change,
...
std::vector<domain::party> party_service::get_parties(const std::vector<std::string>& ids) {
