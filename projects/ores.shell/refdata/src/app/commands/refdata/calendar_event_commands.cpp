...
void calendar_event_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("calendar_events");
    ...
    menu->Insert(
        ...
        "set <id> <calendar_code> <event_date> <diary_entry_type> <name> <description> <source> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <calendar_code> <event_date> <diary_entry_type> <name> "
        "<description> <source> <reason> <commentary>");
...
void calendar_event_commands::process_list(std::ostream& out,
    ...
    using request_type = messaging::list_calendar_events_request;
    ...
    auto result = do_auth_request<messaging::list_calendar_events_response>(
...
void calendar_event_commands::process_get(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_calendar_event_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_calendar_event_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_get_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_calendar_events_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::calendar_event_key key;
    ...
    auto result = do_auth_request<messaging::get_many_calendar_events_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_add(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_calendar_event_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.source, parsed->positionals[next++], "source");
    ...
    auto result = do_auth_request<messaging::put_calendar_event_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_set(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
...
void calendar_event_commands::process_put_many(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_calendar_events_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::calendar_event_change change;
            ...
            read_token(change.write.event_date, parsed->positionals[next++], "event_date");
            read_token(
            ...
            read_token(change.write.source, parsed->positionals[next++], "source");
    ...
    auto result = do_auth_request<messaging::put_many_calendar_events_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_delete(std::ostream& out,
                                             nats_client& session,
                                             const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_calendar_event_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_calendar_event_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_delete_many(std::ostream& out,
                                                  nats_client& session,
                                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_calendar_events_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::calendar_event_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_calendar_events_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_by_calendar_code(std::ostream& out,
                                                       nats_client& session,
                                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-calendar-code request.";

    using request_type = messaging::list_by_calendar_code_calendar_events_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_by_calendar_code_calendar_events_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_by_diary_entry_type(std::ostream& out,
                                                          nats_client& session,
                                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-diary-entry-type request.";

    using request_type = messaging::list_by_diary_entry_type_calendar_events_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        req.diary_entry_type = ores::shell::app::from_token<std::string>(
    ...
    auto result = do_auth_request<messaging::list_by_diary_entry_type_calendar_events_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_versions(std::ostream& out,
                                               nats_client& session,
                                               const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_calendar_event_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_calendar_event_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void calendar_event_commands::process_version(std::ostream& out,
                                              nats_client& session,
                                              const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_calendar_event_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_calendar_event_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
