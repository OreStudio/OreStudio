...
currency_asset_class_service::currency_asset_class_service(context ctx)
    : ctx_(std::move(ctx)) {}

std::vector<domain::currency_asset_class> currency_asset_class_service::list_types() {
    BOOST_LOG_SEV(lg(), debug) << "Listing all currency asset classes";
    return repo_.read_latest(ctx_);
}

std::optional<domain::currency_asset_class>
currency_asset_class_service::find_type(const std::string& code) {
...
void currency_asset_class_service::save_type(const domain::currency_asset_class& v) {
    if (v.code.empty())
...
void currency_asset_class_service::remove_type(const std::string& code) {
...
currency_asset_class_service::get_type_history(const std::string& code) {
