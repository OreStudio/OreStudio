...
#+description: Run the codegen-fix-drift method on ores.scheduler: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the drift gate to cover scheduler-cpp. Supersedes the ad hoc 'Apply safe drift to scheduler-cpp' task.
#+type: story
#+level: s2
...
Bring ores.scheduler to the state the codegen-fix-drift skill defines: every codegen-eligible entity bound to a profile or recorded as a genuine no-match, zero-diff regeneration verified, and any real gap the check surfaces fixed rather than skipped. Once trusted, scheduler-cpp joins the known-drift-free registry that the drift gate checks.
...
- scheduler-cpp joins the =KNOWN_DRIFT_FREE= registry in check_component_drift.py (skill step 8), and the pr-raise skill's local codegen-drift check runs =--all= over the registry, so future PRs touching ores.scheduler fail on any regeneration drift.
