...
create table if not exists "ores_refdata_calendar_events_tbl" (
    "id" uuid not null,
...
create unique index if not exists calendar_events_calendar_code_event_date_diary_entry_type_uniq_idx
on "ores_refdata_calendar_events_tbl" (tenant_id, calendar_code, event_date, diary_entry_type)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists calendar_events_version_uniq_idx
on "ores_refdata_calendar_events_tbl" (tenant_id, id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists calendar_events_id_uniq_idx
on "ores_refdata_calendar_events_tbl" (tenant_id, id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists calendar_events_tenant_idx
on "ores_refdata_calendar_events_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_calendar_events_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    -- Validate diary_entry_type (soft FK to ores_refdata_diary_entry_types_tbl)
    if not exists (
...
create or replace trigger ores_refdata_calendar_events_insert_trg
before insert on "ores_refdata_calendar_events_tbl"
for each row execute function ores_refdata_calendar_events_insert_fn();

create or replace rule ores_refdata_calendar_events_delete_rule as
on delete to "ores_refdata_calendar_events_tbl" do instead (
    update "ores_refdata_calendar_events_tbl"
    set valid_to = clock_timestamp()
