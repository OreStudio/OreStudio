...
boost::program_options::options_description database_configuration::make_options_description() {
    using boost::program_options::value;
    using boost::program_options::options_description;

    options_description r("Database");
    r.add_options()("db-user", value<std::string>(), "Database user name.")(
        "db-password",
        value<std::string>()->default_value(""),
        "Database password. Reads from ORES_<APP>_DB_PASSWORD env var "
        "where <APP> is the service mapper prefix (e.g. CLI, IAM_SERVICE).")(
        "db-host", value<std::string>()->default_value("localhost"), "Database host.")(
        "db-database",
        value<std::string>(),
        "Database name. Convention: ores_dev_<env> (e.g., ores_dev_local2).")(
        "db-port", value<int>()->default_value(5432), "Database port.")(
        "tenant",
        value<std::string>()->default_value(""),
...
database_options
database_configuration::read_options(const boost::program_options::variables_map& vm) {
    database_options r;
    ...
    if (!vm.count(database_database_arg))
        BOOST_THROW_EXCEPTION(
            std::runtime_error("db-database is required but not set. "
