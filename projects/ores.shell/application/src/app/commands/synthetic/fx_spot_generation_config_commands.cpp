...
#include "ores.synthetic.api/domain/fx_spot_generation_config_table_io.hpp" // IWYU pragma: keep.
...
void fx_spot_generation_config_commands::process_get_fx_spot_generation_configs(
    ...
    auto result = do_auth_request<synthetic::messaging::get_fx_spot_generation_configs_response>(
        out, session, "synthetic.v1.fx_spot_generation_configs.list", req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_add_fx_spot_generation_config(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add a fx_spot_generation_config." << std::endl;
        return;
    }
    ...
    v.base_currency_code = std::move(s_base_currency_code);

    // quote_currency_code (str).
    v.quote_currency_code = std::move(s_quote_currency_code);

    // source_name (str).
    v.source_name = std::move(s_source_name);

    // ore_key (str).
    v.ore_key = std::move(s_ore_key);

    // price_source (str).
    v.price_source = std::move(s_price_source);

    // gmm_initial_price (double).
    double n_gmm_initial_price = 0;
    try {
        n_gmm_initial_price = std::stod(s_gmm_initial_price);
    } catch (const std::exception&) {
        fail(out) << "Invalid gmm_initial_price: " << s_gmm_initial_price << std::endl;
        return;
    }
    v.gmm_initial_price = n_gmm_initial_price;
    ...
    auto req = synthetic::messaging::save_fx_spot_generation_config_request::from(std::move(v));

    auto result = do_auth_request<synthetic::messaging::save_fx_spot_generation_config_response>(
        out, session, "synthetic.v1.fx_spot_generation_configs.save", req);
    if (!result)
        return;

    if (result->success) {
        BOOST_LOG_SEV(lg(), info) << "Successfully added fx_spot_generation_config.";
        out << "✓ Fx Spot Generation Config added successfully!" << std::endl;
    } else {
        const auto& msg = result->message.empty() ? "Unknown error" : result->message;
        BOOST_LOG_SEV(lg(), warn) << "Failed to add fx_spot_generation_config: " << msg;
        fail(out) << "Failed to add fx_spot_generation_config: " << msg << std::endl;
    }
}
...
void fx_spot_generation_config_commands::process_delete_fx_spot_generation_config(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete a fx_spot_generation_config." << std::endl;
        return;
    }

    synthetic::messaging::delete_fx_spot_generation_config_request req;
    req.ids = {std::move(id)};

    auto result = do_auth_request<synthetic::messaging::delete_fx_spot_generation_config_response>(
        out, session, "synthetic.v1.fx_spot_generation_configs.delete", req);
    if (!result)
        return;
...
void fx_spot_generation_config_commands::process_get_fx_spot_generation_config_history(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get fx_spot_generation_config history for: " << key;

    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to get fx_spot_generation_config history." << std::endl;
        return;
    }
    ...
    if (result->history.empty()) {
        out << "No history found for this fx_spot_generation_config." << std::endl;
        return;
    }
