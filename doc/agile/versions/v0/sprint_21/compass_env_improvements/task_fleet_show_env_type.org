...
Merged as PR #1292. Added `_worktree_env_value` helper to `compass.py` and wired it into `cmd_fleet` to read `ORES_PROVISION_TYPE` from each worktree's `.env`. Fleet output now shows `[full]` or `[light]` beside the worktree name; legacy worktrees without the field show no label.
