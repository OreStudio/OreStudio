...
void print_help(const options_description& od, std::ostream& info) {
    info << "Workspace service" << std::endl
         << std::endl
         << "Usage: ores.workspace.service [options]" << std::endl
         << std::endl
         << od << std::endl;
}
...
std::optional<options> parse_arguments(const std::vector<std::string>& arguments,
    ...
    const auto od(standard_service_options::make_options_description("ores.workspace.service.log"));
    const auto vm(standard_service_options::parse(od, arguments, "WORKSPACE_SERVICE"));
