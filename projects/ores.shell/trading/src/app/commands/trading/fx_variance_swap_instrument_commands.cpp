...
void fx_variance_swap_instrument_commands::register_commands(cli::Menu& root_menu,
    ...
    fx_variance_swap_instruments_menu->Insert(
        ...
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_add_fx_variance_swap_instrument(std::ref(out), std::ref(session), args);
        },
        "Add FX variance swap instrument (trade_type_code "
        "party_id "
        "trade_id "
        "start_date "
        "end_date "
    ...
    fx_variance_swap_instruments_menu->Insert(
        ...
        "Delete FX variance swap instrument by instrument_id");
    ...
    fx_variance_swap_instruments_menu->Insert(
        ...
        "Show FX variance swap instrument's version history");
...
void fx_variance_swap_instrument_commands::process_get_fx_variance_swap_instruments(
    ...
    BOOST_LOG_SEV(lg(), debug) << "Initiating get FX variance swap instrument request.";
...
void fx_variance_swap_instrument_commands::process_add_fx_variance_swap_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to add FX variance swap instrument." << std::endl;
        return;
    }
    ...
    try {
        ...
        v.end_date =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "end_date");
        v.currency =
        ...
        v.moment_type =
            ores::shell::app::from_token<std::string>(parsed->positionals[next++], "moment_type");
        v.description =
...
void fx_variance_swap_instrument_commands::process_delete_fx_variance_swap_instrument(
    ...
    if (!session.is_logged_in()) {
        fail(out) << "You must be logged in to delete FX variance swap instrument." << std::endl;
        return;
    }
