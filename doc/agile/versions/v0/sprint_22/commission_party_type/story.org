...
#+type: story
#+level: s2
#+filetags: :product:refdata:commissioning:party_type:sprint_22:v0:
#+created: 2026-05-29
#+updated: 2026-06-25
...
| [[id:21E5D45B-9CAD-4146-8ECF-1ACDB6398508][Verify party_type Qt UI end-to-end post-NATS]] | DONE | 2026-07-02 | 2026-07-04 | Unblocked (root cause was a legacy env's NATS port collision, not memory corruption; see PR #1415); verify the Qt list window, detail dialog, history dialog, delete, and eventing for party_type against a live environment post-NATS migration; fix any regressions found. |
...
Shell/CLI implementation, the manual chapter, and Wt/HTTP gap captures
were explicitly abandoned as out of scope for this sprint's
commissioning pass (see the corresponding ABANDONED tasks) — they
remain open scope for a future sprint, not silently dropped.

* Result

Qt UI acceptance criteria fully verified end-to-end on a live
environment (fresh Barclays Plc tenant, full 24-service fleet): list,
detail edit/save, and eventing all confirmed working. Along the way,
this story surfaced and fixed two real bugs unrelated to party_type
itself — a NATS port collision from a stray legacy environment's
server (PR #1415), and a genuine eventing gap caused by duplicated Qt
controller classes across =ores.qt.party= and =ores.qt.refdata=
following an earlier, incomplete Qt codegen migration. Both are
documented in the =Verify party_type Qt UI= task, with two follow-up
backlog captures filed for their broader scope (the =application.cpp=
eventing-registration gap affecting ~14 other =ores.refdata= entities;
dissolving =ores.qt.party= into =ores.qt.refdata= since every entity in
it has backend component=refdata).
