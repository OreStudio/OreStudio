...
- An automated diagram pass is a skeleton, so the manual pass is not
  optional polish. The pass reads data members only, which means every
  interface and every static facade arrives as an empty box, and free
  functions do not arrive at all. The first =ores.platform= diagram passed
  a refresh check, a render and a file review, and still showed an
