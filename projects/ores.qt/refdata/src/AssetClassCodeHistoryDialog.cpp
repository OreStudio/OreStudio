...
#include "ui_AssetClassCodeHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

AssetClassCodeHistoryDialog::AssetClassCodeHistoryDialog(const QString& code,
                                                         ClientManager* clientManager,
                                                         QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::AssetClassCodeHistoryDialog)
    , code_(code)
...
AssetClassCodeHistoryDialog::~AssetClassCodeHistoryDialog() {
    delete ui_;
}

QString AssetClassCodeHistoryDialog::code() const {
    return code_;
}

void AssetClassCodeHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for asset class code: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_asset_class_code_history_request request;
    request.code = code_.toStdString();

    QPointer<AssetClassCodeHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_asset_class_code_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int AssetClassCodeHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString AssetClassCodeHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow AssetClassCodeHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult AssetClassCodeHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void AssetClassCodeHistoryDialog::displayFullDetails(int index) {
...
void AssetClassCodeHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void AssetClassCodeHistoryDialog::revertToVersionAt(int index) {
