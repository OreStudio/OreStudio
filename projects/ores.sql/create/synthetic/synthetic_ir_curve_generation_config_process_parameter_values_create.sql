 ...
 *
 * IR Curve Generation Config Process Parameter Value Table
 *
...
begin
    ...
    -- Validate config_id (soft FK to ores_synthetic_ir_curve_generation_configs_tbl)
    if not exists (
    ...
    -- Validate config_id
    NEW.config_id := ores_synthetic_validate_ir_curve_generation_config_fn(NEW.tenant_id, NEW.config_id);

    -- Validate parameter_definition_id
    NEW.parameter_definition_id := ores_synthetic_validate_yield_curve_process_parameter_definition_fn(NEW.tenant_id, NEW.parameter_definition_id);
...
-- =============================================================================
-- Validation function for ir_curve_generation_config_process_parameter_value
-- Validates that a id exists in the ir_curve_generation_config_process_parameter_values table.
...
-- =============================================================================
create or replace function ores_synthetic_validate_ir_curve_generation_config_process_parameter_value_fn(
    p_tenant_id uuid,
    p_value uuid
) returns uuid as $$
begin
    ...
    if p_value is null then
        raise exception 'Invalid ir_curve_generation_config_process_parameter_value: value cannot be null'
            using errcode = '23502';
    end if;
