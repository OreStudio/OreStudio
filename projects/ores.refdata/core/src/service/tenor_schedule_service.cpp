...
std::vector<domain::tenor_schedule> read_one(repository::tenor_schedule_repository& repo,
                                             ...
                                             const messaging::tenor_schedule_key& key) {
 ...
 */
messaging::tenor_schedule_key key_from(const domain::tenor_schedule& v) {
 ...
 */
domain::tenor_schedule to_domain(const messaging::tenor_schedule_write& write) {
    domain::tenor_schedule v;
    v.code = write.code;
    v.name = write.name;
    v.description = write.description;
    v.display_order = write.display_order;
    v.schedule_source = write.schedule_source;
    v.calendar_code = write.calendar_code;
    v.diary_entry_type = write.diary_entry_type;
    return v;
}
...
messaging::list_by_calendar_code_tenor_schedules_response
...
tenor_schedule_service::get_tenor_schedule(const messaging::get_tenor_schedule_request& request) {
    messaging::get_tenor_schedule_response response;
    ...
    response.tenor_schedule = std::move(found.front());
    return response;
}

messaging::get_many_tenor_schedules_response tenor_schedule_service::get_many_tenor_schedules(
    const messaging::get_many_tenor_schedules_request& request) {
    messaging::get_many_tenor_schedules_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::tenor_schedule_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.tenor_schedule = std::move(found.front());
...
tenor_schedule_service::put_tenor_schedule(const messaging::put_tenor_schedule_request& request) {
    messaging::put_tenor_schedule_response response;
    domain::tenor_schedule value;
    ...
    if (!written.empty())
        response.tenor_schedule = std::move(written.front());
    return response;
}

messaging::put_many_tenor_schedules_response tenor_schedule_service::put_many_tenor_schedules(
    const messaging::put_many_tenor_schedules_request& request) {
    messaging::put_many_tenor_schedules_response response;
    std::vector<domain::tenor_schedule> batch;
    ...
    response.schedules.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.schedules.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_tenor_schedule_response tenor_schedule_service::delete_tenor_schedule(
    const messaging::delete_tenor_schedule_request& request) {
    messaging::delete_tenor_schedule_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::tenor_schedule_repository::remove_status::removed:
            break;
        case repository::tenor_schedule_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::tenor_schedule_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::tenor_schedule_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_tenor_schedules_response tenor_schedule_service::delete_many_tenor_schedules(
    const messaging::delete_many_tenor_schedules_request& request) {
    messaging::delete_many_tenor_schedules_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
tenor_schedule_service::list_tenor_schedule_versions(
    const messaging::list_tenor_schedule_versions_request& request) {
    messaging::list_tenor_schedule_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_tenor_schedule_version_response tenor_schedule_service::get_tenor_schedule_version(
    const messaging::get_tenor_schedule_version_request& request) {
    messaging::get_tenor_schedule_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.tenor_schedule.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
tenor_schedule_service::prepare_change(const messaging::tenor_schedule_change& change,
...
tenor_schedule_service::get_schedules(const std::vector<std::string>& codes) {
