...
| [[id:21306CD6-8B9D-4119-A735-2B7B527EDDC3][Synthetic market data config UI]] | BACKLOG | | | Synthetic-specific Qt UI to create/edit/list the persisted synthetic configs (container per party + FX spot sub-config), scoped to the logged-in tenant/party. Config authoring only; depends on the foundation task. |
...
| [[id:58DEC5DA-5B24-4298-9A29-5A9CC2DA7283][Named feed sources: persisted configs + observation provenance]] | ABANDONED | | 2026-06-28 | Folded into the foundation task (Synthetic market data config — container + FX spot sub-config); the persisted config carries the source name and the service stamps it. |
| [[id:A34E4046-2E97-478B-800C-70EAC8F1D526][Synthetic market data config — container + FX spot sub-config (foundation)]] | STARTED | 2026-06-28 | | Persisted, party/tenant-scoped synthetic config in ores.synthetic: a container with typed sub-configs (FX spot now). The service generates from it, stamping the config's tenant + source name. Subsumes the business-tenant fix and named-sources/provenance. |
| [[id:AC3A8C5C-CBED-4FDC-8FDB-26088EAEB01F][Market data control panel (source-agnostic)]] | BACKLOG | | | A generic market data control panel to start/stop/monitor market data feeds, independent of what is behind each feed (synthetic, Bloomberg, ...). Distinct from the synthetic config UI; lives at the market-data level and expands to other market-data types and sources over time. |
