...
| H01 (diagrams) | The auto half is done and the manual half is not. The generator could not resolve a composite part, which is why every part diagram in the repository rots; it now maps a dotted name to its directory, with a regression test in the codegen suite (527 tests green). The api diagram grew from 22 to 35 classes and the core diagram from 22 to 42, and both images are re-rendered with the CMake flags. The manual section below the sentinel is still empty in all three: the two-pass authoring the audit describes has never been done for this component. | The generator change and its test; the two puml files and their pngs; =git diff --stat= |
