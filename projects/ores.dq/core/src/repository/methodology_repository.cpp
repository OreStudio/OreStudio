...
void methodology_repository::write(const domain::methodology& methodology) {
    BOOST_LOG_SEV(lg(), debug) << "Writing methodology to database: " << methodology.id;
    execute_write_query(
        ctx_, methodology_mapper::map(methodology), lg(), "writing methodology to database");
}

void methodology_repository::write(const std::vector<domain::methodology>& methodologies) {
    BOOST_LOG_SEV(lg(), debug) << "Writing methodologies to database. Count: "
                               << methodologies.size();
    execute_write_query(
        ctx_, methodology_mapper::map(methodologies), lg(), "writing methodologies to database");
}

std::vector<domain::methodology> methodology_repository::read_latest() {
...
std::vector<domain::methodology> methodology_repository::read_latest(const boost::uuids::uuid& id) {
...
std::vector<domain::methodology> methodology_repository::read_latest(std::uint32_t offset,
                                                                     std::uint32_t limit) {
...
std::vector<domain::methodology> methodology_repository::read_all(const boost::uuids::uuid& id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all methodology versions. Id: " << id;

    const auto id_str = boost::uuids::to_string(id);
    const auto query = sqlgen::read<std::vector<methodology_entity>> | where("id"_c == id_str) |
...
void methodology_repository::remove(const boost::uuids::uuid& id) {
    ...
    const auto id_str = boost::uuids::to_string(id);
    const auto query = sqlgen::delete_from<methodology_entity> | where("id"_c == id_str);
