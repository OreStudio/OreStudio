...
([[proj:projects/ores.dq/api/src/CMakeLists.txt][=projects/ores.dq/api/src/CMakeLists.txt=]]), which defeats the entire
...
One point-in-time record, written while working the task. It says what the tree looked like and what was decided on the day; it is not the description of the contract. Read [[id:F27E5DF7-6223-4B6F-80A5-CEFBEB1BC756][Component architecture]] for that.

| Investigation | Question it answered |
|---------------+----------------------|
| [[id:D16238D8-3F4A-4F45-97BC-4F6618601313][API/core split inventory]] | What does every split component keep in =api= versus =core=, what does each =api= link, and where has the lightweight-api contract eroded? |
