...
{{/domain_entity.sql.hypertable}}
{{/domain_entity.has_tenant_in_pk}}
{{! A text key column normally rejects the empty string. A model whose column gives the empty string a meaning of its own -- a wildcard metric, say -- declares :skip_empty_check: on that column to suppress the check. }}
    check ("valid_from" < "valid_to"){{#domain_entity.primary_key.columns}}{{#is_uuid}}{{^skip_uuid_check}},
    check ("{{column}}" <> {{uuid_check_fn}}){{/skip_uuid_check}}{{/is_uuid}}{{#is_text}}{{^skip_empty_check}},
    check ("{{column}}" <> ''){{/skip_empty_check}}{{/is_text}}{{/domain_entity.primary_key.columns}}{{#domain_entity.sql.extra_checks}},
    check ({{{.}}}){{/domain_entity.sql.extra_checks}}
{{/domain_entity.current_state}}
{{#domain_entity.current_state}}
    primary key ({{domain_entity.primary_key.column_list}}){{#domain_entity.primary_key.columns}}{{#is_uuid}}{{^skip_uuid_check}},
    check ("{{column}}" <> {{uuid_check_fn}}){{/skip_uuid_check}}{{/is_uuid}}{{#is_text}}{{^skip_empty_check}},
    check ("{{column}}" <> ''){{/skip_empty_check}}{{/is_text}}{{/domain_entity.primary_key.columns}}{{#domain_entity.sql.extra_checks}},
    check ({{{.}}}){{/domain_entity.sql.extra_checks}}
{{/domain_entity.current_state}}
...
{{#domain_entity.has_tenant_id}}
{{! A nullable tenant column makes NULL a tenant of its own: the system scope.
    PostgreSQL treats NULLs as distinct in a unique index, so without this clause
    two system rows could carry the same natural key. }}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{domain_entity.natural_keys_composite_name}}_uniq_idx
on "{{domain_entity.sql_name_base}}_tbl" (tenant_id, {{domain_entity.natural_keys_composite_columns}}){{#domain_entity.sql.nullable_tenant_id}} nulls not distinct{{/domain_entity.sql.nullable_tenant_id}}
{{^domain_entity.current_state}}where valid_to = ores_utility_infinity_timestamp_fn(){{/domain_entity.current_state}};
{{/domain_entity.has_tenant_id}}
{{^domain_entity.has_tenant_id}}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{domain_entity.natural_keys_composite_name}}_uniq_idx
...
{{! The single-natural-key branch below scopes the index on has_tenant_in_pk
    OR nullable_tenant_id, while the composite branch above scopes on
    has_tenant_id. The two agree for every entity that exists today: a
    nullable-tenant entity is the only shape that has a tenant, keeps it out of
    the primary key, and carries a single natural key. They would disagree for
    a current_state or system_scope entity with one natural key, which would get
    an unscoped index where its composite sibling would scope it. Read the two
    branches together before changing either. }}
...
{{! A nullable tenant column makes NULL a tenant of its own: the system scope.
    PostgreSQL treats NULLs as distinct in a unique index, so without this clause
    two system rows could carry the same natural key, which the tenant-scoped
    index is meant to prevent. }}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{column}}_uniq_idx
on "{{domain_entity.sql_name_base}}_tbl" (tenant_id, {{column}}) nulls not distinct
{{^domain_entity.current_state}}where valid_to = ores_utility_infinity_timestamp_fn(){{/domain_entity.current_state}};
{{/domain_entity.sql.nullable_tenant_id}}
{{^domain_entity.sql.nullable_tenant_id}}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{column}}_uniq_idx
...
{{#domain_entity.has_tenant_in_pk}}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{column}}_uniq_idx
on "{{domain_entity.sql_name_base}}_tbl" (tenant_id, {{column}})
{{^domain_entity.current_state}}where valid_to = ores_utility_infinity_timestamp_fn(){{/domain_entity.current_state}};
{{/domain_entity.has_tenant_in_pk}}
{{^domain_entity.has_tenant_in_pk}}
create unique index if not exists {{domain_entity.index_name_prefix}}_{{column}}_uniq_idx
...
{{/domain_entity.sql.has_cardinality_limit_validations}}
<<paste:B9428A09-3610-4CAD-A200-E6CDE53B7C8D>>
begin
{{#domain_entity.validates_tenant_on_insert}}
    -- Validate tenant_id
    NEW.tenant_id := ores_iam_validate_tenant_fn(NEW.tenant_id);
...
{{#domain_entity.sql.hypertable_retention_interval}}
        ...
        begin
            ...
            if current_license = 'timescale' then
                perform public.add_retention_policy(
                    '"{{domain_entity.sql_name_base}}_tbl"',
                    drop_after => interval '{{domain_entity.sql.hypertable_retention_interval}}',
                    if_not_exists => true
                );
            else
                raise notice 'TimescaleDB community edition - no retention policy is set for {{domain_entity.sql_name_base}}_tbl';
            end if;
        end;
{{/domain_entity.sql.hypertable_retention_interval}}
    else
...
{{#domain_entity.sql.rls_tenant_isolation}}

-- =============================================================================
-- Row-level security: tenant isolation for {{domain_entity.entity_title}}
{{#domain_entity.sql.rls_system_tenant_visible}}
-- System-tenant sessions may also read every tenant's rows.
{{/domain_entity.sql.rls_system_tenant_visible}}
-- =============================================================================
alter table {{domain_entity.sql_name_base}}_tbl enable row level security;

create policy {{domain_entity.rls_table_base}}_tbl_tenant_isolation_policy
on {{domain_entity.sql_name_base}}_tbl
for all using (
    tenant_id = ores_iam_current_tenant_id_fn(){{#domain_entity.sql.rls_system_tenant_visible}}
    OR ores_iam_current_tenant_id_fn() = ores_utility_system_tenant_id_fn(){{/domain_entity.sql.rls_system_tenant_visible}}
)
with check (
    tenant_id = ores_iam_current_tenant_id_fn(){{#domain_entity.sql.rls_system_tenant_visible}}
    OR ores_iam_current_tenant_id_fn() = ores_utility_system_tenant_id_fn(){{/domain_entity.sql.rls_system_tenant_visible}}
);
{{#domain_entity.sql.rls_party_isolation}}
...
-- contexts).
create policy {{domain_entity.rls_table_base}}_tbl_party_isolation_policy
on {{domain_entity.sql_name_base}}_tbl
as restrictive
for all using (
