...
options_description make_options_description() {
    ...
    r.add(logging_configuration::make_options_description("ores.assets.service.log"));
...
void print_help(const options_description& od, std::ostream& info) {
    info << "Asset management service" << std::endl << std::endl
         << "Usage: ores.assets.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(environment_mapper_factory::make_mapper("ASSETS_SERVICE"));
