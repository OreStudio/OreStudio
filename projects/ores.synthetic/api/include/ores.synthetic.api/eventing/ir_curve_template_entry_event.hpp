 ...
 */
#ifndef ORES_SYNTHETIC_API_EVENTING_IR_CURVE_TEMPLATE_ENTRY_EVENT_HPP
#define ORES_SYNTHETIC_API_EVENTING_IR_CURVE_TEMPLATE_ENTRY_EVENT_HPP
...
/**
 * @brief Event traits for ir curve template entry's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::synthetic::messaging::ir_curve_template_entry_event> {
    static constexpr std::string_view subject_prefix =
        "synthetic.v1.ir_curve_template_entries_events";

    static ores::synthetic::messaging::ir_curve_template_entry_event
    from_notification(const entity_event_notification& notification) {
        ores::synthetic::messaging::ir_curve_template_entry_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key =
                rfl::json::read<ores::synthetic::messaging::ir_curve_template_entry_key>(
                    notification.key))
            event.key = *key;
