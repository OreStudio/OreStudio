...
(story: [[id:9023BDE0-979F-422F-BC73-985D6A80E7CB][Retire legacy codegen profile system]]). The =--profile=-based
example, the flat "Profile catalogue" table, and the "Profile
activation" CLI invocation below describe that retired system's shape
and are kept for historical continuity pending a fuller rewrite of
this document to the current =ores.*=/=--address= element-scope model
described in [[*Element-scope activation — the supported/target set model][Element-scope activation]] below, which has already
superseded =--profile= as the primary mechanism.
...
its last state — see the historical note above:
