...
int ClientDayCountFractionTypeModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientDayCountFractionTypeModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh day count fraction type model: disconnected.";
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                ...
                BOOST_LOG_SEV(lg(), debug) << "Fetched " << result->types.size()
                                           << " day count fraction types, total available: "
...
void ClientDayCountFractionTypeModel::onTypesLoaded() {
    is_fetching_ = false;
    ...
    if (new_count > 0) {
        ...
        if (has_recent && !pulseManager_->is_pulsing()) {
            ...
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " day count fraction types newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " day count fraction types."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientDayCountFractionTypeModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientDayCountFractionTypeModel::onPulsingComplete() {
