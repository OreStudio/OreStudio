...
create or replace function ores_compute_platforms_notify_fn()
returns trigger as $$
declare
...
begin
    ...
    perform pg_notify('ores_compute_platforms', notification_payload::text);

    return null;
end;
$$ language plpgsql;

create or replace trigger ores_compute_platforms_notify_trg
after insert or update or delete on ores_compute_platforms_tbl
for each row execute function ores_compute_platforms_notify_fn();
