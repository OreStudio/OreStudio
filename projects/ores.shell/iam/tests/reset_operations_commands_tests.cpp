...
using ores::shell::app::commands::reset_operations_commands;
...
TEST_CASE("reset_operations_registers_every_declared_command", tags) {
...
TEST_CASE("reset_operations_process_reset_tenant_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    reset_operations_commands::process_reset_tenant(out, session, tokens(1));
...
TEST_CASE("reset_operations_process_reset_tenant_reports_the_expected_count", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    reset_operations_commands::process_reset_tenant(out, session, {});
...
TEST_CASE("reset_operations_process_reset_tenant_reaches_the_transport", tags) {
    auto lg(make_logger(test_suite));
...
TEST_CASE("reset_operations_process_reset_system_requires_a_session", tags) {
    auto lg(make_logger(test_suite));
    ...
    command_feedback::reset();
    reset_operations_commands::process_reset_system(out, session, tokens(0));
...
TEST_CASE("reset_operations_process_reset_system_reaches_the_transport", tags) {
    auto lg(make_logger(test_suite));
