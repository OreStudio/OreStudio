 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_CURRENCY_PAIR_CONVENTION_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_CURRENCY_PAIR_CONVENTION_REPOSITORY_HPP
...
/**
 * @brief Reads and writes currency pair conventions to data storage.
 */
class ORES_REFDATA_CORE_EXPORT currency_pair_convention_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.currency_pair_convention_repository";
...
public:
    ...
    /**
     * @brief Writes currency pair conventions to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::currency_pair_convention& v);
    void write(context ctx, const std::vector<domain::currency_pair_convention>& v);
    /**@}*/

    /**
     * @brief Writes a currency pair convention, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of currency pair conventions, each honouring its own
    ...
    /**
     * @brief Reads latest currency pair conventions, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::currency_pair_convention> read_latest(context ctx);
    std::vector<domain::currency_pair_convention> read_latest(context ctx,
                                                              const std::string& pair_code);
    std::vector<domain::currency_pair_convention>
    read_latest(context ctx, const std::vector<std::string>& pair_codes);
    /**@}*/


    /**
     * @brief Reads all currency pair conventions, possibly filtered by primary key.
     */
    std::vector<domain::currency_pair_convention> read_all(context ctx,
                                                           const std::string& pair_code);

    /**
     * @brief Reads a single currency pair convention as it stood at a specific
     ...
     */
    std::uint32_t get_total_convention_count(context ctx);

    /**
     * @brief Deletes a currency pair convention by closing its temporal validity.
     */
    void remove(context ctx, const std::string& pair_code);
    ...
    /**
     * @brief Removes a currency pair convention, refusing a row that moved on.
     *
     ...
     */
    remove_status
    remove(context ctx, const std::string& pair_code, std::optional<std::uint32_t> version);

    /**
     * @brief Deletes currency pair conventions by closing their temporal validity.
     */
    void remove(context ctx, const std::vector<std::string>& pair_codes);
...
private:
     ...
     */
    domain::currency_pair_convention apply_claim(context ctx,
                                                 const domain::currency_pair_convention& v,
                                                 const ores::utility::domain::precondition& claim);
};
