...
SwapConventionMdiWindow::SwapConventionMdiWindow(ClientManager* clientManager,
...
void SwapConventionMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &SwapConventionMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &SwapConventionMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &SwapConventionMdiWindow::deleteSelected);
    ...
    connect(
        historyAction_, &QAction::triggered, this, &SwapConventionMdiWindow::viewHistorySelected);
}
...
void SwapConventionMdiWindow::setupConnections() {
    connect(model_,
            &ClientSwapConventionModel::dataLoaded,
            this,
            &SwapConventionMdiWindow::onDataLoaded);
    connect(
        model_, &ClientSwapConventionModel::loadError, this, &SwapConventionMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &SwapConventionMdiWindow::onSelectionChanged);
    connect(
        tableView_, &QTableView::doubleClicked, this, &SwapConventionMdiWindow::onDoubleClicked);
...
void SwapConventionMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void SwapConventionMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* sc = model_->getConvention(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showConventionHistory for code: " << sc->id;
...
void SwapConventionMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete swap convention while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " swap conventions";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 swap conventions?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Swap Convention", confirmMessage, QMessageBox::Yes | QMessageBox::No);
        ...
        for (const auto& [code, result] : results) {
            if (result.first) {
                ...
                BOOST_LOG_SEV(lg(), error)
                    << "Swap Convention deletion failed: " << code << " - " << result.second;
                failure_count++;
