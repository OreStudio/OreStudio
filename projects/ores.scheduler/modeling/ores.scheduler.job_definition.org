...
#+description: Codegen entity model for the scheduler =job_definition= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
before any facet of it can be regenerated safely. The deleted Qt
templates were excluded for the same reason: their generated
controllers/dialogs bound to the hand-maintained
=scheduler_protocol.hpp= classes, so a regen would have emitted code
against a protocol surface that does not exist. Mirrors
...
- =job_definition_handler.hpp= hand-implements =schedule=/
  ...
  not just by =registrar.cpp= and the removed desktop client's UI
  classes (=JobDefinitionController=/=JobDefinitionMdiWindow=/
  =JobDefinitionDetailDialog=/=JobDefinitionAuditDialog=), but
  cross-component by =ores.reporting='s =report_scheduling_service=.
