...
void fx_accumulator_instrument_repository::write(context ctx,
    ...
    BOOST_LOG_SEV(lg(), debug) << "Writing FX accumulator instrument. "
                               << "instrument_id: " << v.identity.instrument_id;
    execute_write_query(ctx,
                        fx_accumulator_instrument_mapper::map(v),
                        lg(),
                        "Writing FX accumulator instrument to database.");
}
...
void fx_accumulator_instrument_repository::write(
    ...
    execute_write_query(ctx,
                        fx_accumulator_instrument_mapper::map(v),
                        lg(),
                        "Writing FX accumulator instruments to database.");
}
...
fx_accumulator_instrument_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<fx_accumulator_instrument_entity,
                                  domain::fx_accumulator_instrument>(
            ctx,
            query,
            [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
            lg(),
            "Reading latest FX accumulator instruments (workspace resolution chain).");
    }
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::read<std::vector<fx_accumulator_instrument_entity>> |
    ...
    return execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
        ...
        [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX accumulator instruments");
}

std::vector<domain::fx_accumulator_instrument>
fx_accumulator_instrument_repository::read_latest(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FX accumulator instrument. "
    ...
    return execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
        ...
        [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX accumulator instrument by instrument_id.");
}


std::vector<domain::fx_accumulator_instrument>
fx_accumulator_instrument_repository::read_all(context ctx, const std::string& instrument_id) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all FX accumulator instrument versions. "
                               << "instrument_id: " << instrument_id;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<fx_accumulator_instrument_entity>> |
    ...
    return execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
        ...
        [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
        lg(),
        "Reading all FX accumulator instrument versions by instrument_id.");
}

std::optional<domain::fx_accumulator_instrument>
fx_accumulator_instrument_repository::read_at_version(context ctx,
                                                      const std::string& instrument_id,
                                                      std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading FX accumulator instrument at version. "
    ...
    const auto entities =
        execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
            ctx,
            query,
            [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
            lg(),
            "Reading FX accumulator instrument at version.");
...
std::vector<domain::fx_accumulator_instrument> fx_accumulator_instrument_repository::read_latest(
    context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest FX accumulator instruments with offset: "
    ...
    return execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
        ...
        [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
        lg(),
        "Reading latest FX accumulator instruments with pagination.");
}

std::uint32_t
fx_accumulator_instrument_repository::get_total_fx_accumulator_instrument_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active FX accumulator instrument count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<fx_accumulator_instrument_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active FX accumulator instrument count: " << count;
    return count;
}
...
fx_accumulator_instrument_repository::read_latest(context ctx,
    ...
    auto result =
        execute_read_query<fx_accumulator_instrument_entity, domain::fx_accumulator_instrument>(
            ...
            [](const auto& entities) { return fx_accumulator_instrument_mapper::map(entities); },
            lg(),
            "Reading latest FX accumulator instruments by ids.");
    return result;
}

void fx_accumulator_instrument_repository::remove(context ctx,
                                                  const std::vector<std::string>& instrument_ids) {
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::delete_from<fx_accumulator_instrument_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid &&
                             "instrument_id"_c.in(instrument_ids) && "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing FX accumulator instruments.");
}
