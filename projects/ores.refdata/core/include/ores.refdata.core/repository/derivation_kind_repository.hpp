 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_DERIVATION_KIND_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_DERIVATION_KIND_REPOSITORY_HPP
...
/**
 * @brief Reads and writes derivation kinds to data storage.
 */
class ORES_REFDATA_CORE_EXPORT derivation_kind_repository {
private:
    inline static std::string_view logger_name =
        "ores.refdata.repository.derivation_kind_repository";
...
public:
    ...
    /**
     * @brief Writes derivation kinds to database.
     *
     ...
     */
    /**@{*/
    void write(context ctx, const domain::derivation_kind& v);
    void write(context ctx, const std::vector<domain::derivation_kind>& v);
    /**@}*/

    /**
     * @brief Writes a derivation kind, honouring the claim it states.
     *
    ...
    /**
     * @brief Writes a set of derivation kinds, each honouring its own
    ...
    /**
     * @brief Reads latest derivation kinds, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::derivation_kind> read_latest(context ctx);
     ...
     */
    std::vector<domain::derivation_kind> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single derivation kind as it stood at a specific
    ...
    /**
     * @brief Reads latest derivation kinds with pagination support.
     * @param ctx Repository context with database connection
    ...
    /**
     * @brief Deletes a derivation kind by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a derivation kind, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes derivation kinds by closing their temporal validity.
     */
...
private:
     ...
     */
    domain::derivation_kind apply_claim(context ctx,
                                        const domain::derivation_kind& v,
                                        const ores::utility::domain::precondition& claim);
};
