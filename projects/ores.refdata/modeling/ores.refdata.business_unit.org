...
#+description: Codegen entity model for the refdata =business_unit= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
:product:              ores
:component:            refdata
:parent_id_column:     parent_business_unit_id
:hierarchy_name_field: unit_name
...
:service_find_by_code_column:        unit_name
:service_find_by_code_parent_column: party_id
:END:

Note: the plan this flag traces to (task_apply_safe_drift.org) named
=unit_code= as the find-by-code column, but =unit_code= is nullable and
not part of the natural key -- (=party_id=, =unit_name=) is. Using
=unit_name= (with =party_id=) instead keeps the lookup unambiguous.
...
:explorer_interface:      IBusinessUnitBrowser
