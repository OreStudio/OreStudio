...
void ChangeReasonController::showListWindow() {
    ...
    BOOST_LOG_SEV(lg(), debug) << "Change Reason list window created";
}
...
void ChangeReasonController::showHistoryWindow(const QString& code) {
    ...
    auto* historyDialog =
        new HistoryDialog(std::string(entity_type_of(dq::domain::change_reason{})),
                          code.toStdString(),
                          clientManager_,
                          mainWindow_);
...
void ChangeReasonController::onOpenHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<ChangeReasonController> self = this;
    fetchChangeReasonHistory(
        entityId,
        [self, entityId, versionNumber](
            std::expected<std::vector<dq::domain::change_reason>, QString> result) {
            if (!self)
                return;
            if (!result) {
...
void ChangeReasonController::onRevertHistoryVersion(const QString& entityId, int versionNumber) {
    QPointer<ChangeReasonController> self = this;
    fetchChangeReasonHistory(
        entityId,
        [self, entityId, versionNumber](
...
EntityListMdiWindow* ChangeReasonController::listWindow() const {
    return listWindow_;
}

void ChangeReasonController::notifyOpenDialogs(const QStringList& entityIds) {
