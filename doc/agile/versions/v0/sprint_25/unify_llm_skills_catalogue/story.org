...
:END:
#+title: Story: Lay the foundation of the LLM skills catalogue
#+description: Put the skills catalogue on one model: the compass- namespace, cybernetic levels, cited principles, ported methods, the compass-helm mode, selection instrumentation and the regulator specification, with the codegen and build fixes that work surfaced.
#+type: story
#+level: s2
...
Narrowed at sprint 25 close to what shipped. The unmet criteria of the
original story (one word per concept across all skills, no duplicate
imports, the deployed bundle proven end to end) moved to
[[id:8F98FCB8-BF23-478C-ABE0-C92B67200719][Complete the LLM skills catalogue]].

- Every skill carries the compass- prefix, and the naming conventions
  document records the prefix and the verb registers.
- The catalogue is layered into principles, methods and process actions,
  and the regulator specification documents the layering and its
  reasoning.
- Every skill declares its cybernetic level, and the declared System
  attenuates which skills a session sees.
- Upstream skills are referenced, not copied. The pstack Collection
  classifies each one.
- compass-helm exists as the sticky entry point, and skill selection is
...
The design goes =LANDED= when [[id:8F98FCB8-BF23-478C-ABE0-C92B67200719][Complete the LLM skills catalogue]] closes. Its =#+outcome:= names
...
Closed at sprint 25 close with the 21 tasks that shipped: the compass- namespace, the cybernetic levels, the cited principles and ported methods, compass-helm, the selection instrument, the regulator specification, and the codegen and build fixes the work found. The story was renamed from "Unify the LLM skills catalogue" and its acceptance narrowed to match. The 28 open tasks moved, UUIDs intact, to [[id:8F98FCB8-BF23-478C-ABE0-C92B67200719][Complete the LLM skills catalogue]] in the product backlog inbox, for sprint 26.
