...
Disabled by default (=#+default: disabled=): every entity now uses the
single generic =ores::qt::HistoryDialog= (constructed with
=(entity_type, entity_id)= by =ores.cpp.qt.controller_impl=) instead of
a per-entity history dialog class. Retained only for reference until
HistoryDialogBase and its remaining legacy adopters are fully retired.
...
{{#domain_entity.qt.has_version_navigation}}
     ...
     */
    [[nodiscard]] const std::vector<{{domain_entity.qt.domain_class}}>& getHistory() const {
        return versions_;
    }
{{/domain_entity.qt.has_version_navigation}}
