...
PaymentFrequencyTypeDetailDialog::~PaymentFrequencyTypeDetailDialog() {
    delete ui_;
}

QTabWidget* PaymentFrequencyTypeDetailDialog::tabWidget() const {
    return ui_->tabWidget;
}

QWidget* PaymentFrequencyTypeDetailDialog::provenanceTab() const {
    return ui_->provenanceTab;
}

ProvenanceWidget* PaymentFrequencyTypeDetailDialog::provenanceWidget() const {
    return ui_->provenanceWidget;
}

void PaymentFrequencyTypeDetailDialog::setupUi() {
...
void PaymentFrequencyTypeDetailDialog::setClientManager(ClientManager* clientManager) {
    clientManager_ = clientManager;
}

void PaymentFrequencyTypeDetailDialog::setUsername(const std::string& username) {
    username_ = username;
}

void PaymentFrequencyTypeDetailDialog::setType(
    const trading::domain::payment_frequency_type& type) {
    type_ = type;
    updateUiFromType();
}

void PaymentFrequencyTypeDetailDialog::setCreateMode(bool createMode) {
...
void PaymentFrequencyTypeDetailDialog::setReadOnly(bool readOnly) {
    readOnly_ = readOnly;
...
void PaymentFrequencyTypeDetailDialog::updateUiFromType() {
...
void PaymentFrequencyTypeDetailDialog::updateTypeFromUi() {
    if (createMode_) {
...
void PaymentFrequencyTypeDetailDialog::onCodeChanged(const QString& /* text */) {
    hasChanges_ = true;
    updateSaveButtonState();
}

void PaymentFrequencyTypeDetailDialog::onFieldChanged() {
    hasChanges_ = true;
    updateSaveButtonState();
}

void PaymentFrequencyTypeDetailDialog::updateSaveButtonState() {
    bool canSave = hasChanges_ && validateInput() && !readOnly_;
    ui_->saveButton->setEnabled(canSave);
}

bool PaymentFrequencyTypeDetailDialog::validateInput() {
...
void PaymentFrequencyTypeDetailDialog::onSaveClicked() {
    ...
    BOOST_LOG_SEV(lg(), info) << "Saving payment frequency type: " << type_.code;

    QPointer<PaymentFrequencyTypeDetailDialog> self = this;
    ...
    connect(watcher, &QFutureWatcher<SaveResult>::finished,
        ...
        if (result.success) {
            BOOST_LOG_SEV(lg(), info) << "Payment Frequency Type saved successfully";
            QString code = QString::fromStdString(self->type_.code);
            self->hasChanges_ = false;
            self->updateSaveButtonState();
            emit self->typeSaved(code);
            self->notifySaveSuccess(tr("Payment Frequency Type '%1' saved").arg(code));
        } else {
...
void PaymentFrequencyTypeDetailDialog::onDeleteClicked() {
    ...
    BOOST_LOG_SEV(lg(), info) << "Deleting payment frequency type: " << type_.code;
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished,
        ...
        if (result.success) {
            BOOST_LOG_SEV(lg(), info) << "Payment Frequency Type deleted successfully";
            emit self->statusMessage(QString("Payment Frequency Type '%1' deleted").arg(code));
