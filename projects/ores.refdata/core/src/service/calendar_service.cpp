...
calendar_service::calendar_service(context ctx)
    : ctx_(std::move(ctx)) {}
namespace {
 ...
 */
std::vector<domain::calendar> read_one(repository::calendar_repository& repo,
                                       const ores::database::context& ctx,
                                       const messaging::calendar_key& key) {
 ...
 */
messaging::calendar_key key_from(const domain::calendar& v) {
 ...
 */
domain::calendar to_domain(const messaging::calendar_write& write) {
    domain::calendar v;
    v.code = write.code;
    v.name = write.name;
    v.calendar_type = write.calendar_type;
    v.country_code = write.country_code;
    v.image_id = write.image_id;
    v.source = write.source;
    v.is_editable = write.is_editable;
    v.base_calendar_code = write.base_calendar_code;
    return v;
}
...
calendar_service::list_calendars(const messaging::list_calendars_request& request) {
    messaging::list_calendars_response response;
    ...
    response.calendars = repo_.read_latest(ctx_, request.offset, request.limit);
    response.total = repo_.get_total_calendar_count(ctx_);
    return response;
}

messaging::get_calendar_response
calendar_service::get_calendar(const messaging::get_calendar_request& request) {
    messaging::get_calendar_response response;
    ...
    response.calendar = std::move(found.front());
    return response;
}

messaging::get_many_calendars_response
calendar_service::get_many_calendars(const messaging::get_many_calendars_request& request) {
    messaging::get_many_calendars_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::calendar_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.calendar = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_calendar_response
calendar_service::put_calendar(const messaging::put_calendar_request& request) {
    messaging::put_calendar_response response;
    domain::calendar value;
    ...
    if (!written.empty())
        response.calendar = std::move(written.front());
    return response;
}

messaging::put_many_calendars_response
calendar_service::put_many_calendars(const messaging::put_many_calendars_request& request) {
    messaging::put_many_calendars_response response;
    std::vector<domain::calendar> batch;
    ...
    response.calendars.reserve(batch.size());
    for (const auto& value : batch) {
        auto written = read_one(repo_, ctx_, key_from(value));
        response.calendars.push_back(written.empty() ? value : std::move(written.front()));
    }
    return response;
}

messaging::delete_calendar_response
calendar_service::delete_calendar(const messaging::delete_calendar_request& request) {
    messaging::delete_calendar_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::calendar_repository::remove_status::removed:
            break;
        case repository::calendar_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::calendar_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::calendar_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
calendar_service::delete_many_calendars(const messaging::delete_many_calendars_request& request) {
    messaging::delete_many_calendars_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
calendar_service::list_calendar_versions(const messaging::list_calendar_versions_request& request) {
    messaging::list_calendar_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
calendar_service::get_calendar_version(const messaging::get_calendar_version_request& request) {
    messaging::get_calendar_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.calendar.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
calendar_service::prepare_change(const messaging::calendar_change& change,
...
std::vector<domain::calendar> calendar_service::list_calendars(std::uint32_t offset,
                                                               std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all calendars";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t calendar_service::count_calendars() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total calendars count";
...
calendar_service::get_calendars(const std::vector<std::string>& codes) {
