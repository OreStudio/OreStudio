...
/**
 * @brief Detail dialog for viewing and editing contact type records.
 *
 * This dialog allows viewing, creating, and editing contact types.
 * It supports both create mode (for new records) and edit mode (for
 * existing records).
 */
class ContactTypeDetailDialog final : public DetailDialogBase {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.contact_type_detail_dialog";
...
public:
    explicit ContactTypeDetailDialog(QWidget* parent = nullptr);
    ~ContactTypeDetailDialog() override;

    void setClientManager(ClientManager* clientManager);
    void setUsername(const std::string& username);
    void setType(const refdata::domain::contact_type& type);
...
private:
    ...
    refdata::domain::contact_type type_;
