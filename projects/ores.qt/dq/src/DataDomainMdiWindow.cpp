...
void DataDomainMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &DataDomainMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &DataDomainMdiWindow::editSelected);

    deleteAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::Delete, IconUtils::DefaultIconColor), tr("Delete"));
    deleteAction_->setToolTip(tr("Delete selected data domain"));
    deleteAction_->setEnabled(false);
    connect(deleteAction_, &QAction::triggered, this, &DataDomainMdiWindow::deleteSelected);

    historyAction_ = toolbar_->addAction(
        IconUtils::createRecoloredIcon(Icon::History, IconUtils::DefaultIconColor), tr("History"));
    historyAction_->setToolTip(tr("View data domain history"));
    historyAction_->setEnabled(false);
    connect(historyAction_, &QAction::triggered, this, &DataDomainMdiWindow::viewHistorySelected);
}

void DataDomainMdiWindow::setupTable() {
    model_ = new ClientDataDomainModel(clientManager_, this);
...
void DataDomainMdiWindow::setupConnections() {
    ...
    connect(tableView_, &QTableView::doubleClicked, this, &DataDomainMdiWindow::onDoubleClicked);
...
void DataDomainMdiWindow::doReload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading data domains";
    clearStaleIndicator();
    emit statusChanged(tr("Loading data domains..."));
...
void DataDomainMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 data domains").arg(loaded).arg(total));
...
void DataDomainMdiWindow::onDoubleClicked(const QModelIndex& index) {
    if (!index.isValid())
        return;
...
void DataDomainMdiWindow::addNew() {
    BOOST_LOG_SEV(lg(), debug) << "Add new data domain requested";
    emit addNewRequested();
}

void DataDomainMdiWindow::editSelected() {
...
void DataDomainMdiWindow::viewHistorySelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
...
void DataDomainMdiWindow::deleteSelected() {
    const auto selected = tableView_->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete data domain while disconnected.");
        return;
    }
    ...
    if (codes.empty()) {
        BOOST_LOG_SEV(lg(), warn) << "No valid data domains to delete";
        return;
    }

    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " data domains";

    QString confirmMessage;
    if (codes.size() == 1) {
        confirmMessage = QString("Are you sure you want to delete data domain '%1'?")
                             .arg(QString::fromStdString(codes.front()));
    } else {
        confirmMessage =
            QString("Are you sure you want to delete %1 data domains?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Data Domain", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              "Successfully deleted 1 data domain" :
                              QString("Successfully deleted %1 data domains").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "data domain" : "data domains")
                              .arg(first_error);
