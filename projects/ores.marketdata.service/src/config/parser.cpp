...
void print_help(const options_description& od, std::ostream& info) {
    info << "Market data ingestion and query service" << std::endl << std::endl
         << "Usage: ores.marketdata.service [options]" << std::endl << std::endl
         << od << std::endl;
}
...
parse_arguments(const std::vector<std::string>& arguments, std::ostream& info) {
    ...
    const auto name_mapper(
        environment_mapper_factory::make_mapper("MARKETDATA_SERVICE"));
