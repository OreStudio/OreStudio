...
void data_domain_repository::write(const domain::data_domain& data_domain) {
    BOOST_LOG_SEV(lg(), debug) << "Writing data_domain to database: " << data_domain.name;

    execute_write_query(
        ctx_, data_domain_mapper::map(data_domain), lg(), "writing data_domain to database");
}

void data_domain_repository::write(const std::vector<domain::data_domain>& data_domains) {
    BOOST_LOG_SEV(lg(), debug) << "Writing data_domains to database. Count: "
                               << data_domains.size();

    execute_write_query(
        ctx_, data_domain_mapper::map(data_domains), lg(), "writing data_domains to database");
}

std::vector<domain::data_domain> data_domain_repository::read_latest() {
    ...
    return execute_read_query<data_domain_entity, domain::data_domain>(
        ...
        lg(),
        "Reading latest data_domains");
}

std::vector<domain::data_domain> data_domain_repository::read_latest(const std::string& name) {
    ...
    return execute_read_query<data_domain_entity, domain::data_domain>(
        ...
        [](const auto& entities) { return data_domain_mapper::map(entities); },
        lg(),
        "Reading latest data_domain by name.");
}

std::vector<domain::data_domain> data_domain_repository::read_latest(std::uint32_t offset,
                                                                     std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest data_domains with offset: " << offset
                               << " and limit: " << limit;
    ...
    return execute_read_query<data_domain_entity, domain::data_domain>(
        ...
        [](const auto& entities) { return data_domain_mapper::map(entities); },
        lg(),
        "Reading latest data_domains with pagination.");
}
...
std::vector<domain::data_domain> data_domain_repository::read_all(const std::string& name) {
    BOOST_LOG_SEV(lg(), debug) << "Reading all data_domain versions. Name: " << name;
