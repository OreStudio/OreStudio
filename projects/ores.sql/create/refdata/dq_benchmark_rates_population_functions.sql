 ...
 *   SELECT * FROM ores_dq_preview_benchmark_rate_population(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM ores_dq_populate_benchmark_rates(dataset_id, 'upsert');
 */
 ...
 */
create or replace function ores_dq_preview_benchmark_rate_population(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    left join ores_refdata_benchmark_rates_tbl existing
...
/**
 * Populate ores_refdata_benchmark_rates_tbl from a DQ benchmark_rates dataset.
 *
 * @param p_dataset_id  The DQ dataset to populate from.
 * @param p_mode        Population mode: 'upsert', 'insert_only', or 'replace_all'.
 */
create or replace function ores_dq_populate_benchmark_rates(
    p_dataset_id uuid,
