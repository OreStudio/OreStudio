...
void folder_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("folders");
    ...
    menu->Insert(
        ...
        "add <id> <parent_id> <name> <kind> <collection_id> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <id> <parent_id> <name> <kind> <collection_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <parent_id> <name> <kind> <collection_id> <reason> "
        "<commentary>");
...
void folder_commands::process_list(std::ostream& out,
                                   nats_client& session,
                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_folders_request;
    ...
    auto result = do_auth_request<messaging::list_folders_response>(
...
void folder_commands::process_get(std::ostream& out,
                                  nats_client& session,
                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_folder_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_folder_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_get_many(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_folders_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_many_folders_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_add(std::ostream& out,
                                  nats_client& session,
                                  const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_folder_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.parent_id, parsed->positionals[next++], "parent_id");
        ...
        read_token(req.change.write.collection_id, parsed->positionals[next++], "collection_id");
    ...
    auto result = do_auth_request<messaging::put_folder_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_set(std::ostream& out,
                                  nats_client& session,
                                  const std::vector<std::string>& args) {
...
void folder_commands::process_put_many(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_folders_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::folder_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(change.write.parent_id, parsed->positionals[next++], "parent_id");
            read_token(change.write.name, parsed->positionals[next++], "name");
            read_token(change.write.kind, parsed->positionals[next++], "kind");
            read_token(change.write.collection_id, parsed->positionals[next++], "collection_id");
    ...
    auto result = do_auth_request<messaging::put_many_folders_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_delete(std::ostream& out,
                                     nats_client& session,
                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_folder_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::delete_folder_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_delete_many(std::ostream& out,
                                          nats_client& session,
                                          const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_folders_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            ...
            req.removals.push_back(messaging::folder_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_folders_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_versions(std::ostream& out,
                                       nats_client& session,
                                       const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_folder_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::list_folder_versions_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void folder_commands::process_version(std::ostream& out,
                                      nats_client& session,
                                      const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_folder_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result = do_auth_request<messaging::get_folder_version_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
