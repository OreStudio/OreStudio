 ...
 *   SELECT * FROM metadata.dq_preview_party_role_population_fn(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM metadata.dq_populate_party_roles_fn(dataset_id, 'upsert');
 */
 ...
 */
create or replace function metadata.dq_preview_party_role_population_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_party_roles_artefact_tbl dq
    left join production.refdata_party_roles_tbl existing
 ...
 */
create or replace function metadata.dq_populate_party_roles_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_party_roles_tbl existing
        ...
        insert into production.refdata_party_roles_tbl (
