...
** Phase 2 — C++ CDM (ores.marketdata)

Create a new =ores.marketdata= project containing the C++ domain model.

*** Tasks

- [ ] Create =ores.marketdata= project skeleton (CMakeLists, includes, src, tests)
- [ ] =asset_class= enum (FX, RATES, CREDIT, EQUITY, COMMODITY, INFLATION, BOND,
      CROSS_ASSET)
- [ ] =series_subclass= enum (SPOT, FORWARD, VOLATILITY, YIELD, BASIS, FRA, XCCY,
      SPREAD, INDEX, RECOVERY, SWAP, CAPFLOOR, SEASONALITY, PRICE, CORRELATION)
- [ ] =market_series= domain entity
- [ ] =market_observation= domain entity
- [ ] =market_fixing= domain entity
...
** Phase 3 — ORE key decomposition (ores.ore updates)
...
- [ ] Define a =series_key_registry= mapping each ORE =series_type= to
      =(qualifier_depth, is_scalar)=. This is a static compile-time table.
- [ ] Update =market_datum= struct to carry =(series_type, metric, qualifier,
      point_id)= alongside =key= (for validation/roundtrip).
- [ ] Update =fixing= struct to carry =series_type=, =qualifier= (the index name).
- [ ] Update =market_data_parser= to fill the decomposed fields.
- [ ] Update =market_data_serializer= to reconstruct =key= from components.
- [ ] Update all roundtrip tests to validate decomposed fields.
...
** Phase 4 — Database repository (ores.database)

Add repository classes and mappers for the three market data tables.

*** Tasks

- [ ] DB entity structs for =ores_marketdata_series_tbl=,
      =ores_marketdata_observations_tbl=, =ores_marketdata_fixings_tbl=
- [ ] Mapper: =market_series= ↔ series entity
- [ ] Mapper: =market_observation= ↔ observation entity
- [ ] Mapper: =market_fixing= ↔ fixing entity
- [ ] =market_series_repository=: get_all, get_by_id, get_by_type, save
- [ ] =market_observations_repository=: get_by_series, get_by_date_range, save_bulk
- [ ] =market_fixings_repository=: get_by_series, get_by_date_range, save_bulk
