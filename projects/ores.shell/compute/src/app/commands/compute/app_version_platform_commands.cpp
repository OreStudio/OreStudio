...
namespace messaging = ores::compute::messaging;
...
void app_version_platform_commands::register_commands(cli::Menu& root_menu, nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("app_version_platforms");
    ...
    menu->Insert(
        ...
        "get <app_version_id> <platform_id>");
    ...
    menu->Insert(
        ...
        "get-many <app_version_id> <platform_id>");
    ...
    menu->Insert(
        ...
        "add <app_version_id> <platform_id> <package_uri> <sha256> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "set <app_version_id> <platform_id> <package_uri> <sha256> <reason> <commentary> "
        "[--version <n>]");
    ...
    menu->Insert(
        ...
        "put-many --count <n> <app_version_id> <platform_id> <package_uri> <sha256> <reason> "
        "<commentary>");
    ...
    menu->Insert(
        ...
        "delete <app_version_id> <platform_id> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <app_version_id> <platform_id> <reason> <commentary>");

    menu->Insert(
        "by-app-version-id",
        [&session](std::ostream& out, std::vector<std::string> args) {
            process_by_app_version_id(std::ref(out), std::ref(session), std::move(args));
        },
        "by-app-version-id <app_version_id> [--offset <n>] [--limit <n>] [--order <field>] "
        "[--desc]");

    root_menu.Insert(std::move(menu));
}

void app_version_platform_commands::process_list(std::ostream& out,
                                                 nats_client& session,
                                                 const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_app_version_platforms_request;
    ...
    auto result = do_auth_request<messaging::list_app_version_platforms_response>(
...
void app_version_platform_commands::process_get(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_app_version_platform_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.app_version_id, parsed->positionals[next++], "app_version_id");
        read_token(req.key.platform_id, parsed->positionals[next++], "platform_id");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_app_version_platform_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_get_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_app_version_platforms_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 2) {
            ...
            read_token(key.app_version_id, parsed->positionals[i + 0], "app_version_id");
            read_token(key.platform_id, parsed->positionals[i + 1], "platform_id");
    ...
    auto result = do_auth_request<messaging::get_many_app_version_platforms_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_add(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_app_version_platform_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.platform_id, parsed->positionals[next++], "platform_id");
        read_token(req.change.write.package_uri, parsed->positionals[next++], "package_uri");
        read_token(req.change.write.sha256, parsed->positionals[next++], "sha256");
    ...
    auto result = do_auth_request<messaging::put_app_version_platform_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_set(std::ostream& out,
                                                nats_client& session,
                                                const std::vector<std::string>& args) {
...
void app_version_platform_commands::process_put_many(std::ostream& out,
                                                     nats_client& session,
                                                     const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_app_version_platforms_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::app_version_platform_change change;
            read_token(change.write.app_version_id, parsed->positionals[next++], "app_version_id");
            read_token(change.write.platform_id, parsed->positionals[next++], "platform_id");
            read_token(change.write.package_uri, parsed->positionals[next++], "package_uri");
            read_token(change.write.sha256, parsed->positionals[next++], "sha256");
    ...
    auto result = do_auth_request<messaging::put_many_app_version_platforms_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_delete(std::ostream& out,
                                                   nats_client& session,
                                                   const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_app_version_platform_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.app_version_id, parsed->positionals[next++], "app_version_id");
        read_token(req.removal.key.platform_id, parsed->positionals[next++], "platform_id");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result = do_auth_request<messaging::delete_app_version_platform_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_delete_many(std::ostream& out,
                                                        nats_client& session,
                                                        const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_app_version_platforms_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::app_version_platform_key key;
            read_token(key.app_version_id, parsed->positionals[i * 2 + 0], "app_version_id");
            read_token(key.platform_id, parsed->positionals[i * 2 + 1], "platform_id");
            req.removals.push_back(messaging::app_version_platform_removal{.key = std::move(key)});
        }
    ...
    auto result = do_auth_request<messaging::delete_many_app_version_platforms_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void app_version_platform_commands::process_by_app_version_id(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating by-app-version-id request.";

    using request_type = messaging::list_by_app_version_id_app_version_platforms_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
            fail(out) << "You must be logged in to run by-app-version-id." << std::endl;
            return;
        }
    }
    ...
    try {
        ...
        req.app_version_id = ores::shell::app::from_token<boost::uuids::uuid>(
            parsed->positionals[next++], "app_version_id");
    ...
    auto result = do_auth_request<messaging::list_by_app_version_id_app_version_platforms_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
