...
create table if not exists "production"."refdata_asset_classes_tbl" (
    "code" text not null,
...
create unique index if not exists refdata_asset_classes_version_uniq_idx
on "production"."refdata_asset_classes_tbl" (code, coding_scheme_code, version)
where valid_to = public.utility_infinity_timestamp_fn();

create index if not exists refdata_asset_classes_coding_scheme_idx
on "production"."refdata_asset_classes_tbl" (coding_scheme_code)
where valid_to = public.utility_infinity_timestamp_fn();

create or replace function production.refdata_asset_classes_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "production"."refdata_asset_classes_tbl"
...
create or replace trigger refdata_asset_classes_insert_trg
before insert on "production"."refdata_asset_classes_tbl"
for each row
execute function production.refdata_asset_classes_insert_fn();

create or replace rule refdata_asset_classes_delete_rule as
on delete to "production"."refdata_asset_classes_tbl"
do instead
  update "production"."refdata_asset_classes_tbl"
  set valid_to = current_timestamp
  where code = old.code
