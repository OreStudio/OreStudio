...
HostMdiWindow::HostMdiWindow(ClientManager* clientManager, const QString& username, QWidget* parent)
...
void HostMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &HostMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &HostMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &HostMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &HostMdiWindow::viewHistorySelected);
}
...
void HostMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "HostListWindow", {}, {900, 400}, 1);
}

void HostMdiWindow::setupConnections() {
    connect(model_, &ClientHostModel::dataLoaded, this, &HostMdiWindow::onDataLoaded);
    connect(model_, &ClientHostModel::loadError, this, &HostMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &HostMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &HostMdiWindow::onDoubleClicked);
...
void HostMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void HostMdiWindow::viewHistorySelected() {
    ...
    auto sourceIndex = proxyModel_->mapToSource(selected.first());
    if (auto* host = model_->getHost(sourceIndex.row())) {
        BOOST_LOG_SEV(lg(), debug) << "Emitting showHostHistory for code: " << host->external_id;
...
void HostMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete compute host while disconnected.");
        return;
    }
    ...
    for (const auto& index : selected) {
        auto sourceIndex = proxyModel_->mapToSource(index);
        if (auto* host = model_->getHost(sourceIndex.row())) {
            ids.push_back(boost::uuids::to_string(host->id));
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << ids.size() << " compute hosts";
    ...
    if (ids.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 compute hosts?").arg(ids.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Host", confirmMessage, QMessageBox::Yes | QMessageBox::No);
    ...
    QPointer<HostMdiWindow> self = this;
    ...
    auto task = [self, ids, codes]() -> DeleteResult {
        ...
        BOOST_LOG_SEV(lg(), debug)
            << "Making delete request for " << ids.size() << " compute hosts";
    ...
    connect(watcher, &QFutureWatcher<DeleteResult>::finished, self, [self, watcher]() {
        ...
        for (const auto& [id, code, success, message] : results) {
            if (success) {
                BOOST_LOG_SEV(lg(), debug) << "Host deleted: " << code;
                success_count++;
                emit self->hostDeleted(QString::fromStdString(code));
            } else {
                BOOST_LOG_SEV(lg(), error) << "Host deletion failed: " << code << " - " << message;
                failure_count++;
        ...
        if (failure_count == 0) {
            QString msg = success_count == 1 ?
                              "Successfully deleted 1 compute host" :
                              QString("Successfully deleted %1 compute hosts").arg(success_count);
            emit self->statusChanged(msg);
        } else if (success_count == 0) {
            QString msg = QString("Failed to delete %1 %2: %3")
                              .arg(failure_count)
                              .arg(failure_count == 1 ? "compute host" : "compute hosts")
                              .arg(first_error);
