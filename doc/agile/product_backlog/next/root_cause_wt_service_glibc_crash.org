...
The controller's existing restart/backoff already self-heals eventually
(it did recover after ~8 attempts in the diagnosed case), so this isn't a
hard outage — just a noisy, minutes-long crash-loop on every environment
boot where it's hit.
...
- The exponential-backoff calculator this capture once pointed at, =ores.utility/concurrency/retry_strategy.hpp=, was deleted on the ores.utility clean-up pass: its only consumer was its own test. The backoff the controller runs is the one inside =process_supervisor.cpp=.
