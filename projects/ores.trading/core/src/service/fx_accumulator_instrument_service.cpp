...
fx_accumulator_instrument_service::fx_accumulator_instrument_service(context ctx)
    : ctx_(std::move(ctx)) {}

std::vector<domain::fx_accumulator_instrument>
fx_accumulator_instrument_service::list_fx_accumulator_instruments(std::uint32_t offset,
                                                                   std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all FX accumulator instruments";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t fx_accumulator_instrument_service::count_fx_accumulator_instruments() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total FX accumulator instruments count";
    return repo_.get_total_fx_accumulator_instrument_count(ctx_);
}
...
std::optional<domain::fx_accumulator_instrument>
fx_accumulator_instrument_service::get_fx_accumulator_instrument(const std::string& instrument_id) {
