...
bond_trs_service::bond_trs_service(context ctx)
    : ctx_(std::move(ctx)) {}

std::vector<domain::bond_trs> bond_trs_service::list_trs(std::uint32_t offset,
                                                         std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all bond trs";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t bond_trs_service::count_trs() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total bond trs count";
    return repo_.get_total_trs_count(ctx_);
}


std::optional<domain::bond_trs>
bond_trs_service::get_trs_at_version(const std::string& instrument_id, std::uint32_t version) {
...
std::optional<domain::bond_trs> bond_trs_service::get_trs(const std::string& instrument_id) {
...
void bond_trs_service::delete_trs(const std::string& instrument_id) {
