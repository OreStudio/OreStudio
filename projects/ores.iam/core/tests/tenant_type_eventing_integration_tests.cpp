...
// notification" pattern end to end for tenant_type -- the
...
TEST_CASE("write_tenant_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::iam::eventing::tenant_type_changed_event>(
        [&nats](const ores::iam::eventing::tenant_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<ores::iam::eventing::tenant_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.iam.tenant_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.types,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::iam::eventing::tenant_type_changed_event>(
        "ores.iam.tenant_type", "ores_iam_tenant_types");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<ores::iam::eventing::tenant_type_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_tenant_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.type;
    BOOST_LOG_SEV(lg, debug) << "Tenant Type: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.iam.tenant_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenant_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenant_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::iam::service::tenant_type_service svc(crud_ctx);
