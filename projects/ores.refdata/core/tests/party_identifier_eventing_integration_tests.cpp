...
#include "ores.refdata.api/eventing/party_identifier_event.hpp"
...
TEST_CASE("write_party_identifier_publishes_an_event", tags) {
    ...
    using event_type = ores::refdata::messaging::party_identifier_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_refdata_party_identifiers");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for party_identifier " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::party_identifier_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_party_identifier_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_party_identifier(id_str);
        ...
        REQUIRE_FALSE(svc.get_party_identifier(id_str).has_value());
        REQUIRE(svc.get_party_identifier_history(id_str).size() == versions.size());
    }
}
