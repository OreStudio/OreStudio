...
#include "ui_TenorKindHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

TenorKindHistoryDialog::TenorKindHistoryDialog(const QString& code,
                                               ClientManager* clientManager,
                                               QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::TenorKindHistoryDialog)
    , code_(code)
...
TenorKindHistoryDialog::~TenorKindHistoryDialog() {
    delete ui_;
}

QString TenorKindHistoryDialog::code() const {
    return code_;
}

void TenorKindHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for tenor kind: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_tenor_kind_history_request request;
    request.code = code_.toStdString();

    QPointer<TenorKindHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_tenor_kind_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int TenorKindHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString TenorKindHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow TenorKindHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult TenorKindHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
...
void TenorKindHistoryDialog::displayFullDetails(int index) {
...
void TenorKindHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void TenorKindHistoryDialog::revertToVersionAt(int index) {
