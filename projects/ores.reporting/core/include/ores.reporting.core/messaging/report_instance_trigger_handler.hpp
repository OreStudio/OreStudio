    ...
    void trigger(ores::nats::message msg) {
        ...
        if (auto trigger_msg = decode<trigger_report_instance_message>(msg)) {
            try {
                ...
                if (!def) { BOOST_LOG_SEV(report_instance_trigger_handler_lg(), warn) << "Definition not found: " << trigger_msg->report_definition_id; return; }
                ...
                BOOST_LOG_SEV(report_instance_trigger_handler_lg(), info) << "Created report instance " << inst_id_str;
                if (should_dispatch) {
                    ...
                    BOOST_LOG_SEV(report_instance_trigger_handler_lg(), info) << "Dispatched workflow for instance " << inst_id_str;
                }
            } catch (const std::exception& e) { BOOST_LOG_SEV(report_instance_trigger_handler_lg(), error) << "Trigger failed: " << e.what(); }
        } else { BOOST_LOG_SEV(report_instance_trigger_handler_lg(), warn) << "Failed to decode: " << msg.subject; }
    }
