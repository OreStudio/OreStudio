...
// notification" pattern end to end for business_unit_type -- the
...
TEST_CASE("write_business_unit_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::business_unit_type_changed_event>(
        [&nats](const ores::refdata::eventing::business_unit_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::business_unit_type_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.business_unit_type",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.type_ids,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::business_unit_type_changed_event>(
        "ores.refdata.business_unit_type", "ores_refdata_business_unit_types");
    ...
    auto v = generate_synthetic_business_unit_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = boost::uuids::to_string(v.id);
    BOOST_LOG_SEV(lg, debug) << "Business Unit Type: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.business_unit_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for business_unit_type " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for business_unit_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::business_unit_type_service svc(crud_ctx);
