...
// notification" pattern end to end for pricing_engine_type -- the
...
TEST_CASE("write_pricing_engine_type_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::analytics::eventing::pricing_engine_type_changed_event>(
        [&nats](const ores::analytics::eventing::pricing_engine_type_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::analytics::eventing::pricing_engine_type_changed_event>::name),
    ...
    event_source.register_mapping<ores::analytics::eventing::pricing_engine_type_changed_event>(
        "ores.analytics.pricing_engine_type", "ores_analytics_pricing_engine_types");
    ...
    auto v = generate_synthetic_pricing_engine_type(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Pricing Engine Type: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.analytics.pricing_engine_type") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for pricing_engine_type " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for pricing_engine_type " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::analytics::service::pricing_engine_type_service svc(crud_ctx);
