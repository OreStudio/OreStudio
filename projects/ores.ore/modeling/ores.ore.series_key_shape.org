...
The entity's surface is deliberately not generated. It is a reference
table an operator does not edit: the rows are ORE's key grammar, seeded
by a populate script, and the whole of the component's use of it is the
key registry's read. A NATS protocol, a handler, a sub-registrar, a
history provider registrar, an event registrar, a service, a sample
generator, a shell command unit and its recipe, a table renderer and the
TypeScript twins would all be generated for nobody: the nine subjects
answer nothing, the registrars are called from nowhere, and the shell
unit has never been in the build.
...
The data layer — domain, entity, mapper, repository — and the SQL schema
are generated.
