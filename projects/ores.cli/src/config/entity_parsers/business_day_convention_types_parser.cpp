...
/**
 * @brief Creates the options related to adding business day convention types.
 */
options_description make_add_business_day_convention_type_options_description() {
    options_description r("Add Business day convention types Options");
...
/**
 * @brief Reads the add configuration from the variables map for business day convention types.
 */
ores::cli::config::add_business_day_convention_type_options
read_add_business_day_convention_type_options(const variables_map& vm) {
    ores::cli::config::add_business_day_convention_type_options r;
    ...
    if (vm.count("description") != 0) {
        const auto desc = vm["description"].as<std::string>();
        if (!desc.empty())
            r.description = desc;
    }
...
std::optional<options> handle_business_day_convention_types_command(bool has_help,
    ...
    if (has_help && o.empty()) {
        const std::vector<std::pair<std::string, std::string>> operations = {
            {"export", "Export business day convention types to external formats"},
            {"list", "List business day convention types as JSON or table"},
            {"delete", "Delete a business day convention type by code"},
            {"add", "Add a new business day convention type"}};
        print_entity_help("business_day_convention_types",
                          "Manage business day convention types",
                          operations,
                          info);
        return {};
    }
    ...
    if (o.empty()) {
        BOOST_THROW_EXCEPTION(parser_exception("business_day_convention_types command requires an "
                                               "operation (export, list, delete, add)"));
    }

    const auto operation = o.front();
    o.erase(o.begin()); // Remove operation from args

    validate_operation("business_day_convention_types", operation, allowed_operations);
    ...
    if (operation == export_command_name) {
        auto d = add_common_options(make_export_options_description());
        if (has_help) {
            print_help_command("business_day_convention_types export", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.exporting = read_export_options(vm, entity::business_day_convention_types);
    } else if (operation == list_command_name) {
        auto d = add_common_options(make_export_options_description());
        if (has_help) {
            print_help_command("business_day_convention_types list", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        ...
        if (has_help) {
            print_help_command("business_day_convention_types delete", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.deleting = read_delete_options(vm, entity::business_day_convention_types);
    } else if (operation == add_command_name) {
        auto d = add_common_options(make_add_business_day_convention_type_options_description());
        if (has_help) {
            print_help_command("business_day_convention_types add", d, info);
            return {};
        }
        store(command_line_parser(o).options(d).run(), vm);
        store(parse_environment(d, name_mapper), vm);
        r.adding = read_add_business_day_convention_type_options(vm);
    }
