...
#+description: Codegen entity model for the refdata =book= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
#+begin_src cpp :name generator
// X-0 is the sentinel code the eventing integration fixture seeds
...
:skip_check:      true
:nullable:        false
:error_message:   Invalid functional_currency: %. No active currency found with this iso code.
:END:
