...
#+description: Run the codegen-fix-drift method on ores.workflow: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the drift gate to cover workflow-cpp. Supersedes the ad hoc 'Apply safe drift to workflow-cpp' task.
#+type: story
#+level: s2
...
Bring ores.workflow to the state the codegen-fix-drift skill defines: every codegen-eligible entity bound to a profile or recorded as a genuine no-match, zero-diff regeneration verified, and any real gap the check surfaces fixed rather than skipped. Once trusted, workflow-cpp joins the known-drift-free registry that the drift gate checks.
...
- workflow-cpp joins the =KNOWN_DRIFT_FREE= registry in check_component_drift.py (skill step 8), and the pr-raise skill's local codegen-drift check runs =--all= over the registry, so future PRs touching ores.workflow fail on any regeneration drift.
