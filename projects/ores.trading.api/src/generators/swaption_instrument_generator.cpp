 ...
 *
 */
#include "ores.trading.api/generators/swaption_instrument_generator.hpp"
...
domain::swaption_instrument generate_synthetic_swaption_instrument(
...
generate_synthetic_swaption_instruments(std::size_t n,
    utility::generation::generation_context& ctx) {
    std::vector<domain::swaption_instrument> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_swaption_instrument(ctx));
    return r;
}
