 ...
 */
#ifndef ORES_IAM_API_EVENTING_SESSION_EVENT_HPP
#define ORES_IAM_API_EVENTING_SESSION_EVENT_HPP
...
/**
 * @brief Event traits for session's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::iam::messaging::session_event> {
    static constexpr std::string_view subject_prefix = "iam.v1.sessions_events";

    static ores::iam::messaging::session_event
    from_notification(const entity_event_notification& notification) {
        ores::iam::messaging::session_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::iam::messaging::session_key>(notification.key))
            event.key = *key;
