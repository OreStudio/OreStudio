...
create unique index if not exists instrument_codes_name_uniq_idx
on "ores_refdata_instrument_codes_tbl" (tenant_id, name)
where valid_to = ores_utility_infinity_timestamp_fn();

-- Version uniqueness for optimistic concurrency
create unique index if not exists instrument_codes_version_uniq_idx
on "ores_refdata_instrument_codes_tbl" (tenant_id, code, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists instrument_codes_code_uniq_idx
on "ores_refdata_instrument_codes_tbl" (tenant_id, code)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists instrument_codes_tenant_idx
on "ores_refdata_instrument_codes_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_refdata_instrument_codes_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_refdata_instrument_codes_tbl"
...
create or replace trigger ores_refdata_instrument_codes_insert_trg
before insert on "ores_refdata_instrument_codes_tbl"
for each row execute function ores_refdata_instrument_codes_insert_fn();

create or replace rule ores_refdata_instrument_codes_delete_rule as
on delete to "ores_refdata_instrument_codes_tbl" do instead (
    update "ores_refdata_instrument_codes_tbl"
    set valid_to = clock_timestamp()
...
-- =============================================================================
-- Validation function for instrument_code
-- Validates that a code exists in the instrument_codes table.
-- Returns the validated value, or default if null/empty.
-- Uses system tenant data (shared reference data).
-- =============================================================================
create or replace function ores_refdata_validate_instrument_code_fn(
    p_tenant_id uuid,
    p_value text
) returns text as $$
begin
    ...
    if p_value is null or p_value = '' then
        raise exception 'Invalid instrument_code: value cannot be null or empty'
            using errcode = '23502';
    end if;
    ...
    if not exists (
        ...
        raise exception 'Invalid instrument_code: %. Must be one of: %', p_value, (
