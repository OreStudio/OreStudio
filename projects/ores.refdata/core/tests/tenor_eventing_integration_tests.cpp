...
// notification" pattern end to end for tenor -- the
...
TEST_CASE("write_tenor_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::tenor_changed_event>(
        [&nats](const ores::refdata::eventing::tenor_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<ores::refdata::eventing::tenor_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.tenor",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::tenor_changed_event>(
        "ores.refdata.tenor", "ores_refdata_tenors");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<ores::refdata::eventing::tenor_changed_event>::name),
        10);
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.tenor") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenor " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenor " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_tenor(id_str).has_value());
        REQUIRE(svc.get_tenor_history(id_str).size() == versions.size());
    }
}
