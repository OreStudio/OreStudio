...
#include "ui_CalendarHistoryDialog.h"

namespace ores::qt {

using namespace ores::logging;

CalendarHistoryDialog::CalendarHistoryDialog(const QString& code,
                                             ClientManager* clientManager,
                                             QWidget* parent)
    : HistoryDialogBase(parent)
    , ui_(new Ui::CalendarHistoryDialog)
    , code_(code)
...
CalendarHistoryDialog::~CalendarHistoryDialog() {
    delete ui_;
}

QString CalendarHistoryDialog::code() const {
    return code_;
}

void CalendarHistoryDialog::loadHistory() {
    BOOST_LOG_SEV(lg(), debug) << "Loading history for calendar: " << code_.toStdString();
    emit statusChanged(tr("Loading history..."));

    refdata::messaging::get_calendar_history_request request;
    request.code = code_.toStdString();

    QPointer<CalendarHistoryDialog> self = this;
    runHistoryRequest(clientManager_,
                      std::move(request),
                      [self](refdata::messaging::get_calendar_history_response response) {
                          if (!self)
                              return;
                          if (!response.success) {
...
int CalendarHistoryDialog::historySize() const {
    return static_cast<int>(versions_.size());
}

QString CalendarHistoryDialog::historyTitle() const {
    return QString("History for: %1").arg(code_);
}

HistoryDialogBase::VersionRow CalendarHistoryDialog::versionRow(int index) const {
...
HistoryDialogBase::DiffResult CalendarHistoryDialog::calculateDiffAt(int ci, int pi) const {
    DiffResult diffs;
    ...
    checkString(diffs, tr("Type"), curr.calendar_type, prev.calendar_type);
    checkString(diffs, tr("Country"), curr.country_code, prev.country_code);
    return diffs;
}

void CalendarHistoryDialog::displayFullDetails(int index) {
    ...
    ui_->calendarTypeValue->setText(QString::fromStdString(version.calendar_type));
...
void CalendarHistoryDialog::openVersionAt(int index) {
    emit openVersionRequested(versions_[index], versions_[index].version);
}

void CalendarHistoryDialog::revertToVersionAt(int index) {
