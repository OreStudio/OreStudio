...
#+description: Run the compass-codegen-fix-drift method on ores.synthetic: bind every eligible entity to a variability profile, resolve every drift the zero-diff regen check surfaces, and extend the drift gate to cover synthetic.
#+type: story
#+level: s2
...
- synthetic joins the =KNOWN_DRIFT_FREE= registry in check_component_drift.py (skill step 8), and the compass-pr-raise skill's local codegen-drift check runs =--all= over the registry, so future PRs touching ores.synthetic fail on any regeneration drift.
...
Status at close: Story closed: the post-merge live system test passed and the tpl/ppv table-display defect it caught is fixed.
