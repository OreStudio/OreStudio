...
#include "ores.refdata.api/generators/tenor_kind_generator.hpp"
...
// notification" pattern end to end for tenor_kind -- the
...
using ores::refdata::domain::tenor_kind;
using ores::refdata::repository::tenor_kind_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_tenor_kind_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::tenor_kind_changed_event>(
        [&nats](const ores::refdata::eventing::tenor_kind_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::tenor_kind_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.tenor_kind",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::tenor_kind_changed_event>(
        "ores.refdata.tenor_kind", "ores_refdata_tenor_kinds");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(
            ev::domain::event_traits<ores::refdata::eventing::tenor_kind_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_tenor_kind(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Tenor Kind: " << v;

    tenor_kind_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.tenor_kind") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tenor_kind " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tenor_kind " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::tenor_kind_service svc(crud_ctx);
