...
void CrmCrossRatesMatrixMdiWindow::reload() {
    ...
    QFuture<RatesResult> future = QtConcurrent::run([self, party_id, crm_name]() -> RatesResult {
    ...
    connect(watcher, &QFutureWatcher<RatesResult>::finished, this, [self, watcher]() {
        ...
        for (int row = 0; row < rowCurrencies.size(); ++row) {
            for (int col = 0; col < allCurrencies.size(); ++col) {
                ...
                const double display_rate =
                    inverted && item.rate != 0.0 ? 1.0 / item.rate : item.rate;
                ...
                if (item.status == "stale") {
                    ...
                    if (previous_it != self->previousRates_.end() && previous_it->second != 0.0) {
                        ...
                        if (std::abs(pct) > 1e-9) {
                            changeText =
                                (pct >= 0 ? QStringLiteral("▲ +%1%") : QStringLiteral("▼ %1%"))
                                    .arg(QString::number(pct, 'f', 3));
                            changeColor = pct >= 0 ? color_constants::level_info :
