...
| Now          | Every item that does not depend on what the component owns is done: B01 to B05, M01 to M10, G01, G03, G06, H01 to H03, S02, V01, the database half of V03 and the core half of V02. M06 and P04 were the substantive find: the import protocol was hand-written, so it had no TypeScript twin and the web client could not speak it; it is modelled now. Seven findings are recorded, three of them defects. Every codegen gate passes with =ore= registered. The remaining items are the entity protocol and wiring ones and the shell unit, and all of them change shape with the ownership decision. |
| Waiting on   | The ownership decision. [[id:6EA32258-F1DC-45D2-8913-202E845CD969][What ores.ore should own]] measures the four jobs the component does and recommends moving the series key shape entity to =ores.marketdata=; the owner's stated intent is that =ores.ore= be a library for ORE representation mapping with no NATS surface. Until that lands, P01 to P06, W01, D13 and S01 cannot be finished, because the entity protocol they cover may not stay here. |
| Next         | Move the entity, or record why it stays. Move the shell's export step onto a protocol, which both options need. Then work the facet vocabulary and P01 onward. |
...
| Finding: two part test suites are empty | =ores.ore.api.tests= and =ores.ore.service.tests= each contain only =main.cpp=, so both binaries link, run and report "No tests ran". The core part carries 62 test files. The entity's own shell command test exists but is not compiled (D13), so the only test the generated surface has is the eventing integration test in core. The api now has a generated protocol with a TypeScript twin, which gives it something worth testing: nothing asserts that the model's fields and the service's expectations agree. | Running both binaries; =ls projects/ores.ore/{api,service}/tests/= |
| V05/V06 (ready, not taken) | Every codegen gate passes with =ore= added to =COMPONENTS_UNDER_TEST=: component drift =--all=, model drift, protocol twin coverage, handler permissions, populate references, the shell recipe inventory, and the CMake source lists for this component. Registering is deliberately not done yet: the registry's own docstring says a component joins once its whole rollout is finished and verified, and P03, W01, S01 and D13 are still open and change shape with the ownership decision. Once that lands, V06 is a one-line change to =component_registry.py= plus a re-run. | Each gate run with =ore= registered, then the registry reverted |
