...
#+description: Codegen entity model for the iam =tenant= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
Tenant type classification (FK to tenant_types). The synthetic
generator emits the canonical automation type: the type/status insert
validations reject any value with no active tenant_type row, and the
seed data (iam_tenant_types_populate.sql) seeds the four system-tenant
types, of which automation is the system-process default.
...
** Presentation
:PROPERTIES:
:domain_include:         ores.iam.api/domain/tenant.hpp
:domain_class:           iam::domain::tenant
:protocol_include:       ores.iam.api/messaging/tenant_protocol.hpp
:collection_name:        tenants
:item_var:               tenant
:key_field:              code
:get_request_class:      iam::messaging::list_tenants_request
:get_response_class:     iam::messaging::list_tenants_response
:get_message_type:       list_tenants_request
:save_request_class:     iam::messaging::put_tenant_request
:save_response_class:    iam::messaging::put_tenant_response
:save_message_type:      put_tenant_request
...
:history_request_class:  iam::messaging::list_tenant_versions_request
:history_response_class: iam::messaging::list_tenant_versions_response
:history_message_type:   list_tenant_versions_request
