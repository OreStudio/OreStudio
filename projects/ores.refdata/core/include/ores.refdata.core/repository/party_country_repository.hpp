...
public:
    ...
    /**
     * @brief Writes party countries to database.
     *
     ...
     */
    /**@{*/
    void write(const domain::party_country& v);
    void write(const std::vector<domain::party_country>& v);
    /**@}*/

    /**
     * @brief Writes a party country, honouring the claim it states.
     *
     ...
     */
    void write(const domain::party_country& v, const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of party countries, each honouring its own claim, as
     * one statement.
     */
    void write(const std::vector<domain::party_country>& v,
    ...
    /**
     * @brief Reads the party country rows for the given pair of keys.
     *
     * A junction key is the whole pair the link names, so a read that states
     * only one half addresses a set and not a row.
     */
    std::vector<domain::party_country> read_latest(const boost::uuids::uuid& party_id,
    ...
    /**
     * @brief Deletes a party country by its pair of keys.
     */
    ...
    /**
     * @brief Removes a party country, refusing a row that moved on.
     *
    ...
    /**
     * @brief Deletes party countries by their pairs of keys.
     */
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(const domain::party_country& v);
     ...
     */
    domain::party_country apply_claim(const domain::party_country& v,
