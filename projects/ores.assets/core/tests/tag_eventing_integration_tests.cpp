...
#include "ores.assets.api/eventing/tag_event.hpp"
...
// notification" pattern end to end for tag -- the
...
const std::string_view test_suite("assets.tests");
...
TEST_CASE("write_tag_publishes_an_event", tags) {
    ...
    using event_type = ores::assets::messaging::tag_event;
    ...
    event_source.register_entity_event_mapping<event_type>("ores_assets_tags");
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for tag " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for tag " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::assets::service::tag_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_tag_history(v.name);
        ...
        REQUIRE_FALSE(svc.get_tag(v.id).has_value());
        REQUIRE(svc.get_tag_history(v.name).size() == versions.size());
    }
}
