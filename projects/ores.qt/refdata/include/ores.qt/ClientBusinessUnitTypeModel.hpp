 ...
 *
 * This model extends AbstractClientModel and fetches business unit type
 * data asynchronously using the ores.comms client.
 */
class ClientBusinessUnitTypeModel final : public AbstractClientModel {
    Q_OBJECT

private:
    inline static std::string_view logger_name = "ores.qt.client_business_unit_type_model";
...
public:
    ...
    explicit ClientBusinessUnitTypeModel(ClientManager* clientManager, QObject* parent = nullptr);
...
private:
    ...
    void fetch_business_unit_types(std::uint32_t offset, std::uint32_t limit);

    ClientManager* clientManager_;
    std::vector<refdata::domain::business_unit_type> business_unit_types_;
    ...
    RecencyTracker<refdata::domain::business_unit_type, BusinessUnitTypeKeyExtractor>
        recencyTracker_;
