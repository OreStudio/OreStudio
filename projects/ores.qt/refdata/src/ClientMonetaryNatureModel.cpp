...
ClientMonetaryNatureModel::ClientMonetaryNatureModel(ClientManager* clientManager, QObject* parent)
    ...
    connect(watcher_,
            ...
            this,
            &ClientMonetaryNatureModel::onNaturesLoaded);
...
int ClientMonetaryNatureModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
void ClientMonetaryNatureModel::refresh() {
        ...
        return exception_helper::wrap_async_fetch<FetchResult>(
            [&]() -> FetchResult {
                BOOST_LOG_SEV(lg(), debug)
                    << "Making monetary natures request with offset=" << offset
                    << ", limit=" << limit;
                ...
                BOOST_LOG_SEV(lg(), debug)
                    << "Fetched " << result->monetary_natures.size() << " monetary natures";
...
void ClientMonetaryNatureModel::onNaturesLoaded() {
    ...
    if (!result.success) {
        BOOST_LOG_SEV(lg(), error)
            << "Failed to fetch monetary natures: " << result.error_message.toStdString();
...
void ClientMonetaryNatureModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
const refdata::domain::monetary_nature* ClientMonetaryNatureModel::getNature(int row) const {
...
void ClientMonetaryNatureModel::onPulsingComplete() {
