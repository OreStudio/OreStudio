...
generate_synthetic_equity_forward_instrument(utility::generation::generation_context& ctx) {
    ...
    r.identity.trade_type_code = std::string("EquityForward");
...
generate_synthetic_equity_forward_instruments(std::size_t n,
    ...
    std::vector<domain::equity_forward_instrument> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_equity_forward_instrument(ctx));
    return r;
}
