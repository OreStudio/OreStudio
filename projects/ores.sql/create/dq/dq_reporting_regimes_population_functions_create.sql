 ...
 *   SELECT * FROM metadata.dq_preview_reporting_regime_population_fn(dataset_id);
 *
 *   -- Publish to production
 *   SELECT * FROM metadata.dq_populate_reporting_regimes_fn(dataset_id, 'upsert');
 */
 ...
 */
create or replace function metadata.dq_preview_reporting_regime_population_fn(p_dataset_id uuid)
returns table (
    action text,
    code text,
...
begin
    ...
    from metadata.dq_reporting_regimes_artefact_tbl dq
    left join production.refdata_reporting_regimes_tbl existing
 ...
 */
create or replace function metadata.dq_populate_reporting_regimes_fn(
    p_dataset_id uuid,
...
begin
    ...
    loop
        ...
        select exists (
            select 1 from production.refdata_reporting_regimes_tbl existing
        ...
        insert into production.refdata_reporting_regimes_tbl (
