...
paged by-side read per side, an opt-in whole-set replacement per side, a
single-pair =link= and =unlink=, a count per side, and the
=<junction>_view= payload that enriches the junction row with the other
side's display fields. The by-side read and the replacement keep the
subjects and shapes they already emitted, so a junction that renders them
today is unaffected by the added verbs.
 ...
 * pair already exists.
 */
struct link_{{name_singular}}_request {
    using response_type = struct link_{{name_singular}}_response;
    static constexpr std::string_view nats_subject =
        "{{component}}.v1.{{name}}.link_{{name_singular}}";
    std::string {{left.column}};
    std::string {{right.column}};
{{#columns}}
    {{{cpp_type}}} {{name}};
...
struct link_{{name_singular}}_response {
    bool success = false;
    std::string message;
};
 ...
 * set untouched.
 */
struct unlink_{{name_singular}}_request {
    using response_type = struct unlink_{{name_singular}}_response;
    static constexpr std::string_view nats_subject =
        "{{component}}.v1.{{name}}.unlink_{{name_singular}}";
    std::string {{left.column}};
    std::string {{right.column}};
    std::string modified_by;
    std::string performed_by;
...
struct unlink_{{name_singular}}_response {
    bool success = false;
    std::string message;
};
