...
The PR gate on GitHub is minimal: classify (the path classifier) and
pr-gate (the sole required check). The site, roundtrip, and drift
checks moved local: the [[id:368E2B3E-CD2C-4FD5-90DD-D0E935810F4D][absorb-PR-checks-locally story]]'s submit-PR
skills classify the change and run only the matching checks before
raising the PR. GitHub keeps the continuous builds, nightlies, site
deploy, coverage, and housekeeping bots. This document is the
inventory, the fate of every check, and the classification rule table.
...
| Job      | Runs when | What it records                                                    | Cost |
|----------+-----------+--------------------------------------------------------------------+------|
| classify | always    | Path classifier; buckets every changed file into docs/code/ci (audit trail; the local skills mirror the rules) | ~3s |
| pr-gate  | always    | Aggregates classify; the branch-protection required check          | ~4s  |

The four functional jobs (=site=, =roundtrip=,
=cmake-sources-drift=, =codegen-drift=) ran in this workflow until the
gate-reduction task deleted them; they run locally now (see the fate
table below).
...
| site                  | deleted from pr.yml | Local: =compass build --direct site= (same emacs export; ~5-10 min locally). Optional CDash submission as experimental. |
| roundtrip             | deleted from pr.yml | Local: =python3 scripts/ore_domain_roundtrip_check.py=. |
| cmake-sources-drift   | deleted from pr.yml | Local: =python3 projects/ores.codegen/scripts/regenerate_cmake_component_files.py --all --check=; the =codegen-sync-cmake-sources= skill wraps it. |
| codegen-drift         | deleted from pr.yml | Local: tangle codegen templates (=compass build --direct codegen_templates=) then =check_component_drift.py= with the same components. |
