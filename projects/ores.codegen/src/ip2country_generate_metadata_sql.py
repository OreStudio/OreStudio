...
def generate_catalog_sql(manifest: dict, output_file: Path):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("""
/**
 * IP Geolocation Catalog Population Script
 *
 ...
 *
 * Dependencies between IP geolocation datasets.
 * Auto-generated from external/ip2country/manifest.json
 * Must be run after ip2country_dataset_populate.sql.
 */
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate SQL populate scripts for IP to Country metadata.',
    ...
    if args.manifest_dir:
        ...
        manifest_dir = repo_root / 'external' / 'ip2country'

    if args.output_dir:
        output_dir = Path(args.output_dir)
    else:
        output_dir = repo_root / 'projects' / 'ores.sql' / 'populate' / 'ip2country'
    ...
    print(f"IP to Country Metadata SQL Generator")
    print(f"====================================")
    ...
    # Generate files
    generate_catalog_sql(manifest, output_dir / 'ip2country_catalog_populate.sql')
    generate_methodology_sql(manifest, methodology_text,
                              output_dir / 'ip2country_methodology_populate.sql')
