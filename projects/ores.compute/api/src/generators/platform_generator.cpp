...
domain::platform generate_synthetic_platform(utility::generation::generation_context& ctx) {
    ...
    r.os_family = std::string(faker::word::noun());
    r.cpu_arch = std::string(faker::word::noun());
...
generate_synthetic_platforms(std::size_t n, utility::generation::generation_context& ctx) {
    std::vector<domain::platform> r;
    r.reserve(n);
    while (r.size() < n)
        r.push_back(generate_synthetic_platform(ctx));
    return r;
}
