...
void party_type_repository::write(context ctx, const domain::party_type& type) {
    BOOST_LOG_SEV(lg(), debug) << "Writing party type to database: " << type.code;
    execute_write_query(ctx, party_type_mapper::map(type), lg(), "Writing party type to database.");
}

void party_type_repository::write(context ctx, const std::vector<domain::party_type>& types) {
    BOOST_LOG_SEV(lg(), debug) << "Writing party types to database. Count: " << types.size();
    execute_write_query(
        ctx, party_type_mapper::map(types), lg(), "Writing party types to database.");
}
