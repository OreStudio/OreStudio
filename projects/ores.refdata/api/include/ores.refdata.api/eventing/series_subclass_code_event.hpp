 ...
 */
#ifndef ORES_REFDATA_API_EVENTING_SERIES_SUBCLASS_CODE_EVENT_HPP
#define ORES_REFDATA_API_EVENTING_SERIES_SUBCLASS_CODE_EVENT_HPP
...
/**
 * @brief Event traits for series subclass code's changes.
 *
 ...
 */
template <>
struct entity_event_traits<ores::refdata::messaging::series_subclass_code_event> {
    static constexpr std::string_view subject_prefix = "refdata.v1.series_subclass_codes_events";

    static ores::refdata::messaging::series_subclass_code_event
    from_notification(const entity_event_notification& notification) {
        ores::refdata::messaging::series_subclass_code_event event;
        event.event_id = boost::lexical_cast<boost::uuids::uuid>(notification.event_id);
        if (const auto key = rfl::json::read<ores::refdata::messaging::series_subclass_code_key>(
                notification.key))
            event.key = *key;
