...
|---+-----------------+------+----------+-------|
| 1 (PR #1778, x4 reviewers) | No regression test covers =_soft_fk_validation_node_to_dict=/=target_column= behaviour -- exactly the bug class that shipped silently once already | org_loader.py | Fixed | Added =test_org_loader_foreign_keys.py= (3 cases): default =target_column="id"=, explicit override honoured, and an explicit assertion that setting =:referenced_column:= does *not* count as a recognised override -- =target_column= still falls back to =id=. All 93 =ores.codegen= pytest cases pass. |
