...
// notification" pattern end to end for currency_market_tier -- the
...
using ores::refdata::domain::currency_market_tier;
using ores::refdata::repository::currency_market_tier_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_currency_market_tier_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::currency_market_tier_changed_event>(
        [&nats](const ores::refdata::eventing::currency_market_tier_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::currency_market_tier_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::currency_market_tier_changed_event>(
        "ores.refdata.currency_market_tier", "ores_refdata_currency_market_tiers");
    ...
    auto v = generate_synthetic_currency_market_tier(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Currency Market Tier: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.currency_market_tier") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for currency_market_tier " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for currency_market_tier " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::currency_market_tier_service svc(crud_ctx);
