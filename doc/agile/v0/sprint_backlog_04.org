...
#+CAPTION: Clock summary at [2025-11-12 Wed 15:32], for Wednesday, November 12, 2025.
|      | <75>                                       |        |      |      |       |
| Tags | Headline                                   | Time   |      |      |     % |
|------+--------------------------------------------+--------+------+------+-------|
|      | *Total time*                               | *3:21* |      |      | 100.0 |
|------+--------------------------------------------+--------+------+------+-------|
|      | Stories                                    | 3:21   |      |      | 100.0 |
|      | Active                                     |        | 3:21 |      | 100.0 |
| code | Fix the valgrind leaks reported on sqlgen  |        |      | 1:53 |  56.2 |
| code | Review all code created so far with an LLM |        |      | 1:28 |  43.8 |
#+end:

#+begin: clocktable :maxlevel 3 :scope subtree :tags t :indent nil :emphasize nil :scope file :narrow 75 :formula %
#+TBLNAME: sprint_summary
#+CAPTION: Clock summary at [2025-11-12 Wed 15:32]
|          | <75>                                                   |         |       |       |       |
| Tags     | Headline                                               | Time    |       |       |     % |
|----------+--------------------------------------------------------+---------+-------+-------+-------|
|          | *Total time*                                           | *54:26* |       |       | 100.0 |
|----------+--------------------------------------------------------+---------+-------+-------+-------|
|          | Stories                                                | 54:26   |       |       | 100.0 |
|          | Active                                                 |         | 54:26 |       | 100.0 |
| agile    | Sprint and product backlog refinement                  |         |       |  2:33 |   4.7 |
| code     | Clean up client parser                                 |         |       |  0:10 |   0.3 |
| code     | Add connect command line argument                      |         |       |  1:19 |   2.4 |
| infra    | Install Windows package on Windows machine             |         |       |  2:09 |   3.9 |
| code     | Ignore warnings on windows and osx                     |         |       |  0:33 |   1.0 |
| code     | Ensure applications work under OSX using github images |         |       |  0:14 |   0.4 |
| infra    | Create a splash screen and gray background             |         |       |  2:26 |   4.5 |
| code     | Use fluent icons                                       |         |       |  0:37 |   1.1 |
| code     | Add export functionality for currencies                |         |       |  1:30 |   2.8 |
| code     | Add currency editing support                           |         |       |  2:45 |   5.1 |
| code     | Add about to menu                                      |         |       |  1:02 |   1.9 |
| code     | Create a history dialog for currencies                 |         |       |  1:27 |   2.7 |
| code     | Allow MDI windows outside main window                  |         |       |  1:35 |   2.9 |
| code     | Add edit menu and export menu                          |         |       |  0:34 |   1.0 |
| code     | Implement adding new currencies                        |         |       |  3:17 |   6.0 |
| code     | Clean up product website                               |         |       |  4:42 |   8.6 |
| code     | Create a skill for plant uml documentation             |         |       |  2:48 |   5.1 |
| infra    | Add CMake targets for site and skills                  |         |       |  0:17 |   0.5 |
| infra    | Generalise the skill setup for ORE Studio              |         |       |  3:45 |   6.9 |
| infra    | Restructure components to follow the dogen approach    |         |       |  2:54 |   5.3 |
| infra    | Generate PlantUML diagrams for all components          |         |       |  1:05 |   2.0 |
| code     | Fix the valgrind leaks reported on sqlgen              |         |       |  1:53 |   3.5 |
| code     | Make UI/UX look more professional                      |         |       | 11:07 |  20.4 |
| code     | Current issues with package install in debian          |         |       |  0:40 |   1.2 |
| code     | Resolve crash on shutdown problem                      |         |       |  0:50 |   1.5 |
| analysis | Weaving skills and documentation together              |         |       |  0:46 |   1.4 |
| code     | Review all code created so far with an LLM             |         |       |  1:28 |   2.7 |
#+end:
...
*** STARTED Weaving skills and documentation together              :analysis:
    :LOGBOOK:
...
#+begin_quote
I want you do perform a quick code review of code that I will show you. you are
to look for obvious mistakes, cases where we could be using c++ 23 features,
incorrect comments or improvements on comments, and other things you may think
are worth pointing out. The system is a finance system using Data-Oriented
principles· Be succinct. sacrifice grammar over verbosity. If there is logging,
check if the logging is at the appropriate level and if the logging message
makes sense. If there is no logging, if appropriate suggest additional logging
which could provide helpful context. The result should be a small list of
actionable items, preferable with the proposed change. I will show you the
header file followed by the implementation file. You can comment on them
separately.
#+end_quote
