...
regulatory_book_type_service::regulatory_book_type_service(context ctx)
    : ctx_(std::move(ctx)) {}
namespace {
...
read_one(repository::regulatory_book_type_repository& repo,
         ...
         const messaging::regulatory_book_type_key& key) {
 ...
 */
messaging::regulatory_book_type_key key_from(const domain::regulatory_book_type& v) {
    messaging::regulatory_book_type_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::regulatory_book_type to_domain(const messaging::regulatory_book_type_write& write) {
    domain::regulatory_book_type v;
    v.code = write.code;
    v.name = write.name;
...
regulatory_book_type_service::list_regulatory_book_types(
    const messaging::list_regulatory_book_types_request& request) {
    messaging::list_regulatory_book_types_response response;
...
messaging::get_regulatory_book_type_response regulatory_book_type_service::get_regulatory_book_type(
    const messaging::get_regulatory_book_type_request& request) {
    messaging::get_regulatory_book_type_response response;
    ...
    response.regulatory_book_type = std::move(found.front());
    return response;
}

messaging::get_many_regulatory_book_types_response
regulatory_book_type_service::get_many_regulatory_book_types(
    const messaging::get_many_regulatory_book_types_request& request) {
    messaging::get_many_regulatory_book_types_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::regulatory_book_type_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.regulatory_book_type = std::move(found.front());
        response.entries.push_back(std::move(entry));
    }
    return response;
}

messaging::put_regulatory_book_type_response regulatory_book_type_service::put_regulatory_book_type(
    const messaging::put_regulatory_book_type_request& request) {
    messaging::put_regulatory_book_type_response response;
    domain::regulatory_book_type value;
    ...
    if (!written.empty())
        response.regulatory_book_type = std::move(written.front());
    return response;
}

messaging::put_many_regulatory_book_types_response
regulatory_book_type_service::put_many_regulatory_book_types(
    const messaging::put_many_regulatory_book_types_request& request) {
    messaging::put_many_regulatory_book_types_response response;
    std::vector<domain::regulatory_book_type> batch;
...
regulatory_book_type_service::delete_regulatory_book_type(
    const messaging::delete_regulatory_book_type_request& request) {
    messaging::delete_regulatory_book_type_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::regulatory_book_type_repository::remove_status::removed:
            break;
        case repository::regulatory_book_type_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::regulatory_book_type_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::regulatory_book_type_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_regulatory_book_types_response
regulatory_book_type_service::delete_many_regulatory_book_types(
    const messaging::delete_many_regulatory_book_types_request& request) {
    messaging::delete_many_regulatory_book_types_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_regulatory_book_type_versions_response
regulatory_book_type_service::list_regulatory_book_type_versions(
    const messaging::list_regulatory_book_type_versions_request& request) {
    messaging::list_regulatory_book_type_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_regulatory_book_type_version_response
regulatory_book_type_service::get_regulatory_book_type_version(
    const messaging::get_regulatory_book_type_version_request& request) {
    messaging::get_regulatory_book_type_version_response response;
    auto found =
        repo_.read_at_version(ctx_, request.key.regulatory_book_type.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
regulatory_book_type_service::prepare_change(const messaging::regulatory_book_type_change& change,
...
std::vector<domain::regulatory_book_type>
regulatory_book_type_service::list_types(std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Listing all regulatory book types";
    return repo_.read_latest(ctx_, offset, limit);
}

std::uint32_t regulatory_book_type_service::count_types() {
    BOOST_LOG_SEV(lg(), debug) << "Getting total regulatory book types count";
...
std::vector<domain::regulatory_book_type>
regulatory_book_type_service::get_types(const std::vector<std::string>& codes) {
