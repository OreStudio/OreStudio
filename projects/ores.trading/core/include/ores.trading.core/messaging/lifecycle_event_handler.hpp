...
public:
    ...
    void list(ores::nats::message msg) {
        ...
        get_lifecycle_events_response resp;
        if (auto req = decode<get_lifecycle_events_request>(msg)) {
            try {
                resp.events = svc.list_events(req->offset, req->limit);
                resp.total_available_count = static_cast<int>(svc.count_events());
                resp.success = true;
    ...
    void save(ores::nats::message msg) {
        ...
        service::lifecycle_event_service svc(req_ctx);
        if (auto req = decode<save_lifecycle_event_request>(msg)) {
            try {
                svc.save_event(req->data);
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, save_lifecycle_event_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      save_lifecycle_event_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(lifecycle_event_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
    ...
    void history(ores::nats::message msg) {
        ...
        const auto& req_ctx = *req_ctx_expected;
        service::lifecycle_event_service svc(req_ctx);
        if (auto req = decode<get_lifecycle_event_history_request>(msg)) {
            try {
                auto hist = svc.get_event_history(req->code);
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_,
                      msg,
                      get_lifecycle_event_history_response{.history = std::move(hist),
                                                           .success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      get_lifecycle_event_history_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(lifecycle_event_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
    ...
    void remove(ores::nats::message msg) {
        ...
        service::lifecycle_event_service svc(req_ctx);
        if (auto req = decode<delete_lifecycle_event_request>(msg)) {
            try {
                svc.delete_events(req->codes);
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), debug) << "Completed " << msg.subject;
                reply(nats_, msg, delete_lifecycle_event_response{.success = true});
            } catch (const std::exception& e) {
                BOOST_LOG_SEV(lifecycle_event_handler_lg(), error)
                    << msg.subject << " failed: " << e.what();
                reply(nats_,
                      msg,
                      delete_lifecycle_event_response{.success = false, .message = e.what()});
            }
        } else {
            BOOST_LOG_SEV(lifecycle_event_handler_lg(), warn)
                << "Failed to decode: " << msg.subject;
