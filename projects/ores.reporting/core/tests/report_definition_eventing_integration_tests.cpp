...
// Report Definition writes are party-scoped: the session-level
...
TEST_CASE("write_report_definition_publishes_nats_changed_event", tags) {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for report_definition " << id_str << " after "
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::reporting::service::report_definition_service svc(crud_ctx);
        ...
        REQUIRE_FALSE(svc.get_definition(id_str).has_value());
