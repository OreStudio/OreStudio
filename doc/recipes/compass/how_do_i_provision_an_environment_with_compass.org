...
=compass env configure= offers to run =build/scripts/install_debian_packages.sh=
at the end of the configure step.  The flag set depends on the environment type
...
Non-interactive runs (CI, scripts without a TTY) skip package installation
automatically and print the command to run manually.
