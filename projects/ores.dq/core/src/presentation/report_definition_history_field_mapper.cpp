 ...
 *
 */
#include "ores.dq.core/presentation/report_definition_history_field_mapper.hpp"
...
render_report_definition_fields(const domain::report_definition& v) {
    ...
    fields.push_back({.name = "Report Type", .value = v.report_type});
    fields.push_back({.name = "Schedule Expression", .value = v.schedule_expression});
    fields.push_back({.name = "Concurrency Policy", .value = v.concurrency_policy});
