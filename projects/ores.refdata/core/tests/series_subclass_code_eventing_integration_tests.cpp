...
// notification" pattern end to end for series_subclass_code -- the
...
using ores::refdata::domain::series_subclass_code;
using ores::refdata::repository::series_subclass_code_repository;
using ores::testing::scoped_database_helper;
using namespace ores::logging;

TEST_CASE("write_series_subclass_code_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::series_subclass_code_changed_event>(
        [&nats](const ores::refdata::eventing::series_subclass_code_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(ev::domain::event_traits<
                            ores::refdata::eventing::series_subclass_code_changed_event>::name),
                ev::domain::entity_change_event{.entity = "ores.refdata.series_subclass_code",
                                                .timestamp = e.timestamp,
                                                .entity_ids = e.codes,
                                                .tenant_id = e.tenant_id});
        });

    event_source.register_mapping<ores::refdata::eventing::series_subclass_code_changed_event>(
        "ores.refdata.series_subclass_code", "ores_refdata_series_subclass_codes");
    ...
    auto v = generate_synthetic_series_subclass_code(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.code;
    BOOST_LOG_SEV(lg, debug) << "Series Subclass Code: " << v;

    series_subclass_code_repository repo;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.series_subclass_code") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for series_subclass_code " << id_str
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for series_subclass_code " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        ores::refdata::service::series_subclass_code_service svc(crud_ctx);
        v.change_commentary = "updated-by-crud-round-trip";
        repo.write(crud_ctx, v);

        auto versions = svc.get_series_subclass_history(id_str);
        REQUIRE(versions.size() >= 2);
        REQUIRE(versions.front().change_commentary == "updated-by-crud-round-trip");

        svc.delete_series_subclass(id_str);
        ...
        REQUIRE_FALSE(svc.get_series_subclass(id_str).has_value());
        REQUIRE(svc.get_series_subclass_history(id_str).size() == versions.size());
    }
}
