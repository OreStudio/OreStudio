...
// notification" pattern end to end for country -- the
...
TEST_CASE("write_country_publishes_nats_changed_event", tags) {
    ...
    auto sub = bus.subscribe<ores::refdata::eventing::country_changed_event>(
        [&nats](const ores::refdata::eventing::country_changed_event& e) {
            ev::service::publish_entity_event(
                nats,
                std::string(
                    ev::domain::event_traits<ores::refdata::eventing::country_changed_event>::name),
    ...
    event_source.register_mapping<ores::refdata::eventing::country_changed_event>(
        "ores.refdata.country", "ores_refdata_countries");
    ...
    auto observer = nats.subscribe_buffered(
        std::string(ev::domain::event_traits<ores::refdata::eventing::country_changed_event>::name),
        10);
    ...
    auto v = generate_synthetic_country(ctx);
    v.change_reason_code = "system.test";
    const auto id_str = v.alpha2_code;
    BOOST_LOG_SEV(lg, debug) << "Country: " << v;
            ...
            for (const auto& msg : snap) {
                auto decoded =
                    ores::nats::default_wire_codec().decode<ev::domain::entity_change_event>(
                        msg.data);
                if (decoded && decoded->entity == "ores.refdata.country") {
    ...
    if (received.empty()) {
        ...
        BOOST_LOG_SEV(lg, error) << "No notification for country " << id_str << " after "
    ...
    BOOST_LOG_SEV(lg, info) << "Received " << received.size()
                            << " matching NATS notification(s) for country " << id_str;
    ...
    // only growth is asserted, not an exact count.
    {
        ...
        REQUIRE_FALSE(svc.get_country(id_str).has_value());
        REQUIRE(svc.get_country_history(id_str).size() == versions.size());
    }
}
