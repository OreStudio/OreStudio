...
version, so the composite partial indexes below are the hot paths; the
generator owns them from this model.
...
Pagination, the active-count aggregate, and the per-FK list reads come from
the standard repository template; the state-filtered read below is the one
method beyond it.
