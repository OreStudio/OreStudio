...
| G07 | Pass after a review finding. The command tests were generated while their template still asserted a bare truth, which this item forbids; the template is now fixed at source and the tests regenerated, with a stub proof that they fail when =register_commands= does nothing. No diagram was refreshed during this pass, so the two-pass contract does not apply. | =ores.cpp.shell-command.command_tests.org=; commit f6a46d2b05 |
