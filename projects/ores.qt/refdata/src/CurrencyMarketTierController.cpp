...
void CurrencyMarketTierController::showListWindow() {
    BOOST_LOG_SEV(lg(), debug) << "showListWindow called";

    const QString key = build_window_key("list", "currency_market_tiers");
    ...
    // Create new window
    listWindow_ = new CurrencyMarketTierMdiWindow(clientManager_, username_, badgeCache_);
...
void CurrencyMarketTierController::closeAllWindows() {
...
void CurrencyMarketTierController::reloadListWindow() {
    if (listWindow_) {
        listWindow_->reload();
    }
}

void CurrencyMarketTierController::onShowDetails(
    const refdata::domain::currency_market_tier& type) {
    BOOST_LOG_SEV(lg(), debug) << "Show details for: " << type.code;
    showDetailWindow(type);
}

void CurrencyMarketTierController::onAddNewRequested() {
    BOOST_LOG_SEV(lg(), info) << "Add new currency market tier requested";
    showAddWindow();
}
...
void CurrencyMarketTierController::wireDetailDialogCommon(
    CurrencyMarketTierDetailDialog* detailDialog) {
...
void CurrencyMarketTierController::showAddWindow() {
...
void CurrencyMarketTierController::showDetailWindow(
    const refdata::domain::currency_market_tier& type) {
...
void CurrencyMarketTierController::showHistoryWindow(const QString& code) {
    BOOST_LOG_SEV(lg(), info) << "Opening history window for currency market tier: "
                              << code.toStdString();
