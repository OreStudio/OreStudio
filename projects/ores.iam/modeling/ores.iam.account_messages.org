...
#+component: iam
#+subcomponent: api
#+entity_singular: account_operations
...
The publish trigger is declared here beside the operations, because it is a
subject IAM owns and nothing else states it: the DQ artefact-type row that
publishes onto it carries the same string in SQL, and a constant generated
from here is what the registrar subscribes with.

** publish_accounts_from_dq_request
:PROPERTIES:
:subject: iam.v1.accounts.publish-from-dq
:END:
