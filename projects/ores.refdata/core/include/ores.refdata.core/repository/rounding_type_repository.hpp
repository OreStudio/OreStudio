 ...
 */
#ifndef ORES_REFDATA_CORE_REPOSITORY_ROUNDING_TYPE_REPOSITORY_HPP
#define ORES_REFDATA_CORE_REPOSITORY_ROUNDING_TYPE_REPOSITORY_HPP
 ...
 */
class ORES_REFDATA_CORE_EXPORT rounding_type_repository {
private:
    inline static std::string_view logger_name = "ores.refdata.repository.rounding_type_repository";
...
public:
    ...
    /**
     * @brief Writes rounding types to database.
     *
    ...
    /**
     * @brief Writes a rounding type, honouring the claim it states.
     *
    ...
    void write(context ctx,
               const domain::rounding_type& v,
               const ores::utility::domain::precondition& claim);

    /**
     * @brief Writes a set of rounding types, each honouring its own
     * claim, as one statement.
     */
    void write(context ctx,
               const std::vector<domain::rounding_type>& v,
               const std::vector<ores::utility::domain::precondition>& claims);

    /**
     * @brief Reads latest rounding types, possibly filtered by primary key.
     */
    /**@{*/
    std::vector<domain::rounding_type> read_latest(context ctx);
    std::vector<domain::rounding_type> read_latest(context ctx, const std::string& code);
    std::vector<domain::rounding_type> read_latest(context ctx,
    ...
    /**
     * @brief Reads all rounding types, possibly filtered by primary key.
     */
    std::vector<domain::rounding_type> read_all(context ctx, const std::string& code);

    /**
     * @brief Reads a single rounding type as it stood at a specific
    ...
    /**
     * @brief Deletes a rounding type by closing its temporal validity.
     */
    ...
    /**
     * @brief Removes a rounding type, refusing a row that moved on.
     *
...
private:
     ...
     */
    ores::utility::domain::precondition replace_claim(context ctx, const domain::rounding_type& v);
     ...
     */
    domain::rounding_type apply_claim(context ctx,
                                      const domain::rounding_type& v,
                                      const ores::utility::domain::precondition& claim);
};
