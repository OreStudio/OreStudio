...
private:
     ...
     * ReportDefinitionDetailDialog needs regardless of which
     * window opened it (add/edit/history-version/revert) -- kept in one
     * place so those four call sites can't drift from each other.
     */
    void wireDetailDialogCommon(ReportDefinitionDetailDialog* detailDialog);
    void showHistoryWindow(const reporting::domain::report_definition& definition);

    /**
     * @brief Fetches the full typed report definition history (the
     * existing per-entity reporting::messaging::get_report_definition_history_request/
     * reporting::messaging::get_report_definition_history_response, unrelated to the generic
     * history.v1.get subject) and hands it to @p callback on the UI
     * thread. Used to resolve HistoryDialog's generic (entity_id,
     * version) signals back to a typed report definition, since the
     * generic dialog holds no typed domain data.
     */
    void fetchReportDefinitionHistory(
        const QString& entityId,
        std::function<void(
            std::expected<std::vector<reporting::domain::report_definition>, QString>)> callback);
