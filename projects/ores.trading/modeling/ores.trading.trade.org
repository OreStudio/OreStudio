...
#+description: Codegen entity model for the trading =trade= table. Drives the C++ domain class, mapper, repository, service, generator, TypeScript UI metadata, and SQL schema generation from a single literate source.
...
The column is text in the database and an enumeration in the domain
struct, so the generated sample states the enumerator: the generator
builds the struct, not the row.
...
The generated sample leaves it nil. A trade's status is the target of a
guarded transition, so the insert trigger resolves it from the activity;
a random uuid names no seeded state and is rejected before the trigger
ever reaches the transition that would have set it.
...
** book_id
:PROPERTIES:
:table:         ores_refdata_books_tbl
:parent_entity: book
:skip_check:    true
:nullable:      false
:error_message: Invalid book_id: %. Book must exist for tenant.
:END:

The book a trade is booked into. =skip_check= suppresses the inline
check, because the SQL section's "Party id from book id" already
validates the book on its way to deriving the party from it. The entry
is still needed: it is what tells the eventing test to seed a book
...
:skip_check:    true
:nullable:      false
:error_message: Invalid portfolio_id: %. Portfolio must exist for tenant.
:END:

Declared for the same reason, and resolved in the same place: the
derivation above sets it from the book's parent portfolio, so whatever
the caller sent is replaced before any check could run.
...
The state the trade occupies in its lifecycle machine. =skip_check=
because the status is not the caller's to supply: the transition
resolution below assigns it from the transition's target state, which
is a seeded state by construction. Checking the caller's value first
would reject every write before the trigger could correct it.
...
and which portfolio contains it. Neither is therefore the caller's to
supply: the insert trigger derives =party_id= and =portfolio_id= from
the book and ignores whatever was sent for them.

Both are derived the same way because both are determined the same way.
A value the book fully decides cannot disagree with the book without the
caller being wrong, so asking a caller to send it can only introduce an
error that the derivation removes.

** Status transition
:PROPERTIES:
:amend_activity_column: activity_type_code
:amend_activity_code:   amendment
:END:

=amend_activity_code= names the activity a rewrite of an existing trade
carries. A trade cannot be rewritten under =new_booking=: that activity
names a transition which starts the machine, and the row already has a
state. The generated round-trip test uses this to amend the row it has
just booked.

A trade's status is not a value the caller supplies: it is the target of a
guarded transition. The activity names the event, reference data maps the
event to a transition in the =trade_status= machine, and the transition
states which status it may be taken from. An activity that maps to no
transition versions the trade and leaves it where it was, which is why a
fixing on a live trade stays live.

The guard lives in the insert trigger rather than in a service because
every path that writes a version passes through it, and because the
version select above locks the superseding row: reading the prior status
under that lock is what stops two concurrent amendments from both
believing their transition is legal.

#+begin_src sql :name status_transition_declarations :implements B9428A09-3610-4CAD-A200-E6CDE53B7C8D
    v_transition record;
    v_prior_status_id uuid;
#+end_src

Both branches resolve the transition through
=ores_trading_resolve_trade_transition_fn=, which does the lookup and
refuses an activity naming a transition that no longer exists. What
differs between them is the guard, and that is what stays here.

The booking case. The machine has not started, so the transition must be
one that starts it: a transition stating a =from_state_id= is an event
that presumes a history this trade does not have.

#+begin_src sql :name status_transition_on_insert :implements 8697C751-ECD5-4501-AF06-77AD9A8985AB
...
The amendment case. The prior row is locked by the version select above,
so the status read here cannot race another writer.

#+begin_src sql :name status_transition_on_amend :implements 59F4313F-B119-44D8-8E4E-05AE8B18DE95
...
A trade's node-scoped listing is not a column filter: the node may be a
book, a portfolio or a business unit, so the database resolves it to a
book-id set first and the read runs against that set. An empty node
covers the whole tenant, which is why the unfiltered listing needs no
separate call.

#+begin_src cpp :name repository_declarations :implements DCA78C69-E508-48D9-9972-A9B8094D91FB
...
#+begin_src cpp :name repository_methods :implements F2EB1914-5E94-42CA-9C77-46BDB364BF9E
namespace {
...
type, then read each product's table in one batch.
