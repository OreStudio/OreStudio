...
ChangeReasonMdiWindow::ChangeReasonMdiWindow(ClientManager* clientManager,
...
void ChangeReasonMdiWindow::setupToolbar() {
    ...
    connect(addAction_, &QAction::triggered, this, &ChangeReasonMdiWindow::addNew);
    ...
    connect(editAction_, &QAction::triggered, this, &ChangeReasonMdiWindow::editSelected);
    ...
    connect(deleteAction_, &QAction::triggered, this, &ChangeReasonMdiWindow::deleteSelected);
    ...
    connect(historyAction_, &QAction::triggered, this, &ChangeReasonMdiWindow::viewHistorySelected);
}
...
void ChangeReasonMdiWindow::setupTable() {
    ...
    initializeTableSettings(tableView_, model_, "ChangeReasonListWindow", {}, {900, 400}, 1);
}

void ChangeReasonMdiWindow::setupConnections() {
    connect(
        model_, &ClientChangeReasonModel::dataLoaded, this, &ChangeReasonMdiWindow::onDataLoaded);
    connect(model_, &ClientChangeReasonModel::loadError, this, &ChangeReasonMdiWindow::onLoadError);

    connect(tableView_->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &ChangeReasonMdiWindow::onSelectionChanged);
    connect(tableView_, &QTableView::doubleClicked, this, &ChangeReasonMdiWindow::onDoubleClicked);
...
void ChangeReasonMdiWindow::doReload() {
    BOOST_LOG_SEV(lg(), debug) << "Reloading change reasons";
    clearStaleIndicator();
    emit statusChanged(tr("Loading change reasons..."));
...
void ChangeReasonMdiWindow::onDataLoaded() {
    ...
    const auto total = model_->total_available_count();
    emit statusChanged(tr("Loaded %1 of %2 change reasons").arg(loaded).arg(total));
...
void ChangeReasonMdiWindow::onLoadError(const QString& error_message, const QString& details) {
...
void ChangeReasonMdiWindow::deleteSelected() {
    ...
    if (!clientManager_->isConnected()) {
        MessageBoxHelper::warning(
            this, "Disconnected", "Cannot delete change reason while disconnected.");
        return;
    }
    ...
    BOOST_LOG_SEV(lg(), debug) << "Delete requested for " << codes.size() << " change reasons";
    ...
    if (codes.size() == 1) {
        ...
        confirmMessage =
            QString("Are you sure you want to delete %1 change reasons?").arg(codes.size());
    }

    auto reply = MessageBoxHelper::question(
        this, "Delete Change Reason", confirmMessage, QMessageBox::Yes | QMessageBox::No);
