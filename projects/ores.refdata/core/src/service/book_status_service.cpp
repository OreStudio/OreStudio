 ...
 */
std::vector<domain::book_status> read_one(repository::book_status_repository& repo,
                                          const ores::database::context& ctx,
                                          const messaging::book_status_key& key) {
 ...
 */
messaging::book_status_key key_from(const domain::book_status& v) {
    messaging::book_status_key key;
    key.code = v.code;
    return key;
}
 ...
 */
domain::book_status to_domain(const messaging::book_status_write& write) {
    domain::book_status v;
    v.code = write.code;
    v.name = write.name;
...
book_status_service::list_book_statuses(const messaging::list_book_statuses_request& request) {
    messaging::list_book_statuses_response response;
    ...
    if (request.as_of) {
        response.statuses = repo_.read_at_timepoint(ctx_, *request.as_of);
        response.total = response.statuses.size();
        return response;
    }
...
book_status_service::get_book_status(const messaging::get_book_status_request& request) {
    messaging::get_book_status_response response;
    ...
    response.book_status = std::move(found.front());
    return response;
}

messaging::get_many_book_statuses_response book_status_service::get_many_book_statuses(
    const messaging::get_many_book_statuses_request& request) {
    messaging::get_many_book_statuses_response response;
    // One entry per requested key, in the order asked for, so the reply is
    // positional and a caller reads absence from an empty entry rather than
    // from a missing one.
    response.entries.reserve(request.keys.size());
    for (const auto& k : request.keys) {
        messaging::book_status_lookup entry;
        entry.key = k;
        auto found = read_one(repo_, ctx_, k);
        if (!found.empty())
            entry.book_status = std::move(found.front());
...
book_status_service::put_book_status(const messaging::put_book_status_request& request) {
    messaging::put_book_status_response response;
    domain::book_status value;
    ...
    if (!written.empty())
        response.book_status = std::move(written.front());
    return response;
}

messaging::put_many_book_statuses_response book_status_service::put_many_book_statuses(
    const messaging::put_many_book_statuses_request& request) {
    messaging::put_many_book_statuses_response response;
...
book_status_service::delete_book_status(const messaging::delete_book_status_request& request) {
    messaging::delete_book_status_response response;
    ...
    switch (repo_.remove(ctx_, request.removal.key.code, expected)) {
        case repository::book_status_repository::remove_status::removed:
            break;
        case repository::book_status_repository::remove_status::missing:
            response.result.outcome = outcome::missing;
            response.result.code = "not_found";
            break;
        case repository::book_status_repository::remove_status::conflicting:
            response.result.outcome = outcome::conflict;
            response.result.code = "version_conflict";
            break;
        case repository::book_status_repository::remove_status::unsupported:
            response.result.outcome = outcome::invalid;
            response.result.code = "precondition_not_supported";
...
messaging::delete_many_book_statuses_response book_status_service::delete_many_book_statuses(
    const messaging::delete_many_book_statuses_request& request) {
    messaging::delete_many_book_statuses_response response;
    using ores::utility::domain::outcome;
    using ores::utility::domain::precondition_kind;
...
messaging::list_book_status_versions_response book_status_service::list_book_status_versions(
    const messaging::list_book_status_versions_request& request) {
    messaging::list_book_status_versions_response response;
    if (!request.order.field.empty() || request.order.descending) {
        response.result.outcome = ores::utility::domain::outcome::invalid;
        response.result.code = "order_not_supported";
        response.result.message =
...
messaging::get_book_status_version_response book_status_service::get_book_status_version(
    const messaging::get_book_status_version_request& request) {
    messaging::get_book_status_version_response response;
    auto found = repo_.read_at_version(ctx_, request.key.book_status.code, request.key.version);
    if (!found) {
        response.result.outcome = ores::utility::domain::outcome::missing;
        response.result.code = "not_found";
        return response;
...
book_status_service::prepare_change(const messaging::book_status_change& change,
...
book_status_service::get_statuses(const std::vector<std::string>& codes) {
