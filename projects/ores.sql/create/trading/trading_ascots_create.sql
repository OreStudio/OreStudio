...
create table if not exists "ores_trading_ascots_tbl" (
...
create unique index if not exists ascots_version_uniq_idx
on "ores_trading_ascots_tbl" (tenant_id, instrument_id, version)
where valid_to = ores_utility_infinity_timestamp_fn();

create unique index if not exists ascots_id_uniq_idx
on "ores_trading_ascots_tbl" (tenant_id, instrument_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create index if not exists ascots_tenant_idx
on "ores_trading_ascots_tbl" (tenant_id)
where valid_to = ores_utility_infinity_timestamp_fn();

create or replace function ores_trading_ascots_insert_fn()
returns trigger as $$
declare
...
begin
    ...
    if found then
        ...
        update "ores_trading_ascots_tbl"
...
create or replace trigger ores_trading_ascots_insert_trg
before insert on "ores_trading_ascots_tbl"
for each row execute function ores_trading_ascots_insert_fn();

create or replace rule ores_trading_ascots_delete_rule as
on delete to "ores_trading_ascots_tbl" do instead (
    update "ores_trading_ascots_tbl"
    set valid_to = clock_timestamp()
