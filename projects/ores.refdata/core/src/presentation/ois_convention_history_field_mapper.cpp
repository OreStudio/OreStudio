...
render_ois_convention_fields(const domain::ois_convention& v) {
    using ores::diff::domain::field_value;
    std::vector<field_value> fields;

    fields.push_back({.name = "ID", .value = v.id});
    fields.push_back({.name = "Spot Lag", .value = std::to_string(v.spot_lag)});
    ...
    fields.push_back({.name = "Payment Lag",
                      .value = v.payment_lag ? std::to_string(*v.payment_lag) : std::string{}});
    fields.push_back(
        {.name = "End Of Month",
         .value = v.end_of_month ? (*v.end_of_month ? "true" : "false") : std::string{}});
    fields.push_back(
        {.name = "Fixed Frequency", .value = v.fixed_frequency.value_or(std::string{})});
    fields.push_back(
        {.name = "Fixed Convention", .value = v.fixed_convention.value_or(std::string{})});
    fields.push_back({.name = "Fixed Payment Convention",
                      .value = v.fixed_payment_convention.value_or(std::string{})});
