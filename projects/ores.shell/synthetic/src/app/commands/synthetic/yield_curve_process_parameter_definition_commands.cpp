...
void yield_curve_process_parameter_definition_commands::register_commands(cli::Menu& root_menu,
                                                                          nats_client& session) {
    auto menu = std::make_unique<cli::Menu>("yield_curve_process_parameter_definitions");
    ...
    menu->Insert(
        ...
        "set <id> <process_type_code> <parameter_name> <display_name> <symbol> <short_label> "
        "<description> <data_type> <default_value> <min_value> <max_value> <display_order> "
    ...
    menu->Insert(
        ...
        "put-many --count <n> <id> <process_type_code> <parameter_name> <display_name> <symbol> "
        "<short_label> <description> <data_type> <default_value> <min_value> <max_value> "
    ...
    menu->Insert(
        ...
        "delete <parameter_name> <reason> <commentary> [--version <n>]");
    ...
    menu->Insert(
        ...
        "delete-many <parameter_name> <reason> <commentary>");
    ...
    menu->Insert(
        ...
        "versions <parameter_name> [--offset <n>] [--limit <n>] [--order <field>] [--desc]");
    ...
    menu->Insert(
        ...
        "version <parameter_name> --version <n>");

    root_menu.Insert(std::move(menu));
}

void yield_curve_process_parameter_definition_commands::process_list(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating list request.";

    using request_type = messaging::list_yield_curve_process_parameter_definitions_request;
    ...
    auto result =
        do_auth_request<messaging::list_yield_curve_process_parameter_definitions_response>(
...
void yield_curve_process_parameter_definition_commands::process_get(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get request.";

    using request_type = messaging::get_yield_curve_process_parameter_definition_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.parameter_name, parsed->positionals[next++], "parameter_name");
    } catch (const std::exception& e) {
        fail(out) << e.what() << std::endl;
        return;
    }

    auto result = do_auth_request<messaging::get_yield_curve_process_parameter_definition_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_get_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating get-many request.";

    using request_type = messaging::get_many_yield_curve_process_parameter_definitions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < parsed->positionals.size(); i += 1) {
            messaging::yield_curve_process_parameter_definition_key key;
            read_token(key.parameter_name, parsed->positionals[i + 0], "parameter_name");
    ...
    auto result =
        do_auth_request<messaging::get_many_yield_curve_process_parameter_definitions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_add(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating add request.";

    using request_type = messaging::put_yield_curve_process_parameter_definition_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.change.write.symbol, parsed->positionals[next++], "symbol");
        read_token(req.change.write.short_label, parsed->positionals[next++], "short_label");
        ...
        read_token(req.change.write.min_value, parsed->positionals[next++], "min_value");
        read_token(req.change.write.max_value, parsed->positionals[next++], "max_value");
    ...
    auto result = do_auth_request<messaging::put_yield_curve_process_parameter_definition_response>(
        out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_set(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
...
void yield_curve_process_parameter_definition_commands::process_put_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating put-many request.";

    using request_type = messaging::put_many_yield_curve_process_parameter_definitions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::uint32_t i = 0; i < change_count; ++i) {
            messaging::yield_curve_process_parameter_definition_change change;
            read_token(change.write.id, parsed->positionals[next++], "id");
            read_token(
                change.write.process_type_code, parsed->positionals[next++], "process_type_code");
            ...
            read_token(change.write.symbol, parsed->positionals[next++], "symbol");
            read_token(change.write.short_label, parsed->positionals[next++], "short_label");
            ...
            read_token(change.write.min_value, parsed->positionals[next++], "min_value");
            read_token(change.write.max_value, parsed->positionals[next++], "max_value");
    ...
    auto result =
        do_auth_request<messaging::put_many_yield_curve_process_parameter_definitions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_delete(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete request.";

    using request_type = messaging::delete_yield_curve_process_parameter_definition_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.removal.key.parameter_name, parsed->positionals[next++], "parameter_name");
        req.intent.reason_code = parsed->positionals[next++];
        req.intent.commentary = parsed->positionals[next++];
        if (const auto& raw = parsed->flag("version"); !raw.empty()) {
    ...
    auto result =
        do_auth_request<messaging::delete_yield_curve_process_parameter_definition_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_delete_many(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating delete-many request.";

    using request_type = messaging::delete_many_yield_curve_process_parameter_definitions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        for (std::size_t i = 0; i < key_groups; ++i) {
            messaging::yield_curve_process_parameter_definition_key key;
            read_token(key.parameter_name, parsed->positionals[i * 1 + 0], "parameter_name");
            req.removals.push_back(
                messaging::yield_curve_process_parameter_definition_removal{.key = std::move(key)});
        }
    ...
    auto result =
        do_auth_request<messaging::delete_many_yield_curve_process_parameter_definitions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_versions(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating versions request.";

    using request_type = messaging::list_yield_curve_process_parameter_definition_versions_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    auto result =
        do_auth_request<messaging::list_yield_curve_process_parameter_definition_versions_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
...
void yield_curve_process_parameter_definition_commands::process_version(
    std::ostream& out, nats_client& session, const std::vector<std::string>& args) {
    BOOST_LOG_SEV(lg(), debug) << "Initiating version request.";

    using request_type = messaging::get_yield_curve_process_parameter_definition_version_request;
    if constexpr (request_type::requires_session) {
        if (!session.is_logged_in()) {
    ...
    try {
        ...
        read_token(req.key.yield_curve_process_parameter_definition.parameter_name,
    ...
    auto result =
        do_auth_request<messaging::get_yield_curve_process_parameter_definition_version_response>(
            out, session, std::string(req.nats_subject), req);
    if (!result)
        return;
