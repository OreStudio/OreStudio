...
int ClientBadgeSeverityModel::rowCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return static_cast<int>(severities_.size());
}

int ClientBadgeSeverityModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid())
        return 0;
    return ColumnCount;
}
...
QVariant ClientBadgeSeverityModel::data(
    ...
    if (row >= severities_.size())
        return {};

    const auto& severity = severities_[row];
    ...
    if (role == Qt::ForegroundRole) {
        return recency_foreground_color(severity.code);
    }
...
void ClientBadgeSeverityModel::refresh() {
    ...
    if (!clientManager_ || !clientManager_->isConnected()) {
        BOOST_LOG_SEV(lg(), warn) << "Cannot refresh badge severity model: disconnected.";
...
void ClientBadgeSeverityModel::onSeveritysLoaded() {
    ...
    const int new_count = static_cast<int>(result.severities.size());

    if (new_count > 0) {
        beginResetModel();
        severities_ = std::move(result.severities);
        endResetModel();

        const bool has_recent = recencyTracker_.update(severities_);
        if (has_recent && !pulseManager_->is_pulsing()) {
            pulseManager_->start_pulsing();
            BOOST_LOG_SEV(lg(), debug) << "Found " << recencyTracker_.recent_count()
                                       << " badge severities newer than last reload";
        }
    }

    BOOST_LOG_SEV(lg(), info) << "Loaded " << new_count << " badge severities."
                              << " Total available: " << total_available_count_;

    emit dataLoaded();
}

void ClientBadgeSeverityModel::set_page_size(std::uint32_t size) {
    if (size == 0 || size > 1000) {
...
void ClientBadgeSeverityModel::onPulsingComplete() {
