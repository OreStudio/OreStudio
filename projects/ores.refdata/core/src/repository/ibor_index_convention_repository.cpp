...
ibor_index_convention_repository::replace_claim(context ctx,
...
ibor_index_convention_repository::apply_claim(context ctx,
                                              const domain::ibor_index_convention& v,
...
void ibor_index_convention_repository::write(context ctx,
                                             const domain::ibor_index_convention& v,
                                             const ores::utility::domain::precondition& claim) {
    BOOST_LOG_SEV(lg(), debug) << "Writing IBOR index convention. " << "id: " << v.id;
    const auto t = apply_claim(ctx, v, claim);
    execute_write_query(ctx,
                        ibor_index_convention_mapper::map(t),
                        lg(),
...
void ibor_index_convention_repository::write(
    ...
    execute_write_query(ctx,
                        ibor_index_convention_mapper::map(batch),
                        lg(),
...
ibor_index_convention_repository::read_latest(context ctx) {
    ...
    if (!chain.empty()) {
        ...
        return execute_read_query<ibor_index_convention_entity, domain::ibor_index_convention>(
            ...
            lg(),
            "Reading latest IBOR index conventions (workspace resolution chain).");
    }
...
std::optional<domain::ibor_index_convention> ibor_index_convention_repository::read_at_version(
    context ctx, const std::string& id, std::uint32_t version) {
    BOOST_LOG_SEV(lg(), debug) << "Reading IBOR index convention at version. " << "id: " << id
                               << " version: " << version;
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    const auto query = sqlgen::read<std::vector<ibor_index_convention_entity>> |
    ...
    const auto entities =
        execute_read_query<ibor_index_convention_entity, domain::ibor_index_convention>(
            ctx,
            query,
            [](const auto& entities) { return ibor_index_convention_mapper::map(entities); },
            lg(),
            "Reading IBOR index convention at version.");
...
std::vector<domain::ibor_index_convention> ibor_index_convention_repository::read_latest(
    context ctx, std::uint32_t offset, std::uint32_t limit) {
    BOOST_LOG_SEV(lg(), debug) << "Reading latest IBOR index conventions with offset: " << offset
                               << " and limit: " << limit;
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    const auto wid = ctx.workspace_id();
    ...
    return execute_read_query<ibor_index_convention_entity, domain::ibor_index_convention>(
        ...
        [](const auto& entities) { return ibor_index_convention_mapper::map(entities); },
        lg(),
        "Reading latest IBOR index conventions with pagination.");
}

std::uint32_t ibor_index_convention_repository::get_total_ibor_index_convention_count(context ctx) {
    BOOST_LOG_SEV(lg(), debug) << "Retrieving total active IBOR index convention count";
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    ...
    const auto wid = ctx.workspace_id();
    const auto query =
        sqlgen::select_from<ibor_index_convention_entity>(sqlgen::count().as<"count">()) |
        where("tenant_id"_c == tid && "workspace_id"_c == wid && "valid_to"_c == max.value()) |
    ...
    BOOST_LOG_SEV(lg(), debug) << "Total active IBOR index convention count: " << count;
    return count;
}

std::vector<domain::ibor_index_convention>
ibor_index_convention_repository::read_latest(context ctx, const std::vector<std::string>& ids) {
    if (ids.empty())
        return {};
    static const auto max(make_timestamp(MAX_TIMESTAMP, lg()));
    const auto tid = ctx.tenant_id().to_string();
    ...
    const auto query = sqlgen::read<std::vector<ibor_index_convention_entity>> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    auto result = execute_read_query<ibor_index_convention_entity, domain::ibor_index_convention>(
        ctx,
        query,
        [](const auto& entities) { return ibor_index_convention_mapper::map(entities); },
        lg(),
        "Reading latest IBOR index conventions by ids.");
    return result;
}
...
void ibor_index_convention_repository::remove(context ctx, const std::vector<std::string>& ids) {
    ...
    const auto query = sqlgen::delete_from<ibor_index_convention_entity> |
                       where("tenant_id"_c == tid && "workspace_id"_c == wid && "id"_c.in(ids) &&
                             "valid_to"_c == max.value());
    execute_delete_query(ctx, query, lg(), "Batch removing IBOR index conventions.");
}
